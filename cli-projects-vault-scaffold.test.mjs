 ...
 * Historically it folded in the vault remote scaffold
 ...
 * same `vault` / `vault_skipped` / `vault_error` summary keys
...
describe("projects provision — never touches git (first-deploy-agent-dx)", () => {
  ...
  it("provisions and stops: no scaffoldRemote call, no vault keys, remotes byte-identical", async () => {
    ...
    assert.equal(calls.find((c) => c.method === "repos.scaffoldRemote"), undefined, "provision never scaffolds a remote");
    assert.equal(git(repoDir, ["remote", "-v"]), before, "git remotes are untouched");
    for (const key of ["vault", "vault_skipped", "vault_error"]) {
