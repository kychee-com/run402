 ...
 *
 * THE DEFECT (vault dogfood #1, 2026-08-23). The helper took its repository
...
import { R402_PROTECTED_REF_NAMESPACE_REASON, chooseVaultHeadTargetForPush, partitionProtectedRefPushes, shouldRunAutoGc, buildAutoGcCompactionTarget, getCachedSdk } from "./cli/lib/remote-helper-session.mjs";
import { walletFile } from "./cli/lib/config.mjs";
...
// design D6's discrimination — see `vaultRemoteAddressForm`). Every test
...
function runHelper({ cwd, env = {}, stdin }) {
  return spawnSync(process.execPath, [HELPER, "run402", ADDRESS], {
    ...
    env: {
      ...
      // cli-vault-daemon.test.mjs (vault-persistent-helper 3.1).
      RUN402_DAEMON: "0",
      ...env,
    },
  });
...
// SDK level against VaultMemoryTransport
// (sdk/src/node/vault-open-or-create.test.ts) — reimplementing that
// protocol as an HTTP fixture here would duplicate it, not add coverage.
// Recording that `POST /vaults/v1` was reached is the wiring proof
...
/** A gateway stub that answers exactly the two vault creation routes this file needs. */
  ...
  it("push against an unallocated vault reaches the allocate route — org_id came from the parsed address", async () => {
    await withUnallocatedVaultGateway(async (apiBase, calls) => {
      ...
      try {
        ...
        const allocateCalls = calls.filter((c) => c.method === "POST" && c.url === "/vaults/v1");
...
//      (`sdk/src/node/vault-publication.test.ts`'s `planPush` suite) —
...
// `vaultRemoteAddressForm`), routing through `resolveOrCreateAddress`
...
//      (sdk/src/node/vault-address.test.ts); reimplementing it as an HTTP
...
function withResolvableRepoGateway(handler) {
  ...
  const server = createServer((req, res) => {
    ...
    if (
      (req.method === "GET" && req.url === `/vaults/v1?repo=${encodeURIComponent(SLUG_ADDRESS)}`) ||
      (req.method === "GET" && req.url === `/vaults/v1?project_id=${encodeURIComponent("prj_test")}`)
    ) {
      res.writeHead(200, { "content-type": "application/json" });
    ...
    if (req.method === "GET" && req.url === "/vaults/v1/src_00000000000000000000000000000001") {
      res.writeHead(200, { "content-type": "application/json" });
      res.end(JSON.stringify(vaultRecordFixture()));
      return;
    }
...
describe("git-remote-run402 — D6 slug-form id-pinning (task 4.5)", () => {
  ...
  // a local vault IDENTITY (this fresh RUN402_CONFIG_DIR never ran
  // `vault init`), which is a SEPARATE, later concern from address
  ...
  it("a resolvable slug-form remote pins repo_id on first push, and a SECOND push never re-resolves the address", async () => {
    await withResolvableRepoGateway(async (apiBase, calls) => {
      ...
      try {
        ...
        const repoCallsAfterFirst = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?repo=")).length;
        ...
        const repoCallsAfterSecond = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?repo=")).length;
  ...
  it("list against a slug-form remote resolves it cleanly — no repository, so nothing to pin, and no crash beyond the expected typed refusal", async () => {
    await withResolvableRepoGateway(async (apiBase, calls) => {
      ...
      const repoCalls = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?repo=")).length;
      assert.equal(repoCalls, 1, "list resolved the address over the network exactly once");
    });
  });
});

describe("git-remote-run402 — D4 id-form id-pinning (vault-client-round-trips)", () => {
  ...
  it("a resolvable id-form remote pins repo_id on first push, and a SECOND push never re-resolves the address", async () => {
    await withResolvableRepoGateway(async (apiBase, calls) => {
      ...
      try {
        ...
        assert.match(first.stdout, /KEYSTORE_MISSING/, `expected the push to reach the (later, identity-dependent) KEYSTORE_MISSING wall, not fail resolution itself: ${first.stdout}\n---\n${first.stderr}`);
        assert.equal(readPinnedRepoId(target), "src_00000000000000000000000000000001", "the pin survives even though the push itself did not complete");
        const projectCallsAfterFirst = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?project_id=")).length;
        ...
        assert.match(second.stdout, /KEYSTORE_MISSING/, `${second.stdout}\n---\n${second.stderr}`);
        const projectCallsAfterSecond = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?project_id=")).length;
  ...
  it("list against an id-form remote resolves it cleanly — no repository, so nothing to pin, and no crash beyond the expected typed refusal", async () => {
    await withResolvableRepoGateway(async (apiBase, calls) => {
      ...
      assert.match(r.stderr, /KEYSTORE_MISSING/, `${r.stdout}\n---\n${r.stderr}`);
      const projectCalls = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?project_id=")).length;
  ...
  it("a push against a slug-form name that does NOT resolve yet reaches push-to-create with {org_slug, repo_name} — never {project_id}", async () => {
    await withUnallocatedVaultGateway(async (apiBase, calls) => {
      ...
      try {
        ...
        const repoReads = calls.filter((c) => c.method === "GET" && c.url.startsWith("/vaults/v1?repo="));
...
// wallet a deliberately MALFORMED wallet.json (present, but missing a
// valid `address`) — `core/src/wallet.ts#readWallet`'s own throw fires
...
/** A profile directory holding ONLY a deliberately malformed wallet.json
...
function writeMalformedWalletProfile(configDir, name) {
  ...
  mkdirSync(dir, { recursive: true });
  writeFileSync(join(dir, "wallet.json"), JSON.stringify({ notAnAddress: true }));
}
...
describe("git-remote-run402 — wallet selection (kychee-com/run402#558)", () => {
  it("a .run402.json binding is honored — the resolved wallet's own (malformed) key file surfaces, not the default's silence", async () => {
    ...
    try {
      ...
      assert.match(r.stderr, /wallet\.json/, r.stderr);
  ...
  it("the wallet-missing remedy names the resolved wallet/profile and how selection works, instead of blindly suggesting 'run402 init'", async () => {
    ...
    try {
      ...
      // the DEFAULT wallet's key file, not this one. It must not be offered
  ...
  it("a genuinely unselected default wallet keeps the original 'run402 init' remedy — no binding/env means it IS the right fix", async () => {
    ...
    try {
      const configDir = mkdtempSync(join(tmpdir(), "run402-gvh-cfg-"));
      // Malformed DEFAULT wallet, nothing selecting any other wallet.
...
//
// `chooseVaultHeadTargetForPush` is the pure decision this file's own
...
// `sdk/src/node/vault-publication.test.ts`, "push can set head_target
...
describe("chooseVaultHeadTargetForPush — first-push HEAD repair (kychee-com/run402#568)", () => {
  ...
  it("first push of master (a fresh vault, HEAD defaulting to the never-pushed 'main') sets HEAD to master — and the resulting ref map is clone-safe", () => {
    ...
    const result = chooseVaultHeadTargetForPush({
  ...
  it("a dangling HEAD with no branch in this batch (e.g. a tag-only or deletion-only push) is left alone — nothing here can repair it", () => {
    const tagOnly = chooseVaultHeadTargetForPush({
      baseHeadTarget: MAIN_SYMREF,
      baseRefs: {},
    ...
    const deletionOnly = chooseVaultHeadTargetForPush({
      baseHeadTarget: MAIN_SYMREF,
      baseRefs: { "refs/heads/main": oid(0) },
...
// `chooseVaultHeadTargetForPush` precedent for testing helper-local pure
// logic directly.
...
// ─── vault-checkpoint-cadence: the pure auto-gc threshold decision ────────
...
describe("buildAutoGcCompactionTarget — the repo_dir regression (2026-08-31)", () => {
  it("threads repo_dir into the compaction target — the SDK never defaults it", () => {
    // `open()` (sdk/src/namespaces/repos.ts) only sets repo_dir on the
    ...
    // all, so every real post-push trigger failed VAULT_REPO_DIR_REQUIRED
...
// ─── vault-first-op-premium: the per-session SDK construction cache ──────
//
// Attribution (2026-08-31, recorded in openspec/changes/vault-first-op-
...
describe("getCachedSdk — per-session SDK construction cache (vault-first-op-premium task 2.2)", () => {
