/**
 * `run402 deploy` against a `vault_policy: required` project — the lane,
 ...
 * published client was refused 409 `VAULT_CLIENT_UPGRADE_REQUIRED`. The
 ...
 * vault-capable deploy lane. So the client rewrote the envelope, behind a
 * `VAULT_DEPLOY_LANE = "unsupported"` flag, and a CI gate held the flag and
 * the lane in agreement (vault dogfood #1, finding C).
 *
 * Change `vault-deploy-lane` shipped the lane, so both are retired (design
...
describe("the interim vault advisory is retired, not merely unused", () => {
  it("the flag is gone — a dead flag is a second truth waiting to drift", () => {
    assert.equal(
      /VAULT_DEPLOY_LANE/.test(stripComments(deployV2)),
      false,
      "VAULT_DEPLOY_LANE must be deleted, not flipped to 'supported'. (The comment recording its retirement is fine; a live symbol is not.)",
    );
  });

  it("the client no longer rewrites VAULT_CLIENT_UPGRADE_REQUIRED", () => {
    assert.equal(
      /function\s+enhanceVaultDeployError/.test(deployV2),
      false,
      "the advisory rewrite must be deleted; the gateway's envelope is relayed unmodified.",
    );
    assert.equal(
      stripComments(deployV2).includes("VAULT_CLIENT_UPGRADE_REQUIRED"),
      false,
...
describe("the deploy lane speaks vault", () => {
  /**
   * The dogfood's own measurement was `grep -c vault lib/deploy-v2.mjs` → 0.
   * The check that matters is not the word count but whether the apply path
   * routes through the lane at all: `applyWithVault` is what declares the
   * capture at plan time and presents the activation token at commit.
   */
  it("apply routes through applyWithVault rather than a bare apply()", () => {
    const code = stripComments(deployV2);
    assert.match(code, /applyWithVault\s*\(/, "applyCmd must deploy through the vault-aware entry point");
  ...
  it("the CLI stays a thin shim: the lane itself is SDK code (add-vault 5.0)", () => {
    const code = stripComments(deployV2);
    ...
    for (const protocolWork of ["snapshot_oid_hmac", "activation_token_id", "captureSnapshot", "exchangeActivationToken", "runVaultDeploy"]) {
      assert.equal(
  ...
  it("prints the vault_commit line to stderr, so `| jq` stays clean", () => {
...
describe("deploy dirty-tree refusal (Observability + dirty-tree-refusal)", () => {
  /**
   * `--allow-dirty` must reach `applyWithVault` (which threads it into
   * `repos.deploy`'s `snapshot.allowDirty`, same option name captureSnapshot
   ...
   */
  it("--allow-dirty threads to applyWithVault's allowDirty option", () => {
