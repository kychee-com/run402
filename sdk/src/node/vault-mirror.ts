 ...
 * `vault-publication.ts`, which keeps those helpers module-private) so
 ...
 * mirror BACKEND's key space (`vault-mirror-backend.ts`
 * `openVaultMirrorBackend`, whose `root`/`prefix` already bake in
 * `source/<repo_id>`), are both REPO-RELATIVE (`head/<gen>`,
 * `wal/<id>.pack.enc`, `key-envelopes/<epoch>/<fp>.env`, …) — the same shape
 * `vault-recover.ts` and `vault-publication.ts`'s `vaultPaths`
 * already use. The mapping is therefore, in ONE place:
 *
 *   bucket key  =  `source/<repo_id>/` + mirror key (repo-relative)
 *
 * `listVaultObjectsAll` is the ONLY function that ever sees a full,
 ...
 * `vault-recover.ts`, which never talks to the live gateway at all) sees
 * the entry. Every `VaultObjectEntry` handed to `generationRouteForKey`,
 * `objectReadRequestForEntry`, `planMirrorWrite`, `reconcileOne`, and the
 * `VaultMirrorBackend` itself is repo-relative from that point on. An
...
import { formatMirrorDestination, readMirrorConfig, recordMirrorSuccess, type VaultMirrorConfig } from "./vault-mirror-config.js";
import { openVaultMirrorBackend, resolveMirrorCredentials, type VaultMirrorBackend } from "./vault-mirror-backend.js";
 ...
 * OTHER function in this module, `vault-recover.ts`, and the mirror
 * backend interface all consume; nothing here ever sees the raw wire key
 * except `listVaultObjectsAll` itself.
 *
 ...
 */
export interface VaultObjectEntry {
  key: string;
  object_kind: string;
  sha256: string;
  size_bytes: string;
...
/** The WIRE shape of one listing entry, exactly as the gateway's `GET …/objects` returns it — `key` is FULLY QUALIFIED under `source/<repo_id>/`. Never leaves `listVaultObjectsAll`. */
...
/** protocol §3 (`storage-keys.ts` `VAULT_SOURCE_PREFIX`) — duplicated here because the SDK carries no dependency on the gateway package. */
const VAULT_SOURCE_PREFIX = "source";
...
function stripSourcePrefix(repoId: string, key: string): string {
  const expected = `${VAULT_SOURCE_PREFIX}/${repoId}/`;
  if (key.startsWith(expected)) return key.slice(expected.length);
  const foreign = /^source\/([^/]+)\//.exec(key);
  if (foreign) {
    fail(
      "VAULT_MIRROR_FOREIGN_REPO_KEY",
      `the objects listing for ${repoId} returned a key addressed to a different vault (${foreign[1]!}): ${key}`,
      "listing vault objects",
      { key, repo_id: repoId, foreign_repo_id: foreign[1] },
    );
  }
  fail("VAULT_MIRROR_LISTING_INCONSISTENT", `listed key is not in the expected source/<repo_id>/… layout: ${key}`, "listing vault objects", { key, repo_id: repoId });
}

/** Page through `GET /vaults/v1/:vault_id/objects`, collecting every entry, stripped to repo-relative keys. Store-and-echo cursor per D3a — never parsed here either. */
export async function listVaultObjectsAll(client: Client, repoId: string): Promise<VaultObjectEntry[]> {
  const base = `/vaults/v1/${encodeURIComponent(repoId)}/objects`;
  const out: VaultObjectEntry[] = [];
  let cursor: string | undefined;
  for (;;) {
    const qs = cursor ? `?cursor=${encodeURIComponent(cursor)}` : "";
    const page = await client.request<VaultObjectsPage>(`${base}${qs}`, { context: "listing vault objects" });
    ...
    if (!page.next_cursor) fail("VAULT_MIRROR_LISTING_INCONSISTENT", "the objects listing reported has_more with no next_cursor", "listing vault objects");
...
export async function readVaultObjectBytes(client: Client, repoId: string, entry: VaultObjectEntry): Promise<Uint8Array | null> {
  const base = `/vaults/v1/${encodeURIComponent(repoId)}`;
  ...
  if (generationRoute) {
    ...
    const expectedKind =
      generationRoute.route === "admissions" ? "admission_record" : generationRoute.generation === VAULT_GENESIS_GENERATION ? "vault_genesis" : "head";
    if (entry.object_kind !== expectedKind) {
      fail(
        "VAULT_MIRROR_KEY_UNRECOGNIZED",
        `listed object_kind '${entry.object_kind}' does not match its generation-addressed key ${entry.key} (expected '${expectedKind}')`,
        "reading vault mirror source object",
    ...
    const auth = (await client.credentials.getAuth(path, { method: `vault.read_${generationRoute.route}` })) ?? {};
    const r = await client.fetch(`${client.apiBase}${path}`, { method: "GET", headers: { ...auth, accept: "application/json" } });
    if (r.status === 404) return null;
    if (!r.ok) fail("VAULT_MIRROR_READ_FAILED", `${entry.key} read failed (HTTP ${r.status})`, "reading vault mirror source object", { key: entry.key, status: r.status });
    return new Uint8Array(await r.arrayBuffer());
  }
  const read = objectReadRequestForEntry(entry);
  // `edge_url` (vault-read-edge-cache design D5) is the same optional,
  // same-bytes CDN companion `vault-publication.ts`'s object-reads
  // consumers get — see `fetchVaultObjectBytes` (`vault-edge-fetch.ts`)
  // for the prefer-edge/silent-fallback policy shared across both.
  //
  // `inline` (vault-small-object-inline design D1/D2) is the same
  // optional, same-bytes small-object offer those consumers get too — see
  // `vault-publication.ts`'s `resolveObjectReadTarget` for the sibling
  ...
  try {
    presigned = await client.request(`${base}/object-reads`, { method: "POST", body: { objects: [read] }, context: "resolving vault mirror source object" });
  } catch (e) {
  ...
  const r = await fetchVaultObjectBytes(client, target);
  if (r.status === 404) return null;
  if (!r.ok) fail("VAULT_MIRROR_READ_FAILED", `${entry.key} GET failed (HTTP ${r.status})`, "reading vault mirror source object", { key: entry.key, status: r.status });
  return new Uint8Array(await r.arrayBuffer());
}

/** Exported for the key-shape conformance test (`vault-mirror-recover.test.ts`) — never called for its own sake outside this module. */
 ...
 */
export function objectReadRequestForEntry(entry: VaultObjectEntry): { object_kind: string; object_id?: string; epoch?: string; recipient_fingerprint?: string } {
  if (entry.object_kind === "key_envelope") {
    // `key-envelopes/<epoch>/<recipient_fingerprint>.env` — the GATEWAY's
    // real §3 wire key (`packages/gateway/src/services/vault/
    ...
    // `vault-creation-journal.ts` envelope `path` string
    ...
    if (!m) fail("VAULT_MIRROR_KEY_UNRECOGNIZED", `key_envelope key does not match the expected layout: ${entry.key}`, "building vault mirror read request", { key: entry.key });
  ...
  if (!spec) {
    fail("VAULT_MIRROR_KEY_UNRECOGNIZED", `unrecognized object_kind '${entry.object_kind}' for mirror read (key ${entry.key})`, "building vault mirror read request", { key: entry.key, object_kind: entry.object_kind });
  }
  const m = spec.pattern.exec(entry.key);
  if (!m) fail("VAULT_MIRROR_KEY_UNRECOGNIZED", `${entry.object_kind} key does not match the expected §3 layout: ${entry.key}`, "building vault mirror read request", { key: entry.key, object_kind: entry.object_kind });
 ...
 * THIS machine's own identity. A multi-recipient vault (vault-human-
 ...
 * path so `VAULT_MIRROR_KEY_UNRECOGNIZED` still surfaces).
 */
...
/** The local keystore's own `ek_` fingerprint, or `null` when no identity exists on this machine (keyless / never-`vault init`-ed). */
function localEncryptionFingerprint(keystore: VaultKeystore): string | null {
  return keystore.readIdentity()?.encryption_fingerprint ?? null;
}

// ─── Admission-order planning ──────────────────────────────────────────────────

export interface VaultMirrorWritePlan {
  ...
  admissionsAndHeads: VaultObjectEntry[];
}

/** Sort a flat listing into the admission-order write plan (module doc above). */
export function planMirrorWrite(entries: readonly VaultObjectEntry[]): VaultMirrorWritePlan {
  const objects: VaultObjectEntry[] = [];
  const byGeneration = new Map<string, { admission?: VaultObjectEntry; head?: VaultObjectEntry }>();
  ...
  const admissionsAndHeads: VaultObjectEntry[] = [];
...
export interface VaultMirrorCopyResult {
  key: string;
...
export interface VaultMirrorSyncSummary {
  repo_id: string;
  destination: string;
   ...
   * (vault-human-envelopes) makes this the norm. Never counted toward
 ...
 */
async function reconcileOne(client: Client, repoId: string, backend: VaultMirrorBackend, entry: VaultObjectEntry, ownFingerprint: string | null): Promise<VaultMirrorCopyResult> {
  ...
  try {
    ...
    const bytes = await readVaultObjectBytes(client, repoId, entry);
...
export interface VaultMirrorSyncOptions {
...
/**
 * `run402 repos mirror sync` (task 2.3): list the vault's stored objects,
 ...
 */
export async function mirrorSync(client: Client, repoId: string, options: VaultMirrorSyncOptions = {}): Promise<VaultMirrorSyncSummary> {
  ...
  const entries = await listVaultObjectsAll(client, repoId);
  ...
  // vault-mirror-default: a zero-failure pass is the event that clears the
...
function openBackendFromConfig(keystore: VaultKeystore, repoId: string): VaultMirrorBackend {
  const config = readMirrorConfig(keystore, repoId);
  if (!config) fail("VAULT_MIRROR_NOT_CONFIGURED", `no mirror is configured for ${repoId}`, "opening vault mirror", { repo_id: repoId }, [{ action: "run402 repos mirror <destination>" }]);
...
export interface VaultMirrorPushResult {
  ...
  summary?: VaultMirrorSyncSummary;
  error?: string;
}
 ...
 */
export async function mirrorPushForGeneration(client: Client, repoId: string, options: VaultMirrorSyncOptions = {}): Promise<VaultMirrorPushResult> {
  let keystore: VaultKeystore;
  try {
    keystore = options.keystore ?? VaultKeystore.open();
...
// ─── BYO chain-copy dual-write (vault-byo-primary-bucket task 3.3) ─────────

/**
 * The SAME six payload kinds the gateway's `VAULT_BYO_PAYLOAD_KINDS`
 * names (`services/vault/upload-sessions.ts`) — client-written directly
 ...
 */
const VAULT_BYO_PAYLOAD_OBJECT_KINDS: ReadonlySet<string> = new Set([
  "wal_pack",
  "ref_state",
  "retention_roots",
  "checkpoint_manifest",
 ...
 */
function chainOnlyEntries(entries: readonly VaultObjectEntry[]): VaultObjectEntry[] {
  const plan = planMirrorWrite(entries);
  return [...plan.objects.filter((e) => !VAULT_BYO_PAYLOAD_OBJECT_KINDS.has(e.object_kind)), ...plan.admissionsAndHeads];
}
 ...
 */
export async function byoChainCopySync(client: Client, repoId: string, options: VaultMirrorSyncOptions = {}): Promise<VaultMirrorSyncSummary> {
  const keystore = options.keystore ?? VaultKeystore.open();
 ...
 */
export async function byoChainCopyPushForGeneration(client: Client, repoId: string, options: VaultMirrorSyncOptions = {}): Promise<VaultMirrorPushResult> {
  let keystore: VaultKeystore;
  try {
    keystore = options.keystore ?? VaultKeystore.open();
...
// ─── BYO absence adjudication (vault-byo-primary-bucket task 3.3, read half) ─

export interface VaultByoMissingObject {
  key: string;
  object_kind: string;
}

export interface VaultByoPresenceReport {
  repo_id: string;
  destination: string;
  checked: number;
  missing: VaultByoMissingObject[];
}
 ...
 * `VAULT_BYO_OBJECT_MISSING` naming exactly what the chain says should
 * exist and does not.
 */
export async function verifyByoObjectsPresent(client: Client, repoId: string, options: VaultMirrorSyncOptions = {}): Promise<VaultByoPresenceReport> {
  const keystore = options.keystore ?? VaultKeystore.open();
  const backend = options.backend ?? openByoBackendFromConfig(keystore, repoId);
  const destination = options.backend ? backend.describe() : formatMirrorDestination(readByoConfig(keystore, repoId)!.destination);
  const entries = await listVaultObjectsAll(client, repoId);
  const missing: VaultByoMissingObject[] = [];
