/**
 * vault test fixture — an in-memory control plane + bucket implementing
 * the FULL {@link VaultTransport} (5.3 creation + 5.4 publication + 5.6
...
import type { VaultActivationToken, VaultAllocation, VaultCaptureReceipt, VaultHead, VaultHeadsListingPage, VaultHeadsListingRequest, VaultRetentionCutoff } from "../namespaces/vault.types.js";
import type { VaultAdmitGenesisRequest, VaultAdmitGenesisResult, VaultAllocateRequest, VaultObjectReceipt, VaultPutObjectRequest } from "./vault-creation-journal.js";
import { createVault } from "./vault-creation-journal.js";
import { VaultKeystore } from "./vault-keystore.js";
import type { VaultAdmitHeadRequest, VaultAdmitHeadResult, VaultCompactionGrant, VaultDesiredRecipientEntry, VaultEnvelopeRecipientsResponse, VaultMaintenanceLease, VaultMaintenanceLeaseRequest, VaultOrgEncryptionKeyDirectory, VaultOrgEncryptionKeyEntry, VaultRetentionCutoffIssued, VaultTransport, VaultUploadObject, VaultUploadReceipt, VaultRecord, VaultState } from "./vault-publication.js";
import type { VaultOpenReceipt, VaultRecipientConfirmationReceipt, VaultRotationAttemptDescriptor } from "../namespaces/vault.types.js";
import type { VaultPruneIntentRecord } from "./vault-prune.js";
import { Vault, generationToBigInt, bigIntToGeneration, vaultPaths } from "./vault-publication.js";
...
export class VaultMemoryTransport implements VaultTransport {
  readonly service = generateSigningKeypair();
  readonly objects = new Map<string, Uint8Array>();
  readonly allocations = new Map<string, VaultAllocation>();
  ...
  /**
   * vault-multi-writer rev 47 — `repo_id -> (writer_key_id ->
  ...
  readonly desiredRecipients = new Map<string, VaultDesiredRecipientEntry[]>();
  /** `org_id -> desired_state_version`. */
  readonly desiredStateVersions = new Map<string, number>();
  /** `org_id -> {state, revocation}` — D194's org-scoped watermark pair (`internal.vault_recipient_state_counters`). Lazily initialized at 0/0, mirroring the real gateway's `INSERT ... ON CONFLICT DO UPDATE` upsert-lock. */
  ...
  readonly rotationAttempts = new Map<string, { bytes: Uint8Array; descriptor: VaultRotationAttemptDescriptor }>();
  ...
  // ── creation (5.3) ──
  async allocate(req: VaultAllocateRequest): Promise<VaultAllocation> {
    ...
    if (!a) {
      ...
      this.repoCounter += 1;
      const unsigned = {
        format: VAULT_FORMAT, object_kind: "allocation" as const, suite: VAULT_SUITE,
      ...
      a = signVaultObject(unsigned, this.service.seed) as VaultAllocation;
  ...
  async putObject(req: VaultPutObjectRequest): Promise<VaultObjectReceipt> {
  ...
  /**
   * `POST …/head-reads` (vault-batched-head-reads) — ONE logged call
  ...
  async getHeads({ repo_id, generations }: { repo_id: string; generations: string[] }): Promise<Uint8Array[] | null> {
    ...
    for (const g of generations) {
      const bytes = this.objects.get(this.key(repo_id, vaultPaths.head(g)));
  ...
  async admitGenesis(req: VaultAdmitGenesisRequest): Promise<VaultAdmitGenesisResult> {
    const k = this.key(req.repo_id, vaultPaths.head(VAULT_GENESIS_GENERATION));
  ...
  getGenesis({ repo_id }: { repo_id: string }): Promise<Uint8Array | null> {
    return this.getObject({ repo_id, path: vaultPaths.head(VAULT_GENESIS_GENERATION) });
  }
  ...
  async listHeads(request: VaultHeadsListingRequest & { repo_id: string }): Promise<VaultHeadsListingPage> {
    ...
    for (; g <= newest && BigInt(heads.length) < limit; g++) {
      ...
      heads.push({ generation: gen, stored_bytes_sha256: sha256Hex(this.objects.get(this.key(request.repo_id, vaultPaths.head(gen)))!) });
    }
    ...
    return { format: VAULT_FORMAT, repo_id: request.repo_id, after_generation: request.after_generation, heads, has_more: hasMore, next_cursor: hasMore ? cursor : null, total: hasMore ? null : String(newest > anchor ? newest - anchor : 0n) };
  }

  // ── uploads (create-only) ──
  async uploadObjects({ repo_id, objects }: { repo_id: string; objects: VaultUploadObject[] }): Promise<VaultUploadReceipt[]> {
    this.calls.push(`upload:${objects.length}`);
    const receipts: VaultUploadReceipt[] = [];
    objects.forEach((o, i) => {
      const k = this.key(repo_id, o.path);
      const existing = this.objects.get(k);
      if (existing && sha256Hex(existing) !== o.sha256) throw err("VAULT_OBJECT_EXISTS_DIFFERENT", `${o.path} exists with different bytes`);
      if (sha256Hex(o.bytes) !== o.sha256 || String(o.bytes.length) !== o.size_bytes) throw err("VAULT_CHECKSUM_MISMATCH", `${o.path}: bytes do not match the declared checksum`);
  ...
  async admitHead(req: VaultAdmitHeadRequest): Promise<VaultAdmitHeadResult> {
    ...
    const head = parseVaultStrict(new TextDecoder().decode(req.stored_bytes)) as VaultHead;
    // vault-multi-writer rev 47: add_writer_key ACTIVATED, same precedent
    ...
    // vault-multi-writer rev 47: add_writer_key needs none of the
    ...
    if (req.generation !== expected || head.generation !== expected) {
      const winnerBytes = this.objects.get(this.key(req.repo_id, vaultPaths.head(newest)))!;
    ...
    for (const id of receipted) {
      const path = id.startsWith("refs_") ? vaultPaths.refState(id) : id.startsWith("rr_") ? vaultPaths.retentionRoots(id) : vaultPaths.wal(id);
      if (!this.objects.has(this.key(req.repo_id, path))) throw err("VAULT_RECEIPT_UNKNOWN", `receipted object ${id} was never finalized`);
    }
    if (head.checkpoint && !this.objects.has(this.key(req.repo_id, vaultPaths.claimSet(head.checkpoint.claim_set.object_id)))) throw err("VAULT_RECEIPT_UNKNOWN", "claim set never finalized");
    ...
    this.objects.set(this.key(req.repo_id, vaultPaths.head(req.generation)), stored);
    const record = new TextEncoder().encode(JSON.stringify({ admission: req.generation, admitted_sha256: req.stored_bytes_sha256 }));
    this.objects.set(this.key(req.repo_id, vaultPaths.admission(req.generation)), record);
    let capture_receipt: VaultCaptureReceipt | null = null;
    if (head.capture_binding) {
      capture_receipt = signVaultObject({
        format: VAULT_FORMAT, object_kind: "capture_receipt" as const, suite: VAULT_SUITE, repo_id: req.repo_id, service_key_id: "sk_test-1", generation: req.generation,
        ...
        admission_record_sha256: sha256Hex(record), issued_at: formatVaultTimestamp(), effective_admitted_at: formatVaultTimestamp(), authorization_epoch: this.authorizationEpoch,
      }, this.service.seed) as VaultCaptureReceipt;
    }
    return { outcome: "admitted", admission_record_sha256: sha256Hex(record), capture_receipt };
  }

  async requestRetentionCutoff({ repo_id, base_head_sha256 }: { repo_id: string; base_head_sha256: string }): Promise<VaultRetentionCutoffIssued> {
    const ticket = signVaultObject({ format: VAULT_FORMAT, object_kind: "retention_cutoff" as const, suite: VAULT_SUITE, repo_id, object_id: newVaultId("rc"), service_key_id: "sk_test-1", base_head_sha256, cutoff_at: formatVaultTimestamp(), expires_at: formatVaultTimestamp(new Date(Date.now() + 3600_000)), authorization_epoch: this.authorizationEpoch }, this.service.seed) as VaultRetentionCutoff;
    const bytes = new TextEncoder().encode(JSON.stringify(ticket));
    this.objects.set(this.key(repo_id, vaultPaths.cutoffTicket(ticket.object_id)), bytes);
  ...
  async exchangeActivationToken({ repo_id, operation_id, capture_receipt }: { repo_id: string; operation_id: string; capture_receipt: VaultCaptureReceipt }): Promise<VaultActivationToken> {
    ...
    if (!op) throw err("VAULT_ACCESS_DENIED", "unknown operation");
    if (capture_receipt.apply_plan_sha256 === null) throw err("VAULT_TOKEN_NOT_MINTABLE", "no token is mintable from a null plan digest");
    if (capture_receipt.repo_id !== repo_id || capture_receipt.capture_id !== op.capture_id || capture_receipt.apply_plan_sha256 !== op.apply_plan_sha256 || capture_receipt.snapshot_oid_hmac !== op.snapshot_oid_hmac) throw err("VAULT_TOKEN_BINDING_MISMATCH", "receipt does not bind the operation row");
    if (capture_receipt.authorization_epoch !== this.authorizationEpoch) throw err("AUTHORIZATION_EPOCH_STALE", "stale epoch");
    return signVaultObject({
      format: VAULT_FORMAT, object_kind: "activation_token" as const, suite: VAULT_SUITE, repo_id, object_id: newVaultId("ct"), service_key_id: "sk_test-1", operation_id,
      ...
      issued_at: formatVaultTimestamp(), authorization_epoch: this.authorizationEpoch,
    }, this.service.seed) as VaultActivationToken;
  }

  /** Atomic + idempotent consumption (the apply-activation side, modelled for the fake lane). */
  consumeToken(token: VaultActivationToken, operationId: string): void {
    ...
    if (prior !== undefined) {
      ...
      throw err("VAULT_TOKEN_CONSUMED", "token consumed by a different operation");
    }
  ...
  async submitOverrideCompletion({ repo_id, operation_id, capture_receipt }: { repo_id: string; operation_id: string; capture_receipt: VaultCaptureReceipt }): Promise<{ cleared: boolean }> {
    this.calls.push("override-completion");
    const op = this.operations.get(operation_id);
    if (!op || !op.override) throw err("VAULT_ACCESS_DENIED", "no unvaulted operation to complete");
    const headBytes = this.objects.get(this.key(repo_id, vaultPaths.head(capture_receipt.generation)));
  ...
  vaultRecord: Partial<VaultRecord> = {};
  leases = new Map<string, { holder_token: string; released: boolean }>();
  /** `org_id -> directory rows` — vault-human-envelopes task 4.1's org encryption-key directory (`GET /orgs/v1/:org_id/encryption-keys`). Tests set this directly. */
  orgEncryptionKeys = new Map<string, VaultOrgEncryptionKeyEntry[]>();

  async listOrgEncryptionKeys({ org_id }: { org_id: string }): Promise<VaultOrgEncryptionKeyDirectory> {
   ...
   */
  async listEnvelopeRecipients({ repo_id }: { repo_id: string }): Promise<VaultEnvelopeRecipientsResponse> {
  ...
  async getVaultRecord({ repo_id }: { repo_id: string }): Promise<VaultRecord> {
    ...
    // record, but `getState` (vault-composite-state-read task 4.4) needs
    // it correct.
    ...
    return {
      ...
      vault_policy: "required", vault_policy_version: "1", vault_policy_changed_at: null,
      ...
      // vault-agent-envelopes: the signed allocation rides the record, as the
  ...
  /**
   * `GET …/state` (vault-composite-state-read task 4.4) — the pin-current
   ...
   */
  /** vault-delta-fetch: set true to simulate a gateway that predates the delta arm. */
  deltaDisabled = false;

  async getState({ repo_id, since }: { repo_id: string; since?: string }): Promise<VaultState> {
    this.calls.push("state");
    const vault = await this.getVaultRecord({ repo_id });
    if (vault.newest_generation === null || vault.newest_generation === VAULT_GENESIS_GENERATION) {
      return { vault, newest_generation: vault.newest_generation, head: null, carriers: null };
    }
    const headBytes = this.objects.get(this.key(repo_id, vaultPaths.head(vault.newest_generation)));
    ...
    // (`services/vault/reads.ts`): it locates the head's OWN
    // ref_state/retention_roots receipts with a plain parse, never
    // `parseVaultStrict`'s canonical-form check — the client is the sole
    // strict verifier, downstream of this call. Using the strict parser here
    // would make a byte-tampered head throw `VAULT_STRICT_PARSE` from
    // INSIDE the transport, before the SDK's own hash-check-before-parse
    // ordering ({@link Vault.tryStateFastPath}) ever gets a chance to
    ...
    const refState = this.objects.get(this.key(repo_id, vaultPaths.refState(refStateId))) ?? null;
    const retentionRoots = this.objects.get(this.key(repo_id, vaultPaths.retentionRoots(retentionRootsId))) ?? null;
    // vault-delta-fetch: mirror the gateway's delta arm — a qualifying
    ...
    if (since !== undefined && !this.deltaDisabled && /^[0-9a-f]{16}$/.test(since)) {
      ...
      if (span >= 1n && span <= 4n) {
        ...
        for (let g = sinceBig + 1n; g <= newestBig; g += 1n) {
          ...
          const bytes = this.objects.get(this.key(repo_id, vaultPaths.head(generation)));
          if (!bytes) {
            qualified = false;
            break;
          }
          ...
          for (const w of parsed.wal_entries ?? []) {
            ...
            const packBytes = this.objects.get(this.key(repo_id, vaultPaths.wal(w.object_id)));
  ...
  async findVaultByProject({ project_id }: { project_id: string }): Promise<VaultRecord> {
    ...
    if (!repoId) throw err("RESOURCE_NOT_FOUND", `no vault for ${project_id}`);
    return { ...(await this.getVaultRecord({ repo_id: repoId })), project_id };
  }

  /** D6 (task 4.3) — resolve by address-form `org-slug/name`. */
  async findVaultByRepo({ org_slug, repo_name }: { org_slug: string; repo_name: string }): Promise<VaultRecord> {
  ...
  readonly compactionGrants = new Map<string, VaultCompactionGrant>();
  ...
  async openCompactionGrant({ repo_id }: { repo_id: string }): Promise<VaultCompactionGrant> {
    ...
    if (existing) throw err("VAULT_COMPACTION_GRANT_ACTIVE", "a compaction grant is already active for this project", { expires_at: existing.expires_at });
    ...
    // wire-faithful.
    const grant: VaultCompactionGrant = {
   ...
   */
  readonly pruneIntents = new Map<string, { bytes: Uint8Array; record: VaultPruneIntentRecord }>();
  ...
  async submitPruneIntent({ repo_id, intent_bytes }: { repo_id: string; intent_bytes: Uint8Array }): Promise<VaultPruneIntentRecord & { stored: boolean }> {
    ...
    const intent = parseVaultStrict(new TextDecoder().decode(intent_bytes)) as {
      object_id: string;
      repo_id: string;
    ...
    if (intent.repo_id !== repo_id) throw err("VAULT_ACCESS_DENIED", "intent repo_id does not match the vault");
    ...
    for (const ref of intent.verifier_receipts) {
      ...
      const stored = this.objects.get(this.key(repo_id, vaultPaths.verifierReceipt(ref.object_id)));
    ...
    const record: VaultPruneIntentRecord = {
  ...
  async getPruneIntent({ prune_intent_object_id }: { repo_id: string; prune_intent_object_id: string }): Promise<VaultPruneIntentRecord | null> {
  ...
  async createRotationAttempt({ repo_id, descriptor }: { repo_id: string; descriptor: VaultRotationAttemptDescriptor }): Promise<{ rotation_id: string; descriptor: VaultRotationAttemptDescriptor; deduplicated: boolean }> {
    ...
    if (!writerPub || !verifyVaultObject(descriptor as never, writerPub)) throw err("VAULT_ACCESS_DENIED", "Not authorized for this vault");
  ...
  async confirmRecipient({ repo_id, principal_id, new_fingerprint }: { repo_id: string; principal_id: string; new_fingerprint: string }): Promise<VaultRecipientConfirmationReceipt> {
    this.calls.push("confirm");
    const manifest = this.effectivePinManifest.get(repo_id);
    return signVaultObject({
      format: VAULT_FORMAT, object_kind: "recipient_confirmation_receipt" as const, object_id: `rcr_${newHex32()}`, repo_id,
      purpose: "first_pin" as const, principal_id, new_fingerprint, base_pin_manifest_sha256: manifest?.sha256 ?? "0".repeat(64),
      recipient_state_version: "0", issued_at: formatVaultTimestamp(), service_key_id: "sk_test-1",
    }, this.service.seed) as VaultRecipientConfirmationReceipt;
  }

  async repinRecipient({ repo_id, principal_id, old_ek_fingerprint, new_fingerprint }: { repo_id: string; principal_id: string; old_ek_fingerprint: string; new_fingerprint: string }): Promise<VaultRecipientConfirmationReceipt> {
  ...
  async declareWriterAuthorityUnavailable({ repo_id: _repo_id }: { repo_id: string }): Promise<{ declared_at: string; declared_by: string | null }> {
    ...
    return { declared_at: formatVaultTimestamp(), declared_by: null };
  }
  ...
  readonly openReceipts = new Map<string, VaultOpenReceipt>();

  /**
   * D210 (rev 44) — models the SAME checks `services/vault/open-receipts.ts`'s
  ...
  async submitOpenProof({
    ...
    const receipt = signVaultObject({
      format: VAULT_FORMAT, object_kind: "recipient_open_receipt" as const, object_id: `ror_${newHex32()}`, repo_id,
      principal_id, ek_fingerprint, chain_verified_to_generation, decryptable_to_generation, reader_entrypoint,
      source: "recipient_submission" as const, issued_at: formatVaultTimestamp(), service_key_id: "sk_test-1",
    }, this.service.seed) as VaultOpenReceipt;
...
export interface VaultFixture {
  ...
  transport: VaultMemoryTransport;
  repoDir: string;
  repoId: string;
  vault: Vault;
}
...
export async function makeVault(root?: string): Promise<VaultFixture> {
  const r = root ?? mkdtempSync(join(tmpdir(), "run402-vault-fixture-"));
  const keystore = VaultKeystore.open({ rootDir: join(r, "ks") });
  const transport = new VaultMemoryTransport();
  const repoDir = await makeRepo(r);
  const created = await createVault({ keystore, transport, org_id: "org_1", project_id: "proj_1", service_public_key: transport.service.public_key });
  const vault = Vault.open({ keystore, transport, repo_id: created.repo_id, repo_dir: repoDir });
  return { root: r, keystore, transport, repoDir, repoId: created.repo_id, vault };
}

describe("vault memory transport (fixture self-check)", () => {
  it("lists nothing above the genesis on a fresh vault", async () => {
    const t = new VaultMemoryTransport();
