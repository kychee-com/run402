/**
 * vault-client-round-trips (task 1.1) — the measuring stick.
 *
 ...
 * one real round trip): each `VaultTransport` method costs exactly what
 * {@link createVaultHttpTransport} (the real, network-backed
 * implementation in `vault-publication.ts`) actually sends over the
 * wire to fulfill it —
 *
 ...
 *     object fits the vault-composite-state-read design D2 inline caps
 *     ({@link vaultInlineUploadEligible} — the SAME predicate
 *     `createVaultHttpTransport`'s `upload()` consults, so this file
 ...
 *
 * Wraps ANY `VaultTransport` implementation (the in-memory test fixture
...
import { vaultInlineUploadEligible, vaultWireRefForPath } from "./vault-publication.js";
import type { VaultTransport, VaultUploadObject } from "./vault-publication.js";
import type { VaultPutObjectRequest } from "./vault-creation-journal.js";

/** One counted unit, tagged by kind — `ops.length` is the total; group by kind for a breakdown. */
export class VaultOpCounter {
  readonly ops: string[] = [];
 ...
 */
export function assertOpBudget(counter: VaultOpCounter, budget: number, label: string): void {
...
export function countingVaultTransport(inner: VaultTransport, counter: VaultOpCounter): VaultTransport {
    ...
    get(target, prop, receiver) {
      ...
      if (prop === "getObject") {
        return async (req: { repo_id: string; path: string }) => {
          const ref = vaultWireRefForPath(req.path);
      ...
      if (prop === "uploadObjects") {
        return async (req: { repo_id: string; objects: VaultUploadObject[]; resource_binding?: unknown }) => {
          if (req.objects.length > 0) {
            if (vaultInlineUploadEligible(req.objects)) counter.count("upload-inline", 1);
            else {
      ...
      if (prop === "putObject") {
        return async (req: VaultPutObjectRequest) => {
          // `putObject` always wraps exactly one object — same eligibility
          // predicate, applied to that one object's bytes.
          if (vaultInlineUploadEligible([{ bytes: req.bytes }])) counter.count("upload-inline", 1);
          else {
            counter.count("upload-session", 1);
      ...
      // Every other VaultTransport method — one real HTTP request per call.
