/**
 * vault-agent-envelopes design D4 — the COLD OPEN, against the in-memory
 * transport fixture: a second keystore that is a directory recipient but has
 * no repo file for the vault (1) fails `VAULT_ENVELOPE_PENDING` with the
...
import { toBase64url } from "../namespaces/vault.crypto.js";
...
describe("Vault.ensureRepoState — cold open from own envelope (vault-agent-envelopes D4)", () => {
  it("pending until wrapped → restored platform_attested/first_seen → no-op thereafter", async () => {
    ...
    const bRoot = mkdtempSync(join(tmpdir(), "vault-cold-open-b-"));
    const bKeystore = VaultKeystore.open({ rootDir: bRoot });
    ...
    const pending = await rejectsCode(
      Vault.openOrRestore({ keystore: bKeystore, transport: a.transport, repo_id: a.repoId }),
    ...
    const opened = await Vault.openOrRestore({ keystore: bKeystore, transport: a.transport, repo_id: a.repoId });
    ...
    const again = await Vault.openOrRestore({ keystore: bKeystore, transport: a.transport, repo_id: a.repoId });
  ...
  it("refuses a genesis whose creator keys disagree with the signed allocation (substituted genesis)", async () => {
    const a = await makeVault();
    const bRoot = mkdtempSync(join(tmpdir(), "vault-cold-open-sub-"));
    const bKeystore = VaultKeystore.open({ rootDir: bRoot });
    const bIdentity = bKeystore.ensureIdentity();
    a.transport.orgEncryptionKeys.set("org_1", [
    ...
    await rejectsCode(Vault.openOrRestore({ keystore: bKeystore, transport: a.transport, repo_id: a.repoId }), "VAULT_CREATION_CONFLICT");
