/**
 * vault-mirror-and-recover — mirror storage backends (design D1/D2/D7,
 * task 2.2).
 *
 * S3-or-directory behind ONE interface, so the writer/reconcile engine
 * (`vault-mirror.ts`) and the recovery engine (`vault-recover.ts`)
 ...
 * `AWS_SESSION_TOKEN`). Multipart upload is out of scope too — vault
...
export interface VaultMirrorObjectMeta {
  size_bytes: string;
}

/**
 * vault-byo-primary-bucket task 3.1 — allocation-time bucket probe
 ...
 */
export interface VaultBucketProbeResult {
...
export interface VaultMirrorBackend {
  ...
  /**
   * vault-byo-primary-bucket task 3.1 — the allocation-time bucket-policy
   ...
   * three independent facts; the caller ({@link probeVaultByoDestination}
   * in `vault-byo-probe.js`) decides whether to refuse and names which
   ...
   * failed one.
   */
  probeWritePolicy(): Promise<VaultBucketProbeResult>;
}
 ...
 * torn write.
 */
export class DirectoryMirrorBackend implements VaultMirrorBackend {
  ...
  private resolve(key: string): string {
    if (key.includes("..") || key.startsWith("/")) fail("VAULT_MIRROR_KEY_INVALID", `unsafe mirror key: ${key}`, "resolving vault mirror path", { key });
    return join(this.root, ...key.split("/"));
  }

  async head(key: string): Promise<VaultMirrorObjectMeta | null> {
   ...
   */
  async probeWritePolicy(): Promise<VaultBucketProbeResult> {
    const key = `_byo-probe/${randomBytes(16).toString("hex")}`;
    try {
      const first = await this.putCreateOnly(key, new TextEncoder().encode("vault-byo-primary-bucket allocation probe"));
      ...
      const second = await this.putCreateOnly(key, new TextEncoder().encode("vault-byo-primary-bucket allocation probe — second write"));
...
export function resolveMirrorCredentials(credential: VaultMirrorCredential): Sigv4Credentials {
  if (credential.kind === "ambient") {
    const accessKeyId = process.env.AWS_ACCESS_KEY_ID;
    const secretAccessKey = process.env.AWS_SECRET_ACCESS_KEY;
    if (!accessKeyId || !secretAccessKey) {
      fail("VAULT_MIRROR_CREDENTIALS_UNRESOLVED", "no ambient AWS credentials found (AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY unset)", "resolving vault mirror credentials", undefined, [{ action: "export AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY, or configure a named profile with `run402 repos mirror <destination> --profile <name>`" }]);
    }
  ...
  try {
    ...
    fail("VAULT_MIRROR_CREDENTIALS_UNRESOLVED", `no AWS credentials file at ${credsPath}`, "resolving vault mirror credentials", { profile: credential.profile }, [{ action: `run 'aws configure --profile ${credential.profile}', or configure the destination for --ambient credentials instead` }]);
  }
  ...
  if (!accessKeyId || !secretAccessKey) {
    fail("VAULT_MIRROR_CREDENTIALS_UNRESOLVED", `profile '${credential.profile}' is missing aws_access_key_id/aws_secret_access_key in ${credsPath}`, "resolving vault mirror credentials", { profile: credential.profile }, [{ action: `run 'aws configure --profile ${credential.profile}'` }]);
  }
...
export class S3MirrorBackend implements VaultMirrorBackend {
  constructor(
    private readonly bucket: string,
    private readonly prefix: string,
    private readonly region: string,
    private readonly credential: VaultMirrorCredential,
  ...
  async head(key: string): Promise<VaultMirrorObjectMeta | null> {
    ...
    if (!r.ok) fail("VAULT_MIRROR_S3_ERROR", `HEAD ${key} failed (HTTP ${r.status})`, "reading vault mirror object", { key, status: r.status });
  ...
  async get(key: string): Promise<Uint8Array | null> {
    ...
    if (r.status === 404) return null;
    if (!r.ok) fail("VAULT_MIRROR_S3_ERROR", `GET ${key} failed (HTTP ${r.status})`, "reading vault mirror object", { key, status: r.status });
  ...
  async putCreateOnly(key: string, bytes: Uint8Array): Promise<{ created: boolean }> {
    ...
    if (r.status === 400 || r.status === 501) {
      ...
      if (!retry.ok) fail("VAULT_MIRROR_S3_ERROR", `PUT ${key} failed (HTTP ${retry.status})`, "writing vault mirror object", { key, status: retry.status });
      return { created: true };
    }
    fail("VAULT_MIRROR_S3_ERROR", `PUT ${key} failed (HTTP ${r.status})`, "writing vault mirror object", { key, status: r.status });
  }
  ...
  async list(prefix = ""): Promise<string[]> {
    ...
    for (;;) {
      ...
      if (!r.ok) fail("VAULT_MIRROR_S3_ERROR", `ListObjectsV2 failed (HTTP ${r.status})`, "listing vault mirror objects", { status: r.status });
  ...
  /**
   * vault-byo-primary-bucket task 3.1 (design D6). Deliberately does NOT
  ...
  async probeWritePolicy(): Promise<VaultBucketProbeResult> {
    ...
    try {
      const first = await s3Request({
        ...
        body: new TextEncoder().encode("vault-byo-primary-bucket allocation probe"),
      ...
      if (!first.ok) {
        ...
        const second = await s3Request({
          method: "PUT", bucket: this.bucket, region: this.region, endpoint: this.endpoint, key: this.fullKey(key), credentials: creds,
          body: new TextEncoder().encode("vault-byo-primary-bucket allocation probe — second write"),
          extraHeaders: { "if-none-match": "*" },
        });
...
export function openVaultMirrorBackend(destination: VaultMirrorDestination, repoId: string, credential?: VaultMirrorCredential): VaultMirrorBackend {
  if (destination.kind === "directory") {
    return new DirectoryMirrorBackend(join(destination.path, "source", repoId));
  }
  if (!credential) fail("VAULT_MIRROR_CREDENTIAL_REQUIRED", "an s3 mirror destination needs a credential", "opening vault mirror backend");
...
/**
 * vault-byo-primary-bucket task 3.1 — open a backend at the DESTINATION
 ...
 */
export function openVaultDestinationBackend(destination: VaultMirrorDestination, credential?: VaultMirrorCredential): VaultMirrorBackend {
  if (destination.kind === "directory") return new DirectoryMirrorBackend(destination.path);
  if (!credential) fail("VAULT_MIRROR_CREDENTIAL_REQUIRED", "an s3 destination needs a credential", "opening vault destination backend");
 ...
 * openVaultMirrorBackend} expects. Used only to resolve which vault a bare
 * destination URL (no `--repo`) names.
 */
export async function discoverMirroredRepoIds(destination: VaultMirrorDestination, credential?: VaultMirrorCredential): Promise<string[]> {
  const root = openVaultDestinationBackend(destination, credential);
...
export function _rmMirrorRoot(destination: VaultMirrorDestination, repoId: string): void {
