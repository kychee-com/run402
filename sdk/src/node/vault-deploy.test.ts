 ...
 * consumption, the CLOSED five outcomes, the always-printed `vault_commit`,
...
import { storedBytesSha256, verifyVaultObject } from "../namespaces/vault.crypto.js";
import type { VaultActivationToken, VaultCaptureReceipt, VaultSignedObject } from "../namespaces/vault.types.js";
import {
  ...
  readOverrideJournal,
  runVaultDeploy,
  type VaultDeployLane,
  type VaultDeployLaneCommitInput,
  type VaultDeployLanePlan,
  type VaultDeployLanePlanInput,
} from "./vault-deploy.js";
import { Vault } from "./vault-publication.js";
import { VAULT_DEPLOY_REF, captureSnapshot, capturedSetDigest, capturedSetUnchanged, deriveCapturedSet, diffCapturedSets } from "./vault-snapshot.js";
import { git, makeVault, type VaultMemoryTransport, type VaultFixture } from "./vault-memory-transport.test.js";
...
if (!vectorFile) describe("vault deploy vectors", () => it.skip(OPTOUT_SKIP_MESSAGE, () => {}));
...
interface FakeLane extends VaultDeployLane {
  plans: VaultDeployLanePlanInput[];
  commits: VaultDeployLaneCommitInput[];
 ...
 */
function makeLane(transport: VaultMemoryTransport, options: { planFails?: string; commitFails?: string; overrideAuthorized?: boolean; onPlan?: () => void } = {}): FakeLane {
  let n = 0;
  const lane: FakeLane = {
    plans: [], commits: [], seen: [],
    async plan(input) {
      ...
      const plan: VaultDeployLanePlan = { plan_id: `pln_${n}`, operation_id, apply_plan_sha256: `${String(n).repeat(2)}${"ab".repeat(31)}` };
...
describe("§6.5 push-gated deploy — the closed five outcomes", () => {
  ...
  it("DEPLOYED_AND_VAULTED: the snapshot is vaulted, the receipt is exchanged for a token bound to THIS deploy, and the token is consumed once", async (t) => {
    ...
    const r = await runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir, onCommitLine: (l) => lines.push(l) });

    assert.equal(r.outcome, "DEPLOYED_AND_VAULTED");
    assert.equal(r.vault_commit, head, "a clean tree deploys HEAD itself");
    assert.deepEqual(lines, [`vault_commit ${head}`], "the vault_commit line is printed as soon as the snapshot exists");
    ...
    assert.deepEqual(state.refs, { [VAULT_DEPLOY_REF]: head });
  ...
  it("commit ids never reach the platform: the binding carries a keyed commitment, not the oid", async (t) => {
    const f = await deployFixture();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const r = await runVaultDeploy({ vault: f.vault, lane: makeLane(f.transport), repo_dir: f.repoDir });
    if (r.outcome !== "DEPLOYED_AND_VAULTED") return assert.fail(r.outcome);
    assert.notEqual(r.snapshot_oid_hmac, r.vault_commit);
    ...
    for (const carrier of plaintextCarriers) assert.ok(!carrier.includes(r.vault_commit), "no plaintext control-plane message carries the commit id");
  });
  ...
  it("the deploy lane collects from the work tree — correspondence is verified, not isolated", async (t) => {
    ...
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const lane = makeLane(f.transport);
    const r = await runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir });
    assert.equal(r.outcome, "DEPLOYED_AND_VAULTED");
  ...
  it("DEPLOY_FAILED_VAULTED: a build that fails before a plan exists still captures the attempt — with a NULL digest, from which no token is mintable", async (t) => {
    ...
    await rejectsCode(f.transport.exchangeActivationToken({ repo_id: f.repoId, operation_id: "op_null", capture_receipt: r.capture_receipt! }), "VAULT_TOKEN_NOT_MINTABLE");
  });
  ...
  it("DEPLOY_BLOCKED_PUSH_FAILED: the previous release keeps serving, and the override is offered WITH its capability", async (t) => {
    ...
    assert.equal(r.push_error.code, "VAULT_RECEIPT_MISMATCH");
  ...
  it("DEPLOY_FAILED_UNVAULTED: neither lane landed", async (t) => {
    ...
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    f.transport.tamperReceiptAt = 1;
    const r = await runVaultDeploy({ vault: f.vault, lane: makeLane(f.transport, { planFails: "BUILD_FAILED" }), repo_dir: f.repoDir, allow_unvaulted: { reason: "prod is down" } });
  ...
  it("DEPLOYED_UNVAULTED_OVERRIDE: the journal is written BEFORE the commit, carrying the full §6.5 contents", async (t) => {
    ...
    const head = await git(f.repoDir, ["rev-parse", "HEAD"]);
    const r = await runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir, allow_unvaulted: { reason: "hotfix while the vault is unreachable" } });
    ...
    assert.deepEqual(
      { repo_id: j.repo_id, operation_id: j.operation_id, capture_id: j.capture_id, apply_plan_sha256: j.apply_plan_sha256, snapshot: j.snapshot, snapshot_oid_hmac: j.snapshot_oid_hmac, publication_state: j.publication_state, reason: j.reason },
      {
        ...
        snapshot: { ref: VAULT_DEPLOY_REF, oid: head, head_target: { kind: "symref", ref: "refs/heads/main" } },
  ...
  it("an unauthorized override throws OVERRIDE_NOT_AUTHORIZED and leaves NO advisory (nothing activated)", async (t) => {
    ...
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    f.transport.tamperReceiptAt = 1;
    await rejectsCode(runVaultDeploy({ vault: f.vault, lane: makeLane(f.transport, { overrideAuthorized: false }), repo_dir: f.repoDir, allow_unvaulted: { reason: "nope" } }), "OVERRIDE_NOT_AUTHORIZED");
  ...
  it("a snapshot refusal precedes both lanes: nothing is planned, pushed, or journaled", async (t) => {
    ...
    const lane = makeLane(f.transport);
    await rejectsCode(runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir }), "SNAPSHOT_CONFLICTED_INDEX");
    assert.deepEqual(lane.plans, []);
    assert.equal(f.transport.calls.filter((c) => c.startsWith("admit:")).length, 0);
  });
});

// ─── Snapshot correspondence (change `vault-deploy-lane`, design D1–D3) ───
...
describe("snapshot correspondence — the captured set may not move under a deploy", () => {
  it("a tracked file edited between capture and commit REFUSES, and commits nothing", async (t) => {
    ...
    await rejectsCode(runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir }), "SNAPSHOT_MOVED_DURING_DEPLOY");
  ...
  it("the refusal NAMES the changed paths, and says which side moved", async (t) => {
    ...
    try {
      await runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir });
      assert.fail("expected SNAPSHOT_MOVED_DURING_DEPLOY");
    } catch (e) {
      assert.equal(codeOf(e), "SNAPSHOT_MOVED_DURING_DEPLOY");
      const details = (e as LocalError).details as { modified: string[]; added: string[]; removed: string[]; captured_digest: string; observed_digest: string; vault_commit: string };
  ...
  it("an untouched DIRTY tree proceeds too — the synthetic-commit path re-derives identically", async (t) => {
    ...
    const lane = makeLane(f.transport);
    const r = await runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir, snapshot: { allowDirty: true } });
  ...
  it("a dirty tree WITHOUT allowDirty refuses SNAPSHOT_DIRTY_TREE before the deploy-lane capture runs — same rule, same lane", async (t) => {
    ...
    writeFileSync(join(f.repoDir, "app.js"), "uncommitted work\n");
    const lane = makeLane(f.transport);
    await rejectsCode(runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir }), "SNAPSHOT_DIRTY_TREE");
  ...
  it("refuses BEFORE the override journal is written — a refusal leaves no advisory to drain", async (t) => {
    ...
    await rejectsCode(
      runVaultDeploy({ vault: f.vault, lane, repo_dir: f.repoDir, allow_unvaulted: { reason: "vault unreachable" } }),
  ...
  it("the refusal carries the vaulted facts, so the caller knows the source DID reach the vault", async (t) => {
    ...
    try {
      ...
      const details = (e as LocalError).details as { generation?: string; vault_commit: string; capture_id: string };
      assert.equal(details.generation, "0000000000000001");
      assert.match(details.vault_commit, /^[0-9a-f]{40}$/);
...
describe("§6.5 override completion — equality on EVERY field or no clear", () => {
  it("a later push of the JOURNALED snapshot completes the operation and clears the advisory", async (t) => {
    ...
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    f.transport.tamperReceiptAt = 1;
    const r = await runVaultDeploy({ vault: f.vault, lane: makeLane(f.transport, { overrideAuthorized: true }), repo_dir: f.repoDir, allow_unvaulted: { reason: "vault unreachable" } });
    assert.equal(r.outcome, "DEPLOYED_UNVAULTED_OVERRIDE");
    ...
    assert.deepEqual((await f.vault.materialize()).refs, { [VAULT_DEPLOY_REF]: r.vault_commit });
  ...
  it("a receipt that differs on ANY field never clears the advisory", async (t) => {
    ...
    assert.equal(report.remaining[0]!.error.code, "VAULT_OVERRIDE_COMPLETION_REFUSED");
  ...
  it("matchCaptureReceipt names every field that differs", () => {
    const receipt = { repo_id: "src_1", capture_id: "a".repeat(32), apply_plan_sha256: "b".repeat(64), snapshot_oid_hmac: "c".repeat(64), head_sha256: "d".repeat(64) } as VaultCaptureReceipt;
  ...
  it("an operation_id that is not a safe file name never becomes a path", () => {
    ...
    throwsCode(() => overrideJournalPath(ks, "../../etc/passwd"), "VAULT_BAD_ID");
    throwsCode(() => overrideJournalPath(ks, "op/with/slash"), "VAULT_BAD_ID");
  });
});
...
describe("§4.10 activation tokens — client-side binding checks", () => {
  ...
  const token = { object_kind: "activation_token", object_id: "ct_" + "9".repeat(32), authorization_epoch: "e".repeat(32), ...bound } as VaultActivationToken;
  ...
  it("a mint that hands back a token bound to a DIFFERENT operation is refused before it is ever presented", async (t) => {
    ...
    assert.equal(r.outcome, "DEPLOY_FAILED_VAULTED");
    if (r.outcome !== "DEPLOY_FAILED_VAULTED") return;
    assert.equal(r.deploy_error.code, "VAULT_TOKEN_BINDING_MISMATCH");
...
vectors("§4.10 activation tokens — vector class `activation-token`", () => {
  it("replays all four: the signed token's stored bytes, the CONSUMED terminal state, and bytewise epoch comparison", async (t) => {
    ...
    for (const v of byClass("activation-token")) {
      seen.add(v.id);
      if (v.id === "token-001") {
        const object = v.inputs.object as VaultActivationToken;
        assert.equal(storedBytesSha256(object as unknown as VaultSignedObject), v.expected.stored_bytes_sha256, v.id);
        assert.equal(verifyVaultObject(object as unknown as VaultSignedObject, v.inputs.service_pubkey), true, `${v.id} service signature`);
      ...
      if (v.id === "token-004") {
        ...
        const tok = { object_id: "ct_" + "4".repeat(32), authorization_epoch: f.transport.authorizationEpoch } as VaultActivationToken;
        f.transport.consumeToken(tok, v.inputs.consumed_by_operation_id);
        throwsCode(() => f.transport.consumeToken(tok, v.inputs.attempting_operation_id), "VAULT_TOKEN_CONSUMED", v.description);
...
vectors("§5A / §4.10 state machines — vector class `state-scenario` (the client-observable half)", () => {
  ...
  it("fence-002: a DIFFERENT canonical winner surfaces to the client as HEAD_CAS_CONFLICT and re-applies", async (t) => {
    const f = await deployFixture();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const noRetry = new Vault({ keystore: f.keystore, transport: f.transport, repo_id: f.repoId, repo_dir: f.repoDir, conflict_retries: 0 });
  ...
  it("token-002 / token-003: consumption is atomic, and a replay for the SAME operation is a READ of the terminal state", async (t) => {
    const f = await makeVault();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const tok = { object_id: "ct_" + "2".repeat(32), authorization_epoch: f.transport.authorizationEpoch } as VaultActivationToken;
  ...
  it("token-006: a DR gate that installs a fresh epoch invalidates an already-issued token", async (t) => {
    const f = await makeVault();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const tok = { object_id: "ct_" + "6".repeat(32), authorization_epoch: f.transport.authorizationEpoch } as VaultActivationToken;
  ...
  it("the client-observable / server-only partition covers the whole class", () => {
    ...
    console.log(`vault deploy vectors rev ${vectorFile!["x-r402s-revision"]}: activation-token ${vectorFile!.counts_by_class["activation-token"]}/${vectorFile!.counts_by_class["activation-token"]}, state-scenario ${clientSide.length}/${all.length} client-side (${SERVER_ONLY.size} server-only)`);
  });
});
