/**
 * vault — the principal keystore (protocol rev 41 §5.1; task 5.2).
 *
 ...
 * {@link VaultKeystore.assess} and acted on by explicit methods — the
...
export interface VaultIdentityFile {
  version: 1;
...
export interface VaultRepoFile {
  version: 1;
  repo_id: string;
  org_id: string;
  project_id: string;
  /** Raw 32-byte secret for {@link VaultRepoFile.epoch} (`K_repo`/`K_1` pre-rotation, `K_e` after one), lowercase hex. */
  k_repo_hex: string;
  ...
  /**
   * vault-object-host-predial (design D1): the object-store ORIGINS
   ...
   * {@link VAULT_OBJECT_STORE_ORIGINS_CAP}. Same locally-learned,
   ...
   * state — it feeds the connection prewarm ONLY (`predialVaultObjectStore`
   * in `vault-prewarm.ts`), never verification, never a source of truth
  ...
  /**
   * TOFU pins for {@link Vault.reconcileEnvelopeRecipients}
   * (vault-human-envelopes design D4 point 3): `principal_id ->` the
   ...
   * Vault.publishPinManifestUpdate} or {@link
   * Vault.rotateEpoch}'s `pending_confirmations` fold. Read-side
   * short-circuit ONLY: {@link Vault} `readPinManifestObject`
  ...
  /**
   * vault-multi-writer rev 47 (protocol §4.15) — the writer-set analog of
   * {@link head_pin}: the chain-replayed writer state as of THIS repo's own
   * `head_pin`/`verified_prefix` generation, persisted so `Vault.verifyToNewest`
  ...
  /**
   * vault-multi-writer rev 47 — THIS principal's own relationship to the
  ...
  /**
   * vault-multi-writer rev 47 (task 5.6) — set by `resume()` the moment a
...
export interface VaultPermissionFinding {
  path: string;
...
export type VaultKeystoreState =
  | { state: "ready"; repo: VaultRepoFile }
  | { state: "repo_state_lost_identity_intact"; next_action: "restore_from_envelope" }
  | { state: "read_only"; repo: VaultRepoFile; reason: "signing_key_lost" }
  | { state: "stale_pin"; repo: VaultRepoFile; next_action: "reverify_from_genesis" }
  | { state: "unrecoverable"; code: "VAULT_UNRECOVERABLE"; statement: string; doctor_text: string };

export interface VaultKeystoreOptions {
  /** Override the keystore root (tests). Defaults to `<config dir>/vault`. */
  rootDir?: string;
  /** Clock injection (tests). */
  now?: () => Date;
  /** When true, a permission finding on open throws `VAULT_KEYSTORE_PERMISSIONS` instead of only auditing. */
...
export interface VaultLockOptions {
  ...
  staleAfterMs?: number;
  /** Total wait budget before `VAULT_KEYSTORE_LOCKED` (ms). Default 30 s. */
  timeoutMs?: number;
}
...
export const VAULT_OBJECT_CACHE_WINDOW = 8;

/** vault-object-host-predial (design D1): the max persisted object-store origins per repo — small on purpose, this is a predial hint, not an inventory. */
export const VAULT_OBJECT_STORE_ORIGINS_CAP = 4;

/** Bare hex16 generation comparison for cache eviction — deliberately NOT `generationToBigInt` (vault-publication.ts), which this lower-level module must not import (publication already imports FROM here). Malformed input sorts as "evict" rather than throwing: a corrupt cache entry should never block eviction of everything else. */
...
/** The keystore directory's earlier name under the same config dir. */
const FORMER_KEYSTORE_DIRNAME = "gitvault";

/**
 * Default keystore root: `<active profile config dir>/vault`.
 *
 * This is the one place the keystore path is first resolved, so it is also
 * where a keystore still at `<config>/gitvault` moves to `<config>/vault`:
 * one same-directory `rename`, which is atomic, and only when `vault` does
 * not exist yet. Nothing reads the earlier path after that. A move another
 * process already made is not an error; any other refusal throws
 * `VAULT_KEYSTORE_MOVE_FAILED` naming both paths, because answering with an
 * empty `vault` directory would make a live keystore look lost.
 */
export function getVaultKeystoreRoot(): string {
  const configDir = getConfigDir();
  const root = join(configDir, "vault");
  moveFormerKeystoreDir(join(configDir, FORMER_KEYSTORE_DIRNAME), root);
  return root;
}

function moveFormerKeystoreDir(former: string, root: string): void {
  if (existsSync(root)) return;
  let stat;
  try {
    stat = lstatSync(former);
  } catch {
    return; // nothing to move
  }
  if (!stat.isDirectory() && !stat.isSymbolicLink()) return;
  try {
    renameSync(former, root);
  } catch (e) {
    const code = (e as NodeJS.ErrnoException).code;
    if (existsSync(root) || code === "ENOENT") return; // another process moved it first
    throw new LocalError(
      `could not move the vault keystore from ${former} to ${root}: ${e instanceof Error ? e.message : String(e)}`,
      "resolving the vault keystore",
      { code: "VAULT_KEYSTORE_MOVE_FAILED", details: { from: former, to: root }, cause: e },
    );
  }
}
...
export function writeFileAtomic0600(path: string, text: string): void {
  ...
  if (existsSync(path) && lstatSync(path).isSymbolicLink()) {
    fail("VAULT_KEYSTORE_SYMLINK", `refusing to write through a symlink: ${path}`, "writing vault keystore file", { path });
  }
...
export function readFileNoFollow(path: string): string | null {
  ...
  if (st.isSymbolicLink()) {
    fail("VAULT_KEYSTORE_SYMLINK", `refusing to read through a symlink: ${path}`, "reading vault keystore file", { path });
  }
...
function auditPath(path: string, expectDir: boolean): VaultPermissionFinding[] {
  const findings: VaultPermissionFinding[] = [];
  let st;
  try {
    st = lstatSync(path);
  } catch {
...
export class VaultKeystore {
  ...
  constructor(options: VaultKeystoreOptions = {}) {
    this.rootDir = options.rootDir ?? getVaultKeystoreRoot();
  ...
  static open(options: VaultKeystoreOptions = {}): VaultKeystore {
    const ks = new VaultKeystore(options);
    ...
    if (findings.length > 0 && ks.strictPermissions) {
      fail("VAULT_KEYSTORE_PERMISSIONS", "vault keystore permission audit failed", "opening vault keystore", { findings });
    }
    return ks;
  }
  ...
  repoPath(repoId: string): string {
    if (!VAULT_SRC_RE.test(repoId)) fail("VAULT_BAD_REPO_ID", `not a src_ id: ${repoId}`, "resolving vault repo path");
    return join(this.reposDir, `${repoId}.json`);
  }
  recoveryReceiptPath(repoId: string): string {
    if (!VAULT_SRC_RE.test(repoId)) fail("VAULT_BAD_REPO_ID", `not a src_ id: ${repoId}`, "resolving vault receipt path");
  ...
  auditPermissions(): VaultPermissionFinding[] {
    const findings: VaultPermissionFinding[] = [];
  ...
  audit(event: VaultAuditEvent, repoId?: string, details?: Record<string, unknown>): void {
    const entry: VaultAuditEntry = { at: formatVaultTimestamp(this.now()), event, ...(repoId ? { repo_id: repoId } : {}), ...(details ? { details } : {}) };
    mkdir0700(this.rootDir);
    if (isSymlink(this.auditLogPath)) {
      fail("VAULT_KEYSTORE_SYMLINK", "refusing to append through a symlinked audit.log", "appending vault audit log");
    }
  ...
  ensureIdentity(): VaultIdentityFile {
    ...
    if (existsSync(this.identityPath)) {
      fail("VAULT_KEYSTORE_CONFLICT", "identity.json appeared concurrently; refusing to overwrite", "creating vault identity");
    }
  ...
  bindIdentityPrincipal(principalId: string): void {
    ...
    if (identity.enrolled_principal_id) {
      fail("VAULT_IDENTITY_PROFILE_MISMATCH", `this keystore identity enrolled as principal ${identity.enrolled_principal_id}, not ${principalId} — refusing cross-profile reuse`, "binding vault identity", { enrolled_principal_id: identity.enrolled_principal_id, principal_id: principalId });
    }
  ...
  signingKeypair(identity: VaultIdentityFile = this.requireIdentity()): VaultSigningKeypair | null {
    ...
    if (vkFingerprint(kp.public_key) !== identity.signing_fingerprint) {
      fail("VAULT_KEYSTORE_CORRUPT", "identity signing seed does not derive the recorded fingerprint", "loading vault signing key");
    }
    return kp;
  }

  /** The encryption keypair, or `null` when the X25519 private key is lost. */
  encryptionKeypair(identity: VaultIdentityFile = this.requireIdentity()): VaultEncryptionKeypair | null {
    ...
    if (ekFingerprint(kp.public_key) !== identity.encryption_fingerprint) {
      fail("VAULT_KEYSTORE_CORRUPT", "identity encryption key does not derive the recorded fingerprint", "loading vault encryption key");
    }
    return kp;
  }

  private requireIdentity(): VaultIdentityFile {
    const id = this.readIdentity();
    if (!id) fail("VAULT_UNRECOVERABLE", VAULT_TERMINAL_LOSS_DOCTOR_TEXT, "loading vault identity", { statement: VAULT_TERMINAL_LOSS_STATEMENT });
    return id;
  }

  // ── repo files ──

  readRepo(repoId: string): VaultRepoFile | null {
    const text = readFileNoFollow(this.repoPath(repoId));
    if (!text) return null;
    const parsed = JSON.parse(text) as VaultRepoFile;
    if (parsed.version !== 1 || parsed.repo_id !== repoId || typeof parsed.k_repo_hex !== "string") {
      fail("VAULT_KEYSTORE_CORRUPT", `repos/${repoId}.json is not a version-1 vault repo file`, "reading vault repo file", { repo_id: repoId });
    }
    return parsed;
  }
  ...
  listRepoIds(): string[] {
    ...
    return readdirSync(this.reposDir).filter((n) => n.endsWith(".json")).map((n) => n.slice(0, -5)).filter((id) => VAULT_SRC_RE.test(id)).sort();
  }

  /**
   * vault-offline-clone-resolve (design D1/D2/D6, task 1.1): the
   * keystore's project→repo lookup that lets {@link resolveVaultAddress}
   * (`vault-address.ts`) answer an id-form address WITHOUT the network
   ...
   * through the same {@link recoverStaleVaultPin} first-use-refusal path.
   *
   ...
   */
  findRepoByProject(projectId: string, orgId?: string): VaultRepoFile | null {
    let best: { repo: VaultRepoFile; mtimeMs: number } | null = null;
    for (const repoId of this.listRepoIds()) {
      let repo: VaultRepoFile | null;
      try {
  ...
  saveRepo(repo: Omit<VaultRepoFile, "version" | "updated_at">): VaultRepoFile {
    const full: VaultRepoFile = { version: 1, ...repo, updated_at: formatVaultTimestamp(this.now()) };
  ...
  updateRepo(repoId: string, patch: Partial<Pick<VaultRepoFile, "head_pin" | "materialized_pin" | "verified_prefix" | "last_ref_transaction" | "epoch" | "envelope_recipient_pins" | "k_repo_hex" | "epoch_keys" | "known_pin_manifest" | "checkpoint_covers_through" | "object_store_origins" | "writer_set_pin" | "writer_status" | "pending_writer_admission">>): VaultRepoFile {
    return this.withRepoLock(repoId, () => {
      const existing = this.readRepo(repoId);
      if (!existing) fail("VAULT_REPO_STATE_MISSING", `no repo file for ${repoId}`, "updating vault repo file", { repo_id: repoId });
      const full: VaultRepoFile = { ...existing, ...patch, updated_at: formatVaultTimestamp(this.now()) };
  ...
  /**
   * vault-object-host-predial (design D1/D4, task 1.2): record that
   ...
   * deduped; capped at {@link VAULT_OBJECT_STORE_ORIGINS_CAP}. A true
  ...
  recordObjectStoreOrigins(repoId: string, origins: readonly string[]): void {
    ...
    try {
      ...
      for (const o of current) {
        if (next.length >= VAULT_OBJECT_STORE_ORIGINS_CAP) break;
        if (!next.includes(o)) next.push(o);
      }
      const capped = next.slice(0, VAULT_OBJECT_STORE_ORIGINS_CAP);
  ...
  // ── local immutable-object cache (vault-client-round-trips design D3) ──
  //
  ...
  // does not itself re-verify anything — every caller (`Vault`)
  ...
  private objectsRepoDir(repoId: string): string {
    if (!VAULT_SRC_RE.test(repoId)) fail("VAULT_BAD_REPO_ID", `not a src_ id: ${repoId}`, "resolving vault object cache path");
  ...
  /** Writes the head, then evicts anything more than {@link VAULT_OBJECT_CACHE_WINDOW} generations behind it. */
  ...
  /**
   * Drop heads and carriers more than {@link VAULT_OBJECT_CACHE_WINDOW}
  ...
  private evictObjectCache(repoId: string, latestGeneration: string): void {
    const cutoff = genToBigInt(latestGeneration) - BigInt(VAULT_OBJECT_CACHE_WINDOW - 1);
   ...
   */
  recordEpochRotation(repoId: string, input: { new_epoch: string; new_k_repo_hex: string }): VaultRepoFile {
    return this.withRepoLock(repoId, () => {
      const existing = this.readRepo(repoId);
      if (!existing) fail("VAULT_REPO_STATE_MISSING", `no repo file for ${repoId}`, "recording vault epoch rotation", { repo_id: repoId });
      const epochKeys = { ...(existing.epoch_keys ?? {}), [existing.epoch]: existing.k_repo_hex, [input.new_epoch]: input.new_k_repo_hex };
      const full: VaultRepoFile = { ...existing, epoch: input.new_epoch, k_repo_hex: input.new_k_repo_hex, epoch_keys: epochKeys, updated_at: formatVaultTimestamp(this.now()) };
      writeFileAtomic0600(this.repoPath(repoId), JSON.stringify(full, null, 2));
  ...
  saveRecoveryReceipt(receipt: VaultRecoveryReceipt): void {
  ...
  readRecoveryReceipt(repoId: string): VaultRecoveryReceipt | null {
    const text = readFileNoFollow(this.recoveryReceiptPath(repoId));
    return text ? (JSON.parse(text) as VaultRecoveryReceipt) : null;
  }
   ...
   */
  withRepoLock<T>(repoId: string, fn: () => T, options: VaultLockOptions = {}): T {
    ...
    for (;;) {
      try {
        ...
        if (Date.now() >= deadline) {
          fail("VAULT_KEYSTORE_LOCKED", `another process holds the lock for ${repoId}`, "acquiring vault repo lock", { repo_id: repoId, lock: lockDir });
        }
    ...
    try {
      writeFileAtomic0600(join(lockDir, "owner.json"), JSON.stringify({ pid: process.pid, at: formatVaultTimestamp(this.now()) }));
    } catch {
   ...
   */
  assess(repoId: string, expectedGenesisSha256?: string): VaultKeystoreState {
    const identity = this.readIdentity();
    const repo = this.readRepo(repoId);
    let state: VaultKeystoreState;
    if (!identity && !repo) {
      state = { state: "unrecoverable", code: "VAULT_UNRECOVERABLE", statement: VAULT_TERMINAL_LOSS_STATEMENT, doctor_text: VAULT_TERMINAL_LOSS_DOCTOR_TEXT };
      ...
      state = identity.encryption_private_key_hex
        ? { state: "repo_state_lost_identity_intact", next_action: "restore_from_envelope" }
        : { state: "unrecoverable", code: "VAULT_UNRECOVERABLE", statement: VAULT_TERMINAL_LOSS_STATEMENT, doctor_text: VAULT_TERMINAL_LOSS_DOCTOR_TEXT };
  ...
  async restoreRepoFromEnvelope(input: {
    ...
    recovery_receipt?: VaultRecoveryReceipt | null;
    /**
     * vault-agent-envelopes D4: the caller verified genesis's creator
    ...
    /**
     * vault-multi-writer (rev 47): the vault's CURRENTLY admitted writers,
  ...
  }): Promise<{ repo: VaultRepoFile; trust: "receipt" | "platform_attested" | "unauthenticated_salvage"; continuity: "first_seen" | "pinned"; independently_verified: boolean }> {
    const identity = this.readIdentity();
    if (!identity) fail("VAULT_UNRECOVERABLE", VAULT_TERMINAL_LOSS_DOCTOR_TEXT, "restoring vault repo state", { statement: VAULT_TERMINAL_LOSS_STATEMENT });
    const recipient = this.encryptionKeypair(identity);
    if (!recipient) fail("VAULT_UNRECOVERABLE", VAULT_TERMINAL_LOSS_DOCTOR_TEXT, "restoring vault repo state", { statement: VAULT_TERMINAL_LOSS_STATEMENT });
    const { genesis, envelope } = input;
    if (!verifyVaultObject(genesis as unknown as VaultSignedObject, genesis.creator_signing_pubkey)) {
      fail("VAULT_SIGNATURE_INVALID", "vault_genesis signature does not verify", "restoring vault repo state", { repo_id: genesis.repo_id });
    }
    ...
    // wrapped post-genesis — vault-agent-envelopes D4's cold open) is
    ...
    if (bindings.length > 0) {
      fail("VAULT_CREATION_CONFLICT", "genesis key bindings do not hold for the stored envelope", "restoring vault repo state", { repo_id: genesis.repo_id, problems: bindings });
    }
    const genesisSha = storedBytesSha256(genesis as unknown as VaultSignedObject);
    ...
    if (priorPin && priorPin !== genesisSha) {
      fail("VAULT_CREATION_CONFLICT", `the admitted genesis (${genesisSha}) is not the one this keystore pinned (${priorPin}); refusing a substituted vault`, "restoring vault repo state", { repo_id: genesis.repo_id, admitted: genesisSha, pinned: priorPin });
    }
    ...
    if (receipt) {
      ...
      if (problems.length > 0) {
        fail("VAULT_CREATION_CONFLICT", "recovery receipt does not pin this genesis (substituted vault?)", "restoring vault repo state", { repo_id: genesis.repo_id, problems });
      }
      trust = "receipt";
    }
    if (envelope.recipient_fingerprint !== identity.encryption_fingerprint) {
      fail("VAULT_ENVELOPE_NOT_FOR_RECIPIENT", "the stored envelope is addressed to another principal", "restoring vault repo state", { recipient_fingerprint: envelope.recipient_fingerprint });
    }
  ...
  identitySummary(): { signing_fingerprint: string; encryption_fingerprint: string; signing_key_present: boolean; encryption_key_present: boolean } | null {
    const id = this.readIdentity();
    if (!id) return null;
    if (!VAULT_VK_RE.test(id.signing_fingerprint) || !VAULT_EK_RE.test(id.encryption_fingerprint)) {
      fail("VAULT_KEYSTORE_CORRUPT", "identity fingerprints are malformed", "reading vault identity");
    }
    return {
