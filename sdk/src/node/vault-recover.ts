/**
 * vault-mirror-and-recover — `r402s-recover`, the offline recovery engine
 * (design D4/D5, tasks 3.1–3.5).
 *
 * Turns a mirrored `source/<repo_id>/` prefix (any {@link VaultMirrorBackend}
 ...
 * INVOLVED: everything here reads from the {@link VaultMirrorBackend}
 ...
 * checkClaimSetEquality}, {@link vaultPaths}) — this file adds no new
...
import { assertNoTransition, checkChainLink, checkClaimSetEquality, vaultPaths, nextGeneration, reconcileRetainedTipRefs } from "./vault-publication.js";
import type { VaultEncounteredRotation, VaultEpochDecryptFailure, VaultRetainedRefsReconcileResult } from "./vault-publication.js";
import { VaultKeystore, type VaultIdentityFile } from "./vault-keystore.js";
import {
  discoverMemberBundles,
  parseMemberRecoveryBundle,
  unwrapMemberRecoveryBundle,
  type VaultMemberBundleHint,
  type VaultMemberRecoveryBundle,
} from "./vault-member-bundle.js";
import { hardenedGit, hasObject } from "./vault-snapshot.js";
import type { VaultMirrorBackend } from "./vault-mirror-backend.js";
...
export interface VaultChainEntry {
  generation: string;
  head: VaultHead;
  head_sha256: string;
}

export interface VaultChainBreak {
  generation: string;
  reason: string;
}
...
export interface VaultDiscoveryResult {
  ...
  chain_break: VaultChainBreak | null;
  /**
 ...
 */
export async function discoverAndVerifyChain(options: VaultDiscoverOptions): Promise<VaultDiscoveryResult> {
  const { backend } = options;
  const genesisBytes = await backend.get(vaultPaths.head(VAULT_GENESIS_GENERATION));
  if (!genesisBytes) fail("CHAIN_BROKEN", "the mirror holds no genesis (head/0000000000000000) — nothing to recover", "discovering vault from mirror", { destination: backend.describe() });
  const genesis = parseVaultStrict(new TextDecoder().decode(genesisBytes)) as VaultGenesis;
  if (genesis.object_kind !== "vault_genesis") fail("CHAIN_BROKEN", "head/0000000000000000 is not a vault_genesis object", "discovering vault from mirror");
  if (!verifyVaultObject(genesis as unknown as VaultSignedObject, genesis.creator_signing_pubkey)) {
    fail("VAULT_SIGNATURE_INVALID", "vault_genesis signature does not verify under its own claimed creator key", "discovering vault from mirror", { repo_id: genesis.repo_id });
  }
  ...
  let prevGen: string = VAULT_GENESIS_GENERATION;
  let prevEpoch: string = VAULT_GENESIS_EPOCH;
  let chainBreak: VaultChainBreak | null = null;
  const rotations: VaultEncounteredRotation[] = [];
  for (;;) {
    const nextGen = nextGeneration(prevGen);
    const bytes = await backend.get(vaultPaths.head(nextGen));
    if (!bytes) break; // natural end — the mirror holds nothing further at this generation
    let head: VaultHead;
    try {
      head = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
      checkChainLink({
 ...
 */
export async function closureForGeneration(backend: VaultMirrorBackend, chain: readonly VaultChainEntry[], targetGeneration: string): Promise<VaultRequiredObject[]> {
  const byGen = new Map(chain.map((e) => [e.generation, e]));
  const out: VaultRequiredObject[] = [];
  let gen = targetGeneration;
  for (;;) {
    if (gen === VAULT_GENESIS_GENERATION) break;
    const entry = byGen.get(gen);
    if (!entry) fail("CHAIN_BROKEN", `generation ${gen} is not in the verified chain`, "resolving vault recovery closure", { generation: gen });
    out.push({ key: vaultPaths.refState(entry.head.ref_state.object_id), object_id: entry.head.ref_state.object_id, kind: "ref_state" });
    out.push({ key: vaultPaths.retentionRoots(entry.head.retention_roots.object_id), object_id: entry.head.retention_roots.object_id, kind: "retention_roots" });
    if (entry.head.checkpoint) {
      const claimSetId = entry.head.checkpoint.claim_set.object_id;
      out.push({ key: vaultPaths.claimSet(claimSetId), object_id: claimSetId, kind: "checkpoint_claim_set" });
      ...
      const claimBytes = await backend.get(vaultPaths.claimSet(claimSetId));
      if (claimBytes) {
        const claimSet = parseVaultStrict(new TextDecoder().decode(claimBytes)) as VaultCheckpointClaimSet;
        out.push({ key: vaultPaths.checkpointManifest(claimSet.manifest_receipt.object_id), object_id: claimSet.manifest_receipt.object_id, kind: "checkpoint_manifest" });
        for (const p of claimSet.ordered_pack_receipts) out.push({ key: vaultPaths.checkpointPack(p.object_id), object_id: p.object_id, kind: "checkpoint_pack" });
      }
      break; // the checkpoint's recovery bundle is self-contained; no earlier WAL is needed
    }
    for (const w of entry.head.wal_entries) out.push({ key: vaultPaths.wal(w.object_id), object_id: w.object_id, kind: "wal_pack" });
 ...
 * recoverWithFallback}).
 */
export async function adjudicateAbsences(backend: VaultMirrorBackend, required: readonly VaultRequiredObject[]): Promise<VaultAbsenceAdjudication[]> {
  const missing: VaultRequiredObject[] = [];
  ...
  for (const key of pruneKeys) {
    ...
    if (!bytes) continue;
    try {
      const intent = parseVaultStrict(new TextDecoder().decode(bytes)) as { object_id: string; core?: { delete_set?: Array<{ object_id: string }> } };
  ...
  return missing.map((m): VaultAbsenceAdjudication => {
...
export interface VaultRecoveryReport {
  ...
  /** Non-null iff the chain walk itself broke (see {@link VaultDiscoveryResult.chain_break}). */
  chain_break: VaultChainBreak | null;
  /**
   ...
   * generic absence. Always `null` on {@link VaultVerifyReport} (mode
   * `keyless_verify` never opens a key).
   */
  epoch_decrypt_failure: VaultEpochDecryptFailure | null;
  /** Every absence encountered while resolving closures, across every generation tried, oldest attempt first. */
  absences: VaultAbsenceAdjudication[];
...
export interface VaultVerifyReport extends VaultRecoveryReport {
  mode: "keyless_verify";
   ...
   * this prefix — UNVERIFIED availability hints (vault-recovery-custody):
   ...
   */
  member_recovery_bundles: VaultMemberBundleHint[];
}
 ...
 */
export async function verifyVaultMirror(backend: VaultMirrorBackend, options: { recovery_receipt?: VaultRecoveryReceipt; keystore?: VaultKeystore } = {}): Promise<VaultVerifyReport> {
  ...
  if (generation !== VAULT_GENESIS_GENERATION) {
  ...
  return {
    ...
    validity_not_freshness: VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT,
    keystore_still_required: VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT,
 ...
 * vocabulary.
 */
async function resolveWithFallback(backend: VaultMirrorBackend, discovery: VaultDiscoveryResult, options: { requireMaterializable: boolean; start_generation?: string }): Promise<{ generation: string; absences: VaultAbsenceAdjudication[] }> {
  const allAbsences: VaultAbsenceAdjudication[] = [];
  let gen = options.start_generation ?? discovery.newest_generation;
  for (;;) {
    if (gen === VAULT_GENESIS_GENERATION) return { generation: gen, absences: allAbsences };
...
export interface VaultRecoverResult extends VaultRecoveryReport {
  mode: "recovered";
  out_dir: string;
   ...
   * every other on-disk vault layout; it is named explicitly here (rather
...
export interface VaultRecoverOptions {
  backend: VaultMirrorBackend;
  out_dir: string;
  recovery_receipt?: VaultRecoveryReceipt;
  keystore?: VaultKeystore;
  /**
   * vault-recovery-custody — the human-member path: a member recovery
   ...
   */
  member_bundle?: VaultMemberRecoveryBundle;
...
export type VaultRecoveryIdentity = Pick<VaultIdentityFile, "encryption_private_key_hex" | "encryption_fingerprint">;
...
async function resolveGenesisEpochKey(backend: VaultMirrorBackend, genesis: VaultGenesis, identity: VaultRecoveryIdentity): Promise<Uint8Array> {
  if (!identity.encryption_private_key_hex) {
    fail("VAULT_UNRECOVERABLE", "no encryption key available — recovery can verify and adjudicate but cannot decrypt or materialize anything", "materializing vault recovery", undefined, [
  ...
  if (genesis.envelopes.length === 0) fail("VAULT_CREATION_CONFLICT", "genesis carries no key envelope", "materializing vault recovery");
  ...
  // `vault-mirror.ts`), so this function never looks for, and never fails
  ...
  if (!envelopeReceipt) {
    fail("VAULT_ENVELOPE_NOT_FOR_RECIPIENT", "genesis carries no key envelope for this machine's identity — this keystore is not a recipient of this vault", "materializing vault recovery", { own_fingerprint: identity.encryption_fingerprint });
  }
  ...
  // exactly. The SDK's OWN `vault-creation-journal.ts` envelope `path`
  ...
  if (!envelopeBytes) fail("VAULT_UNRECOVERABLE", "the mirror holds no key envelope for this genesis — cannot derive K_repo", "materializing vault recovery");
  ...
  if (bindingProblems.length > 0) fail("VAULT_CREATION_CONFLICT", `genesis key bindings do not hold for the stored envelope (${bindingProblems.join(", ")})`, "materializing vault recovery", { problems: bindingProblems });
  if (envelope.recipient_fingerprint !== identity.encryption_fingerprint) {
    fail("VAULT_ENVELOPE_NOT_FOR_RECIPIENT", "the stored envelope is addressed to another principal — this keystore cannot open it", "materializing vault recovery", { recipient_fingerprint: envelope.recipient_fingerprint });
  }
 ...
 * `vault-epoch-reader.test.ts`'s own offline-recovery test, which is
...
async function resolveRotationEpochKeys(
  backend: VaultMirrorBackend,
  genesis: VaultGenesis,
  rotations: readonly VaultEncounteredRotation[],
  identity: VaultRecoveryIdentity,
): Promise<{ epoch_keys_hex: Record<string, string>; failure: VaultEpochDecryptFailure | null }> {
 ...
 * (`Vault.materialize`) so both surfaces report the same concept
 * under the same name.
 */
function computeDecryptableBoundaryGeneration(
...
): { generation: string; failure: VaultEpochDecryptFailure | null } {
  ...
  for (let i = 0; i < rotations.length; i++) {
    ...
    if (!epochKeysHex[rot.epoch]) {
      return {
        generation: boundary,
        failure: { generation: rot.generation, epoch: rot.epoch, rotation_id: rot.payload.rotation_id, code: "VAULT_EPOCH_NOT_OPENABLE", message: `epoch ${rot.epoch} (rotation ${rot.payload.rotation_id}) could not be opened by this keystore — recovery caps at the last generation under the prior, openable epoch` },
      };
    }
...
async function openCarrier<T>(backend: VaultMirrorBackend, kRepo: Uint8Array, repoId: string, epoch: string, kind: "ref_state" | "retention_roots" | "checkpoint_manifest", objectId: string, ciphertextSha256: string, key: string): Promise<T> {
  const frame = await backend.get(key);
  if (!frame) fail("CHAIN_UNUSABLE", `${kind} ${objectId} is absent from the mirror`, "materializing vault recovery", { object_id: objectId });
  ...
  return parseVaultStrict(new TextDecoder().decode(plaintext)) as T;
}
 ...
 */
export async function recoverVaultMirror(options: VaultRecoverOptions): Promise<VaultRecoverResult> {
  ...
  // `GIT_CWD_MISSING` disambiguation in `vault-snapshot.ts` — this is the
  // belt; that is the suspenders).
  try {
    mkdirSync(options.out_dir, { recursive: true });
  } catch (e) {
    fail("VAULT_RECOVER_OUT_DIR_UNWRITABLE", `could not create the recovery output directory: ${options.out_dir}`, "materializing vault recovery", { out_dir: options.out_dir, cause: e instanceof Error ? e.message : String(e) });
  }
  const { backend } = options;
  const keystore = options.keystore ?? VaultKeystore.open();
  ...
  //
  // vault-recovery-custody: the decrypt identity comes from EITHER the
  ...
  let identity: VaultRecoveryIdentity;
  let memberRecovery: VaultRecoverResult["member_recovery"] = null;
  if (options.source_recovery_code != null || options.member_bundle != null) {
    if (options.source_recovery_code == null) {
      fail("SOURCE_RECOVERY_CODE_REQUIRED", "a member recovery bundle was supplied without its source recovery code — the bundle is ciphertext; the code is what opens it", "materializing vault recovery", undefined, [
    ...
    if (options.member_bundle) {
      ...
      if (!opened) {
        fail("WRAPPER_DID_NOT_OPEN", `this mirror carries ${discovered.length} member recovery-bundle sidecar(s) but the supplied code opened none of them — a wrong code, or a bundle belonging to a different member`, "materializing vault recovery", { tried: discovered.map((d) => d.key), cause: lastErr instanceof Error ? lastErr.message : String(lastErr) });
      }
    ...
    if (!stored?.encryption_private_key_hex) {
      fail("VAULT_UNRECOVERABLE", "no encryption key in this keystore — recovery can verify and adjudicate but cannot decrypt or materialize anything", "materializing vault recovery", undefined, [
        { action: "restore the principal keystore (identity.json) from backup, then re-run recover" },
  ...
  const epochKeysHex: Record<string, string> = { [VAULT_GENESIS_EPOCH]: bytesToHex(k1), ...rotationKeys.epoch_keys_hex };
  ...
  if (generation === VAULT_GENESIS_GENERATION) {
    fail(
      ...
      epochDecryptFailure && startGeneration === VAULT_GENESIS_GENERATION
  ...
  const refState = await openCarrier<VaultRefState>(backend, kRepo, repoId, epoch, "ref_state", entry.head.ref_state.object_id, entry.head.ref_state.ciphertext_sha256, vaultPaths.refState(entry.head.ref_state.object_id));
  const roots = await openCarrier<VaultRetentionRoots>(backend, kRepo, repoId, epoch, "retention_roots", entry.head.retention_roots.object_id, entry.head.retention_roots.ciphertext_sha256, vaultPaths.retentionRoots(entry.head.retention_roots.object_id));
  ...
  // own `.git` (vault-byo-primary-bucket design D4), and a re-run lands
  ...
  let manifest: VaultCheckpointManifest | null = null;
  const first = heads[0]!;
  ...
  if (first.head.checkpoint) {
    const claimBytes = await backend.get(vaultPaths.claimSet(first.head.checkpoint.claim_set.object_id));
    if (!claimBytes) fail("CHECKPOINT_INCOMPLETE", "checkpoint claim set absent from the mirror", "materializing vault recovery");
    const claimSet = parseVaultStrict(new TextDecoder().decode(claimBytes)) as VaultCheckpointClaimSet;
    if (!verifyVaultObject(claimSet as unknown as VaultSignedObject, discovery.genesis.creator_signing_pubkey)) fail("CHECKPOINT_INCOMPLETE", "checkpoint claim set signature fails", "materializing vault recovery");
    manifest = await openCarrier<VaultCheckpointManifest>(backend, kRepoForEpoch(first.head.epoch), repoId, first.head.epoch, "checkpoint_manifest", claimSet.manifest_receipt.object_id, claimSet.manifest_receipt.ciphertext_sha256, vaultPaths.checkpointManifest(claimSet.manifest_receipt.object_id));
    checkClaimSetEquality(claimSet, manifest, first.head.checkpoint.covers_through_generation);
    for (const p of manifest.packs) {
      const frame = await backend.get(vaultPaths.checkpointPack(p.object_id));
      if (!frame) fail("CHECKPOINT_INCOMPLETE", `checkpoint pack ${p.object_id} absent`, "materializing vault recovery");
      ...
      if (sha256Hex(plain) !== p.plaintext_sha256 || String(plain.length) !== p.plaintext_size_bytes) fail("CHECKPOINT_INCOMPLETE", `checkpoint pack ${p.object_id} plaintext mismatch`, "materializing vault recovery");
      await hardenedGit(options.out_dir, ["index-pack", "--stdin", "--strict"], { input: plain });
    }
  }
  for (const h of heads) {
    for (const w of h.head.wal_entries) {
      const frame = await backend.get(vaultPaths.wal(w.object_id));
      if (!frame) fail("CHAIN_UNUSABLE", `WAL pack ${w.object_id} absent`, "materializing vault recovery", { object_id: w.object_id });
  ...
  for (const tip of coverageTips) {
    if (!(await hasObject(options.out_dir, tip))) fail("CHAIN_UNUSABLE", `covered tip ${tip} does not resolve after restore`, "materializing vault recovery", { oid: tip });
  }
  ...
  if (fsck.status !== 0) fail("CHECKPOINT_INCOMPLETE", `fsck reports missing connectivity after restore: ${fsck.stderr.slice(0, 500)}`, "materializing vault recovery");
  ...
  if (manifest) {
    ...
    if (!refmapOk || !rootsetOk || !objectsetOk) {
      fail("CHECKPOINT_INCOMPLETE", `restored state does not match the checkpoint's keyed commitments (refmap ${refmapOk ? "ok" : "MISMATCH"}, rootset ${rootsetOk ? "ok" : "MISMATCH"}, objectset ${objectsetOk ? "ok" : "MISMATCH"})`, "materializing vault recovery", { refmapOk, rootsetOk, objectsetOk });
    }
  }
