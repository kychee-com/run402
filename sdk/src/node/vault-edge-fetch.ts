 ...
 * folded into `vault-publication.ts` and exported from there:
 * `vault-mirror.ts` reimplements its own presigned-read call site
 ...
 */
export interface VaultEdgeFetchClient {
 ...
 */
export interface VaultEdgeReadTarget {
  url: string;
  edge_url?: string;
}
 ...
 */
let vaultEdgeDisabledForProcess = false;
 ...
 */
export function _resetVaultEdgeFetchStateForTest(): void {
  vaultEdgeDisabledForProcess = false;
}

/**
 * Fetch one vault object's bytes, preferring `target.edge_url` over
 ...
 */
export async function fetchVaultObjectBytes(
  client: VaultEdgeFetchClient,
  target: VaultEdgeReadTarget,
  init: RequestInit = { method: "GET" },
): Promise<Response> {
  if (target.edge_url && !vaultEdgeDisabledForProcess) {
    try {
    ...
    vaultEdgeDisabledForProcess = true;
  }
