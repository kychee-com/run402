/**
 * vault-mirror-and-recover — behavioral tests for groups 2 (mirror) and 3
 * (recovery), tasks 2.5 and 3.6.
 *
 * Reuses the project's own `VaultMemoryTransport` fixture (`makeVault` /
 ...
 * how the bytes were fetched — see `vault-mirror.ts`'s own module doc).
 */
...
import { jcs, sha256Hex, toBase64url } from "../namespaces/vault.crypto.js";
import { vaultPaths, vaultRetainedRefName } from "./vault-publication.js";
import { VaultKeystore } from "./vault-keystore.js";
import { hardenedGit } from "./vault-snapshot.js";
import { commitFile, git, makeVault } from "./vault-memory-transport.test.js";
import { DirectoryMirrorBackend, type VaultMirrorBackend } from "./vault-mirror-backend.js";
import {
  ...
  type VaultObjectEntry,
} from "./vault-mirror.js";
import { _resetVaultEdgeFetchStateForTest } from "./vault-edge-fetch.js";
import { adjudicateAbsences, discoverAndVerifyChain, recoverVaultMirror, verifyVaultMirror } from "./vault-recover.js";

function scratchDir(prefix: string): string {
  return mkdtempSync(join(tmpdir(), prefix));
}

/** A root, unrelated commit — never an ancestor of anything else built this way (no `-p` parent). Mirrors `vault-publication.test.ts`'s own helper of the same name. */
...
/**
 * `VaultMemoryTransport` stores every upload at the SDK's OWN client-local
 * `path` string (`vault-memory-transport.test.ts` `uploadObjects`: `this.key(repo_id,
 ...
 * `vault-creation-journal.ts` labels its upload manifest entry
 ...
 * `vault-mirror.ts`/`vault-recover.ts` against a spelling no REAL
 ...
 * `vault-mirror.ts`'s module doc mirror-layout mapping), the same as a
...
async function seedBackend(backend: VaultMirrorBackend, entries: readonly { key: string; bytes: Uint8Array }[]): Promise<void> {
...
/**
 * The WIRE shape of `GET /vaults/v1/:vault_id/objects` — full
...
// builders for every stored kind — `packages/gateway/src/services/vault/
...
// from the run402-private worktree named in the vault-mirror-and-recover
// brief. This is the standing guard against the exact bug this test file
// once carried: an SDK-internal label (`vault-creation-journal.ts`'s
...
describe("vault mirror/recover key-shape conformance (protocol §3)", () => {
  for (const row of KEY_SHAPE_TABLE) {
    it(`${row.kind}: ${row.key}`, () => {
      const entry: VaultObjectEntry = { key: row.key, object_kind: row.kind, sha256: "0".repeat(64), size_bytes: "1" };
  ...
  it("chain kinds: correctly-labeled entries read via the generation route and never touch object-reads", async () => {
    ...
    const genesis = await readVaultObjectBytes(fakeClient, repoId, { key: `head/${GENESIS_GEN}`, object_kind: "vault_genesis", sha256: "", size_bytes: "1" });
    assert.deepEqual([...genesis!], [9]);
    const head = await readVaultObjectBytes(fakeClient, repoId, { key: "head/0000000000000005", object_kind: "head", sha256: "", size_bytes: "1" });
    assert.deepEqual([...head!], [9]);
    const genesisAdmission = await readVaultObjectBytes(fakeClient, repoId, { key: `admissions/${GENESIS_GEN}`, object_kind: "admission_record", sha256: "", size_bytes: "1" });
    assert.deepEqual([...genesisAdmission!], [9]);
    const admission = await readVaultObjectBytes(fakeClient, repoId, { key: "admissions/0000000000000005", object_kind: "admission_record", sha256: "", size_bytes: "1" });
  ...
  it("chain kinds: a listed object_kind that does NOT match its generation-addressed key is refused, never silently read", async () => {
    ...
    const isKeyUnrecognized = (e: unknown): boolean => (e as { code?: string }).code === "VAULT_MIRROR_KEY_UNRECOGNIZED";

    // genesis mislabeled as an ordinary head
    await assert.rejects(readVaultObjectBytes(fakeClient, repoId, { key: `head/${GENESIS_GEN}`, object_kind: "head", sha256: "", size_bytes: "1" }), isKeyUnrecognized);
    // an ordinary generation's head mislabeled as vault_genesis
    await assert.rejects(readVaultObjectBytes(fakeClient, repoId, { key: "head/0000000000000005", object_kind: "vault_genesis", sha256: "", size_bytes: "1" }), isKeyUnrecognized);
    // an admission record mislabeled as a head (at any generation, including genesis)
    await assert.rejects(readVaultObjectBytes(fakeClient, repoId, { key: `admissions/${GENESIS_GEN}`, object_kind: "vault_genesis", sha256: "", size_bytes: "1" }), isKeyUnrecognized);
    await assert.rejects(readVaultObjectBytes(fakeClient, repoId, { key: "admissions/0000000000000005", object_kind: "head", sha256: "", size_bytes: "1" }), isKeyUnrecognized);
  });
});

// ─── vault-read-edge-cache design D5: edge_url wiring in the mirror path ──
//
// The mirror/recover reader (`readVaultObjectBytes`) reimplements its own
// `object-reads` call site rather than depending on `vault-publication.ts`
// (see `vault-mirror.ts`'s own header comment on why) — but for a
// non-generation-addressed (object-reads) entry it still routes its GET
// through the SAME `fetchVaultObjectBytes` (`vault-edge-fetch.ts`) the
...
// unit-tested directly against `fetchVaultObjectBytes` itself in
// `vault-edge-fetch.test.ts`.
describe("vault mirror reader — edge_url from object-reads reaches a non-generation-addressed read (vault-read-edge-cache design D5)", () => {
  ...
  it("prefers `edge_url` over `url` when the object-reads response carries one", async () => {
    ...
    const bytes = await readVaultObjectBytes(fakeClient, repoId, { key: `wal/${WAL_ID}.pack.enc`, object_kind: "wal_pack", sha256: "", size_bytes: "1" });
...
// ─── vault-small-object-inline: `inline` wiring in the mirror path ────────
//
...
// `vault-publication.ts` (this module's own header comment explains why),
...
describe("vault mirror reader — inline object-reads bytes (vault-small-object-inline)", () => {
  ...
  it("consumes `inline` when it hashes to the listing's own sha256 — no GET at all", async () => {
    ...
    const result = await readVaultObjectBytes(fakeClient, repoId, { key: `wal/${WAL_ID}.pack.enc`, object_kind: "wal_pack", sha256: sha256Hex(bytes), size_bytes: "3" });
  ...
  it("falls back to `url` when `inline` does not hash to the listing's own sha256 — a plain miss, byte-identical to the no-inline path", async () => {
    ...
    const result = await readVaultObjectBytes(fakeClient, repoId, { key: `wal/${WAL_ID}.pack.enc`, object_kind: "wal_pack", sha256: sha256Hex(real), size_bytes: "3" });
  ...
  it("an entry with no `inline` at all is untouched — byte-identical to before this change", async () => {
    ...
    const result = await readVaultObjectBytes(fakeClient, repoId, { key: `wal/${WAL_ID}.pack.enc`, object_kind: "wal_pack", sha256: sha256Hex(bytes), size_bytes: "2" });
    assert.deepEqual([...result!], [...bytes]);
  });
});
...
// regex — see `NON_GENERATION_KEY_SPECS`'s doc comment in vault-mirror.ts).
describe("vault mirror objects listing — source/<repo_id>/ prefix handling (task 5.3)", () => {
  ...
  it("strips the exact source/<repo_id>/ prefix, leaving a repo-relative key", async () => {
    ...
    const entries = await listVaultObjectsAll(client, REPO_ID);
  ...
  it("refuses (never silently drops or accepts) a listed key addressed to a DIFFERENT repo id", async () => {
    ...
    await assert.rejects(listVaultObjectsAll(client, REPO_ID), (e: unknown) => (e as { code?: string }).code === "VAULT_MIRROR_FOREIGN_REPO_KEY");
  });
  ...
  it("refuses a listed key with no source/<repo_id>/ prefix at all", async () => {
    ...
    await assert.rejects(listVaultObjectsAll(client, REPO_ID), (e: unknown) => (e as { code?: string }).code === "VAULT_MIRROR_LISTING_INCONSISTENT");
  });
});

// ─── Group 3 — recovery engine (task 3.6) ─────────────────────────────────────

describe("vault recovery engine (task 3.6)", () => {
  it("golden-vault fixture: recovered byte-exact — refs, HEAD, and generation all match the live vault", async () => {
    ...
    const result = await recoverVaultMirror({ backend, out_dir: outDir, keystore: f.keystore });
  ...
  it("recover creates a non-pre-existing --out directory (the CLI's own documented usage) instead of crashing GIT_UNAVAILABLE", async () => {
    ...
    // mkdtempSync'd itself, matching `run402 repos recover s3://… --out
  ...
  it("keyless mode never touches key material and still reports a recoverable generation + inventory", async () => {
    ...
    const bareKeystore = VaultKeystore.open({ rootDir: bareKeystoreRoot });
    ...
    const report = await verifyVaultMirror(backend, { keystore: bareKeystore, recovery_receipt: receipt! });
  ...
  it("pruned-object adjudication: an absence covered by a stored prune_intent is named 'intentionally_pruned', never treated as a chain-verified gap", async () => {
    ...
    const required = [
      { key: vaultPaths.wal("wal_" + "b".repeat(32)), object_id: "wal_" + "b".repeat(32), kind: "wal_pack" as const },
      { key: vaultPaths.wal("wal_" + "c".repeat(32)), object_id: "wal_" + "c".repeat(32), kind: "wal_pack" as const },
    ];
  ...
  it("unexplained-absence fallback: a genuinely missing object with NO covering prune intent falls back one generation and NAMES the loss — never a silent skip", async () => {
    ...
    const gen2WalKey = vaultPaths.wal(gen2WalId);
  ...
  it("recoverVaultMirror installs a retained ref for a branch-unreachable, force-displaced tip; git fsck is silent", async () => {
    const f = await makeVault();
    const c1 = await commitFile(f.repoDir, "a.txt", "a\n");
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
...
describe("vault mirror writer + sync engine (task 2.5)", () => {
  it("planMirrorWrite: every plain object precedes every admission/head; admissions/heads are strictly ascending with admission before its own head", () => {
    const entries: VaultObjectEntry[] = [
  ...
  it("torn-mirror property: interrupted at every write boundary recovers at exactly the expected earlier generation", async () => {
    ...
    const entries = transportEntries(f.transport, f.repoId);
    const listing: VaultObjectEntry[] = entries.map((e) => ({ key: e.key, object_kind: "x", sha256: sha256Hex(e.bytes), size_bytes: String(e.bytes.length) }));
  ...
  it("mirrorPushForGeneration NEVER throws — a broken backend surfaces as outcome:'failed', isolated from any caller's own control flow (design D6)", async () => {
    ...
    const brokenBackend: VaultMirrorBackend = {
...
// NORM on a multi-recipient vault (vault-human-envelopes), never a
...
describe("vault mirror sync — foreign-recipient key_envelope handling (task 5.3 follow-up)", () => {
  ...
  it("recoverVaultMirror decrypts using the OWN envelope even when a foreign one is absent from the mirror entirely", async () => {
    const f = await makeVault();
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: await commitFile(f.repoDir, "a.txt", "a\n"), force: false }] } });
...
function keyForRead(read: Record<string, string>): string {
  ...
  if (read.object_kind === "wal_pack") return vaultPaths.wal(id);
  if (read.object_kind === "ref_state") return vaultPaths.refState(id);
  if (read.object_kind === "retention_roots") return vaultPaths.retentionRoots(id);
  if (read.object_kind === "checkpoint_manifest") return vaultPaths.checkpointManifest(id);
  if (read.object_kind === "checkpoint_pack") return vaultPaths.checkpointPack(id);
  if (read.object_kind === "checkpoint_claim_set") return vaultPaths.claimSet(id);
  throw new Error(`unhandled object_kind ${read.object_kind}`);
}

// ─── vault-recovery-custody — the human-member path (bundle + code) ────────
//
...
// `vault-member-bundle.test.ts` proves this seal shape matches the
...
describe("vault recovery — member bundle + source recovery code (vault-recovery-custody)", () => {
  ...
  async function sealForTest(scalarHex: string, context: Uint8Array): Promise<string> {
    ...
    const { hexToBytes, toBase64url, randomBytes } = await import("../namespaces/vault.crypto.js");
  ...
  async function memberBundleFor(keystore: VaultKeystore, rpId = "console.run402.com"): Promise<Record<string, unknown>> {
    const { buildSourceWrapperContext, fromBase64url, sha256Hex: hex } = await import("../namespaces/vault.crypto.js");
  ...
  it("full recovery from mirror sidecar + code + receipt pin, no keystore identity — same repository a keystore recovery yields", async () => {
    ...
    const receipt = f.keystore.readRecoveryReceipt(f.repoId);
    assert.ok(receipt);
    const bareKeystore = new VaultKeystore({ rootDir: scratchDir("run402-bare-member-ks-") });
    ...
    const result = await recoverVaultMirror({
  ...
  it("code alone is truthfully insufficient: no bundle anywhere → RECOVERY_BUNDLE_MISSING names the absent input", async () => {
    ...
    const receipt = f.keystore.readRecoveryReceipt(f.repoId)!;
    const bareKeystore = new VaultKeystore({ rootDir: scratchDir("run402-bare-code-only-ks-") });
    await assert.rejects(
      recoverVaultMirror({ backend, out_dir: scratchDir("run402-recover-code-only-"), keystore: bareKeystore, recovery_receipt: receipt, source_recovery_code: MEMBER_CODE_DISPLAY }),
  ...
  it("substituted mirror still refused for human holders: bundle + code never override the recovery-receipt pin", async () => {
    ...
    const bareKeystore = new VaultKeystore({ rootDir: scratchDir("run402-bare-substituted-ks-") });
    await assert.rejects(
      recoverVaultMirror({ backend, out_dir: scratchDir("run402-recover-substituted-"), keystore: bareKeystore, recovery_receipt: foreignReceipt, source_recovery_code: MEMBER_CODE_DISPLAY }),
      (e: unknown) => (e as { code?: string }).code === "VAULT_CREATION_CONFLICT",
    );
    rmSync(mirrorRoot, { recursive: true, force: true });
  });
  ...
  it("a bundle without its code refuses by name (the bundle is ciphertext)", async () => {
    ...
    await assert.rejects(
      recoverVaultMirror({
        ...
        keystore: new VaultKeystore({ rootDir: scratchDir("run402-bare-no-code-ks-") }),
  ...
  it("keyless verify reports the sidecar as an unverified availability hint", async () => {
    ...
    const report = await verifyVaultMirror(backend, { recovery_receipt: f.keystore.readRecoveryReceipt(f.repoId)! });
    assert.equal(report.mode, "keyless_verify");
