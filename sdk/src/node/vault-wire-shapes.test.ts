/**
 * Wire-shape reconciliation (add-vault task 5.6c).
 *
...
import { createVaultHttpTransport, vaultLedgerId, vaultManifestEntry, vaultPaths, vaultWireRefForPath } from "./vault-publication.js";
import type { VaultUploadObject } from "./vault-publication.js";
import { checkActivationTokenBinding } from "./vault-deploy.js";
import { _resetVaultEdgeFetchStateForTest } from "./vault-edge-fetch.js";
import { toBase64url, sha256Hex } from "../namespaces/vault.crypto.js";
import { ApiError } from "../errors.js";
import type { VaultActivationToken } from "../namespaces/vault.types.js";
import { VaultOpCounter, countingVaultTransport } from "./vault-transport-counter.test-helper.js";
...
describe("vaultWireRefForPath — every §3 storage path maps to a control-plane identity", () => {
  it("routes heads and admission records to their generation-addressed routes, NOT object-reads", () => {
    assert.deepEqual(vaultWireRefForPath(vaultPaths.head(GEN)), { kind: "head", generation: GEN });
    assert.deepEqual(vaultWireRefForPath(vaultPaths.admission(GEN)), { kind: "admission", generation: GEN });
  });

  it("maps every uploadable kind to its object_kind + object_id", () => {
    const cases: Array<[string, string, string]> = [
      [vaultPaths.wal(WAL), "wal_pack", WAL],
      [vaultPaths.refState(REFS), "ref_state", REFS],
      [vaultPaths.retentionRoots(RR), "retention_roots", RR],
      [vaultPaths.checkpointManifest(CHK), "checkpoint_manifest", CHK],
      [vaultPaths.checkpointPack(CKP), "checkpoint_pack", CKP],
      [vaultPaths.claimSet(CCS), "checkpoint_claim_set", CCS],
    ];
    for (const [path, kind, id] of cases) {
      assert.deepEqual(vaultWireRefForPath(path), { kind: "object", read: { object_kind: kind, object_id: id } }, path);
    }
  });

  it("maps a key envelope to epoch + recipient_fingerprint (it is path-addressed in storage, identity-addressed on the wire)", () => {
    assert.deepEqual(vaultWireRefForPath(`envelopes/${EPOCH}/${EK}`), {
      kind: "object",
  ...
  it("maps a recipient_pin_manifest to pin_manifest_version, NOT epoch/recipient_fingerprint (D197, rev 42 — the SECOND path-addressed, null-idScalar kind)", () => {
    ...
    assert.deepEqual(vaultWireRefForPath(vaultPaths.pinManifest(VERSION)), {
      kind: "object",
    ...
    // this test exists to catch. See Vault.readPinManifestObject's
    // doc comment (vault-publication.ts) for the local cache that lets
    // a keystore skip re-fetching a manifest it itself just published.
    const ref = vaultWireRefForPath(vaultPaths.pinManifest(VERSION));
  ...
  it("does not confuse `retention/<rr>.enc` with `retention/<id>.ticket.json` — the ticket has no wire identity", () => {
    assert.deepEqual(vaultWireRefForPath(vaultPaths.retentionRoots(RR)), { kind: "object", read: { object_kind: "retention_roots", object_id: RR } });
    assert.equal(vaultWireRefForPath(vaultPaths.cutoffTicket(RR)), null, "a cutoff ticket is held locally; it is not a readable vault object");
  });
  ...
  it("returns null for anything not a §3 path, rather than guessing a kind", () => {
    for (const path of ["", "head/", "head/zzzz", "wal/wal_short.pack.enc", "checkpoints/nope.enc", "../etc/passwd", `envelopes/${EPOCH}/not-a-fingerprint`]) {
      assert.equal(vaultWireRefForPath(path), null, path);
    }
  });
});

describe("vaultManifestEntry — the closed-key upload manifest", () => {
  const obj = (over: Partial<VaultUploadObject> = {}): VaultUploadObject => ({
    path: vaultPaths.refState(REFS),
  ...
  it("NEVER emits `path` — the control plane derives the bucket key and refuses an unexpected member", () => {
    const entry = vaultManifestEntry(obj());
  ...
  it("emits epoch + recipient_fingerprint for a key envelope, and no object_id", () => {
    const entry = vaultManifestEntry(obj({ path: `envelopes/${EPOCH}/${EK}`, object_kind: "key_envelope", object_id: null }));
  ...
  it("emits pin_manifest_version for a recipient_pin_manifest upload, and no object_id/epoch/recipient_fingerprint", () => {
    const VERSION = "0000000000000001";
    const entry = vaultManifestEntry(obj({ path: vaultPaths.pinManifest(VERSION), object_kind: "recipient_pin_manifest", object_id: null }));
  ...
  it("carries base_generation for a wal_pack — the ONLY receipt kind that has one (§4.1)", () => {
    const entry = vaultManifestEntry(obj({ path: vaultPaths.wal(WAL), object_kind: "wal_pack", object_id: WAL, base_generation: GEN }));
  ...
  it("refuses a wal_pack with no base_generation instead of letting the server reject the whole session", () => {
    assert.throws(
      () => vaultManifestEntry(obj({ path: vaultPaths.wal(WAL), object_kind: "wal_pack", object_id: WAL })),
      (e: unknown) => (e as { code?: string }).code === "VAULT_UPLOAD_SESSION_INVALID",
    );
  });

  it("refuses to build a manifest entry for a non-uploadable path (a head is admitted, never uploaded)", () => {
    assert.throws(
      () => vaultManifestEntry(obj({ path: vaultPaths.head(GEN), object_kind: "head", object_id: null })),
      (e: unknown) => (e as { code?: string }).code === "VAULT_UPLOAD_SESSION_INVALID",
    );
  });
});

describe("vaultLedgerId — the key both sides pair receipts on", () => {
  it("is the object id for id-addressed kinds", () => {
    assert.equal(vaultLedgerId({ object_kind: "ref_state", object_id: REFS }), REFS);
  });

  it("is the synthetic `key_envelope:<epoch>:<fingerprint>` for envelopes, matching the ledger's own key", () => {
    assert.equal(vaultLedgerId({ object_kind: "key_envelope", epoch: EPOCH, recipient_fingerprint: EK }), `key_envelope:${EPOCH}:${EK}`);
  });

  it("round-trips through the path resolver, so an upload and its receipt pair on the same key", () => {
    for (const path of [vaultPaths.wal(WAL), vaultPaths.claimSet(CCS), `envelopes/${EPOCH}/${EK}`]) {
      const ref = vaultWireRefForPath(path);
      assert.ok(ref && ref.kind === "object", path);
      assert.equal(typeof vaultLedgerId(ref.read), "string");
    }
  });
});
 ...
 *   });
 *
 * (gateway `routes/vault-admission.ts`, `POST …/activation-tokens` — §6.5.)
 *
 * The envelope's SIBLING `object_id` is what makes this so quiet: the shape
 * looks close enough to a token to pass a glance, and `VaultActivationToken`
 * is a plain interface, so `client.request<VaultActivationToken>` asserts
 ...
 */
describe("createVaultHttpTransport — head-reads is a POST batch whose bytes stay untrusted (vault-batched-head-reads)", () => {
  ...
  function transportOver(handler: (call: WireCall) => unknown): { transport: ReturnType<typeof createVaultHttpTransport>; calls: WireCall[] } {
    ...
    } as unknown as Parameters<typeof createVaultHttpTransport>[0];
    return { transport: createVaultHttpTransport(client), calls };
  }
  ...
  it("POSTs `{generations}` to …/head-reads and returns the decoded bytes in request order", async () => {
    ...
    assert.deepEqual(calls, [{ path: `/vaults/v1/${REPO}/head-reads`, method: "POST", body: { generations: [G(1), G(2)] } }]);
  ...
  it("reads any failure as UNSUPPORTED, and remembers only a route-absent 404", async () => {
    ...
    const notFound = new ApiError("no such route", 404, {}, "reading vault heads in a batch");
    const serverError = new ApiError("boom", 500, {}, "reading vault heads in a batch");
...
describe("createVaultHttpTransport — compaction headroom grant open/close (vault-checkpoint-cadence design D3)", () => {
  ...
  it("opens with a bare POST …/compaction-grant and returns the grant verbatim", async () => {
    ...
    assert.deepEqual(calls, [{ path: `/vaults/v1/${REPO}/compaction-grant`, method: "POST", body: undefined }]);
  ...
  it("closes with a bare DELETE …/compaction-grant and normalizes `closed` to a strict boolean", async () => {
    ...
    assert.deepEqual(calls, [{ path: `/vaults/v1/${REPO}/compaction-grant`, method: "DELETE", body: undefined }]);
  ...
  it("propagates a 409 VAULT_COMPACTION_GRANT_ACTIVE / 404 route-absent refusal verbatim — the namespace layer, not the transport, interprets them", async () => {
    const conflict = { status: 409, code: "VAULT_COMPACTION_GRANT_ACTIVE", details: { expires_at: "2026-01-01T00:00:00.000Z" } };
    const { transport } = transportOver(() => {
      throw conflict;
    });
    await assert.rejects(transport.openCompactionGrant({ repo_id: REPO }), (e: unknown) => (e as { code?: string }).code === "VAULT_COMPACTION_GRANT_ACTIVE");
 ...
 * collapses every real refusal to the opaque `VAULT_PRUNE_SUBMIT_FAILED
 * {details: null}` fallback.
 */
describe("createVaultHttpTransport — submitPruneIntent surfaces the gateway's refusal verbatim (kychee-com/run402#578 fix 1)", () => {
  const REPO = `r402s_${"c".repeat(32)}`;

  function transportOverRawFetch(status: number, body: unknown): { transport: ReturnType<typeof createVaultHttpTransport> } {
    const client = {
      apiBase: "https://api.example.test",
    ...
    return { transport: createVaultHttpTransport(client) };
  }
  ...
  it("rides the gateway's FLAT envelope through verbatim — code, message, details, trace_id — never the opaque fallback", async () => {
    ...
    // (`services/vault/prune.ts`: `VaultRefusal("UPGRADE_REQUIRED", "a
    ...
    await assert.rejects(
      ...
      (e: unknown) => {
        ...
        assert.notEqual(err.code, "VAULT_PRUNE_SUBMIT_FAILED");
  ...
  it("falls back to the opaque VAULT_PRUNE_SUBMIT_FAILED only when the body carries no usable code/message at all", async () => {
    const { transport } = transportOverRawFetch(500, { some: "unrelated shape" });
    await assert.rejects(
      transport.submitPruneIntent({ repo_id: REPO, intent_bytes: new TextEncoder().encode("{}") }),
      (e: unknown) => {
        const err = e as { code?: string; message?: string; details?: Record<string, unknown> };
        assert.equal(err.code, "VAULT_PRUNE_SUBMIT_FAILED");
...
describe("createVaultHttpTransport — the mint's envelope is unwrapped, never passed on as the token", () => {
  ...
  const token = (): VaultActivationToken => ({
    format: "r402s/v0",
  ...
  const expected = (t: VaultActivationToken) => ({
    repo_id: t.repo_id,
  ...
  function transportReturning(body: unknown): { transport: ReturnType<typeof createVaultHttpTransport>; calls: Call[] } {
    const calls: Call[] = [];
    const client = {
      apiBase: "https://api.example.test",
      async request<T>(path: string, opts: { method?: string; body?: unknown }): Promise<T> {
  ...
  it("unwraps `activation_token` — the envelope the shipped route actually returns", async () => {
    ...
    assert.deepEqual(calls, [{
      path: `/vaults/v1/${REPO}/activation-tokens`,
      method: "POST",
  ...
  it("the unwrapped token satisfies the binding check — the envelope fails ALL NINE fields", () => {
    ...
    // and the commit is never attempted (`VAULT_TOKEN_BINDING_MISMATCH`).
    const envelope = { activation_token: t, object_id: t.object_id, reissued: false } as unknown as VaultActivationToken;
    assert.deepEqual(
...
// ─── RUN402_VAULT_TRACE=1 (design D7) ──────────────────────────────────────

describe("createVaultHttpTransport — RUN402_VAULT_TRACE debug trace", () => {
  const REPO = `r402s_${"a".repeat(32)}`;

  function fakeVaultRecordClient(): Parameters<typeof createVaultHttpTransport>[0] {
    return {
      apiBase: "https://api.example.test",
  ...
  it("is silent by default — no RUN402_VAULT_TRACE, no stderr writes", async (t) => {
    const original = process.env.RUN402_VAULT_TRACE;
    delete process.env.RUN402_VAULT_TRACE;
    t.after(() => { if (original === undefined) delete process.env.RUN402_VAULT_TRACE; else process.env.RUN402_VAULT_TRACE = original; });
    ...
    const transport = createVaultHttpTransport(fakeVaultRecordClient());
    await transport.getVaultRecord({ repo_id: REPO });
    assert.deepEqual(writes.filter((w) => w.includes("vault-trace")), []);
  });

  it("RUN402_VAULT_TRACE=1 prints one stderr line per operation, naming the op kind", async (t) => {
    const original = process.env.RUN402_VAULT_TRACE;
    process.env.RUN402_VAULT_TRACE = "1";
    t.after(() => { if (original === undefined) delete process.env.RUN402_VAULT_TRACE; else process.env.RUN402_VAULT_TRACE = original; });
    ...
    const transport = createVaultHttpTransport(fakeVaultRecordClient());
    await transport.getVaultRecord({ repo_id: REPO });
    const traceLines = writes.filter((w) => w.includes("vault-trace"));
    assert.equal(traceLines.length, 1, `expected exactly one trace line, saw: ${JSON.stringify(writes)}`);
    assert.match(traceLines[0]!, /vault-trace: getVaultRecord/);
...
// ─── vault-read-edge-cache design D5: edge_url wiring through the real routes ──

/**
 * `fetchVaultObjectBytes` (`vault-edge-fetch.ts`) itself is unit-tested
 * against a bare `{fetch}` fake in `vault-edge-fetch.test.ts` — every
 ...
 */
describe("createVaultHttpTransport — edge_url from the wire response reaches every read (vault-read-edge-cache design D5)", () => {
  const REPO = `r402s_${"a".repeat(32)}`;
  const WAL_ID = `wal_${"9".repeat(32)}`;
  const REFS_ID = `refs_${"8".repeat(32)}`;

  beforeEach(() => {
    _resetVaultEdgeFetchStateForTest();
  });

  function fakeClient(opts: { requestResponses: Record<string, unknown>; fetchHandlers: Record<string, () => Response> }): {
    client: Parameters<typeof createVaultHttpTransport>[0];
    fetchCalls: string[];
  } {
  ...
  it("getObject prefers the object-reads response's `edge_url` over its `url`", async () => {
    const path = vaultPaths.wal(WAL_ID);
    const { client, fetchCalls } = fakeClient({
      requestResponses: {
        [`/vaults/v1/${REPO}/object-reads`]: {
    ...
    const transport = createVaultHttpTransport(client);
  ...
  it("getObjects (batch) prefers each target's own `edge_url` independently", async () => {
    const walPath = vaultPaths.wal(WAL_ID);
    const refsPath = vaultPaths.refState(REFS_ID);
    const { client, fetchCalls } = fakeClient({
      requestResponses: {
        [`/vaults/v1/${REPO}/object-reads`]: {
          reads: [
  ...
  it("getState carries `since` on the wire and decodes the delta — inline packs only, refs dropped (vault-delta-fetch)", async () => {
    const headBytes = new Uint8Array([9, 9]);
    const packBytes = new Uint8Array([1, 2, 3]);
    const { client } = fakeClient({
      requestResponses: {
        [`/vaults/v1/${REPO}/state?since=0000000000000001`]: {
          vault: { repo_id: REPO, project_id: "prj_1", org_id: "org_1" },
  ...
  it("getState sends `restore=1` on the wire and decodes `restore_plan` — inline packs consumed, url-only packs fetched (vault-restore-recipe)", async () => {
    ...
    const { client, fetchCalls } = fakeClient({
      requestResponses: {
        [`/vaults/v1/${REPO}/state?restore=1`]: {
          vault: { repo_id: REPO, project_id: "prj_1", org_id: "org_1" },
          newest_generation: "0000000000000002",
  ...
  it("getState combines `since` and `restore` on the wire — orthogonal query params (vault-restore-recipe design D2)", async () => {
    const { client } = fakeClient({
      requestResponses: {
        [`/vaults/v1/${REPO}/state?since=0000000000000001&restore=1`]: {
          vault: { repo_id: REPO, project_id: "prj_1", org_id: "org_1" },
          newest_generation: "0000000000000001",
          head: { stored_bytes: toBase64url(new Uint8Array([9])), stored_bytes_sha256: "h" },
          carriers: { ref_state: { inline: toBase64url(new Uint8Array([8])) }, retention_roots: { inline: toBase64url(new Uint8Array([7])) } },
  ...
  it("getState drops the whole restore_plan on a structurally malformed head — never a partial/pretend plan (vault-restore-recipe)", async () => {
    ...
    // hash/chain/signature verification is `Vault`'s own job,
...
// ─── vault-object-host-predial task 1.2: origin observation ──────────────
//
...
describe("createVaultHttpTransport — onObjectStoreOriginObserved (vault-object-host-predial task 1.2)", () => {
  const REPO = `r402s_${"c".repeat(32)}`;
  ...
  function fakeClientWithFetchStatus(opts: { requestResponses: Record<string, unknown>; fetchHandlers: Record<string, () => Response | Promise<Response>> }): Parameters<typeof createVaultHttpTransport>[0] {
    return {
      apiBase: "https://api.example.test",
      async request<T>(path: string): Promise<T> {
        if (!(path in opts.requestResponses)) throw new Error(`unexpected request: ${path}`);
  ...
  it("reports the bare origin of `url` — never the full presigned URL (path/query/signature stripped)", async () => {
    ...
    const client = fakeClientWithFetchStatus({
      requestResponses: { [`/vaults/v1/${REPO}/object-reads`]: { reads: [{ object_kind: "wal_pack", object_id: WAL_ID, url: "https://bucket.s3.amazonaws.com/source/repo/wal/x?X-Amz-Signature=deadbeef&X-Amz-Expires=900", stored_bytes_sha256: "x", size_bytes: "1" }] } },
    ...
    const transport = createVaultHttpTransport(client, { onObjectStoreOriginObserved: (repoId, origins) => observed.push([repoId, origins]) });
  ...
  it("reports BOTH the `url` and `edge_url` origins on a successful edge fetch — the origin not actually dialed is still worth a future predial", async () => {
    const path = vaultPaths.wal(WAL_ID);
    const client = fakeClientWithFetchStatus({
      requestResponses: { [`/vaults/v1/${REPO}/object-reads`]: { reads: [{ object_kind: "wal_pack", object_id: WAL_ID, url: "https://bucket.s3.amazonaws.com/x", edge_url: "https://cdn.run402.com/x", stored_bytes_sha256: "x", size_bytes: "1" }] } },
  ...
  it("still fires on a 404 (a completed round trip proving the origin is reachable, even though the object itself is absent)", async () => {
    const path = vaultPaths.wal(WAL_ID);
    const client = fakeClientWithFetchStatus({
      requestResponses: { [`/vaults/v1/${REPO}/object-reads`]: { reads: [{ object_kind: "wal_pack", object_id: WAL_ID, url: "https://bucket.s3.amazonaws.com/x", stored_bytes_sha256: "x", size_bytes: "1" }] } },
      fetchHandlers: { "https://bucket.s3.amazonaws.com/x": () => new Response(null, { status: 404 }) },
    });
    const observed: string[][] = [];
    const transport = createVaultHttpTransport(client, { onObjectStoreOriginObserved: (_repoId, origins) => observed.push(origins) });
    const bytes = await transport.getObject({ repo_id: REPO, path });
  ...
  it("never fires for an `inline`-satisfied read — no URL was dialed", async () => {
    const path = vaultPaths.wal(WAL_ID);
    const client = fakeClientWithFetchStatus({
      requestResponses: { [`/vaults/v1/${REPO}/object-reads`]: { reads: [{ object_kind: "wal_pack", object_id: WAL_ID, url: "https://bucket.s3.amazonaws.com/x", inline: toBase64url(new Uint8Array([1, 2, 3])), stored_bytes_sha256: "x", size_bytes: "1" }] } },
      fetchHandlers: {},
    });
    const observed: string[][] = [];
    const transport = createVaultHttpTransport(client, { onObjectStoreOriginObserved: (_repoId, origins) => observed.push(origins) });
...
// ─── vault-small-object-inline design D1/D2/D3: `inline` on object-reads ──
//
...
describe("createVaultHttpTransport — inline object-reads bytes (vault-small-object-inline)", () => {
  ...
  it("getObject consumes `inline` with no expectation supplied — no GET at all, an entry without inline is untouched", async () => {
    const claimPath = vaultPaths.claimSet(CCS_ID);
  ...
  it("counted transport-operation budgets are structurally unchanged by a gateway that inlines (design D4)", async () => {
    ...
    const counterA = new VaultOpCounter();
    const transportA = countingVaultTransport(createVaultHttpTransport(notInlining.client), counterA);
    ...
    const counterB = new VaultOpCounter();
    const transportB = countingVaultTransport(createVaultHttpTransport(inlining.client), counterB);
