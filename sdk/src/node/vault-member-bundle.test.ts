/**
 * vault-recovery-custody — the member recovery bundle open path.
 *
...
import {
  VAULT_MEMBER_BUNDLE_FORMAT,
  VAULT_MEMBER_BUNDLE_MIRROR_PREFIX,
...
function goldenBundle(overrides: Partial<VaultMemberRecoveryBundle> = {}): VaultMemberRecoveryBundle {
  return {
    format: VAULT_MEMBER_BUNDLE_FORMAT,
...
describe("discoverMemberBundles (mirror sidecars — availability hints)", () => {
  function memoryBackend(files: Record<string, string>): VaultMirrorBackend {
    return {
      describe: () => "memory",
  ...
  it("reports parsable sidecars with identity hints and malformed ones with their error — never a silent skip", async () => {
    const backend = memoryBackend({
      [`${VAULT_MEMBER_BUNDLE_MIRROR_PREFIX}tal.json`]: JSON.stringify(goldenBundle()),
      [`${VAULT_MEMBER_BUNDLE_MIRROR_PREFIX}broken.json`]: "{not json",
