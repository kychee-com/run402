/**
 * vault conformance-vector loading (add-vault task 5.6b).
 *
 * WHY THIS FILE EXISTS. Every vault suite used to resolve the vector set
 * through `process.env.VAULT_VECTORS_DIR ?? "<a hard-coded absolute path on
 ...
 *   - the only way to skip is the explicit `VAULT_VECTORS_OPTOUT=1`;
 ...
 * pinned protocol revision. A location override (`VAULT_VECTORS_DIR`) is
...
export const OPTOUT_ENV = "VAULT_VECTORS_OPTOUT";
/** Point the suites at a different copy. The integrity assertions still run. */
export const DIR_ENV = "VAULT_VECTORS_DIR";

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export type VaultVector = { id: string; class: string; description: string; inputs: any; expected: any; reject_reason?: string; reject_code?: string; schema?: string };

export interface VaultVectorFile {
  vectors: VaultVector[];
...
export interface VaultVectorSet {
  dir: string;
  file: VaultVectorFile;
  /** `hpke-interop/golden.json`, parsed. */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  golden: any;
  revision: string;
  byClass(cls: string): VaultVector[];
}
...
function resolveDir(): string {
  ...
  throw new Error(
    `vault conformance vectors are missing.\n` +
...
/**
 * Load the vector set, or `null` when `VAULT_VECTORS_OPTOUT=1`.
 *
 ...
 */
export function loadVaultVectors(): VaultVectorSet | null {
  if (process.env[OPTOUT_ENV] === "1") return null;
  const dir = resolveDir();
  const { revision } = assertVectorContinuity(dir);
  const file = JSON.parse(readFileSync(join(dir, "vectors.json"), "utf8")) as VaultVectorFile;
