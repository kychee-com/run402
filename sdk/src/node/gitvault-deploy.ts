...
export async function runGitvaultDeploy(options: GitvaultDeployOptions): Promise<GitvaultDeployResult> {
  ...
  if (!pub) {
    ...
    if (!planned) {
      return { ...common, outcome: "DEPLOY_FAILED_UNVAULTED", push_error, deploy_error: summarize(planError), next_actions: [{ action: "fix the build, then redeploy" }, { action: "retry the push (run402 repos snapshot)" }] };
    }
    if (options.allow_unvaulted) {
      ...
      try {
        ...
        return { ...common, outcome: "DEPLOY_FAILED_UNVAULTED", push_error, deploy_error: summarize(e), next_actions: [{ action: "retry the push (run402 repos snapshot) — the override journal is kept and will drain" }, { action: "retry the deploy" }] };
      }
