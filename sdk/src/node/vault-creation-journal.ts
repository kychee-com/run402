/**
 * vault — the six-stage crash-safe creation journal (protocol rev 41 §5.2;
 * task 5.3).
 *
 ...
 *
 * The control-plane calls ride an injectable {@link VaultCreationTransport}
...
export const VAULT_CREATION_STAGES = [
  "LOCAL_KEYS_PREPARED",
  "ALLOCATED",
  "OBJECTS_PREPARED",
  "OBJECTS_FINALIZED",
...
export type VaultCreationStage = (typeof VAULT_CREATION_STAGES)[number];

/** One prepared object: exact bytes + identity, durable before any PUT. */
export interface VaultJournaledObject {
  ...
  receipt: VaultKeyEnvelopeReceipt;
 ...
 * project atomically inside the SAME allocate call — see routes/repos.ts's
 * `resolveAllocationAuthz` in run402-private) — see `VaultCreationJournal`'s
 * doc comment on why `org_id`/`project_id` are nullable.
 */
export interface VaultPushToCreateAddress {
  org_slug: string;
  repo_name: string;
}
 ...
 */
export interface VaultCreationJournal {
  version: 1;
  ...
  push_to_create: VaultPushToCreateAddress | null;
  /** vault-byo-primary-bucket task 3.1 — `"byo"` when this journal requested a BYO vault at open time; `null` (managed, byte-identical to today) otherwise. Set once at open, never changes across the journal's life (mirrors `push_to_create`'s own resumability-key discipline). */
   ...
   * `pushToCreateVault` needs to resolve to it and proceed as an ordinary
...
/**
 * `POST /vaults/v1` — the allocate REQUEST.
 *
 ...
 */
export type VaultAllocateRequest =
  | {
      client_creation_id: string;
      org_id: string;
      project_id: string;
      ...
      /** vault-byo-primary-bucket task 3.1/3.5 — omitted/absent is byte-identical to today ("managed"). Present only to request a BYO vault; `byo_destination` is the address only, never credential material (D2). */
      storage_profile?: "byo";
  ...
  | {
       ...
       * (`POST /vaults/v1`, same route, alternate body shape).
       */
      client_creation_id: string;
      org_slug: string;
      repo_name: string;
...
export interface VaultPutObjectRequest {
  repo_id: string;
  path: string;
  bytes: Uint8Array;
  ...
  /** vault-byo-primary-bucket task 3.2 — present iff this vault is `storage_profile: "byo"`; the object is written directly to `byo.destination` (never run402's own bucket). Omitted is byte-identical to today. */
  byo?: { destination: import("./vault-mirror-config.js").VaultMirrorDestination; credential?: import("./vault-mirror-config.js").VaultMirrorCredential };
}

export interface VaultObjectReceipt {
  stored_bytes_sha256: string;
  size_bytes: string;
}

export interface VaultAdmitGenesisRequest {
  repo_id: string;
...
export type VaultAdmitGenesisResult =
...
export interface VaultCreationTransport {
  ...
  admitGenesis(request: VaultAdmitGenesisRequest): Promise<VaultAdmitGenesisResult>;
...
export interface VaultCreationOptions {
  ...
  transport: VaultCreationTransport;
  /**
  ...
  push_to_create?: VaultPushToCreateAddress;
  /** Resume an existing journal, or pin the idempotency key (tests). Fresh CSPRNG when omitted. */
  client_creation_id?: string;
  /**
   * vault-byo-primary-bucket task 3.1/3.5 — request a BYO vault at
   ...
   * ({@link ../node/vault-byo-probe.js probeVaultByoDestination})
  ...
  byo_destination?: string;
  /**
   * vault-byo-primary-bucket task 3.1/3.2 — the LOCAL write target that
   ...
   * bypassing `Vault.uploadAll`'s resolver since no `Vault`
   ...
   * run402's own bucket.
   */
  byo_write_target?: { destination: import("./vault-mirror-config.js").VaultMirrorDestination; credential?: import("./vault-mirror-config.js").VaultMirrorCredential };
  /** Clock injection. */
  now?: () => Date;
  ...
  onStage?: (stage: VaultCreationStage, journal: VaultCreationJournal) => void | Promise<void>;
}
...
export function vaultJournalPath(keystore: VaultKeystore, clientCreationId: string): string {
  if (!/^[0-9a-f]{32}$/.test(clientCreationId)) fail("VAULT_BAD_ID", "client_creation_id must be 32 lowercase hex", "resolving vault journal path");
  return join(keystore.journalDir, `${clientCreationId}.json`);
}

export function readVaultJournal(keystore: VaultKeystore, clientCreationId: string): VaultCreationJournal | null {
  const text = readFileNoFollow(vaultJournalPath(keystore, clientCreationId));
  if (!text) return null;
  const journal = JSON.parse(text) as VaultCreationJournal;
  if (journal.version !== 1 || !VAULT_CREATION_STAGES.includes(journal.stage)) {
    fail("VAULT_JOURNAL_CORRUPT", "creation journal is not a version-1 vault journal", "reading vault creation journal", { client_creation_id: clientCreationId });
  }
  return journal;
}

/** Every journal on disk that has not reached ACTIVE (what `doctor` lists as "creation in progress"). */
export function listIncompleteVaultJournals(keystore: VaultKeystore): VaultCreationJournal[] {
  if (!existsSync(keystore.journalDir)) return [];
  const out: VaultCreationJournal[] = [];
  for (const entry of readdirSync(keystore.journalDir)) {
    if (!entry.endsWith(".json")) continue;
    const id = entry.slice(0, -5);
    if (!/^[0-9a-f]{32}$/.test(id)) continue;
    const j = readVaultJournal(keystore, id);
 ...
 * skipped: `VaultCreation.run()` refuses to retry a refused journal
 ...
 */
export function findResumableVaultJournal(keystore: VaultKeystore, orgId: string, projectId: string): VaultCreationJournal | null {
  const candidates = listIncompleteVaultJournals(keystore).filter(
...
/**
 * The push-to-create sibling of {@link findResumableVaultJournal} (task
 ...
 * only reproduce the same refusal; `pushToCreateVault` starts a fresh
 ...
 */
export function findResumablePushToCreateJournal(keystore: VaultKeystore, orgSlug: string, repoName: string): VaultCreationJournal | null {
  const candidates = listIncompleteVaultJournals(keystore).filter(
...
export class VaultCreation {
  ...
  private readonly transport: VaultCreationTransport;
  private readonly now: () => Date;
  private readonly options: VaultCreationOptions;
  private journal: VaultCreationJournal;
  private resumed = false;

  private constructor(options: VaultCreationOptions, journal: VaultCreationJournal, resumed: boolean) {
   ...
   */
  static open(options: VaultCreationOptions): VaultCreation {
    ...
    if (usingPushToCreate === (options.org_id !== undefined || options.project_id !== undefined)) {
      fail("VAULT_BAD_ADDRESS", "pass exactly one of {org_id, project_id} or push_to_create, never both or neither", "opening vault creation");
    }
    ...
    if (!signing || !encryption) {
      fail("VAULT_READ_ONLY", "this principal cannot create a vault: a private key is missing from identity.json", "opening vault creation", { signing_key_present: Boolean(signing), encryption_key_present: Boolean(encryption) });
    }
    const id = options.client_creation_id ?? newHex32();
    const existing = readVaultJournal(ks, id);
    if (existing) {
      ...
      const mismatch = (usingPushToCreate
        ...
        // vault-byo-primary-bucket task 3.1: a resumed journal's own
      ...
      if (mismatch) {
        fail("VAULT_CREATION_CONFLICT", "journal belongs to a different org/project (or addressing form)", "resuming vault creation", { client_creation_id: id });
      }
      if (existing.creator_signing_fingerprint !== identity.signing_fingerprint || existing.creator_encryption_fingerprint !== identity.encryption_fingerprint) {
        fail("VAULT_CREATION_CONFLICT", "journal was prepared by a different identity; refusing the destructive retry", "resuming vault creation", { client_creation_id: id });
      }
      return new VaultCreation(options, existing, true);
    }
    const at = formatVaultTimestamp((options.now ?? (() => new Date()))());
    const journal: VaultCreationJournal = {
      version: 1,
    ...
    const c = new VaultCreation(options, journal, false);
    c.persist();
    return c;
  }

  get current(): VaultCreationJournal {
    return this.journal;
  }

  private persist(): void {
    this.journal = { ...this.journal, updated_at: formatVaultTimestamp(this.now()) };
    writeFileAtomic0600(vaultJournalPath(this.keystore, this.journal.client_creation_id), JSON.stringify(this.journal, null, 2));
  ...
  private async advance(stage: VaultCreationStage, patch: Partial<VaultCreationJournal>): Promise<void> {
  ...
  private refuse(code: "VAULT_CREATION_CONFLICT" | "ALLOCATION_SUPERSEDED" | "REPO_CREATION_CONFLICT", message: string, details?: Record<string, unknown>): never {
    this.journal = { ...this.journal, refusal: { code, at: formatVaultTimestamp(this.now()), ...(details ? { details } : {}) } };
    this.persist();
    fail(code, message, "reconciling vault creation", { client_creation_id: this.journal.client_creation_id, ...details });
  }
   ...
   */
  private async reconcileAllocateConflict(err: unknown): Promise<VaultCreationResult> {
    ...
    fail(
      ...
      "reconciling vault creation",
      { repo_id: repoId, org_id: this.journal.org_id, project_id: this.journal.project_id },
      [
        { action: "restore the vault keystore onto this machine (run402 repos view prints keystore.root; back it up from the machine that holds it)" },
        ...crossProfileVaultHint(repoId),
      ],
    );
  }
   ...
   */
  async run(): Promise<VaultCreationResult> {
    if (this.journal.refusal) {
      fail(this.journal.refusal.code, "this creation attempt was refused earlier; start a new attempt (the journal is kept for diagnosis)", "resuming vault creation", {
    ...
    if (this.journal.stage === "LOCAL_KEYS_PREPARED") {
      ...
      // the gateway derives from them. See VaultAllocateRequest.
      // vault-byo-primary-bucket task 3.1: the journal's own
      ...
      const request: VaultAllocateRequest = this.journal.push_to_create
        ? {
            client_creation_id: this.journal.client_creation_id,
      ...
      try {
        ...
        // refused with the winner's project_id so `pushToCreateVault` can
    ...
    const epoch = VAULT_GENESIS_EPOCH;
    ...
    if (this.journal.stage === "ALLOCATED") {
      ...
      const object: VaultJournaledObject = {
        object_kind: "key_envelope",
    ...
    if (this.journal.stage === "OBJECTS_PREPARED") {
      ...
      for (const object of objects) {
        ...
        const receipt = await this.transport.putObject({
          ...
          // vault-byo-primary-bucket task 3.1/3.2: the genesis
          ...
          // (`VaultCreationOptions`'s own doc comment) — a resume that
    ...
    if (this.journal.stage === "OBJECTS_FINALIZED") {
      ...
      const storedEnvelope = parseVaultStrict(new TextDecoder().decode(unb64u(envelopeObject.stored_bytes_b64u))) as VaultKeyEnvelope;
      ...
      await this.advance("GENESIS_PREPARED", { genesis, genesis_sha256: sha256Hex(storedBytes(genesis as unknown as VaultSignedObject)) });
    }
    ...
    if (this.journal.stage === "GENESIS_PREPARED") {
      ...
      const bytes = storedBytes(genesis as unknown as VaultSignedObject);
  ...
  private verifyAllocation(allocation: VaultAllocation): void {
    ...
    if (problems.length > 0) {
      fail("VAULT_ALLOCATION_INVALID", "allocation does not match this creation attempt", "verifying vault allocation", { problems });
    }
...
export async function createVault(options: VaultCreationOptions): Promise<VaultCreationResult> {
  return VaultCreation.open(options).run();
}

// ─── Doctor text ─────────────────────────────────────────────────────────────

/** What `doctor`/`status` print about V0-A recovery — the verbatim statement plus the pointers it must carry. */
export interface VaultDoctorRecoveryText {
  statement: typeof VAULT_TERMINAL_LOSS_STATEMENT;
  doctor_text: typeof VAULT_TERMINAL_LOSS_DOCTOR_TEXT;
...
export function vaultDoctorRecoveryText(): VaultDoctorRecoveryText {
  return {
    statement: VAULT_TERMINAL_LOSS_STATEMENT,
    doctor_text: VAULT_TERMINAL_LOSS_DOCTOR_TEXT,
