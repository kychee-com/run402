/**
 * vault epoch rotation — the READER half (D193-D203, rev 42, incident
 * follow-up to `vault-rotate.test.ts`'s producer-only coverage).
 *
 * The producer (`Vault.rotateEpoch`/`rotateEpochForKeyRevocation`,
 * `vault-rotate.test.ts`) shipped in 8ecd36c8 with no reader half: a
 ...
 * project's own `VaultMemoryTransport` fixture, then exercises the
 * reader from a genuinely SEPARATE keystore:
 *
 *   - clone/materialize (`Vault.materialize`) reads through a
 *     rotation it never produced;
 *   - a keystore with no envelope for the new epoch fails CLOSED with
 *     `VAULT_EPOCH_NOT_OPENABLE`, never a bare AEAD failure;
 *   - `verifyToNewest({decryptValidate, strict:false})` — what
 *     `Repos.fsck()` calls — reports `generation` (chain-verified) equal
 ...
 *   - `recoverVaultMirror` (the offline mirror path) traverses the SAME
...
import { ekFingerprint } from "../namespaces/vault.crypto.js";
import { VaultKeystore } from "./vault-keystore.js";
import { VaultMemoryTransport, commitFile, makeVault } from "./vault-memory-transport.test.js";
import { Vault } from "./vault-publication.js";
import { recoverVaultMirror, verifyVaultMirror } from "./vault-recover.js";
...
/**
 * `VaultMemoryTransport` stores uploads under the SDK's own client-local
 * `path` string, which for `key_envelope` differs from the gateway's real §3
 * key — mirrors `vault-mirror-recover.test.ts`'s own
...
/** Bootstrap a principal's pin (D197) — needed before any rotation can validly include them. Mirrors `vault-rotate.test.ts`'s own helper. */
async function bootstrapPin(vault: Vault, principalId: string, ekFp: string): Promise<void> {
 ...
 * a separate, unscoped follow-up — see `vault-rotate.test.ts`'s own
...
async function makeTwoRecipientRotatedVault(dir: string): Promise<TwoRecipientFixture> {
  ...
  const readerKeystore = VaultKeystore.open({ rootDir: join(dir, "ks-reader") });
...
describe("vault epoch rotation — reader traversal (Part A: clone/materialize)", () => {
  it("a keystore that did NOT drive the rotation opens the new epoch's own envelope and decrypts through it", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-");
    try {
      ...
      const readerVault = Vault.open({ keystore: f.readerKeystore, transport: f.transport, repo_id: f.repoId, repo_dir: null });
  ...
  it("chains through MULTIPLE sequential rotations", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-multi-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
  ...
  it("VAULT_EPOCH_NOT_OPENABLE: a keystore excluded from the rotation fails CLOSED, naming epoch/rotation/fingerprint — never a bare AEAD failure", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-excluded-");
    try {
      const { transport, vault, repoDir, repoId } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: repoId })).org_id;
      const c1 = await commitFile(repoDir, "a.txt", "a\n");
      ...
      const outsiderKeystore = VaultKeystore.open({ rootDir: join(dir, "ks-outsider") });
      ...
      const outsiderVault = Vault.open({ keystore: outsiderKeystore, transport, repo_id: repoId, repo_dir: null });
      await assert.rejects(
        ...
        (e: unknown) => {
          ...
          assert.equal(err.code, "VAULT_EPOCH_NOT_OPENABLE");
...
describe("vault epoch rotation — reader traversal (Part C: fsck's chain_verified_to vs decryptable_to)", () => {
  it("healthy vault: verifyToNewest({decryptValidate, strict:false}) reports chain_verified_to == decryptable_to, no failure", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-fsck-healthy-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
      const readerVault = Vault.open({ keystore: f.readerKeystore, transport: f.transport, repo_id: f.repoId, repo_dir: null });
  ...
  it("the incident's own shape: a keystore whose rotation envelope is absent reports chain_verified_to > decryptable_to, VAULT_EPOCH_NOT_OPENABLE named exactly", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-fsck-split-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
      const readerVault = Vault.open({ keystore: f.readerKeystore, transport: f.transport, repo_id: f.repoId, repo_dir: null });
      ...
      assert.equal(state.decrypt!.failure!.code, "VAULT_EPOCH_NOT_OPENABLE");
      ...
      await assert.rejects(readerVault.materialize(), (e: unknown) => (e as { code?: string }).code === "VAULT_EPOCH_NOT_OPENABLE");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
...
describe("vault epoch rotation — reader traversal (Part B: offline mirror recovery)", () => {
  // `recoverVaultMirror` derives `K_1` purely from `vault_genesis.envelopes[]`
  // (this SDK's genesis stays single-recipient — D198's N-recipient genesis
  // is a separate, unscoped follow-up, `vault-rotate.test.ts`'s own
  ...
  // exercises what THIS task fixed: `recoverVaultMirror` is a from-scratch
  ...
  it("recoverVaultMirror traverses a real rotation offline, using the mirrored rotation envelope", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-recover-");
    const mirrorRoot = scratchDir("run402-vault-epoch-reader-mirror-");
    const outDir = scratchDir("run402-vault-epoch-reader-out-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
      const backend = new DirectoryMirrorBackend(mirrorRoot);
      await seedBackend(backend, transportEntries(f.transport, f.repoId));

      const result = await recoverVaultMirror({ backend, out_dir: outDir, keystore: f.creatorVault.keystore });
  ...
  it("keyless verifyVaultMirror walks the FULL chain through the rotation (structural only) — the transition no longer stops it", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-mirror-verify-");
    const mirrorRoot = scratchDir("run402-vault-epoch-reader-mirror-verify-root-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
      const backend = new DirectoryMirrorBackend(mirrorRoot);
      await seedBackend(backend, transportEntries(f.transport, f.repoId));

      const report = await verifyVaultMirror(backend, { keystore: f.readerKeystore });
  ...
  it("synthetic missing-envelope: recover falls back at the EPOCH BOUNDARY (never a generic absence), naming the same VAULT_EPOCH_NOT_OPENABLE detail fsck would report", async () => {
    const dir = scratchDir("run402-vault-epoch-reader-recover-fallback-");
    const mirrorRoot = scratchDir("run402-vault-epoch-reader-mirror-fallback-");
    const outDir = scratchDir("run402-vault-epoch-reader-out-fallback-");
    try {
      const f = await makeTwoRecipientRotatedVault(dir);
      // Same genesis-envelope-resolution caveat as the healthy-path test
      // above: only the CREATOR can drive `recoverVaultMirror` at all in
      ...
      assert.equal(result.epoch_decrypt_failure!.code, "VAULT_EPOCH_NOT_OPENABLE");
