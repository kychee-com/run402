/**
 * vault — snapshot + hardened git execution (protocol rev 41 §6.6; task 5.5).
 *
...
import { VAULT_OID40_RE, deriveDigestKey, keyedCommitment, sha256Hex, snapshotOidContent } from "../namespaces/vault.crypto.js";
...
export const VAULT_MAX_GIT_OBJECT_BYTES = 200 * 1024 * 1024;
/** `refs/run402/deploys/latest` — the single protocol-owned deploy ref (round-3 H9). */
export const VAULT_DEPLOY_REF = "refs/run402/deploys/latest";
/** Probed git version range: `[2.32, ∞)` — `GIT_CONFIG_GLOBAL` landed in 2.32. */
export const VAULT_MIN_GIT_VERSION = [2, 32] as const;
...
function hooksPath(): string {
  if (!emptyHooksDir) {
    emptyHooksDir = mkdtempSync(join(tmpdir(), "run402-vault-nohooks-"));
  }
  return emptyHooksDir;
}

/** The environment every vault git process runs under — nothing inherited except PATH. */
export function hardenedGitEnv(extra: Record<string, string> = {}): Record<string, string> {
  const env: Record<string, string> = {
    ...
    GIT_AUTHOR_NAME: "run402 vault",
    GIT_AUTHOR_EMAIL: "vault@run402.com",
    GIT_COMMITTER_NAME: "run402 vault",
    GIT_COMMITTER_EMAIL: "vault@run402.com",
  };
 ...
 * vault git invocation that touches the network (`cloneVaultRemote`):
 * that clone spawns the `git-remote-run402` helper,
 ...
 * `VAULT_ACCESS_DENIED` "while resolving the project's vault" right
...
/** `git version` parsed as [major, minor]; refuses below {@link VAULT_MIN_GIT_VERSION}. */
export async function probeGitVersion(cwd: string = process.cwd()): Promise<[number, number]> {
  ...
  const [minMajor, minMinor] = VAULT_MIN_GIT_VERSION;
...
export type GlobalExcludesRefusal =
  | { code: "VAULT_CONFIG_INCLUDE_REFUSED"; file: string; key: string }
  | { code: "VAULT_CONFIG_UNREADABLE"; file: string }
  | { code: "VAULT_EXCLUDES_PATH_UNSUPPORTED"; file: string; value: string }
  | { code: "VAULT_EXCLUDES_UNREADABLE"; file: string };
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (header) {
      ...
      if (section === "include" || section === "includeif") {
        throw new LocalError(`${file}: [${section}] is not supported by the frozen ignore-authority discovery; remove it or set core.excludesFile directly`, "discovering global excludes", { code: "VAULT_CONFIG_INCLUDE_REFUSED", details: { file, key: section } });
      }
      continue;
    }
...
function expandConfigPath(value: string, home: string | undefined, file: string): string {
  if (value.startsWith("~/")) {
    if (!home) fail("VAULT_EXCLUDES_PATH_UNSUPPORTED", `${file}: core.excludesFile uses ~/ but HOME is unset`, "discovering global excludes", { file, value });
    return join(home, value.slice(2));
  }
  if (value.startsWith("$HOME/") || value === "$HOME") {
    if (!home) fail("VAULT_EXCLUDES_PATH_UNSUPPORTED", `${file}: core.excludesFile uses $HOME but HOME is unset`, "discovering global excludes", { file, value });
    return join(home, value.slice(5));
  }
  if (value.startsWith("/")) return value;
  fail("VAULT_EXCLUDES_PATH_UNSUPPORTED", `${file}: core.excludesFile "${value}" is relative or environment-dependent; only absolute, ~/, and $HOME/ paths are supported`, "discovering global excludes", { file, value });
}

function readAsData(file: string, code: "VAULT_CONFIG_UNREADABLE" | "VAULT_EXCLUDES_UNREADABLE", context: string): string | null {
  try {
...
export function discoverGlobalExcludes(env: GitConfigDiscoveryEnv = { HOME: process.env.HOME, XDG_CONFIG_HOME: process.env.XDG_CONFIG_HOME }): GlobalExcludesDiscovery {
  ...
  for (const file of configs) {
    const text = readAsData(file, "VAULT_CONFIG_UNREADABLE", "discovering global excludes");
  ...
  if (path) {
    patterns = readAsData(path, "VAULT_EXCLUDES_UNREADABLE", "reading global excludes as data");
...
export type VaultRepositoryRefusalCode =
  | "VAULT_BARE_REPOSITORY"
  | "VAULT_LINKED_WORKTREE"
  | "VAULT_SPARSE_CHECKOUT"
  | "VAULT_SHALLOW_CLONE"
  | "VAULT_SUBMODULES_UNSUPPORTED"
  | "GIT_OBJECT_FORMAT_UNSUPPORTED"
  | "VAULT_ALTERNATES_UNSUPPORTED"
  | "VAULT_GRAFTS_UNSUPPORTED"
  | "VAULT_CONFIG_INCLUDE_REFUSED"
  | "VAULT_NOT_A_REPOSITORY";

export interface VaultRepositoryInspection {
  top_level: string;
  git_dir: string;
  git_common_dir: string;
  object_format: "sha1";
  head: VaultHeadTarget & { resolved_oid: string | null };
...
export async function inspectRepository(dir: string): Promise<VaultRepositoryInspection> {
  ...
  try {
    ...
    fail("VAULT_NOT_A_REPOSITORY", `${abs} is not inside a git repository`, "inspecting repository", { dir: abs, cause: String((e as Error).message) });
  }
  if (bare === "true") fail("VAULT_BARE_REPOSITORY", "bare repositories are refused: a snapshot needs a work tree", "inspecting repository", { dir: abs });
  let top: string;
  try {
    top = await gitRevParse(abs, "--show-toplevel");
  } catch (e) {
    fail("VAULT_NOT_A_REPOSITORY", `${abs} is not inside a git work tree`, "inspecting repository", { dir: abs, cause: String((e as Error).message) });
  }
  const gitDir = resolve(top, await gitRevParse(top, "--git-dir"));
  const commonDir = resolve(top, await gitRevParse(top, "--git-common-dir"));
  if (gitDir !== commonDir) fail("VAULT_LINKED_WORKTREE", `linked worktrees are refused by name: ${top} is a linked worktree of ${commonDir}`, "inspecting repository", { worktree: top, git_dir: gitDir, git_common_dir: commonDir });
  if ((await gitRevParse(top, "--is-shallow-repository")) === "true") fail("VAULT_SHALLOW_CLONE", "shallow/partial clones are refused: the object set is incomplete", "inspecting repository", { dir: top });
  ...
  for (const [file, code, what] of [
    [join(commonDir, "objects", "info", "alternates"), "VAULT_ALTERNATES_UNSUPPORTED", "objects/info/alternates"],
    [join(commonDir, "info", "grafts"), "VAULT_GRAFTS_UNSUPPORTED", "info/grafts"],
  ] as const) {
  ...
  if (sparse.text().trim() === "true" || existsSync(join(gitDir, "info", "sparse-checkout"))) fail("VAULT_SPARSE_CHECKOUT", "sparse checkouts are refused: the work tree is not the whole tree", "inspecting repository", { dir: top });
  ...
  for (const name of cfg.lines()) {
    ...
    if (lower.startsWith("include.") || lower.startsWith("includeif.")) fail("VAULT_CONFIG_INCLUDE_REFUSED", `repository config carries ${name}; include/includeIf is refused by name`, "inspecting repository", { key: name });
  }
  // submodules: .gitmodules or gitlinks in the index
  if (existsSync(join(top, ".gitmodules"))) fail("VAULT_SUBMODULES_UNSUPPORTED", ".gitmodules present; submodules are not supported in V0", "inspecting repository", { dir: top });
  const gitlinks = (await hardenedGit(top, ["ls-files", "-s", "-z"])).nul().filter((l) => l.startsWith("160000 "));
  if (gitlinks.length > 0) fail("VAULT_SUBMODULES_UNSUPPORTED", "the index contains gitlinks; submodules are not supported in V0", "inspecting repository", { paths: gitlinks.map((l) => l.split("\t")[1]) });
  // HEAD
  ...
  let head: VaultRepositoryInspection["head"];
  if (symref.status === 0) {
    ...
    if (!resolvedOid) fail("VAULT_NOT_A_REPOSITORY", "HEAD is neither a symbolic ref nor a commit", "inspecting repository", { dir: top });
...
export interface VaultSnapshotOptions {
   ...
   * BOTH capture lanes (manual `repos capture` and the deploy-lane
   ...
   * refuses regardless). See {@link VaultSnapshot.modified_captured} /
   * {@link VaultSnapshot.untracked_captured} for the disclosure this
   * produces once set.
   */
  allowDirty?: boolean;
}
...
export interface VaultSnapshotRefusalPath {
  path: string;
  filter: string;
}
...
export interface VaultCapturedFile {
  path: string;
  mode: "100644" | "100755" | "120000";
  oid: string;
}

type Entry = VaultCapturedFile;
 ...
 */
export function capturedSetDigest(files: VaultCapturedFile[]): string {
 ...
 */
export async function deriveCapturedSet(options: { top_level: string; global_excludes_path?: string | null }): Promise<VaultCapturedFile[]> {
...
export interface VaultCapturedSetDrift {
  added: string[];
  removed: string[];
  modified: string[];
}
...
export function diffCapturedSets(before: VaultCapturedFile[], after: VaultCapturedFile[]): VaultCapturedSetDrift {
  const b = new Map(before.map((e) => [e.path, e]));
  const a = new Map(after.map((e) => [e.path, e]));
  const drift: VaultCapturedSetDrift = { added: [], removed: [], modified: [] };
...
export function capturedSetUnchanged(drift: VaultCapturedSetDrift): boolean {
 ...
 */
export async function captureSnapshot(options: VaultSnapshotOptions): Promise<VaultSnapshot> {
  ...
  const headTarget: VaultHeadTarget = repo.head.kind === "symref" ? { kind: "symref", ref: repo.head.ref } : { kind: "detached", oid: repo.head.oid };
  ...
  // applies to both capture lanes (manual `repos capture` and the
  ...
  const oversize = present.find((e) => e.size > VAULT_MAX_GIT_OBJECT_BYTES);
  if (oversize) {
    fail("GIT_OBJECT_TOO_LARGE", `${oversize.path} is ${oversize.size} bytes; the per-object cap is ${VAULT_MAX_GIT_OBJECT_BYTES} bytes (200 MiB)`, "capturing snapshot", { path: oversize.path, size_bytes: String(oversize.size), cap_bytes: String(VAULT_MAX_GIT_OBJECT_BYTES) });
  }
  ...
  if (active.length > 0) {
    ...
    const code = lfs.length > 0 ? "VAULT_LFS_UNSUPPORTED" : "VAULT_FILTER_ACTIVE";
    fail(code, `${active.length} captured path(s) have an active clean/smudge filter (${[...new Set(active.map((a) => a.filter))].join(", ")}); vault never runs filters, so the capture is refused by name — first: ${active[0]!.path}`, "capturing snapshot", { paths: active });
  }
  ...
  const tmp = mkdtempSync(join(tmpdir(), "run402-vault-snapshot-"));
  try {
    ...
    const commit = (await hardenedGit(top, ["commit-tree", tree, ...parentArgs, "-m", options.message ?? "run402 repos capture (synthetic commit of the work tree)"], { env: dateEnv })).text().trim();
    if (options.update_local_deploy_ref !== false) {
      await hardenedGit(top, ["update-ref", VAULT_DEPLOY_REF, commit]);
    }
...
export async function detectActiveFilters(top: string, paths: string[]): Promise<VaultSnapshotRefusalPath[]> {
  if (paths.length === 0) return [];
  const out = await hardenedGit(top, ["check-attr", "-z", "--stdin", "filter"], { input: paths.join("\0") + "\0" });
  const parts = out.nul();
  const active: VaultSnapshotRefusalPath[] = [];
 ...
 */
export const VAULT_HANDOFF_SENSITIVE_DENYLIST: readonly string[] = [
  ".env",
  ".env.*",
  "*.pem",
  "*.key",
...
export function isHandoffSensitivePath(relPath: string, includeSensitive: readonly string[] = []): boolean {
  ...
  return VAULT_HANDOFF_SENSITIVE_DENYLIST.some((g) => globMatchesGitPath(g, relPath));
}

/** What {@link VaultHandoffCaptureOptions.message} learns once capture figures are known — everything a Handoff Note's `capture` block needs, before the outer commit exists. */
export interface VaultHandoffCaptureStats {
...
export interface VaultHandoffCaptureOptions {
  /** The work tree (any directory inside it). */
  dir: string;
  env?: GitConfigDiscoveryEnv;
  /**
   ...
   */
  message: string | ((stats: VaultHandoffCaptureStats) => string);
  now?: () => Date;
 ...
 */
export interface VaultHandoffSnapshot {
  oid: string;
  tree_oid: string;
  base_head_oid: string;
 ...
 */
export async function captureHandoffSnapshot(options: VaultHandoffCaptureOptions): Promise<VaultHandoffSnapshot> {
  const repo = await inspectRepository(options.dir);
  const top = repo.top_level;
  if (repo.unmerged_paths.length > 0) {
  ...
  const oversize = [...trackedPresent, ...untrackedPresent].find((e) => e.size > VAULT_MAX_GIT_OBJECT_BYTES);
  if (oversize) {
    fail("GIT_OBJECT_TOO_LARGE", `${oversize.path} is ${oversize.size} bytes; the per-object cap is ${VAULT_MAX_GIT_OBJECT_BYTES} bytes (200 MiB)`, "capturing handoff snapshot", { path: oversize.path, size_bytes: String(oversize.size), cap_bytes: String(VAULT_MAX_GIT_OBJECT_BYTES) });
  }
  ...
  if (active.length > 0) {
    const lfs = active.filter((a) => a.filter === "lfs");
    const code = lfs.length > 0 ? "VAULT_LFS_UNSUPPORTED" : "VAULT_FILTER_ACTIVE";
    fail(code, `${active.length} captured path(s) have an active clean/smudge filter (${[...new Set(active.map((a) => a.filter))].join(", ")}); vault never runs filters, so the capture is refused by name — first: ${active[0]!.path}`, "capturing handoff snapshot", { paths: active });
  }
  ...
  async function buildTree(entries: Entry[]): Promise<string> {
    const tmp = mkdtempSync(join(tmpdir(), "run402-vault-handoff-"));
    try {
      const indexFile = join(tmp, "index");
      const info = entries.map((e) => `${e.mode} ${e.oid}\t${e.path}\n`).join("");
      await hardenedGit(top, ["update-index", "--index-info"], { input: info, env: { GIT_INDEX_FILE: indexFile } });
  ...
  const indexCommitOid = (await hardenedGit(
    top,
    ["commit-tree", indexTreeOid, "-p", baseHeadOid, "-m", "vault handoff: index"],
    { env: dateEnv },
  )).text().trim();
  ...
  const untrackedCommitOid = (await hardenedGit(
    top,
    ["commit-tree", untrackedTreeOid, "-m", "vault handoff: untracked"],
    { env: dateEnv },
  )).text().trim();
  ...
  const stats: VaultHandoffCaptureStats = {
...
export async function materializeSnapshot(repoDir: string, oid: string, targetDir?: string): Promise<string> {
  if (!VAULT_OID40_RE.test(oid)) fail("VAULT_BAD_OID", `not a 40-hex oid: ${oid}`, "materializing snapshot");
  const top = (await hardenedGit(resolve(repoDir), ["rev-parse", "--show-toplevel"])).text().trim();
  const target = targetDir ? resolve(targetDir) : mkdtempSync(join(tmpdir(), "run402-vault-materialize-"));
  mkdirSync(target, { recursive: true });
  const tmp = mkdtempSync(join(tmpdir(), "run402-vault-index-"));
  try {
    const indexFile = join(tmp, "index");
...
/** The client-side `vault_commit` line — printed ALWAYS; commit ids never reach the platform. */
export function vaultCommitLine(snapshot: Pick<VaultSnapshot, "oid" | "kind">): string {
  return `vault_commit ${snapshot.oid}${snapshot.kind === "synthetic" ? " (synthetic)" : ""}`;
}

/** Object sizes for a set of oids (`cat-file --batch-check`), for the per-object cap on tracked history. */
export async function findOversizeObjects(repoDir: string, tips: string[], cap = VAULT_MAX_GIT_OBJECT_BYTES): Promise<Array<{ oid: string; size_bytes: number }>> {
 ...
 * the whole set (vault-delta-fetch task 3.1), identical per-oid
