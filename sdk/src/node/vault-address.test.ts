/**
 * vault D6 — named addressing, id-pinning, and push-to-create
 * (repo-first-onramp task 4.5). Exercised against `VaultMemoryTransport`
...
import { vaultRemoteAddressForm, vaultRemoteUrl, vaultRemoteUrlForRepo, vaultSlugReleasedInfo, parseVaultRemoteUrl } from "../namespaces/repos.js";
import { VaultKeystore } from "./vault-keystore.js";
import { findResumablePushToCreateJournal, listIncompleteVaultJournals, readVaultJournal } from "./vault-creation-journal.js";
import { VaultMemoryTransport, makeRepo } from "./vault-memory-transport.test.js";
import { pinVaultRepo, readPinnedVaultRepo, recoverStaleVaultPin, resolveVaultAddress } from "./vault-address.js";
import { pushToCreateVault } from "./vault-push-to-create.js";

let root: string;
let keystore: VaultKeystore;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-address-"));
  keystore = VaultKeystore.open({ rootDir: join(root, "ks") });
});
afterEach(() => {
...
describe("vaultRemoteAddressForm", () => {
  it("classifies a UUID org id + prj_-prefixed project id as id-form", () => {
    assert.equal(vaultRemoteAddressForm({ org_id: "11111111-1111-4111-8111-111111111111", project_id: "prj_abc123" }), "id");
  });
  it("classifies a non-UUID org half as slug-form even when the name half is prj_-prefixed", () => {
    assert.equal(vaultRemoteAddressForm({ org_id: "acme", project_id: "prj_abc123" }), "slug");
  });
  it("classifies a non-prj_ name half as slug-form even when the org half is a UUID", () => {
    assert.equal(vaultRemoteAddressForm({ org_id: "11111111-1111-4111-8111-111111111111", project_id: "my-notes" }), "slug");
  });
  it("classifies an ordinary org-slug/name pair as slug-form", () => {
    assert.equal(vaultRemoteAddressForm({ org_id: "acme", project_id: "my-notes" }), "slug");
  });
});

describe("vaultRemoteUrlForRepo / parseVaultRemoteUrl round-trip", () => {
  it("round-trips a slug-form address", () => {
    const url = vaultRemoteUrlForRepo("acme", "my-notes");
    assert.equal(url, "run402::acme/my-notes");
    const parsed = parseVaultRemoteUrl(url);
    assert.deepEqual(parsed, { org_id: "acme", project_id: "my-notes" });
    assert.equal(vaultRemoteAddressForm(parsed!), "slug");
  });
  it("id-form (vaultRemoteUrl) still parses to id-form", () => {
    const url = vaultRemoteUrl("11111111-1111-4111-8111-111111111111", "prj_abc123");
    const parsed = parseVaultRemoteUrl(url)!;
    assert.equal(vaultRemoteAddressForm(parsed), "id");
  });
});

// ─── resolveVaultAddress — id-form ──────────────────────────────────────

describe("resolveVaultAddress — id-form", () => {
  it("resolves via findVaultByProject and pins repo_id (design D4 — no resolved_from, id-form has no slug pair)", async () => {
    ...
    const { createVault } = await import("./vault-creation-journal.js");
    const allocation = await createVault({ keystore, transport, org_id: "org_1", project_id: "prj_1" });
    ...
    // vault-client-round-trips design D4: the first successful id-form
    ...
    assert.deepEqual(await readPinnedVaultRepo(repoDir), { repo_id: allocation.repo_id, resolved_from: null, project_id: resolution.project_id, org_id: resolution.org_id, room: resolution.project_id });
  ...
  it("a second id-form resolution follows the pin — never calls findVaultByProject again", async () => {
    ...
    const first = await resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir });
    ...
    const second = await resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir });
  ...
  it("a legacy id-less pin self-upgrades through exactly one validation read, then goes offline", async () => {
    ...
    await pinVaultRepo(repoDir, allocation.repo_id);

    const upgraded = await resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir });
    ...
    const third = await resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir });
  ...
  it("a stale id-form pin (repo_id no longer resolves) clears and re-resolves once", async () => {
    const inner = new VaultMemoryTransport();
    const { createVault } = await import("./vault-creation-journal.js");
    const allocation = await createVault({ keystore, transport: inner, org_id: "org_1", project_id: "prj_1" });
    const repoDir = await makeRepo(root);
    const address = { org_id: "11111111-1111-4111-8111-111111111111", project_id: "prj_1" };
    await pinVaultRepo(repoDir, allocation.repo_id);
    assert.notEqual(await readPinnedVaultRepo(repoDir), null);
      ...
      get(target, prop, receiver) {
        if (prop === "getVaultRecord") {
          return async (req: { repo_id: string }) => {
            ...
            if (req.repo_id === allocation.repo_id) throw new LocalError("no such vault", "reading the vault record", { code: "RESOURCE_NOT_FOUND" });
        ...
        if (prop === "findVaultByProject") {
          return async (req: { project_id: string }) => {
            ...
            throw new LocalError(`no vault for ${req.project_id}`, "resolving the project's vault", { code: "RESOURCE_NOT_FOUND" });
          };
        }
    ...
    await assert.rejects(
      resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir }),
      (e: unknown) => {
    ...
    assert.equal(await readPinnedVaultRepo(repoDir), null);
  });
});

// ─── resolveVaultAddress — id-form, keystore consult (vault-offline-clone-resolve) ───

describe("resolveVaultAddress — id-form, keystore consult (vault-offline-clone-resolve)", () => {
  ...
  it("a keystore-known repo resolves with ZERO transport operations, marked via 'keystore'", async () => {
    const transport = new VaultMemoryTransport();
    const { createVault } = await import("./vault-creation-journal.js");
    const allocation = await createVault({ keystore, transport, org_id: ID_ORG, project_id: ID_PROJECT });
    const repoDir = await makeRepo(root);
    const address = { org_id: ID_ORG, project_id: ID_PROJECT };

    const callsBefore = transport.calls.length;
    const resolution = await resolveVaultAddress({ keystore, transport, address, repo_dir: repoDir });
    assert.equal(resolution.form, "id");
    ...
    assert.deepEqual(await readPinnedVaultRepo(repoDir), { repo_id: allocation.repo_id, resolved_from: null, project_id: ID_PROJECT, org_id: ID_ORG, room: ID_PROJECT });
  });
  ...
  it("a keystore hit needs no repo_dir at all — cwd-independent, e.g. a bare `git ls-remote`", async () => {
    ...
    const callsBefore = transport.calls.length;
    const resolution = await resolveVaultAddress({ keystore, transport, address });
    assert.equal(resolution.via, "keystore");
    assert.equal(resolution.offline, true);
    assert.equal(resolution.repo_id, allocation.repo_id);
  ...
  it("a project_id match under a DIFFERENT org_id is a miss, never a guess", async () => {
    ...
    const resolution = await resolveVaultAddress({ keystore, transport, address: wrongOrgAddress, repo_dir: repoDir });
  ...
  it("the git-config pin is preferred over the keystore once it exists", async () => {
    const transport = new VaultMemoryTransport();
    const { createVault } = await import("./vault-creation-journal.js");
    await createVault({ keystore, transport, org_id: ID_ORG, project_id: ID_PROJECT });
    const repoDir = await makeRepo(root);
    const address = { org_id: ID_ORG, project_id: ID_PROJECT };
  ...
  it("slug-form addresses never consult the keystore — a miss is still an ordinary not-found refusal", async () => {
    ...
    await assert.rejects(
      resolveVaultAddress({ keystore, transport, address: { org_id: "acme", project_id: "my-notes" }, repo_dir: repoDir, allow_create: false }),
      (e: unknown) => {
        assert.ok(e instanceof LocalError);
        assert.equal((e as LocalError).code, "RESOURCE_NOT_FOUND");
        return true;
...
// ─── resolveVaultAddress — slug-form, resolve-only (no create) ─────────

describe("resolveVaultAddress — slug-form, allow_create: false", () => {
...
describe("resolveVaultAddress — slug-form id-pinning", () => {
  it("pins repo_id on first resolution, and every later open follows the PIN, surviving a rename", async () => {
    ...
    const second = await resolveVaultAddress({
      keystore, transport,
      address: { org_id: "acme", project_id: "my-notes" },
      repo_dir: repoDir,
    });
  ...
  it("pinVaultRepo omits resolvedFrom for an id-form pin — reads back as resolved_from: null", async () => {
    const repoDir = await makeRepo(root);
    assert.equal(await readPinnedVaultRepo(repoDir), null);
    await pinVaultRepo(repoDir, "src_deadbeef"); // no third argument — the id-form shape
    const pinned = await readPinnedVaultRepo(repoDir);
    assert.deepEqual(pinned, { repo_id: "src_deadbeef", resolved_from: null, project_id: null, org_id: null, room: null });
  });

  it("pinVaultRepo / readPinnedVaultRepo round-trip directly", async () => {
    const repoDir = await makeRepo(root);
    assert.equal(await readPinnedVaultRepo(repoDir), null);
    await pinVaultRepo(repoDir, "src_deadbeef", { org_slug: "acme", repo_name: "widgets" });
    const pinned = await readPinnedVaultRepo(repoDir);
  ...
  it("pinVaultRepo round-trips the resolved ids (the offline-pin schema)", async () => {
    const repoDir = await makeRepo(root);
    await pinVaultRepo(repoDir, "src_deadbeef", { org_slug: "acme", repo_name: "widgets" }, { project_id: "prj_1", org_id: "org_1" });
    const pinned = await readPinnedVaultRepo(repoDir);
...
describe("pushToCreateVault — the happy path", () => {
  it("allocates a NEW project + vault atomically when the address does not resolve yet", async () => {
    const transport = new VaultMemoryTransport();
    const result = await pushToCreateVault({ keystore, transport, org_slug: "acme", repo_name: "my-notes" });
    ...
    assert.equal(listIncompleteVaultJournals(keystore).length, 0);

    // A SECOND push-to-create call for the SAME address is an ordinary
    // resolve (the fast path) — no second allocate.
    const second = await pushToCreateVault({ keystore, transport, org_slug: "acme", repo_name: "my-notes" });
  ...
  it("resumes an interrupted push-to-create journal by (org_slug, repo_name), not a fresh one", async () => {
    ...
    const incomplete = listIncompleteVaultJournals(keystore);
    ...
    assert.equal(readVaultJournal(keystore, incomplete[0]!.client_creation_id)?.stage, "ACTIVE");
    assert.equal(transport.calls.filter((c) => c === "allocate").length, 1, "resuming must never re-allocate");
  });
});

describe("pushToCreateVault — REPO_CREATION_CONFLICT (design D6's race)", () => {
  it("the LOSER resolves to the WINNER's repo and proceeds as an ordinary push; zero orphans", async () => {
    ...
    const winnerKeystore = VaultKeystore.open({ rootDir: join(root, "ks-winner") });
    const loserKeystore = VaultKeystore.open({ rootDir: join(root, "ks-loser") });

    const [winner, loser] = await Promise.all([
      pushToCreateVault({ keystore: winnerKeystore, transport, org_slug: "acme", repo_name: "my-notes" }),
      pushToCreateVault({ keystore: loserKeystore, transport, org_slug: "acme", repo_name: "my-notes" }),
    ]);
...
describe("pushToCreateVault — SLUG_RELEASED is never auto-followed", () => {
  it("rethrows the typed refusal unchanged, and creates nothing", async () => {
    ...
    await assert.rejects(
      pushToCreateVault({ keystore, transport, org_slug: "acme", repo_name: "my-notes" }),
      (e: unknown) => {
        assert.ok(e instanceof LocalError);
        assert.equal((e as LocalError).code, "SLUG_RELEASED");
        const info = vaultSlugReleasedInfo(e);
  ...
  it("vaultSlugReleasedInfo returns null for an unrelated error", () => {
    assert.equal(vaultSlugReleasedInfo(new LocalError("nope", "ctx", { code: "RESOURCE_NOT_FOUND" })), null);
    assert.equal(vaultSlugReleasedInfo(new Error("plain")), null);
  });
});

describe("resolveVaultAddress — push-to-create pins on creation too", () => {
...
// ─── recoverStaleVaultPin (vault-force-spelling-and-pin-fold) ────────

describe("recoverStaleVaultPin — stale-pin discovery moved to first use", () => {
  ...
  it("a vault-absent failure under an offline pin re-resolves to the NEW vault and rewrites the pin", async () => {
    ...
    const fresh = await recoverStaleVaultPin({
      ...
      error: new LocalError("denied", "materializing", { code: "VAULT_ACCESS_DENIED" }),
    });
    assert.ok(fresh, "a different repo_id is a recovery");
    assert.equal(fresh!.repo_id, "src_recreated");
    const pinned = await readPinnedVaultRepo(f.repoDir);
  ...
  it("a non-vault-absent error answers null with zero transport calls and an untouched pin", async () => {
    ...
    assert.equal((await readPinnedVaultRepo(f.repoDir))?.repo_id, f.resolution.repo_id);
  });
...
// ─── recoverStaleVaultPin — keystore-sourced offline resolution (vault-offline-clone-resolve design D3) ───

describe("recoverStaleVaultPin — keystore-sourced offline resolution (vault-offline-clone-resolve)", () => {
