/**
 * vault-recovery-custody — the member recovery bundle
 ...
 * (vault-offline-recovery spec, as modified by vault-recovery-custody).
 *
...
export const VAULT_MEMBER_BUNDLE_FORMAT = "r402s-member-recovery-bundle/v1" as const;

/** The mirrored-prefix sidecar directory recovery/verify scan for bundles. */
export const VAULT_MEMBER_BUNDLE_MIRROR_PREFIX = "member-recovery-bundles/" as const;
 ...
 * wrong-key open.
 */
export const VAULT_DEFAULT_SOURCE_WRAPPER_RP_ID = "console.run402.com" as const;

export interface VaultMemberBundleWrapper {
  wrapper_id: string;
...
export interface VaultMemberRecoveryBundle {
  format: typeof VAULT_MEMBER_BUNDLE_FORMAT;
  exported_at?: string;
  principal_id: string;
  ...
  wrappers: VaultMemberBundleWrapper[];
...
export function parseMemberRecoveryBundle(input: unknown, context = "parsing member recovery bundle"): VaultMemberRecoveryBundle {
  ...
  const b = value as Partial<VaultMemberRecoveryBundle> | null;
  if (!b || typeof b !== "object") fail("RECOVERY_BUNDLE_INVALID", "bundle is not an object", context);
  if (b.format !== VAULT_MEMBER_BUNDLE_FORMAT) {
    fail("RECOVERY_BUNDLE_INVALID", `unsupported bundle format ${JSON.stringify(b.format ?? null)} — expected ${VAULT_MEMBER_BUNDLE_FORMAT}`, context, { format: b.format ?? null });
  }
  ...
  for (const w of b.wrappers) {
    if (!w || typeof w !== "object" || typeof (w as VaultMemberBundleWrapper).wrapper_id !== "string" || typeof (w as VaultMemberBundleWrapper).wrapper_ciphertext !== "string") {
      fail("RECOVERY_BUNDLE_INVALID", "bundle wrappers[] entry is malformed (needs wrapper_id + wrapper_ciphertext)", context);
    }
  }
  return b as VaultMemberRecoveryBundle;
}

export interface VaultMemberUnwrapResult {
...
export function unwrapMemberRecoveryBundle(input: {
  bundle: VaultMemberRecoveryBundle;
  source_recovery_code: string;
  /** Seal-time ceremony host override; default = bundle.rp_id, then {@link VAULT_DEFAULT_SOURCE_WRAPPER_RP_ID}. */
  rp_id?: string;
}): VaultMemberUnwrapResult {
  const context = "opening member recovery bundle";
  const bundle = input.bundle;
  const rpId = input.rp_id ?? bundle.rp_id ?? VAULT_DEFAULT_SOURCE_WRAPPER_RP_ID;
...
export interface VaultMemberBundleHint {
  key: string;
 ...
 */
export async function discoverMemberBundles(backend: VaultMirrorBackend): Promise<Array<VaultMemberBundleHint & { bundle: VaultMemberRecoveryBundle | null }>> {
  let keys: string[];
  try {
    keys = (await backend.list(VAULT_MEMBER_BUNDLE_MIRROR_PREFIX)).filter((k) => k.endsWith(".json"));
  } catch {
    return [];
  }
  const out: Array<VaultMemberBundleHint & { bundle: VaultMemberRecoveryBundle | null }> = [];
