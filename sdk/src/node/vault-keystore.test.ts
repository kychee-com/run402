/**
 * vault keystore (task 5.2) — layout, permissions, atomic writes, locks,
...
import { VaultKeystore, getVaultKeystoreRoot, writeFileAtomic0600, VAULT_OBJECT_STORE_ORIGINS_CAP } from "./vault-keystore.js";
import {
  VAULT_GENESIS_EPOCH,
...
import type { VaultSignedObject } from "../namespaces/vault.types.js";
...
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-ks-"));
});
afterEach(() => {
...
async function sealedVault(ks: VaultKeystore) {
  ...
  const sealed = await sealKeyEnvelope({ k_repo: kRepo, repo_id: REPO, epoch: VAULT_GENESIS_EPOCH, recipient_public_key: recipient.public_key, signer, created_at: "2026-08-22T12:00:00.000Z" });
...
describe("vault keystore — layout + discipline", () => {
  it("defaults to <config dir>/vault and honors RUN402_CONFIG_DIR", () => {
    process.env.RUN402_CONFIG_DIR = root;
    assert.equal(getVaultKeystoreRoot(), join(root, "vault"));
  });

  it("moves a keystore at <config dir>/gitvault to <config dir>/vault once, when vault is absent", () => {
    process.env.RUN402_CONFIG_DIR = root;
    mkdirSync(join(root, "gitvault", "repos"), { recursive: true, mode: 0o700 });
    writeFileSync(join(root, "gitvault", "identity.json"), "{\"id\":1}", { mode: 0o600 });
    writeFileSync(join(root, "gitvault", "repos", "src_x.json"), "{}", { mode: 0o600 });
    assert.equal(getVaultKeystoreRoot(), join(root, "vault"));
    assert.equal(existsSync(join(root, "gitvault")), false, "the earlier directory is gone");
    assert.equal(readFileSync(join(root, "vault", "identity.json"), "utf8"), "{\"id\":1}");
    assert.equal(existsSync(join(root, "vault", "repos", "src_x.json")), true);
    assert.equal(mode(join(root, "vault", "identity.json")), 0o600, "the move keeps file modes");
    // a second resolution is a no-op
    assert.equal(getVaultKeystoreRoot(), join(root, "vault"));
    assert.equal(readFileSync(join(root, "vault", "identity.json"), "utf8"), "{\"id\":1}");
  });

  it("never moves over an existing vault directory — both are left exactly as they are", () => {
    process.env.RUN402_CONFIG_DIR = root;
    mkdirSync(join(root, "gitvault"), { recursive: true });
    writeFileSync(join(root, "gitvault", "identity.json"), "old");
    mkdirSync(join(root, "vault"), { recursive: true });
    writeFileSync(join(root, "vault", "identity.json"), "new");
    assert.equal(getVaultKeystoreRoot(), join(root, "vault"));
    assert.equal(readFileSync(join(root, "vault", "identity.json"), "utf8"), "new");
    assert.equal(readFileSync(join(root, "gitvault", "identity.json"), "utf8"), "old");
  });

  it("creates nothing when there is no keystore at either path", () => {
    process.env.RUN402_CONFIG_DIR = root;
    assert.equal(getVaultKeystoreRoot(), join(root, "vault"));
    assert.equal(existsSync(join(root, "vault")), false);
    assert.equal(existsSync(join(root, "gitvault")), false);
  });

  it("open() creates 0700 directories; identity.json is 0600 and idempotent", () => {
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
  ...
  it("writes are atomic (no partial file is ever visible) and refuse symlinks", () => {
    ...
    assert.throws(() => writeFileAtomic0600(link, "{}"), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_SYMLINK");
  });
  ...
  it("the permission audit names a world-readable identity and a symlinked repo file", () => {
    ...
    assert.throws(() => VaultKeystore.open({ rootDir: root, strictPermissions: true }), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_PERMISSIONS");
    ...
    assert.throws(() => ks.readRepo(REPO), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_SYMLINK");
  });
});

describe("vault keystore — repo files + locks", () => {
  it("saveRepo/readRepo/updateRepo round-trip under the per-repo lock and are audited", () => {
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
    const saved = ks.saveRepo({ repo_id: REPO, org_id: "o", project_id: "p", k_repo_hex: "00".repeat(32), epoch: VAULT_GENESIS_EPOCH, genesis_sha256: "a".repeat(64), head_pin: null, last_ref_transaction: null, provenance: "created" });
  ...
  it("withRepoLock is re-entrant, times out on a live foreign lock, and reclaims a stale one", () => {
    ...
    assert.throws(() => ks.withRepoLock(REPO, () => 1, { timeoutMs: 60, staleAfterMs: 60_000 }), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_LOCKED");
...
describe("vault keystore — recordObjectStoreOrigins (vault-object-host-predial task 1.2)", () => {
  function freshKs() {
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
    ks.saveRepo({ repo_id: REPO, org_id: "o", project_id: "p", k_repo_hex: "00".repeat(32), epoch: VAULT_GENESIS_EPOCH, genesis_sha256: "a".repeat(64), head_pin: null, last_ref_transaction: null, provenance: "created" });
    return ks;
  }
  ...
  it("caps at VAULT_OBJECT_STORE_ORIGINS_CAP, evicting the least-recently-observed", () => {
    const ks = freshKs();
    for (let i = 0; i < VAULT_OBJECT_STORE_ORIGINS_CAP + 2; i += 1) ks.recordObjectStoreOrigins(REPO, [`https://origin-${i}.example.com`]);
    const origins = ks.readRepo(REPO)!.object_store_origins!;
    assert.equal(origins.length, VAULT_OBJECT_STORE_ORIGINS_CAP);
    // most-recently-observed survive; the earliest ones were evicted
    assert.deepEqual(origins, [`https://origin-${VAULT_OBJECT_STORE_ORIGINS_CAP + 1}.example.com`, `https://origin-${VAULT_OBJECT_STORE_ORIGINS_CAP}.example.com`, `https://origin-${VAULT_OBJECT_STORE_ORIGINS_CAP - 1}.example.com`, `https://origin-${VAULT_OBJECT_STORE_ORIGINS_CAP - 2}.example.com`]);
  });
  ...
  it("saveRepo/readRepo tolerate a HAND-WRITTEN repo file that already carries the field — additive-schema forward compat", () => {
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
    const saved = ks.saveRepo({ repo_id: REPO, org_id: "o", project_id: "p", k_repo_hex: "00".repeat(32), epoch: VAULT_GENESIS_EPOCH, genesis_sha256: "a".repeat(64), head_pin: null, last_ref_transaction: null, provenance: "created", object_store_origins: ["https://pre-existing.example.com"] });
...
describe("vault keystore — findRepoByProject (vault-offline-clone-resolve task 1.1)", () => {
  const REPO_A = `src_${"a".repeat(32)}`;
  const REPO_B = `src_${"b".repeat(32)}`;

  function repo(overrides: Partial<Parameters<VaultKeystore["saveRepo"]>[0]>) {
    return {
      repo_id: REPO_A,
      org_id: "org_1",
      project_id: "prj_1",
...
describe("vault keystore — §5.1 transitions", () => {
  it("whole-keystore loss is VAULT_UNRECOVERABLE with the verbatim statement", () => {
    ...
    if (s.state === "unrecoverable") {
      ...
      assert.equal(s.statement, VAULT_TERMINAL_LOSS_STATEMENT);
  ...
  it("rev 47: a MEMBER's envelope wrapped by an ADMITTED non-genesis writer restores; without the admitted set it is refused as stored_envelope_created_by", async () => {
    ...
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
    const v = await sealedVault(ks);
    const ks2 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-ks2-")), now: fixedNow });
    const writer2 = ks2.signingKeypair(ks2.ensureIdentity())!;
    const ks3 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-ks3-")), now: fixedNow });
    const member = ks3.encryptionKeypair(ks3.ensureIdentity())!;
    const wrapped = await sealKeyEnvelope({ k_repo: v.kRepo, repo_id: REPO, epoch: VAULT_GENESIS_EPOCH, recipient_public_key: member.public_key, signer: writer2, created_at: "2026-09-04T00:00:00.000Z" });
  ...
  it("a restore without any receipt or pin is labeled unauthenticated salvage; a substituted vault is refused", async () => {
    ...
    const badReceipt = buildRecoveryReceipt({ repo_id: REPO, org_id: "o", project_id: "p", genesis_sha256: storedBytesSha256(other as unknown as VaultSignedObject), creator_signing: v.signer, creator_encryption_public_key: v.recipient.public_key });
  ...
  it("signing key lost with K_repo held → read_only; stale pin → reverify", async () => {
    const ks = VaultKeystore.open({ rootDir: root, now: fixedNow });
    const v = await sealedVault(ks);
    ks.saveRepo({ repo_id: REPO, org_id: "o", project_id: "p", k_repo_hex: bytesToHex(v.kRepo), epoch: VAULT_GENESIS_EPOCH, genesis_sha256: v.genesisSha, head_pin: null, last_ref_transaction: null, provenance: "created" });
  ...
  it("a tampered identity (seed ≠ fingerprint) is reported as corrupt, never silently used", () => {
    ...
    assert.throws(() => ks.signingKeypair(), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_CORRUPT");
    ...
    assert.throws(() => ks.encryptionKeypair(), (e: unknown) => e instanceof LocalError && e.code === "VAULT_KEYSTORE_CORRUPT");
  });
});
