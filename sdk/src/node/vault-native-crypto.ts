/**
 * The Node-native bulk-AEAD backend (vault-native-bulk-crypto, design
 * D1/D2/D4).
 *
 ...
 * SAME `VAULT_AEAD_AUTH_FAILURE` it always did.
 */
import { createCipheriv, createDecipheriv, createHash, getCiphers } from "node:crypto";
import { hchacha } from "@noble/ciphers/chacha.js";
import { _setVaultAeadBackend, _setVaultHashBackend, type VaultAeadBackend, type VaultHashBackend } from "../namespaces/vault.crypto.js";
...
export const nodeVaultAeadBackend: VaultAeadBackend = {
 ...
 */
export function installNodeVaultAeadBackend(): boolean {
  try {
    ...
    const probe = nodeVaultAeadBackend.seal(new Uint8Array(32), new Uint8Array(24), new Uint8Array([1]), new Uint8Array([2, 3]));
    const opened = nodeVaultAeadBackend.open(new Uint8Array(32), new Uint8Array(24), new Uint8Array([1]), probe);
...
/**
 * The Node-native protocol-hash backend (vault-native-hash). Same seam
 ...
 * after the AEAD moved to OpenSSL. `createHash` is sync (the vault crypto
 * surface is sync public API), byte-compatible, and already hashes packs in
 * `vault-recover.ts`/`vault-mirror.ts`.
 */
export const nodeVaultHashBackend: VaultHashBackend = {
  sha256(bytes) {
...
/**
 * Install the native hash backend after a live probe (vault-native-hash
 ...
 */
export function installNodeVaultHashBackend(backend: VaultHashBackend = nodeVaultHashBackend): boolean {
  try {
