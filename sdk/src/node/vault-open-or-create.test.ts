/**
 * vault D2 — lazy allocation on first push (repo-first-onramp task 2.2).
 *
 * `openOrCreateVault` is the primitive the remote helper and the capture
 * lane (`run402 repos capture`/`snapshot`) drive so a push against an
 * unallocated project runs the six-stage creation journal inline. Exercised
 * against the same `VaultMemoryTransport` fixture every other
...
import { createVault, listIncompleteVaultJournals, readVaultJournal } from "./vault-creation-journal.js";
import { VaultMemoryTransport } from "./vault-memory-transport.test.js";
import { openOrCreateVault } from "./vault-open-or-create.js";

let root: string;
let keystore: VaultKeystore;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-open-or-create-"));
...
describe("openOrCreateVault — found (the common case)", () => {
  it("resolves without creating anything when the vault already exists", async () => {
    const transport = new VaultMemoryTransport();
    const created = await createVault({ keystore, transport, org_id: ORG, project_id: PROJECT, service_public_key: transport.service.public_key });
    assert.equal(transport.calls.filter((c) => c === "allocate").length, 1);

    const r = await openOrCreateVault({ keystore, transport, project_id: PROJECT, org_id: ORG });
...
describe("openOrCreateVault — unresolved and no org_id: byte-identical to a plain resolve", () => {
  it("rethrows the original resolution failure unchanged", async () => {
    const transport = new VaultMemoryTransport();
    await assert.rejects(
      openOrCreateVault({ keystore, transport, project_id: PROJECT }),
      (err: unknown) => {
...
describe("openOrCreateVault — unresolved and org_id supplied: lazy creation (D2)", () => {
  it("allocates the vault and reports it as freshly created", async () => {
    ...
    const second = await openOrCreateVault({ keystore, transport, project_id: PROJECT, org_id: ORG });
...
describe("openOrCreateVault — resumability (D2's central guarantee)", () => {
  it("a creation interrupted mid-flight is resumed by the NEXT call, not restarted as a competing attempt", async () => {
    ...
    const resumedJournal = readVaultJournal(keystore, crashedId);
    ...
    assert.equal(listIncompleteVaultJournals(keystore).length, 0, "nothing left incomplete — exactly one vault, fully reconciled");
    ...
    const third = await openOrCreateVault({ keystore, transport, project_id: PROJECT, org_id: ORG });
