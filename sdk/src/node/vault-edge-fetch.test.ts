/**
 * vault — CDN-edge read preference + silent fallback (vault-read-edge-
 * cache, task 2.4, design D5). Unit tests for `fetchVaultObjectBytes`
 * itself — every call site in `vault-publication.ts` and
 * `vault-mirror.ts` shares this one function, so pinning its behavior
 * here is what keeps every call site's behavior pinned too.
 */

import { describe, it, beforeEach } from "node:test";
import assert from "node:assert/strict";
import { _resetVaultEdgeFetchStateForTest, fetchVaultObjectBytes, type VaultEdgeFetchClient } from "./vault-edge-fetch.js";

/** A `.fetch`-only fake client, keyed by exact URL, recording every URL it was asked to fetch (in call order). */
function fakeClient(handlers: Record<string, () => Response | Promise<Response>>): { client: VaultEdgeFetchClient; calls: string[] } {
  const calls: string[] = [];
  const client: VaultEdgeFetchClient = {
...
describe("fetchVaultObjectBytes — prefer edge_url, silently fall back to url per-process (vault-read-edge-cache design D5)", () => {
  beforeEach(() => {
  ...
  it("(a) edge_url present and healthy — edge is fetched, and `url` is never touched", async () => {
    ...
    const r = await fetchVaultObjectBytes(client, { url: "https://origin.example/obj", edge_url: "https://edge.example/obj" });
  ...
  it("(d) once edge has failed once in this process, a LATER read with its own healthy edge_url still goes straight to `url` (process-wide stickiness)", async () => {
    ...
    const r1 = await fetchVaultObjectBytes(client, { url: "https://origin.example/first", edge_url: "https://edge.example/first" });
    assert.equal(r1.status, 200);
    calls.length = 0; // isolate the second call's own fetch pattern

    const r2 = await fetchVaultObjectBytes(client, { url: "https://origin.example/second", edge_url: "https://edge.example/second" });
  ...
  it("(e) edge_url absent — `url` fetched directly, byte-identical to calling client.fetch(url) today", async () => {
    ...
    const r = await fetchVaultObjectBytes(client, { url: "https://origin.example/obj" });
    assert.equal(r.status, 200);
  ...
  it("(e') an absent edge_url never consults or flips the process-wide fallback flag — a LATER read with edge_url still prefers it", async () => {
    ...
    await fetchVaultObjectBytes(client, { url: "https://origin.example/no-edge" });
    const r2 = await fetchVaultObjectBytes(client, { url: "https://origin.example/has-edge", edge_url: "https://edge.example/has-edge" });
    assert.equal(r2.status, 200);
  ...
  it("passes the given `init` through unchanged to whichever URL is actually fetched", async () => {
    ...
    await fetchVaultObjectBytes(client, { url: "https://origin.example/obj", edge_url: "https://edge.example/obj" }, init);
