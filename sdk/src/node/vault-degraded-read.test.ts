 ...
 * MIRROR HALF ONLY). Behavioral tests for `vault-degraded-read.ts`.
 *
 ...
 * object store — the same pattern `vault-mirror-recover.test.ts` uses
 * (real signed heads, real encrypted objects, not hand-rolled fixture
 * bytes).
 *
 * Run: node --test --import tsx sdk/src/node/vault-degraded-read.test.ts
 */
...
import { commitFile, makeVault } from "./vault-memory-transport.test.js";
import { DirectoryMirrorBackend, openVaultMirrorBackend, type VaultMirrorBackend } from "./vault-mirror-backend.js";
import type { VaultMirrorDestination } from "./vault-mirror-config.js";
import { VaultKeystore } from "./vault-keystore.js";
import { saveMirrorConfig } from "./vault-mirror-config.js";
import { isNetworkClassVaultReadError, resolveDegradedReadSource, tryVaultDegradedRead } from "./vault-degraded-read.js";
...
/**
 * Reproduces `vault-mirror-recover.test.ts`'s own `toRepoRelativeGatewayKey`
...
function refusingBackend(): { backend: VaultMirrorBackend; calls: () => number } {
  let calls = 0;
  const fail = (): never => {
    calls += 1;
    throw new Error("server/mirror-blindness violation: a mirror backend method was called");
  };
  const backend: VaultMirrorBackend = {
...
// ─── Trigger discipline (isNetworkClassVaultReadError) ────────────────────

describe("vault degraded read — trigger discipline (design D4)", () => {
  it("a NetworkError (fetch threw before any response) is network-class", () => {
    assert.equal(isNetworkClassVaultReadError(new NetworkError("boom", new Error("ECONNREFUSED"), "ctx")), true);
  });

  it("an unbranded raw throw (the shape a DNS/connect failure ACTUALLY takes through vault's own direct-fetch reads) is network-class", () => {
    assert.equal(isNetworkClassVaultReadError(new TypeError("fetch failed")), true);
    assert.equal(isNetworkClassVaultReadError("a plain string throw"), true);
  });
  ...
  it("the payment-capable client's X402_INITIAL_REQUEST_FAILED (no response ever existed) is network-class; a later-phase payment failure is not", () => {
    ...
    assert.equal(isNetworkClassVaultReadError(mk("X402_INITIAL_REQUEST_FAILED")), true);
    assert.equal(isNetworkClassVaultReadError(mk("X402_PAYMENT_SIGNING_FAILED")), false);
  });

  it("a typed 5xx ApiError is network-class", () => {
    assert.equal(isNetworkClassVaultReadError(new ApiError("boom", 500, null, "ctx")), true);
    assert.equal(isNetworkClassVaultReadError(new ApiError("boom", 503, null, "ctx")), true);
  });

  it("a typed 401/403/404 is NEVER network-class — an authorization refusal must not silently reroute", () => {
    assert.equal(isNetworkClassVaultReadError(new Unauthorized("boom", 401, null, "ctx")), false);
    assert.equal(isNetworkClassVaultReadError(new Unauthorized("boom", 403, null, "ctx")), false);
    assert.equal(isNetworkClassVaultReadError(new ApiError("boom", 404, null, "ctx")), false);
  });

  it("a typed 400 is not network-class", () => {
    assert.equal(isNetworkClassVaultReadError(new ApiError("boom", 400, null, "ctx")), false);
  });

  it("vault's own direct-fetch reads (getGenerationBytes/getObjectBytes) throw via a LocalError with the real HTTP status folded into details.status — a 5xx there is STILL network-class", () => {
    const e = new LocalError("heads/… read failed (HTTP 500)", "reading vault heads", { code: "VAULT_OBJECT_READ_FAILED", details: { generation: "0000000000000001", status: 500 } });
    assert.equal(e.status, null, "sanity: LocalError always reports status: null on e.status itself");
    assert.equal(isNetworkClassVaultReadError(e), true);
  });

  it("...and a 403 through that SAME path is NOT network-class", () => {
    const e = new LocalError("access denied", "reading vault heads", { code: "VAULT_ACCESS_DENIED", details: { generation: "0000000000000001", status: 403 } });
    assert.equal(isNetworkClassVaultReadError(e), false);
  });

  it("a protocol/chain error with no status signal at all is not network-class (never folded into a generic 'try the mirror')", () => {
    const e = new LocalError("head 0000000000000002 does not match the chain during restore", "restoring vault objects", { code: "CHAIN_BROKEN" });
    assert.equal(isNetworkClassVaultReadError(e), false);
  });
});

// ─── tryVaultDegradedRead — retry/fallback orchestration ──────────────────

describe("vault degraded read — retry and fallback orchestration", () => {
  it("a 4xx/authorization failure is NEVER retried and NEVER falls back — the original error surfaces unchanged, and attemptLive is called exactly once", async () => {
    let calls = 0;
    const original = new Unauthorized("nope", 403, { code: "VAULT_ACCESS_DENIED" }, "ctx");
    ...
    await assert.rejects(
      tryVaultDegradedRead({ attemptLive, keystore: VaultKeystore.open({ rootDir: scratchDir("run402-degraded-ks-403-") }), repo_id: "src_" + "1".repeat(32), out_dir: scratchDir("run402-degraded-out-403-"), backend }),
  ...
  it("a network-class failure gets ONE bounded retry against the live attempt before falling back", async () => {
    ...
    await assert.rejects(tryVaultDegradedRead({ attemptLive, keystore: VaultKeystore.open({ rootDir: scratchDir("run402-degraded-ks-retry-") }), repo_id: "src_" + "2".repeat(32), out_dir: null, backend }));
  ...
  it("no fallback source configured (no mirror) → the ORIGINAL error surfaces unchanged, and zero mirror I/O happens", async () => {
    ...
    const keystore = VaultKeystore.open({ rootDir: scratchDir("run402-degraded-ks-nomirror-") });
    ...
    await assert.rejects(
      tryVaultDegradedRead({ attemptLive, keystore, repo_id: repoId, out_dir: scratchDir("run402-degraded-out-nomirror-") }),
      (e: unknown) => e === original,
    );
  });
  ...
  it("out_dir: null disables the fallback even with a mirror configured — nowhere to materialize into", async () => {
    ...
    await assert.rejects(
      tryVaultDegradedRead({ attemptLive, keystore: VaultKeystore.open({ rootDir: scratchDir("run402-degraded-ks-nodir-") }), repo_id: "src_" + "4".repeat(32), out_dir: null, backend }),
      (e: unknown) => e === original,
    );
  ...
  it("a genuinely successful live attempt never touches the fallback at all", async () => {
    ...
    const outcome = await tryVaultDegradedRead({ attemptLive: async () => live, keystore: VaultKeystore.open({ rootDir: scratchDir("run402-degraded-ks-live-") }), repo_id: "src_" + "5".repeat(32), out_dir: scratchDir("run402-degraded-out-live-"), backend });
...
describe("vault degraded read — served from a real DirectoryMirrorBackend", () => {
  it("degrades to the mirror: correct refs/generation, degraded: true, source provenance, validity-not-freshness stated verbatim", async () => {
    ...
    // `resolveDegradedReadSource` (via `openVaultMirrorBackend`) opens a
    ...
    const destination: VaultMirrorDestination = { kind: "directory", path: mirrorRoot };
    const backend = openVaultMirrorBackend(destination, f.repoId);
    ...
    const outcome = await tryVaultDegradedRead({
      attemptLive: async () => {
        attempts += 1;
        throw new NetworkError("run402 is unreachable", new Error("ECONNREFUSED"), "listing vault objects");
      },
      keystore: f.keystore,
      repo_id: f.repoId,
      out_dir: outDir,
    ...
    assert.equal(outcome.result.validity_not_freshness, VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT);
  ...
  it("materializing into a checkout's OWN .git (the remote helper's degraded fetch) leaves it a non-bare repository with its working tree intact", async () => {
    ...
    const c1 = await commitFile(f.repoDir, "a.txt", "a\n");
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const destination: VaultMirrorDestination = { kind: "directory", path: scratchDir("run402-degraded-mirror-wt-") };
    await seedBackend(openVaultMirrorBackend(destination, f.repoId), transportEntries(f.transport, f.repoId));
    saveMirrorConfig(f.keystore, { repo_id: f.repoId, destination });
    ...
    const outcome = await tryVaultDegradedRead({
      attemptLive: async () => {
        throw new NetworkError("run402 is unreachable", new Error("ECONNREFUSED"), "restoring vault objects");
      },
      keystore: f.keystore,
      repo_id: f.repoId,
      out_dir: gitDir,
...
describe("vault degraded read — writes are never rerouted (D194: admission is irreducibly live-server)", () => {
  it("a push during a simulated gateway outage still fails, and performs ZERO mirror writes, even with a mirror configured", async () => {
    ...
    f.transport.admitHead = async () => {
      throw new NetworkError("run402 is unreachable", new Error("ECONNREFUSED"), "admitting vault head");
    };
