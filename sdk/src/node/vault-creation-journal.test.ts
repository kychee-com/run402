/**
 * vault six-stage creation journal (task 5.3) — the ordering invariant
...
import {
  ...
  readVaultJournal,
  type VaultAdmitGenesisRequest,
  type VaultAllocateRequest,
  type VaultCreationStage,
  type VaultCreationTransport,
  type VaultPutObjectRequest,
...
import type { VaultAllocation, VaultKeyEnvelope, VaultSignedObject, VaultGenesis } from "../namespaces/vault.types.js";
import { LocalError } from "../errors.js";

let root: string;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-journal-"));
});
afterEach(() => {
...
class MemoryTransport implements VaultCreationTransport {
  readonly service = generateSigningKeypair();
  readonly objects = new Map<string, Uint8Array>();
  readonly genesis = new Map<string, Uint8Array>();
  readonly calls: string[] = [];
  allocations = new Map<string, VaultAllocation>();
  allocationsByRepoId = new Map<string, VaultAllocation>();
  /**
  ...
  readonly allocateRequests: VaultAllocateRequest[] = [];
  ...
  async putObject(req: VaultPutObjectRequest) {
  ...
  async admitGenesis(req: VaultAdmitGenesisRequest) {
...
describe("the allocate REQUEST wire shape (the field pair that differs from the allocation RECORD)", () => {
  ...
  // RECORD carries — and `POST /vaults/v1` answered
  ...
  it("sends raw PUBLIC KEYS, and never the fingerprints", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: t.service.public_key });
  ...
  it("the RECORD's fingerprints are DERIVED by the control plane, and the client checks them back", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    const result = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: t.service.public_key });
    const identity = ks.readIdentity()!;
...
describe("vault creation journal — happy path + ordering", () => {
  it("walks the six stages, emits the recovery receipt, and pins the repo file", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    const stages: VaultCreationStage[] = [];
    const result = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: t.service.public_key, onStage: (s) => { stages.push(s); } });
    assert.deepEqual(stages, VAULT_CREATION_STAGES.slice(1));
    ...
    assert.equal(verifyVaultObject(genesis as unknown as VaultSignedObject, genesis.creator_signing_pubkey), true);
    ...
    const env = parseVaultStrict(new TextDecoder().decode(envBytes)) as VaultKeyEnvelope;
    ...
    assert.deepEqual(listIncompleteVaultJournals(ks), []);
  });

  it("no ciphertext, object id, or K_repo exists before ALLOCATED (repo_id is a KDF input)", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    const c = VaultCreation.open({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID });
    const j = readVaultJournal(ks, CCID)!;
    ...
    const order: Array<[VaultCreationStage, boolean]> = [];
    const ks2 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-vault-order-")) });
    await createVault({ keystore: ks2, transport: new MemoryTransport(), org_id: "o", project_id: "p", client_creation_id: CCID, onStage: (s, jj) => { order.push([s, jj.k_repo_hex !== null || jj.objects.length > 0]); } });
...
describe("vault creation journal — crash-between-stages replay", () => {
  for (const crashAfter of VAULT_CREATION_STAGES.slice(1, -1)) {
    it(`crash after ${crashAfter} is reconciled on restart without re-sealing or re-PUTting`, async () => {
      ...
      await assert.rejects(
        createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, onStage: (s) => { if (s === crashAfter) throw crash; } }),
        (e: unknown) => e === crash,
      );
      const durable = readVaultJournal(ks, CCID)!;
      assert.equal(durable.stage, crashAfter);
      assert.deepEqual(listIncompleteVaultJournals(ks).map((j) => j.client_creation_id), [CCID]);
      const callsBefore = t.calls.slice();
      const objectsBefore = durable.objects.map((o) => o.stored_bytes_sha256);
      const result = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID });
  ...
  it("a crash between the PUT and the OBJECTS_FINALIZED write is a read-and-compare on resume", async () => {
    ...
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, onStage: (s) => { if (s === "OBJECTS_PREPARED") throw new Error("crash"); } }));
    const j = readVaultJournal(ks, CCID)!;
...
describe("vault creation journal — refusals (never destructive)", () => {
  it("an object already present with DIFFERENT bytes → VAULT_CREATION_CONFLICT, refusal persisted", async () => {
    ...
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID }), (e: unknown) => e instanceof LocalError && e.code === "VAULT_CREATION_CONFLICT");
    assert.equal(readVaultJournal(ks, CCID)!.refusal?.code, "VAULT_CREATION_CONFLICT");
  ...
  it("a foreign genesis already admitted → VAULT_CREATION_CONFLICT, never overwritten", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, onStage: (s) => { if (s === "GENESIS_PREPARED") throw new Error("crash"); } }));
    const j = readVaultJournal(ks, CCID)!;
  ...
  it("a superseded allocation (reclaimed by the owner) → ALLOCATION_SUPERSEDED at allocate AND at admit", async () => {
    ...
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID }), (e: unknown) => e instanceof LocalError && e.code === "ALLOCATION_SUPERSEDED");
    assert.equal(readVaultJournal(ks, CCID)!.objects.length, 0, "no ciphertext was ever produced");
    const t2 = new MemoryTransport();
    const ks2 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-vault-sup-")) });
    await assert.rejects(createVault({ keystore: ks2, transport: t2, org_id: "o", project_id: "p", client_creation_id: CCID, onStage: (s) => { if (s === "GENESIS_PREPARED") throw new Error("crash"); } }));
    t2.supersedeOnAdmit = true;
    await assert.rejects(createVault({ keystore: ks2, transport: t2, org_id: "o", project_id: "p", client_creation_id: CCID }), (e: unknown) => e instanceof LocalError && e.code === "ALLOCATION_SUPERSEDED");
  ...
  it("an allocation that does not match this attempt (or a bad service signature) is refused before anything is journaled as ALLOCATED", async () => {
    ...
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: wrongService.public_key }), (e: unknown) => e instanceof LocalError && e.code === "VAULT_ALLOCATION_INVALID");
    assert.equal(readVaultJournal(ks, CCID)!.stage, "LOCAL_KEYS_PREPARED");
  });

  it("a journal from another identity or org/project is never resumed", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    VaultCreation.open({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID });
    assert.throws(() => VaultCreation.open({ keystore: ks, transport: t, org_id: "other", project_id: "p", client_creation_id: CCID }), (e: unknown) => e instanceof LocalError && e.code === "VAULT_CREATION_CONFLICT");
  });
  ...
  it("the admitted genesis read back must hash to the journaled bytes (substitution is named)", async () => {
    ...
    const t = new MemoryTransport();
    const result = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID });
    const g = result.journal.genesis as VaultGenesis;
    assert.equal(storedBytesSha256(g as unknown as VaultSignedObject), result.genesis_sha256);
  });
});
...
describe("kychee-com/run402#563 — init idempotence against a COMPLETED vault (no local journal to resume)", () => {
  it("a fresh attempt (new client_creation_id, no resumable journal) reconciles: deduplicated, zero re-mint, exactly one allocate probe", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    const original = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: t.service.public_key });
    ...
    // dogfood repro (`vault init` on an already-provisioned project).
    const fresh = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", service_public_key: t.service.public_key });
  ...
  it("the CLI-facing dedupe shape matches vault init's already-rendered case (result.how drives result.deduplicated one level up)", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", client_creation_id: CCID, service_public_key: t.service.public_key });
    const fresh = await createVault({ keystore: ks, transport: t, org_id: "o", project_id: "p", service_public_key: t.service.public_key });
    // `Repos.init()` (sdk/src/namespaces/repos.ts) sets
  ...
  it("a machine whose keystore does NOT hold the key gets a truthful, enriched refusal — never the word 'foreign'", async () => {
    ...
    const ks2 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-vault-nonholder-")) });
    let caught: unknown;
    try {
      await createVault({ keystore: ks2, transport: t, org_id: "o", project_id: "p", service_public_key: t.service.public_key });
    } catch (e) {
      caught = e;
    }
    ...
    assert.ok(actions.some((a) => /restore the vault keystore/.test(a.action)));
  });
  ...
  it("no allocate() retry and no key material minted on the non-holder path", async () => {
    ...
    const ks2 = VaultKeystore.open({ rootDir: mkdtempSync(join(tmpdir(), "run402-vault-nonholder2-")) });
    await assert.rejects(createVault({ keystore: ks2, transport: t, org_id: "o", project_id: "p", service_public_key: t.service.public_key }));
...
describe("findResumableVaultJournal (repo-first-onramp D2 — client-side resumability)", () => {
  it("finds an incomplete journal matching the exact (org, project) pair", async () => {
    const ks = VaultKeystore.open({ rootDir: root });
    const t = new MemoryTransport();
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "org_a", project_id: "proj_a", client_creation_id: CCID, onStage: (s) => { if (s === "ALLOCATED") throw new Error("crash"); } }));
    const found = findResumableVaultJournal(ks, "org_a", "proj_a");
  ...
  it("never matches a different org or a different project", async () => {
    ...
    const t = new MemoryTransport();
    await assert.rejects(createVault({ keystore: ks, transport: t, org_id: "org_a", project_id: "proj_a", client_creation_id: CCID, onStage: (s) => { if (s === "ALLOCATED") throw new Error("crash"); } }));
    assert.equal(findResumableVaultJournal(ks, "org_b", "proj_a"), null);
    assert.equal(findResumableVaultJournal(ks, "org_a", "proj_b"), null);
  });
  ...
  it("never resumes a REFUSED journal — that would only reproduce the refusal", async () => {
    ...
    assert.equal(findResumableVaultJournal(ks, "o", "p"), null, "a refused attempt is never offered up for reuse");
  });

  it("returns null when nothing is incomplete (a fresh call gets a fresh id)", () => {
    const ks = VaultKeystore.open({ rootDir: root });
    assert.equal(findResumableVaultJournal(ks, "o", "p"), null);
  });
});

describe("vault creation journal — doctor text", () => {
  it("carries the V0-A terminal-loss statement verbatim and the CAS-restore pointer, never implying a receipt decrypts", () => {
    const d = vaultDoctorRecoveryText();
    assert.equal(d.statement, VAULT_TERMINAL_LOSS_STATEMENT);
    assert.equal(d.statement, "whole-machine or whole-keystore loss is terminal for vault history until human envelopes ship");
    assert.equal(d.doctor_text, VAULT_TERMINAL_LOSS_DOCTOR_TEXT);
