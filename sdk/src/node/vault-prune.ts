/**
 * vault — prune (protocol rev 40 §7.3; task 5.12a).
 *
...
import type {
  ...
  VaultCheckpointManifestReceipt,
  VaultCheckpointPackReceipt,
  VaultCheckpointClaimSetReceipt,
  VaultHead,
...
export const VAULT_MAX_PRUNE_CANDIDATES = 10_000;

/** The CLOSED V0 verifier identity set (`common.json#/$defs/implementation_id`). */
export const VAULT_VERIFIER_IMPLEMENTATIONS = ["run402-cli", "r402s-verify"] as const;
export type VaultVerifierImplementation = (typeof VAULT_VERIFIER_IMPLEMENTATIONS)[number];

/** This SDK's identity when it attests. The other half of the pair is never ours to produce. */
export const VAULT_SDK_VERIFIER_IMPLEMENTATION: VaultVerifierImplementation = "run402-cli";
 ...
 */
export const VAULT_SDK_VERIFIER_VERSION = "run402-sdk/r402s-v0-rev40";

/** `common.json#/$defs/receipt_pruneable` — the CLOSED union a delete set may name. */
export type VaultPruneableReceipt =
  | VaultWalPackReceipt
  | VaultRefStateReceipt
  | VaultRetentionRootsReceipt
  | VaultCheckpointManifestReceipt
...
export interface VaultPruneIntentCore {
  format: typeof VAULT_FORMAT;
  ...
  delete_set: VaultPruneableReceipt[];
  nonce: string;
  signature: string;
}

/** `prune_intent.json` — the wrapper the gateway publishes create-only under `prune/`. */
export interface VaultPruneIntent {
  format: typeof VAULT_FORMAT;
  object_kind: "prune_intent";
  suite: typeof VAULT_SUITE;
  repo_id: string;
  object_id: string;
  core: VaultPruneIntentCore;
  intent_core_sha256: string;
  verifier_receipts: VaultVerifierReceiptRef[];
  signature: string;
}

/** One entry of the completion's three-valued vector. */
export interface VaultPruneOutcome {
  object_id: string;
...
export interface VaultPruneIntentRecord {
  object_id: string;
  repo_id: string;
  state: string;
  gc_epoch: string;
  ...
  completion: {
    ...
    sha256: string | null;
    per_object: VaultPruneOutcome[] | null;
...
function receiptsOfHead(head: VaultHead): VaultPruneableReceipt[] {
 ...
 */
export function checkpointRecoveryBundle(entry: VaultChainEntry): VaultPruneableReceipt[] {
  const block = entry.head.checkpoint;
  if (!block) return [];
  const out: VaultPruneableReceipt[] = [block.claim_set, entry.head.ref_state, entry.head.retention_roots];
...
export function collectPruneUniverse(entries: readonly VaultChainEntry[]): VaultPruneableReceipt[] {
  const seen = new Map<string, VaultPruneableReceipt>();
 ...
 */
export function computeGcRootSet(entries: readonly VaultChainEntry[]): VaultGcRootSet {
  ...
  const seen = new Map<string, VaultPruneableReceipt>();
  const protect = (rs: readonly VaultPruneableReceipt[]): void => {
...
export interface VaultPrunePlan {
  /** Candidates in canonical order, capped at {@link VAULT_MAX_PRUNE_CANDIDATES}. */
  candidates: VaultPruneableReceipt[];
  /** How many pruneable objects were left out by the per-intent cap (chunk into a later intent). */
  deferred_count: number;
  root_set: VaultGcRootSet;
  universe_count: number;
}

/** universe − GC root set, canonical order, capped. Never mutates its inputs. */
export function planPruneCandidates(entries: readonly VaultChainEntry[]): VaultPrunePlan {
  ...
  return {
    candidates: eligible.slice(0, VAULT_MAX_PRUNE_CANDIDATES),
    deferred_count: Math.max(0, eligible.length - VAULT_MAX_PRUNE_CANDIDATES),
    root_set: rootSet,
...
export interface VaultRetentionEvolutionCheck {
  ok: boolean;
...
export function checkRetentionEvolution(
  entries: readonly VaultChainEntry[],
  roots: (entry: VaultChainEntry) => Array<{ ref: string; oid: string; dropped_at_generation: string }>,
...
): VaultRetentionEvolutionCheck {
  const ordered = [...entries].sort((a, b) => (a.head.generation < b.head.generation ? -1 : a.head.generation > b.head.generation ? 1 : 0));
  const unproven: VaultRetentionEvolutionCheck["unproven"] = [];
...
export interface VaultBuildPruneCoreInput {
  repo_id: string;
  gc_epoch: string;
  authorizing_head_sha256: string;
  checkpoint_claim_set_sha256: string;
  gc_root_set_hmac: string;
  retention_state_hmac: string;
  delete_set: readonly VaultPruneableReceipt[];
 ...
 */
export function buildPruneIntentCore(input: VaultBuildPruneCoreInput, signingSeed: Uint8Array): VaultPruneIntentCore {
  const deleteSet = sortPruneReceipts(input.delete_set);
  if (deleteSet.length === 0) fail("UPGRADE_REQUIRED", "a prune intent must name at least one candidate", "building the vault prune intent");
  if (deleteSet.length > VAULT_MAX_PRUNE_CANDIDATES) {
    fail("UPGRADE_REQUIRED", `${deleteSet.length} candidates exceed the ${VAULT_MAX_PRUNE_CANDIDATES} per-intent maximum; chunk into successive intents`, "building the vault prune intent", { candidates: deleteSet.length });
  }
  for (let i = 1; i < deleteSet.length; i++) {
    if (!(deleteSet[i - 1]!.object_id < deleteSet[i]!.object_id)) {
      fail("UPGRADE_REQUIRED", `delete_set is not sorted and pairwise-distinct at index ${i}`, "building the vault prune intent", { object_id: deleteSet[i]!.object_id });
    }
  }
  ...
  const unsigned = {
    ...
    object_id: input.object_id ?? newVaultId("pi"),
  ...
  return signVaultObject(unsigned, signingSeed) as VaultPruneIntentCore;
}

/** The stored-bytes hash of the complete signed core — what BOTH verifier receipts sign. */
export function pruneIntentCoreSha256(core: VaultPruneIntentCore): string {
  return storedBytesSha256(core as unknown as VaultSignedObject);
}

export interface VaultBuildVerifierReceiptInput {
  repo_id: string;
  intent_core_sha256: string;
  checkpoint_head_sha256: string;
  cutoff_ticket_sha256: string | null;
 ...
 */
export function buildVerifierReceipt(input: VaultBuildVerifierReceiptInput, signingSeed: Uint8Array): VaultVerifierReceipt {
  const unsigned = {
    format: VAULT_FORMAT,
    object_kind: "verifier_receipt" as const,
    suite: VAULT_SUITE,
    repo_id: input.repo_id,
    object_id: input.object_id ?? newVaultId("vr"),
    ...
    result: (input.retention_evolution_ok && input.candidates_outside_roots_ok ? "restored_and_verified" : "failed") as VaultVerifierReceipt["result"],
  };
  return signVaultObject(unsigned, signingSeed) as VaultVerifierReceipt;
}

/** The `verifier_receipt_ref` an intent carries for an uploaded receipt. */
export function verifierReceiptRef(receipt: VaultVerifierReceipt): VaultVerifierReceiptRef {
  const bytes = storedBytes(receipt as unknown as VaultSignedObject);
  return {
    object_id: receipt.object_id,
    object_kind: "verifier_receipt",
    stored_bytes_sha256: storedBytesSha256(receipt as unknown as VaultSignedObject),
 ...
 */
export function buildPruneIntent(core: VaultPruneIntentCore, receipts: readonly VaultVerifierReceipt[], signingSeed: Uint8Array): VaultPruneIntent {
  const coreSha = pruneIntentCoreSha256(core);
  if (receipts.length !== 2) {
    fail(
      "UPGRADE_REQUIRED",
      `a prune intent carries exactly two verifier receipts, one per implementation (${VAULT_VERIFIER_IMPLEMENTATIONS.join(", ")}); got ${receipts.length}`,
      "building the vault prune intent",
  ...
  for (const id of VAULT_VERIFIER_IMPLEMENTATIONS) {
    const matching = receipts.filter((r) => r.implementation_id === id);
    if (matching.length !== 1) {
      fail("UPGRADE_REQUIRED", `exactly one verifier receipt per implementation is required; ${id} has ${matching.length}`, "building the vault prune intent", { implementation_id: id });
    }
  }
  const [a, b] = receipts as [VaultVerifierReceipt, VaultVerifierReceipt];
  ...
  if (a.object_id === b.object_id || refs[0]!.stored_bytes_sha256 === refs[1]!.stored_bytes_sha256) {
    fail("UPGRADE_REQUIRED", "the two verifier receipts must be distinct objects with distinct hashes", "building the vault prune intent");
  }
  for (const r of receipts) {
    if (r.repo_id !== core.repo_id) fail("VAULT_ACCESS_DENIED", `verifier receipt ${r.object_id} attests a different vault`, "building the vault prune intent", { repo_id: r.repo_id });
    if (r.intent_core_sha256 !== coreSha) {
      fail("UPGRADE_REQUIRED", `verifier receipt ${r.object_id} signs a different intent core (${r.intent_core_sha256} ≠ ${coreSha})`, "building the vault prune intent", { object_id: r.object_id });
    }
    if (r.result !== "restored_and_verified" || r.retention_evolution_ok !== true || r.candidates_outside_roots_ok !== true) {
      fail("UPGRADE_REQUIRED", `verifier receipt ${r.object_id} does not attest restored_and_verified with both attestations true`, "building the vault prune intent", {
  ...
  return signVaultObject(unsigned, signingSeed) as VaultPruneIntent;
}
...
export function assertPruneCoreStillCurrent(
  ...
  if (core.repo_id !== current.repo_id) {
    fail("VAULT_ACCESS_DENIED", `the supplied intent core belongs to ${core.repo_id}, not ${current.repo_id}`, "submitting the vault prune intent", { repo_id: core.repo_id });
  }
 ...
 */
export function pruneIntentBytes(intent: VaultPruneIntent): Uint8Array {
  return storedBytes(intent as unknown as VaultSignedObject);
}

// ─── Believing the completion, and nothing else ──────────────────────────────

export interface VaultPruneConfirmation {
 ...
 */
export function summarizePruneCompletion(candidateIds: readonly string[], record: VaultPruneIntentRecord | null): VaultPruneConfirmation {
  ...
  const present: VaultPruneOutcome[] = [];
