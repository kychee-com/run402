/**
 * vault D6 — push-to-create (repo-first-onramp task 4.5).
 *
 * `pushToCreateVault` is the primitive the remote helper and `vault
 ...
 * (design D6), atomically, through the gateway's `POST /vaults/v1
 ...
 * ({@link createVault}) drives it exactly as it drives an ordinary
 ...
 *      ({@link VaultTransport.findVaultByRepo})? If so this is an
...
import { createVault, findResumablePushToCreateJournal } from "./vault-creation-journal.js";
import type { VaultKeystore } from "./vault-keystore.js";
import type { VaultTransport } from "./vault-publication.js";

export interface PushToCreateVaultOptions {
...
export type PushToCreateVaultResult =
  ...
  | {
      ...
      org_id: string;
      created: {
        /** Mirrors `VaultCreationResult.how === "reconciled"`: an existing local journal was resumed to completion, nothing re-minted. */
 ...
 */
async function pollFindVaultByProject(transport: VaultTransport, projectId: string, attempts = 20, intervalMs = 50) {
 ...
 */
export async function pushToCreateVault(options: PushToCreateVaultOptions): Promise<PushToCreateVaultResult> {
