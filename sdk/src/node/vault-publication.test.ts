/**
 * vault publication (task 5.4) — the §6.1 ref-transaction semantics and
...
import { loadVaultVectors, OPTOUT_SKIP_MESSAGE, type VaultVector } from "./vault-vectors.test-helper.js";
...
import { generateSigningKeypair, hexToBytes, jcs, parseVaultStrict, sha256Hex, signVaultObject, toBase64url } from "../namespaces/vault.crypto.js";
import type { VaultHead, VaultHeadsListingPage, VaultHeadsListingRequest, VaultRetentionRoot } from "../namespaces/vault.types.js";
import {
  VAULT_MAX_CANONICAL_REFS,
  VAULT_MAX_REF_UPDATES_PER_TRANSACTION,
  VAULT_PACK_SCRATCH_PREFIX,
  ...
  type VaultListingProgress,
  type VaultRefMap,
} from "./vault-publication.js";
import { VAULT_DEPLOY_REF, hardenedGit, hasObject } from "./vault-snapshot.js";
import { commitFile, git, makeRepo, makeVault, type VaultFixture } from "./vault-memory-transport.test.js";
import { VaultKeystore } from "./vault-keystore.js";
import { applyAddWriterKey, initialWriterState, writerKeyIdOf } from "./vault-writer-state.js";

// ─── Vector loading (same contract as the crypto suite) ──────────────────────

// Task 5.6b: missing vectors FAIL; only VAULT_VECTORS_OPTOUT=1 skips.
const vectorSet = loadVaultVectors();
type Vector = VaultVector;
const vectorFile = vectorSet?.file ?? null;
const vectors = vectorFile ? describe : describe.skip;
if (!vectorFile) describe("vault publication vectors", () => it.skip(OPTOUT_SKIP_MESSAGE, () => {}));
...
describe("§6.1 ref transactions", () => {
  ...
  it("a non-force branch update must be BOTH fast-forward and expected-old-matching", async () => {
    const cur: VaultRefMap = { "refs/heads/main": OID(1) };
  ...
  it("a non-fast-forward refusal names the working force spelling; a bare expected-old mismatch does not (vault-force-spelling-and-pin-fold)", async () => {
    const cur: VaultRefMap = { "refs/heads/main": OID(1) };
  ...
  it("tags are immutable without force — and the ancestry oracle is never consulted for a tag", async () => {
    const cur: VaultRefMap = { "refs/tags/v1": OID(1) };
  ...
  it("refs/run402/* is protocol-owned: user pushes refuse, the protocol's own deploy move is allowed", async () => {
    ...
    assert.deepEqual(tx, { updates: [{ ref: VAULT_DEPLOY_REF, expected_old_oid: null, new_oid: OID(1), force: false }] });
    ...
    assert.deepEqual(allowed.refs, { [VAULT_DEPLOY_REF]: OID(1) });
    ...
    assert.deepEqual(move.updates[0], { ref: VAULT_DEPLOY_REF, expected_old_oid: OID(1), new_oid: OID(2), force: true });
    const moved = await evaluateRefTransaction(allowed.refs, move, { isAncestor: () => false, protocol_refs: "allow" });
    assert.deepEqual(moved.dropped, [{ ref: VAULT_DEPLOY_REF, oid: OID(1), reason: "force_displaced" }]);
  });
   ...
   *
   * That is exactly what `repos.capture()` did: it built the deploy-ref move and
   * left the option off, making `run402 repos capture` a dead command while the
  ...
  it("cardinality bounds (§6.5 H10) refuse BEFORE acceptance: >1000 updates, >10 000 resulting refs", async () => {
    const many = Array.from({ length: VAULT_MAX_REF_UPDATES_PER_TRANSACTION + 1 }, (_v, i) => ({ ref: `refs/heads/b${i}`, expected_old_oid: null, new_oid: OID(1), force: false }));
    await rejectsCode(evaluateRefTransaction({}, { updates: many }, ff), "REF_STATE_LIMIT_EXCEEDED");
    const full: VaultRefMap = {};
    for (let i = 0; i < VAULT_MAX_CANONICAL_REFS; i++) full[`refs/heads/b${i}`] = OID(1);
...
vectors("§4.5 retention roots — vector class `retention-schedule`", () => {
  ...
  it("evolution: roots(g+1) = roots(g) ∪ dropped (RENEWING an existing (ref,oid) key), sorted; expiry only at a checkpoint", () => {
    for (const v of byClass("retention-schedule")) {
      if (v.inputs.retention_cutoff_ticket_cutoff_at) continue;
      mark(v);
      const out = evolveRetentionRoots(v.inputs.roots_g as VaultRetentionRoot[], { generation: v.inputs.g_plus_1 ?? "0000000000000002", dropped: v.inputs.dropped_by_g_plus_1 ?? [] });
  ...
  it("a non-checkpoint generation can never remove a root, however expired", () => {
    const roots: VaultRetentionRoot[] = [{ ref: "refs/heads/old", oid: OID(1), dropped_at_generation: "0000000000000001" }];
  ...
  it("at a checkpoint expiry is PERMISSIVE: an unresolvable effective_admitted_at RETAINS the root", () => {
    const roots: VaultRetentionRoot[] = [
  ...
  it("more than 50 000 roots is refused before acceptance", () => {
    const roots: VaultRetentionRoot[] = Array.from({ length: 50_000 }, (_v, i) => ({ ref: `refs/heads/r${i}`, oid: OID(i + 1), dropped_at_generation: "0000000000000001" }));
...
vectors("§6.3 heads listing — vector class `heads_listing_pagination`", () => {
  ...
  function serverCursorCheck(v: Vector): void {
    const req = v.inputs.request as VaultHeadsListingRequest | null;
  ...
  function progressThroughPriorPages(v: Vector): VaultListingProgress {
    const anchor = anchorOf(v);
    let progress: VaultListingProgress = { after_generation: anchor, last_generation: anchor, delivered: 0 };
    for (const page of (v.inputs.prior_pages ?? []) as VaultHeadsListingPage[]) {
  ...
  it("replays every pagination vector: request validation, cursor binding, coupling, gaplessness, truthful total", () => {
    for (const v of byClass("heads_listing_pagination")) {
      ...
      if (v.expected.request_schema_valid === "false") {
        ...
        throwsCode(() => validateHeadsListingRequest(req!), v.reject_code ?? "VAULT_LISTING_REQUEST_INVALID", `${v.id}: ${v.description}`);
        continue;
      }
      ...
      const page = v.inputs.response as VaultHeadsListingPage | null;
      ...
      if (v.expect_reject === "true") {
        const expected = v.reject_code ?? (v.reject_reason === "generation-not-above-anchor" ? "GENERATION_REGRESSION" : v.reject_reason === "total-untruthful" ? "CHAIN_BROKEN" : "VAULT_LISTING_PAGE_INVALID");
  ...
  it("the continuation echoes the cursor UNCHANGED and never re-anchors (the 007/008 refusals are what editing earns)", () => {
    const page1 = byClass("heads_listing_pagination").find((v) => v.id === "heads-page-001")!.inputs.response as VaultHeadsListingPage;
    const drift = byClass("heads_listing_pagination").find((v) => v.id === "heads-page-007")!.inputs.request as VaultHeadsListingRequest;
    const tampered = byClass("heads_listing_pagination").find((v) => v.id === "heads-page-008")!.inputs.request as VaultHeadsListingRequest;
    ...
    const final = byClass("heads_listing_pagination").find((v) => v.id === "heads-page-003")!.inputs.response as VaultHeadsListingPage;
...
vectors("§6.4 pins + transitions — vector classes `chain` (005) and `transition-fail-closed`", () => {
  ...
  it("an ADMITTED non-null transition fails a V0 client CLOSED with UPGRADE_REQUIRED; an unknown kind is a closed-enum reject — EXCEPT rotate_epoch (D193, rev 42) and add_writer_key (vault-multi-writer, rev 47), both ACTIVATED after this frozen vector set was authored", () => {
    for (const v of byClass("transition-fail-closed")) {
      ...
      if (v.id === "transition-007") {
        ...
        const rotationHead = { generation: "0000000000000002", transition: { kind: v.inputs.admitted_head_transition_kind, payload_format: "base64url-jcs", payload: "e30", payload_sha256: "0".repeat(64) } } as unknown as VaultHead;
      ...
      const head = v.inputs.object as VaultHead;
      ...
      if (head.transition.kind === "add_writer_key") {
        assertNoTransition(head); // vault-multi-writer, rev 47: no longer fail-closed — this vector (transition-003) predates activation, same precedent as rotate_epoch above
        continue;
      }
  ...
  it("the fence recomputes the binding from the record's OWN fields; a same-open-id retry with a different binding is CLIENT_OPEN_ID_CONFLICT", () => {
    for (const v of byClass("request-to-c1-binding")) {
      ...
      if (v.inputs.c1_record) {
        ...
        else throwsCode(() => checkOpenBinding(v.inputs.client_open_id, v.inputs.c1_record, v.inputs.issuance_open_binding_sha256), "VAULT_OPEN_BINDING_MISMATCH", `${v.id}: ${v.description}`);
        continue;
      }
...
describe("§6.2 push — the complete publication path", () => {
  ...
  it("a finalization receipt that does not match the local manifest refuses BEFORE the head is signed", async (t) => {
    ...
    await rejectsCode(f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: main, force: false }] } }), "VAULT_RECEIPT_MISMATCH");
  ...
  it("an admitted head that does not read back is NOT reported as landed and advances no pin", async (t) => {
    ...
    await rejectsCode(f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: main, force: false }] } }), "VAULT_HEAD_READBACK_MISMATCH");
  ...
  it("a lost admission race re-verifies from storage, re-applies to the WINNER's map, and retries", async (t) => {
    ...
    const other = new Vault({ keystore: f.keystore, transport: f.transport, repo_id: f.repoId, repo_dir: f.repoDir });
  ...
  it("design D1: a supplied base skips push's own materialize entirely on a clean first attempt", async (t) => {
    ...
    const { VaultOpCounter, countingVaultTransport } = await import("./vault-transport-counter.test-helper.js");
    const counter = new VaultOpCounter();
    const counted = new Vault({ keystore: f.keystore, transport: countingVaultTransport(f.transport, counter), repo_id: f.repoId, repo_dir: f.repoDir });
    const c2 = await commitFile(f.repoDir, "b.txt", "b\n");
  ...
  it("the verification budget is RESUMABLE: the verified prefix persists and the next call continues", async (t) => {
    ...
    const budgeted = new Vault({ keystore: f.keystore, transport: f.transport, repo_id: f.repoId, repo_dir: f.repoDir, verification_budget: 1 });
  ...
  it("an authenticated-but-undecryptable carrier is CHAIN_UNUSABLE (read-only at the materialized pin)", async (t) => {
    ...
    const main = await git(f.repoDir, ["rev-parse", "HEAD"]);
    const r = await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: main, force: false }] } });
    f.transport.objects.delete(`${f.repoId}/${vaultPaths.refState(r.head.ref_state.object_id)}`);
  ...
  it("a vault that lost a generation this client authenticated is a GENERATION_REGRESSION, not a broken link", async (t) => {
    ...
    f.transport.objects.delete(`${f.repoId}/${vaultPaths.head("0000000000000001")}`);
  ...
  it("a head whose stored bytes were substituted after admission is CHAIN_BROKEN", async (t) => {
    ...
    const main = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: main, force: false }] } });
    const key = `${f.repoId}/${vaultPaths.head("0000000000000001")}`;
  ...
  it("detached HEAD is representable end to end and its commit is covered", async (t) => {
    ...
    assert.deepEqual(Vault.coverageTips(r.refs, [], { kind: "detached", oid: main }), [main]);
  });
});
...
describe("§4.7 checkpoint sets", () => {
  it("a forced checkpoint head carries an EMPTY WAL set, an owner-signed claim set, and passes acceptance from the set ALONE", async (t) => {
    ...
    const stored = f.transport.objects.get(`${f.repoId}/${vaultPaths.claimSet(r.head.checkpoint!.claim_set.object_id)}`)!;
...
describe("buildPacks scratch dir — on the object store's own filesystem, never os.tmpdir() (EXDEV in containers)", () => {
  it("allocatePackScratchDir lands INSIDE the git common dir, dotted and clearly named", async (t) => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-packs-scratch-"));
    ...
    assert.ok(basename(scratch).startsWith(VAULT_PACK_SCRATCH_PREFIX), `dotted, clearly named: ${basename(scratch)}`);
  ...
  it("buildPacks produces a pack from a repository and leaves no scratch dir behind; a crash leftover does not break git fsck", async (t) => {
    ...
    const leftovers = () => readdirSync(commonDir).filter((n) => n.startsWith(VAULT_PACK_SCRATCH_PREFIX));
    ...
    const stale = join(commonDir, `${VAULT_PACK_SCRATCH_PREFIX}crash`);
    mkdirSync(stale);
...
describe("§4.3 repair + restore", () => {
  ...
  it("restore pulls the newest checkpoint plus every later WAL pack and proves the whole coverage set resolves", async (t) => {
    ...
    const target = mkdtempSync(join(tmpdir(), "run402-vault-restore-"));
...
// ─── local object cache (vault-client-round-trips design D3) ──────────────
...
describe("local immutable-object cache — re-verified on every use", () => {
  it("a tampered cached head is discarded and refetched — the refetch is verified exactly as a cold read", async (t) => {
    ...
    const targetA = mkdtempSync(join(tmpdir(), "run402-vault-cache-tamper-a-"));
    ...
    const targetB = mkdtempSync(join(tmpdir(), "run402-vault-cache-tamper-b-"));
  ...
  it("a cold cache (nothing ever cached) degrades to plain network reads with no user-visible failure", async (t) => {
    ...
    const fresh = new Vault({ keystore: f.keystore, transport: f.transport, repo_id: f.repoId, repo_dir: f.repoDir });
    const state = await fresh.materialize();
    assert.deepEqual(state.refs, { "refs/heads/main": c1 });
  });
});

// ─── incremental restore (vault-client-round-trips design D5) ─────────────
...
describe("restoreObjectsInto — incremental above restored_through", () => {
  it("a one-generation pull replays only the new generation's pack, and advances the marker", async (t) => {
    const f = await makeVault();
    // vault-delta-fetch: this test pins the FALLBACK wire mechanism
    ...
    const target = mkdtempSync(join(tmpdir(), "run402-vault-restore-incr-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    ...
    const markerAfterFirst = await readVaultRestoreMarker(target);
    ...
    const markerAfterSecond = await readVaultRestoreMarker(target);
    assert.deepEqual(markerAfterSecond, { generation: g2.generation, head_sha256: g2.head_sha256 });
  });

  it("a fetch that is already up to date makes no pack read, and only ONE state read (vault-composite-state-read D1)", async (t) => {
    const f = await makeVault();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-restore-uptodate-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    ...
    // Pre-vault-composite-state-read, the "server still holds the pin"
    ...
    // `Vault.tryStateFastPath`), so a dedicated head read is no
  ...
  it("a repair landing above the marker forces the wholesale path, and still restores the correct state", async (t) => {
    ...
    const target = mkdtempSync(join(tmpdir(), "run402-vault-restore-repair-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    ...
    const fresh = mkdtempSync(join(tmpdir(), "run402-vault-restore-fresh-"));
    ...
    assert.equal(await hasObject(target, c1), true);
    const markerAfter = await readVaultRestoreMarker(target);
  ...
  it("an interrupted restore leaves the marker unadvanced, and the next fetch heals", async (t) => {
    ...
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-restore-interrupt-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    ...
    const markerAfterFailure = await readVaultRestoreMarker(target);
...
// ─── vault-session-state-reuse: the list-phase-state handoff into fetch ───

describe("restoreObjectsInto — session-state reuse (vault-session-state-reuse)", () => {
  it("a matching reuse handoff skips this method's own state read entirely", async (t) => {
    const f = await makeVault();
    t.after(() => rmSync(f.root, { recursive: true, force: true }));
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-reuse-match-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    ...
    const marker = await readVaultRestoreMarker(target);
  ...
  it("a marker mismatch (a different, or stale, handoff) falls back to this method's own read — never a wrong result", async (t) => {
    ...
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-reuse-mismatch-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    await f.vault.restoreObjectsInto(target); // marker at generation 1
  ...
  it("with no reuse argument, behavior is byte-identical to before this change (its own state read runs)", async (t) => {
    ...
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-reuse-absent-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    const callsBefore = f.transport.calls.length;
...
// ─── vault-checkpoint-cadence: the local auto-gc threshold config ─────────

describe("readVaultAutoGcThreshold / writeVaultAutoGcThreshold (vault-checkpoint-cadence design D1)", () => {
  async function tempRepo(): Promise<string> {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-auto-gc-cfg-"));
  ...
  it("reads the default when unset", async () => {
    ...
    try {
      assert.equal(await readVaultAutoGcThreshold(dir), VAULT_AUTO_GC_GENERATIONS_DEFAULT);
    } finally {
  ...
  it("round-trips a written value, including 0 (disabled)", async () => {
    const dir = await tempRepo();
    try {
      await writeVaultAutoGcThreshold(dir, 10);
      assert.equal(await readVaultAutoGcThreshold(dir), 10);
      await writeVaultAutoGcThreshold(dir, 0);
      assert.equal(await readVaultAutoGcThreshold(dir), 0);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("is scoped to the target directory, never to the vault/repo_id (like the restore marker)", async () => {
    ...
    try {
      await writeVaultAutoGcThreshold(dirA, 5);
      assert.equal(await readVaultAutoGcThreshold(dirA), 5);
      assert.equal(await readVaultAutoGcThreshold(dirB), VAULT_AUTO_GC_GENERATIONS_DEFAULT);
    } finally {
...
async function makeRetainTestRepo(): Promise<string> {
  const dir = mkdtempSync(join(tmpdir(), "run402-vault-retain-repo-"));
...
describe("clone-installs-retained-refs — refs/r402/retain/* reconciliation", () => {
  it("restoreObjectsInto installs a retained ref for a branch-unreachable, force-displaced tip; git fsck is silent", async (t) => {
    ...
    const target = mkdtempSync(join(tmpdir(), "run402-vault-retain-clone-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    const restored = await f.vault.restoreObjectsInto(target);
    ...
    assert.deepEqual(restored.refs, { "refs/heads/main": c2 });
    const retainRef = vaultRetainedRefName(c1);
  ...
  it("skips a retained tip already reachable from a canonical ref — no redundant ref (D2)", async (t) => {
    ...
    const listed = await git(repoDir, ["for-each-ref", VAULT_RETAIN_REF_PREFIX]);
    assert.equal(listed, "");
  });
  ...
  it("a vault-canonical protocol ref's tip (refs/run402/*) joins the retain set — git never writes it locally (live-acceptance catch)", async (t) => {
    ...
    assert.equal(await git(repoDir, ["rev-parse", `${VAULT_RETAIN_REF_PREFIX}${capture}`]), capture);
    // And main's own tip gained no redundant ref.
    const listed = await git(repoDir, ["for-each-ref", "--format=%(refname)", VAULT_RETAIN_REF_PREFIX]);
    assert.deepEqual(listed.split("\n").filter(Boolean), [`${VAULT_RETAIN_REF_PREFIX}${capture}`]);
  });
  ...
  it("adds refs for every retained tip, then retracts one on the next reconcile — namespace-scoped, nothing else touched", async (t) => {
    ...
    assert.deepEqual(new Set(first.written), new Set([vaultRetainedRefName(oidX), vaultRetainedRefName(oidY)]));
    ...
    assert.deepEqual(second.deleted, [vaultRetainedRefName(oidX)]);
    assert.equal(second.retained_count, 1);

    assert.equal(await git(repoDir, ["rev-parse", vaultRetainedRefName(oidY)]), oidY, "Y's ref survives — only X's own root was pruned");
    await assert.rejects(git(repoDir, ["rev-parse", "--verify", vaultRetainedRefName(oidX)]), "X's ref was retracted");
  ...
  it("is a silent no-op — not a warning — when repoDir names no git repository at all", async (t) => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-retain-norepo-"));
...
// ─── vault-multi-writer rev 47 — verifyToNewest actually accepts a second writer ──
//
...
// (already covered exhaustively in vault-writer-state.test.ts).
describe("vault-multi-writer rev 47 — a second writer's head is accepted, a stranger's is not", () => {
  const fixtures: VaultFixture[] = [];
  const open = async (): Promise<VaultFixture> => {
    const f = await makeVault();
    fixtures.push(f);
  ...
  function buildNextHead(base: { generation: string; head_sha256: string; head: VaultHead }, seed: Uint8Array, writerKeyId: string, transition: VaultHead["transition"]): { stored_bytes: Uint8Array; stored_bytes_sha256: string; generation: string } {
    ...
    const signed = signVaultObject(headWithoutSig, seed);
  ...
  it("an existing writer adds a second writer; the second writer's OWN later push is then accepted; a stranger's push is not", async (t) => {
    ...
    const gen2Result = { generation: gen2.generation, head_sha256: gen2.stored_bytes_sha256, head: parseVaultStrict(new TextDecoder().decode(gen2.stored_bytes)) as VaultHead };
    ...
    const gen3Result = { generation: gen3.generation, head_sha256: gen3.stored_bytes_sha256, head: parseVaultStrict(new TextDecoder().decode(gen3.stored_bytes)) as VaultHead };
    ...
    await rejectsCode(f.vault.verifyToNewest(), "VAULT_WRITER_NOT_ADMITTED");
  });
});

describe("vault-multi-writer (task 5.8) — the push pre-check refuses locally, before any network work", () => {
  const fixtures: VaultFixture[] = [];
  const open = async (): Promise<VaultFixture> => {
    const f = await makeVault();
    fixtures.push(f);
  ...
  it("a stranger's own push() is refused VAULT_WRITER_NOT_ADMITTED, with zero objects uploaded and zero heads admitted", async (t) => {
    t.after(cleanup);
    const f = await open();
    const main = await git(f.repoDir, ["rev-parse", "HEAD"]);
    ...
    // the "stranger" (same shape as vault-clone-scaling's coldFixture).
    const strangerKeystore = VaultKeystore.open({ rootDir: join(f.root, "ks-stranger") });
    ...
    const stranger = Vault.open({ keystore: strangerKeystore, transport: f.transport, repo_id: f.repoId, repo_dir: f.repoDir });
    ...
    assert.equal(codeOf(thrown), "VAULT_WRITER_NOT_ADMITTED");
  ...
  it("an ADMITTED writer surviving an ORDINARY CAS conflict (nothing removed) is not falsely refused on retry", async (t) => {
    ...
    // attempt should throw VAULT_WRITER_NOT_ADMITTED/VAULT_WRITER_REMOVED.
...
vectors("vector coverage tally", () => {
  it("every vector of each fully-replayed class was exercised", () => {
    ...
    console.log(`vault publication vectors rev ${vectorFile!["x-r402s-revision"]}: ${JSON.stringify(summary)}`);
  });
});

// ─── vault-clone-scaling: page-batched walk fidelity + checkpoint staleness ─

describe("vault-clone-scaling — the batched page walk keeps unbatched failure fidelity", () => {
  ...
  const coldFixture = async () => {
    ...
    const repo = f.keystore.readRepo(f.repoId)!;
    const coldKeystore = VaultKeystore.open({ rootDir: join(f.root, "ks-cold") });
  ...
  it("a head whose stored bytes were substituted fails CHAIN_BROKEN through the batch, exactly as unbatched", async (t) => {
    ...
    const key = `${f.repoId}/${vaultPaths.head("0000000000000002")}`;
    ...
    const cold = Vault.open({ keystore: f.coldKeystore, transport: f.transport, repo_id: f.repoId, repo_dir: null });
  ...
  it("a prefetch that LIES about one head (bytes ≠ the listing's hash) self-heals: the entry stays unserved and the ordered walk's own read succeeds", async (t) => {
    ...
    // (vault-batched-head-reads): on a gateway serving the batch, the
    ...
    // own lying case is covered in `vault-round-trip-budgets.test.ts`.
    // Same for the state delta (vault-delta-fetch): it would blind-warm
    // this head's cache before the walk ever issued the lying single.
    f.transport.headReadsUnsupported = true;
    f.transport.deltaDisabled = true;
    const lyingPath = vaultPaths.head("0000000000000002");
    let lied = false;
    ...
    const cold = Vault.open({ keystore: f.coldKeystore, transport: lying, repo_id: f.repoId, repo_dir: null });
    f.transport.calls.length = 0;
...
describe("vault-clone-scaling — the restore walk's stop is first-hand coverage", () => {
  it("a wholesale restore stopping at a checkpoint head records its coverage, so the next walk's window is exact", async (t) => {
    ...
    const repo = f.keystore.readRepo(f.repoId)!;
    const ks2 = VaultKeystore.open({ rootDir: join(f.root, "ks-restorer") });
    ks2.ensureIdentity();
    ks2.saveRepo({
      repo_id: f.repoId, org_id: repo.org_id, project_id: repo.project_id,
      k_repo_hex: repo.k_repo_hex, epoch: repo.epoch, genesis_sha256: repo.genesis_sha256,
    ...
    const target = mkdtempSync(join(tmpdir(), "run402-vault-coverage-restore-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    await git(target, ["init", "-q", "--bare", "."]);
    const restorer = Vault.open({ keystore: ks2, transport: f.transport, repo_id: f.repoId, repo_dir: null });
...
// ─── pipelined restore (vault-pipelined-restore) ──────────────────────────

describe("pipelined restore — per-object settlement (vault-pipelined-restore 3.1)", () => {
  /**
  ...
  it("applies in order and lands the identical state when objects settle out of order", async (t) => {
    ...
    const piped = new Vault({ keystore: f.keystore, transport: outOfOrderSettled(f.transport), repo_id: f.repoId, repo_dir: f.repoDir });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-piped-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
  ...
  it("a corrupted middle pack fails with the SAME envelope as the barrier path", async (t) => {
    ...
    const barrierVault = new Vault({ keystore: f.keystore, transport: corruptBarrier, repo_id: f.repoId, repo_dir: f.repoDir });
    const targetA = mkdtempSync(join(tmpdir(), "run402-vault-piped-corrupt-a-"));
    t.after(() => rmSync(targetA, { recursive: true, force: true }));
    mkdirSync(targetA, { recursive: true });
    await git(targetA, ["init", "-q", "--bare", "."]);
    let barrierCode = "";
    ...
    const pipedVault = new Vault({ keystore: f.keystore, transport: outOfOrderSettled(f.transport, 1), repo_id: f.repoId, repo_dir: f.repoDir });
    const targetB = mkdtempSync(join(tmpdir(), "run402-vault-piped-corrupt-b-"));
    t.after(() => rmSync(targetB, { recursive: true, force: true }));
    mkdirSync(targetB, { recursive: true });
    await git(targetB, ["init", "-q", "--bare", "."]);
    let pipedCode = "";
...
// ─── delta-first pull (vault-delta-fetch) ─────────────────────────────────
...
describe("delta-first pull — a lying delta falls back with identical outcomes", () => {
  it("corrupt delta packs and self-inconsistent delta heads are misses, never applied", async (t) => {
    ...
    const c1 = await git(f.repoDir, ["rev-parse", "HEAD"]);
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    const target = mkdtempSync(join(tmpdir(), "run402-vault-lying-delta-"));
    t.after(() => rmSync(target, { recursive: true, force: true }));
    mkdirSync(target, { recursive: true });
    await git(target, ["init", "-q", "--bare", "."]);
    await f.vault.restoreObjectsInto(target);
    ...
    const vault = new Vault({ keystore: f.keystore, transport: lying, repo_id: f.repoId, repo_dir: f.repoDir });
...
describe("checkpoint coverage another writer pushed (vault-multi-writer)", () => {
  it("a checkpoint built from a checkout that lacks a covered tip restores it from the vault first", async () => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-ckpt-restore-"));
    try {
      ...
      const vault2 = Vault.open({ keystore: f.keystore, transport: f.transport, repo_id: f.repoId, repo_dir: other });
