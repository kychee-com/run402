...
import { vaultWithoutSignature, jcs, keyFingerprintHex, sha256Hex, signaturePreimage, toBase64url } from "../namespaces/vault.crypto.js";
import {
  applyAddWriterKey,
...
function buildHandoffAuthorization(opts: {
  ...
  grant.signature = sign(opts.grantor.seed, signaturePreimage("writer_admission_grant", vaultWithoutSignature(grant) as Record<string, unknown>));
...
describe("vault writer-state (protocol §4.15-§4.18, D221-D228) — client mirror of the gateway module", () => {
  ...
  describe('authorization.kind:"writer" — the membership-driven default door (D223/D224)', () => {
    ...
    it("rejects: an unknown signer with no add_writer_key transition — VAULT_WRITER_NOT_ADMITTED for a signer that ISN'T the added key either", () => {
      const creator = keypair();
      const member = keypair();
      ...
      assert.equal(v.ok, false);
      if (!v.ok) assert.equal(v.code, "VAULT_WRITER_NOT_ADMITTED");
    });
  ...
  describe("meetsWriterEligibleRole (developer or above)", () => {
    it("developer, admin, owner meet the threshold; billing, viewer, unknown, null, undefined do not", async () => {
      const { meetsWriterEligibleRole } = await import("./vault-writer-state.js");
  ...
  describe("predictMintedRole (vault-multi-writer task 5.5 — mirrors mintHandoff's own attenuation formula)", () => {
    it("no requested role: predicts the minter's own role, for every role", async () => {
      const { predictMintedRole } = await import("./vault-writer-state.js");
  ...
  describe("buildAddWriterKeyActivationPayload (vault-multi-writer task 5.6 — Vault.submitWriterActivationHead's pure assembly step)", () => {
    ...
    it("accepts a bare {version, sha256, writers} predecessor pin — the exact shape VaultRepoFile.writer_set_pin persists, no WriterChainState (with its Sets) required", () => {
      const grantor = keypair();
      const added = keypair();
      const base = initialWriterState(REPO_ID, genesisFor(grantor));
  ...
  describe("buildWriterDoorAddWriterKeyPayload (vault-multi-writer task 5.7 — Vault.admitPendingWriter's pure assembly step)", () => {
