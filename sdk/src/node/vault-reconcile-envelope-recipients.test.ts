/**
 * vault-human-envelopes task 4.1 — the ADD-path workaround's unit
 * coverage. Exercises `Vault.reconcileEnvelopeRecipients` against
 * the in-memory transport fixture (`vault-memory-transport.test.ts`):
 ...
 * conflict, and the `VAULT_READ_ONLY` refusal for a principal with no
 * local signing key.
 */
...
import type { VaultKeyEnvelope } from "../namespaces/vault.types.js";
import { vaultPaths, type VaultOrgEncryptionKeyEntry } from "./vault-publication.js";
import { makeVault } from "./vault-memory-transport.test.js";
...
function principalEntry(entry: { principal_id: string; ek_fingerprint: string; public_key?: string }): VaultOrgEncryptionKeyEntry {
  return { display_name: null, suite: "r402s-1", created_at: "2026-08-26T00:00:00.000Z", ...entry };
}

describe("Vault.reconcileEnvelopeRecipients — vault-human-envelopes task 4.1", () => {
  it("wraps a missing recipient and leaves an already-covered one alone; the sealed envelope opens to the SAME K_repo the vault holds", async () => {
    ...
    assert.equal(result.epoch, VAULT_GENESIS_EPOCH);
    ...
    const bytes = await v.transport.getObject({ repo_id: v.repoId, path: vaultPaths.envelope(result.epoch, bFp) });
    ...
    assert.equal(envelope.epoch, VAULT_GENESIS_EPOCH);
  ...
  it("pins a recipient's fingerprint at first wrap (TOFU), and REFUSES — never silently re-wraps — when the directory later names a different key for the same principal_id", async () => {
    ...
    const bytes = await v.transport.getObject({ repo_id: v.repoId, path: vaultPaths.envelope(second.epoch, b2Fp) });
  ...
  it("treats a concurrent writer's DIFFERENT valid wrap of the same recipient as a benign race ONLY after verifying the winner (vault-agent-envelopes, consult 7.6)", async () => {
    const v = await makeVault();
    const b = generateEncryptionKeypair();
    const bFp = ekFingerprint(b.public_key);
    v.transport.orgEncryptionKeys.set("org_1", [principalEntry({ principal_id: "principal_b", ek_fingerprint: bFp, public_key: toBase64url(b.public_key) })]);
    ...
    const winner = await sealKeyEnvelope({ k_repo: hexToBytes(repo.k_repo_hex), repo_id: v.repoId, epoch: VAULT_GENESIS_EPOCH, recipient_public_key: b.public_key, signer, created_at: formatVaultTimestamp(new Date()) });
    v.transport.objects.set(`${v.repoId}/${vaultPaths.envelope(VAULT_GENESIS_EPOCH, bFp)}`, winner.stored_bytes);
  ...
  it("a race whose winner cannot be verified is FATAL (VAULT_ENVELOPE_ALTERED) — a conflicting immutable object that does not verify is tampering evidence, never a benign skip (consult round 2 §7)", async () => {
    const v = await makeVault();
    const b = generateEncryptionKeypair();
    const bFp = ekFingerprint(b.public_key);
    v.transport.orgEncryptionKeys.set("org_1", [principalEntry({ principal_id: "principal_b", ek_fingerprint: bFp, public_key: toBase64url(b.public_key) })]);
    ...
    v.transport.objects.set(`${v.repoId}/${vaultPaths.envelope(VAULT_GENESIS_EPOCH, bFp)}`, new TextEncoder().encode(JSON.stringify({ raced: true })));

    await assert.rejects(
      () => v.vault.reconcileEnvelopeRecipients(),
      (e: unknown) => (e as { code?: string }).code === "VAULT_ENVELOPE_ALTERED",
    );
    assert.deepEqual(v.keystore.readRepo(v.repoId)!.envelope_recipient_pins ?? {}, {});
  });

  it("VAULT_READ_ONLY when this principal holds no local signing key — refused before any network read", async () => {
    const v = await makeVault();
    ...
    await rejectsCode(v.vault.reconcileEnvelopeRecipients(), "VAULT_READ_ONLY");
