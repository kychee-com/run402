 ...
 * `services/vault/writer-state.ts` (task vault-multi-writer 2.3) — same
...
/**
 * vault-multi-writer rev 47 (task 5.5) — predicts the role a handoff mint
 * will actually confer, mirroring the gateway's OWN attenuation exactly
 * (`services/vault/claims.ts mintHandoff`: `requestedRole &&
...
export function writerKeyIdOf(signingPubkeyB64u: string): string | null {
  if (!VAULT_B64U_32_RE.test(signingPubkeyB64u) || !isCanonicalBase64url(signingPubkeyB64u)) return null;
...
function verifyDetachedB64u(publicKeyB64u: string, signatureB64u: string, message: Uint8Array): boolean {
  if (!VAULT_B64U_32_RE.test(publicKeyB64u) || !isCanonicalBase64url(publicKeyB64u)) return false;
  if (!VAULT_B64U_64_RE.test(signatureB64u) || !isCanonicalBase64url(signatureB64u)) return false;
 ...
 */
/**
 * vault-multi-writer (task 5.6) — builds a well-formed `handoff`-door
 ...
 * assembly step `Vault.submitWriterActivationHead` delegates to,
...
/**
 * vault-multi-writer (task 5.7) — builds a well-formed `writer`-door
 ...
 * before concluding `VAULT_WRITER_NOT_ADMITTED`.
 */
...
export function validateAddWriterKeyPayload(
  ...
  if (payload.authorization.kind === "writer") {
    ...
    if (!predecessorState.writers.some((w) => w.writer_key_id === headWriterKeyId)) {
      return { ok: false, code: "VAULT_WRITER_NOT_ADMITTED", detail: "the carrying head's signer is not an active writer at the predecessor generation" };
    }
    return { ok: true };
  }
  ...
  const grantPreimage = signaturePreimage("writer_admission_grant", vaultWithoutSignature(grant) as Record<string, unknown>);
