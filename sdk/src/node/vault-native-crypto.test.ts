 ...
 * (vault-native-bulk-crypto, design D5 — the acceptance bar).
 *
 ...
 * `VAULT_AEAD_AUTH_FAILURE` envelope.
 *
...
import {
  _vaultAeadBackend,
  _vaultHashBackend,
  _setVaultAeadBackend,
  _setVaultHashBackend,
  deriveObjectKey,
  frameAad,
  openFrame,
  openFrameWithAad,
...
import { installNodeVaultAeadBackend, installNodeVaultHashBackend, nodeVaultAeadBackend, nodeVaultHashBackend } from "./vault-native-crypto.js";
...
const aeadVectors: VaultVector[] = (vectorSet?.file.vectors ?? []).filter((v) => v.class === "aead-frame");
...
function withNative(fn: () => void): void {
  _setVaultAeadBackend(nodeVaultAeadBackend);
  try {
    fn();
  } finally {
    _setVaultAeadBackend(null);
  }
}

describe("vault native bulk crypto — registration (design D4)", () => {
  after(() => _setVaultAeadBackend(null));

  it("installs on a build whose OpenSSL carries the cipher, and is a no-op otherwise", () => {
    _setVaultAeadBackend(null);
    const installed = installNodeVaultAeadBackend();
    ...
    assert.equal(_vaultAeadBackend() !== null, installed);
    _setVaultAeadBackend(null);
    assert.equal(_vaultAeadBackend(), null, "installing null must restore the @noble default");
  });

  it("the ISOMORPHIC entry registers nothing — only sdk/src/node may install (D3/3.4)", async () => {
    _setVaultAeadBackend(null);
    await import("../index.js");
    assert.equal(_vaultAeadBackend(), null, "the isomorphic entry must never register a backend");
...
describe("vault native bulk crypto — the frozen frame vectors replay identically (3.1, D5)", { skip: vectorSet ? false : OPTOUT_SKIP_MESSAGE }, () => {
  before(() => _setVaultAeadBackend(null));
  after(() => _setVaultAeadBackend(null));
...
describe("vault native bulk crypto — cross-backend interoperation (3.2, D5)", () => {
  after(() => _setVaultAeadBackend(null));
...
describe("vault native bulk crypto — tampering fails identically (3.3, D4)", () => {
  after(() => _setVaultAeadBackend(null));
  ...
  it("a flipped ciphertext byte, tag byte, or AAD member is VAULT_AEAD_AUTH_FAILURE under either backend", () => {
    const kObj = new Uint8Array(randomBytes(32));
    ...
    for (const [label, run] of [
      ...
      _setVaultAeadBackend(null);
      assertLocalCode(run, "VAULT_AEAD_AUTH_FAILURE", `@noble/${label}`);
      withNative(() => assertLocalCode(run, "VAULT_AEAD_AUTH_FAILURE", `native/${label}`));
    }
  });
  ...
  it("a truncated frame is refused, never opened, under the native backend", () => {
    ...
    withNative(() => assertLocalCode(() => openFrame({ ...base, k_obj: kObj, frame: truncated }), "VAULT_AEAD_AUTH_FAILURE", "native/truncated"));
  });
});

/** Run `fn` with the native HASH backend installed, always restoring the default. */
function withNativeHash(fn: () => void): void {
  _setVaultHashBackend(nodeVaultHashBackend);
  try {
    fn();
  } finally {
    _setVaultHashBackend(null);
  }
}

describe("vault native hash — registration + probe (vault-native-hash D3)", () => {
  after(() => _setVaultHashBackend(null));

  it("installs after the live probe, and the slot agrees with the verdict", () => {
    _setVaultHashBackend(null);
    const installed = installNodeVaultHashBackend();
    assert.equal(_vaultHashBackend() !== null, installed);
    _setVaultHashBackend(null);
    assert.equal(_vaultHashBackend(), null, "installing null must restore the @noble default");
  });

  it("a backend that fails the probe registers NOTHING", () => {
    _setVaultHashBackend(null);
    const lying = { sha256: () => new Uint8Array(32) };
    assert.equal(installNodeVaultHashBackend(lying), false);
    assert.equal(_vaultHashBackend(), null, "a failed probe must leave the default in place");
    const throwing = {
      sha256: () => {
        throw new Error("boom");
      },
    };
    assert.equal(installNodeVaultHashBackend(throwing), false);
    assert.equal(_vaultHashBackend(), null);
  });

  it("the ISOMORPHIC entry registers no hash backend", async () => {
    _setVaultHashBackend(null);
    await import("../index.js");
    assert.equal(_vaultHashBackend(), null, "the isomorphic entry must never register a hash backend");
  });
});

describe("vault native hash — byte-identical digests (vault-native-hash 3.1/3.2)", () => {
  after(() => {
    _setVaultHashBackend(null);
    _setVaultAeadBackend(null);
  });
