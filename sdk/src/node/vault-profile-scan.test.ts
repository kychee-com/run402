/**
 * vault cross-profile repo-key scan — a purely
...
import { findLocalProfilesHoldingVaultRepo, crossProfileVaultHint } from "./vault-profile-scan.js";
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), "run402-vault-profile-scan-"));
...
/** Drop an (empty-content — filenames only matter) repo-key file under a profile's vault keystore. */
...
describe("findLocalProfilesHoldingVaultRepo", () => {
  it("finds nothing when no local profile holds the key", () => {
    ensureProfileDir("platform-deploy");
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_A), []);
  });

  it("finds a NAMED profile holding the key", () => {
    dropRepoFile("platform-deploy", REPO_A);
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_A), ["platform-deploy"]);
  });
  ...
  it("finds the default profile holding the key — requires default/wallet.json to exist, matching listProfileNames' own convention", () => {
    ...
    const dir = join(profileDir("default"), "vault", "repos");
    mkdirSync(dir, { recursive: true });
    writeFileSync(join(dir, `${REPO_A}.json`), "{}");
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_A), ["default"]);
  });

  it("does not match a DIFFERENT repo_id", () => {
    dropRepoFile("platform-deploy", REPO_A);
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_B), []);
  });
  ...
  it("reports every matching profile when more than one holds the same key (a restored/shared keystore)", () => {
    ...
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_A).sort(), ["platform-deploy", "staging"]);
  });

  it("excludes the caller's own profile via excludeProfile", () => {
    dropRepoFile("platform-deploy", REPO_A);
    dropRepoFile("staging", REPO_A);
    assert.deepEqual(findLocalProfilesHoldingVaultRepo(REPO_A, { excludeProfile: "platform-deploy" }), ["staging"]);
  });

  it("never throws on a malformed repo_id — resolves to no match", () => {
    dropRepoFile("platform-deploy", REPO_A);
    assert.deepEqual(findLocalProfilesHoldingVaultRepo("../../../etc/passwd"), []);
    assert.deepEqual(findLocalProfilesHoldingVaultRepo("not-a-src-id"), []);
  });
...
describe("crossProfileVaultHint", () => {
  it("names the wallet and both selection mechanisms when a key is found", () => {
    dropRepoFile("platform-deploy", REPO_A);
    const hint = crossProfileVaultHint(REPO_A);
  ...
  it("is empty when the key exists nowhere locally — never a hint pointing at nothing", () => {
    ensureProfileDir("platform-deploy");
    assert.deepEqual(crossProfileVaultHint(REPO_A), []);
  });
});
