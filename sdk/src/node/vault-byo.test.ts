/**
 * vault-byo-primary-bucket — client (public repo) Phase 3 tests (task 3.6).
 *
 * Covers:
 ...
 *
 * Run: node --experimental-test-module-mocks --test --import tsx sdk/src/node/vault-byo.test.ts
 */
...
import { readByoConfig, saveByoConfig } from "./vault-byo-config.js";
import { probeVaultByoDestination, vaultByoProbeFailingProperties } from "./vault-byo-probe.js";
import { VaultKeystore } from "./vault-keystore.js";
import { DirectoryMirrorBackend, openVaultDestinationBackend, openVaultMirrorBackend, S3MirrorBackend } from "./vault-mirror-backend.js";
import { saveMirrorConfig } from "./vault-mirror-config.js";
import { byoChainCopySync, verifyByoObjectsPresent } from "./vault-mirror.js";
import { resolveDegradedReadSource } from "./vault-degraded-read.js";
import { createVaultHttpTransport, vaultLedgerId, vaultManifestEntry, vaultPaths, VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES } from "./vault-publication.js";
...
describe("vault BYO bucket probe (task 3.1) — DirectoryMirrorBackend", () => {
  it("a fresh, writable directory satisfies every property", async () => {
    ...
    assert.deepEqual(vaultByoProbeFailingProperties(result), []);
  });

  it("probeVaultByoDestination against a writable directory succeeds and reports nothing failing", async () => {
    const root = scratchDir("run402-byo-probe-happy-");
    const result = await probeVaultByoDestination({ kind: "directory", path: root });
...
describe("vault BYO bucket probe (task 3.1) — S3MirrorBackend fail-closed matrix", () => {
  ...
  it("write_permitted:false when the first probe PUT is refused (e.g. 403) — versioning is still checked and reported honestly", async () => {
    await withAmbientCreds(async () => {
      ...
      assert.deepEqual(vaultByoProbeFailingProperties(result), ["write_permitted", "create_only_honored"]);
    });
  });
  ...
  it("create_only_honored:false when a second create-only PUT to the same key silently succeeds (overwrite)", async () => {
    await withAmbientCreds(async () => {
      ...
      assert.equal(result.create_only_honored, false);
      assert.equal(result.versioning_off, true);
      assert.deepEqual(vaultByoProbeFailingProperties(result), ["create_only_honored"]);
    });
  });
  ...
  it("versioning_off:false when the bucket reports Enabled — the exact hazard the profile guards against", async () => {
    await withAmbientCreds(async () => {
      ...
      assert.deepEqual(vaultByoProbeFailingProperties(result), ["versioning_off"]);
    });
  });
  ...
  it("probeVaultByoDestination throws VAULT_BYO_BUCKET_PROBE_FAILED naming every failing property, and no allocation call is reachable from it", async () => {
    await withAmbientCreds(async () => {
      globalThis.fetch = (async () => new Response(null, { status: 403 })) as typeof fetch;
      await assert.rejects(
        () => probeVaultByoDestination({ kind: "s3", bucket: "bucket", prefix: "prefix", region: "us-east-1" }, credential()),
        (e: unknown) => {
          assert.ok(isRun402Error(e));
          assert.equal((e as { code?: string }).code, "VAULT_BYO_BUCKET_PROBE_FAILED");
...
describe("vault BYO payload write path + finalize attestation round-trip (task 3.2)", () => {
  interface WireCall { path: string; method?: string; body?: unknown }

  function walObject(bytes: Uint8Array): VaultUploadObject {
    return { path: vaultPaths.wal(WAL), object_kind: "wal_pack", object_id: WAL, bytes, sha256: sha256Hex(bytes), size_bytes: String(bytes.length), base_generation: "0000000000000000" };
  }

  function verifierReceiptObject(bytes: Uint8Array): VaultUploadObject {
    return { path: vaultPaths.verifierReceipt(VR), object_kind: "verifier_receipt", object_id: VR, bytes, sha256: sha256Hex(bytes), size_bytes: String(bytes.length) };
  }

  function byoTransportOver(destinationRoot: string, handler: (call: WireCall) => unknown): { transport: ReturnType<typeof createVaultHttpTransport>; calls: WireCall[] } {
  ...
  it("a payload-kind object (put:null + key) is written DIRECTLY to the BYO destination — never through client.fetch — and finalize carries the matching attestation", async () => {
    ...
    const entry = vaultManifestEntry(walObject(walBytes));
  ...
  it("a chain-adjacent object (put: presigned URL) in the SAME BYO session still goes through client.fetch, byte-identical to the managed path", async () => {
    ...
    const entry = vaultManifestEntry(verifierReceiptObject(vrBytes));
  ...
  it("a write refused by the customer bucket fails the push with VAULT_BYO_BUCKET_WRITE_REFUSED — the primary is never advisory", async () => {
    ...
    const backend = openVaultDestinationBackend({ kind: "directory", path: root });
    ...
    await assert.rejects(
      ...
      (e: unknown) => {
        assert.ok(isRun402Error(e));
        assert.equal((e as { code?: string }).code, "VAULT_BYO_BUCKET_WRITE_REFUSED");
        return true;
      },
    );
  });
  ...
  it("the inline (bytes-in-body) fast path is NEVER used for a BYO vault, even for a tiny payload well under the inline caps", async () => {
    ...
    const entry = vaultManifestEntry(walObject(tiny));
    const key = `source/${REPO}/wal/${WAL}.pack.enc`;
...
describe("vault BYO chain-copy dual-write, scoped to chain kinds, admission order (task 3.3)", () => {
  ...
  it("copies ONLY the chain (heads/admissions + chain-adjacent), never the payload kinds already written directly by task 3.2's push path", async () => {
    ...
    // vault-mirror.ts's own default resolution (`openByoBackendFromConfig`)
    // opens a REPO-SCOPED backend (`openVaultMirrorBackend`, which bakes
    // `source/<repo_id>` into its root) because `listVaultObjectsAll`
    ...
    // root-scoped `openVaultDestinationBackend` (section B, above).
    const summary = await byoChainCopySync(fullListingClient(), REPO, { backend: openVaultMirrorBackend({ kind: "directory", path: root }, REPO, undefined) });
  ...
  it("torn chain-copy (interrupted mid-admissions/heads) still leaves a fully valid, resumable state — objects phase always completes before any admission/head write", async () => {
    ...
    const backend = openVaultMirrorBackend({ kind: "directory", path: root }, REPO, undefined);
...
describe("vault degraded-read BYO branch (task 3.4)", () => {
  let root: string;
  ...
  beforeEach(() => {
    ...
    keystore = new VaultKeystore({ rootDir: root });
  });
...
describe("managed vaults are byte-identical — uploadObjects with no `byo` arg never touches BYO machinery", () => {
  it("a session response with a real presigned put (no key, no null) goes through the ordinary PUT path even when the caller never passes byo", async () => {
    // Sized ABOVE VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES on purpose: a
    // small payload is inline-eligible (`vaultInlineUploadEligible`) and
    // `upload()` would take the one-POST inline fast path instead of the
    // session+PUT+finalize path this test names and exercises.
    const vrBytes = new Uint8Array(VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES + 1).fill(3);
    ...
    const receipts = await transport.uploadObjects({
      repo_id: REPO,
      objects: [{ path: vaultPaths.verifierReceipt(VR), object_kind: "verifier_receipt", object_id: VR, bytes: vrBytes, sha256: entry.sha256, size_bytes: entry.size_bytes }],
    });
...
// ─── C. BYO payload READ path (vault-byo-primary-bucket task 5.1 finding) ─
//
...
describe("vault BYO payload read path: byo_key entries are read from the local destination backend", () => {
  const KEY = `source/${REPO}/wal/${WAL}.pack.enc`;
  ...
  function fakeBackend(store: Map<string, Uint8Array>, gets: string[]) {
    ...
    } as unknown as import("./vault-mirror-backend.js").VaultMirrorBackend;
  }
  ...
  it("getObject resolves a byo_key entry through byoBackend and never dials run402 for the bytes", async () => {
    ...
    const transport = createVaultHttpTransport(
      client((path) => {
    ...
    const got = await transport.getObject({ repo_id: REPO, path: vaultPaths.wal(WAL) });
  ...
  it("an absent key in the customer bucket is `null` (absence adjudication takes over), not an error", async () => {
    ...
    assert.equal(await transport.getObject({ repo_id: REPO, path: vaultPaths.wal(WAL) }), null);
  });

  it("a machine with no local BYO config refuses VAULT_BYO_NOT_CONFIGURED by name — never a false absence", async () => {
    for (const opts of [{}, { byoBackend: () => null }]) {
      ...
      await assert.rejects(
        transport.getObject({ repo_id: REPO, path: vaultPaths.wal(WAL) }),
        (e: unknown) => (e as { code?: string }).code === "VAULT_BYO_NOT_CONFIGURED",
      );
    }
  });
