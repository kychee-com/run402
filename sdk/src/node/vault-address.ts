/**
 * vault D6 — named addressing resolution + id-pinning (repo-first-onramp
 * task 4.5).
 *
 ...
 *   - `resolveVaultAddress`: the orchestrator the remote helper and
 *     `vault snapshot` drive — pinned id first, then dispatch on the
 *     address's form (id vs slug — {@link vaultRemoteAddressForm}), then
...
import { vaultRemoteAddressForm, type VaultRemoteAddress } from "../namespaces/repos.js";
import type { VaultRecord, VaultTransport } from "./vault-publication.js";
import { pushToCreateVault } from "./vault-push-to-create.js";
import type { VaultCreationResult } from "./vault-creation-journal.js";
...
/**
 * The resolved ids beside the pin (vault-force-spelling-and-pin-fold).
 ...
 * dedicated config key to read rather than special-casing vault's own
 * `r402.projectId` name. Every existing pin site gets this for free, not
 * just `resume` — one write, every vault checkout benefits.
 */
const CONFIG_KEY_ROOM = "r402.room";

export interface VaultPinnedRepo {
  repo_id: string;
...
export async function readPinnedVaultRepo(repoDir: string): Promise<VaultPinnedRepo | null> {
 ...
 * only). `resolvedFrom` is omitted for an id-form pin (vault-client-
 * round-trips design D4): an id-form address has no org-slug/name pair to
 * record, and — per {@link resolveVaultAddress}'s own doc comment — the
 ...
 */
export async function pinVaultRepo(repoDir: string, repoId: string, resolvedFrom?: { org_slug: string; repo_name: string }, ids?: { project_id: string; org_id: string; room_key?: string }): Promise<void> {
 ...
 * LOCAL git config — the SAME two keys {@link pinVaultRepo} writes
 ...
 * independent of {@link readPinnedVaultRepo}'s own gate on `r402.repoId`
...
async function clearPinnedVaultRepo(repoDir: string): Promise<void> {
...
export interface VaultAddressResolution {
   ...
   * keystore's own project→repo file (vault-offline-clone-resolve,
   * id-form only — {@link VaultKeystore.findRepoByProject}), a live
   ...
   * them apart — {@link recoverStaleVaultPin}'s generic `offline` check
  ...
  created: { deduplicated: boolean; recovery_receipt: VaultCreationResult["recovery_receipt"]; genesis_sha256: string } | null;
  /**
   ...
   * verb's first repo-scoped read — see {@link recoverStaleVaultPin}.
   */
  offline: boolean;
}

export interface ResolveVaultAddressOptions {
  keystore: VaultKeystore;
  transport: VaultTransport;
  /** The parsed remote address (`parseVaultRemoteUrl`'s output) — id-form or slug-form, undiscriminated. */
  ...
  repo_dir?: string;
  /** Push-to-create on a slug-form miss (D6). `false` (the default) makes a miss an ordinary not-found refusal — the read-only path (`list`/`fetch`, `vault status`). */
  allow_create?: boolean;
  client_creation_id?: string;
  service_public_key?: Uint8Array | string;
  /** Fires once, only when THIS call allocated the vault (mirrors `Repos.capture`'s `onVaultCreated`). */
  onVaultCreated?: (created: NonNullable<VaultAddressResolution["created"]>) => void | Promise<void>;
  /**
   * vault-offline-clone-resolve (design D3): recovery-only. Skips BOTH
   ...
   * recoverStaleVaultPin}'s internal re-resolve call: a stale local
 ...
 * of EITHER form (vault-client-round-trips design D4 widens the
 ...
 */
export async function resolveVaultAddress(options: ResolveVaultAddressOptions): Promise<VaultAddressResolution> {
  if (options.repo_dir && !options.bypass_local_answers) {
    const pinned = await readPinnedVaultRepo(options.repo_dir);
    if (pinned) {
      ...
      if (pinned.project_id && pinned.org_id) {
        ...
        // arms {@link recoverStaleVaultPin} for that (client-surface
        // spec, "Id-form remotes pin repo_id like slug-form remotes").
        return { repo_id: pinned.repo_id, project_id: pinned.project_id, org_id: pinned.org_id, form: vaultRemoteAddressForm(options.address), via: "pin", address: pinned.resolved_from, created: null, offline: true };
      }
      try {
        ...
        await pinVaultRepo(options.repo_dir, record.repo_id, pinned.resolved_from ?? undefined, { project_id: record.project_id, org_id: record.org_id });
        return { repo_id: record.repo_id, project_id: record.project_id, org_id: record.org_id, form: vaultRemoteAddressForm(options.address), via: "pin", address: pinned.resolved_from, created: null, offline: false };
      } catch (e) {
        if (!isPinStale(e)) throw e;
        await clearPinnedVaultRepo(options.repo_dir);
        // fall through to an ordinary (non-pinned) resolution, below
      }
    }
  }

  const form = vaultRemoteAddressForm(options.address);
  if (form === "id") {
    if (!options.bypass_local_answers) {
      // vault-offline-clone-resolve (design D1/D2): consult the LOCAL
      ...
      // `VAULT_REPO_STATE_MISSING` no matter how resolution went —
      ...
      // carries a real `org_id` (see `vaultRemoteAddressForm`), so the
      ...
      if (keystoreMatch) {
        ...
        // recoverStaleVaultPin}'s generic `resolution.offline` check
        ...
        if (options.repo_dir) await pinVaultRepo(options.repo_dir, keystoreMatch.repo_id, undefined, { project_id: keystoreMatch.project_id, org_id: keystoreMatch.org_id });
    ...
    const record: VaultRecord = await options.transport.findVaultByProject({ project_id: options.address.project_id });
    ...
    if (options.repo_dir) await pinVaultRepo(options.repo_dir, record.repo_id, undefined, { project_id: record.project_id, org_id: record.org_id });
  ...
  // `allow_create` delegates straight to `pushToCreateVault`, which
  ...
  if (options.allow_create) {
    const result = await pushToCreateVault({
    ...
    if (options.repo_dir) await pinVaultRepo(options.repo_dir, result.repo_id, { org_slug: orgSlug, repo_name: repoName }, { project_id: result.project_id, org_id: result.org_id });
  ...
  if (options.repo_dir) await pinVaultRepo(options.repo_dir, record.repo_id, { org_slug: orgSlug, repo_name: repoName }, { project_id: record.project_id, org_id: record.org_id });
 ...
 * vault-offline-clone-resolve design D3 to the keystore-sourced offline
 ...
 *     gateway's authorize-before-reveal `VAULT_ACCESS_DENIED` fold, which
 ...
 */
export async function recoverStaleVaultPin(options: {
  keystore: VaultKeystore;
  transport: VaultTransport;
  address: VaultRemoteAddress;
  repo_dir: string;
  /** The offline resolution the failing verb ran under. */
  resolution: VaultAddressResolution;
  /** The verb's failure. */
  error: unknown;
}): Promise<VaultAddressResolution | null> {
  if (!options.resolution.offline) return null;
  const err = options.error as { status?: number; code?: string } | null;
  const vaultAbsentSignal = Boolean(err && (isPinStale(err) || err.code === "VAULT_ACCESS_DENIED" || err.status === 403));
  if (!vaultAbsentSignal) return null;
  try {
    await clearPinnedVaultRepo(options.repo_dir);
    const fresh = await resolveVaultAddress({ keystore: options.keystore, transport: options.transport, address: options.address, repo_dir: options.repo_dir, bypass_local_answers: true });
