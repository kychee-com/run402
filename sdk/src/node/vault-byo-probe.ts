/**
 * vault-byo-primary-bucket — the allocation-time bucket-policy probe
...
import type { VaultBucketProbeResult } from "./vault-mirror-backend.js";
import { openVaultDestinationBackend } from "./vault-mirror-backend.js";
import type { VaultMirrorCredential, VaultMirrorDestination } from "./vault-mirror-config.js";
import { formatMirrorDestination } from "./vault-mirror-config.js";
 ...
 * entry maps 1:1 to one of {@link VaultBucketProbeResult}'s three
 * booleans, so a caller (or a test) can assert on exactly what broke.
 */
export function vaultByoProbeFailingProperties(result: VaultBucketProbeResult): string[] {
 ...
 * instead) and run {@link VaultMirrorBackend.probeWritePolicy}. A failed
 * property (or a thrown transport error) refuses with
 * `VAULT_BYO_BUCKET_PROBE_FAILED`, naming exactly which propert(y/ies)
 * failed and the underlying fact — the caller MUST NOT proceed to
 * allocation on a failed probe.
 */
export async function probeVaultByoDestination(destination: VaultMirrorDestination, credential?: VaultMirrorCredential): Promise<VaultBucketProbeResult> {
  const backend = openVaultDestinationBackend(destination, credential);
  let result: VaultBucketProbeResult;
  try {
    result = await backend.probeWritePolicy();
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e);
    fail(
      "VAULT_BYO_BUCKET_PROBE_FAILED",
  ...
  const failing = vaultByoProbeFailingProperties(result);
  if (failing.length > 0) {
    fail(
      "VAULT_BYO_BUCKET_PROBE_FAILED",
