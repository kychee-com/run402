/**
 * vault-multi-writer rev 47 — THE SURVIVOR THAT DID NOT ROTATE.
 *
 * The live-only failure this file reproduces on the in-memory transport
 * (`docs/runbooks/vault-live-rehearsal.md`, "Both survivors, not only the
...
import { ekFingerprint, parseVaultStrict, parseRotateEpochPayload } from "../namespaces/vault.crypto.js";
...
describe("vault-multi-writer rev 47 — the survivor that did NOT rotate opens the vault after a removal", () => {
  it("C (a keyed, pinned recipient pinned decrypt-blind past the rotation) materializes across a removal: the new epoch opens under the ROTATOR's key and B's last head re-verifies under B's RETIRED key", async (t) => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-survivor-catchup-"));
    ...
    const ksB = VaultKeystore.open({ rootDir: join(root, "ksB") });
    ...
    const ksC = VaultKeystore.open({ rootDir: join(root, "ksC") });
    ...
    const vaultB = Vault.open({ keystore: ksB, transport, repo_id: repoId, repo_dir: repoDirB });
    ...
    // rotation's signer as "the genesis creator" dies VAULT_SIGNATURE_INVALID
    ...
    const vaultC = Vault.open({ keystore: ksC, transport, repo_id: repoId, repo_dir: null });
    ...
    // ── A removes B: the gateway-side facts a real `orgs members rm` sets ───
    transport.desiredRecipients.set(orgId, [
      { principal_id: A_PRINCIPAL, display_name: "A (creator, rotator)", status: "active", ek_fingerprint: ekFingerprint(ekA.public_key), public_key: b64u(ekA.public_key), suite: "r402s-1", covered: true },
    ...
    const legacyVault = Vault.open({ keystore: ksC, transport, repo_id: repoId, repo_dir: null });
