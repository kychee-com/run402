/**
 * vault — cross-profile repo-key scan.
 *
 * A keystore-miss refusal (`KEYSTORE_MISSING`, `VAULT_REPO_STATE_MISSING`,
 * or the non-holder half of a `VAULT_CREATION_CONFLICT` reconciliation
 * attempt — see `vault-creation-journal.ts`) is read against the ACTIVE
 ...
 *
 * Shared by every keystore-miss enrichment point (`vault-publication.ts`'s
 * `Vault.repoFile()`, and `vault-creation-journal.ts`'s
 ...
 */
export function findLocalProfilesHoldingVaultRepo(repoId: string, options: { excludeProfile?: string } = {}): string[] {
  if (!VAULT_SRC_RE.test(repoId)) return [];
  const found: string[] = [];
  for (const profile of listProfileNames()) {
    if (options.excludeProfile && profile === options.excludeProfile) continue;
    try {
      const repoPath = join(profileDir(profile), "vault", "repos", `${repoId}.json`);
 ...
 */
export function crossProfileVaultHint(repoId: string, options: { excludeProfile?: string } = {}): { action: string }[] {
  const profiles = findLocalProfilesHoldingVaultRepo(repoId, options);
