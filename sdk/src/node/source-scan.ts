...
export interface SourceScanFinding { code: string; severity: string; file: string; line?: number; message: string; canonical_name?: string; [key: string]: unknown; }
interface ScanOptions { excludeFiles?: Set<string>; cwd?: string; filePath?: string; declaredCapabilities?: Set<string> | string[]; }
/**
...
export function scanFileContent(content: string, opts: ScanOptions = {}) {
  ...
  function lineIndexFor(charIndex: number) {
...
export function scanSourceTree(srcDir: string, opts: ScanOptions = {}) {
  const findings: SourceScanFinding[] = [];
  ...
  walk(srcDir, (filePath) => {
    if (!SCANNED_EXTENSIONS.has(extname(filePath)) || opts.excludeFiles?.has(filePath)) return;
    let content;
    try {
 ...
 *  Used by `run402 deploy` for manifest/spec/stdin deploys, where
...
export function scanSourceFiles(filePaths: string[], opts: ScanOptions = {}) {
  const findings: SourceScanFinding[] = [];
...
function walk(dir: string, visitor: (path: string) => void) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    if (entry.isDirectory()) {
      ...
      if (existsSync(join(child, ".git")) || ["run402.json", "run402.deploy.json", "app.json"].some((name) => existsSync(join(child, name)))) continue;
      walk(child, visitor);
...
export function readDeclaredCapabilities(cwd = process.cwd()) {
  ...
  for (const bucket of ["replace", "set", "patch"]) {
    ...
    for (const entry of Object.values(entries) as Array<{ capabilities?: unknown }>) {
...
export function scanDeploymentSources(spec: Partial<ReleaseSpec>, appRoot: string, sourceRoot?: string) {
  ...
  for (const bucket of [spec.functions?.replace, spec.functions?.patch?.set]) {
    for (const fn of Object.values(bucket ?? {})) {
      if (fn.capabilities?.includes("astro.ssr.v1") && fn.source && typeof fn.source === "object" && "path" in fn.source) generated.add(fn.source.path);
    }
  }
  ...
  if (sourceRoot || scanRoot !== appRoot) walk(scanRoot, (path) => {
    if (SCANNED_EXTENSIONS.has(extname(path)) && !generated.has(path)) files.add(path);
  });
  for (const reference of references) if (SCANNED_EXTENSIONS.has(extname(reference.path))) files.add(reference.path);
