/**
 * vault — the Handoff Key and the Invite Key, one kind-parameterized
 ...
 * `@noble/hashes`, the SAME `_vaultAeadBackend()` XChaCha20-Poly1305
...
// ─── Writer-admission grant + acceptance (vault-multi-writer D4/§4.17) ───
//
...
// `ekFingerprint` from `vault.crypto.ts` — the SAME "r402s/v0/<kind>\n" +
...
// OWN `services/vault/claims.ts` (not just the protocol prose, which
...
export type VaultWriterMintedRole = "owner" | "admin" | "developer" | "billing" | "viewer";
const WRITER_MINTED_ROLES: readonly VaultWriterMintedRole[] = ["owner", "admin", "developer", "billing", "viewer"];
...
export interface WriterAdmissionGrant {
  ...
  minted_role: VaultWriterMintedRole;
...
export function buildWriterAdmissionGrant(input: BuildWriterAdmissionGrantInput): WriterAdmissionGrant {
  if (!VAULT_SHA256_RE.test(input.auth_hash)) fail("VALIDATION_FAILED", "auth_hash must be 64 lowercase hex", "building writer_admission_grant", { field: "auth_hash" });
  if (!VAULT_HEX16_RE.test(input.checkpoint_generation)) fail("VALIDATION_FAILED", "checkpoint_generation must be 16 lowercase hex", "building writer_admission_grant", { field: "checkpoint_generation" });
  if (!VAULT_SHA256_RE.test(input.checkpoint_head_sha256)) fail("VALIDATION_FAILED", "checkpoint_head_sha256 must be 64 lowercase hex", "building writer_admission_grant", { field: "checkpoint_head_sha256" });
...
export function verifyWriterAdmissionGrant(grant: WriterAdmissionGrant, grantorSigningPubkeyRaw: Uint8Array): boolean {
  if (grant.format !== VAULT_FORMAT || grant.object_kind !== "writer_admission_grant" || grant.suite !== VAULT_SUITE) return false;
...
export function buildWriterAcceptance(input: BuildWriterAcceptanceInput): WriterAcceptance {
  if (!VAULT_SHA256_RE.test(input.auth_hash)) fail("VALIDATION_FAILED", "auth_hash must be 64 lowercase hex", "building writer_acceptance", { field: "auth_hash" });
...
/** Frame tag for the handoff envelope format — DISTINCT from `r402s/v0`'s `"R402S0"` object frame (design D3: never stored as a vault object). The LEGACY v1 tag: nothing seals it any more, but a pre-vault-multi-writer handoff row still carries one and {@link openHandoffEnvelope} still opens it. */
...
function redeemAeadSeal(key32: Uint8Array, nonce24: Uint8Array, aad: Uint8Array, plaintext: Uint8Array): Uint8Array {
  const backend = _vaultAeadBackend();
...
// ─── Envelope v2 (vault-multi-writer D4/§4.17; kygit-invite design D3) ───
//
...
export function sealHandoffEnvelopeV2(handoffIdBytes: Uint8Array, wrapKey: Uint8Array, payload: HandoffEnvelopePayloadV2, nonce?: Uint8Array): { sealed_envelope: string; envelope_kind: string } {
  if (!VAULT_SHA256_RE.test(payload.writer_admission_grant_sha256)) {
 ...
 * BEFORE attempting to decrypt anything — post-vault-multi-writer, a
...
export function openHandoffEnvelopeV2(handoffIdBytes: Uint8Array, wrapKey: Uint8Array, sealedEnvelope: string, envelopeKind: string): HandoffEnvelopePayloadV2 {
  if (envelopeKind !== HANDOFF_ENVELOPE_V2_KIND) {
    fail(
      ...
      `this handoff's envelope is ${envelopeKind ? `\`${envelopeKind}\`` : "of an unrecognized kind"}, not \`${HANDOFF_ENVELOPE_V2_KIND}\` — writer activation needs the v2 envelope's writer_admission_grant_sha256, which a pre-vault-multi-writer handoff never carries`,
  ...
  if (
    ...
    !VAULT_SHA256_RE.test(p.writer_admission_grant_sha256)
  ) {
