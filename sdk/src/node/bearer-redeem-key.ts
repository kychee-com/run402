/**
 * Bearer-redeem key format — the pure, kind-agnostic primitives shared by
 * every bearer-addressed membership redeem key this SDK mints or parses
...
// ─── Key prefix / redeem-kind registry (design D9 rule 4; kygit-invite D3; add-room-invite D3) ────

export type RedeemKind = "handoff" | "invite" | "room";
...
function entryForKind(kind: RedeemKind): HandoffKeyPrefixEntry {
...
export interface RedeemKeyParts {
  kind: RedeemKind;
  id_bytes: Uint8Array;
...
/** A fresh RFC 4122 v4 UUID — the client-generated key id every bearer-redeem kind mints with (design D3: `auth_secret` derives off it, so it cannot be gateway-assigned). */
export function randomRedeemId(): string {
...
/** Assemble a printed redeem key from a fresh 32-byte master secret and the (client-generated or gateway-minted) key id, for `entry`'s kind. */
...
/**
 * Parse ANY registered redeem-family key by its prefix registry against the
 ...
 */
export function parseRedeemKey(raw: string, expectedKind: RedeemKind): RedeemKeyParts {
  const expectedEntry = entryForKind(expectedKind);
  const trimmed = raw.trim();
  const entry = HANDOFF_KEY_PREFIXES.find((e) => trimmed.startsWith(e.prefix));
  if (!entry) {
    fail(`${expectedEntry.errorPrefix}_KEY_INVALID`, `not a recognized run402 redeem key (expected a key starting ${expectedEntry.prefix})`, "parsing redeem key");
  }
  ...
  try {
    ...
    fail(`${entry.errorPrefix}_KEY_INVALID`, "the key body is not valid base64url", "parsing redeem key");
  }
  if (bytes.length !== CLAIM_KEY_BODY_BYTES) {
    fail(`${entry.errorPrefix}_KEY_INVALID`, `the key decodes to ${bytes.length} bytes; expected ${CLAIM_KEY_BODY_BYTES}`, "parsing redeem key");
  }
...
/**
 * Parse ANY registered redeem-family key by its prefix registry. A
...
export function parseHandoffKey(raw: string): HandoffKeyParts {
  const parsed = parseRedeemKey(raw, "handoff");
...
export function parseInviteKey(raw: string): InviteKeyParts {
  const parsed = parseRedeemKey(raw, "invite");
...
export function parseRoomInviteKey(raw: string): RoomInviteKeyParts {
  const parsed = parseRedeemKey(raw, "room");
...
function deriveClaimSecrets(kind: "handoff" | "invite", idBytes: Uint8Array, masterSecret: Uint8Array): HandoffSecrets {
  ...
  // gateway recomputes at redeem (`computeAuthHash`). Through 4.68.1 this line
