/**
 * vault prune — the client half of §7.3 (add-vault task 5.12a).
 *
...
import {
  VAULT_MAX_PRUNE_CANDIDATES,
  VAULT_SDK_VERIFIER_IMPLEMENTATION,
  VAULT_SDK_VERIFIER_VERSION,
  VAULT_VERIFIER_IMPLEMENTATIONS,
  ...
  verifierReceiptRef,
  type VaultChainEntry,
  type VaultPruneIntentRecord,
  type VaultPruneableReceipt,
  type VaultVerifierReceipt,
} from "./vault-prune.js";
import { generateSigningKeypair, jcsString, parseVaultStrict, storedBytesSha256, verifyVaultObject } from "../namespaces/vault.crypto.js";
import type { VaultCheckpointClaimSet, VaultHead, VaultSignedObject } from "../namespaces/vault.types.js";
import { commitFile, makeVault } from "./vault-memory-transport.test.js";
import { vaultPaths } from "./vault-publication.js";
 ...
 */
function twoCheckpointChain(): VaultChainEntry[] {
  return [
...
describe("the intent core — the acyclic base BOTH receipts sign", () => {
  ...
  it("sorts the delete set and signs the whole core", () => {
    ...
    assert.ok(verifyVaultObject(core as unknown as VaultSignedObject, owner.public_key));
  });

  it("`intent_core_sha256` hashes the COMPLETE signed core, signature included", () => {
    const core = buildPruneIntentCore(coreInput, owner.seed);
    assert.equal(pruneIntentCoreSha256(core), storedBytesSha256(core as unknown as VaultSignedObject));
    // and it therefore changes if any member changes, including the signature
    const tampered = { ...core, nonce: hex32("f") };
    assert.notEqual(storedBytesSha256(tampered as unknown as VaultSignedObject), pruneIntentCoreSha256(core));
  });
  ...
  it("refuses an empty or oversized delete set rather than letting the gateway reject a wasted round trip", () => {
    ...
    const many = Array.from({ length: VAULT_MAX_PRUNE_CANDIDATES + 1 }, (_, i) => ({
...
describe("the two verifier receipts — one per implementation, never both from us", () => {
  ...
  const receipt = (over: Partial<Parameters<typeof buildVerifierReceipt>[0]> = {}): VaultVerifierReceipt =>
    buildVerifierReceipt(
      {
        repo_id: REPO,
        ...
        implementation_version: VAULT_SDK_VERIFIER_VERSION,
        ...over,
      },
      owner.seed,
    );
  ...
  it("this SDK is `run402-cli`, and the CLOSED set names exactly two identities", () => {
    assert.equal(VAULT_SDK_VERIFIER_IMPLEMENTATION, "run402-cli");
    assert.deepEqual([...VAULT_VERIFIER_IMPLEMENTATIONS], ["run402-cli", "r402s-verify"]);
  });
  ...
  it("builds the ref the intent carries — never the receipt body", () => {
    ...
    assert.equal(ref.stored_bytes_sha256, storedBytesSha256(one as unknown as VaultSignedObject));
  ...
  it("accepts exactly one receipt per identity and signs the wrapper", () => {
    ...
    assert.ok(verifyVaultObject(intent as unknown as VaultSignedObject, owner.public_key), "the wrapper is signed too — the double signature of §7.3");
  });
});
...
describe("a stale core is refused BEFORE the receipts are burned", () => {
  ...
  it("refuses a foreign vault with the registry's access code", () => {
    assert.throws(
      ...
      (e: unknown) => (e as { code?: string }).code === "VAULT_ACCESS_DENIED",
    );
  });
...
describe("the wire — the route reads EXACT BYTES", () => {
  ...
  it("serializes to its own JCS, so the bytes the gateway strict-parses are the bytes we signed", () => {
    ...
    const reparsed = parseVaultStrict(text) as typeof intent;
    assert.equal(storedBytesSha256(reparsed as unknown as VaultSignedObject), storedBytesSha256(intent as unknown as VaultSignedObject));
    assert.ok(verifyVaultObject(reparsed as unknown as VaultSignedObject, owner.public_key));
  });
...
describe("the completion — only `deleted` means gone", () => {
  ...
  const record = (perObject: Array<{ object_id: string; result: "deleted" | "present_not_attempted" | "present_after_attempt" }> | null): VaultPruneIntentRecord =>
    ({
...
describe("retention-root evolution — an unprovable departure is `false`, not `true`", () => {
  ...
  const rootsAt = (present: Record<string, boolean>) => (e: VaultChainEntry) => (present[e.head.generation] ? [dropped] : []);
  ...
  it("refuses a departure at a generation carrying no checkpoint", () => {
    const noCp = [chain[0]!, { ...chain[1]!, head: { ...chain[1]!.head, checkpoint: null } as VaultHead }];
...
describe("end to end against the memory control plane", () => {
  it("plans from a real chain, submits exact bytes, and believes only the signed completion", async (t) => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-prune-e2e-"));
    ...
    const receipts = [mk("run402-cli", VAULT_SDK_VERIFIER_VERSION), mk("r402s-verify", "r402s-verify/0.1.0")];

    // Both receipts must be STORED before the intent may name them.
    const { storedBytes, sha256Hex } = await import("../namespaces/vault.crypto.js");
    await f.transport.uploadObjects({
      repo_id: f.repoId,
      objects: receipts.map((r) => {
        const bytes = storedBytes(r as unknown as VaultSignedObject);
        return { path: vaultPaths.verifierReceipt(r.object_id), object_kind: "verifier_receipt", object_id: r.object_id, bytes, sha256: sha256Hex(bytes), size_bytes: String(bytes.length) };
      }),
    });
  ...
  it("reports a partially-failed prune as partial — never as a success", async (t) => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-prune-partial-"));
    t.after(() => rmSync(root, { recursive: true, force: true }));
    const f = await makeVault(root);
    const c1 = await commitFile(f.repoDir, "a.txt", "one");
    await f.vault.push({ transaction: { updates: [{ ref: "refs/heads/main", expected_old_oid: null, new_oid: c1, force: false }] } });
    ...
    await f.transport.uploadObjects({
      repo_id: f.repoId,
      objects: receipts.map((r) => {
        const b = storedBytes(r as unknown as VaultSignedObject);
        return { path: vaultPaths.verifierReceipt(r.object_id), object_kind: "verifier_receipt", object_id: r.object_id, bytes: b, sha256: sha256Hex(b), size_bytes: String(b.length) };
      }),
    });
