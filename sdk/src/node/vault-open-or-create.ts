 ...
 *
 * The orchestration `Repos.openOrCreate` (the namespace method the remote
 ...
 * (`findVaultByProject`, already on {@link VaultTransport} — no second
...
export type OpenOrCreateVaultResult =
  | { found: true; repo_id: string; created: null }
  | {
      found: false;
      repo_id: string;
      created: {
        /** Mirrors `VaultCreationResult.how === "reconciled"`: a local journal was resumed to completion, nothing re-minted. */
 ...
 */
export async function openOrCreateVault(options: OpenOrCreateVaultOptions): Promise<OpenOrCreateVaultResult> {
  ...
  const resumable = options.client_creation_id ? null : findResumableVaultJournal(options.keystore, options.org_id, options.project_id);
