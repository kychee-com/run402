/**
 * The vault deploy lane, supplied at last (change `vault-deploy-lane`).
 *
 * `add-vault` 5.6 built `runVaultDeploy` with the lane INJECTED and never
 * supplied one, so a project whose `vault_policy` is `required` could not be
 ...
 *   - {@link applyWithVault} decides whether any of that happens at all. A
 *     project that is not `required` takes the untouched path: no capture, no
 *     token, no extra refusal, one policy read.
 *
 * THE COROUTINE, because it is the non-obvious part. `runVaultDeploy` wants
...
export interface ApplyDeployLaneOptions {
  ...
  apply?: Omit<ApplyOptions, "vault">;
}

/** A lane that is also cancellable, because `runVaultDeploy` may never commit. */
export interface ApplyDeployLane extends VaultDeployLane {
  /**
...
export function createApplyDeployLane(options: ApplyDeployLaneOptions): ApplyDeployLane {
  const planned = deferred<VaultDeployLanePlan>();
  const authorized = deferred<VaultCommitDeclaration>();
  ...
  const lane: ApplyDeployLane = {
          ...
          authorize: async (p) => {
            if (!p.apply_plan_sha256) {
              ...
              const missing = new LocalError(
                ...
                "planning a vault deploy",
                { code: "VAULT_PLAN_DIGEST_MISSING", details: { plan_id: p.plan_id, operation_id: p.operation_id } },
              );
    ...
    async commit(input: VaultDeployLaneCommitInput) {
      authorized.resolve(commitDeclaration(input));
      if (!applyPromise) throw new LocalError("commit was called before plan", "committing vault deploy", { code: "VAULT_LANE_OUT_OF_ORDER" });
    ...
    abandon(reason) {
      ...
      authorized.reject(
        reason instanceof Error
          ? reason
          : new LocalError("the vault deploy was abandoned before commit", "committing vault deploy", { code: "VAULT_DEPLOY_ABANDONED", details: { reason } }),
      );
    },
    result: () => result,
  };
...
function commitDeclaration(input: VaultDeployLaneCommitInput): VaultCommitDeclaration {
  ...
  if (!tokenId) {
    throw new LocalError(
      ...
      "committing vault deploy",
      { code: "VAULT_ACTIVATION_TOKEN_MISSING" },
    );
  }
...
/** What the deploy did about vault, and why. */
export type VaultApplyMode =
  ...
  /**
   * A vault exists but `vault_policy` was never set either way — D3:
   * allocating a vault no longer flips the policy, so this is the ordinary
   * shape for a project whose vault came from `run402 repos create` (or a
...
export function vaultPolicyRequiredNextAction(repoId: string): NextAction {
  return {
    type: "vault_policy_required",
    command: "run402 repos policy required",
    why: `vault ${repoId} exists but vault_policy was never set, so this deploy ran without requiring a vaulted capture. Set it to require one, or run \`run402 repos policy grandfathered --reason <why>\` to explicitly accept the current state.`,
  };
}

/** The standing `warnings[]` entry D3 attaches to every ungated deploy, until the policy is set either way. */
export function vaultUngatedWarning(repoId: string): LegacyWarningEntry {
  return {
    code: "VAULT_POLICY_UNSET",
    severity: "low",
    requires_confirmation: false,
    message: `vault ${repoId} is not gated: vault_policy was never set, so a deploy can activate without a vaulted capture. Run \`run402 repos policy required\` (or \`grandfathered --reason <why>\` to explicitly accept this) to clear the drift.`,
    affected: [repoId],
  };
}
...
function decorateUngatedResult(result: DeployResult, repoId: string): DeployResult {
  return {
    ...result,
    warnings: [...(result.warnings ?? []), vaultUngatedWarning(repoId)],
    next_actions: [...(result.next_actions ?? []), vaultPolicyRequiredNextAction(repoId)],
  };
}

export interface ApplyWithVaultResult {
  mode: VaultApplyMode;
  ...
  vault: VaultDeployResult | null;
}

export interface ApplyWithVaultOptions {
  sdk: { _applyEngine: Deploy; repos: Repos };
  spec: ReleaseSpec;
  apply?: Omit<ApplyOptions, "vault">;
   ...
   * default as the manual `repos capture` lane; see `vault-snapshot.ts`).
   * Has no effect on a `none`/`grandfathered`/`ungated` deploy, which never
   * captures.
   */
  allowDirty?: boolean;
  /** Receives the `vault_commit` line the moment the snapshot exists. */
...
/**
 * Deploy, taking the project's `vault_policy` into account.
 *
 ...
 */
export async function applyWithVault(options: ApplyWithVaultOptions): Promise<ApplyWithVaultResult> {
  const applyOpts: Omit<ApplyOptions, "vault"> = {
    ...(options.apply ?? {}),
    ...(options.target ? { target: options.target } : {}),
  };
  const plain = async (mode: VaultApplyMode): Promise<ApplyWithVaultResult> => ({
    mode,
  ...
  const record = await readVaultRecord(options.sdk.repos, options.spec.project_id);
  ...
  if (record.vault_policy !== "required") {
    // D3: allocation no longer sets `vault_policy`, so a vault whose
    ...
    // runs the untouched plain path (same options object, no vault hooks);
    ...
    return { mode: { kind: "ungated", repo_id: record.repo_id }, deploy: decorateUngatedResult(deploy, record.repo_id), vault: null };
  }

  const lane = createApplyDeployLane({ engine: options.sdk._applyEngine, spec: options.spec, apply: applyOpts });
  let vault: VaultDeployResult;
  try {
    vault = await options.sdk.repos.deploy({
      lane,
    ...
    lane.abandon(new LocalError("the deploy ended without committing this plan", "committing vault deploy", { code: "VAULT_DEPLOY_ABANDONED" }));
  }
  return { mode: { kind: "vaulted", repo_id: record.repo_id }, deploy: lane.result(), vault };
}
 ...
 */
async function readVaultRecord(repos: Repos, projectId: string): Promise<VaultRecord | null> {
  try {
    return await repos.forProject(projectId);
  } catch {
    return null;
  }
}
 ...
 * `VAULT_REPO_STATE_MISSING` — surfacing the typed outcome rather than
...
function enrichCaptureRefusal(err: unknown, repoId: string): unknown {
  ...
  if (e.code !== "KEYSTORE_MISSING" && e.code !== "VAULT_REPO_STATE_MISSING") return err;
  return new LocalError(
    `${e.message} — this project's vault_policy is \`required\`, so a deploy from this machine needs the vault keystore`,
    "capturing for a vault-required deploy",
    {
      code: e.code,
