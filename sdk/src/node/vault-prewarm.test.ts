/**
 * vault-connection-amortization — the prewarm's spec-pinned guarantees:
...
import { prewarmVaultConnection, kickVaultConnection, predialVaultObjectStore, prewarmDeps } from "./vault-prewarm.js";

const flush = () => new Promise((r) => setImmediate(() => setImmediate(r)));

describe("prewarmVaultConnection", () => {
  ...
  it("fires exactly one abort-bounded GET to <base>/health and cancels the body", async () => {
    prewarmVaultConnection("https://api.example.test");
    await flush();
  ...
  it("returns void immediately — nothing a caller could await", () => {
    const out = prewarmVaultConnection("https://api.example.test") as unknown;
  ...
  it("a malformed base never throws", async () => {
    prewarmVaultConnection("not a url");
    await flush();
    assert.equal(fetched.length, 0);
  });

  // vault-first-op-premium task 2.1 — the narrow connection-only half a
  ...
  // probe every session (~150-300ms wasted, two live RPC calls a vault
  // session never needs) — the daemon already warmed that stack once at
  // boot. `kickVaultConnection` must do ONLY the dial, never the deferred
  // half, so a per-session pre-connect kick cannot regress into that cost.

  it("kickVaultConnection fires the same one abort-bounded GET to <base>/health and cancels the body", async () => {
    kickVaultConnection("https://api.example.test");
    await flush();
    assert.equal(fetched.length, 1);
    assert.equal(fetched[0]!.url, "https://api.example.test/health");
    assert.equal(fetched[0]!.hasSignal, true);
  ...
  it("kickVaultConnection never touches the signer or paid-stack warmup — that is the FULL prewarm's job, once per process, not per session", async () => {
    kickVaultConnection("https://api.example.test");
    await flush();
    assert.equal(signerCalls, 0, "kickVaultConnection must not re-warm the signer on every session");
  });

  it("kickVaultConnection returns void immediately", () => {
    const out = kickVaultConnection("https://api.example.test") as unknown;
    assert.equal(out, undefined);
  });

  it("kickVaultConnection swallows a rejecting fetch entirely — never a session-accept failure", async () => {
    prewarmDeps.fetch = (async () => {
      throw new Error("ECONNREFUSED");
    }) as typeof globalThis.fetch;
    assert.doesNotThrow(() => kickVaultConnection("https://api.example.test"));
    await flush();
  });

  it("kickVaultConnection on a malformed base never throws", async () => {
    kickVaultConnection("not a url");
    await flush();
    assert.equal(fetched.length, 0);
  });
});

// vault-object-host-predial task 2.1/3.1 — the object-store predial's
...
describe("predialVaultObjectStore", () => {
  let fetched: Array<{ url: string; hasSignal: boolean }> = [];
  let bodyCancelled = 0;
  ...
  it("a first-ever repo (nothing persisted) predials nothing — byte-identical to before this change", () => {
    prewarmDeps.readObjectStoreOrigins = () => [];
    predialVaultObjectStore("src_" + "1".repeat(32));
    assert.equal(fetched.length, 0);
  });
  ...
  it("a throwing origin lookup is swallowed entirely — never surfaces from the predial", () => {
    ...
    assert.doesNotThrow(() => predialVaultObjectStore("src_" + "1".repeat(32)));
    assert.equal(fetched.length, 0);
  });
  ...
  it("never touches sdk.stats() / the request kernel — the injected fetch is the ONLY transport surface it can reach, and it is the same non-kernel `sdkFetch` the existing prewarm already uses", async () => {
    // Structural guarantee, not a runtime one: `predialVaultObjectStore`
    // calls `prewarmDeps.fetch` exclusively (same as `kickVaultConnection`
