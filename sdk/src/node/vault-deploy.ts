 ...
 *   1. snapshot (§6.6) → the `vault_commit`, printed ALWAYS, never sent,
...
import { formatVaultTimestamp, hexToBytes, newHex32 } from "../namespaces/vault.crypto.js";
import type { VaultActivationToken, VaultCaptureBinding, VaultCaptureReceipt, VaultHeadTarget } from "../namespaces/vault.types.js";
import { VaultKeystore, readFileNoFollow, writeFileAtomic0600 } from "./vault-keystore.js";
import { Vault, captureBinding, deployRefTransaction, type VaultPublishResult } from "./vault-publication.js";
import { VAULT_DEPLOY_REF, capturedSetDigest, capturedSetUnchanged, captureSnapshot, deriveCapturedSet, diffCapturedSets, vaultCommitLine, snapshotCommitment, type VaultCapturedSetDrift, type VaultSnapshot, type VaultSnapshotOptions } from "./vault-snapshot.js";

// ─── Outcomes ────────────────────────────────────────────────────────────────

export const VAULT_DEPLOY_OUTCOMES = [
...
export type VaultDeployOutcome = (typeof VAULT_DEPLOY_OUTCOMES)[number];

/** A serializable error summary (never plaintext, never presigned URLs). */
export interface VaultDeployError {
  code: string;
  message: string;
  retryable: boolean;
  details?: unknown;
...
export interface VaultDeployNextAction {
  action: string;
  /** Present when the action needs a capability the caller may lack. */
  requires?: string;
}

interface VaultDeployResultBase {
  outcome: VaultDeployOutcome;
  /** The snapshot commit — ALWAYS present; the `vault_commit` line is derived from it. */
  vault_commit: string;
  ...
  next_actions: VaultDeployNextAction[];
}

export type VaultDeployResult =
  | (VaultDeployResultBase & { outcome: "DEPLOYED_AND_VAULTED"; generation: string; head_sha256: string; capture_receipt: VaultCaptureReceipt; activation_token: VaultActivationToken; commit: unknown })
  | (VaultDeployResultBase & { outcome: "DEPLOY_BLOCKED_PUSH_FAILED"; push_error: VaultDeployError; previous_release_keeps_serving: true })
  | (VaultDeployResultBase & { outcome: "DEPLOY_FAILED_VAULTED"; generation: string; head_sha256: string; capture_receipt: VaultCaptureReceipt | null; deploy_error: VaultDeployError })
  | (VaultDeployResultBase & { outcome: "DEPLOY_FAILED_UNVAULTED"; push_error: VaultDeployError; deploy_error: VaultDeployError })
  | (VaultDeployResultBase & { outcome: "DEPLOYED_UNVAULTED_OVERRIDE"; push_error: VaultDeployError; override_journal: VaultOverrideJournal; commit: unknown });

// ─── The deploy lane (injected; 5.10 wires `Run402.deploy`) ──────────────────

export interface VaultDeployLanePlanInput {
  /**
...
export interface VaultDeployLanePlan {
  plan_id: string;
...
export interface VaultDeployLaneCommitInput {
  plan_id: string;
  operation_id: string;
  /** Present on the vaulted path. */
  activation_token?: VaultActivationToken;
...
export interface VaultDeployLane {
  plan(input: VaultDeployLanePlanInput): Promise<VaultDeployLanePlan>;
  commit(input: VaultDeployLaneCommitInput): Promise<unknown>;
}

// ─── Override journal ────────────────────────────────────────────────────────

export interface VaultOverrideJournal {
  version: 1;
  repo_id: string;
  operation_id: string;
  capture_id: string;
  apply_plan_sha256: string;
  snapshot: { ref: string; oid: string; head_target: VaultHeadTarget };
  snapshot_oid_hmac: string;
...
export function overrideJournalDir(keystore: VaultKeystore): string {
  return join(keystore.journalDir, "overrides");
}

export function overrideJournalPath(keystore: VaultKeystore, operationId: string): string {
  if (!/^[A-Za-z0-9_.-]{1,128}$/.test(operationId)) throw new LocalError("operation_id is not a safe file name", "resolving override journal path", { code: "VAULT_BAD_ID" });
  return join(overrideJournalDir(keystore), `${operationId}.json`);
}

export function writeOverrideJournal(keystore: VaultKeystore, journal: VaultOverrideJournal): void {
...
export function readOverrideJournal(keystore: VaultKeystore, operationId: string): VaultOverrideJournal | null {
  const text = readFileNoFollow(overrideJournalPath(keystore, operationId));
  return text ? (JSON.parse(text) as VaultOverrideJournal) : null;
}

/** Every override journal not yet `completed` — the doctor-visible advisory list. */
export function listPendingOverrideJournals(keystore: VaultKeystore, repoId?: string): VaultOverrideJournal[] {
  const dir = overrideJournalDir(keystore);
  if (!existsSync(dir)) return [];
  const out: VaultOverrideJournal[] = [];
  for (const f of readdirSync(dir)) {
    if (!f.endsWith(".json")) continue;
    const text = readFileNoFollow(join(dir, f));
    if (!text) continue;
    const j = JSON.parse(text) as VaultOverrideJournal;
...
export interface VaultCaptureReceiptMatch {
  equal: boolean;
...
export function matchCaptureReceipt(receipt: VaultCaptureReceipt, expected: { repo_id: string; capture_id: string; apply_plan_sha256: string | null; snapshot_oid_hmac: string; head_sha256: string }): VaultCaptureReceiptMatch {
...
export function checkActivationTokenBinding(token: VaultActivationToken, expected: { repo_id: string; operation_id: string; generation: string; head_sha256: string; capture_id: string; apply_plan_sha256: string; snapshot_oid_hmac: string }): string[] {
...
export interface VaultDeployOptions {
  vault: Vault;
  lane: VaultDeployLane;
  /** The work tree to snapshot. */
  repo_dir: string;
  snapshot?: Omit<VaultSnapshotOptions, "dir">;
  ...
  capture_id?: string;
  now?: () => Date;
  /** Called with the `vault_commit` line as soon as the snapshot exists — the CLI prints it. */
  onCommitLine?: (line: string) => void;
}

function summarize(e: unknown): VaultDeployError {
  if (isRun402Error(e)) {
 ...
 * enrichment (`repos.ts#enrichEpochRotationRequired`) — same three
...
export interface VaultSnapshotMovedDetails extends VaultCapturedSetDrift {
  vault_commit: string;
  capture_id: string;
...
function driftSummary(drift: VaultCapturedSetDrift): string {
  const parts: string[] = [];
...
async function assertCapturedSetUnchanged(
  ...
  const details: VaultSnapshotMovedDetails = {
    ...drift,
    vault_commit: snapshot.oid,
 ...
 */
export async function runVaultDeploy(options: VaultDeployOptions): Promise<VaultDeployResult> {
  ...
  const commitLine = vaultCommitLine(snapshot);
  ...
  const base = (fields: Partial<VaultDeployResultBase>): VaultDeployResultBase => ({
    outcome: "DEPLOY_FAILED_UNVAULTED", vault_commit: snapshot.oid, vault_commit_line: commitLine, snapshot, capture_id: captureId, snapshot_oid_hmac: snapshotOidHmac,
  ...
  let plan: VaultDeployLanePlan | null = null;
  ...
  let published: VaultPublishResult | null = null;
  ...
  const pushPromise = (async () => {
    try {
      ...
      published = await vault.push({
        ...
        protocol_refs: "allow",
        capture_binding: async (): Promise<VaultCaptureBinding> => {
          await planPromise; // the digest (or its absence) must be known before the head is signed
          return captureBinding(captureId, plan ? (plan as VaultDeployLanePlan).apply_plan_sha256 : null, snapshotOidHmac);
        },
      });
    } catch (e) { pushError = e; }
  })();
  ...
  const planned = plan as VaultDeployLanePlan | null;
  const pub = published as VaultPublishResult | null;
  ...
  if (!pub) {
    ...
    if (!planned) {
      return { ...common, outcome: "DEPLOY_FAILED_UNVAULTED", push_error, deploy_error: summarize(planError), next_actions: [{ action: "fix the build, then redeploy" }, { action: "retry the push (run402 repos capture)" }] };
    }
    if (options.allow_unvaulted) {
      ...
      const journal: VaultOverrideJournal = {
        ...
        snapshot: { ref: VAULT_DEPLOY_REF, oid: snapshot.oid, head_target: snapshot.head }, snapshot_oid_hmac: snapshotOidHmac,
        publication_state: "pending", generation: null, head_sha256: null, capture_receipt: null, reason: options.allow_unvaulted.reason,
        created_at: formatVaultTimestamp(now()), updated_at: formatVaultTimestamp(now()),
      };
      ...
      try {
        ...
        return { ...common, outcome: "DEPLOY_FAILED_UNVAULTED", push_error, deploy_error: summarize(e), next_actions: [{ action: "retry the push (run402 repos capture) — the override journal is kept and will drain" }, { action: "retry the deploy" }] };
      }
    ...
    return {
      ...
      next_actions: [
        ...
        // lane would only ever fail — see Vault.rotateEpoch's doc
  ...
  if (!pub.capture_receipt) {
    return { ...common, outcome: "DEPLOY_FAILED_VAULTED", ...vaulted, deploy_error: { code: "VAULT_CAPTURE_RECEIPT_MISSING", message: "admission returned no capture receipt for a head carrying a capture binding; no activation token can be exchanged", retryable: true }, next_actions: [{ action: "retry the deploy" }] };
  }
  ...
  let token: VaultActivationToken;
  try {
    ...
    if (problems.length > 0) throw new LocalError(`the minted activation token does not bind this deploy (${problems.join(", ")}); refusing to present it`, "checking activation token", { code: "VAULT_TOKEN_BINDING_MISMATCH", details: { problems } });
...
export interface VaultOverrideDrainReport {
  completed: VaultOverrideJournal[];
  /** Still pending, with the reason the drain could not complete them this time. */
  remaining: Array<{ journal: VaultOverrideJournal; error: VaultDeployError }>;
}
 ...
 */
export async function drainOverrideJournals(vault: Vault, options: { now?: () => Date } = {}): Promise<VaultOverrideDrainReport> {
  const now = options.now ?? (() => new Date());
  const report: VaultOverrideDrainReport = { completed: [], remaining: [] };
  for (const journal of listPendingOverrideJournals(vault.keystore, vault.repoId)) {
    try {
      ...
      if (j.publication_state === "pending") {
        ...
        j = { ...j, publication_state: "published", generation: pub.generation, head_sha256: pub.head_sha256, capture_receipt: pub.capture_receipt, updated_at: formatVaultTimestamp(now()) };
        writeOverrideJournal(vault.keystore, j);
      }
      if (!j.capture_receipt || !j.head_sha256) throw new LocalError("the journaled publication has no capture receipt to present", "draining override journal", { code: "VAULT_CAPTURE_RECEIPT_MISSING" });
      ...
      if (!match.equal) throw new LocalError(`the capture receipt does not match the journaled operation on ${match.mismatched_fields.join(", ")}; the advisory persists`, "draining override journal", { code: "VAULT_OVERRIDE_COMPLETION_MISMATCH", details: match });
      ...
      if (!res.cleared) throw new LocalError("the platform did not clear the override advisory", "draining override journal", { code: "VAULT_OVERRIDE_COMPLETION_REFUSED" });
      j = { ...j, publication_state: "completed", updated_at: formatVaultTimestamp(now()) };
      writeOverrideJournal(vault.keystore, j);
