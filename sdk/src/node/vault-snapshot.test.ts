/**
 * vault snapshot (task 5.5) — policies (clean/dirty/unborn/detached/
...
import { commitFile, git, makeRepo } from "./vault-memory-transport.test.js";
import { deriveDigestKey, keyedCommitment, randomBytes } from "../namespaces/vault.crypto.js";

let root: string;
let home: string;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-snapshot-"));
  home = join(root, "home");
...
describe("snapshot policies", () => {
  it("clean tree → HEAD (kind head), symref head_target, vault_commit line", async () => {
    ...
    assert.equal(vaultCommitLine(s), `vault_commit ${head}`);
  });
  ...
  it("dirty tree → synthetic commit of tracked + untracked-not-ignored (allowDirty); git status unchanged; branches untouched; disclosure lists what was captured", async () => {
    ...
    assert.equal(vaultCommitLine(s), `vault_commit ${s.oid} (synthetic)`);
    ...
    assert.equal(await git(dir, ["rev-parse", VAULT_DEPLOY_REF]), s.oid);
  });
...
describe("frozen ignore authority — global excludes as data", () => {
  ...
  it("an existing-but-unreadable excludes file refuses the capture BY NAME, never a silently widened snapshot", async () => {
    ...
    try {
      const caught = await refusal(captureSnapshot({ dir, env: env() }));
      assert.equal(caught?.code, "VAULT_EXCLUDES_UNREADABLE");
  ...
  it("an unreadable CONFIG file (not only the excludes file) refuses by name", async () => {
    ...
    try {
      assert.throws(() => discoverGlobalExcludes(env()), (e: { code?: string; details?: { file?: string } }) => e.code === "VAULT_CONFIG_UNREADABLE" && e.details?.file === join(home, ".gitconfig"));
    } finally {
  ...
  it("include / includeIf constructs refuse the capture by name; relative or env-dependent paths refuse by name", () => {
    ...
    assert.throws(() => discoverGlobalExcludes(env()), (e: { code?: string }) => e.code === "VAULT_CONFIG_INCLUDE_REFUSED");
    writeFileSync(join(home, ".gitconfig"), '[includeIf "gitdir:~/work/"]\n  path = ~/other\n');
    assert.throws(() => discoverGlobalExcludes(env()), (e: { code?: string }) => e.code === "VAULT_CONFIG_INCLUDE_REFUSED");
    writeFileSync(join(home, ".gitconfig"), "[core]\n  excludesFile = relative/ignores\n");
    assert.throws(() => discoverGlobalExcludes(env()), (e: { code?: string }) => e.code === "VAULT_EXCLUDES_PATH_UNSUPPORTED");
...
describe("filter-free object creation (no-filter acceptance)", () => {
  it("a hostile clean filter on an UNTRACKED file never runs: capture refuses by name and the sentinel is never written", async () => {
    ...
    assert.equal(caught?.code, "VAULT_FILTER_ACTIVE");
  ...
  it("LFS is refused by its own name", async () => {
    ...
    assert.equal(await code(captureSnapshot({ dir, env: env(), allowDirty: true })), "VAULT_LFS_UNSUPPORTED");
  });
...
describe("unsupported repositories are refused by name", () => {
  it("linked worktree", async () => {
    ...
    assert.equal(await code(inspectRepository(linked)), "VAULT_LINKED_WORKTREE");
    assert.equal(await code(captureSnapshot({ dir: linked, env: env() })), "VAULT_LINKED_WORKTREE");
  ...
  it("bare repository, sparse checkout, submodule gitlink, non-repository, repo-config include", async () => {
    ...
    assert.equal(await code(inspectRepository(bare)), "VAULT_BARE_REPOSITORY");

    const sparse = await makeRepo(root, "sparse");
    await git(sparse, ["config", "core.sparseCheckout", "true"]);
    assert.equal(await code(inspectRepository(sparse)), "VAULT_SPARSE_CHECKOUT");
    ...
    assert.equal(await code(inspectRepository(sub)), "VAULT_SUBMODULES_UNSUPPORTED");

    const plain = join(root, "plain");
    mkdirSync(plain);
    assert.equal(await code(inspectRepository(plain)), "VAULT_NOT_A_REPOSITORY");

    const inc = await makeRepo(root, "inc");
    writeFileSync(join(inc, ".git", "config"), readFileSync(join(inc, ".git", "config"), "utf8") + "[include]\n\tpath = /tmp/nothing\n");
    assert.equal(await code(inspectRepository(inc)), "VAULT_CONFIG_INCLUDE_REFUSED");
  });
});
