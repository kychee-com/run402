/**
 * vault — publication (protocol rev 41 §6 + §4.4–4.7 + §5A client side;
 * task 5.4).
 *
 * What lives here:
 ...
 *   - {@link VaultTransport} — the control-plane + bucket operations the
 *     vault needs (extends task 5.3's creation transport) — and
 *     {@link createVaultHttpTransport}, the `fetch`-backed implementation
 *     over the SDK kernel (upload sessions → presigned PUT with
 *     `If-None-Match: *` → finalize);
 *   - {@link Vault} — discover/verify to newest with the RESUMABLE
...
import { fetchVaultObjectBytes } from "./vault-edge-fetch.js";
import { openVaultDestinationBackend, type VaultMirrorBackend } from "./vault-mirror-backend.js";
import type { VaultMirrorCredential, VaultMirrorDestination } from "./vault-mirror-config.js";
import {
  VAULT_FORMAT,
  VAULT_GENESIS_EPOCH,
  VAULT_GENESIS_GENERATION,
  VAULT_HEX16_RE,
  VAULT_OID40_RE,
  VAULT_SUITE,
  ...
  ekFingerprint,
  vkFingerprint,
  VAULT_TERMINAL_LOSS_DOCTOR_TEXT,
  VAULT_TERMINAL_LOSS_STATEMENT,
  ...
  toBase64url,
  verifyVaultObject,
} from "../namespaces/vault.crypto.js";
import { VAULT_ZERO_SHA256_SENTINEL } from "../namespaces/vault.types.js";
import { vaultCheckpointStaleness, type VaultCheckpointStaleness } from "../namespaces/repos.js";
import type {
  VaultActivationToken,
  VaultAddWriterKeyPayload,
  VaultAllocation,
  VaultCaptureBinding,
  VaultCaptureReceipt,
  VaultCheckpointBlock,
  VaultCheckpointClaimSet,
  VaultCheckpointManifest,
  VaultCheckpointManifestPack,
  VaultCheckpointPackReceipt,
  VaultDigestLabel,
  VaultEpochRotationSelfOpen,
  VaultHead,
  VaultHeadTarget,
  VaultHeadsListingEntry,
  VaultHeadsListingPage,
  VaultHeadsListingRequest,
  VaultOpenReceipt,
  VaultPinManifestReceipt,
  VaultRecipientConfirmationReceipt,
  VaultRecipientPinManifestEntry,
  VaultRefState,
  VaultRefStateReceipt,
  VaultRefTransaction,
  VaultRefUpdate,
  VaultRepairDescriptor,
  VaultRetentionCutoff,
  VaultRetentionCutoffReceipt,
  VaultRetentionRoot,
  VaultRetentionRoots,
  VaultRetentionRootsReceipt,
  VaultRotateEpochPayload,
  VaultRotationAttemptDescriptor,
  VaultRotationEnvelopePair,
  VaultRotationReason,
  VaultSignedObject,
  VaultSigningKeypair,
  VaultTransitionEnvelope,
  VaultGenesis,
  VaultKeyEnvelope,
  VaultWalPackReceipt,
} from "../namespaces/vault.types.js";
import type {
...
import { VaultKeystore, type VaultHeadPin, type VaultRepoFile } from "./vault-keystore.js";
import { crossProfileVaultHint } from "./vault-profile-scan.js";
import {
  applyAddWriterKey,
...
import type { VaultPruneIntentRecord } from "./vault-prune.js";
import { VAULT_DEPLOY_REF, hardenedGit, hasObject, hasObjects, isAncestor } from "./vault-snapshot.js";

// ─── Constants (constants.json) ──────────────────────────────────────────────

export const VAULT_MAX_CANONICAL_REFS = 10_000;
export const VAULT_MAX_REF_UPDATES_PER_TRANSACTION = 1_000;
export const VAULT_MAX_RETENTION_ROOT_ENTRIES = 50_000;
export const VAULT_MAX_REPAIR_ADDED_ROOTS = 10_000;
export const VAULT_MAX_WAL_RECEIPTS_PER_HEAD = 64;
export const VAULT_MAX_CHECKPOINT_PACKS = 4_096;
export const VAULT_MAX_CHECKPOINT_TOTAL_STORED_BYTES = 858_993_459_200n;
export const VAULT_MULTI_OBJECT_PACK_TARGET_BYTES = 201_326_592;
export const VAULT_MAX_REF_STATE_OBJECT_BYTES = 33_554_432;
export const VAULT_MAX_HEADS_PER_LISTING_PAGE = 1_000;
export const VAULT_VERIFICATION_BUDGET_HEADS = 100_000;
export const VAULT_RETENTION_MIN_DAYS = 90;
/** Default admission-conflict retries before the push gives up (each retry re-verifies + re-applies to the winner). */
export const VAULT_PUSH_CONFLICT_RETRIES = 5;
 ...
 * cross-runtime `namespaces/repos.ts` via a dynamic `import()`, task
 * 5.0's public crypto surface) back onto the Node CLI/SDK entry module.
 */
const VAULT_SDK_PACKAGE_VERSION: string = (() => {
  try {
 ...
 */
export function vaultReaderEntrypoint(entrypoint: string): string {
  return `run402@${VAULT_SDK_PACKAGE_VERSION}/${entrypoint}`;
}

// ─── Generations ─────────────────────────────────────────────────────────────

export function generationToBigInt(generation: string): bigint {
  if (!VAULT_HEX16_RE.test(generation)) fail("CHAIN_BROKEN", `malformed generation: ${generation}`, "parsing generation");
...
export type VaultRefMap = Record<string, string>;

/** A tip that left the canonical map in this transaction — it enters `retention_roots` in the same generation. */
export interface VaultDroppedTip {
  ref: string;
  oid: string;
  reason: "deleted" | "force_displaced";
}

export interface VaultRefTransactionEvaluation {
  refs: VaultRefMap;
  dropped: VaultDroppedTip[];
}

export interface VaultEvaluateRefTransactionOptions {
...
export interface VaultRefUpdateFailure {
  ref: string;
 ...
 */
export async function evaluateRefTransaction(current: VaultRefMap, transaction: VaultRefTransaction, options: VaultEvaluateRefTransactionOptions): Promise<VaultRefTransactionEvaluation> {
  ...
  if (updates.length > VAULT_MAX_REF_UPDATES_PER_TRANSACTION) {
    fail("REF_STATE_LIMIT_EXCEEDED", `${updates.length} updates exceed the ${VAULT_MAX_REF_UPDATES_PER_TRANSACTION}-update transaction bound`, "evaluating ref transaction", { updates: updates.length, bound: VAULT_MAX_REF_UPDATES_PER_TRANSACTION });
  }
  ...
  for (const u of updates) {
    ...
    if (u.expected_old_oid !== null && !VAULT_OID40_RE.test(u.expected_old_oid)) fail("REF_TRANSACTION_INVALID", `expected_old_oid for ${u.ref} is not a 40-hex oid`, "evaluating ref transaction", { ref: u.ref });
    if (u.new_oid !== null && !VAULT_OID40_RE.test(u.new_oid)) fail("REF_TRANSACTION_INVALID", `new_oid for ${u.ref} is not a 40-hex oid`, "evaluating ref transaction", { ref: u.ref });
  ...
  const failures: VaultRefUpdateFailure[] = [];
  const next: VaultRefMap = { ...current };
  const dropped: VaultDroppedTip[] = [];
  for (const u of updates) {
  ...
  if (failures.length > 0) {
    ...
    // The force-with-lease inversion (vault-force-spelling-and-pin-fold):
...
export function assertRefMapCardinality(refs: VaultRefMap): void {
  const n = Object.keys(refs).length;
  if (n > VAULT_MAX_CANONICAL_REFS) fail("REF_STATE_LIMIT_EXCEEDED", `${n} canonical refs exceed the ${VAULT_MAX_CANONICAL_REFS} bound`, "checking ref-state cardinality", { refs: n });
  if (jcs(refs).length > VAULT_MAX_REF_STATE_OBJECT_BYTES) fail("REF_STATE_LIMIT_EXCEEDED", "the serialized ref map exceeds 32 MiB", "checking ref-state cardinality");
}

/** The §4.4 deploy-ref move: force-with-lease `refs/run402/deploys/latest` → `oid` (creation when absent). */
export function deployRefTransaction(current: VaultRefMap, oid: string): VaultRefTransaction {
  const old = current[VAULT_DEPLOY_REF] ?? null;
  return { updates: [{ ref: VAULT_DEPLOY_REF, expected_old_oid: old, new_oid: oid, force: old !== null }] };
}
...
export function isRootEligibleForRemoval(effectiveAdmittedAtIso: string, cutoffAtIso: string, retentionDays = VAULT_RETENTION_MIN_DAYS): boolean {
  const expiry = Date.parse(effectiveAdmittedAtIso) + retentionDays * 24 * 60 * 60 * 1000;
  return expiry < Date.parse(cutoffAtIso);
}

export interface VaultEvolveRootsOptions {
...
export function compareRoots(a: VaultRetentionRoot, b: VaultRetentionRoot): number {
...
export function evolveRetentionRoots(previous: VaultRetentionRoot[], options: VaultEvolveRootsOptions): VaultRetentionRoot[] {
  const map = new Map<string, VaultRetentionRoot>();
  ...
  if (roots.length > VAULT_MAX_RETENTION_ROOT_ENTRIES) fail("REF_STATE_LIMIT_EXCEEDED", `${roots.length} retention roots exceed the ${VAULT_MAX_RETENTION_ROOT_ENTRIES} bound`, "evolving retention roots", { roots: roots.length });
  return roots;
}
 ...
 * VAULT-side `refs/run402/*` (e.g. `VAULT_DEPLOY_REF`) that rides the wire
 ...
 */
export const VAULT_R402_REF_NAMESPACE = "refs/r402/";
/** Where a retained (branch-unreachable) deploy-capture tip gets its local ref (design D1/D2). */
export const VAULT_RETAIN_REF_PREFIX = `${VAULT_R402_REF_NAMESPACE}retain/`;

/**
 * D1's ref-identity choice, recorded here because the design doc's own
 * assumption did not hold: `VaultRetentionRoot` carries no per-capture
 * stable id (only `{ref, oid, dropped_at_generation}`) — the capture id that
 * DOES exist (`VaultHead.capture_binding.capture_id`) lives on the head
 ...
 */
export function vaultRetainedRefName(oid: string): string {
  return `${VAULT_RETAIN_REF_PREFIX}${oid}`;
}

export interface VaultRetainedRefsReconcileResult {
 ...
 */
/**
 * Name prefix of the scratch directory {@link Vault.buildPacks}
 ...
 */
export const VAULT_PACK_SCRATCH_PREFIX = ".run402-vault-packs-";

/**
 * Allocate the pack-objects scratch directory on the OBJECT STORE's own
 * filesystem: `<git-common-dir>/.run402-vault-packs-XXXXXX`, never under
 * `os.tmpdir()`.
 *
...
export async function allocatePackScratchDir(repoDir: string): Promise<string> {
  ...
  return mkdtempSync(join(commonDir, VAULT_PACK_SCRATCH_PREFIX));
}

export async function reconcileRetainedTipRefs(repoDir: string, state: { refs: VaultRefMap; roots: readonly VaultRetentionRoot[]; head_target: VaultHeadTarget }): Promise<VaultRetainedRefsReconcileResult> {
  const empty = (warning: string | null = null): VaultRetainedRefsReconcileResult => ({ written: [], deleted: [], retained_count: 0, warning });
  let isRepo: boolean;
  try {
  ...
  try {
    ...
    const existing = await listRefsUnderPrefix(repoDir, VAULT_RETAIN_REF_PREFIX);
    const desired = new Map(retainedOids.map((oid) => [vaultRetainedRefName(oid), oid]));
...
export interface VaultListingProgress {
...
export function validateHeadsListingRequest(request: VaultHeadsListingRequest): void {
  if (!VAULT_HEX16_RE.test(request.after_generation)) fail("VAULT_LISTING_REQUEST_INVALID", "after_generation is REQUIRED and must be 16 lowercase hex (the verification anchor)", "listing heads", { after_generation: request.after_generation });
  if (!LIMIT_RE.test(request.limit)) fail("VAULT_LISTING_REQUEST_INVALID", "limit is REQUIRED and must be 1..1000", "listing heads", { limit: request.limit });
 ...
 * violation / retired member → `VAULT_LISTING_PAGE_INVALID`; an entry at or
 * below the anchor → `GENERATION_REGRESSION`; a gap within or across pages →
 * `CHAIN_BROKEN`; an untruthful final `total` → `CHAIN_BROKEN`.
 */
export function verifyHeadsListingPage(page: VaultHeadsListingPage, request: VaultHeadsListingRequest, progress: VaultListingProgress, expectedRepoId?: string): VaultListingProgress {
  const p = page as VaultHeadsListingPage & Record<string, unknown>;
  if (p.format !== VAULT_FORMAT) fail("VAULT_LISTING_PAGE_INVALID", "listing page has the wrong format", "listing heads");
  if ("next_after_generation" in p) fail("VAULT_LISTING_PAGE_INVALID", "listing page carries the retired next_after_generation member (D186 forbids it)", "listing heads");
  if (expectedRepoId && p.repo_id !== expectedRepoId) fail("VAULT_LISTING_PAGE_INVALID", "listing page is for a different vault", "listing heads", { repo_id: p.repo_id });
  if (p.after_generation !== request.after_generation || p.after_generation !== progress.after_generation) fail("VAULT_LISTING_PAGE_INVALID", "the page did not echo the request's after_generation anchor", "listing heads", { echoed: p.after_generation, anchor: request.after_generation });
  if (typeof p.has_more !== "boolean") fail("VAULT_LISTING_PAGE_INVALID", "has_more must be a boolean", "listing heads");
  if (p.has_more === false && p.next_cursor !== null) fail("VAULT_LISTING_PAGE_INVALID", "has_more=false with a non-null next_cursor is contradictory", "listing heads");
  if (p.has_more === true && (typeof p.next_cursor !== "string" || !CURSOR_RE.test(p.next_cursor))) fail("VAULT_LISTING_PAGE_INVALID", "has_more=true requires a non-null opaque next_cursor", "listing heads");
  if (!Array.isArray(p.heads) || p.heads.length > VAULT_MAX_HEADS_PER_LISTING_PAGE || p.heads.length > Number(request.limit)) fail("VAULT_LISTING_PAGE_INVALID", "heads[] exceeds the page limit", "listing heads");
  if (p.total !== null && !/^(0|[1-9][0-9]{0,14})$/.test(p.total)) fail("VAULT_LISTING_PAGE_INVALID", "total must be an exact decimal string or null", "listing heads");
 ...
 */
export function nextListingRequest(request: VaultHeadsListingRequest, page: VaultHeadsListingPage): VaultHeadsListingRequest | null {
  if (!page.has_more) return null;
  if (page.next_cursor === null) fail("VAULT_LISTING_PAGE_INVALID", "has_more=true requires a non-null opaque next_cursor", "listing heads");
...
export interface VaultOpenBindingRecord {
...
export function openBindingDigest(clientOpenId: string, record: VaultOpenBindingRecord): string {
 ...
 * `VAULT_OPEN_BINDING_MISMATCH`; a same-`client_open_id` retry that carries a
 * DIFFERENT binding is the registry's `CLIENT_OPEN_ID_CONFLICT`.
 */
export function checkOpenBinding(clientOpenId: string, record: VaultOpenBindingRecord, issuanceOpenBindingSha256: string, options: { retry?: boolean } = {}): void {
  ...
  fail("VAULT_OPEN_BINDING_MISMATCH", `the C1 record does not recompute to the signed issuance binding for ${clientOpenId}`, "checking maintenance open binding", { recomputed, issued: issuanceOpenBindingSha256 });
}

// ─── Chain + transition (pure) ───────────────────────────────────────────────

export interface VaultChainLinkInput {
  head: VaultHead;
  ...
  /**
   * The predecessor's own `epoch` (D194, rev 42) — `VAULT_GENESIS_EPOCH`
...
export function checkChainLink(input: VaultChainLinkInput): void {
  const h = input.head;
  ...
  if (h.format !== VAULT_FORMAT || h.object_kind !== "head" || h.suite !== VAULT_SUITE) fail("CHAIN_BROKEN", `head ${input.expected_generation}: not an r402s/v0 head`, "verifying head chain");
  ...
  // keyed step (`Vault`'s rotation-envelope open, `openEpochRotationForRecipient`).
  ...
  if (!verifyVaultObject(h as unknown as VaultSignedObject, input.writer_public_key)) fail("CHAIN_BROKEN", `head ${h.generation}: signature does not verify under the registered writer key`, "verifying head chain");
  ...
  if (h.wal_entries.length > VAULT_MAX_WAL_RECEIPTS_PER_HEAD) fail("CHAIN_BROKEN", `head ${h.generation}: ${h.wal_entries.length} WAL receipts exceed the 64 budget`, "verifying head chain");
}
 ...
 * (`Vault.verifyToNewest`) handle it fully. `add_writer_key` is
 * EXEMPT as of rev 47 (vault-multi-writer) the same way — {@link
 * parseAddWriterKeyPayload} + `validateAddWriterKeyPayload`
 * (`vault-writer-state.js`) handle it fully, INSIDE `verifyToNewest`'s
 ...
 */
export function assertNoTransition(head: VaultHead): void {
...
export interface VaultUploadObject {
  /**
...
export interface VaultUploadReceipt {
  path: string;
  object_id: string | null;
  sha256: string;
  size_bytes: string;
 ...
 * the SDK's internal `vaultPaths` strings onto that identity so the rest of
 * the vault can keep addressing objects the way §3 describes them.
 */
export type VaultWireRef =
  | { kind: "head"; generation: string }
  | { kind: "admission"; generation: string }
  | { kind: "object"; read: VaultObjectReadRequest };

/** One entry of a `POST …/object-reads` batch. */
export interface VaultObjectReadRequest {
  object_kind: string;
  object_id?: string;
  epoch?: string;
...
const PATH_PATTERNS: Array<[RegExp, (m: RegExpExecArray) => VaultWireRef]> = [
...
export function vaultWireRefForPath(path: string): VaultWireRef | null {
...
export function vaultManifestEntry(object: VaultUploadObject): VaultObjectReadRequest & { sha256: string; size_bytes: string; base_generation?: string } {
  const ref = vaultWireRefForPath(object.path);
  if (!ref || ref.kind !== "object") {
    fail("VAULT_UPLOAD_SESSION_INVALID", `${object.path} is not an uploadable object path; the control plane addresses uploads by object_kind + ledger identity`, "building the vault upload manifest", { path: object.path });
  }
  const entry = { ...ref.read, sha256: object.sha256, size_bytes: object.size_bytes } as VaultObjectReadRequest & { sha256: string; size_bytes: string; base_generation?: string };
  if (entry.object_kind === "wal_pack") {
    if (object.base_generation === undefined) fail("VAULT_UPLOAD_SESSION_INVALID", "a wal_pack upload must declare base_generation (§4.1)", "building the vault upload manifest", { path: object.path });
    entry.base_generation = object.base_generation;
  }
  return entry;
}

// ─── Inline upload (vault-composite-state-read design D2) ─────────────────

/** Mirrors the gateway's `VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES` (`services/vault/upload-sessions.ts`). */
export const VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES = 262_144;
/** Mirrors the gateway's `VAULT_INLINE_UPLOAD_MAX_REQUEST_BYTES`. */
export const VAULT_INLINE_UPLOAD_MAX_REQUEST_BYTES = 1_048_576;
 ...
 *
 * Takes the narrowest shape that satisfies every call site (`VaultUploadObject[]`
 ...
 * question.
 */
export function vaultInlineUploadEligible(objects: readonly { bytes: Uint8Array }[]): boolean {
  if (objects.length === 0) return false;
  let total = 0;
  for (const o of objects) {
    if (o.bytes.length > VAULT_INLINE_UPLOAD_MAX_OBJECT_BYTES) return false;
    total += o.bytes.length;
  }
  return total <= VAULT_INLINE_UPLOAD_MAX_REQUEST_BYTES;
}
 ...
 * (services/vault/epoch-rotation.ts) exactly; drift here breaks receipt
 * pairing at upload finalize for a rotation-attempt envelope or a pin
 * manifest.
 */
export function vaultLedgerId(read: VaultObjectReadRequest): string {
...
export interface VaultAdmitHeadRequest {
  repo_id: string;
  generation: string;
  stored_bytes: Uint8Array;
...
export type VaultAdmitHeadResult =
  | { outcome: "admitted"; admission_record_sha256: string; capture_receipt: VaultCaptureReceipt | null }
  | { outcome: "conflict"; winner: { generation: string; stored_bytes_sha256: string } };

export interface VaultRetentionCutoffIssued {
  ticket: VaultRetentionCutoff;
  receipt: VaultRetentionCutoffReceipt;
}
...
export type VaultResourceBinding =
  | { kind: "ordinary_push" }
...
export interface VaultMaintenanceLeaseRequest {
  repo_id: string;
  base_head_sha256: string;
...
export interface VaultMaintenanceLease {
...
/** `POST …/compaction-grant`'s result (vault-checkpoint-cadence design D3). */
export interface VaultCompactionGrant {
  /**
...
export interface VaultTransport extends VaultCreationTransport {
  /**
   * Read N independent carrier objects (ref_state, retention_roots, WAL/
   * checkpoint packs — anything `object-reads`-addressed, never a
   * generation-addressed head/admission) in ONE presigned batch (vault-
   ...
   * index, the same "absent" reading {@link VaultCreationTransport.getObject}
   * gives for one object. Callers that need exactly one object still use
   * {@link VaultCreationTransport.getObject} — this is for the plural
   * case only, so a single-object caller never pays a batch's overhead.
   *
   * `expected` (vault-small-object-inline design D3), when supplied, is
   ...
   * {@link getObjects} (vault-pipelined-restore D2): one presign POST,
   ...
   * (vault-small-object-inline design D3 — see {@link getObjects}'s doc
   ...
   * POST (`…/head-reads`, vault-batched-head-reads).
   *
  ...
  listHeads(request: VaultHeadsListingRequest & { repo_id: string }): Promise<VaultHeadsListingPage>;
  /**
   * Session → create-only presigned PUTs (`If-None-Match: *`) → finalize;
   * receipts in request order.
   *
   * vault-byo-primary-bucket task 3.2 — `byo`, when present, marks this
  ...
  uploadObjects(request: {
    repo_id: string;
    objects: VaultUploadObject[];
    resource_binding?: VaultResourceBinding;
    byo?: { destination: VaultMirrorDestination; credential?: VaultMirrorCredential };
  }): Promise<VaultUploadReceipt[]>;
  admitHead(request: VaultAdmitHeadRequest): Promise<VaultAdmitHeadResult>;
  requestRetentionCutoff(request: { repo_id: string; base_head_sha256: string }): Promise<VaultRetentionCutoffIssued>;
  exchangeActivationToken(request: { repo_id: string; operation_id: string; capture_receipt: VaultCaptureReceipt }): Promise<VaultActivationToken>;
  submitOverrideCompletion(request: { repo_id: string; operation_id: string; capture_receipt: VaultCaptureReceipt }): Promise<{ cleared: boolean }>;
  /** The vault record — policy, allocation generation, storage + maintenance state (§9.2). */
  getVaultRecord(request: { repo_id: string }): Promise<VaultRecord>;
  /**
   * `GET …/state` (vault-composite-state-read design D1) — the pin-current
   ...
   * VaultCreationTransport.getObject} gives for one object — the caller
   * treats it identically, never a route-level distinction).
   *
   * {@link Vault} verifies every field here EXACTLY as it does
   ...
   *
   * `since` (vault-delta-fetch): the caller's materialized generation.
   ...
   *
   * `restore` (vault-restore-recipe design D2), ORTHOGONAL to `since` —
   ...
   * bytes are UNTRUSTED exactly like `delta`'s: {@link Vault}
   ...
   * any failure.
   */
  getState(request: { repo_id: string; since?: string; restore?: boolean }): Promise<VaultState>;
  /** Resolve a project's vault without local state (the cold-restart entry point). */
  findVaultByProject(request: { project_id: string }): Promise<VaultRecord>;
  /**
   * Resolve a vault by its address-form `org-slug/name` (repo-first-onramp
   * task 4.3, design D6) — `GET /vaults/v1?repo=<org-slug>/<name>`.
   ...
   * auto-followed.
   */
  findVaultByRepo(request: { org_slug: string; repo_name: string }): Promise<VaultRecord>;
  acquireMaintenanceLease(request: VaultMaintenanceLeaseRequest): Promise<VaultMaintenanceLease>;
  ...
  /**
   * `POST …/compaction-grant` (vault-checkpoint-cadence design D3) — a
   ...
   * (`--force-headroom`). Rejects `VAULT_COMPACTION_GRANT_ACTIVE` (409)
   ...
   */
  openCompactionGrant(request: { repo_id: string }): Promise<VaultCompactionGrant>;
  /**
   ...
   */
  submitPruneIntent(request: { repo_id: string; intent_bytes: Uint8Array }): Promise<VaultPruneIntentRecord & { stored: boolean }>;
  /** `GET …/prune-intents/:id` — the intent's state and, once signed, its completion. */
  getPruneIntent(request: { repo_id: string; prune_intent_object_id: string }): Promise<VaultPruneIntentRecord | null>;
  /**
   * The org's directory of envelope-capable principals (vault-human-
   ...
   * {@link Vault.reconcileEnvelopeRecipients} to diff against a
   * vault's current recipient set.
   */
  listOrgEncryptionKeys(request: { org_id: string }): Promise<VaultOrgEncryptionKeyDirectory>;
  /**
   * The `ek_` fingerprints already covering this vault, at any epoch
   * (`GET /vaults/v1/:vault_id/envelope-recipients`, task 2.2) —
   * fingerprints only, never envelope bytes; the recipient-only rule on
   * envelope BYTES elsewhere is unaffected.
   */
  listEnvelopeRecipients(request: { repo_id: string }): Promise<VaultEnvelopeRecipientsResponse>;
   ...
   */
  createRotationAttempt(request: { repo_id: string; descriptor: VaultRotationAttemptDescriptor }): Promise<{ rotation_id: string; descriptor: VaultRotationAttemptDescriptor; deduplicated: boolean }>;
  /** `POST …/recipients/:principal_id/confirm` (D197) — first-seen pin confirmation; owner + step-up. */
  confirmRecipient(request: { repo_id: string; principal_id: string; new_fingerprint: string }): Promise<VaultRecipientConfirmationReceipt>;
  /** `POST …/recipients/:principal_id/repin` (D197) — re-pin ceremony; owner + step-up. */
  repinRecipient(request: { repo_id: string; principal_id: string; old_ek_fingerprint: string; new_fingerprint: string }): Promise<VaultRecipientConfirmationReceipt>;
   ...
   * `VAULT_ACCESS_DENIED`, not a `proof-of-open`-specific error.
  ...
  }): Promise<{ receipt: VaultOpenReceipt; deduplicated: boolean }>;
}

/**
 * One row of the org's envelope-capable-principal directory
 * ({@link VaultTransport.listOrgEncryptionKeys}).
 *
 ...
 * same way `desired[]` does on {@link VaultEnvelopeRecipientsResponse}:
 ...
 * (`missing_public_key`) from {@link Vault.reconcileEnvelopeRecipients},
 * never a hardcoded assumption or a thrown error.
 */
export interface VaultOrgEncryptionKeyEntry {
  principal_id: string;
  ...
  public_key?: string;
  /**
   * vault-multi-writer (rev 47) D9 — the SAME row's vault-WRITER signing
...
/** `GET /orgs/v1/:org_id/encryption-keys` — {@link VaultTransport.listOrgEncryptionKeys}'s result. */
export interface VaultOrgEncryptionKeyDirectory {
  org_id: string;
  keys: VaultOrgEncryptionKeyEntry[];
}
 ...
 * `"pending_removal"` means membership removed this principal but vault
 ...
 */
export interface VaultDesiredRecipientEntry {
  principal_id: string;
  display_name: string | null;
...
/** `GET /vaults/v1/:vault_id/envelope-recipients` — {@link VaultTransport.listEnvelopeRecipients}'s result. */
export interface VaultEnvelopeRecipientsResponse {
  vault_id: string;
  recipient_fingerprints: string[];
  /**
   * The org's desired-recipient state, cross-referenced against this
   * vault's coverage — see {@link VaultDesiredRecipientEntry}. OPTIONAL:
   ...
   */
  desired?: VaultDesiredRecipientEntry[];
   ...
   * from ({@link Vault.rotateEpochForMemberRemoval}). OPTIONAL:
...
/** `GET /vaults/v1/:vault_id` — the shape `reads.ts:getVaultRecord` returns. */
export interface VaultRecord {
  repo_id: string;
  project_id: string;
  org_id: string;
  vault_policy: "required" | "grandfathered" | null;
  vault_policy_version: string;
  vault_policy_changed_at: string | null;
  ...
  /**
   * vault-byo-primary-bucket task 3.1/3.5 (protocol-v0.md rev 46 §9.2,
  ...
  /**
   * vault-multi-writer (rev 47) task 3.7 — the writer chain state's read
   * surface, mirrored client-side from the gateway's `VaultRecord` (see
   * `services/vault/reads.ts` in run402-private for the authoritative doc
   ...
   */
  writer_set?: { version: string; sha256: string | null; writers: VaultWriterSetEntry[] };
  pending_writers?: VaultPendingWriter[];
  ineligible_members?: VaultIneligibleMember[];
  /** `vault_vaults.read_only_terminal_at IS NOT NULL` — the D228 forced sole-writer-removal terminal state: no further writer-authenticated push is admissible. Absent (never `undefined`-checked as `true`) on an older gateway — treat as `false`. */
  read_only_terminal?: boolean;
  /**
   * vault-multi-writer (rev 47) D6/D227, task 5.9 — decimal-string
   ...
   * vault-agent-envelopes D4: a cold open compares genesis's creator
...
/** One chain-recognized active writer on {@link VaultRecord.writer_set} — the on-chain shape only; `gateway_blocked_at` (gateway-only) rides {@link VaultIneligibleMember} instead. */
export interface VaultWriterSetEntry {
  writer_key_id: string;
  principal_id: string;
...
/** An org member eligible to become a writer (active membership at role developer+, a published possession-verified signing key) who is NOT yet in {@link VaultRecord.writer_set}'s `writers` — a candidate for the "writer"-door `add_writer_key` ({@link Vault.reconcileWriterAdmissions}). */
export interface VaultPendingWriter {
  principal_id: string;
  writer_key_id: string;
}
...
export interface VaultIneligibleMember {
  principal_id: string;
  writer_key_id: string;
...
/**
 * {@link VaultTransport.getState}'s result — the SAME bytes the per-object
...
export interface VaultState {
  ...
  /** Present only when the gateway answered a `since` request with a qualifying span (vault-delta-fetch); absent otherwise, including on every older gateway. */
  delta?: VaultStateDelta | null;
  /** Present only when the caller declared `restore: true` AND the gateway located a qualifying checkpoint boundary (vault-restore-recipe); absent otherwise, including on every older gateway. */
  restore_plan?: VaultRestorePlan | null;
}

/**
 * The state read's restore recipe (vault-restore-recipe design D1-D5):
 ...
 * Vault} re-derives the full chain-link/signature/cross-equality/
...
/**
 * The state read's bounded delta (vault-delta-fetch): the span's heads in
 ...
 */
export interface VaultStateDelta {
  heads: Array<{ generation: string; stored_bytes: Uint8Array; stored_bytes_sha256: string }>;
  packs: Array<{ object_id: string; bytes: Uint8Array }>;
}

export interface VaultHttpTransportOptions {
  /** Wire shape: every vault-scoped route is `/vaults/v1/:vault_id/...`; `vault_id` is the `repo_id` unless a mapping is supplied (D185). */
  vaultIdFor?: (repoId: string) => string;
  /**
   * vault-byo-primary-bucket: where THIS machine reads a BYO vault's
   ...
   * `VAULT_BYO_NOT_CONFIGURED` by name — never a silent "absent". Absent
   * option: same as `null`. Managed vaults never consult it.
   */
  byoBackend?: (repoId: string) => VaultMirrorBackend | null;
  /**
   * vault-object-host-predial (design D1, task 1.2): called with the
   ...
   * see `vault-prewarm.ts`). Fired ONLY after a completed fetch (any
   ...
   * expected to persist them via `VaultKeystore.recordObjectStoreOrigins`;
 ...
 * {@link VaultHttpTransportOptions.onObjectStoreOriginObserved} hands a
 ...
 */
function vaultObjectStoreOrigin(url: string): string | null {
  try {
...
function vaultObjectStoreOrigins(urls: Array<string | undefined>): string[] {
  const out: string[] = [];
  for (const u of urls) {
    if (!u) continue;
    const origin = vaultObjectStoreOrigin(u);
...
interface UploadSessionResponse {
  ...
  /**
   * vault-byo-primary-bucket task 3.2 — `put` is `null` for a BYO
   ...
   */
  objects: Array<VaultObjectReadRequest & { put: { url: string; headers?: Record<string, string> } | null; key?: string }>;
}
interface FinalizeResponse {
  receipts: Array<VaultObjectReadRequest & { stored_bytes_sha256?: string; ciphertext_sha256?: string; size_bytes: string; storage_verification?: "gateway" | "client_attested" }>;
}
/** vault-byo-primary-bucket task 3.2 (D219) — one client-submitted per-object attestation, echoed back from the session's OWN declared manifest values (never re-derived from the write). */
...
interface ObjectReadsResponse {
  /**
   * `edge_url` (vault-read-edge-cache design D5) names the SAME bytes as
   ...
   * `fetchVaultObjectBytes` (`vault-edge-fetch.ts`) for the
   * prefer-edge/silent-fallback policy; this type never implies a
   * verification difference between the two arms.
   *
   * `inline` (vault-small-object-inline design D1/D2) is the object's
   ...
   */
  reads: Array<VaultObjectReadRequest & { url?: string; edge_url?: string; inline?: string; byo_key?: string; stored_bytes_sha256: string; size_bytes: string }>;
}
/**
 * `GET …/state` wire shape (vault-composite-state-read design D1) —
 ...
 * `ObjectReadsResponse.reads[].edge_url` (vault-read-edge-cache design
...
interface VaultStateResponse {
  ...
  /** vault-delta-fetch: present only for a qualifying `since` span; packs are inline (`inline`) or presigned refs (ignored by this client version). */
  delta?: {
  ...
  /**
   * vault-restore-recipe: present only when the caller sent `restore=1`
...
/**
 * Independent reads/PUTs within one vault step run at this concurrency
 ...
 */
export const VAULT_TRANSPORT_CONCURRENCY = 6;
 ...
 */
export function createVaultHttpTransport(client: Client, options: VaultHttpTransportOptions = {}): VaultTransport {
  const vid = (repoId: string) => encodeURIComponent((options.vaultIdFor ?? ((r) => r))(repoId));
  const base = (repoId: string) => `/vaults/v1/${vid(repoId)}`;
  ...
  async function getGenerationBytes(repoId: string, route: "heads" | "admissions", generation: string): Promise<Uint8Array | null> {
    ...
    const auth = (await client.credentials.getAuth(path, { method: `vault.read_${route}` })) ?? {};
    ...
    if (!r.ok) {
      ...
      // `VAULT_ACCESS_DENIED`) rather than flattening every refusal into a
      // generic read failure — the codes carry the caller's next action.
      let code = "VAULT_OBJECT_READ_FAILED";
      ...
      fail(code, message, `reading vault ${route}`, { generation, status: r.status });
    }
    return new Uint8Array(await r.arrayBuffer());
  }

  /**
   * Resolve ONE `object-reads` presign target to bytes (vault-small-
   ...
   *
   * vault-object-host-predial (task 1.2): once the fetch below
   ...
   */
  /**
   * vault-byo-primary-bucket: a payload object of a BYO vault, named by
   ...
   * `VAULT_BYO_OBJECT_MISSING`); a machine with no local BYO config for
  ...
  async function resolveObjectReadTarget(repoId: string, target: ObjectReadsResponse["reads"][number], expectedSha256: string | undefined, path: string): Promise<Uint8Array | null> {
    ...
    if (target.url === undefined) fail("VAULT_OBJECT_READ_FAILED", `object-reads entry for ${path} carries neither a presign nor a byo_key`, "reading vault object", { path });
    const r = await fetchVaultObjectBytes(client, { url: target.url, ...(target.edge_url ? { edge_url: target.edge_url } : {}) });
    try {
      options.onObjectStoreOriginObserved?.(repoId, vaultObjectStoreOrigins([target.url, target.edge_url]));
    } catch {
      /* a caller's own persistence failure must never surface into this read */
    }
    if (r.status === 404) return null;
    if (!r.ok) fail("VAULT_OBJECT_READ_FAILED", `object GET failed (HTTP ${r.status}) for ${path}`, "reading vault object", { path, status: r.status });
    return new Uint8Array(await r.arrayBuffer());
  }
  ...
  async function getObjectBytes(repoId: string, path: string, expectedSha256?: string): Promise<Uint8Array | null> {
    const ref = vaultWireRefForPath(path);
    if (!ref) fail("VAULT_OBJECT_READ_FAILED", `${path} has no control-plane wire identity; it is not a readable vault object`, "reading vault object", { path });
    ...
    try {
      presigned = await client.request<ObjectReadsResponse>(`${base(repoId)}/object-reads`, { method: "POST", body: { objects: [ref.read] }, context: "resolving vault object" });
    } catch (e) {
      ...
      if (ref.read.object_kind === "recipient_pin_manifest" && isRun402Error(e) && (e as { code?: string }).code === "VALIDATION_FAILED" && /epoch must be 16 hex/.test((e as Error).message ?? "")) {
        throw new LocalError(
          ...
          "reading vault object",
          { code: "VAULT_PIN_MANIFEST_READ_UNSUPPORTED", details: { path, pin_manifest_version: ref.read.pin_manifest_version }, cause: e },
        );
      }
      throw e;
    }
  ...
  /**
   * Presign + fetch N independent objects (vault-client-round-trips
   * design D2): ONE `object-reads` POST naming every path, then the GETs
   * with bounded concurrency ({@link VAULT_TRANSPORT_CONCURRENCY}).
  ...
  async function getObjectsBytes(repoId: string, paths: string[], expected?: Array<string | undefined>): Promise<Array<Uint8Array | null>> {
    ...
    return mapBounded(targets, VAULT_TRANSPORT_CONCURRENCY, async (target, i) => {
      if (!target) return null;
  ...
  async function presignObjectBatch(repoId: string, paths: string[]): Promise<Array<ObjectReadsResponse["reads"][number] | null> | null> {
    const refs = paths.map((path) => {
      const ref = vaultWireRefForPath(path);
      if (!ref || ref.kind !== "object") fail("VAULT_OBJECT_READ_FAILED", `${path} is not a batch-readable carrier object`, "reading vault objects", { path });
      return ref.read;
    });
    let presigned: ObjectReadsResponse;
    try {
      presigned = await client.request<ObjectReadsResponse>(`${base(repoId)}/object-reads`, { method: "POST", body: { objects: refs }, context: "resolving vault objects" });
    } catch (e) {
      if (isRun402Error(e) && (e as { status?: number }).status === 404) return null;
      if (isRun402Error(e) && (e as { code?: string }).code === "RESOURCE_NOT_FOUND") return null;
      throw e;
    }
    const byLedgerId = new Map(presigned.reads.map((r) => [vaultLedgerId(r), r]));
    return refs.map((r) => byLedgerId.get(vaultLedgerId(r)) ?? null);
  }

  /**
   * Per-object settlement over the same batch (vault-pipelined-restore
   ...
   * VAULT_OBJECT_READ_FAILED), including `expected` (vault-small-
   * object-inline design D3 — see {@link VaultTransport.getObjects}'s
  ...
  async function getObjectsSettledBytes(repoId: string, paths: string[], expected?: Array<string | undefined>): Promise<Array<Promise<Uint8Array | null>>> {
    ...
    void Promise.all(Array.from({ length: Math.min(VAULT_TRANSPORT_CONCURRENCY, targets.length) }, () => worker()));
    return deferreds.map((d) => d.promise);
  }

  /**
   * `POST …/head-reads` — many generation-addressed heads' EXACT stored bytes
   * in ONE round trip (vault-batched-head-reads task 4.1).
   *
  ...
  async function getHeadsBytes(repoId: string, generations: string[]): Promise<Uint8Array[] | null> {
    ...
    try {
      const res = await client.request<{ heads?: Array<{ generation?: string; stored_bytes?: string }> }>(`${base(repoId)}/head-reads`, {
        method: "POST",
        body: { generations },
        context: "reading vault heads in a batch",
      });
  ...
  /** Resolve ONE `GET …/state` carrier arm to raw bytes — inline decode, or a plain GET on the presigned URL (preferring its `edge_url` companion, vault-read-edge-cache design D5), `null` on a 404 (mirrors {@link getObjectBytes}'s absent reading; both arms indistinguishable after this). Origin observation (vault-object-host-predial task 1.2) mirrors `resolveObjectReadTarget`'s. */
  async function resolveVaultStateCarrier(repoId: string, carrier: VaultStateCarrierWire): Promise<Uint8Array | null> {
    ...
    const r = await fetchVaultObjectBytes(client, { url: carrier.presigned_url, edge_url: carrier.edge_url });
    try {
      options.onObjectStoreOriginObserved?.(repoId, vaultObjectStoreOrigins([carrier.presigned_url, carrier.edge_url]));
    } catch {
      /* a caller's own persistence failure must never surface into this read */
    }
    if (r.status === 404) return null;
    if (!r.ok) fail("VAULT_OBJECT_READ_FAILED", `vault-state carrier GET failed (HTTP ${r.status})`, "reading the vault state", { status: r.status });
    return new Uint8Array(await r.arrayBuffer());
  }
   ...
   * own doc comment on {@link VaultTransport.getState}).
   */
  async function getVaultStateOut(repoId: string, since?: string, restore?: boolean): Promise<VaultState> {
    const qs: string[] = [];
    if (since) qs.push(`since=${encodeURIComponent(since)}`);
    if (restore) qs.push("restore=1");
    const raw = await client.request<VaultStateResponse>(`${base(repoId)}/state${qs.length > 0 ? `?${qs.join("&")}` : ""}`, { context: "reading the vault state" });
    ...
    // every failure mode (vault-delta-fetch D2/D4).
    let delta: VaultStateDelta | null = null;
    ...
    // Restore-plan decode (vault-restore-recipe design D1-D5): the SAME
    ...
    const decodeRestorePlan = async (): Promise<VaultRestorePlan | null> => {
      ...
      try {
        ...
        const resolved = await mapBounded(packEntries, VAULT_TRANSPORT_CONCURRENCY, async (pk) => {
          ...
          // receipt happens later, in `Vault`, before any byte is
  ...
  function receiptsFromFinalize(objects: VaultUploadObject[], entries: Array<VaultObjectReadRequest & { sha256: string; size_bytes: string; base_generation?: string }>, fin: FinalizeResponse): VaultUploadReceipt[] {
    const receipts = new Map(fin.receipts.map((r) => [vaultLedgerId(r), r]));
    return objects.map((o, i) => {
      const id = vaultLedgerId(entries[i]!);
      const r = receipts.get(id);
      if (!r) fail("VAULT_RECEIPT_MISSING", `finalize returned no receipt for ${id}`, "finalizing vault upload session", { object_id: id, path: o.path });
  ...
  async function upload(
    repoId: string,
    objects: VaultUploadObject[],
    resourceBinding?: VaultResourceBinding,
    byo?: { destination: VaultMirrorDestination; credential?: VaultMirrorCredential },
  ): Promise<VaultUploadReceipt[]> {
    ...
    const entries = objects.map((o) => vaultManifestEntry(o));
    // vault-byo-primary-bucket task 3.2: the inline (bytes-in-body) fast
    ...
    if (!byo && vaultInlineUploadEligible(objects)) {
      // vault-composite-state-read design D2: every object fits under the
      ...
      const fin = await client.request<FinalizeResponse>(`${base(repoId)}/upload-sessions`, {
        ...
        context: "uploading vault objects inline",
      });
    ...
    const session = await client.request<UploadSessionResponse>(`${base(repoId)}/upload-sessions`, {
      ...
      context: "opening vault upload session",
    });
    const issued = new Map(session.objects.map((u) => [vaultLedgerId(u), u]));
    // vault-byo-primary-bucket task 3.2 — one backend, opened ONCE, for
    ...
    // too — see the resolution in `Vault.uploadAll`).
    const byoBackend = byo ? openVaultDestinationBackend(byo.destination, byo.credential) : null;
    ...
    await mapBounded(objects, VAULT_TRANSPORT_CONCURRENCY, async (o, i) => {
      const id = vaultLedgerId(entries[i]!);
      const target = issued.get(id);
      if (!target) fail("VAULT_UPLOAD_SESSION_INVALID", `the session issued no upload for ${id}`, "uploading vault objects", { object_id: id, path: o.path });
      if (target.put === null) {
        // BYO payload object (task 2.2's VAULT_BYO_PAYLOAD_KINDS
        ...
        if (!byoBackend || !target.key) {
          fail("VAULT_BYO_BUCKET_WRITE_REFUSED", `the session named ${o.path} as a BYO-written object but no BYO write target was resolved`, "uploading vault objects", { path: o.path, object_id: id });
        }
        let put: { created: boolean };
        try {
          put = await byoBackend.putCreateOnly(target.key, o.bytes);
        } catch (e) {
          fail("VAULT_BYO_BUCKET_WRITE_REFUSED", `writing ${o.path} to the BYO destination failed: ${e instanceof Error ? e.message : String(e)}`, "uploading vault objects", { path: o.path, key: target.key });
        }
        if (!put.created) {
          ...
          if (!existing || sha256Hex(existing) !== o.sha256) {
            fail("VAULT_BYO_BUCKET_WRITE_REFUSED", `${o.path} already exists in the BYO destination with different bytes`, "uploading vault objects", { path: o.path, key: target.key });
          }
        }
      ...
      if (r.status === 412 || r.status === 409) {
        ...
        if (!existing || sha256Hex(existing) !== o.sha256) fail("VAULT_OBJECT_EXISTS_DIFFERENT", `${o.path} already exists with different bytes`, "uploading vault objects", { path: o.path });
        return;
      }
      if (!r.ok) fail("VAULT_UPLOAD_FAILED", `presigned PUT failed (HTTP ${r.status}) for ${o.path}`, "uploading vault objects", { path: o.path, status: r.status });
    });
    const fin = await client.request<FinalizeResponse>(`${base(repoId)}/upload-sessions/${encodeURIComponent(session.upload_session_id)}/finalize`, {
      ...
      context: "finalizing vault upload session",
    });
    return receiptsFromFinalize(objects, entries, fin);
  }

  async function admit(repoId: string, generation: string, bytes: Uint8Array, hash: string, extra: Record<string, unknown> = {}): Promise<VaultAdmitHeadResult> {
    try {
      const r = await client.request<{ outcome?: string; admission_record_sha256: string; capture_receipt?: VaultCaptureReceipt | null }>(`${base(repoId)}/admissions`, {
        method: "POST",
        body: { generation, stored_bytes: b64u(bytes), stored_bytes_sha256: hash, ...extra },
        context: "admitting vault head",
      });
  ...
  const transport: VaultTransport = {
    // ── creation (5.3) ──
    async allocate(request: VaultAllocateRequest): Promise<VaultAllocation> {
      ...
      const res = await client.request<{ allocation?: VaultAllocation } & VaultAllocation>("/vaults/v1", { method: "POST", body: request, context: "allocating vault" });
      return res.allocation ?? (res as VaultAllocation);
    },
    async putObject(request: VaultPutObjectRequest): Promise<VaultObjectReceipt> {
    ...
    async admitGenesis(request: VaultAdmitGenesisRequest): Promise<VaultAdmitGenesisResult> {
      try {
        const r = await admit(request.repo_id, VAULT_GENESIS_GENERATION, request.stored_bytes, request.stored_bytes_sha256, { allocation_generation: request.allocation_generation });
    ...
    getGenesis: ({ repo_id }) => getObjectBytes(repo_id, `head/${VAULT_GENESIS_GENERATION}`),
    // ── publication (5.4) ──
    async listHeads(request) {
      ...
      return client.request<VaultHeadsListingPage>(`${base(request.repo_id)}/heads?${qs.toString()}`, { context: "listing vault heads" });
    },
    ...
    // Both routes SHIPPED (`routes/vault-admission.ts`). `retention-cutoffs`
    // answers `{ticket, receipt, next_actions}`, which IS
    // `VaultRetentionCutoffIssued` plus routing sugar — no unwrap needed.
    requestRetentionCutoff: ({ repo_id, base_head_sha256 }) => client.request<VaultRetentionCutoffIssued>(`${base(repo_id)}/retention-cutoffs`, { method: "POST", body: { base_head_sha256 }, context: "requesting retention cutoff ticket" }),
    async exchangeActivationToken({ repo_id, operation_id, capture_receipt }): Promise<VaultActivationToken> {
      ...
      // (`VAULT_TOKEN_BINDING_MISMATCH`) and the deploy never commits — even
      // though the envelope's sibling `object_id` makes the shape look close
      // enough to be a plausible token at a glance. Pinned by
      // `vault-wire-shapes.test.ts`.
      const res = await client.request<{ activation_token?: VaultActivationToken } & VaultActivationToken>(
      ...
      return res.activation_token ?? (res as VaultActivationToken);
    },
    ...
    getVaultRecord: ({ repo_id }) => client.request<VaultRecord>(base(repo_id), { context: "reading the vault record" }),
    getState: ({ repo_id, since, restore }) => getVaultStateOut(repo_id, since, restore),
    openCompactionGrant: ({ repo_id }) => client.request<VaultCompactionGrant>(`${base(repo_id)}/compaction-grant`, { method: "POST", context: "opening the vault compaction headroom grant" }),
    closeCompactionGrant: async ({ repo_id }) => {
      const r = await client.request<{ closed?: boolean }>(`${base(repo_id)}/compaction-grant`, { method: "DELETE", context: "closing the vault compaction headroom grant" });
      return { closed: r.closed === true };
    },
    findVaultByProject: ({ project_id }) => client.request<VaultRecord>(`/vaults/v1?project_id=${encodeURIComponent(project_id)}`, { context: "resolving the project's vault" }),
    findVaultByRepo: ({ org_slug, repo_name }) => client.request<VaultRecord>(`/vaults/v1?repo=${encodeURIComponent(`${org_slug}/${repo_name}`)}`, { context: "resolving the vault by repo address" }),
    listOrgEncryptionKeys: ({ org_id }) => client.request<VaultOrgEncryptionKeyDirectory>(`/orgs/v1/${encodeURIComponent(org_id)}/encryption-keys`, { context: "reading the org encryption-key directory" }),
    listEnvelopeRecipients: ({ repo_id }) => client.request<VaultEnvelopeRecipientsResponse>(`${base(repo_id)}/envelope-recipients`, { context: "reading the vault envelope recipients" }),
    // ── epoch rotation (D193-D203, rev 42, §9.2) ──
    async createRotationAttempt({ repo_id, descriptor }) {
      const res = await client.request<{ rotation_id: string; descriptor: VaultRotationAttemptDescriptor; deduplicated?: boolean }>(`${base(repo_id)}/rotation-attempts`, {
        method: "POST",
        body: descriptor,
        context: "creating a vault rotation attempt",
      });
    ...
    confirmRecipient: ({ repo_id, principal_id, new_fingerprint }) =>
      client.request<VaultRecipientConfirmationReceipt>(`${base(repo_id)}/recipients/${encodeURIComponent(principal_id)}/confirm`, { method: "POST", body: { new_fingerprint }, context: "confirming a vault recipient's first pin" }),
    repinRecipient: ({ repo_id, principal_id, old_ek_fingerprint, new_fingerprint }) =>
      client.request<VaultRecipientConfirmationReceipt>(`${base(repo_id)}/recipients/${encodeURIComponent(principal_id)}/repin`, { method: "POST", body: { old_ek_fingerprint, new_fingerprint }, context: "re-pinning a vault recipient" }),
    declareRecipientKeyRevoked: ({ repo_id, principal_id }) =>
      client.request<{ recipient_state_version: string; recipient_revocation_version: string }>(`${base(repo_id)}/recipients/${encodeURIComponent(principal_id)}/key-revocation`, { method: "POST", body: {}, context: "declaring a vault recipient key revoked" }),
    declareEpochSecretExposed: ({ repo_id }) =>
      client.request<{ epoch_secret_exposure_version: string }>(`${base(repo_id)}/epoch-secret-exposure`, { method: "POST", body: {}, context: "declaring a vault epoch secret exposed" }),
    declareWriterAuthorityUnavailable: ({ repo_id }) =>
      client.request<{ declared_at: string; declared_by: string | null }>(`${base(repo_id)}/writer-authority/declare-unavailable`, { method: "POST", body: {}, context: "declaring vault writer authority unavailable" }),
    async submitOpenProof({ repo_id, principal_id, ek_fingerprint, chain_verified_to_generation, decryptable_to_generation, reader_entrypoint }) {
      ...
      // BODY is identical either way (routes/repos.ts:
      // `res.status(inserted ? 201 : 200).json(receipt)`).
      const res = await client.requestWithResponse<VaultOpenReceipt>(
        `${base(repo_id)}/recipients/${encodeURIComponent(principal_id)}/proof-of-open`,
        { method: "POST", body: { ek_fingerprint, chain_verified_to_generation, decryptable_to_generation, reader_entrypoint }, context: "submitting a vault proof-of-open receipt" },
      );
    ...
    acquireMaintenanceLease: ({ repo_id, base_head_sha256, current_checkpoint_hash, r1_size_bytes, r2_cap_size_bytes, p_before_c1_size_bytes, p_before_c2_size_bytes }) =>
      client.request<VaultMaintenanceLease>(`${base(repo_id)}/maintenance-leases`, {
        method: "POST",
        body: {
          base_head_sha256,
        ...
        context: "acquiring the vault maintenance lease",
      }),
    heartbeatMaintenanceLease: ({ repo_id, maintenance_lease_id, holder_token }) =>
      client.request<{ maintenance_lease_id: string; expires_at: string | null }>(`${base(repo_id)}/maintenance-leases/${encodeURIComponent(maintenance_lease_id)}/heartbeat`, { method: "POST", body: { holder_token }, context: "renewing the vault maintenance lease" }),
    releaseMaintenanceLease: ({ repo_id, maintenance_lease_id, holder_token }) =>
      client.request<{ maintenance_lease_id: string; status: string }>(`${base(repo_id)}/maintenance-leases/${encodeURIComponent(maintenance_lease_id)}`, { method: "DELETE", body: { holder_token }, context: "releasing the vault maintenance lease" }),
    // ── prune (§7.3) ──
    async submitPruneIntent({ repo_id, intent_bytes }) {
      ...
      const auth = (await client.credentials.getAuth(path, { method: "repos.prune" })) ?? {};
      ...
      if (!r.ok) {
        ...
        // `VAULT_PRUNE_SUBMIT_FAILED {details: null}`. Read the real
        ...
        fail(
          envelope?.code ?? "VAULT_PRUNE_SUBMIT_FAILED",
          envelope?.message ?? (typeof envelope?.error === "string" ? envelope.error : undefined) ?? `prune intent submission failed (HTTP ${r.status})`,
          "submitting the vault prune intent",
          details,
          Array.isArray(envelope?.next_actions) ? envelope.next_actions : undefined,
        );
      }
      const body = (parsed ?? {}) as VaultPruneIntentRecord & { stored?: boolean };
      return { ...body, stored: body.stored === true };
    },
    async getPruneIntent({ repo_id, prune_intent_object_id }) {
      try {
        return await client.request<VaultPruneIntentRecord>(`${base(repo_id)}/prune-intents/${encodeURIComponent(prune_intent_object_id)}`, { context: "reading the vault prune intent" });
      } catch (e) {
  ...
  return process.env.RUN402_VAULT_TRACE === "1" ? traceVaultTransport(transport) : transport;
}

/**
 * `RUN402_VAULT_TRACE=1` (design D7) — one stderr line per real
 ...
 * `VaultOpCounter`, a test-only instrument over the SAME operation
 * shapes). The client-side env var carries no gateway env-registry policy.
 */
function traceVaultTransport(inner: VaultTransport): VaultTransport {
  let opCount = 0;
  let totalMs = 0;
  const sessionStart = Date.now();
  process.on("exit", () => {
    if (opCount === 0) return;
    process.stderr.write(`vault-trace: session summary — ${opCount} op(s), ${totalMs.toFixed(1)}ms in transport, ${Date.now() - sessionStart}ms wall-clock\n`);
  });
    ...
    get(target, prop, receiver) {
      ...
      return async (...args: unknown[]) => {
        ...
        try {
          ...
          opCount += 1;
          totalMs += elapsed;
          process.stderr.write(`vault-trace: ${prop}${describeRequest(args[0])}${describeResult(result)} ${elapsed}ms\n`);
          return result;
        } catch (e) {
          const elapsed = Date.now() - start;
          opCount += 1;
          totalMs += elapsed;
          process.stderr.write(`vault-trace: ${prop}${describeRequest(args[0])} FAILED ${elapsed}ms\n`);
          throw e;
        }
      };
    },
...
export const vaultPaths = {
   ...
   * `vault-creation-journal.ts`'s private `envelopePath` (the genesis
...
// ─── Incremental restore marker (vault-client-round-trips design D5) ──────
//
...
const VAULT_RESTORE_MARKER_GENERATION_KEY = "r402.restoredThrough";
const VAULT_RESTORE_MARKER_SHA256_KEY = "r402.restoredThroughSha256";

export interface VaultRestoreMarker {
  generation: string;
  head_sha256: string;
}
...
export async function readVaultRestoreMarker(targetRepoDir: string): Promise<VaultRestoreMarker | null> {
  const generation = await readLocalGitConfigValue(targetRepoDir, VAULT_RESTORE_MARKER_GENERATION_KEY);
  const head_sha256 = await readLocalGitConfigValue(targetRepoDir, VAULT_RESTORE_MARKER_SHA256_KEY);
...
async function writeVaultRestoreMarker(targetRepoDir: string, generation: string, headSha256: string): Promise<void> {
  await hardenedGit(targetRepoDir, ["config", "--local", VAULT_RESTORE_MARKER_GENERATION_KEY, generation]);
  await hardenedGit(targetRepoDir, ["config", "--local", VAULT_RESTORE_MARKER_SHA256_KEY, headSha256]);
}

// ─── auto-gc cadence threshold (vault-checkpoint-cadence design D1) ───────
//
...
const VAULT_AUTO_GC_GENERATIONS_KEY = "r402.autoGcGenerations";
...
export const VAULT_AUTO_GC_GENERATIONS_DEFAULT = 32;
 ...
 */
export async function readVaultAutoGcThreshold(targetRepoDir: string): Promise<number> {
  const raw = await readLocalGitConfigValue(targetRepoDir, VAULT_AUTO_GC_GENERATIONS_KEY);
  if (raw === null) return VAULT_AUTO_GC_GENERATIONS_DEFAULT;
  const trimmed = raw.trim();
  if (!/^\d+$/.test(trimmed)) return VAULT_AUTO_GC_GENERATIONS_DEFAULT;
  const n = Number.parseInt(trimmed, 10);
  return Number.isSafeInteger(n) && n >= 0 ? n : VAULT_AUTO_GC_GENERATIONS_DEFAULT;
}

/** Set this checkout's auto-gc threshold. `0` disables auto-gc entirely. */
export async function writeVaultAutoGcThreshold(targetRepoDir: string, generations: number): Promise<void> {
  await hardenedGit(targetRepoDir, ["config", "--local", VAULT_AUTO_GC_GENERATIONS_KEY, String(generations)]);
}

// ─── The vault ───────────────────────────────────────────────────────────────

/**
 * {@link Vault.ensureRepoState}'s report when a cold keystore was
 * restored from its own envelope (vault-agent-envelopes D4). `trust` is
 ...
 */
export interface VaultColdOpenResult {
  repo_id: string;
  org_id: string;
...
export interface VaultCutoffOptions {
  /**
...
/** One admitted `rotate_epoch` transition {@link Vault.verifyToNewest} walked over, KEYLESS (structural only — no envelope opened yet). */
export interface VaultEncounteredRotation {
  /** The rotate_epoch head's own generation. */
  generation: string;
  /** == `payload.new_epoch` == the head's own `epoch`. */
  epoch: string;
  payload: VaultRotateEpochPayload;
  /**
   * vault-multi-writer rev 47 — the writer who actually signed THIS
...
/** Named detail for a reader's own `VAULT_EPOCH_NOT_OPENABLE` / decrypt stop point (Part C: `repos fsck`'s honest `chain_verified_to` vs `decryptable_to` split). */
export interface VaultEpochDecryptFailure {
  generation: string;
  epoch: string;
 ...
 */
export interface VaultDecryptValidationResult {
  decryptable_to_generation: string;
  ...
  ref_state: VaultRefState | null;
  retention_roots: VaultRetentionRoots | null;
  ...
  failure: VaultEpochDecryptFailure | null;
}

export interface VaultVerifiedState {
  generation: string;
  head_sha256: string;
  ...
  rotations: VaultEncounteredRotation[];
  /** Present iff `decryptValidate` was requested. */
  decrypt: VaultDecryptValidationResult | null;
}

export interface VaultMaterializedState extends VaultVerifiedState {
  ref_state: VaultRefState | null;
  retention_roots: VaultRetentionRoots | null;
  refs: VaultRefMap;
  roots: VaultRetentionRoot[];
...
/** One directory entry {@link Vault.reconcileEnvelopeRecipients} wrapped a fresh `key_envelope` for. */
export interface VaultReconcileEnvelopeRecipientsWrapped {
  principal_id: string;
  ek_fingerprint: string;
}

/** Why {@link Vault.reconcileEnvelopeRecipients} did NOT wrap a directory entry it otherwise would have. */
export type VaultReconcileEnvelopeRecipientsSkipReason = "missing_public_key" | "invalid_public_key" | "pinned_key_mismatch";

export interface VaultReconcileEnvelopeRecipientsSkipped {
  principal_id: string;
  ek_fingerprint: string;
  reason: VaultReconcileEnvelopeRecipientsSkipReason;
...
/** {@link Vault.reconcileEnvelopeRecipients}'s full per-recipient breakdown. */
export interface VaultReconcileEnvelopeRecipientsResult {
  repo_id: string;
  org_id: string;
  /** The epoch every wrap in this call used (V0: always {@link VAULT_GENESIS_EPOCH}). */
  epoch: string;
  /** Directory entries this call itself wrapped a NEW `key_envelope` for. */
  wrapped: VaultReconcileEnvelopeRecipientsWrapped[];
  ...
  skipped: VaultReconcileEnvelopeRecipientsSkipped[];
}

/** One `pending_writers[]` entry {@link Vault.reconcileWriterAdmissions} admitted via a fresh `add_writer_key{"writer"}` head. */
export interface VaultReconcileWriterAdmissionsAdmitted {
  principal_id: string;
  writer_key_id: string;
  generation: string;
}

/** Why {@link Vault.reconcileWriterAdmissions} did NOT admit a `pending_writers[]` entry it otherwise would have. */
export type VaultReconcileWriterAdmissionsSkipReason = "missing_signing_pubkey" | "invalid_signing_pubkey";

export interface VaultReconcileWriterAdmissionsSkipped {
  principal_id: string;
  writer_key_id: string;
  reason: VaultReconcileWriterAdmissionsSkipReason;
}

/**
 * {@link Vault.reconcileWriterAdmissions}'s full per-candidate
 * breakdown (task 5.7). Structurally parallel to
 * {@link VaultReconcileEnvelopeRecipientsResult} (same repo/wrapped-or-
 ...
 * writer set.
 */
export interface VaultReconcileWriterAdmissionsResult {
  repo_id: string;
  org_id: string;
  /**
  ...
  admitted: VaultReconcileWriterAdmissionsAdmitted[];
  ...
  skipped: VaultReconcileWriterAdmissionsSkipped[];
}

export interface VaultPushOptions {
  transaction: VaultRefTransaction;
  ...
  cutoff?: VaultCutoffOptions;
  /** Built lazily at head-sign time — the deploy lane may still be computing the plan digest. */
  capture_binding?: VaultCaptureBinding | (() => Promise<VaultCaptureBinding | null> | VaultCaptureBinding | null);
  /**
   * A caller-supplied base (vault-client-round-trips design D1) — used
   * VERBATIM for the first attempt instead of calling {@link Vault.materialize}
...
export interface VaultPublishResult {
  ...
  capture_receipt: VaultCaptureReceipt | null;
  /** `wal` = direct WAL receipts; `checkpoint` = the delta shipped as a checkpoint set. */
  form: "wal" | "checkpoint";
  conflicts_retried: number;
  refs: VaultRefMap;
  /**
   * vault-clone-scaling (P3): generations-since-checkpoint against the
   ...
   * never gate.
   */
  checkpoint_staleness: VaultCheckpointStaleness;
}

/** {@link Vault.rotateEpoch}'s result (D193-D203, rev 42). */
export interface VaultRotationResult {
   ...
   * new-epoch envelope opened (via {@link import("../namespaces/vault.crypto.js").openEpochRotationForRecipient},
   ...
   * recipient (design D1 of `services/vault/desired-recipients.ts`:
  ...
  /**
   * vault-multi-writer (task 5.9, D6/D227) — writer keys this SAME head
...
/**
 * What {@link Vault.planPush} reports — a REAL
 ...
 */
export interface VaultPushPlan {
  ...
  objects: Array<{ object_kind: VaultUploadObject["object_kind"]; size_bytes: string }>;
  object_count: number;
...
/**
 * What {@link Vault.verifyStoredCheckpoint} observed. Every boolean is
 * a FINDING, not a promise: a `false` here is what makes a truthful negative
 * `verifier_receipt` possible.
 */
export interface VaultStoredCheckpointAttestation {
...
export interface VaultBuiltCheckpoint {
  manifest: VaultCheckpointManifest;
  claim_set: VaultCheckpointClaimSet;
  claim_set_receipt: VaultCheckpointBlock["claim_set"];
  objects: VaultUploadObject[];
...
export class Vault {
  readonly keystore: VaultKeystore;
  ...
  /**
   * vault-clone-scaling (bench P2): the CURRENT walk window's prefetched
  ...
  /**
   * WAL pack bytes carried by the state read's delta (vault-delta-fetch),
  ...
  /**
   * The state read's restore plan (vault-restore-recipe design D1-D5),
   ...
   */
  private stateRestorePlan: VaultRestorePlan | null = null;
  private readonly retries: number;
  private readonly servicePublicKey: Uint8Array | string | null;
  private readonly allowUnauthenticatedSalvage: boolean;
  private genesisCache: { genesis: VaultGenesis; sha256: string } | null = null;
  /**
   * vault-byo-primary-bucket task 3.2 — this vault's resolved BYO write
   ...
   */
  private byoResolution: { destination: import("./vault-mirror-config.js").VaultMirrorDestination; credential?: import("./vault-mirror-config.js").VaultMirrorCredential } | null | undefined = undefined;
  ...
  constructor(options: VaultOptions) {
    ...
    this.budget = options.verification_budget ?? VAULT_VERIFICATION_BUDGET_HEADS;
    this.retries = options.conflict_retries ?? VAULT_PUSH_CONFLICT_RETRIES;
  ...
  static open(options: VaultOptions): Vault {
    const v = new Vault(options);
    v.repoFile(); // KEYSTORE_MISSING / VAULT_REPO_STATE_MISSING surface here
    return v;
  }

  repoFile(): VaultRepoFile {
    ...
    if (!this.keystore.readIdentity()) {
      fail("KEYSTORE_MISSING", "no vault identity in the keystore", "opening vault", undefined, [
        { action: "restore ~/.config/run402/vault from backup or accept vault loss" },
        ...crossProfileVaultHint(this.repoId),
      ]);
    }
    const repo = this.keystore.readRepo(this.repoId);
    if (!repo) {
      const hint = crossProfileVaultHint(this.repoId);
      fail(
        "VAULT_REPO_STATE_MISSING",
  ...
  /**
   * vault-agent-envelopes D4 — the COLD OPEN. A keystore that holds an
   ...
   * its OWN `key_envelope` instead of dying `VAULT_REPO_STATE_MISSING`:
   *
   ...
   *      covered — if not, `VAULT_ENVELOPE_PENDING` names the key-holders
   ...
   */
  /**
   * vault-multi-writer (rev 47) — the vault's CURRENT admitted writer set
   ...
   */
  private async resolveAdmittedWritersForColdOpen(genesis: VaultGenesis, genesisBytes: Uint8Array): Promise<WriterChainState> {
    ...
    let expectedGeneration = nextGeneration(VAULT_GENESIS_GENERATION);
    let progress: VaultListingProgress = { after_generation: VAULT_GENESIS_GENERATION, last_generation: VAULT_GENESIS_GENERATION, delivered: 0 };
    let request: VaultHeadsListingRequest = { after_generation: VAULT_GENESIS_GENERATION, limit: String(VAULT_MAX_HEADS_PER_LISTING_PAGE) };
    for (;;) {
      ...
      for (const entry of page.heads) {
        ...
        const head = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
        const isWriterAdd = head.transition !== null && head.transition.kind === "add_writer_key";
        let addWriterKeyPayload: VaultAddWriterKeyPayload | null = null;
        ...
        if (isWriterAdd) {
          ...
          const signer = resolveActiveWriter(writerState, head.writer_key_id);
          if (!signer) fail("VAULT_WRITER_NOT_ADMITTED", `head ${head.generation}: writer_key_id ${head.writer_key_id} is not a currently admitted writer`, "resolving the vault's writer set for a cold open", { generation: head.generation });
          signerPubkey = signer!.signing_pubkey;
          signerKeyId = signer!.writer_key_id;
        }
  ...
  async ensureRepoState(): Promise<VaultColdOpenResult | null> {
    ...
    if (!ownKeypair) {
      fail("VAULT_UNRECOVERABLE", VAULT_TERMINAL_LOSS_DOCTOR_TEXT, "opening vault", { statement: VAULT_TERMINAL_LOSS_STATEMENT, repo_id: this.repoId });
    }
    ...
    if (!genesisBytes) fail("CHAIN_BROKEN", "the vault has no admitted genesis", "opening vault", { repo_id: this.repoId });
    const genesis = parseVaultStrict(new TextDecoder().decode(genesisBytes)) as VaultGenesis;
    if (genesis.epoch !== VAULT_GENESIS_EPOCH) {
      fail("VAULT_CREATION_CONFLICT", `the served genesis declares epoch ${genesis.epoch}, not the genesis epoch`, "opening vault", { repo_id: this.repoId, epoch: genesis.epoch });
    }
    if (genesis.repo_id !== this.repoId) {
      fail("VAULT_CREATION_CONFLICT", `the served genesis names repo ${genesis.repo_id}, not ${this.repoId}`, "opening vault", { repo_id: this.repoId, served: genesis.repo_id });
    }
    ...
    if (!coverage.recipient_fingerprints.includes(ownFingerprint)) {
      ...
      fail(
        "VAULT_ENVELOPE_PENDING",
        `this keystore (fingerprint ${ownFingerprint}) is not yet a recipient on vault ${this.repoId} — no key_envelope has been wrapped for it; a key-holder's next vault operation fulfils the org's desired state`,
        "opening vault",
        {
          repo_id: this.repoId,
        ...
        [
          ...
          { action: "ask a key-holder to run any vault operation (run402 repos view / git push)", why: "a key-holding client wraps every pending desired recipient on its next operation; nothing else is required of them" },
        ],
      );
    }

    const envelopeBytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.envelope(genesis.epoch, ownFingerprint) });
    if (!envelopeBytes) {
      fail(
        "VAULT_EPOCH_NOT_OPENABLE",
    ...
    const envelope = parseVaultStrict(new TextDecoder().decode(envelopeBytes)) as VaultKeyEnvelope;
  ...
  static async openOrRestore(options: VaultOptions): Promise<{ vault: Vault; restored: VaultColdOpenResult | null }> {
    const v = new Vault(options);
  ...
  /**
   * vault-clone-scaling (P3): staleness of the newest checkpoint coverage
   ...
   */
  private checkpointStalenessNow(newestGeneration: string): VaultCheckpointStaleness {
    return vaultCheckpointStaleness({ newest_generation: newestGeneration, covers_through_generation: this.repoFile().checkpoint_covers_through ?? null });
  }
   ...
   * keyed by head path.
   *
   * BATCH-FIRST (vault-batched-head-reads task 4.2): one
   ...
   * own thing.
   *
   * FALLBACK (the shipped vault-clone-scaling P2 shape): on ANY
   ...
   * just early and overlapped at {@link VAULT_TRANSPORT_CONCURRENCY}. The
  ...
  private async prefetchHeadsConcurrent(generations: string[]): Promise<Map<string, Uint8Array>> {
    ...
    if (batched && batched.length === ascending.length) {
      for (let i = 0; i < ascending.length; i++) {
        const b = batched[i];
        if (b) map.set(vaultPaths.head(ascending[i]!), b);
      }
      return map;
    }

    const paths = generations.map((g) => vaultPaths.head(g));
    const fetched = await mapBounded(paths, VAULT_TRANSPORT_CONCURRENCY, (path) => this.transport.getObject({ repo_id: this.repoId, path }).catch(() => null));
  ...
  private git(): string {
    if (!this.repoDir) fail("VAULT_REPO_DIR_REQUIRED", "this operation needs the local git repository (repo_dir)", "vault publication");
    return this.repoDir;
  }

  /** Fetch + pin-check the genesis (the writer key source). */
  async genesis(): Promise<{ genesis: VaultGenesis; sha256: string }> {
    ...
    if (!bytes) fail("CHAIN_BROKEN", "the vault has no admitted genesis", "reading vault genesis", { repo_id: this.repoId });
    const sha256 = sha256Hex(bytes);
    if (sha256 !== repo.genesis_sha256) fail("VAULT_CREATION_CONFLICT", `the admitted genesis (${sha256}) is not the pinned one (${repo.genesis_sha256}); refusing a substituted vault`, "reading vault genesis", { admitted: sha256, pinned: repo.genesis_sha256 });
    const genesis = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultGenesis;
  ...
  /**
   * vault-composite-state-read design D1 — the pin-current fast path
   ...
   * established cache discipline; see `VaultKeystore`'s class doc
   ...
   */
  private async tryStateFastPath(pin: VaultHeadPin, deltaSince?: string, restore?: boolean): Promise<{ entries: VaultHeadsListingEntry[]; pinnedHead: VaultHead | null; prevEpoch: string } | null> {
    // vault-delta-fetch: carry the caller's MATERIALIZED position as
    ...
    // bench runs.
    //
    // `restore` (vault-restore-recipe design D2/D6) rides the SAME state
    ...
    checkGenerationRegression(state.newest_generation ?? VAULT_GENESIS_GENERATION, pin.generation);
    ...
    // Vault.genesis} owns verifying it via its OWN cache), so this
    // path must never parse `state.head` as a {@link VaultHead} when it
    // is actually genesis's bytes.
    if (state.delta) this.consumeStateDelta(state.delta);
    // vault-restore-recipe: stashed RAW (never verified here — see the
    ...
    const noOrdinaryHeadYet = state.newest_generation === null || state.newest_generation === VAULT_GENESIS_GENERATION;
    ...
    if (diff === 0n) {
      if (noOrdinaryHeadYet) return { entries: [], pinnedHead: null, prevEpoch: VAULT_GENESIS_EPOCH }; // matches today's `lastHead = null` for a genesis-only vault
      if (!state.head) fail("CHAIN_BROKEN", "the vault state reports an admitted generation but carries no head bytes", "verifying vault chain", { generation: state.newest_generation });
      const sha = sha256Hex(state.head.stored_bytes);
      if (sha !== pin.head_sha256) fail("CHAIN_BROKEN", `pinned head ${pin.generation} no longer hashes to the pin`, "reading pinned vault head", { generation: pin.generation });
      this.keystore.writeCachedHead(this.repoId, pin.generation, sha, state.head.stored_bytes);
      const head = parseVaultStrict(new TextDecoder().decode(state.head.stored_bytes)) as VaultHead;
    ...
    if (diff === 1n) {
      ...
      if (pin.generation === VAULT_GENESIS_GENERATION) {
        prevEpoch = VAULT_GENESIS_EPOCH;
      } else {
        ...
        prevEpoch = (parseVaultStrict(new TextDecoder().decode(cached.bytes)) as VaultHead).epoch;
      }
      if (!state.head) fail("CHAIN_BROKEN", "the vault state reports a newer generation but carries no head bytes", "verifying vault chain", { generation: state.newest_generation });
  ...
  /**
   * Absorb a state read's delta (vault-delta-fetch): heads blind-warm the
   ...
   */
  private consumeStateDelta(delta: VaultStateDelta): void {
   ...
   */
  private warmStateCarrierCache(generation: string, head: VaultHead, carriers: { ref_state: Uint8Array | null; retention_roots: Uint8Array | null }): void {
   ...
   * first `VAULT_EPOCH_NOT_OPENABLE` / AEAD failure it hits — the ordinary
   ...
   */
  async verifyToNewest(options: { persist?: boolean; decryptValidate?: boolean; strict?: boolean; deltaSince?: string; restore?: boolean } = {}): Promise<VaultVerifiedState> {
    const persist = options.persist ?? true;
    const decryptValidate = options.decryptValidate ?? false;
    const strict = options.strict ?? true;
    const { genesis, sha256: genesisSha } = await this.genesis();
    // vault-multi-writer rev 47: `writerKey`/`writerKeyId` (kept for the
    ...
    const repo = this.repoFile();
    let pin: VaultHeadPin = repo.verified_prefix ?? repo.head_pin ?? { generation: VAULT_GENESIS_GENERATION, head_sha256: genesisSha, pinned_at: formatVaultTimestamp(this.now()) };
    // vault-multi-writer rev 47 (task 5.1): the writer set as chain state.
    ...
    // (see `VaultRepoFile.writer_set_pin`'s own doc comment), so a repeat
    ...
    if (legacyWriterPin) pin = { generation: VAULT_GENESIS_GENERATION, head_sha256: genesisSha, pinned_at: formatVaultTimestamp(this.now()) };
    ...
    const persistWriterSetPin = (): void => {
      if (!persist) return;
      this.keystore.updateRepo(this.repoId, { writer_set_pin: { version: writerState.version, sha256: writerState.sha256, writers: [...writerState.writers], retired_writers: [...(writerState.retiredWriters ?? [])], pinned_at: formatVaultTimestamp(this.now()) } });
    };
    // vault-composite-state-read design D1: try the pin-current fast path
    ...
    // `options.restore` (vault-restore-recipe) rides along unchanged —
    ...
    let lastHead: VaultHead | null = fastPath ? fastPath.pinnedHead : pin.generation === VAULT_GENESIS_GENERATION ? null : await this.readHead(pin.generation, pin.head_sha256);
    const anchor = pin.generation;
    let prevEpoch = fastPath ? fastPath.prevEpoch : (lastHead?.epoch ?? VAULT_GENESIS_EPOCH);
    let progress: VaultListingProgress = { after_generation: anchor, last_generation: anchor, delivered: 0 };
    let request: VaultHeadsListingRequest = { after_generation: anchor, limit: String(VAULT_MAX_HEADS_PER_LISTING_PAGE) };
    let verified = 0;
    const rotations: VaultEncounteredRotation[] = [];
    // vault-clone-scaling (P3): coverage this walk LEARNS. A checkpoint
    ...
    const walkedFromGenesis = anchor === VAULT_GENESIS_GENERATION;
    ...
    let syntheticEntries: VaultHeadsListingEntry[] | null = fastPath ? fastPath.entries : null;
    ...
    let decryptPin: VaultHeadPin | null = repo.materialized_pin ?? null;
    let decryptedRefState: VaultRefState | null = null;
    let decryptedRoots: VaultRetentionRoots | null = null;
    let decryptFailure: VaultEpochDecryptFailure | null = null;
     ...
     */
    const tryDecrypt = async (head: VaultHead, headPin: VaultHeadPin, pendingRotations: readonly VaultEncounteredRotation[]): Promise<boolean> => {
      ...
      try {
        for (const rot of pendingRotations) {
          ...
          if (!identity || !ownKeypair) {
            fail(
              "VAULT_EPOCH_NOT_OPENABLE",
              `no local vault identity — cannot open epoch ${rot.epoch} (rotation ${rot.payload.rotation_id})`,
          ...
          const kE = await openEpochRotationForRecipient({
            ...
            // vault-multi-writer rev 47: the writer who actually drove
            // THIS rotation (resolved by the forward walk when it collected
            // `rot` — see `VaultEncounteredRotation.signing_pubkey`'s own
            ...
            envelope_path: (epoch, fp, rotationId) => vaultPaths.envelope(epoch, fp, rotationId),
          });
        ...
        if (!kRepoHex) {
          fail("VAULT_EPOCH_NOT_OPENABLE", `no locally known key for epoch ${head.epoch} at generation ${head.generation}`, "materializing vault head", { epoch: head.epoch, generation: head.generation });
        }
        ...
        // vault-multi-writer rev 47: `head`'s OWN signer, resolved from
        ...
        const { refState, roots } = await this.openMaterializeCarriers(head.ref_state, vaultPaths.refState(head.ref_state.object_id), head.retention_roots, vaultPaths.retentionRoots(head.retention_roots.object_id), headSigner, { epoch: head.epoch, k_repo: kRepo });
        if (refState.generation !== head.generation || roots.generation !== head.generation) fail("CHAIN_UNUSABLE", "carrier generation does not match the head", "materializing vault head");
        decryptedRefState = refState;
        decryptedRoots = roots;
        decryptPin = { ...headPin };
        return true;
      } catch (e) {
        const code = isRun402Error(e) && e.code ? e.code : "VAULT_EPOCH_NOT_OPENABLE";
    ...
    for (;;) {
      ...
      const page: VaultHeadsListingPage =
        syntheticEntries !== null
          ? { format: VAULT_FORMAT, repo_id: this.repoId, after_generation: request.after_generation, heads: syntheticEntries, has_more: false, next_cursor: null, total: null }
          : await this.transport.listHeads({ repo_id: this.repoId, ...request });
      syntheticEntries = null;
      progress = verifyHeadsListingPage(page, request, progress, this.repoId);
      // vault-clone-scaling (bench P2): the page just verified names every
      ...
      // (one direct read costs the same).
      {
        ...
        try {
          ...
          const cachedHeadIfMatching = (e: VaultHeadsListingEntry): Uint8Array | null => {
          ...
          if (missingHeads.length > 1) {
            ...
            for (const e of missingHeads) {
              const path = vaultPaths.head(e.generation);
          ...
          if (decryptValidate && wanted.length > 1) {
            ...
            for (const e of wanted) {
              const headBytes = prefetch.get(vaultPaths.head(e.generation)) ?? cachedHeadIfMatching(e);
              if (!headBytes) continue;
              let parsed: VaultHead;
              try {
                parsed = parseVaultStrict(new TextDecoder().decode(headBytes)) as VaultHead;
              } catch {
                continue; // the ordered loop's checkChainLink owns rejecting it
              }
              for (const w of [
                { receipt: parsed.ref_state, path: vaultPaths.refState(parsed.ref_state.object_id) },
                { receipt: parsed.retention_roots, path: vaultPaths.retentionRoots(parsed.retention_roots.object_id) },
              ]) {
      ...
      for (const entry of page.heads) {
        ...
        const bytes = await this.readCachedHeadBytes(entry.generation, entry.stored_bytes_sha256);
        if (!bytes) fail("CHAIN_BROKEN", `listed head ${entry.generation} is absent from storage`, "verifying vault chain", { generation: entry.generation });
        const head = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;

        // vault-multi-writer rev 47 (task 5.1): resolve WHO must have
        ...
        const failClosedTransition = (code: string, detail: string): never => {
          ...
          persistWriterSetPin();
          persistMaterializedIfAny();
          fail(code, `head ${head.generation}: ${detail}`, "verifying vault chain", { generation: head.generation });
        };
        const isWriterAdd = head.transition !== null && head.transition.kind === "add_writer_key";
        let addWriterKeyPayload: VaultAddWriterKeyPayload | null = null;
        ...
        if (isWriterAdd) {
          ...
          const signer = resolveActiveWriter(writerState, head.writer_key_id);
          if (!signer) failClosedTransition("VAULT_WRITER_NOT_ADMITTED", `writer_key_id ${head.writer_key_id} is not a currently admitted writer`);
          resolvedWriterPubkey = signer!.signing_pubkey;
          resolvedWriterKeyId = signer!.writer_key_id;
        }
        ...
        let rotationPayload: VaultRotateEpochPayload | null = null;
        if (isRotation) {
          ...
          // vault-multi-writer rev 47 (D227): writer REMOVAL rides
        ...
        prevEpoch = head.epoch;
        pin = { generation: head.generation, head_sha256: entry.stored_bytes_sha256, pinned_at: formatVaultTimestamp(this.now()) };
        lastHead = head;
    ...
    if (persist) {
      const learned = walkCheckpointCoverage ?? (walkedFromGenesis ? VAULT_GENESIS_GENERATION : null);
    ...
    if (decryptValidate && !decryptFailure && lastHead && !decryptedRefState) {
      const backwardRotations: VaultEncounteredRotation[] = [];
      let cur: VaultHead | null = lastHead;
      let curPin: VaultHeadPin = pin;
      while (cur && !epochKeys[cur.epoch]) {
        if (cur.transition !== null && cur.transition.kind === "rotate_epoch") {
          ...
          // vault-multi-writer rev 47: best-effort resolution against the
        ...
        if (!bytes || sha256Hex(bytes) !== cur.prev_sha256) fail("CHAIN_BROKEN", `head ${prevGen} does not match the chain during epoch-key catch-up`, "materializing vault head", { generation: prevGen });
        cur = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
        curPin = { generation: prevGen, head_sha256: bytes ? sha256Hex(bytes) : curPin.head_sha256, pinned_at: formatVaultTimestamp(this.now()) };
      }
    ...
    return {
      ...
      decrypt: decryptValidate
        ? {
            decryptable_to_generation: decryptPin?.generation ?? VAULT_GENESIS_GENERATION,
  ...
  private async readCachedHeadBytes(generation: string, expectedSha256: string): Promise<Uint8Array | null> {
    ...
    // vault-clone-scaling (P2): a page-prefetched head serves exactly as
    ...
    const path = vaultPaths.head(generation);
   ...
   */
  private async readHead(generation: string, expectedSha256: string): Promise<VaultHead> {
    const bytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.head(generation) });
    ...
    if (!bytes) {
      fail("GENERATION_REGRESSION", `the vault no longer holds the authenticated generation ${generation}; it regressed below the pin`, "reading pinned vault head", { generation }, [
        { action: "do not publish; escalate — the vault regressed below a generation this client authenticated" },
      ]);
    }
    if (sha256Hex(bytes) !== expectedSha256) fail("CHAIN_BROKEN", `pinned head ${generation} no longer hashes to the pin`, "reading pinned vault head", { generation });
    this.keystore.writeCachedHead(this.repoId, generation, expectedSha256, bytes);
    return parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
  }
  ...
  private decodeCarrierFrame<T extends { object_kind: string; signature: string }>(
    ...
    if (!frame) fail("CHAIN_UNUSABLE", `${kind} ${receipt.object_id} is absent from storage`, "materializing vault head", { object_id: receipt.object_id }, [{ action: "stay read-only at the materialized pin; run the repair path" }]);
    ...
    try {
      ...
      fail("CHAIN_UNUSABLE", `${kind} ${receipt.object_id} cannot be opened: ${(e as Error).message}`, "materializing vault head", { object_id: receipt.object_id }, [{ action: "stay read-only at the materialized pin; run the repair path" }]);
    }
    const object = parseVaultStrict(new TextDecoder().decode(plaintext)) as T;
    if (object.object_kind !== kind || (object as { object_id?: string }).object_id !== receipt.object_id || (object as { repo_id?: string }).repo_id !== this.repoId || !verifyVaultObject(object as unknown as VaultSignedObject, writerKey)) {
      fail("CHAIN_UNUSABLE", `${kind} ${receipt.object_id} plaintext fails its identity/signature checks`, "materializing vault head", { object_id: receipt.object_id });
    }
    return object;
  }
   ...
   */
  private async openMaterializeCarriers(refStateReceipt: VaultRefStateReceipt, refStatePath: string, rootsReceipt: VaultRetentionRootsReceipt, rootsPath: string, writerKey: string, keyOverride?: { epoch: string; k_repo: Uint8Array }): Promise<{ refState: VaultRefState; roots: VaultRetentionRoots }> {
    ...
    // vault-clone-scaling (P2): a page-prefetched frame serves exactly as
    ...
    const refState = this.decodeCarrierFrame<VaultRefState>("ref_state", refStateReceipt, refStateFrame, writerKey, keyOverride);
    const roots = this.decodeCarrierFrame<VaultRetentionRoots>("retention_roots", rootsReceipt, rootsFrame, writerKey, keyOverride);
   ...
   * `VAULT_EPOCH_NOT_OPENABLE`, never a bare `VAULT_AEAD_AUTH_FAILURE`)
   * — `strict: true` means this call throws exactly where the OLD
   * (pre-fix) `materialize()` silently produced a wrong `k_obj` instead.
   */
  async materialize(options: { persist?: boolean; deltaSince?: string; restore?: boolean } = {}): Promise<VaultMaterializedState> {
    const persist = options.persist ?? true;
    ...
    if (!state.head) {
      if (persist) this.keystore.updateRepo(this.repoId, { materialized_pin: { generation: state.generation, head_sha256: state.head_sha256, pinned_at: formatVaultTimestamp(this.now()) } });
    ...
    if (refState.generation !== state.generation || roots.generation !== state.generation) fail("CHAIN_UNUSABLE", "carrier generation does not match the head", "materializing vault head");
  ...
  // ── envelope recipients (vault-human-envelopes task 4.1, the ADD-path workaround) ──
   ...
   * the single constant `VAULT_GENESIS_EPOCH` on every head, so there
   ...
   * doc comment on {@link VaultOrgEncryptionKeyEntry}), so against a
   ...
   * (e.g. `VAULT_READ_ONLY` when this principal holds no signing key).
   * Callers that want "never block on this" (the deploy hook) wrap the call
   * themselves — see `Repos.capture`'s `#tryReconcileEnvelopeRecipients`.
   */
  async reconcileEnvelopeRecipients(): Promise<VaultReconcileEnvelopeRecipientsResult> {
    const repo = this.repoFile();
    ...
    const wrapped: VaultReconcileEnvelopeRecipientsWrapped[] = [];
    const alreadyCovered: string[] = [];
    const skipped: VaultReconcileEnvelopeRecipientsSkipped[] = [];
   ...
   */
  private seal(kind: "wal_pack" | "ref_state" | "retention_roots" | "checkpoint_manifest" | "checkpoint_pack", objectId: string, plaintext: Uint8Array, path: string, keyOverride?: { k_repo: Uint8Array; epoch: string }): VaultUploadObject {
    const kRepo = keyOverride?.k_repo ?? this.kRepo();
    const epoch = keyOverride?.epoch ?? this.epoch();
  ...
  /**
   * The owner signing seed, or `VAULT_READ_ONLY`.
   *
  ...
  signer(): Uint8Array {
    ...
    if (!kp) fail("VAULT_READ_ONLY", "the signing key is missing from identity.json; this principal is read-only", "signing vault object", undefined, [{ action: "stay read-only at the materialized pin" }]);
    return kp.seed;
  }
  ...
  /**
   * vault-multi-writer (task 5.8) — the push pre-check: before signing an
   ...
   * `VAULT_WRITER_NOT_ADMITTED`. `true` is the CAS-LOSER path
   ...
   * `VAULT_WRITER_REMOVED` (D10) instead: a real prior attempt just lost
  ...
  private assertCallerIsWriter(context: string, removedMidRace = false): void {
    ...
    const code = removedMidRace ? "VAULT_WRITER_REMOVED" : "VAULT_WRITER_NOT_ADMITTED";
    fail(
      code,
      removedMidRace
      ...
      [
        {
          ...
          why: "This session's key is not (yet, or no longer) an admitted writer on this vault. If you were just added as an org member, any current writer's next vault operation (push, deploy, or r.repos.reconcile()) admits pending writers automatically.",
        },
      ],
    );
  }

  /** The owner's full signing keypair, or `VAULT_READ_ONLY` — same refusal path as {@link signer}, which returns only the seed; {@link sealKeyEnvelope} needs both halves. */
  private signingKeypair(): VaultSigningKeypair {
    const identity = this.keystore.ensureIdentity();
    const kp = this.keystore.signingKeypair(identity);
    if (!kp) fail("VAULT_READ_ONLY", "the signing key is missing from identity.json; this principal is read-only", "signing vault object", undefined, [{ action: "stay read-only at the materialized pin" }]);
    return kp;
  ...
  private buildRefState(generation: string, refs: VaultRefMap, headTarget: VaultHeadTarget, keyOverride?: { k_repo: Uint8Array; epoch: string }): { object: VaultRefState; upload: VaultUploadObject } {
    ...
    if (headTarget.kind === "detached" && !VAULT_OID40_RE.test(headTarget.oid)) fail("REF_TRANSACTION_INVALID", "detached head_target needs a 40-hex oid", "building ref_state");
    assertRefMapCardinality(refs);
    const sorted: VaultRefMap = {};
    for (const k of Object.keys(refs).sort()) sorted[k] = refs[k]!;
    const id = newVaultId("refs");
    const object = signVaultObject({ format: VAULT_FORMAT, object_kind: "ref_state" as const, suite: VAULT_SUITE, repo_id: this.repoId, object_id: id, generation, refs: sorted, head_target: headTarget }, this.signer()) as VaultRefState;
    const plaintext = storedBytes(object as unknown as VaultSignedObject);
    if (plaintext.length > VAULT_MAX_REF_STATE_OBJECT_BYTES) fail("REF_STATE_LIMIT_EXCEEDED", "ref_state object exceeds 32 MiB", "building ref_state");
    return { object, upload: this.seal("ref_state", id, plaintext, vaultPaths.refState(id), keyOverride) };
  }

  private buildRetentionRoots(generation: string, roots: VaultRetentionRoot[], cutoff: VaultRetentionRoots["cutoff"], keyOverride?: { k_repo: Uint8Array; epoch: string }): { object: VaultRetentionRoots; upload: VaultUploadObject } {
    const id = newVaultId("rr");
    const object = signVaultObject({ format: VAULT_FORMAT, object_kind: "retention_roots" as const, suite: VAULT_SUITE, repo_id: this.repoId, object_id: id, generation, cutoff, roots: [...roots].sort(compareRoots) }, this.signer()) as VaultRetentionRoots;
    const plaintext = storedBytes(object as unknown as VaultSignedObject);
    if (plaintext.length > VAULT_MAX_REF_STATE_OBJECT_BYTES) fail("REF_STATE_LIMIT_EXCEEDED", "retention_roots object exceeds 32 MiB", "building retention_roots");
    return { object, upload: this.seal("retention_roots", id, plaintext, vaultPaths.retentionRoots(id), keyOverride) };
  }
  ...
  async buildPacks(tips: string[], base: string[]): Promise<Uint8Array[]> {
    ...
    for (const t of new Set(tips)) {
      if (!VAULT_OID40_RE.test(t)) continue;
      if (await hasObject(dir, t)) { uniqueTips.push(t); continue; }
      if (baseSet.has(t)) continue;
      fail("GIT_COMMAND_FAILED", `tip ${t} is not in this checkout's object store and the vault does not cover it yet — fetch it before publishing`, "building vault packs", { oid: t });
    }
    if (uniqueTips.length === 0) return [];
    const presentBase: string[] = [];
    for (const b of baseSet) if (VAULT_OID40_RE.test(b) && (await hasObject(dir, b))) presentBase.push(b);
    ...
    try {
      ...
      await hardenedGit(dir, ["pack-objects", "--revs", "--no-reuse-delta", "--delta-base-offset", `--max-pack-size=${VAULT_MULTI_OBJECT_PACK_TARGET_BYTES}`, "-q", join(tmp, "p")], { input: revs });
  ...
  async objectSetIn(dir: string, tips: string[]): Promise<string[]> {
    const unique = [...new Set(tips)].filter((t) => VAULT_OID40_RE.test(t));
   ...
   */
  async openRetentionRootsAt(receipt: VaultRetentionRootsReceipt): Promise<VaultRetentionRoots> {
    const { genesis } = await this.genesis();
    return this.openCarrier<VaultRetentionRoots>("retention_roots", receipt, vaultPaths.retentionRoots(receipt.object_id), genesis.creator_signing_pubkey);
  }

  private digest(label: VaultDigestLabel, content: unknown): string {
   ...
   */
  keyedDigest(label: VaultDigestLabel, content: unknown): string {
  ...
  static coverageTips(refs: VaultRefMap, roots: VaultRetentionRoot[], headTarget: VaultHeadTarget): string[] {
   ...
   */
  async buildCheckpoint(input: { generation: string; ref_state: VaultRefState; retention_roots: VaultRetentionRoots }): Promise<VaultBuiltCheckpoint> {
    const tips = Vault.coverageTips(input.ref_state.refs, input.retention_roots.roots, input.ref_state.head_target);
    const absent: string[] = [];
    ...
    if (packs.length > VAULT_MAX_CHECKPOINT_PACKS) fail("CHECKPOINT_SET_LIMIT_EXCEEDED", `${packs.length} packs exceed the ${VAULT_MAX_CHECKPOINT_PACKS}-pack V0 checkpoint maximum`, "building checkpoint set", undefined, [{ action: "compact or prune before adding coverage" }]);
    const entries: VaultCheckpointManifestPack[] = [];
    const objects: VaultUploadObject[] = [];
    let totalPlain = 0n;
    let totalStored = 0n;
    for (const pack of packs) {
      const id = newVaultId("ckp");
      const upload = this.seal("checkpoint_pack", id, pack, vaultPaths.checkpointPack(id));
      objects.push(upload);
    ...
    const manifestUnsigned = {
      format: VAULT_FORMAT, object_kind: "checkpoint_manifest" as const, suite: VAULT_SUITE, repo_id: this.repoId, object_id: newVaultId("chk"),
    ...
    const manifest = signVaultObject(manifestUnsigned, this.signer()) as VaultCheckpointManifest;
    const manifestUpload = this.seal("checkpoint_manifest", manifest.object_id, storedBytes(manifest as unknown as VaultSignedObject), vaultPaths.checkpointManifest(manifest.object_id));
    objects.unshift(manifestUpload);
    totalStored += BigInt(manifestUpload.size_bytes);
    if (totalStored > VAULT_MAX_CHECKPOINT_TOTAL_STORED_BYTES) fail("CHECKPOINT_SET_LIMIT_EXCEEDED", "the checkpoint's stored bytes exceed the 800 GiB V0 maximum", "building checkpoint set", { total_stored_size_bytes: String(totalStored) }, [{ action: "compact or prune before adding coverage" }]);
    const claimSet = signVaultObject({
      format: VAULT_FORMAT, object_kind: "checkpoint_claim_set" as const, suite: VAULT_SUITE, repo_id: this.repoId, object_id: newVaultId("ccs"),
      ...
      ordered_pack_receipts: entries.map((e): VaultCheckpointPackReceipt => ({ object_id: e.object_id, object_kind: "checkpoint_pack", ciphertext_sha256: e.ciphertext_sha256, size_bytes: e.size_bytes })),
      total_stored_size_bytes: String(totalStored), covers_through_generation: input.generation, writer_key_id: this.writerKeyId(),
    }, this.signer()) as VaultCheckpointClaimSet;
    const claimBytes = storedBytes(claimSet as unknown as VaultSignedObject);
    const claimUpload: VaultUploadObject = { path: vaultPaths.claimSet(claimSet.object_id), object_kind: "checkpoint_claim_set", object_id: claimSet.object_id, bytes: claimBytes, sha256: sha256Hex(claimBytes), size_bytes: String(claimBytes.length) };
    objects.push(claimUpload);
    const built: VaultBuiltCheckpoint = { manifest, claim_set: claimSet, claim_set_receipt: { object_id: claimSet.object_id, object_kind: "checkpoint_claim_set", stored_bytes_sha256: claimUpload.sha256, size_bytes: claimUpload.size_bytes }, objects, packs, covered_tips: tips };
  ...
  async acceptCheckpoint(built: VaultBuiltCheckpoint, refState: VaultRefState, roots: VaultRetentionRoots): Promise<void> {
    checkClaimSetEquality(built.claim_set, built.manifest, built.claim_set.covers_through_generation);
    const scratch = mkdtempSync(join(tmpdir(), "run402-vault-accept-"));
    try {
      ...
      const tips = Vault.coverageTips(refState.refs, roots.roots, refState.head_target);
      for (const t of tips) {
   ...
   */
  async verifyStoredCheckpoint(head: VaultHead, headSha256: string): Promise<VaultStoredCheckpointAttestation> {
    ...
    const { genesis } = await this.genesis();
    // vault-multi-writer rev 47: `head` may have been signed by any
    ...
    const claimBytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.claimSet(block.claim_set.object_id), expected_sha256: block.claim_set.stored_bytes_sha256 });
    ...
    const claimSet = parseVaultStrict(new TextDecoder().decode(claimBytes)) as VaultCheckpointClaimSet;
    if (!verifyVaultObject(claimSet as unknown as VaultSignedObject, writerKey)) fail("CHECKPOINT_INCOMPLETE", "checkpoint claim set signature fails", "verifying a stored checkpoint", { object_id: claimSet.object_id });
    const manifest = await this.openCarrier<VaultCheckpointManifest>("checkpoint_manifest", claimSet.manifest_receipt, vaultPaths.checkpointManifest(claimSet.manifest_receipt.object_id), writerKey);
    checkClaimSetEquality(claimSet, manifest, block.covers_through_generation);
    const refState = await this.openCarrier<VaultRefState>("ref_state", head.ref_state, vaultPaths.refState(head.ref_state.object_id), writerKey);
    const roots = await this.openCarrier<VaultRetentionRoots>("retention_roots", head.retention_roots, vaultPaths.retentionRoots(head.retention_roots.object_id), writerKey);
    const tips = Vault.coverageTips(refState.refs, roots.roots, refState.head_target);
    const scratch = mkdtempSync(join(tmpdir(), "run402-vault-attest-"));
    try {
      await hardenedGit(scratch, ["init", "-q", "--bare", "--object-format=sha1", "."]);
      for (const p of manifest.packs) {
        const frame = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.checkpointPack(p.object_id), expected_sha256: p.ciphertext_sha256 });
   ...
   */
  async chainEntries(): Promise<Array<{ head: VaultHead; head_sha256: string; claim_set: VaultCheckpointClaimSet | null }>> {
    ...
    const out: Array<{ head: VaultHead; head_sha256: string; claim_set: VaultCheckpointClaimSet | null }> = [];
    const { genesis } = await this.genesis();
    // vault-multi-writer rev 47: `verifyToNewest()` above already
    ...
    for (const gen of generations) {
      ...
      let claimSet: VaultCheckpointClaimSet | null = null;
      ...
      if (block) {
        ...
        const bytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.claimSet(block.claim_set.object_id), expected_sha256: block.claim_set.stored_bytes_sha256 });
        if (!bytes || sha256Hex(bytes) !== block.claim_set.stored_bytes_sha256) {
          fail("CHECKPOINT_INCOMPLETE", `checkpoint claim set ${block.claim_set.object_id} (generation ${gen}) is absent or altered`, "walking the vault chain", { generation: gen, object_id: block.claim_set.object_id });
        }
        claimSet = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultCheckpointClaimSet;
        if (!verifyVaultObject(claimSet as unknown as VaultSignedObject, writerKey)) {
          fail("CHECKPOINT_INCOMPLETE", `checkpoint claim set ${claimSet.object_id} signature fails`, "walking the vault chain", { generation: gen });
        }
      }
  ...
  /**
   * vault-byo-primary-bucket task 3.2 — resolve (once, cached) this
   ...
   * belongs to (surfaced downstream as `VAULT_BYO_BUCKET_WRITE_REFUSED`
   * when the session actually names a `put: null` object with no target).
   */
  private async resolveByoWriteTarget(): Promise<{ destination: import("./vault-mirror-config.js").VaultMirrorDestination; credential?: import("./vault-mirror-config.js").VaultMirrorCredential } | null> {
    if (this.byoResolution !== undefined) return this.byoResolution;
    const { readByoConfig } = await import("./vault-byo-config.js");
  ...
  private async uploadAll(objects: VaultUploadObject[]): Promise<void> {
    const byo = await this.resolveByoWriteTarget();
    const receipts = await this.transport.uploadObjects({ repo_id: this.repoId, objects, ...(byo ? { byo } : {}) });
    if (receipts.length !== objects.length) fail("VAULT_RECEIPT_MISMATCH", `${receipts.length} receipts for ${objects.length} objects`, "comparing finalization receipts");
    ...
    if (mismatches.length > 0) {
      fail("VAULT_RECEIPT_MISMATCH", `the server's finalization receipts do not match the expected manifest for ${mismatches.map((m) => m.path).join(", ")}; refusing to sign a head over them`, "comparing finalization receipts", { paths: mismatches.map((m) => m.path) });
    }
  }
  ...
  private async publishGeneration(input: {
    ...
    cutoff: VaultCutoffOptions | null;
    /** vault-multi-writer (task 5.6) — rides this generation's head verbatim; `undefined` for every ordinary push. */
    transition?: VaultTransitionEnvelope;
    ...
    capture_binding?: VaultPushOptions["capture_binding"];
    /**
    ...
    | { outcome: "admitted"; generation: string; head: VaultHead; head_sha256: string; admission_record_sha256: string; capture_receipt: VaultCaptureReceipt | null; form: "wal" | "checkpoint"; refs: VaultRefMap }
    | { outcome: "conflict"; generation: string; winner: { generation: string; stored_bytes_sha256: string } }
    | { outcome: "dry_run"; generation: string; form: "wal" | "checkpoint"; refs: VaultRefMap; head_target: VaultHeadTarget; objects: VaultUploadObject[]; raw_pack_bytes: number }
  > {
    const { base } = input;
    ...
    const baseTips = Vault.coverageTips(base.refs, base.roots, base.head_target);
    const newTips = Vault.coverageTips(input.refs, roots, input.head_target);
    ...
    const walEntries: VaultWalPackReceipt[] = [];
    let checkpoint: VaultCheckpointBlock | null = null;
    const objects: VaultUploadObject[] = [refState.upload, rootsObj.upload];
    let rawPackBytes = 0;
    if (input.force_checkpoint || walPacks.length > VAULT_MAX_WAL_RECEIPTS_PER_HEAD) {
      form = "checkpoint";
      ...
      for (const pack of walPacks) {
        const id = newVaultId("wal");
        const upload = { ...this.seal("wal_pack", id, pack, vaultPaths.wal(id)), base_generation: base.generation };
        objects.push(upload);
    ...
    const head = this.signHead(
      ...
      // vault-multi-writer (task 5.6) — an `add_writer_key` activation
   ...
   * supplied.
   */
  async push(options: VaultPushOptions): Promise<VaultPublishResult> {
    let conflicts = 0;
    let removalRotated = false;
    let base = options.base ?? (await this.materialize());
    for (;;) {
      // vault-multi-writer (task 5.8) — checked fresh on EVERY attempt
      ...
      // re-materializing from the winner) — VAULT_WRITER_REMOVED there,
      // the ordinary VAULT_WRITER_NOT_ADMITTED on the first attempt.
      this.assertCallerIsWriter("publishing vault head", conflicts > 0);
      ...
      try {
        ...
        // vault-human-envelopes D5 ("Remove: next capture rotates the
        // epoch key to the remaining set") / vault-multi-writer D6: an
      ...
      if (published.outcome === "conflict") {
        conflicts += 1;
        if (conflicts > this.retries) fail("HEAD_CAS_CONFLICT", `admission lost ${conflicts} races at generation ${published.generation}; giving up`, "publishing vault head", { generation: published.generation, winner: published.winner }, [{ action: "verify the attached winner from storage, rebase, retry" }]);
      ...
      if (published.outcome === "dry_run") fail("GIT_COMMAND_FAILED", "internal: push() received a dry-run result it never requested", "publishing vault head");
      this.keystore.updateRepo(this.repoId, { last_ref_transaction: { generation: published.generation, transaction: options.transaction, at: formatVaultTimestamp(this.now()) } });
   ...
   * resolve the vault READ-ONLY first (see `Repos.planCapture` in
   * `../namespaces/repos.js`, which reports `allocation_needed: true` in
   * that case instead of calling this method).
   */
  async planPush(options: VaultPushOptions): Promise<VaultPushPlan> {
    ...
    if (published.outcome !== "dry_run") fail("GIT_COMMAND_FAILED", "internal: planPush did not receive a dry-run result", "planning vault push");
   ...
   *
   * This is NOT `run402 repos gc`: the §7.2 maintenance CYCLE (purpose
   ...
   */
  async publishCheckpoint(options: { cutoff?: VaultCutoffOptions | false } = {}): Promise<VaultPublishResult> {
    let conflicts = 0;
    for (;;) {
      ...
      if (published.outcome === "conflict") {
        conflicts += 1;
        if (conflicts > this.retries) fail("HEAD_CAS_CONFLICT", `the checkpoint lost ${conflicts} races at generation ${published.generation}; giving up`, "publishing vault checkpoint", { generation: published.generation, winner: published.winner }, [{ action: "verify the attached winner from storage, retry" }]);
        continue;
      }
      // `publishCheckpoint` never sets `dry_run`, so this outcome is
      // unreachable here — narrows `published` to `"admitted"` below.
      if (published.outcome === "dry_run") fail("GIT_COMMAND_FAILED", "internal: publishCheckpoint() received a dry-run result it never requested", "publishing vault checkpoint");
      return { generation: published.generation, head_sha256: published.head_sha256, head: published.head, admission_record_sha256: published.admission_record_sha256, capture_receipt: published.capture_receipt, form: published.form, conflicts_retried: conflicts, refs: published.refs, checkpoint_staleness: this.checkpointStalenessNow(published.generation) };
    }
  }

  /** Request a `retention_cutoff` ticket and check it binds THIS base head (and the service key, when one is pinned). */
  private async issueRetentionCutoff(baseHeadSha256: string): Promise<VaultRetentionCutoffIssued> {
    ...
    if (t.object_kind !== "retention_cutoff" || t.repo_id !== this.repoId || t.base_head_sha256 !== baseHeadSha256) {
      fail("VAULT_CUTOFF_TICKET_INVALID", `the retention cutoff ticket does not bind this vault's base head ${baseHeadSha256}`, "requesting retention cutoff ticket", { object_id: t.object_id, base_head_sha256: t.base_head_sha256 });
    }
    if (Date.parse(t.expires_at) <= this.now().getTime()) fail("VAULT_CUTOFF_TICKET_INVALID", `the retention cutoff ticket expired at ${t.expires_at}`, "requesting retention cutoff ticket", { object_id: t.object_id });
    if (this.servicePublicKey && !verifyVaultObject(t as unknown as VaultSignedObject, this.servicePublicKey)) {
      fail("VAULT_CUTOFF_TICKET_INVALID", "the retention cutoff ticket is not signed by the pinned service key", "requesting retention cutoff ticket", { object_id: t.object_id, service_key_id: t.service_key_id });
    }
    return issued;
  }
   ...
   */
  private signHead(
    fields: Omit<VaultHead, "format" | "object_kind" | "suite" | "repo_id" | "epoch" | "transition" | "pin_manifest" | "writer_key_id" | "created_at" | "signature">,
    overrides?: { epoch?: string; transition?: VaultTransitionEnvelope | null; pin_manifest?: VaultPinManifestReceipt },
  ): VaultHead {
    const unsigned = {
      format: VAULT_FORMAT, object_kind: "head" as const, suite: VAULT_SUITE, repo_id: this.repoId,
      ...
      writer_key_id: this.writerKeyId(), created_at: formatVaultTimestamp(this.now()),
    };
    return signVaultObject(unsigned, this.signer()) as VaultHead;
  }

  /** Admit a signed head; on success read it back from storage and compare BEFORE any pin advances. */
  private async admit(head: VaultHead): Promise<{ outcome: "admitted"; head_sha256: string; admission_record_sha256: string; capture_receipt: VaultCaptureReceipt | null } | { outcome: "conflict"; winner: { generation: string; stored_bytes_sha256: string } }> {
    const bytes = storedBytes(head as unknown as VaultSignedObject);
    ...
    const back = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.head(head.generation) });
    if (!back || sha256Hex(back) !== hash) {
      fail("VAULT_HEAD_READBACK_MISMATCH", `the admitted head at generation ${head.generation} read back ${back ? sha256Hex(back) : "absent"} ≠ ${hash}; the push is NOT reported as landed and no pin advances`, "reading back admitted head", { generation: head.generation, expected: hash, observed: back ? sha256Hex(back) : null });
    }
    ...
    const pin: VaultHeadPin = { generation: head.generation, head_sha256: hash, pinned_at: formatVaultTimestamp(this.now()) };
    this.keystore.updateRepo(this.repoId, {
      head_pin: pin,
      materialized_pin: pin,
      verified_prefix: null,
      // vault-clone-scaling (P3): a checkpoint-form head IS fresh
  ...
  // ── epoch rotation (D193-D203, rev 42, change vault-human-envelopes) ──
  ...
  private async loadEffectivePinManifest(newestGeneration: string): Promise<{ pinManifestVersion: string; pinManifestSha256: string; pinnedFingerprintOf: Map<string, string> }> {
    ...
    return { pinManifestVersion: "0".repeat(16), pinManifestSha256: VAULT_ZERO_SHA256_SENTINEL, pinnedFingerprintOf: new Map() };
  }

  /**
   * Local-cache short-circuit (see {@link VaultRepoFile.known_pin_manifest}'s
   ...
   */
  private async readPinManifestObject(receipt: VaultPinManifestReceipt): Promise<{ pinManifestVersion: string; pinManifestSha256: string; pinnedFingerprintOf: Map<string, string> }> {
    ...
    const bytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.pinManifest(receipt.pin_manifest_version), expected_sha256: receipt.stored_bytes_sha256 });
    if (!bytes || sha256Hex(bytes) !== receipt.stored_bytes_sha256) {
      fail("VAULT_RECEIPT_MISMATCH", `recipient_pin_manifest ${receipt.pin_manifest_version} is absent or does not match its receipted hash`, "resolving the effective recipient pin manifest", { pin_manifest_version: receipt.pin_manifest_version });
    }
    const manifest = parseVaultStrict(new TextDecoder().decode(bytes)) as { pins: VaultRecipientPinManifestEntry[]; writer_key_id: string; signature: string; object_kind: string };
    const { genesis } = await this.genesis();
    // vault-multi-writer rev 47: resolve the manifest's OWN declared
    ...
    if (!verifyVaultObject(manifest as unknown as VaultSignedObject, manifestSigner)) {
      fail("VAULT_SIGNATURE_INVALID", `recipient_pin_manifest ${receipt.pin_manifest_version} signature does not verify`, "resolving the effective recipient pin manifest", { pin_manifest_version: receipt.pin_manifest_version });
    }
  ...
  private buildPinManifestUpdate(
    ...
    updates: { principal_id: string; ek_fingerprint: string; confirmed_by: "operator_confirmation"; receipt: VaultRecipientConfirmationReceipt }[],
  ): { nextVersion: string; manifestSha: string; upload: VaultUploadObject; pins: { principal_id: string; ek_fingerprint: string }[] } {
    for (const u of updates) {
    ...
    const pins: VaultRecipientPinManifestEntry[] = [];
    for (const [principalId, ekFp] of prior.pinnedFingerprintOf) {
      if (updateIds.has(principalId)) continue; // replaced below
      pins.push({ principal_id: principalId, ek_fingerprint: ekFp, pinned_at: formatVaultTimestamp(this.now()), confirmed_by: "operator_confirmation", confirmation_receipt_sha256: VAULT_ZERO_SHA256_SENTINEL });
    }
    for (const u of updates) {
      pins.push({ principal_id: u.principal_id, ek_fingerprint: u.ek_fingerprint, pinned_at: formatVaultTimestamp(this.now()), confirmed_by: "operator_confirmation", confirmation_receipt_sha256: storedBytesSha256(u.receipt as unknown as VaultSignedObject) });
    }
    pins.sort((a, b) => (a.principal_id < b.principal_id ? -1 : a.principal_id > b.principal_id ? 1 : 0));
    const unsignedManifest = { format: VAULT_FORMAT, object_kind: "recipient_pin_manifest" as const, suite: VAULT_SUITE, repo_id: this.repoId, pin_manifest_version: nextVersion, base_pin_manifest_sha256: prior.pinManifestSha256, pins, writer_key_id: this.writerKeyId() };
    const manifest = signVaultObject(unsignedManifest, this.signer());
    const manifestBytes = storedBytes(manifest as unknown as VaultSignedObject);
    const manifestSha = sha256Hex(manifestBytes);
    const upload: VaultUploadObject = { path: vaultPaths.pinManifest(nextVersion), object_kind: "recipient_pin_manifest", object_id: null, bytes: manifestBytes, sha256: manifestSha, size_bytes: String(manifestBytes.length) };
   ...
   * deadlock the incident behind {@link Vault.rotateEpoch}'s
   ...
   */
  async publishPinManifestUpdate(input: { principal_id: string; ek_fingerprint: string; confirmed_by: "operator_confirmation"; receipt: VaultRecipientConfirmationReceipt }): Promise<VaultPublishResult> {
    let conflicts = 0;
    for (;;) {
      const base = await this.materialize();
      this.assertCallerIsWriter("publishing a recipient_pin_manifest update", conflicts > 0); // vault-multi-writer task 5.8
   ...
   * `packages/gateway/src/services/vault/reads.ts:getVaultRecord` and
   * every `routes/vault*.ts` handler, not inferred), not a client
   ...
   * the admitted predecessor. See {@link VaultRotationResult.pin_manifest_published}.
   *
  ...
  async rotateEpoch(options: {
    ...
    ikm_e?: Uint8Array;
    pending_confirmations?: { principal_id: string; ek_fingerprint: string; receipt: VaultRecipientConfirmationReceipt }[];
    /**
     * vault-multi-writer (task 5.9, D7/D228) — an outstanding
    ...
    for (;;) {
      const base = await this.materialize();
      this.assertCallerIsWriter("driving an epoch rotation", conflicts > 0); // vault-multi-writer task 5.8
      const repo = this.repoFile();
      ...
      if (!desired) {
        fail("VAULT_DESIRED_STATE_UNAVAILABLE", "the gateway did not report desired-recipient state (desired[]) for this vault — an epoch rotation cannot compute its H-partition without it", "computing the epoch-rotation H-partition");
      }
      ...
      if (pinBlocked.length > 0) {
        fail(
          "VAULT_ROTATION_BLOCKED_PIN_MISMATCH",
      ...
      // vault-multi-writer (task 5.9, D6/D227) — fold in a writer_set_update
      ...
      let writerSetUpdate: NonNullable<VaultRotateEpochPayload["writer_set_update"]> | null = null;
      if (ineligible.length > 0) {
        if (!writerPin) {
          fail("VAULT_WRITER_STATE_UNAVAILABLE", "no locally verified writer_set_pin for this vault — the chain must be verified (materialize/verifyToNewest) before a writer_set_update can be built against a known base writer set", "computing the writer_set_update");
        }
      ...
      const descriptorFields = {
        format: VAULT_FORMAT, object_kind: "rotation_attempt_descriptor" as const, suite: VAULT_SUITE,
      ...
      const signedDescriptor = signVaultObject({ ...descriptorFields, attempt_key_commitment: attemptCommitment }, this.signer()) as VaultRotationAttemptDescriptor;
      ...
      const sealedReceipts: VaultRotationEnvelopePair[] = [];
      const envelopeUploads: VaultUploadObject[] = [];
      const nowIso = formatVaultTimestamp(this.now());
      for (const p of included) {
        ...
        envelopeUploads.push({ path: vaultPaths.envelope(newEpoch, p.ek_fingerprint, rotationId), object_kind: "key_envelope", object_id: null, bytes: sealed.stored_bytes, sha256: sealed.stored_bytes_sha256, size_bytes: sealed.size_bytes });
      }
      ...
      const payloadBase: VaultRotateEpochPayload = {
        ...
        // vault-multi-writer (task 5.9, D227): present IFF this rotation ALSO removes one or more writers.
      ...
      // VAULT_EPOCH_NOT_OPENABLE / EPOCH_KEY_COMMITMENT_MISMATCH — never
      ...
      let selfOpenAttestation: VaultEpochRotationSelfOpen;
      let selfCheck: VaultRotationResult["self_check"] = "not_a_recipient";
      if (ownFingerprint && ownKeypair && ownIncluded) {
        await openEpochRotationForRecipient({
          ...
          get_envelope_bytes: (path) => this.transport.getObject({ repo_id: this.repoId, path }),
          envelope_path: (epoch, fp, rid) => vaultPaths.envelope(epoch, fp, rid),
        });
        ...
        selfOpenAttestation = {
          ...
          reader_entrypoint: vaultReaderEntrypoint("openEpochRotationForRecipient"),
        };
        selfCheck = "passed";
      } else {
      ...
      const payload: VaultRotateEpochPayload = { ...payloadBase, self_open_attestation: selfOpenAttestation };
      const payloadBytes = jcs(payload);
      const transition: VaultTransitionEnvelope = { kind: "rotate_epoch", payload_format: "base64url-jcs", payload: toBase64url(payloadBytes), payload_sha256: sha256Hex(payloadBytes) };
      ...
      // vault-multi-writer (task 5.9) — the SAME local-pin advance
      ...
      if (writerSetUpdate) {
        this.keystore.updateRepo(this.repoId, { writer_set_pin: { version: writerSetUpdate.next_version, sha256: writerSetUpdate.next_sha256, writers: [...writerSetUpdate.writers], pinned_at: formatVaultTimestamp(this.now()) } });
      }
   ...
   * (vault-multi-writer D6: "`member_removed` keeps its automatic
   ...
   * `VAULT_ROTATION_COUNTERS_UNAVAILABLE` on a gateway that does not yet
   * carry the counters on that read.
   */
  async rotateEpochForMemberRemoval(options: { client_idempotency_key?: string } = {}): Promise<VaultRotationResult> {
   ...
   */
  async rotateEpochForKeyRevocation(principalId: string, options: { client_idempotency_key?: string } = {}): Promise<VaultRotationResult> {
  ...
  // ── writer activation (vault-multi-writer task 5.6, design D4/D5) ──────
  ...
  async submitWriterActivationHead(input: {
  ...
  }): Promise<{ outcome: "activated"; result: VaultPublishResult } | { outcome: "already_admitted"; generation: string }> {
    return this.#publishAddWriterKeyTransition(
      ...
      // vault-multi-writer task 5.8: the handoff door is signed by the
  ...
  /**
   * vault-multi-writer (task 5.7) — the "writer"-door twin of {@link
   ...
   * writer, refuse EARLY and LOCALLY (`VAULT_WRITER_NOT_ADMITTED` /
   * `VAULT_WRITER_REMOVED`) rather than let the head reach the gateway
  ...
  async #publishAddWriterKeyTransition(
    addedWriterKeyId: string,
    buildPayload: (pin: { version: string; sha256: string; writers: readonly import("./vault-writer-state.js").WriterKeyEntry[] }) => import("./vault-writer-state.js").AddWriterKeyPayload,
    errorContext: string,
    ...
    for (;;) {
      ...
      if (!pin) {
        fail(
          "VAULT_WRITER_STATE_UNAVAILABLE",
      ...
      const transition: VaultTransitionEnvelope = { kind: "add_writer_key", payload_format: "base64url-jcs", payload: toBase64url(payloadBytes), payload_sha256: sha256Hex(payloadBytes) };
      const published = await this.publishGeneration({
        base, refs: base.refs, dropped: [], head_target: base.head_target,
      ...
      this.keystore.updateRepo(this.repoId, { writer_set_pin: { version: nextState.version, sha256: nextState.sha256, writers: [...nextState.writers], pinned_at: formatVaultTimestamp(this.now()) } });
      return {
        outcome: "activated",
  ...
  /**
   * vault-multi-writer (task 5.7) — the vault's OWN writer-admission
   ...
   * "not yet admitted" case {@link Repos.capture}'s own pre-push check
   ...
   */
  async reconcileWriterAdmissions(): Promise<VaultReconcileWriterAdmissionsResult> {
    const repo = this.repoFile();
    const admitted: VaultReconcileWriterAdmissionsAdmitted[] = [];
    const alreadyCovered: string[] = [];
    const skipped: VaultReconcileWriterAdmissionsSkipped[] = [];
    const assembleResult = (eligible: boolean): VaultReconcileWriterAdmissionsResult => ({ repo_id: this.repoId, org_id: repo.org_id, eligible, admitted, already_covered: alreadyCovered, skipped });
   ...
   */
  async repair(input: { base_generation: string; reason: VaultRepairDescriptor["reason"] }): Promise<VaultPublishResult> {
    const newest = await this.verifyToNewest();
    // vault-multi-writer task 5.8 — repair requires OWNER+step-up
    ...
    const baseRefState = await this.openCarrier<VaultRefState>("ref_state", baseEntry.head.ref_state, vaultPaths.refState(baseEntry.head.ref_state.object_id), writerKey);
    const baseRoots = await this.openCarrier<VaultRetentionRoots>("retention_roots", baseEntry.head.retention_roots, vaultPaths.retentionRoots(baseEntry.head.retention_roots.object_id), writerKey);
    ...
    for (let g = baseGen + 1n; g <= newestGen; g++) {
      ...
      let state: VaultRefState;
      try {
        state = await this.openCarrier<VaultRefState>("ref_state", entry.head.ref_state, vaultPaths.refState(entry.head.ref_state.object_id), writerKey);
      } catch {
    ...
    if (added.length > VAULT_MAX_REPAIR_ADDED_ROOTS) fail("REF_STATE_LIMIT_EXCEEDED", `${added.length} repair-added roots exceed the ${VAULT_MAX_REPAIR_ADDED_ROOTS} bound`, "preparing repair");
    ...
    let built: VaultBuiltCheckpoint;
    try {
   ...
   */
  private async chainFrom(baseGeneration: string, newest: VaultVerifiedState, transitions: "refuse" | "activated"): Promise<Map<string, { head: VaultHead; sha256: string }>> {
    const out = new Map<string, { head: VaultHead; sha256: string }>();
    ...
    while (g >= stop) {
      ...
      const bytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.head(gen) });
      if (!bytes || sha256Hex(bytes) !== expected) fail("CHAIN_BROKEN", `head ${gen} no longer matches the verified chain`, "re-reading vault chain", { generation: gen });
      const head = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
      if (transitions === "refuse") {
        if (head.transition !== null) fail("UPGRADE_REQUIRED", "a repair must not cross an admitted transition", "re-reading vault chain", { generation: gen });
      } else {
        assertNoTransition(head);
      }
   ...
   *
   * Design D5 (vault-client-round-trips): incremental above the local
   * `restored_through` marker (this target directory's own local git
   * config, read/written by {@link readVaultRestoreMarker}/{@link
   * writeVaultRestoreMarker}) — replaying only the WAL packs of
  ...
  async restoreObjectsInto(
    ...
    reuse?: { marker: VaultRestoreMarker | null; state: VaultMaterializedState },
  ): Promise<{ refs: VaultRefMap; head_target: VaultHeadTarget; generation: string; retained_refs: VaultRetainedRefsReconcileResult }> {
    // vault-delta-fetch: the marker is read BEFORE materialize so the
    ...
    //
    // vault-session-state-reuse: `reuse` lets a caller that ALREADY
    ...
    const marker = await readVaultRestoreMarker(targetRepoDir);
    const markerMatches = (a: VaultRestoreMarker | null, b: VaultRestoreMarker | null): boolean =>
      a === null ? b === null : b !== null && a.generation === b.generation && a.head_sha256 === b.head_sha256;
    // vault-restore-recipe design D2/D6: a marker-absent materialize is,
    ...
    if (marker && marker.generation === newest.generation && marker.head_sha256 === newest.head_sha256) {
      const tips = Vault.coverageTips(newest.refs, newest.roots, newest.head_target);
      const present = await hasObjects(targetRepoDir, tips);
      for (const t of tips) {
        if (!present.has(t)) fail("CHAIN_UNUSABLE", `covered tip ${t} does not resolve after restore`, "restoring vault objects", { oid: t });
      }
    ...
    // vault-restore-recipe (design D1-D6): consume a plan that already
    ...
    const heads: VaultHead[] = [];
    let cur: VaultHead | null = newest.head;
    let incremental = marker !== null;
    // vault-clone-scaling (bench P2): the backward walk's head PATHS are
    ...
    const prefetchBackwardWindow = async (hi: bigint): Promise<void> => {
      ...
      const pageCap = BigInt(VAULT_MAX_HEADS_PER_LISTING_PAGE);
    ...
    while (cur) {
      ...
      if (incremental && (cur.checkpoint !== null || cur.transition !== null)) {
        ...
        heads.length = 0;
        cur = newest.head;
        // vault-restore-recipe (design D1/D2, task 2.1's named fallback
      ...
      const bytes = await this.readCachedHeadBytes(prevGen, cur.prev_sha256);
      if (!bytes || sha256Hex(bytes) !== cur.prev_sha256) fail("CHAIN_BROKEN", `head ${prevGen} does not match the chain during restore`, "restoring vault objects");
      cur = parseVaultStrict(new TextDecoder().decode(bytes)) as VaultHead;
    }
  ...
  /**
   * The shared restore tail (vault-restore-recipe): apply `heads`'
   ...
   * pre-vault-restore-recipe shape.
   */
  /**
   * vault-multi-writer (rev 47): the key a head's own checkpoint claim set
   ...
   */
  private writerKeyForHead(head: VaultHead, fallback: string): string {
    const pin = this.repoFile().writer_set_pin;
  ...
  private async applyRestoreHeads(
    ...
    heads: VaultHead[],
    incremental: boolean,
    newest: VaultMaterializedState,
    writerKey: string,
    precheckedCheckpoint: { claimSet: VaultCheckpointClaimSet; manifest: VaultCheckpointManifest } | null,
    planPacks: Map<string, Uint8Array> | null,
  ): Promise<{ refs: VaultRefMap; head_target: VaultHeadTarget; generation: string; retained_refs: VaultRetainedRefsReconcileResult }> {
    // vault-clone-scaling (P3): a WHOLESALE walk just stopped at the
    ...
    {
      const stop = heads[0]!;
      const learned = stop.checkpoint ? stop.checkpoint.covers_through_generation : !incremental && stop.generation === "0000000000000001" ? VAULT_GENESIS_GENERATION : null;
      if (learned !== null) {
    ...
    // map `materialize()` just resolved (throwing `VAULT_EPOCH_NOT_OPENABLE`
    ...
    const first = heads[0]!;
    // vault-pipelined-restore: per-object settlement when the transport
    ...
    //
    // `expected` (vault-small-object-inline design D3) is index-aligned
    ...
    // vault-pipelined-restore D3: the WAL entry list derives from the
    ...
    // vault-delta-fetch: packs the state read already delivered inline
    ...
    // consumed exactly once. vault-restore-recipe extends the SAME
    ...
    const walFramesP = (async (): Promise<Array<Promise<Uint8Array | null>>> => {
      const fetched = uncovered.length > 0 ? await settled(uncovered.map((i) => vaultPaths.wal(walEntries[i]!.w.object_id)), uncovered.map((i) => walEntries[i]!.w.ciphertext_sha256)) : [];
    ...
    if (first.checkpoint) {
      let claimSet: VaultCheckpointClaimSet;
      let manifest: VaultCheckpointManifest;
      if (precheckedCheckpoint) {
        // vault-restore-recipe: already fetched + verified (signature,
        ...
        const claimBytes = await this.transport.getObject({ repo_id: this.repoId, path: vaultPaths.claimSet(first.checkpoint.claim_set.object_id), expected_sha256: first.checkpoint.claim_set.stored_bytes_sha256 });
        if (!claimBytes || sha256Hex(claimBytes) !== first.checkpoint.claim_set.stored_bytes_sha256) fail("CHECKPOINT_INCOMPLETE", "claim set absent or altered", "restoring vault objects");
        claimSet = parseVaultStrict(new TextDecoder().decode(claimBytes)) as VaultCheckpointClaimSet;
        if (!verifyVaultObject(claimSet as unknown as VaultSignedObject, this.writerKeyForHead(first, writerKey))) fail("CHECKPOINT_INCOMPLETE", "claim set signature fails", "restoring vault objects");
        manifest = await this.openCarrier<VaultCheckpointManifest>("checkpoint_manifest", claimSet.manifest_receipt, vaultPaths.checkpointManifest(claimSet.manifest_receipt.object_id), this.writerKeyForHead(first, writerKey), { epoch: first.epoch, k_repo: kRepoForEpoch(first.epoch) });
        checkClaimSetEquality(claimSet, manifest, first.checkpoint.covers_through_generation);
      }
      // Design D2 + vault-pipelined-restore D1: every checkpoint pack is
      ...
      // hash) still completes before any byte reaches git. vault-restore-
      ...
      const ckFrames = uncoveredCk.length > 0 ? await settled(uncoveredCk.map((i) => vaultPaths.checkpointPack(manifest.packs[i]!.object_id)), uncoveredCk.map((i) => manifest.packs[i]!.ciphertext_sha256)) : [];
      for (let i = 0; i < manifest.packs.length; i++) {
        ...
        if (!frame) fail("CHECKPOINT_INCOMPLETE", `checkpoint pack ${p.object_id} absent`, "restoring vault objects");
        ...
        if (sha256Hex(plain) !== p.plaintext_sha256 || String(plain.length) !== p.plaintext_size_bytes) fail("CHECKPOINT_INCOMPLETE", `checkpoint pack ${p.object_id} plaintext mismatch`, "restoring vault objects");
    ...
    for (let i = 0; i < walEntries.length; i++) {
      ...
      if (!frame) fail("CHAIN_UNUSABLE", `WAL pack ${w.object_id} absent`, "restoring vault objects");
    ...
    // ONE batched git invocation for the whole set (vault-delta-fetch
    ...
    await writeVaultRestoreMarker(targetRepoDir, newest.generation, newest.head_sha256);
    return { refs: newest.refs, head_target: newest.head_target, generation: newest.generation, retained_refs };
  }
   ...
   * (throwing `VAULT_EPOCH_NOT_OPENABLE` fail-closed if any needed epoch
   * could not be opened), so every lookup through this helper is guaranteed
   * present.
   */
  private epochKeyFor(newest: VaultMaterializedState, epoch: string): Uint8Array {
    const hex = newest.epoch_keys_hex[epoch];
    if (!hex) fail("VAULT_EPOCH_NOT_OPENABLE", `no locally known key for epoch ${epoch} while restoring objects`, "restoring vault objects", { epoch });
  ...
  private async verifyRestorePlan(
    ...
    newest: VaultMaterializedState,
    writerKey: string,
  ): Promise<{ heads: VaultHead[]; checkpoint: { claimSet: VaultCheckpointClaimSet; manifest: VaultCheckpointManifest } | null } | null> {
    ...
    // `parseVaultStrict`/`decodeCarrierFrame`/`checkClaimSetEquality`/
    // `verifyVaultObject` can raise (a `fail()`-thrown `Run402Error`, or a
    ...
    try {
      ...
      const parsed: { generation: string; bytes: Uint8Array; head: VaultHead }[] = [];
      for (const h of plan.heads) {
        if (sha256Hex(h.stored_bytes) !== h.stored_bytes_sha256) return null; // self-hash mismatch
        const head = parseVaultStrict(new TextDecoder().decode(h.stored_bytes)) as VaultHead;
      ...
      if (plan.boundary_generation === VAULT_GENESIS_GENERATION) {
      ...
      for (const p of parsed) this.walkPrefetch.set(vaultPaths.head(p.generation), p.bytes);

      let checkpoint: { claimSet: VaultCheckpointClaimSet; manifest: VaultCheckpointManifest } | null = null;
      if (first.head.checkpoint) {
        ...
        const claimSet = parseVaultStrict(new TextDecoder().decode(plan.checkpoint.claim_set.stored_bytes)) as VaultCheckpointClaimSet;
        if (!verifyVaultObject(claimSet as unknown as VaultSignedObject, this.writerKeyForHead(first.head, writerKey))) return null; // claim set signature fails
        ...
        const manifest = this.decodeCarrierFrame<VaultCheckpointManifest>("checkpoint_manifest", claimSet.manifest_receipt, plan.checkpoint.manifest.stored_bytes, this.writerKeyForHead(first.head, writerKey), { epoch: first.head.epoch, k_repo: this.epochKeyFor(newest, first.head.epoch) });
  ...
  /**
   * Consume a stashed restore plan (vault-restore-recipe design D1-D6):
  ...
  private async tryConsumeRestorePlan(
    ...
    newest: VaultMaterializedState,
    writerKey: string,
  ): Promise<{ refs: VaultRefMap; head_target: VaultHeadTarget; generation: string; retained_refs: VaultRetainedRefsReconcileResult } | null> {
  ...
  // ── compaction headroom grant (vault-checkpoint-cadence design D3) ──

  /** Thin passthrough to the transport — see {@link VaultTransport.openCompactionGrant}. */
  async openCompactionGrant(): Promise<VaultCompactionGrant> {
    return this.transport.openCompactionGrant({ repo_id: this.repoId });
  }

  /** Thin passthrough to the transport — see {@link VaultTransport.closeCompactionGrant}. Always safe best-effort; never throws by construction of the route (idempotent). */
...
export function checkClaimSetEquality(claimSet: VaultCheckpointClaimSet, manifest: VaultCheckpointManifest, headCoversThrough: string): void {
...
export function captureBinding(captureId: string, applyPlanSha256: string | null, snapshotOidHmac: string): VaultCaptureBinding {
  if (!/^[0-9a-f]{32}$/.test(captureId)) fail("VAULT_BAD_ID", "capture_id must be 32 lowercase hex", "building capture binding");
  return { capture_id: captureId, apply_plan_sha256: applyPlanSha256, snapshot_oid_hmac: snapshotOidHmac };
}

export type { VaultRefUpdate };
