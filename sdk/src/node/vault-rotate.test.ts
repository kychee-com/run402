/**
 * vault epoch rotation — the client-half producer (D193-D203, rev 42,
 * change `vault-human-envelopes`). Two layers:
 *
 ...
 *     fixture (`vault-memory-transport.test.ts`): `rotateEpochForKeyRevocation`
...
import type { VaultHead } from "../namespaces/vault.types.js";
...
import { Vault, vaultPaths } from "./vault-publication.js";
...
describe("epoch rotation — pure decisive vectors (D193-D203)", () => {
  ...
  it("D195 producer obligation: a freshly sampled K_e equal to a PRIOR epoch key is refused before anything is sealed", () => {
    ...
    assert.throws(() => checkFreshEpochKeyAgainstPriorKeys(prior1, [prior1, prior2]), (e: unknown) => (e as { code?: string }).code === "VAULT_EPOCH_KEY_NOT_FRESH");
    assert.throws(() => checkFreshEpochKeyAgainstPriorKeys(prior2, [prior1, prior2]), (e: unknown) => (e as { code?: string }).code === "VAULT_EPOCH_KEY_NOT_FRESH");
  });
  ...
  it("D195 complementary-path mosaic: two attempts sampling DIFFERENT K_e derive DIFFERENT rotation_id, even with every OTHER field identical", () => {
    ...
    const descriptor1 = signVaultObject({ ...baseFields, attempt_key_commitment: commitment1 }, writer.seed);
    const descriptor2 = signVaultObject({ ...baseFields, attempt_key_commitment: commitment2 }, writer.seed);
  ...
  it("D195: a byte-identical resume of one's OWN prior attempt derives the SAME rotation_id", () => {
    ...
    const descriptorA = signVaultObject({ ...fields, attempt_key_commitment: commitment }, writer.seed);
    const descriptorB = signVaultObject({ ...fields, attempt_key_commitment: commitment }, writer.seed);
...
describe("epoch rotation — producer end-to-end (rotateEpochForKeyRevocation)", () => {
  it("drives a real rotation, self-checks its own envelope, advances the local keystore, and an ordinary push immediately after succeeds (the signHead epoch fix)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-"));
    try {
      ...
      const reopened = Vault.open({ keystore: vault.keystore, transport, repo_id: vault.repoId, repo_dir: repoDir });
  ...
  /**
   * Local pin-manifest cache (`VaultRepoFile.known_pin_manifest`):
   ...
   * `vault-wire-shapes.test.ts`.) This test pins BOTH halves — the cache
  ...
  it("local pin-manifest cache: a manifest this keystore just published is read back locally (no network round trip); a cache miss still falls through to the network path unchanged", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-cache-"));
    try {
      const { transport, vault, repoDir } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
      ...
      // CLI invocations, each a brand-new Vault instance sharing
      // one on-disk keystore, per Repos.open()'s "new instance per call"
      // contract).
      const reopened = Vault.open({ keystore: vault.keystore, transport, repo_id: vault.repoId, repo_dir: repoDir });
      ...
      assert.deepEqual(pinManifestReadsAfterCacheMiss, [`get:${vaultPaths.pinManifest("0000000000000001")}`], "a cache miss falls through to the SAME network object-reads read the pre-fix code always made");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  it("EPOCH_ROTATION_REQUIRED blocks an ordinary push while a migration condition is outstanding, and a rotation clears it", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-migrate-"));
    try {
      const { transport, vault, repoDir } = await makeVault(dir);
  ...
  it("elective_rekey refuses locally (never round-trips) when coverage is a proper subset — a keyless desired principal", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-elective-"));
    try {
  ...
  it("member removal with no pins: a surviving writer is included on its directory key; without the writer roster the rotation is refused UNCOVERED", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-writer-incl-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  // vault-multi-writer D6 / human-envelopes D5: a membership removal is
  ...
  it("member removal completes on the survivors' side: a plain push auto-rotates under member_removed and then lands; the removed key is not in the new epoch", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-member-removed-"));
    try {
      const { transport, vault, repoDir } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
      ...
      // The gateway-side effect of `orgs members rm`: the member's desired row
  ...
  it("rotateEpochForMemberRemoval refuses VAULT_ROTATION_COUNTERS_UNAVAILABLE when the read carries no counters (older gateway), never guessing a fence", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-member-removed-old-gw-"));
    try {
      const { transport, vault } = await makeVault(dir);
      ...
      await assert.rejects(vault.rotateEpochForMemberRemoval(), (e: unknown) => (e as { code?: string }).code === "VAULT_ROTATION_COUNTERS_UNAVAILABLE");
  ...
  it("a pending_removal principal is excluded from H entirely — never wrapped into the new epoch (the forward-revocation point of member_removed)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-removal-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  it("D196/D197 fold: rotateEpoch's pending_confirmations durably publishes a receipted pin-manifest update on the SAME head as the rotation, closing the standalone publishPinManifestUpdate deadlock while an urgent condition is outstanding", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-fold-"));
    try {
      const { transport, vault, repoDir } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  it("PRE-FIX negative, pinned: a STANDALONE publishPinManifestUpdate for a receipt minted while an urgent condition is outstanding is refused EPOCH_ROTATION_REQUIRED — the deadlock the previous test's fold closes", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-fold-negative-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
...
async function readAdmittedRotatePayload(transport: VaultMemoryTransport, repoId: string, generation: string) {
  const bytes = await transport.getObject({ repo_id: repoId, path: vaultPaths.head(generation) });
...
describe("epoch rotation — D209 self_open_attestation (rev 44)", () => {
  it("'opened' branch: the writer is itself an included recipient — the admitted payload's self_open_attestation names its own fingerprint, the predecessor generation, and the admitted head's own generation", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-self-open-opened-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  it("'writer_not_recipient' branch: the writer holds no included envelope — attestation carries ONLY outcome + chain_verified_to_generation, no opened_fingerprint/decryptable_to_generation/reader_entrypoint", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-self-open-writer-not-recipient-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      ...
      // CI-writer shape (D1 of services/vault/desired-recipients.ts:
  ...
  it("a round-trip failure (this principal's own committed envelope cannot be opened) aborts CLIENT-SIDE, before anything is submitted — never a false attestation, never a partially-committed rotation", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-self-open-abort-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
      ...
      await assert.rejects(
        ...
        (e: unknown) => (e as { code?: string }).code === "VAULT_EPOCH_NOT_OPENABLE",
      );
...
/**
 * vault-multi-writer (task 5.9, D6/D227) — the writer_set_update fold-in:
 ...
 */
describe("epoch rotation — writer_set_update fold-in (vault-multi-writer task 5.9)", () => {
  it("an empty ineligible_members set is byte-identical to pre-5.9 behavior: no writer_set_update on the wire, writers_removed: [] on the result", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-wsu-noop-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  it("the sole writer's own key gateway-blocked: refused EPOCH_ROTATION_WOULD_LEAVE_VAULT_UNCOVERED without force_empty_writer_set; succeeds to an empty writer set WITH it", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-wsu-sole-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
  ...
  it("epoch_secret_exposed carries its reason down to each removed writer entry; every other top-level reason collapses to member_removed", async () => {
    const dir = mkdtempSync(join(tmpdir(), "run402-vault-rotate-wsu-reason-"));
    try {
      const { transport, vault } = await makeVault(dir);
      const orgId = (await vault.transport.getVaultRecord({ repo_id: vault.repoId })).org_id;
      const identity = vault.keystore.ensureIdentity();
