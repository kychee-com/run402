/**
 * vault-mirror-default — the `vault_unmirrored` standing finding.
 *
 ...
 *
 * Run: node --test --import tsx sdk/src/node/vault-mirror-default.test.ts
 */
...
import { Repos, vaultUnmirroredFinding } from "../namespaces/repos.js";
import { VAULT_UNMIRRORED_FINDING_STATEMENT } from "../namespaces/vault.crypto.js";
import { VaultKeystore } from "./vault-keystore.js";
import { readMirrorConfig, recordMirrorSuccess, saveMirrorConfig } from "./vault-mirror-config.js";
import { mirrorPushForGeneration, mirrorSync } from "./vault-mirror.js";
...
describe("vaultUnmirroredFinding — the pure computation", () => {
  it("no mirror configured → the finding, in its configure form", () => {
    const finding = vaultUnmirroredFinding({ configured: false, last_success_at: null, mirrored_generation: null });
    assert.ok(finding);
    assert.equal(finding.kind, "vault_unmirrored");
    assert.equal(finding.message, VAULT_UNMIRRORED_FINDING_STATEMENT);
  ...
  it("configured but no success evidence yet → the finding, in its backfill form", () => {
    const finding = vaultUnmirroredFinding({ configured: true, last_success_at: null, mirrored_generation: null });
    assert.ok(finding);
    assert.equal(finding.kind, "vault_unmirrored");
  ...
  it("a recorded success clears it — and keeps it cleared through a transiently unreachable mirror", () => {
    assert.equal(vaultUnmirroredFinding({ configured: true, last_success_at: "2026-08-30T00:00:00.000Z", mirrored_generation: null }), null);
  });

  it("a mirror that verifiably holds a generation clears it even without the local stamp (pre-change synced mirrors)", () => {
    assert.equal(vaultUnmirroredFinding({ configured: true, last_success_at: null, mirrored_generation: "0000000000000001" }), null);
  });
...
describe("mirror config — last_success_at lifecycle", () => {
  ...
  it("moving the destination resets the stamp — a success against the old bucket says nothing about the new one", () => {
    ...
    const finding = vaultUnmirroredFinding({ configured: true, last_success_at: moved.last_success_at ?? null, mirrored_generation: null });
    assert.equal(finding?.setup_command, "run402 repos mirror --backfill");
  });
...
describe("mirrorStatus + mirrorSync — present, cleared, and gateway-blind", () => {
  ...
  it("unconfigured vault: mirrorStatus carries the finding and performs ZERO network calls (the gateway never learns whether a mirror exists)", async () => {
    ...
    const status = await new Repos(client).mirrorStatus({ repo_id: REPO_ID, keystore_root: root });
  ...
  it("a zero-failure sync stamps last_success_at, and the finding no longer appears", async () => {
    ...
    const finding = vaultUnmirroredFinding({ configured: true, last_success_at: config!.last_success_at!, mirrored_generation: null });
