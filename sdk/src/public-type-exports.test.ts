...
test("typed config helpers and execution modes compile from config entrypoints", () => {
  ...
  try {
    writeFileSync(
      contractPath,
      [
        'import { defineConfig, dir, file, nodeFunction, scheduleTrigger, sqlFile, type Run402ExecutionMode } from "@run402/sdk/config";',
        ...
        '  functions: { replace: { api: nodeFunction("dist/functions/api.js", { requireAuth: true, triggers: [scheduleTrigger("maintenance_every_15m", "*/15 * * * *", { run: { event_type: "maintenance" } })] }) } },',
