/**
 * vault crypto core — byte-for-byte replay of the frozen `r402s/v0` vector
 ...
 * `docs/strategy/products/kygit/vectors/`) and VENDORED here at
 * `test-vectors/r402s-v0/`, so CI replays them without a private checkout.
 * Resolution + integrity live in `vault-vectors.test-helper.ts`:
 * `$VAULT_VECTORS_DIR` overrides the location, the vendored copy is the
 * default, `CONTINUITY.json` is asserted either way, and a directory that
 * cannot be resolved FAILS — only `VAULT_VECTORS_OPTOUT=1` skips (5.6b:
...
import { loadVaultVectors, OPTOUT_SKIP_MESSAGE, type VaultVector } from "../node/vault-vectors.test-helper.js";
...
// single case runs. `VAULT_VECTORS_OPTOUT=1` is the only skip.
...
describe("vault crypto — offline invariants", () => {
  ...
  it("jcs sorts members, escapes per RFC 8785, and refuses JSON numbers", () => {
    ...
    assertLocalCode(() => jcs({ size_bytes: 12 }), "VAULT_JSON_NUMBER");
  });
  it("timestamps are calendar-checked (D187)", () => {
    assert.equal(isValidVaultTimestamp("2026-02-31T00:00:00.000Z"), false);
    assert.equal(isValidVaultTimestamp("2027-02-29T00:00:00.000Z"), false);
    assert.equal(isValidVaultTimestamp("2028-02-29T00:00:00.000Z"), true);
    assert.equal(isValidVaultTimestamp("2026-08-22T12:00:00.000Z"), true);
    assert.equal(isValidVaultTimestamp("2026-08-22T12:00:00Z"), false);
  });
  it("a fresh keypair round-trips seal → open and a 1-byte tamper fails closed", async () => {
    ...
    assert.match(sealed.envelope.ct, VAULT_HPKE_CT_RE);
    assert.match(sealed.envelope.enc, VAULT_B64U_32_RE);
    assert.match(sealed.envelope.signature, VAULT_B64U_64_RE);
    ...
    await assertLocalCodeAsync(openKeyEnvelope({ envelope: tampered, recipient, signer_public_key: signer.public_key }), "VAULT_SIGNATURE_INVALID");
    const other = generateEncryptionKeypair();
    await assertLocalCodeAsync(openKeyEnvelope({ envelope: sealed.envelope, recipient: other, signer_public_key: signer.public_key }), "VAULT_ENVELOPE_NOT_FOR_RECIPIENT");
  });
  it("frames round-trip and every header byte is bound", () => {
    ...
    flipped[0] ^= 1;
    assertLocalCode(() => openFrame({ ...base, frame: flipped }), "VAULT_FRAME_INVALID");
    assertLocalCode(() => openFrame({ ...base, frame: flipped, expected_ciphertext_sha256: sealed.ciphertext_sha256 }), "VAULT_RECEIPT_MISMATCH");
    assertLocalCode(() => openFrame({ ...base, object_id: `wal_${"c".repeat(32)}`, frame: sealed.frame }), "VAULT_AEAD_AUTH_FAILURE");
  });
  it("the terminal-loss statement is the reviewed sentence, verbatim", () => {
    assert.equal(VAULT_TERMINAL_LOSS_STATEMENT, "whole-machine or whole-keystore loss is terminal for vault history until human envelopes ship");
  });
});

// ─── Vector replay ───────────────────────────────────────────────────────────

vectors("vault vectors — zip215 (strict RFC 8032, zip215:false)", () => {
  it("replays every zip215 vector", () => {
    for (const v of byClass("zip215")) {
      ...
      if (v.expected.schema_valid === "false") {
        ...
        const obj = v.inputs.object as VaultSignedObject & { creator_signing_pubkey?: string };
        if (v.id === "zip215-004") {
          assert.equal(VAULT_B64U_64_RE.test(obj.signature), false, v.id);
          assert.equal(verifyVaultObject(obj, vectorFile!.test_keys.creator_signing_pubkey!), false, v.id);
        } else {
          assert.equal(VAULT_B64U_32_RE.test(obj.creator_signing_pubkey!), false, v.id);
          assert.equal(verifyVaultObject(obj, obj.creator_signing_pubkey!), false, v.id);
        }
        continue;
      }
...
vectors("vault vectors — hkdf (k_obj / K_digest / keyed commitments)", () => {
...
vectors("vault vectors — golden-preimage (open-id / open-binding)", () => {
...
vectors("vault vectors — stored-bytes-preimage", () => {
  it("replays stored bytes, preimages, domain separation, and the signature-covering hash rule", () => {
    for (const v of byClass("stored-bytes-preimage")) {
      ...
      if (v.id === "stored-001" || v.id === "stored-002") {
        const obj = v.inputs.object as VaultSignedObject;
        ...
        assert.equal(verifyVaultObject(obj, pub), true, `${v.id} signature`);
        if (v.id === "stored-001") {
...
vectors("vault vectors — strict-parse", () => {
  it("replays every strict-parse accept/reject", () => {
    for (const v of byClass("strict-parse")) {
      ...
      if (v.reject_reason === "schema") {
        ...
        const value = parseVaultStrict(text) as Record<string, string>;
        if (v.id === "parse-008") assert.equal(VAULT_SHA256_RE.test(value.base_head_sha256!), false, v.id);
        continue;
      }
      if (v.reject_reason) {
        let reason: string | null = null;
        try {
          parseVaultStrict(text);
        } catch (e) {
          reason = vaultStrictParseReason(e);
        }
        assert.equal(reason, v.reject_reason, `${v.id}: ${v.description}`);
        continue;
      }
      const value = parseVaultStrict(text);
...
vectors("vault vectors — aead-frame (XChaCha20-Poly1305 framing)", () => {
  it("replays the frame seal/open, the 7 AAD flips, and the 5 frame mutations", () => {
    for (const v of byClass("aead-frame")) {
      ...
      if (v.id === "aead-001") {
        ...
        assertLocalCode(() => openFrameWithAad(hexToBytes(v.inputs.k_obj_hex), hexToBytes(v.inputs.frame_hex), utf8(v.inputs.aad_jcs)), "VAULT_AEAD_AUTH_FAILURE");
      } else {
        ...
        assertLocalCode(() => openFrame({ ...base, expected_ciphertext_sha256: v.inputs.receipt_ciphertext_sha256 }), "VAULT_RECEIPT_MISMATCH");
        let code = "";
        try {
          openFrame(base);
        } catch (e) {
        ...
        assert.ok(code === "VAULT_FRAME_INVALID" || code === "VAULT_AEAD_AUTH_FAILURE", `${v.id}: ${code}`);
      }
    }
  });
});

vectors("vault vectors — hpke-rfc9180-a2 (the r402s-1 suite KATs)", () => {
...
vectors("vault vectors — hpke-envelope (key_envelope seal/open + tampers)", () => {
    ...
    for (const v of byClass("hpke-envelope")) {
      ...
      if (v.id === "hpke-envelope-001") {
        ...
        await assertLocalCodeAsync(
          ...
          "VAULT_HPKE_OPEN_FAILED",
        );
      } else if (v.id === "hpke-envelope-014") {
        const env = v.inputs.object as VaultKeyEnvelope;
        assert.equal(verifyVaultObject(env as unknown as VaultSignedObject, v.inputs.signer_pubkey), false);
        const recipient = generateEncryptionKeypair(hexToBytes(v.inputs.recipient_encryption_seed_hex));
        // signature-before-open: the refusal is the SIGNATURE code, so the open was never attempted
        await assertLocalCodeAsync(openKeyEnvelope({ envelope: env, recipient, signer_public_key: v.inputs.signer_pubkey }), "VAULT_SIGNATURE_INVALID");
      } else if (v.id === "hpke-envelope-015") {
        const env = v.inputs.object as VaultKeyEnvelope;
        assert.equal(VAULT_HPKE_CT_RE.test(env.ct), false);
        const recipient = generateEncryptionKeypair(hexToBytes(keys.creator_encryption_seed_hex!));
        await assertLocalCodeAsync(openKeyEnvelope({ envelope: env, recipient, signer_public_key: keys.creator_signing_pubkey! }), "VAULT_SCHEMA_REJECT");
      } else {
...
vectors("vault vectors — hpke-info-near-neighbors (D188)", () => {
...
vectors("vault vectors — hpke-genesis-binding", () => {
  it("schema-valid, correctly signed genesis objects with broken key bindings never canonize", async () => {
    for (const v of byClass("hpke-genesis-binding")) {
      ...
      if (v.id === "hpke-genesis-binding-004") {
        const genesis = v.inputs.genesis as VaultGenesis;
        const env = v.inputs.stored_key_envelope as VaultKeyEnvelope;
        ...
        assert.equal(verifyVaultObject(env as unknown as VaultSignedObject, genesis.creator_signing_pubkey), v.expected.envelope_signature_valid === "true");
        assert.equal(env.created_by === genesis.writer_key_id, v.expected.created_by_matches === "true");
        await assertLocalCodeAsync(openKeyEnvelope({ envelope: env, recipient, signer_public_key: genesis.creator_signing_pubkey }), "VAULT_HPKE_OPEN_FAILED");
        continue;
      }
      const genesis = v.inputs.object as VaultGenesis;
      if (v.expected.signature_valid === "true") assert.equal(verifyVaultObject(genesis as unknown as VaultSignedObject, v.inputs.signer_pubkey), true, v.id);
      const problems = checkGenesisKeyBindings(genesis, v.inputs.stored_key_envelope as VaultKeyEnvelope | undefined);
...
vectors("vault vectors — hpke-recovery (receipt-led recovery)", () => {
  it("receipt → genesis → envelope → open → K_repo → k_obj → the WAL frame decrypts; mismatches are named", async () => {
    for (const v of byClass("hpke-recovery")) {
      ...
      if (v.id === "hpke-recovery-002") {
        const receipt = v.inputs.recovery_receipt as VaultRecoveryReceipt;
        assert.deepEqual(checkRecoveryReceipt(receipt, genesis), []);
        assert.equal(storedBytesSha256(genesis as unknown as VaultSignedObject), v.expected.genesis_sha256);
        assertVaultSignature(genesis as unknown as VaultSignedObject, genesis.creator_signing_pubkey);
        const env = v.inputs.key_envelope as VaultKeyEnvelope;
        assert.deepEqual(checkGenesisKeyBindings(genesis, env), []);
        assert.equal(storedBytesSha256(env as unknown as VaultSignedObject), v.expected.envelope_stored_bytes_sha256);
        const recipient = generateEncryptionKeypair(hexToBytes(v.inputs.recipient_encryption_seed_hex));
      ...
      const receipt = v.inputs.object as VaultRecoveryReceipt;
      const problems = checkRecoveryReceipt(receipt, genesis);
      if (v.id === "hpke-recovery-001") {
        assert.deepEqual(problems, []);
        assert.equal(storedBytesSha256(receipt as unknown as VaultSignedObject), v.expected.stored_bytes_sha256);
...
vectors("vault vectors — chain (the signature/linkage half)", () => {
  it("genesis → head 1 → head 2 stored hashes link; a post-signing byte change no longer verifies", () => {
    for (const v of byClass("chain")) {
      mark(v);
      if (v.id === "chain-004") {
        assert.equal(verifyVaultObject(v.inputs.object as VaultSignedObject, v.inputs.signer_pubkey), false);
        continue;
      }
      if (v.id === "chain-005") continue; // GENERATION_REGRESSION is a pin comparison (task 5.4)
      const genesis = v.inputs.genesis as VaultGenesis;
      const heads = v.inputs.heads as VaultSignedObject[];
      const hashes = [genesis, ...heads].map((o) => storedBytesSha256(o as unknown as VaultSignedObject));
      if (v.expected.stored_bytes_sha256) assert.deepEqual(hashes, v.expected.stored_bytes_sha256, v.id);
      for (const h of heads) assert.equal(verifyVaultObject(h, genesis.creator_signing_pubkey), true, `${v.id} head signature`);
      let ok = true;
      let prev = hashes[0]!;
      let gen = 0;
...
vectors("vault vectors — hpke-interop/golden.json (D182 acceptance 2)", () => {
  it("opens every reference envelope (signature first), reproduces the seal, and refuses every tamper", async () => {
    ...
    for (const c of golden!.cases) {
      ...
      const env = c.sealed_by_reference.key_envelope as VaultKeyEnvelope;
      ...
      assert.equal(storedBytesSha256(env as unknown as VaultSignedObject), c.sealed_by_reference.stored_bytes_sha256);
...
vectors("vault vectors — coverage tally", () => {
  it("every vector of each replayed class was exercised (counts from the file's generated counts_by_class)", () => {
    ...
    console.log(`vault vectors rev ${vectorFile!["x-r402s-revision"]}: ${JSON.stringify(summary)}; interop cases: ${golden?.cases.length ?? 0}`);
  });
});
...
// ── vault-agent-envelopes: keystore possession proof (client half) ────────
import { computeKeystorePossessionProof, VAULT_KEYSTORE_POSSESSION_PREFIX } from "./vault.crypto.js";
...
describe("computeKeystorePossessionProof (vault-agent-envelopes D2)", () => {
  it("reproduces the gateway's HMAC(ECDH(esk, pk), prefix + JCS) exactly, and a different scalar cannot", () => {
    ...
    const expected = _hmac(_sha256, _x25519.getSharedSecret(esk, memberPub), _concat(_utf8(VAULT_KEYSTORE_POSSESSION_PREFIX), _utf8(jcs)));
    ...
    assert.equal(VAULT_KEYSTORE_POSSESSION_PREFIX, "r402s/v0/keystore-key-possession/v1\n");
...
// ── vault-multi-writer: signing-key possession proof (client half, D9) ──
import { VAULT_SIGNING_KEY_POSSESSION_DOMAIN, computeSigningKeyPossessionSignature, signingKeyPossessionMessage } from "./vault.crypto.js";
import { ed25519 as _ed25519 } from "@noble/curves/ed25519.js";

describe("computeSigningKeyPossessionSignature (vault-multi-writer D9)", () => {
  it("reproduces the gateway's JSON.stringify({domain, principal_id, signing_pubkey, encryption_pubkey}) preimage byte-for-byte — field order matters, this is NOT jcs()", () => {
    ...
    const expectedJson = `{"domain":"${VAULT_SIGNING_KEY_POSSESSION_DOMAIN}","principal_id":"${principalId}","signing_pubkey":"${signingPub}","encryption_pubkey":"${encryptionPub}"}`;
    ...
    assert.equal(VAULT_SIGNING_KEY_POSSESSION_DOMAIN, "r402s/v0/signing-key-possession/v1");
