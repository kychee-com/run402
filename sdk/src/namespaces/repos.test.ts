/**
 * `r.repos` namespace tests (add-vault task 5.10).
 *
 * These cover the SEAM, not the protocol: the protocol itself is replayed
 * against the frozen vectors in `vault.crypto.test.ts`,
 * `vault-publication.test.ts`, and `vault-deploy.test.ts`. What is
...
import { VAULT_BYO_OBJECT_MISSING_LIST_CAP, VAULT_CHECKPOINT_ADVISORY_GENERATIONS, computeByoPresenceOutcome, computeOpenProofOutcome, vaultCheckpointStaleness, vaultRemoteUrl, vaultRemoteUrlForRepo, parseVaultRemoteUrl } from "./repos.js";
import type { VaultHandle } from "./repos.js";
import type { VaultOpenReceipt, VaultSignedObject } from "./vault.types.js";
import { VAULT_TERMINAL_LOSS_DOCTOR_TEXT, VAULT_TERMINAL_LOSS_STATEMENT, generateSigningKeypair, sha256Hex, storedBytes, toBase64url } from "./vault.crypto.js";
import { hardenedGit } from "../node/vault-snapshot.js";
import { pinVaultRepo } from "../node/vault-address.js";
import { VaultKeystore } from "../node/vault-keystore.js";
import type { CredentialsProvider } from "../credentials.js";
import { commitFile, makeVault } from "../node/vault-memory-transport.test.js";
import { vaultPaths } from "../node/vault-publication.js";
import { buildVerifierReceipt } from "../node/vault-prune.js";
import { writerKeyIdOf } from "../node/vault-writer-state.js";
 ...
 * (`vaultRemoteAddressForm`'s own discrimination — repo-first-onramp
...
describe("vault remote URLs", () => {
  ...
  it("renders run402:: for both address forms", () => {
    assert.equal(vaultRemoteUrl("org_1", "prj_1"), "run402::org_1/prj_1");
    assert.equal(vaultRemoteUrlForRepo("acme", "notes"), "run402::acme/notes");
  });
});

describe("vault control-plane reads", () => {
  it("get() addresses the vault by :vault_id while the wire object stays repo_id (D185)", async () => {
    ...
    const record = await sdk.repos.get("src_11111111111111111111111111111111");
    assert.equal(calls[0]!.url, "https://api.test/vaults/v1/src_11111111111111111111111111111111");
  ...
  it("forProject() is the cold-restart entry point — an agent with no local state finds its repo_id", async () => {
    const { sdk, calls } = sdkWith(() => ({ body: VAULT_RECORD }));
    const record = await sdk.repos.forProject("prj_demo");
    assert.match(calls[0]!.url, /\/vaults\/v1\?project_id=prj_demo$/);
  ...
  it("heads() sends after_generation AND limit — limit is REQUIRED (rev 40), not optional", async () => {
    ...
    await sdk.repos.heads(VAULT_RECORD.repo_id, { after_generation: "0000000000000000", limit: "100" });
  ...
  it("heads() refuses a request without a valid anchor rather than letting the server guess", async () => {
    ...
    await assert.rejects(() => sdk.repos.heads(VAULT_RECORD.repo_id, { after_generation: "nope", limit: "100" }), /after_generation is REQUIRED/);
    await assert.rejects(() => sdk.repos.heads(VAULT_RECORD.repo_id, { after_generation: "0000000000000000", limit: "0" }), /limit is REQUIRED/);
  ...
  it("allHeads() echoes each page's next_cursor UNCHANGED and keeps the anchor constant", async () => {
    ...
    const out = await sdk.repos.allHeads(VAULT_RECORD.repo_id, { after_generation: "0000000000000000", limit: "1" });
  ...
  it("allHeads() refuses a page that moved the anchor — that is not a continuation", async () => {
    ...
    await assert.rejects(() => sdk.repos.allHeads(VAULT_RECORD.repo_id, { after_generation: "0000000000000000", limit: "1" }), /after_generation anchor/);
  });
  ...
  it("allHeads() reports a GAP as CHAIN_BROKEN rather than silently skipping a generation", async () => {
    ...
    await assert.rejects(() => sdk.repos.allHeads(VAULT_RECORD.repo_id, { after_generation: "0000000000000000", limit: "10" }), (e: unknown) => (e as { code?: string }).code === "CHAIN_BROKEN");
  });
});

describe("vault compaction headroom preflight (vault-compaction-headroom-preflight)", () => {
  /**
  ...
  // block's own unrelated mock, failing VAULT_STRICT_PARSE.
  const KEYSTORE_ROOT = join(tmpdir(), "vault-compaction-headroom-preflight-ks");

  it("reports the arithmetic when the projection fits", async () => {
    const { sdk } = sdkWith(route(withSource("50"), { body: tierStatus(100, 1000) }));
    assert.deepEqual(await sdk.repos.compactHeadroom({ repo_id: REPO }), {
      pool_used_bytes: 100,
  ...
  it("reports ok:false — without throwing — when the standalone read is asked", async () => {
    ...
    const headroom = await sdk.repos.compactHeadroom({ repo_id: REPO });
  ...
  it("the boundary is inclusive — exactly filling the pool still fits", async () => {
    ...
    assert.equal((await sdk.repos.compactHeadroom({ repo_id: REPO }))?.ok, true);
  });
  ...
  it("returns null when the tier-status read fails, rather than guessing (D2 posture 3)", async () => {
    ...
    assert.equal(await sdk.repos.compactHeadroom({ repo_id: REPO }), null);
  });
  ...
  it("returns null when tier status carries no usable pooled figures (shape drift)", async () => {
    for (const body of [{}, { pool_usage: {} }, { pool_usage: { total_storage_bytes: 1, storage_bytes_limit: 0 } }]) {
      ...
      assert.equal(await sdk.repos.compactHeadroom({ repo_id: REPO }), null, JSON.stringify(body));
    }
  });
  ...
  it("refuses a compaction that will not fit BEFORE any upload, naming the arithmetic and the override", async () => {
    ...
    let thrown: unknown;
    try {
      await sdk.repos.compact({ repo_id: REPO, keystore_root: KEYSTORE_ROOT });
    } catch (e) {
      thrown = e;
    }
    const err = thrown as { code?: string; message?: string; details?: Record<string, number | string> };
    assert.equal(err?.code, "VAULT_COMPACT_INSUFFICIENT_HEADROOM");
    ...
    // compaction-grant open+close (vault-checkpoint-cadence — `compact()`
    ...
    assert.deepEqual(
      ...
      [`POST /vaults/v1/${REPO}/compaction-grant`, `DELETE /vaults/v1/${REPO}/compaction-grant`],
...
describe("vault compaction headroom grant (vault-checkpoint-cadence)", () => {
  const REPO = VAULT_RECORD.repo_id;
  const withSource = (bytes: string): Record<string, unknown> => ({ ...VAULT_RECORD, storage: { ...VAULT_RECORD.storage, source_bytes: bytes } });
  // See the sibling describe block above for why compact() needs an
  // explicit, isolated keystore root.
  const KEYSTORE_ROOT = join(tmpdir(), "vault-compaction-headroom-grant-ks");
  function tierStatus(used: number, limit: number): Record<string, unknown> {
    return { tier: "prototype", active: true, pool_usage: { projects: 1, total_api_calls: 0, total_storage_bytes: used, api_calls_limit: 500_000, storage_bytes_limit: limit } };
  }

  it("409 VAULT_COMPACTION_GRANT_ACTIVE refuses BEFORE any preflight read — single-flight-per-vault", async () => {
    const { sdk, calls } = sdkWith((call) => {
      if (call.url.includes("/compaction-grant")) {
        return { status: 409, body: { code: "VAULT_COMPACTION_GRANT_ACTIVE", message: "active", details: { expires_at: "2026-09-01T00:00:00.000Z" } } };
      }
    ...
    assert.equal(err?.code, "VAULT_COMPACTION_IN_PROGRESS");
  ...
  it("a grant whose byte fields are WIRE STRINGS relieves an otherwise-refused preflight (the 2026-08-31 live bug)", async () => {
    ...
    assert.notEqual((thrown as { code?: string } | undefined)?.code, "VAULT_COMPACT_INSUFFICIENT_HEADROOM", `preflight refused despite a covering grant: ${String((thrown as Error | undefined)?.message)}`);
 ...
 *
 * `sdk.repos.open()` always builds the REAL HTTP transport
 * (`createVaultHttpTransport(this.#client)`) — there is no injection
 * point for a different `VaultTransport`. `open` is a plain (non-`#`)
 * instance method, so these tests override it on the `sdk.repos`
 * instance to hand back a handle wrapping a REAL vault built on
 * `VaultMemoryTransport` (the same fixture `vault-prune.test.ts` uses
 * at the `Vault` level) — genuinely correct materialize/chain-walk/
 * attestation behavior, with zero wire-translation risk, while still
 * exercising the actual `namespaces/repos.ts` `prune()` orchestration
 ...
 * itself).
 */
describe("vault prune({ submit }) — receipt idempotency + compaction headroom grant (kychee-com/run402#578)", () => {
  ...
  async function preparedPruneFixture(t: { after: (fn: () => void) => void }) {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-prune-submit-"));
    ...
    sdk.repos.open = async (): Promise<VaultHandle> => ({ repo_id: f.repoId, keystore: f.keystore, transport: f.transport, vault: f.vault });

    const planned = await sdk.repos.prune({ repo_id: f.repoId });
    assert.ok(planned.intent_core && planned.attestation, "fixture must produce a real, submittable prune plan");
    return { f, sdk, planned };
  }

  /** A `r402s-verify`-shaped receipt over the SAME plan, signed by the vault's own owner key (matches `vault-prune.test.ts`'s convention). */
   ...
   * matching `services/vault/upload-sessions.ts`'s actual
  ...
  it("fix 2: a receipt refused as reused, with IDENTICAL stored bytes, is treated as already-uploaded — the submit still completes", async (t) => {
    ...
    const bytes = storedBytes(verifierReceipt as unknown as VaultSignedObject);
    await f.transport.uploadObjects({
      repo_id: f.repoId,
      objects: [{ path: vaultPaths.verifierReceipt(FIXED_ID), object_kind: "verifier_receipt", object_id: FIXED_ID, bytes, sha256: sha256Hex(bytes), size_bytes: String(bytes.length) }],
    });
    makeUploadStrictAboutReuse(f);

    const result = await sdk.repos.prune({ repo_id: f.repoId, submit: { core: planned.intent_core!, verifier_receipt: verifierReceipt } });
  ...
  it("fix 2: a reused id whose stored bytes DIFFER is refused by name, with both hashes — never silently proceeds", async (t) => {
    ...
    const staleBytes = storedBytes(staleReceipt as unknown as VaultSignedObject);
    await f.transport.uploadObjects({
      repo_id: f.repoId,
      objects: [{ path: vaultPaths.verifierReceipt(FIXED_ID), object_kind: "verifier_receipt", object_id: FIXED_ID, bytes: staleBytes, sha256: sha256Hex(staleBytes), size_bytes: String(staleBytes.length) }],
    });
    makeUploadStrictAboutReuse(f);

    const verifierReceipt = verifierReceiptFor(f, planned, { object_id: FIXED_ID });
    await assert.rejects(
      sdk.repos.prune({ repo_id: f.repoId, submit: { core: planned.intent_core!, verifier_receipt: verifierReceipt } }),
      (e: unknown) => {
        const err = e as { code?: string; details?: { object_id?: string; existing_sha256?: string; this_attempt_sha256?: string } };
        assert.equal(err.code, "VAULT_RECEIPT_ID_REUSED_DIFFERENT");
  ...
  it("fix 3: closes the grant even when the submit itself fails after the receipts uploaded", async (t) => {
    ...
    await assert.rejects(sdk.repos.prune({ repo_id: f.repoId, submit: { core: planned.intent_core!, verifier_receipt: verifierReceipt } }));
    assert.deepEqual(grantCalls, ["open", "close"], "the grant must close in the `finally` even though the submit threw");
  });

  it("fix 3: a 409 VAULT_COMPACTION_GRANT_ACTIVE surfaces as the single-flight conflict BEFORE any receipt upload is attempted", async (t) => {
    const { f, sdk, planned } = await preparedPruneFixture(t);
    const verifierReceipt = verifierReceiptFor(f, planned);
    f.transport.openCompactionGrant = async () => {
      throw Object.assign(new Error("a compaction grant is already active for this project"), {
        isRun402Error: true,
        code: "VAULT_COMPACTION_GRANT_ACTIVE",
    ...
    await assert.rejects(
      sdk.repos.prune({ repo_id: f.repoId, submit: { core: planned.intent_core!, verifier_receipt: verifierReceipt } }),
      (e: unknown) => {
        assert.equal((e as { code?: string }).code, "VAULT_COMPACTION_IN_PROGRESS");
        return true;
      },
    );
...
describe("vault owner writes", () => {
  it("setPolicy() PATCHes the policy route and carries the reason", async () => {
    const { sdk, calls } = sdkWith(() => ({ body: { vault_policy: "grandfathered", vault_policy_version: "2", changed: true, warnings: [] } }));
    const out = await sdk.repos.setPolicy(VAULT_RECORD.repo_id, { vault_policy: "grandfathered", reason: "migrating an existing project" });
    assert.equal(calls[0]!.method, "PATCH");
    assert.match(calls[0]!.url, /\/policy$/);
    assert.deepEqual(calls[0]!.body, { vault_policy: "grandfathered", reason: "migrating an existing project" });
  ...
  it("completeOverride() posts the full capture receipt — a partial match must never clear an advisory", async () => {
    ...
    const out = await sdk.repos.completeOverride(VAULT_RECORD.repo_id, { operation_id: "op_1", capture_receipt: receipt as any });
...
describe("vault secret handling", () => {
  it("the maintenance lease is NEVER memoised — holder_token is issued once, so a second call must reach the server", async () => {
    ...
    const first = await sdk.repos.acquireMaintenanceLease({ repo_id: VAULT_RECORD.repo_id, base_head_sha256: "a".repeat(64), r1_size_bytes: "0", r2_cap_size_bytes: "0" });
    const second = await sdk.repos.acquireMaintenanceLease({ repo_id: VAULT_RECORD.repo_id, base_head_sha256: "a".repeat(64), r1_size_bytes: "0", r2_cap_size_bytes: "0" });
  ...
  it("sends the p_before_* accounting fields with explicit zeros rather than omitting them", async () => {
    ...
    await sdk.repos.acquireMaintenanceLease({ repo_id: VAULT_RECORD.repo_id, base_head_sha256: "a".repeat(64), r1_size_bytes: "0", r2_cap_size_bytes: "0" });
...
describe("vault status — the terminal-loss statement is normative copy", () => {
  it("states the V0 terminal-loss sentence VERBATIM, and never implies the receipt can decrypt", async () => {
    ...
    const status = await sdk.repos.status({ project_id: "prj_demo", keystore_root: join(tmpdir(), "vault-absent-keystore-fixture") });
    assert.equal(status.terminal_loss_statement, VAULT_TERMINAL_LOSS_STATEMENT);
    ...
    assert.equal(status.terminal_loss_detail, VAULT_TERMINAL_LOSS_DOCTOR_TEXT);
  ...
  it("stays truthful for a VAULT-ONLY project: never a deploy-related warning for the absence of deploys (D183)", async () => {
    ...
    assert.equal(status.vault_policy, "required");
  });

  it("raises a persistent warning while the policy is grandfathered", async () => {
    const { sdk } = sdkWith(() => ({ body: { ...VAULT_RECORD, vault_policy: "grandfathered" } }));
    const status = await sdk.repos.status({ project_id: "prj_demo", keystore_root: join(tmpdir(), "vault-absent-keystore-fixture") });
  ...
  // crosses. See repos.ts's `vaultLossWarningTrip` for the OR-composite.
  ...
  it("does NOT mint key material — observing a vault must never create the identity it reports on", async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), "vault-status-noalloc-"));
    rmSync(root, { recursive: true, force: true });
    const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
    const status = await sdk.repos.status({ project_id: "prj_demo", keystore_root: root });
  ...
  it("reports no vault (rather than throwing) when the project has none, and says what to do next", async () => {
    const { sdk } = sdkWith(() => ({ status: 404, body: { error: { code: "RESOURCE_NOT_FOUND", message: "no vault for this project" } } }));
    const status = await sdk.repos.status({ project_id: "prj_none", keystore_root: join(tmpdir(), "vault-absent-keystore-fixture") });
  ...
  it("says where the keystore lives, so 'whole-keystore loss is terminal' is actionable", async () => {
    const root = join(tmpdir(), "vault-keystore-paths-fixture");
    const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
    const status = await sdk.repos.status({ project_id: "prj_demo", keystore_root: root });
  ...
  it("leaves the ref map null unless it is asked for — status is an observation, not a verification", async () => {
    const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
    const status = await sdk.repos.status({ project_id: "prj_demo", keystore_root: join(tmpdir(), "vault-refs-off-fixture") });
  ...
  describe("status() remote detection checks both conventional names (kychee-com/run402#559c)", () => {
    ...
    async function freshRepoWithRemote(name: string, url: string): Promise<string> {
      root = mkdtempSync(join(tmpdir(), "run402-vault-status-remote-"));
    ...
    it("reports an 'origin' remote (the common case) — not just one literally named 'run402'", async () => {
      const url = vaultRemoteUrl(VAULT_TEST_ID_ORG, "prj_demo");
      const dir = await freshRepoWithRemote("origin", url);
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks") });
    ...
    it("still reports a 'run402'-named remote when that is what exists (the pre-fix behavior, preserved)", async () => {
      ...
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks2") });
    ...
    it("prefers 'run402' over 'origin' when BOTH exist (D1's own naming precedence)", async () => {
      const runUrl = vaultRemoteUrl(VAULT_TEST_ID_ORG, "prj_demo");
      ...
      try {
        ...
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks3") });
        assert.equal(status.remote!.name, "run402");
    ...
    it("an 'origin' that is NOT run402-form is skipped in favor of a run402-form remote under the other name", async () => {
      ...
      try {
        ...
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks4") });
        assert.equal(status.remote!.name, "run402");
        assert.equal(status.remote!.url, url);
      } finally {
        rmSync(root, { recursive: true, force: true });
    ...
    it("reports matches: false when the remote names a DIFFERENT project", async () => {
      const url = vaultRemoteUrl(VAULT_TEST_ID_ORG, "prj_other");
      const dir = await freshRepoWithRemote("origin", url);
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks5") });
        assert.equal(status.remote!.name, "origin");
    ...
    it("neither name present: remote stays null, same as before", async () => {
      ...
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-ks6") });
  ...
  describe("status() remote.matches is a tri-state for slug-form remotes (kychee-com/run402#562)", () => {
    const SLUG_URL = vaultRemoteUrlForRepo("acme", "my-notes");
    let root: string;

    async function freshRepoWithSlugRemote(): Promise<string> {
      root = mkdtempSync(join(tmpdir(), "run402-vault-status-remote-slug-"));
      const dir = join(root, "repo");
      await hardenedGit(root, ["init", "-q", "-b", "main", "repo"]);
    ...
    it("pin present + equal → matches: true", async () => {
      ...
      try {
        await pinVaultRepo(dir, VAULT_RECORD.repo_id, { org_slug: "acme", repo_name: "my-notes" });
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ repo_id: VAULT_RECORD.repo_id, repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-slug-ks1") });
    ...
    it("pin present + DIFFERENT → matches: false", async () => {
      const dir = await freshRepoWithSlugRemote();
      try {
        await pinVaultRepo(dir, "src_99999999999999999999999999999999", { org_slug: "acme", repo_name: "my-notes" });
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ repo_id: VAULT_RECORD.repo_id, repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-slug-ks2") });
        assert.equal(status.remote!.matches, false);
        assert.equal(status.remote!.reason, null);
      } finally {
        rmSync(root, { recursive: true, force: true });
    ...
    it("no pin yet → matches: null, with a reason — never a mismatch", async () => {
      ...
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ repo_id: VAULT_RECORD.repo_id, repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-slug-ks3") });
    ...
    it("an id-form remote is entirely unaffected — its own URL text is still the whole comparison, no pin involved", async () => {
      root = mkdtempSync(join(tmpdir(), "run402-vault-status-remote-idform-"));
      const dir = join(root, "repo");
      await hardenedGit(root, ["init", "-q", "-b", "main", "repo"]);
      const idUrl = vaultRemoteUrl(VAULT_TEST_ID_ORG, "prj_demo");
      await hardenedGit(dir, ["remote", "add", "origin", idUrl]);
      try {
        const { sdk } = sdkWith(() => ({ body: VAULT_RECORD }));
        const status = await sdk.repos.status({ project_id: "prj_demo", repo_dir: dir, keystore_root: join(tmpdir(), "vault-status-remote-idform-ks") });
        assert.equal(status.remote!.matches, true);
 ...
 * `desired[]` yet.
 */
describe("vault access() — envelope_state + stale_access from desired[]", () => {
  const REPO = VAULT_RECORD.repo_id;
  ...
  function routeAccess(directoryKeys: unknown[], coverageBody: Record<string, unknown>) {
    return (call: Call) => {
      ...
      if (url.pathname === `/vaults/v1/${REPO}`) return { body: VAULT_RECORD };
      if (url.pathname === `/orgs/v1/${ORG}/encryption-keys`) return { body: { org_id: ORG, keys: directoryKeys } };
      if (url.pathname === `/vaults/v1/${REPO}/envelope-recipients`) return { body: coverageBody };
  ...
  it("reports real per-recipient envelope_state and stale_access when the gateway ships desired[]", async () => {
    ...
    const result = await sdk.repos.access({ repo_id: REPO });
  ...
  it("breaks this keystore's own fingerprint out of unmatched_covered_fingerprints (dogfood #4)", async () => {
    const root = mkdtempSync(join(tmpdir(), "vault-access-this-keystore-"));
    try {
      const identity = new VaultKeystore({ rootDir: root }).ensureIdentity();
      ...
      const result = await sdk.repos.access({ repo_id: REPO, keystore_root: root });

      // vault-agent-envelopes: this_keystore is reported for every principal
  ...
  it("this_keystore is null when this machine holds no local identity for the vault", async () => {
    const root = mkdtempSync(join(tmpdir(), "vault-access-no-keystore-"));
    try {
      const { sdk } = sdkWith(
        routeAccess(
          [{ principal_id: "prn_alice", display_name: "Alice", ek_fingerprint: "ek_alice" }],
  ...
  it("never wraps a key — access() issues only GET requests", async () => {
    ...
    await sdk.repos.access({ repo_id: REPO });
...
describe("listByOrg follows the keyset cursor (never a silent page one)", () => {
  it("aggregates every page and returns has_more:false at the end", async () => {
    ...
    const out = await sdk.repos.listByOrg("57035b1e-ec41-4ce6-a7a5-a5b2560efdd7");
 ...
 *
 *  1. `Repos.submitProofOfOpen` — the wire call itself (exact body sent,
 ...
 *     against the REAL `VaultMemoryTransport` fixture in
 *     `vault-epoch-reader.test.ts`/`vault-rotate.test.ts`; what this
 *     file owns is the auto-submission WIRING `fsck()` layers on top).
 */
describe("vault D210 proof-of-open submission (rev 44)", () => {
  it("Repos.submitProofOfOpen POSTs the evidence VERBATIM to the self-match route and reports deduplicated:false on a fresh 201 mint", async () => {
    const { sdk, calls } = sdkWith((call) => {
      return {
        status: 201,
        body: {
    ...
    const out = await sdk.repos.submitProofOfOpen("src_test", "prn_alice", {
    ...
    assert.match(calls[0]!.url, /\/vaults\/v1\/src_test\/recipients\/prn_alice\/proof-of-open$/);
    assert.deepEqual(calls[0]!.body, {
      ek_fingerprint: "ek_alice",
      chain_verified_to_generation: "0000000000000002",
      decryptable_to_generation: "0000000000000003",
...
describe("computeOpenProofOutcome — fsck's D210 auto-submission decision logic (rev 44)", () => {
  ...
  const stubReceipt = { object_id: "ror_" + "c".repeat(32) } as unknown as VaultOpenReceipt;
  ...
  it("principal resolution returning null (e.g. a delegate-authenticated caller GET /agent/v1/whoami cannot resolve) is REPORTED, never thrown, and submit is never called", async () => {
    ...
    assert.equal(out.error?.code, "VAULT_PROOF_OF_OPEN_PRINCIPAL_UNRESOLVED");
    assert.equal(submitCalled, false);
  });
...
// ─── vault-byo-primary-bucket task 3.3: fsck's BYO presence check ─────────

describe("computeByoPresenceOutcome — repos fsck's BYO absence-adjudication decision logic (vault-byo-primary-bucket task 3.3)", () => {
  ...
  it("a BYO vault with a missing object THROWS VAULT_BYO_OBJECT_MISSING naming repo_id/destination/checked_count/missing_count/missing, never a silently-embedded finding", async () => {
    ...
    await assert.rejects(
      ...
      (e: unknown) => {
        assert.equal((e as { code?: string }).code, "VAULT_BYO_OBJECT_MISSING");
  ...
  it("a missing-object list beyond the cap is truncated in `missing`, but `missing_count` still names the TRUE total and `missing_truncated` says so", async () => {
    const total = VAULT_BYO_OBJECT_MISSING_LIST_CAP + 7;
    ...
    await assert.rejects(
      ...
      (e: unknown) => {
        ...
        assert.equal((details.missing as unknown[]).length, VAULT_BYO_OBJECT_MISSING_LIST_CAP);
...
// ─── vault-clone-scaling (P3): checkpoint-staleness helper ────────────────

describe("vaultCheckpointStaleness — pure, never-throwing, threshold in one place", () => {
  const staleness = (newest: string, covered: string | null) =>
    vaultCheckpointStaleness({ newest_generation: newest, covers_through_generation: covered });
  ...
  it("the threshold boundary is >= (24 quiet, 25 advises) — the one constant to tune", () => {
    assert.equal(VAULT_CHECKPOINT_ADVISORY_GENERATIONS, 25);
...
/**
 * `Repos.reconcile()` (vault-multi-writer task 5.7/5.10) — the public
 * namespace entry point had no dedicated test of its own: `vault-
 * publication.test.ts` covers `Vault.reconcileWriterAdmissions()`
 ...
 * suite above already established — a REAL vault on `VaultMemoryTransport`,
 * `sdk.repos.open` handed back a wrapping handle, fetch never reached.
 */
describe("Repos.reconcile() — the public namespace entry point (vault-multi-writer task 5.7/5.10)", () => {
  it("admits an eligible pending_writers[] candidate end to end, through open() + the namespace wrapper", async (t) => {
    const root = mkdtempSync(join(tmpdir(), "run402-vault-reconcile-ns-"));
    t.after(() => rmSync(root, { recursive: true, force: true }));
    const f = await makeVault(root);
    ...
    const { sdk } = sdkWith(() => ({ body: {} })); // fetch is never reached — open() is overridden below
    sdk.repos.open = async (): Promise<VaultHandle> => ({ repo_id: f.repoId, keystore: f.keystore, transport: f.transport, vault: f.vault, restored: null, reconcile_recipients: null, writer_reconcile: null, enrollment: { outcome: "skipped_error", ek_fingerprint: null, signing_fingerprint: null } });

    const result = await sdk.repos.reconcile({ repo_id: f.repoId });
