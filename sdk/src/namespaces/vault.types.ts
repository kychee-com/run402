/**
 * vault — `r402s/v0` wire types (protocol rev 40).
 *
 * Every object here mirrors a frozen JSON Schema under the private repo's
 * `docs/strategy/products/kygit/schemas/` (`key_envelope.json`,
...
export type VaultFormat = "r402s/v0";
/** `suite` const on every `r402s/v0` object. */
export type VaultSuite = "r402s-1";
...
export type VaultObjectKind =
  ...
  /** vault-multi-writer rev 47 (protocol §4.17) — a writer's signed grant authorizing a handoff recipient to become a vault writer. */
  | "writer_admission_grant";

/** Kinds whose stored bytes are an AEAD frame (protocol §2 framing), keyed by `k_obj`. */
export type VaultEncryptedObjectKind =
  | "wal_pack"
  | "ref_state"
  | "retention_roots"
  | "checkpoint_manifest"
...
export interface VaultKeyEnvelopeReceipt {
  object_kind: "key_envelope";
  epoch: string;
  recipient_fingerprint: string;
  ...
  size_bytes: string;
  /** D195, rev 42 — see {@link VaultKeyEnvelope.rotation_id}. */
  rotation_id?: string;
}

/** One `{principal_id, envelope}` pair in `rotate_epoch_payload.envelopes[]` / an N-recipient `vault_genesis.envelopes[]` (D196, rev 42). */
export interface VaultRotationEnvelopePair {
  principal_id: string;
  envelope: VaultKeyEnvelopeReceipt;
}
...
export interface VaultGenesis {
   ...
   */
  envelopes: VaultKeyEnvelopeReceipt[];
   ...
   * still builds.
   */
  pin_manifest?: VaultPinManifestReceipt;
  created_at: string;
  signature: string;
}
...
export interface VaultRecoveryReceipt {
  format: VaultFormat;
...
export interface VaultCaptureReceipt {
  format: VaultFormat;
...
export interface VaultWalPackReceipt {
  object_id: string;
...
export interface VaultRefStateReceipt {
  object_id: string;
...
export interface VaultRetentionRootsReceipt {
  object_id: string;
...
export interface VaultCaptureBinding {
  capture_id: string;
  apply_plan_sha256: string | null;
  snapshot_oid_hmac: string;
}
...
export interface VaultRepairDescriptor {
  base_generation: string;
...
export interface VaultTransitionEnvelope {
...
export interface VaultCheckpointManifestReceipt {
  object_id: string;
...
export interface VaultCheckpointPackReceipt {
  object_id: string;
...
export interface VaultCheckpointClaimSetReceipt {
  object_id: string;
...
export interface VaultRetentionCutoffReceipt {
  object_id: string;
...
export interface VaultRetentionRoot {
  ref: string;
  oid: string;
...
export interface VaultRetentionRoots {
  format: VaultFormat;
  object_kind: "retention_roots";
  suite: VaultSuite;
  repo_id: string;
...
export interface VaultCheckpointManifestPack {
  object_id: string;
...
export interface VaultCheckpointManifest {
  ...
  covers_through_generation: string;
  git_object_format: "sha1";
  packs: VaultCheckpointManifestPack[];
...
export interface VaultCheckpointClaimSet {
  format: VaultFormat;
  object_kind: "checkpoint_claim_set";
  suite: VaultSuite;
  repo_id: string;
  /** `ccs_` + 32 hex. */
  object_id: string;
  manifest_receipt: VaultCheckpointManifestReceipt;
  ordered_pack_receipts: VaultCheckpointPackReceipt[];
...
export interface VaultRetentionCutoff {
  format: VaultFormat;
  object_kind: "retention_cutoff";
  suite: VaultSuite;
  repo_id: string;
...
export interface VaultActivationToken {
  format: VaultFormat;
...
export interface VaultRefUpdate {
  ref: string;
...
export interface VaultRefTransaction {
  /** 1..1000 updates naming pairwise-distinct refs. */
  updates: VaultRefUpdate[];
}

/** The query of `GET /vaults/v1/:vault_id/heads` (schema `heads_listing_request.json`, D186). */
export interface VaultHeadsListingRequest {
...
export interface VaultHeadsListingEntry {
  generation: string;
  stored_bytes_sha256: string;
}

/** The ONE frozen response of the heads listing (schema `heads_listing_page.json`). */
export interface VaultHeadsListingPage {
  format: VaultFormat;
  repo_id: string;
  after_generation: string;
  heads: VaultHeadsListingEntry[];
  has_more: boolean;
...
export interface VaultOverrideCompletionRequest {
  operation_id: string;
  capture_receipt: VaultCaptureReceipt;
}
...
export interface VaultHead {
   ...
   */
  epoch: string;
  wal_entries: VaultWalPackReceipt[];
  ref_state: VaultRefStateReceipt;
  retention_roots: VaultRetentionRootsReceipt;
  checkpoint: VaultCheckpointBlock | null;
  checkpoint_purpose: "ordinary_push" | "maintenance_cycle" | "repair" | null;
  capture_binding: VaultCaptureBinding | null;
  repair: VaultRepairDescriptor | null;
  transition: VaultTransitionEnvelope | null;
  /**
...
export interface VaultPinManifestReceipt {
...
export interface VaultSignedObject {
...
export interface VaultFrameAad {
  repo_id: string;
...
export interface VaultSealedFrame {
...
export interface VaultSealedKeyEnvelope {
...
export interface VaultSigningKeypair {
...
export interface VaultEncryptionKeypair {
...
export type VaultStrictParseReason =
  | "json-number"
  | "duplicate-member"
  | "invalid-json"
 ...
 * elective. `writer_key_revoked` (D6/D227, vault-multi-writer rev 47,
 ...
 */
export type VaultRotationReason = "member_removed" | "recipient_key_revoked" | "epoch_secret_exposed" | "elective_rekey" | "writer_key_revoked";
 ...
 */
export interface VaultRotationAttemptDescriptor {
  format: VaultFormat;
  ...
  /**
   * D227 (rev 47, vault-multi-writer, task 5.9, NEW fields).
 ...
 * ({@link import("./vault.crypto.js").openEpochRotationForRecipient})
 ...
 */
export interface VaultEpochRotationSelfOpen {
  /**
 ...
 */
export interface VaultRotateEpochPayload {
  new_epoch: string;
  ...
  envelopes: VaultRotationEnvelopePair[];
  /**
   ...
   * import("../node/vault-publication.js").Vault.rotateEpoch}
   * always populates it on every submitted rotation.
   */
  self_open_attestation?: VaultEpochRotationSelfOpen;
  /**
   * D227 (rev 47, vault-multi-writer, NEW field). SCHEMA-OPTIONAL, present
   ...
   * `WriterSetUpdatePayload` (services/vault/writer-state.ts) exactly.
   */
  writer_set_update?: {
    base_version: string;
    base_sha256: string;
    ...
    next_sha256: string;
    /**
     * A NARROWER 3-value vocabulary than {@link VaultRotationReason}
 ...
 * vault-multi-writer rev 47, protocol §4.16/§4.17) — the JCS bytes
 ...
 * the gateway's own `AddWriterKeyPayload` (services/vault/writer-state.ts)
 * and this SDK's `vault-writer-state.ts` port of it — this is the WIRE
 * type the parser below decodes into that same shape.
 */
export interface VaultAddWriterKeyPayload {
...
export interface VaultRecipientPinManifestEntry {
  principal_id: string;
 ...
 */
export interface VaultRecipientPinManifest {
  format: VaultFormat;
  object_kind: "recipient_pin_manifest";
  suite: VaultSuite;
  repo_id: string;
 ...
 */
export interface VaultRecipientConfirmationReceipt {
  format: VaultFormat;
 ...
 * 44, `ror_`) — the EVIDENCE mirror of {@link VaultRecipientConfirmationReceipt}'s
 ...
 */
export interface VaultOpenReceipt {
  format: VaultFormat;
