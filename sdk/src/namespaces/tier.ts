...
export interface TierSetResult {
  ...
  /** The organization's allowance after this purchase. */
  allowance_remaining_usd_micros: number;
  /**
   * What settled the purchase. `allowance` means the organization's allowance
   * paid, ahead of the paywall: no 402, no signed authorization, and no USDC
   * needed in the wallet.
   */
  paid_with?: "allowance" | "x402" | "mpp";
  /** Allowance consumed by this purchase (0 unless `paid_with` is `allowance`). */
  allowance_used_usd_micros?: number;
}
