/**
 * `r.repos.resume` — gateway error propagation, and the raw Handoff Key's
 ...
 *
 * Run: node --test --import tsx sdk/src/namespaces/repos-resume-errors.test.ts
 */
...
// own round-trip is covered by vault-handoff.test.ts; this file only
...
describe("r.repos.resume — gateway claim refusals propagate untouched", () => {
  const cases = [
  ...
  for (const c of cases) {
    it(`${c.code} (${c.status}) surfaces as an ApiError carrying that exact code`, async () => {
      ...
      await assert.rejects(
        r.repos.resume({ key: FABRICATED_KEY }),
        (err: unknown) => {
      ...
      assert.match(calls[0]!.url, /\/vaults\/v1\/handoffs\/.+\/redeem$/);
    });
  }
});

describe("r.repos.resume — the raw key never appears in a thrown error (design: key-once contract)", () => {
  it("a claim refusal's error never carries the raw Handoff Key, its master secret, or its derived auth_secret", async () => {
    ...
    let caught: unknown;
    try {
      await r.repos.resume({ key: FABRICATED_KEY });
  ...
  it("a kgi1_ invite key handed to resume() is refused by name, before any network call (kygit-invite design D9)", async () => {
    ...
    await assert.rejects(
      r.repos.resume({ key: inviteLookingKey }),
      (err: unknown) => {
  ...
  it("a malformed key's parse refusal names no part of the input", async () => {
    ...
    let caught: unknown;
    try {
      await r.repos.resume({ key: malformed });
