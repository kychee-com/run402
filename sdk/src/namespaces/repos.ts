/**
 * `r.repos` — the host-blind encrypted Git remote, as the SDK's programmatic
 * API (add-vault task 5.10).
 *
 ...
 * ONCE here. `run402 repos …`, `git-remote-run402`, and the MCP tools are
...
import {
  ...
  VAULT_DEGRADED_READ_STATEMENT,
  VAULT_DURABILITY_STATEMENT,
  VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT,
  VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT,
  VAULT_TERMINAL_LOSS_DOCTOR_TEXT,
  VAULT_TERMINAL_LOSS_STATEMENT,
  VAULT_UNMIRRORED_FINDING_STATEMENT,
  bytesToHex,
...
/**
 * vault-agent-envelopes — per-process memo for the session-start
...
import type { VaultCaptureReceipt, VaultHeadsListingPage, VaultHeadsListingRequest, VaultHeadTarget, VaultOpenReceipt, VaultRecipientConfirmationReceipt, VaultRecoveryReceipt, VaultRotationReason } from "./vault.types.js";
import type {
  VaultCompactionGrant,
  VaultEnvelopeRecipientsResponse,
  VaultMaintenanceLease,
  VaultMaintenanceLeaseRequest,
  VaultTransport,
  VaultRecord,
} from "../node/vault-publication.js";
import type { VaultDeployOptions, VaultDeployResult } from "../node/vault-deploy.js";
import type { VaultPublishResult, VaultPushOptions, VaultRefMap, VaultReconcileEnvelopeRecipientsResult, VaultReconcileWriterAdmissionsResult, VaultVerifiedState } from "../node/vault-publication.js";
import type { VaultCreationResult } from "../node/vault-creation-journal.js";
import type { VaultKeystore } from "../node/vault-keystore.js";
import type { VaultSnapshot } from "../node/vault-snapshot.js";
import type { VaultMirrorConfig, VaultMirrorCredential, VaultMirrorDestination } from "../node/vault-mirror-config.js";
import type { VaultByoMissingObject, VaultByoPresenceReport, VaultMirrorPushResult, VaultMirrorSyncSummary } from "../node/vault-mirror.js";
import type { VaultRecoverResult, VaultVerifyReport } from "../node/vault-recover.js";
import type { VaultMemberRecoveryBundle } from "../node/vault-member-bundle.js";
import type { VaultDegradedReadLive, VaultDegradedReadOutcome, VaultDegradedReadSource } from "../node/vault-degraded-read.js";
...
type PublicationModule = typeof import("../node/vault-publication.js");
type DeployModule = typeof import("../node/vault-deploy.js");
type KeystoreModule = typeof import("../node/vault-keystore.js");
type CreationModule = typeof import("../node/vault-creation-journal.js");
type SnapshotModule = typeof import("../node/vault-snapshot.js");
type PruneModule = typeof import("../node/vault-prune.js");
type OpenOrCreateModule = typeof import("../node/vault-open-or-create.js");
type AddressModule = typeof import("../node/vault-address.js");
type MirrorModule = typeof import("../node/vault-mirror.js");
type MirrorConfigModule = typeof import("../node/vault-mirror-config.js");
type MirrorBackendModule = typeof import("../node/vault-mirror-backend.js");
type RecoverModule = typeof import("../node/vault-recover.js");
type DegradedReadModule = typeof import("../node/vault-degraded-read.js");
type ByoConfigModule = typeof import("../node/vault-byo-config.js");
type ByoProbeModule = typeof import("../node/vault-byo-probe.js");
type HandoffModule = typeof import("../node/vault-handoff.js");
type RestoreModule = typeof import("../node/vault-restore.js");
type WriterStateModule = typeof import("../node/vault-writer-state.js");
...
async function nodeOnly<T>(load: () => Promise<T>, verb: string): Promise<T> {
  try {
    ...
    throw new LocalError(
      `\`r.repos.${verb}\` needs the Node runtime (keystore + git); import it from a Node process, or use the read-only vault methods in a browser.`,
      `running vault ${verb}`,
      { code: "VAULT_NODE_ONLY", details: { cause: e instanceof Error ? e.message : String(e) } },
    );
  }
}
 ...
 * cannot be gateway-assigned the way `internal.vault_claims.id` alone
...
/** What `r.repos.status()` reports. Never carries key material. */
export interface VaultStatus {
  repo_id: string | null;
  project_id: string | null;
  /** The control plane's view. `null` when no vault is allocated for the project. */
  vault: VaultRecord | null;
  /** vault-agent-envelopes D5: what this KEY-HOLDER's session-start fulfilment did (`null` on a machine that holds no K_repo, or `reconcile: "forbidden"`). */
  reconcile_recipients?: VaultSessionReconcileResult | null;
  /** vault-agent-envelopes D3: what the enroll-if-absent step did for this keystore's key (`null` when it did not run). */
  enrollment?: VaultEnrollmentOutcome | null;
  keystore: {
    present: boolean;
   ...
   * `resolveVaultAddress`'s doc comment), so a checkout on one always
  ...
  head_target: VaultHeadTarget | null;
  pins: {
   ...
   * same envelope-recipients read `Repos.access` uses), taken inside
   ...
   */
  terminal_loss_statement: typeof VAULT_TERMINAL_LOSS_STATEMENT | null;
  /** `null` exactly when `terminal_loss_statement` is — same condition, same reasoning. */
  terminal_loss_detail: typeof VAULT_TERMINAL_LOSS_DOCTOR_TEXT | null;
  /**
   ...
   */
  durability_statement: typeof VAULT_DURABILITY_STATEMENT | null;
  warnings: { kind: string; message: string }[];
  next_actions: { action: string; command?: string }[];
}

/**
 * What {@link Repos.scaffoldRemote} did, and why (D1). `name` is `origin`
 ...
 */
export interface VaultScaffoldRemoteResult {
...
export interface VaultInitResult {
  ...
  recovery_receipt: VaultCreationResult["recovery_receipt"];
  genesis_sha256: string;
  /** The git remote that was added, when a working tree was scaffolded. */
  remote: VaultScaffoldRemoteResult | null;
  deduplicated: boolean;
  terminal_loss_statement: typeof VAULT_TERMINAL_LOSS_STATEMENT;
  /**
   * vault-byo-primary-bucket task 3.1/3.5 — the AUTHORITATIVE profile
...
export interface VaultCompactResult {
  generation: string;
  head_sha256: string;
  form: "wal" | "checkpoint";
   ...
   * (vault-compaction-headroom-preflight). `null` when the preflight could
   * not be answered — see {@link VaultCompactHeadroom} for why an
   * unanswerable check never blocks maintenance.
   */
  headroom: VaultCompactHeadroom | null;
}
 ...
 */
export interface VaultCompactHeadroom {
  ...
  overridden: boolean;
  /**
   * vault-checkpoint-cadence design D3: `pool_limit_bytes` PLUS an active
...
/**
 * `run402 repos capture --dry-run`'s report shape
 * — {@link Repos.planCapture}'s return type. Every sizing field is `null`,
 ...
 */
export interface VaultSnapshotPushPlan {
...
export interface VaultPruneObjectCandidate {
  object_id: string;
  object_kind: string;
  size_bytes: string;
}

export interface VaultPruneResult {
  /** Retention ROOTS and their windows — the retention view, unchanged. */
  candidates: VaultPruneCandidate[];
   ...
   */
  object_candidates: VaultPruneObjectCandidate[];
   ...
   */
  intent_core: import("../node/vault-prune.js").VaultPruneIntentCore | null;
  intent_core_sha256: string | null;
  /** What this SDK observed while restoring the latest checkpoint — the receipt's evidence. */
  attestation: import("../node/vault-publication.js").VaultStoredCheckpointAttestation | null;
  ...
  intent: import("../node/vault-prune.js").VaultPruneIntentRecord | null;
  /**
   ...
   */
  confirmation: import("../node/vault-prune.js").VaultPruneConfirmation | null;
  note: string;
}
...
export interface VaultHandleOptions {
  ...
  project_id?: string;
  /**
   * vault-agent-envelopes D5 — the session-start envelope fulfilment a
   * KEY-HOLDING client runs on its first ordinary vault operation in a
  ...
  repo_dir?: string;
  /** Keystore root override (defaults to `~/.config/run402/vault`; `~/.config/run402/profiles/<wallet>/vault` under a named wallet). */
...
export interface VaultHandle {
  ...
  vault: import("../node/vault-publication.js").Vault;
  /**
   * vault-agent-envelopes D4: non-null when this open restored the repo
   ...
   */
  restored: import("../node/vault-publication.js").VaultColdOpenResult | null;
  /**
   * vault-agent-envelopes D5: the session-start fulfilment outcome for a
  ...
  reconcile_recipients: VaultSessionReconcileResult | null;
  /**
   * vault-multi-writer (task 5.7) — the session-start writer-admission
   ...
   * import("../node/vault-publication.js").Vault.
   ...
   * writer.
   */
  writer_reconcile: VaultReconcileWriterAdmissionsPushResult | null;
  /** vault-agent-envelopes D3: what the enroll-if-absent step did for this keystore's key on this open. */
  enrollment: VaultEnrollmentOutcome;
}

/** The session-start reconcile's outcome (vault-agent-envelopes D5). */
export interface VaultSessionReconcileResult {
  attempted: boolean;
  outcome: "reconciled" | "skipped_error" | "deferred_by_local_policy" | "forbidden" | "custody_unverified";
  result?: VaultReconcileEnvelopeRecipientsResult;
...
export interface VaultEnrollmentOutcome {
  ...
  /**
   * vault-multi-writer rev 47 (task 5.3) — the keystore's vault-WRITER
   ...
   * `VaultIdentityFile.signing_seed_hex`'s own doc comment) or when the
...
/** {@link Repos.capture}'s best-effort envelope-recipient reconcile outcome, reported beside (never folded into) the vault result — same non-blocking contract as {@link VaultMirrorPushResult}. */
export interface VaultReconcileEnvelopeRecipientsPushResult {
  attempted: boolean;
...
/**
 * vault-multi-writer (task 5.7) — the writer-admission twin of {@link
 * VaultReconcileEnvelopeRecipientsPushResult}: best-effort, reported
 ...
 * richer {@link VaultSessionReconcileResult}, `"forbidden"` is included
 ...
 * see {@link VaultHandle.writer_reconcile}) honors only that one value;
 * it is simply unreachable at the push/snapshot/deploy sites, which never
 * check `options.reconcile` for this hook.
 */
export interface VaultReconcileWriterAdmissionsPushResult {
  attempted: boolean;
  outcome: "reconciled" | "skipped_error" | "forbidden";
  result?: VaultReconcileWriterAdmissionsResult;
  error?: string;
}

// ─── Handoff / resume result shapes (kygit-handoff design D10) ──────────────

/** {@link Repos.handoff}'s result. `handoff_key` (the assembled `kgh1_…`) is returned exactly ONCE. */
export interface VaultHandoffMintResult {
  handoff_key: string;
  handoff_id: string;
  kind: "handoff";
  minted_role: string;
  ...
  snapshot: import("../node/vault-snapshot.js").VaultHandoffSnapshot;
  warnings: { code: string; message: string }[];
  next_actions: NextAction[];
}

export interface VaultHandoffListEntry {
  handoff_id: string;
  kind: string;
...
export interface VaultHandoffListResult {
  handoffs: VaultHandoffListEntry[];
}

/** {@link Repos.resume}'s result. */
export interface VaultHandoffResumeResult {
  handoff_id: string;
  kind: "handoff";
  deduplicated: boolean;
  /** The Handoff Note, parsed — `null` when the commit message could not be read/parsed (still restored either way). */
  note: import("../node/vault-handoff.js").KygitHandoffNote | null;
  ...
  members: unknown[];
  expires_at: string;
  /** vault-multi-writer rev 47 (task 5.6, design D5) — this checkout's own writer activation. `outcome: "active"` covers BOTH a fresh submission this call made and the idempotent-skip case (a prior attempt's activation already landed) — the writer IS active either way. */
  writer_activation: { outcome: "active"; writer_key_id: string; generation: string };
  reconcile_recipients: VaultReconcileEnvelopeRecipientsPushResult;
  next_actions: NextAction[];
}
 ...
 * BOTH the handoff mint (`POST /vaults/v1/:vault_id/handoffs`) and
 * claim (`POST /vaults/v1/handoffs/:handoff_id/redeem`) responses. The SDK
 ...
 */
export function handoffVaultFromWire(wire: { repo_id: string; org_id: string; project_id: string }, address: string | null = null): VaultHandoffMintResult["vault"] {
...
export function handoffMembershipFromWire(wire: { org_id: string; role: string; status: string }): VaultHandoffResumeResult["membership"] {
...
export interface VaultInviteRoom {
  organization_id: string;
  room_key: string;
}

/** Best-effort report of whether {@link Repos.invite}'s room message was posted — a failure here (e.g. the daily quota) never voids the mint (design D4). */
export interface VaultInviteRoomMessageResult {
  posted: boolean;
  message_id?: string;
...
export interface VaultInvitePresenceResult {
  registered: boolean;
  presence_id?: string;
  name?: string;
  error?: string;
...
/** {@link Repos.invite}'s result. `invite_key` (the assembled `kgi1_…`) is returned exactly ONCE. */
export interface VaultInviteMintResult {
  invite_key: string;
  invite_id: string;
  kind: "invite";
  minted_role: string;
  ...
  /** The full local capture result, for a caller that wants more than the summarized `capture` block. */
  snapshot: import("../node/vault-snapshot.js").VaultHandoffSnapshot;
  inviter_presence: VaultInvitePresenceResult;
  room_message: VaultInviteRoomMessageResult;
  warnings: { code: string; message: string }[];
  next_actions: NextAction[];
}

export interface VaultInviteListEntry {
  invite_id: string;
  kind: string;
  state: "issued" | "redeemed" | "expired" | "revoked";
  minted_role: string;
...
export interface VaultInviteListResult {
  invites: VaultInviteListEntry[];
}

/** The inviter's presence, resolved live at redeem time (design vault-invite's own claim requirement) — `null` when the inviter never registered one. */
export interface VaultInviteInviter {
  presence_id: string;
  name: string;
  program: string | null;
  model: string | null;
...
/** {@link Repos.join}'s result. */
export interface VaultInviteJoinResult {
  invite_id: string;
  kind: "invite";
  deduplicated: boolean;
  /** The Invite Note, parsed — `null` when the commit message could not be read/parsed (still restored either way). */
  note: import("../node/vault-handoff.js").KygitInviteNote | null;
  /** The note's raw commit-message text, for a caller that wants Markdown rendering over the parsed shape. */
  note_raw: string | null;
  restored: { dir: string; branch: string; base_head_oid: string; stash_oid: string };
  membership: { organization_id: string; role: string; status: string };
  members: unknown[];
  room: VaultInviteRoom;
  inviter: VaultInviteInviter | null;
  ...
  expires_at: string;
  /**
   * vault-multi-writer rev 47 (kygit-invite design D5/D9) — this
   * checkout's own writer activation, driven through the SAME shared
   * `add_writer_key` door {@link Repos.resume} uses, BEFORE `join()`
...
/** What {@link Repos.openOrCreate} did. `created` is `null` exactly when `found` is `true`. */
...
export interface VaultMirrorStatus {
  repo_id: string;
  configured: boolean;
  ...
  /** vault-mirror-default: when a mirror write/sync last completed with zero failures (local config fact — never transmitted). `null` when unconfigured or never succeeded. */
  last_success_at: string | null;
  /** vault-mirror-default: the standing `vault_unmirrored` finding — informational, never blocking; `null` once a mirror write or sync has succeeded (see {@link vaultUnmirroredFinding}). */
  finding: VaultUnmirroredFinding | null;
  validity_not_freshness: typeof VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT;
  keystore_still_required: typeof VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT;
}

/**
 * vault-mirror-default — the named standing finding for a vault with no
 * customer-held mirror copy yet. vault-byo-primary-bucket task 3.5
 * widens `message` to the BYO remedy wording ({@link
 * VAULT_BYO_UNMIRRORED_REMEDY_STATEMENT}) for a BYO vault — it applies
 ...
 */
export interface VaultUnmirroredFinding {
  kind: "vault_unmirrored";
  message: typeof VAULT_UNMIRRORED_FINDING_STATEMENT | typeof VAULT_BYO_UNMIRRORED_REMEDY_STATEMENT;
...
/**
 * vault-byo-primary-bucket task 3.3 — the number of `{key, object_kind}`
 * entries `VAULT_BYO_OBJECT_MISSING`'s `details.missing` lists before
 ...
 */
export const VAULT_BYO_OBJECT_MISSING_LIST_CAP = 50;

/**
 * vault-byo-primary-bucket task 3.3 (design D6) — `repos fsck`'s wiring of
 * the shipped {@link import("../node/vault-mirror.js").verifyByoObjectsPresent}
 * read-half primitive. Present on {@link VaultFsckResult.byo_presence}
 ...
 * throws `VAULT_BYO_OBJECT_MISSING` instead of returning here (see
 * {@link Repos.fsck}'s own doc comment) — this shape only ever reports
 * the two honest non-failure outcomes, "checked, all present" and "could
 * not check".
 */
export interface VaultFsckByoPresence {
  /**
 ...
 */
export interface VaultFsckResult {
  repo_id: string;
   ...
   * `VAULT_AEAD_AUTH_FAILURE`). `refs`/`head_target`/`pin_after.highest_materialized`
  ...
  epoch_decrypt_failure: import("../node/vault-publication.js").VaultEpochDecryptFailure | null;
   ...
   * never a warning).
   */
  retained_refs: import("../node/vault-publication.js").VaultRetainedRefsReconcileResult | null;
  /**
   ...
   */
  open_proof: VaultOpenProofOutcome;
  /**
   * vault-byo-primary-bucket task 3.3 — see {@link VaultFsckByoPresence}.
   ...
   * doc comment for why that case throws `VAULT_BYO_OBJECT_MISSING`
   * instead.
   */
  byo_presence?: VaultFsckByoPresence;
}

/** {@link VaultFsckResult.open_proof} — the outcome of `fsck`'s best-effort D210 proof-of-open submission. */
export interface VaultOpenProofOutcome {
 ...
 * proof-of-open submission, factored out of {@link Repos}'s private
 ...
 * /agent/v1/whoami`, {@link Repos.submitProofOfOpen}) — this function
...
/**
 * vault-byo-primary-bucket task 3.3 (design D6) — the decision function
 * behind `fsck`'s BYO presence check, factored out of {@link Repos}'s
 ...
 * credential-less returning agent (see {@link VaultFsckByoPresence}'s
 * own doc comment). A confirmed absence is the opposite: it THROWS
 * `VAULT_BYO_OBJECT_MISSING` — the same severity class as {@link
 * Repos.mirrorVerify}'s own `VAULT_MIRROR_NOT_CONFIGURED` throw when
 ...
 * {@link VAULT_BYO_OBJECT_MISSING_LIST_CAP} (a chain-referenced object
...
export async function computeByoPresenceOutcome(input: {
  ...
  verifyPresence: () => Promise<VaultByoPresenceReport>;
}): Promise<VaultFsckByoPresence | undefined> {
  ...
  if (report.missing.length > 0) {
    const missing: VaultByoMissingObject[] = report.missing.slice(0, VAULT_BYO_OBJECT_MISSING_LIST_CAP);
    throw new LocalError(
 ...
 * vault protocol v0 pins a single fixed epoch for a vault's entire
 ...
 * mid-fold in `vault-human-envelopes` under adversarial protocol review.
 *
 ...
 */
export interface VaultAccessRecipient {
  principal_id: string;
...
/** A principal whose membership was removed (desired state `pending_removal`) but who STILL holds a covering `key_envelope` on this vault — real, continuing access that removal did not revoke, because vault v0 has no epoch-rotation mechanism yet. See {@link VaultAccessResult}'s doc comment. */
export interface VaultStaleAccessEntry {
  principal_id: string;
  display_name: string | null;
  fingerprint: string;
}
export interface VaultAccessResult {
  repo_id: string;
  org_id: string | null;
  /** vault-agent-envelopes D5: this KEY-HOLDER's session-start fulfilment outcome (`null` when this machine holds no K_repo for the vault, or `reconcile: "forbidden"`). */
  reconcile_recipients?: VaultSessionReconcileResult | null;
  /** vault-agent-envelopes D3: the enroll-if-absent step's outcome (`null` when it did not run). */
  enrollment?: VaultEnrollmentOutcome | null;
  recipients: VaultAccessRecipient[];
  ...
  this_keystore: {
    fingerprint: string;
    /** `true` iff the directory holds this keystore's key as ACTIVE (vault-agent-envelopes: agents enroll too). */
    enrolled: boolean;
    /** `absent` = no published key (the next vault operation enrolls); `pending` = published, possession unproven; `rotation_required` = the principal's published key is a DIFFERENT key; `unknown` = whoami unavailable. */
  ...
  /** Removed members who still decrypt this vault — see {@link VaultStaleAccessEntry}. Always `[]` when `envelope_state_available` is `false` (no desired-state substrate to compute it from). */
  stale_access: VaultStaleAccessEntry[];
 ...
 */
export interface VaultOrgVaultSummary {
  repo_id: string;
  project_id: string;
  ...
  created_at: string;
  /** vault-byo-primary-bucket task 3.5 — absent-or-`"managed"` is byte-identical to today. */
  storage_profile?: "managed" | "byo";
}
export interface VaultOrgVaultsListing {
  vaults: VaultOrgVaultSummary[];
  /** Keyset pagination (agent-response-design): opaque, store-and-echo. */
  has_more?: boolean;
  next_cursor?: string | null;
}

export interface VaultOpenOrCreateResult {
  handle: VaultHandle;
  /** `true` when the vault already existed — nothing was allocated by this call. */
  found: boolean;
  created: {
    /** Mirrors `VaultInitResult.deduplicated`: an existing local creation journal was resumed to completion rather than started fresh — nothing was re-minted. */
    deduplicated: boolean;
...
/**
 * {@link Repos.deploy}'s post-push glue: attach the best-effort
 ...
 */
export async function attachVaultDeployHooks(
  result: VaultDeployResult,
  mirror: () => Promise<VaultMirrorPushResult>,
  reconcile: () => Promise<VaultReconcileEnvelopeRecipientsPushResult>,
): Promise<VaultDeployResult & { mirror_push?: VaultMirrorPushResult; reconcile_recipients?: VaultReconcileEnvelopeRecipientsPushResult }> {
  if (result.outcome !== "DEPLOYED_AND_VAULTED" && result.outcome !== "DEPLOY_FAILED_VAULTED") return result;
  // Sequential, not Promise.all — same ordering capture() uses, so a reconcile
...
export class Repos {
  ...
  async get(repoId: string): Promise<VaultRecord> {
    return this.#client.request<VaultRecord>(`/vaults/v1/${encodeURIComponent(repoId)}`, { context: "reading the vault record" });
  }
  ...
  async forProject(projectId: string): Promise<VaultRecord> {
    return this.#client.request<VaultRecord>(`/vaults/v1?project_id=${encodeURIComponent(projectId)}`, { context: "resolving the project's vault" });
  }

  /**
   * Every vault the organization owns, one round trip — `repos list`'s bulk
   * read. See {@link
   * VaultOrgVaultsListing}'s doc comment for the FROZEN response shape and
   * the 404-until-shipped fallback contract.
   */
  async listByOrg(orgId: string): Promise<VaultOrgVaultsListing> {
    ...
    const all: VaultOrgVaultSummary[] = [];
    let cursor: string | null | undefined;
    for (let page = 0; page < 100; page++) {
      const url = `/vaults/v1?org_id=${encodeURIComponent(orgId)}` + (cursor ? `&cursor=${encodeURIComponent(cursor)}` : "");
      const res = await this.#client.request<VaultOrgVaultsListing>(url, { context: "listing the organization's vaults" });
      all.push(...res.vaults);
   ...
   * `GET /vaults/v1?repo=<org-slug>/<name>`.
   * `RESOURCE_NOT_FOUND` for no such org OR no such name (deliberately
   * collapsed — see the design's slug-namespace-probing note);
   * `SLUG_RELEASED` (read it with {@link vaultSlugReleasedInfo}) while the
   * slug is in its post-rename cooldown — never auto-followed.
   */
  async forRepo(address: { org_slug: string; repo_name: string }): Promise<VaultRecord> {
    const repo = `${address.org_slug}/${address.repo_name}`;
    return this.#client.request<VaultRecord>(`/vaults/v1?repo=${encodeURIComponent(repo)}`, { context: "resolving the vault by repo address" });
  }

  /**
   * Resolve a parsed remote address (`parseVaultRemoteUrl`'s output),
   ...
   */
  async resolveAddress(address: VaultRemoteAddress): Promise<VaultRecord> {
    return vaultRemoteAddressForm(address) === "id"
   ...
   */
  async heads(repoId: string, request: VaultHeadsListingRequest): Promise<VaultHeadsListingPage> {
    ...
    return this.#client.request<VaultHeadsListingPage>(`/vaults/v1/${encodeURIComponent(repoId)}/heads?${qs.toString()}`, { context: "listing vault heads" });
  }
   ...
   */
  async allHeads(repoId: string, options: { after_generation: string; limit?: string }): Promise<{ heads: VaultHeadsListingPage["heads"]; pages: number; total: string | null }> {
    const { verifyHeadsListingPage, nextListingRequest } = await this.#publication();
    let request: VaultHeadsListingRequest | null = { after_generation: options.after_generation, limit: options.limit ?? "1000" };
    let progress = { after_generation: options.after_generation, last_generation: options.after_generation, delivered: 0 };
    const heads: VaultHeadsListingPage["heads"] = [];
    let pages = 0;
    let total: string | null = null;
    while (request) {
      const page: VaultHeadsListingPage = await this.heads(repoId, request);
   ...
   * `required`.
   */
  async setPolicy(repoId: string, input: { vault_policy: "required" | "grandfathered"; reason?: string }): Promise<{ vault_policy: string; vault_policy_version: string; changed: boolean; warnings: { kind: string; message: string }[] }> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/policy`, {
      method: "PATCH",
      body: { vault_policy: input.vault_policy, ...(input.reason !== undefined ? { reason: input.reason } : {}) },
      context: "setting the vault activation policy",
    });
  }
   ...
   * clears it.
   */
  async completeOverride(repoId: string, input: { operation_id: string; capture_receipt: VaultCaptureReceipt }): Promise<{ operation_id: string; advisory_cleared: boolean; generation: string; head_sha256: string }> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/override-completions`, { method: "POST", body: input, context: "submitting the vault override completion" });
  }
  ...
  // core and lives on `Vault` (`sdk/src/node/vault-publication.ts`);
  ...
  async confirmRecipient(repoId: string, principalId: string, newFingerprint: string): Promise<VaultRecipientConfirmationReceipt> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/recipients/${encodeURIComponent(principalId)}/confirm`, {
      method: "POST", body: { new_fingerprint: newFingerprint }, context: "confirming a vault recipient's first pin",
    });
  }

  /** `POST …/recipients/:principal_id/repin` (D197) — re-pin ceremony. */
  async repinRecipient(repoId: string, principalId: string, input: { old_ek_fingerprint: string; new_fingerprint: string }): Promise<VaultRecipientConfirmationReceipt> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/recipients/${encodeURIComponent(principalId)}/repin`, {
      method: "POST", body: input, context: "re-pinning a vault recipient",
    });
  }

  /**
   * vault-agent-envelopes D3 — a KEY-HOLDER explicitly accepts a recipient's
   ...
   */
  async acceptRecipientKeyChange(options: VaultHandleOptions & { principal_id: string; new_fingerprint: string }): Promise<{ repo_id: string; principal_id: string; old_fingerprint: string | null; new_fingerprint: string; receipt: VaultRecipientConfirmationReceipt | null }> {
    ...
    if (!repo) throw new LocalError("this keystore holds no repo state for the vault — only a key-holder can accept a recipient's key change", "accepting a recipient key change", { code: "VAULT_REPO_STATE_MISSING", details: { repo_id: repoId } });
    ...
    let receipt: VaultRecipientConfirmationReceipt | null = null;
  ...
  async declareRecipientKeyRevoked(repoId: string, principalId: string): Promise<{ recipient_state_version: string; recipient_revocation_version: string }> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/recipients/${encodeURIComponent(principalId)}/key-revocation`, {
      method: "POST", body: {}, context: "declaring a vault recipient key revoked",
    });
  }
  ...
  async declareEpochSecretExposed(repoId: string): Promise<{ epoch_secret_exposure_version: string }> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/epoch-secret-exposure`, { method: "POST", body: {}, context: "declaring a vault epoch secret exposed" });
  }
  ...
  async declareWriterAuthorityUnavailable(repoId: string): Promise<{ declared_at: string; declared_by: string | null }> {
    return this.#client.request(`/vaults/v1/${encodeURIComponent(repoId)}/writer-authority/declare-unavailable`, { method: "POST", body: {}, context: "declaring vault writer authority unavailable" });
  }
   ...
   * mismatch is the ordinary 403 `VAULT_ACCESS_DENIED`. Idempotent on
  ...
  async submitProofOfOpen(
    ...
    const res = await this.#client.requestWithResponse<VaultOpenReceipt>(
      `/vaults/v1/${encodeURIComponent(repoId)}/recipients/${encodeURIComponent(principalId)}/proof-of-open`,
      { method: "POST", body: evidence, context: "submitting a vault proof-of-open receipt" },
    );
    return { receipt: res.body, deduplicated: res.status === 200 };
  }

  async acquireMaintenanceLease(request: VaultMaintenanceLeaseRequest): Promise<VaultMaintenanceLease> {
    const { repo_id, ...body } = request;
    return this.#client.request<VaultMaintenanceLease>(`/vaults/v1/${encodeURIComponent(repo_id)}/maintenance-leases`, {
      method: "POST",
      body: {
   ...
   */
  async open(options: VaultHandleOptions = {}): Promise<VaultHandle> {
    const [{ Vault, createVaultHttpTransport }, { VaultKeystore }] = await Promise.all([this.#publication(), this.#keystore()]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    // vault-object-host-predial (design D1, task 1.2): this is the ONE
    ...
    const transport = createVaultHttpTransport(this.#client, {
    ...
    // vault-agent-envelopes D3: enroll the keystore's key if this principal
    ...
    // VAULT_ENVELOPE_PENDING with the exact next actions). The lazy
    ...
    // (`VAULT_REPO_STATE_MISSING` at first use). EVERYTHING else —
    ...
    let restored: import("../node/vault-publication.js").VaultColdOpenResult | null = null;
    try {
    ...
    let reconcileRecipients: VaultSessionReconcileResult | null = null;
    if (policy === "forbidden") {
    ...
    // vault-multi-writer (task 5.7) — the SAME "session-start"/"read"
  ...
  /**
   * vault-agent-envelopes D3 — enroll-if-absent, checked FRESH on every
   ...
   *   - current key ≠ this keystore's key → `VAULT_KEY_ROTATION_REQUIRED`.
   ...
   */
  async #ensureEnrolled(keystore: VaultKeystore): Promise<VaultEnrollmentOutcome> {
    ...
    try {
      ...
      // (and a cold open will say `VAULT_ENVELOPE_PENDING` honestly if it
    ...
    if (identity?.enrolled_principal_id && identity.enrolled_principal_id !== whoami.principal.id) {
      throw new LocalError(
        ...
        { code: "VAULT_IDENTITY_PROFILE_MISMATCH", details: { enrolled_principal_id: identity.enrolled_principal_id, principal_id: whoami.principal.id, keystore_root: keystore.rootDir } },
      );
    }
    ...
    if (!identity) {
      throw new LocalError(
        ...
        "enrolling the keystore encryption key",
        {
          code: "VAULT_KEY_ROTATION_REQUIRED",
          ...
          next_actions: [
            { type: "edit_request", why: "restore the keystore backup that holds the current key (~/.config/run402/vault or the wallet profile's vault dir)" },
            { type: "edit_request", why: "have an org owner revoke the stale key: DELETE /orgs/v1/:org_id/members/:principal_id/encryption-key (owner + step-up); your next vault operation then enrolls a fresh key" },
    ...
    if (current && current.ek_fingerprint !== identity.encryption_fingerprint) {
      throw new LocalError(
        ...
        "enrolling the keystore encryption key",
        {
          ...
          next_actions: [
            { type: "edit_request", why: "restore the keystore backup that holds the current key (~/.config/run402/vault or the wallet profile's vault dir)" },
            { type: "edit_request", why: "have an org owner revoke the stale key: DELETE /orgs/v1/:org_id/members/:principal_id/encryption-key (owner + step-up); your next vault operation then enrolls this key" },
            { type: "edit_request", why: "if this principal is the sole custody-eligible member of every org it belongs to: POST /agent/v1/whoami/encryption-key with replace_current: true" },
          ],
        },
      );
    }
    // vault-multi-writer rev 47 (task 5.3, design D9) — the signing half's
    ...
    if (!keypair) {
      throw new LocalError("the keystore identity has no X25519 private key — it cannot enroll or open envelopes", "enrolling the keystore encryption key", { code: "VAULT_UNRECOVERABLE", details: { statement: VAULT_TERMINAL_LOSS_STATEMENT } });
    }
   ...
   * (`run402 repos capture`/`snapshot`) drive so that a push against an
   ...
   */
  async openOrCreate(options: VaultHandleOptions & { org_id?: string; client_creation_id?: string }): Promise<VaultOpenOrCreateResult> {
    if (!options.repo_id && !options.project_id) {
      throw new LocalError("pass repo_id, or project_id to resolve it from the control plane", "opening the vault", { code: "VAULT_UNRESOLVED" });
    }
    ...
    if (options.repo_id) {
      ...
      return { handle, found: true, created: null, terminal_loss_statement: VAULT_TERMINAL_LOSS_STATEMENT };
    }

    const [{ createVaultHttpTransport }, { VaultKeystore }, { openOrCreateVault }] = await Promise.all([this.#publication(), this.#keystore(), this.#openOrCreate()]);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const transport = createVaultHttpTransport(this.#client, { byoBackend: await this.#byoBackendResolver(keystore) });
    ...
    return result.found
      ? { handle, found: true, created: null, terminal_loss_statement: VAULT_TERMINAL_LOSS_STATEMENT }
      : { handle, found: false, created: result.created, terminal_loss_statement: VAULT_TERMINAL_LOSS_STATEMENT };
  }
   ...
   * (vault-client-round-trips design D4 widens the original slug-form-only
   ...
   * it. This is what the remote helper and `vault snapshot` drive for a
   ...
   * with {@link vaultSlugReleasedInfo} for the successor slug and cooldown.
   */
  async resolveOrCreateAddress(
    options: VaultHandleOptions & {
      address: VaultRemoteAddress;
      ...
      onVaultCreated?: (created: NonNullable<import("../node/vault-address.js").VaultAddressResolution["created"]>) => void | Promise<void>;
    },
  ): Promise<VaultOpenOrCreateResult & { resolution: import("../node/vault-address.js").VaultAddressResolution }> {
    const [{ createVaultHttpTransport }, { VaultKeystore }, { resolveVaultAddress }] = await Promise.all([this.#publication(), this.#keystore(), this.#address()]);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const transport = createVaultHttpTransport(this.#client, { byoBackend: await this.#byoBackendResolver(keystore) });
    const resolution = await resolveVaultAddress({
      keystore,
      transport,
    ...
    return {
      ...
      terminal_loss_statement: VAULT_TERMINAL_LOSS_STATEMENT,
      resolution,
    };
  }
   ...
   * (vault-force-spelling-and-pin-fold): an id-carrying pin resolves with
  ...
  async recoverStalePin(
    options: VaultHandleOptions & {
      address: VaultRemoteAddress;
      repo_dir: string;
      resolution: import("../node/vault-address.js").VaultAddressResolution;
      error: unknown;
    },
  ): Promise<(VaultOpenOrCreateResult & { resolution: import("../node/vault-address.js").VaultAddressResolution }) | null> {
    const [{ createVaultHttpTransport }, { VaultKeystore }, { recoverStaleVaultPin }] = await Promise.all([this.#publication(), this.#keystore(), this.#address()]);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const transport = createVaultHttpTransport(this.#client, { byoBackend: await this.#byoBackendResolver(keystore) });
    const fresh = await recoverStaleVaultPin({
      keystore,
      transport,
      address: options.address,
  ...
  async init(options: {
     ...
     * another one (see {@link Repos.scaffoldRemote}'s `nested`). Default
    ...
    service_public_key?: Uint8Array | string;
    /**
     * vault-byo-primary-bucket task 3.1/3.2/3.5 — request a BYO vault.
     ...
     * request — a failed probe throws `VAULT_BYO_BUCKET_PROBE_FAILED`
     * and nothing is created.
     */
    byo?: { destination_url: string; credential?: import("../node/vault-mirror-config.js").VaultMirrorCredential; region?: string; endpoint?: string };
  }): Promise<VaultInitResult> {
    const [{ createVaultHttpTransport }, { VaultKeystore }, { createVault }] = await Promise.all([this.#publication(), this.#keystore(), this.#creation()]);
    const { parseMirrorDestinationUrl, formatMirrorDestination } = await this.#mirrorConfig();
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const transport = createVaultHttpTransport(this.#client, { byoBackend: await this.#byoBackendResolver(keystore) });
    let byoWriteTarget: { destination: import("../node/vault-mirror-config.js").VaultMirrorDestination; credential?: import("../node/vault-mirror-config.js").VaultMirrorCredential } | undefined;
    ...
    if (options.byo) {
      ...
      // vault-byo-primary-bucket task 3.1 (design D6) — probe BEFORE any
      // allocation request; a failed probe throws and nothing is created.
      const { probeVaultByoDestination } = await this.#byoProbe();
      await probeVaultByoDestination(destination, options.byo.credential);
    }
    const created: VaultCreationResult = await createVault({
      keystore,
      transport,
    ...
    let remote: VaultInitResult["remote"] = null;
    ...
    // vault-byo-primary-bucket task 3.1/3.5 — read back the AUTHORITATIVE
   ...
   */
  async scaffoldRemote(options: { repo_dir: string; org_id: string; project_id: string; remote_name?: string; remote_url?: string; nested?: boolean }): Promise<VaultScaffoldRemoteResult> {
    const { hardenedGit } = await this.#snapshot();
    const url = options.remote_url ?? vaultRemoteUrl(options.org_id, options.project_id);
    ...
    let nestedFields: Pick<VaultScaffoldRemoteResult, "nested" | "enclosing_toplevel" | "excluded_in_enclosing"> = {};
   ...
   * in `vault-restore.ts`). Located via `rev-parse --git-path
   ...
   */
  async status(options: VaultHandleOptions & {
    /**
    ...
    let pending: import("../node/vault-deploy.js").VaultOverrideJournal[] = [];
    if (repoId && keystorePresent) {
      try {
    ...
    if (repoId && record) {
      try {
        const coverage = await this.#client.request<VaultEnvelopeRecipientsResponse>(
          `/vaults/v1/${encodeURIComponent(repoId)}/envelope-recipients`,
          { context: "reading the vault envelope recipients" },
        );
    ...
    if (record?.vault_policy === "grandfathered") {
    ...
    if (record) {
      const trip = vaultLossWarningTrip(record);
      if (vaultLossWarningTripped(trip)) {
        if (isMultiPrincipal) {
          warnings.push({ kind: "keystore_backup_reminder", message: vaultKeystoreBackupReminderMessage(trip, coveringRecipients!) });
        } else {
          warnings.push({ kind: "terminal_loss_risk", message: vaultLossWarningMessage(trip) });
        }
      }
    }
    ...
    let pinned: VaultStatus["pinned"] = null;
    // kygit-invite design D9: whether `.git/info/exclude` already carries
    // `.run402/` — a pure read, computed alongside the pin.
    let messagingCacheExcluded: VaultStatus["messaging_cache_excluded"] = null;
    if (options.repo_dir) {
      const { readPinnedVaultRepo } = await this.#address();
      const p = await readPinnedVaultRepo(options.repo_dir);
    ...
    let remote: VaultStatus["remote"] = null;
    if (options.repo_dir) {
      const { hardenedGit } = await this.#snapshot();
      ...
      for (const name of ["run402", "origin"]) {
        ...
        if (!url) continue;
        let parsed: VaultRemoteAddress | null;
        try {
          parsed = parseVaultRemoteUrl(url);
        } catch {
          continue; // another scheme's remote — not a vault remote
        }
        if (!parsed) continue;
        if (vaultRemoteAddressForm(parsed) === "id") {
    ...
    let refs: VaultStatus["refs"] = null;
    let headTarget: VaultStatus["head_target"] = null;
    ...
    // vault-agent-envelopes D3/D5: a KEY-HOLDER's ordinary read is a
    ...
    let reconcileRecipients: VaultSessionReconcileResult | null = null;
    let enrollment: VaultEnrollmentOutcome | null = null;
    ...
    const nextActions: VaultStatus["next_actions"] = [];
    ...
    else if (pending.length > 0) nextActions.push({ action: `complete ${pending.length} unvaulted-override journal(s)`, command: "run402 repos capture" });
    ...
    return {
      ...
      vault_policy: record?.vault_policy ?? null,
      pending_overrides: pending.length,
      covering_recipients: coveringRecipients,
      terminal_loss_statement: isMultiPrincipal ? null : VAULT_TERMINAL_LOSS_STATEMENT,
      terminal_loss_detail: isMultiPrincipal ? null : VAULT_TERMINAL_LOSS_DOCTOR_TEXT,
      durability_statement: isMultiPrincipal ? VAULT_DURABILITY_STATEMENT : null,
      warnings,
   ...
   * vault-client-surface, "Faithful helper"; `run402 repos capture` is
  ...
  async capture(
    options: VaultHandleOptions & {
       ...
       * project's vault does not exist yet, capture() runs the six-stage
       * creation journal inline (via {@link openOrCreate}) before capturing
       * and publishing — one command, no prior `vault init`. Without it,
       * capture() behaves exactly as it always did: a push against an
       * unallocated vault throws the resolution failure unchanged.
       */
      org_id?: string;
      /**
       * A parsed remote address (`parseVaultRemoteUrl`'s output).
       ...
       * helper, for callers (`vault snapshot`) that resolve the target
       ...
       */
      onVaultCreated?: (created: NonNullable<VaultOpenOrCreateResult["created"]>) => void | Promise<void>;
      /**
       ...
       */
      snapshot?: Omit<import("../node/vault-snapshot.js").VaultSnapshotOptions, "dir">;
      onCommitLine?: (line: string) => void;
      checkpoint?: boolean;
    },
  ): Promise<VaultPublishResult & { snapshot: VaultSnapshot; vault_commit: string; vault_commit_line: string; mirror_push: VaultMirrorPushResult; byo_chain_copy: VaultMirrorPushResult; reconcile_recipients: VaultReconcileEnvelopeRecipientsPushResult; writer_reconcile: VaultReconcileWriterAdmissionsPushResult }> {
    const [{ deployRefTransaction }, { captureSnapshot, vaultCommitLine }] = await Promise.all([this.#publication(), this.#snapshot()]);
    ...
    const line = vaultCommitLine(snapshot);
    ...
    // vault-byo-primary-bucket task 3.3 — the SAME non-blocking contract,
    ...
    // `capture()` throw, same non-blocking contract as the mirror hook above.
    const reconcileRecipients = await this.#tryReconcileEnvelopeRecipients(handle.vault);
    // vault-multi-writer (task 5.7) — the SAME best-effort contract,
    ...
    return { ...result, snapshot, vault_commit: snapshot.oid, vault_commit_line: line, mirror_push: mirrorPush, byo_chain_copy: byoChainCopy, reconcile_recipients: reconcileRecipients, writer_reconcile: writerReconcile };
  }
   ...
   * push it (retained, on no branch — {@link VAULT_DEPLOY_REF} carries
   * it exactly like an ordinary `capture()`, never `refs/heads/*`), seal the
  ...
  async handoff(
    options: VaultHandleOptions & {
      ...
      role?: string;
      ttlSeconds?: number;
      note: Omit<import("../node/vault-handoff.js").KygitHandoffNote, "capture">;
    ...
    if (!repoFile) {
      throw new LocalError(
        ...
        "minting a handoff",
        { code: "VAULT_UNRESOLVED" },
      );
    }
    const kRepo = hexToBytes(repoFile.k_repo_hex);

    // vault-multi-writer rev 47 (task 5.5, design D4): only an ACTIVE
    ...
    // role-attenuation (`services/vault/claims.ts mintHandoff`) will
    ...
    // `vault-writer-state.ts` already uses elsewhere in this codebase.
    ...
    const snapshot = await captureHandoffSnapshot({
      ...
      message: (stats) => {
        const note: import("../node/vault-handoff.js").KygitHandoffNote = {
          ...options.note,
          capture: {
    ...
    // vault-multi-writer rev 47 (task 5.5, design D4) — the MINTER's own
    ...
    const grant = buildWriterAdmissionGrant({
      ...
      minted_role: grantMintedRole as import("../node/vault-handoff.js").VaultWriterMintedRole,
    ...
    const sealed = sealHandoffEnvelopeV2(handoff_id_bytes, secrets.wrap_key, {
      ...
      // and its clone died VAULT_EPOCH_NOT_OPENABLE at generation 1.
    ...
    const response = await this.#client.request<{
      ...
      warnings?: { code: string; message: string }[];
      next_actions?: NextAction[];
    }>(`/vaults/v1/${encodeURIComponent(handle.repo_id)}/handoffs`, {
      method: "POST",
      body: {
        handoff_id: handoffId,
    ...
    return {
      ...
      vault: handoffVaultFromWire(
        response,
        options.address && vaultRemoteAddressForm(options.address) === "slug" ? `${options.address.org_id}/${options.address.project_id}` : null,
      ),
  ...
  async listHandoffs(options: VaultHandleOptions): Promise<VaultHandoffListResult> {
    const repoId = await this.#resolveRepoId(options);
    return this.#client.request<VaultHandoffListResult>(`/vaults/v1/${encodeURIComponent(repoId)}/handoffs`, { context: "listing handoffs" });
  }

  /** Revoke a handoff (idempotent — a second revoke of an already-revoked/redeemed/expired row still answers `200`). */
  async revokeHandoff(handoffId: string, options: VaultHandleOptions): Promise<{ handoff_id: string; state: string }> {
    const repoId = await this.#resolveRepoId(options);
    return this.#client.request<{ handoff_id: string; state: string }>(`/vaults/v1/${encodeURIComponent(repoId)}/handoffs/${encodeURIComponent(handoffId)}`, { method: "DELETE", context: "revoking a handoff" });
  }
   ...
   * {@link Repos.handoff} does (design D1), push it (retained, on no
  ...
  async invite(
    options: VaultHandleOptions & {
      ...
      roomKey?: string;
      note: Omit<import("../node/vault-handoff.js").KygitInviteNote, "capture">;
      includeSensitive?: string[];
      onCommitLine?: (line: string) => void;
    ...
    // vault-multi-writer rev 47 (kygit-invite design D4): only an ACTIVE
    ...
    if (!localWriterKeyId || !repoFile.writer_set_pin?.writers.some((w) => w.writer_key_id === localWriterKeyId)) {
      throw new LocalError(
        ...
        "minting an invite",
        { code: "VAULT_WRITER_NOT_ADMITTED", details: { next_action: "request_writer_sync", command: "run402 repos access sync" } },
      );
    }
    ...
    const snapshot = await captureHandoffSnapshot({
      ...
      ...(options.includeSensitive !== undefined ? { includeSensitive: options.includeSensitive } : {}),
      message: (stats) => {
        const note: import("../node/vault-handoff.js").KygitInviteNote = {
          ...options.note,
          capture: {
            base_head: stats.base_head_oid,
            branch: stats.branch,
    ...
    let inviterPresenceReport: VaultInvitePresenceResult = { registered: false };
    try {
    ...
    // vault-multi-writer rev 47 (kygit-invite design D4) — the MINTER's
    ...
    // "Handoff / resume", vault-invite spec): `role` (the minted role),
    ...
    const response = await this.#client.request<{
      ...
      warnings?: { code: string; message: string }[];
      next_actions?: NextAction[];
    }>(`/vaults/v1/${encodeURIComponent(handle.repo_id)}/invites`, {
      method: "POST",
      body: {
        invite_id: inviteId,
        ...(options.role !== undefined ? { role: options.role } : {}),
    ...
    let roomMessage: VaultInviteRoomMessageResult = { posted: false, reason: "inviter presence was not registered" };
    if (inviterPresence) {
  ...
  async listInvites(options: VaultHandleOptions): Promise<VaultInviteListResult> {
    const repoId = await this.#resolveRepoId(options);
    return this.#client.request<VaultInviteListResult>(`/vaults/v1/${encodeURIComponent(repoId)}/invites`, { context: "listing invites" });
  }

  /** Revoke an invite (idempotent — a second revoke of an already-revoked/redeemed/expired row still answers `200`). */
  async revokeInvite(inviteId: string, options: VaultHandleOptions): Promise<{ invite_id: string; state: string }> {
    const repoId = await this.#resolveRepoId(options);
    return this.#client.request<{ invite_id: string; state: string }>(`/vaults/v1/${encodeURIComponent(repoId)}/invites/${encodeURIComponent(inviteId)}`, { method: "DELETE", context: "revoking an invite" });
  }
   ...
   */
  /**
   * vault-multi-writer rev 47 (task 5.6, design D4/D5) — order: parse →
  ...
  async resume(options: { key: string; to?: string; keystore_root?: string; onLine?: (line: string) => void }): Promise<VaultHandoffResumeResult> {
    ...
    if (!signingKeypair) {
      throw new LocalError(
        "this keystore has no local signing seed — resuming a vault-multi-writer handoff requires signing writer_acceptance with a full Ed25519 seed, which a read-only recovery identity does not hold",
        "resuming a handoff",
        { code: "VAULT_UNRECOVERABLE" },
      );
    }
    ...
    const redeem = await this.#client.request<{
      ...
      next_actions?: NextAction[];
    }>(`/vaults/v1/handoffs/${encodeURIComponent(parsed.handoff_id)}/redeem`, {
      method: "POST",
    ...
    // pre-vault-multi-writer mint's envelope outright.
    ...
    // (`Vault.genesis()` requires a keystore repo file — this is
    ...
    const { createVaultHttpTransport } = await this.#publication();
    const transport = createVaultHttpTransport(this.#client);
    ...
    const genesis = parseVaultStrict(new TextDecoder().decode(genesisBytes)) as { creator_signing_pubkey: string };
    if (!verifyVaultObject(genesis as unknown as Parameters<typeof verifyVaultObject>[0], genesis.creator_signing_pubkey)) {
      throw new LocalError("vault_genesis signature does not verify", "resuming a handoff", { code: "VAULT_SIGNATURE_INVALID", details: { repo_id: vault.vault_id } });
    }
    ...
    // vault resolution path uses, which also writes `r402.room`.
    const { pinVaultRepo } = await this.#address();
    ...
    // verify chain — `open()` here mainly constructs the `Vault`
    ...
    // field's own doc comment in vault-keystore.ts.
    ...
    // within minutes of use; run the same reconcile `capture()` runs,
   ...
   * (design D5, mirroring {@link Repos.resume}'s own bare-wallet
  ...
  async join(options: {
    ...
    const redeem = await this.#client.request<{
      ...
      writer_activation: { state: string };
      next_actions?: NextAction[];
    }>(`/vaults/v1/invites/${encodeURIComponent(parsed.invite_id)}/redeem`, {
      method: "POST",
    ...
    const genesis = parseVaultStrict(new TextDecoder().decode(genesisBytes)) as { creator_signing_pubkey: string };
    if (!verifyVaultObject(genesis as unknown as Parameters<typeof verifyVaultObject>[0], genesis.creator_signing_pubkey)) {
      throw new LocalError("vault_genesis signature does not verify", "joining an invite", { code: "VAULT_SIGNATURE_INVALID", details: { repo_id: vault.vault_id } });
    }
    const genesisSha = sha256Hex(genesisBytes);
    ...
    // active project. Reuses the SAME pin-writer every other vault
    ...
    // `VAULT_WRITER_NOT_ADMITTED` refusal. `added_writer.principal_id`
    ...
    let writerActivation: VaultInviteJoinResult["writer_activation"];
    try {
      const who = await this.#client.request<{ principal: { id: string } }>("/agent/v1/whoami", { context: "resolving this principal's id for the writer activation head" });
      const activation = await handle.vault.submitWriterActivationHead({
        addedWriterKeyId: myWriterKeyId,
    ...
    // same reconcile `capture()` runs, best-effort (never a `join()` throw),
   ...
   * source — see `Vault.rotateEpoch`'s own doc comment). Submitting
   ...
   */
    ...
    // `../node/vault-deploy.ts` byte-for-byte (same three causes, same
   ...
   * network call).
   */
  async postPublishCopies(input: { repo_id: string; keystore: VaultKeystore }): Promise<{ mirror_push: VaultMirrorPushResult; byo_chain_copy: VaultMirrorPushResult }> {
  ...
  async #tryMirrorPush(repoId: string, keystore: VaultKeystore): Promise<VaultMirrorPushResult> {
    try {
  ...
  /**
   * vault-byo-primary-bucket task 3.3 — the chain's every-push dual-write
   ...
   * result on its own field, never a `capture()`/`deploy()` throw).
   */
  async #tryByoChainCopyPush(repoId: string, keystore: VaultKeystore): Promise<VaultMirrorPushResult> {
    try {
  ...
  /** Best-effort envelope-recipient reconcile: availability and local-policy failures never surface as a `capture()` throw (mirroring {@link #tryMirrorPush}), but INTEGRITY verdicts do — `RECONCILE_FATAL_CODES` propagates tampering evidence instead of demoting it to `skipped_error` (round 3 blocker 2). */
  async #tryReconcileEnvelopeRecipients(vault: import("../node/vault-publication.js").Vault): Promise<VaultReconcileEnvelopeRecipientsPushResult> {
    try {
      const result = await vault.reconcileEnvelopeRecipients();
  ...
  /**
   * vault-multi-writer (task 5.7) — best-effort writer-admission
   ...
   * a CAS-conflict-exhausted admission) never surface as a `capture()`/
   * `deploy()`/`open()` throw — `Vault.reconcileWriterAdmissions()`
   ...
   */
  async #tryReconcileWriterAdmissions(vault: import("../node/vault-publication.js").Vault): Promise<VaultReconcileWriterAdmissionsPushResult> {
    try {
  ...
  /**
   * A REAL preview of what {@link capture} would publish
   * — never publishes anything, and never allocates. `run402 repos
   * capture --dry-run` and `git-remote-run402`'s `option dry-run true` are
   ...
   * {@link capture}, this NEVER allocates one (push-to-create must not allocate
   ...
   * {@link import("../node/vault-publication.js").Vault.planPush},
  ...
  async planCapture(
    options: VaultHandleOptions & {
      /** Same as {@link capture}'s `address` — a parsed remote address. Resolved READ-ONLY (`allow_create: false`); never push-to-creates. */
      address?: VaultRemoteAddress;
      snapshot?: Omit<import("../node/vault-snapshot.js").VaultSnapshotOptions, "dir">;
      onCommitLine?: (line: string) => void;
      checkpoint?: boolean;
    },
  ): Promise<VaultSnapshotPushPlan> {
    const { captureSnapshot, vaultCommitLine } = await this.#snapshot();
    const repoDir = options.repo_dir ?? process.cwd();

    // READ-ONLY resolution — mirrors `capture()`'s own dispatch on `address`,
    ...
    if (!handle) {
      return {
        ...
        allocation_needed: true, snapshot, vault_commit: snapshot.oid, vault_commit_line: line,
      };
    }
    ...
    const push: VaultPushOptions = {
      transaction: deployRefTransaction(materialized.refs, snapshot.oid),
      head_target: snapshot.head,
      protocol_refs: "allow", // same opt-in `capture()` makes for the SAME move — see its doc comment.
      ...(options.checkpoint ? { checkpoint: true } : {}),
    };
    const plan = await handle.vault.planPush(push);
    return { ...plan, allocation_needed: false, snapshot, vault_commit: snapshot.oid, vault_commit_line: line };
  }

  /**
   * Compaction's transient-storage preflight
   * (vault-compaction-headroom-preflight, design D1/D2).
   *
   ...
   */
  async compactHeadroom(options: VaultHandleOptions = {}): Promise<VaultCompactHeadroom | null> {
  ...
  /**
   * Open this vault's compaction headroom grant directly (vault-checkpoint-cadence
   ...
   * operator/diagnostic surface). Throws `VAULT_COMPACTION_GRANT_ACTIVE`
   * (409) verbatim when another compaction already holds this project's
   * grant.
   */
  async openCompactionGrant(options: VaultHandleOptions = {}): Promise<VaultCompactionGrant> {
    const handle = await this.open(options);
  ...
  async closeCompactionGrant(options: VaultHandleOptions = {}): Promise<{ closed: boolean }> {
    const handle = await this.open(options);
   ...
   *
   * `effectiveLimitOverride` (vault-checkpoint-cadence design D3): when a
   ...
   * the tier grew.
   */
  async #readCompactHeadroom(repoId: string, effectiveLimitOverride?: number | string): Promise<VaultCompactHeadroom | null> {
  ...
  async #compactHeadroomPreflight(repoId: string, ignore: boolean, effectiveLimitOverride?: number | string): Promise<VaultCompactHeadroom | null> {
    ...
    if (!ok && !ignore) {
      ...
      throw new LocalError(
        ...
        "preflighting vault compaction storage headroom",
        {
          code: "VAULT_COMPACT_INSUFFICIENT_HEADROOM",
          details: {
            pool_used_bytes: poolUsed,
            pool_limit_bytes: poolLimit,
            vault_source_bytes: sourceBytes,
  ...
  /** A 409 `VAULT_COMPACTION_GRANT_ACTIVE` refusal — another compaction already holds this project's grant. */
  #isCompactionGrantActive(e: unknown): e is { status?: number; code?: string; details?: { expires_at?: string } } {
    ...
    return Boolean(err && err.code === "VAULT_COMPACTION_GRANT_ACTIVE");
  }
   ...
   *   - 409 `VAULT_COMPACTION_GRANT_ACTIVE`: another maintenance op (this
   ...
   */
  async #openMaintenanceGrantOrConflict(handle: VaultHandle, context: string): Promise<VaultCompactionGrant | null> {
    try {
      ...
      if (this.#isCompactionGrantActive(e)) {
        throw new LocalError(
          ...
          context,
          { code: "VAULT_COMPACTION_IN_PROGRESS", details: { expires_at: (e as { details?: { expires_at?: string } }).details?.expires_at ?? null } },
        );
      }
  ...
  async compact(
  ...
  ): Promise<VaultCompactResult> {
    const handle = await this.open(options);

    // vault-checkpoint-cadence design D3: open the compaction headroom
    ...
    // {@link VaultNamespace.#openMaintenanceGrantOrConflict}.
    const grant = await this.#openMaintenanceGrantOrConflict(handle, "opening the vault compaction headroom grant");

    // Everything from here on is bracketed by the grant's own lifetime —
    // including the preflight's OWN possible throw (`VAULT_COMPACT_
  ...
  async #compactAfterGrant(
    ...
    let lease: VaultMaintenanceLease | null = null;
    ...
    try {
      ...
      let published: VaultPublishResult;
      try {
      ...
      if (lease) {
        await this.#client
          .request(`/vaults/v1/${encodeURIComponent(handle.repo_id)}/maintenance-leases/${encodeURIComponent(lease.maintenance_lease_id)}`, { method: "DELETE", body: { holder_token: lease.holder_token }, context: "releasing the vault maintenance lease" })
          .catch(() => undefined);
      }
  ...
  async prune(
    options: VaultHandleOptions & {
       ...
       */
      submit?: {
        core: import("../node/vault-prune.js").VaultPruneIntentCore;
        verifier_receipt: import("../node/vault-prune.js").VaultVerifierReceipt;
  ...
  ): Promise<VaultPruneResult> {
    const pub = await this.#publication();
    const prune = await this.#prune();
    const { isRootEligibleForRemoval, VAULT_RETENTION_MIN_DAYS } = pub;
    const handle = await this.open(options);
    // vault-byo-primary-bucket: prune deletion on a BYO vault is not
    ...
    // (VAULT_BYO_PRUNE_NOT_SUPPORTED). Say so before any planning or
    ...
    if (record.storage_profile === "byo") {
      return {
        ...
        blocked_reason: "this vault keeps its payload in your own bucket (storage_profile byo); run402 cannot delete from a bucket it holds no credential to, and client-executed prune deletion has not shipped — the gateway refuses a prune intent on a BYO vault by name (VAULT_BYO_PRUNE_NOT_SUPPORTED); compaction still works and superseded history stays in your bucket",
        intent_core: null,
    ...
    const candidates: VaultPruneCandidate[] = base.roots.map((root) => {
      ...
      return {
        ...
        reason:
          admittedAt === null
            ...
            : eligible
              ? `dropped more than ${VAULT_RETENTION_MIN_DAYS} days before the cutoff; removable at the next checkpoint-bearing generation, then at the next successful prune`
              : `still inside the ${VAULT_RETENTION_MIN_DAYS}-day retention window`,
      };
    });
    const rootView = {
      candidates,
    ...
    const objectCandidates: VaultPruneObjectCandidate[] = plan.candidates.map((r) => ({ object_id: r.object_id, object_kind: r.object_kind, size_bytes: r.size_bytes }));
    const blocked =
    ...
    const reportedCandidates: VaultPruneObjectCandidate[] = options.submit
      ? core.delete_set.map((r) => ({ object_id: r.object_id, object_kind: r.object_kind, size_bytes: r.size_bytes }))
      : objectCandidates;
    const planned: VaultPruneResult = {
      ...rootView,
    ...
    const ours = prune.buildVerifierReceipt(
      {
        ...
        implementation_id: prune.VAULT_SDK_VERIFIER_IMPLEMENTATION,
        implementation_version: prune.VAULT_SDK_VERIFIER_VERSION,
      },
      handle.vault.signer(),
    );
    ...
    const grant = await this.#openMaintenanceGrantOrConflict(handle, "opening the vault compaction headroom grant for prune submission");
    let stored: import("../node/vault-prune.js").VaultPruneIntentRecord & { stored: boolean };
    try {
    ...
    let intentRecord: import("../node/vault-prune.js").VaultPruneIntentRecord | null = stored;
  ...
  async #uploadVerifierReceipts(
    ...
    receipts: readonly import("../node/vault-prune.js").VaultVerifierReceipt[],
  ): Promise<void> {
    const { storedBytes, sha256Hex } = await import("./vault.crypto.js");
    const objects = receipts.map((r) => {
      const bytes = storedBytes(r as unknown as import("./vault.types.js").VaultSignedObject);
      return {
        path: pub.vaultPaths.verifierReceipt(r.object_id),
    ...
    try {
      ...
      for (const o of objects) {
        ...
        throw new LocalError(
          ...
          "uploading vault verifier receipts",
          { code: "VAULT_RECEIPT_ID_REUSED_DIFFERENT", details: { object_id: o.object_id, existing_sha256: existingSha256, this_attempt_sha256: o.sha256 } },
        );
      }
   ...
   * (`services/vault/upload-sessions.ts`: "an object id in the manifest is
  ...
  async #checkRootsEvolution(
    handle: VaultHandle,
    entries: Array<{ head: import("./vault.types.js").VaultHead; head_sha256: string; claim_set: unknown }>,
   ...
   */
  async verify(options: VaultHandleOptions & { persist?: boolean } = {}): Promise<VaultVerifiedState> {
    const handle = await this.open(options);
   ...
   * `VAULT_BYO_OBJECT_MISSING` on a confirmed absence). A managed vault
   * is byte-identical to before this task.
   */
  async fsck(options: VaultHandleOptions & { write?: boolean; mirror?: boolean } = {}): Promise<VaultFsckResult> {
    ...
    if (!repoId) {
      throw new LocalError(
        ...
        "running vault fsck",
        { code: "VAULT_UNRESOLVED", details: { project_id: options.project_id ?? null } },
      );
    }
    const write = options.write ?? true;
    // vault-agent-envelopes D5: fsck is observational — it never wraps an
    ...
    let retainedRefs: import("../node/vault-publication.js").VaultRetainedRefsReconcileResult | null = null;
    ...
    // vault-byo-primary-bucket task 3.3 (design D6) — LAST, deliberately:
    ...
    return computeOpenProofOutcome({
      ...
      resolvePrincipalId: async () => {
        const who = await this.#client.request<{ principal: { id: string } | null }>("/agent/v1/whoami", {
          context: "resolving this principal's identity for a vault proof-of-open submission",
        });
        return who.principal?.id ?? null;
      },
      readerEntrypoint: async () => {
        const { vaultReaderEntrypoint } = await this.#publication();
        return vaultReaderEntrypoint("fsck");
      },
  ...
  /**
   * vault-byo-primary-bucket task 3.3 (design D6) — `repos fsck`'s
   * wiring of the shipped read-half primitive
   * {@link import("../node/vault-mirror.js").verifyByoObjectsPresent}.
   ...
   */
  async #checkByoPresence(vault: VaultRecord | null, repoId: string, keystore: VaultKeystore): Promise<VaultFsckByoPresence | undefined> {
   ...
   * — vault-human-envelopes task 4.1's ADD-path workaround. See {@link
   * import("../node/vault-publication.js").Vault.
   ...
   *
   * No CLI verb runs this directly — `repos access` is the inspection
   * read. `deploy()` runs it, best-effort,
   ...
   * that decided deploy-implies-capture." `capture()` (capture-and-publish
   ...
   * {@link Repos.access} for the READ half repo-surface-consolidation
   * ships in its place.
   */
  async reconcileEnvelopeRecipients(options: VaultHandleOptions = {}): Promise<VaultReconcileEnvelopeRecipientsResult> {
    const handle = await this.open(options);
    return handle.vault.reconcileEnvelopeRecipients();
  }

  /**
   * vault-multi-writer (task 5.7) — admit every eligible `pending_writers[]`
   ...
   */
  async reconcile(options: VaultHandleOptions = {}): Promise<VaultReconcileWriterAdmissionsResult> {
    const handle = await this.open(options);
   ...
   * See {@link import("../node/vault-publication.js").Vault.
   ...
   */
  async rotateEpoch(
    options: VaultHandleOptions & { reason: VaultRotationReason; recipient_state_version: string; recipient_revocation_version: string; client_idempotency_key?: string; pending_confirmations?: { principal_id: string; ek_fingerprint: string; receipt: VaultRecipientConfirmationReceipt }[] },
  ): Promise<import("../node/vault-publication.js").VaultRotationResult> {
    const handle = await this.open(options);
   ...
   * (`reason:"member_removed"`, vault-multi-writer D6): counters come off
   * the envelope-recipients read, no declaration, no owner step-up — any
   * surviving writer can run it. `capture()` runs it automatically when the
   ...
   */
  async rotateEpochForMemberRemoval(options: VaultHandleOptions & { client_idempotency_key?: string } = {}): Promise<import("../node/vault-publication.js").VaultRotationResult> {
    const handle = await this.open(options);
  ...
  async publishPinManifestUpdate(
    input: { principal_id: string; ek_fingerprint: string; receipt: VaultRecipientConfirmationReceipt } & VaultHandleOptions,
  ): Promise<import("../node/vault-publication.js").VaultPublishResult> {
   ...
   * `capture()`, but the remedy is different: a manifest-only publish should
   ...
   * principal is already exhausted (see `Vault.rotateEpoch`'s own
   ...
   * ships (gated on `vault-human-envelopes`' epoch-rotation work). See
   * {@link VaultAccessResult}'s doc comment for the honest gap this
   * reports rather than invents.
   */
  async access(options: VaultHandleOptions = {}): Promise<VaultAccessResult> {
    const repoId = await this.#resolveRepoId(options);
    const record = await this.get(repoId).catch(() => null);
    const orgId = record?.org_id ?? null;

    // vault-agent-envelopes D3/D5: `access` is one of the ordinary reads a
    ...
    if (options.reconcile !== "forbidden") {
      try {
        const { VaultKeystore } = await this.#keystore();
        const probe = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    ...
    const [directory, coverage] = await Promise.all([
      ...
      this.#client.request<VaultEnvelopeRecipientsResponse>(`/vaults/v1/${encodeURIComponent(repoId)}/envelope-recipients`, { context: "reading the vault envelope recipients" }),
    ]);
    const covered = new Set(coverage.recipient_fingerprints);

    // `desired` is OPTIONAL on the wire (see VaultEnvelopeRecipientsResponse):
    ...
    const recipients: VaultAccessResult["recipients"] = directory.keys.map((k) => {
      const pinned = pins[k.principal_id];
      const desired = desiredByPrincipal.get(k.principal_id) ?? null;
      const envelopeState: VaultAccessRecipient["envelope_state"] =
    ...
    // revoke, because vault v0 has no epoch-rotation mechanism yet. See
    // VaultAccessResult's doc comment.
    const staleAccess: VaultAccessResult["stale_access"] = (desiredList ?? [])
    ...
    // vault-agent-envelopes: THIS machine's keystore is reported for every
    ...
    const thisKeystore: VaultAccessResult["this_keystore"] = ownFingerprint !== null
      ? {
          ...
          ...(!covered.has(ownFingerprint) && publishState === "active"
            ? { next_actions: [
                ...
                { action: "ask a key-holder to run any vault operation (run402 repos view / git push)", why: "a key-holding client wraps every pending desired recipient on its next operation" },
              ] }
            : {}),
    ...
    const gap = envelopeStateAvailable
      ...
      : "the gateway did not report desired-recipient state (desired[]) for this vault_id — likely an older gateway than this SDK " +
        "expects, so per-recipient envelope_state and stale_access are unavailable. history_scope is unavailable regardless: vault " +
   ...
   * import("../node/vault-mirror.js").mirrorPushForGeneration}'s own
   ...
   */
  async deploy(
    options: Omit<VaultDeployOptions, "vault"> & VaultHandleOptions,
  ): Promise<VaultDeployResult & { mirror_push?: VaultMirrorPushResult; byo_chain_copy?: VaultMirrorPushResult; reconcile_recipients?: VaultReconcileEnvelopeRecipientsPushResult; writer_reconcile?: VaultReconcileWriterAdmissionsPushResult }> {
    const { runVaultDeploy } = await this.#deploy();
    const handle = await this.open(options);
    const result = await runVaultDeploy({ ...options, vault: handle.vault, repo_dir: options.repo_dir ?? process.cwd() });
    // Same ordering capture() uses (mirror, then reconcile) — sequential, not
    ...
    // without standing up a live vault — see repos-deploy-hooks.test.ts.
    const hooked = await attachVaultDeployHooks(
      result,
      () => this.#tryMirrorPush(handle.repo_id, handle.keystore),
      () => this.#tryReconcileEnvelopeRecipients(handle.vault),
    );
    // vault-byo-primary-bucket task 3.3 / vault-multi-writer task 5.7
    // — composed AFTER, not inside, `attachVaultDeployHooks` (kept
   ...
   */
  async drainOverrides(options: VaultHandleOptions = {}): Promise<import("../node/vault-deploy.js").VaultOverrideDrainReport> {
   ...
   */
  async restore(options: VaultHandleOptions & { target_dir: string }): Promise<{ refs: Record<string, string>; generation: string; retained_refs: import("../node/vault-publication.js").VaultRetainedRefsReconcileResult }> {
    const handle = await this.open(options);
  ...
  // ── degraded read (vault-byo-primary-bucket, design D4, task 3.4 — mirror half) ──
   ...
   * 4xx — see `isNetworkClassVaultReadError`), fall back to the vault's
   ...
   */
  async withDegradedRead<T extends VaultDegradedReadLive>(options: {
    attemptLive: () => Promise<T>;
    keystore: VaultKeystore;
    repo_id: string;
    out_dir: string | null;
  }): Promise<VaultDegradedReadOutcome<T>> {
    const { tryVaultDegradedRead } = await this.#degradedRead();
    return tryVaultDegradedRead(options);
  }

  /**
   * vault-byo-primary-bucket (design D4): the degraded read for the OPEN
   ...
   * address (vault-offline-clone-resolve) — the whole read is served from
   ...
   */
  async degradedOpenFallback(input: { repo_dir: string; error: unknown; address?: import("../node/vault-address.js").ResolveVaultAddressOptions["address"]; repo_id?: string }): Promise<VaultDegradedReadOutcome<VaultDegradedReadLive> | null> {
    const [{ isNetworkClassVaultReadError, tryVaultDegradedRead }, { VaultKeystore }, { readPinnedVaultRepo, resolveVaultAddress }, { createVaultHttpTransport }] = await Promise.all([this.#degradedRead(), this.#keystore(), this.#address(), this.#publication()]);
    if (!isNetworkClassVaultReadError(input.error)) return null;
    const keystore = new VaultKeystore({});
    let repoId: string | null = input.repo_id ?? null;
    if (!repoId) {
      const pinned = await readPinnedVaultRepo(input.repo_dir).catch(() => null);
    ...
    if (!repoId && input.address) {
      ...
      try {
        const resolution = await resolveVaultAddress({ keystore, transport: createVaultHttpTransport(this.#client), address: input.address, repo_dir: input.repo_dir, allow_create: false });
    ...
    if (!repoId) return null;
    try {
      return await tryVaultDegradedRead<VaultDegradedReadLive>({ attemptLive: () => Promise.reject(input.error), keystore, repo_id: repoId, out_dir: input.repo_dir });
    } catch {
      return null;
    }
  }

  // ── mirror (vault-mirror-and-recover, design D1/D2/D7) ─────────────────
   ...
   */
  async mirrorSet(options: VaultHandleOptions & { destination_url: string; credential?: VaultMirrorCredential; region?: string; endpoint?: string }): Promise<VaultMirrorConfig> {
    const [{ VaultKeystore }, { parseMirrorDestinationUrl, saveMirrorConfig }] = await Promise.all([this.#keystore(), this.#mirrorConfig()]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const destination: VaultMirrorDestination = parseMirrorDestinationUrl(options.destination_url, { region: options.region, endpoint: options.endpoint });
  ...
  async mirrorRemove(options: VaultHandleOptions = {}): Promise<{ repo_id: string; removed: boolean }> {
    const [{ VaultKeystore }, { removeMirrorConfig }] = await Promise.all([this.#keystore(), this.#mirrorConfig()]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
   ...
   */
  async mirrorStatus(options: VaultHandleOptions & { is_byo?: boolean } = {}): Promise<VaultMirrorStatus> {
    const [{ VaultKeystore }, { readMirrorConfig, formatMirrorDestination }, { openVaultMirrorBackend }, { verifyVaultMirror }] = await Promise.all([
      this.#keystore(), this.#mirrorConfig(), this.#mirrorBackend(), this.#recovery(),
    ]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const config = readMirrorConfig(keystore, repoId);
    const base = { repo_id: repoId, validity_not_freshness: VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT, keystore_still_required: VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT };
    const lastSuccessAt = config?.last_success_at ?? null;
    if (!config) {
      // vault-mirror-default: the unconfigured branch stays gateway-blind
      // by construction — it returns before ANY network call, finding included.
      // `options.is_byo` is a caller-supplied hint (vault-byo-primary-bucket
      ...
      return {
        ...
        finding: vaultUnmirroredFinding({ configured: false, last_success_at: lastSuccessAt, mirrored_generation: null, is_byo: options.is_byo }),
      };
    }
    let mirroredGeneration: string | null = null;
    try {
      const backend = openVaultMirrorBackend(config.destination, repoId, config.credential);
      const report = await verifyVaultMirror(backend, { keystore });
    ...
    return {
      ...
      last_success_at: lastSuccessAt,
      finding: vaultUnmirroredFinding({ configured: true, last_success_at: lastSuccessAt, mirrored_generation: mirroredGeneration, is_byo: options.is_byo }),
    };
  }
  ...
  async mirrorSync(options: VaultHandleOptions = {}): Promise<VaultMirrorSyncSummary> {
    const [{ VaultKeystore }, { mirrorSync }] = await Promise.all([this.#keystore(), this.#mirror()]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
  ...
  async mirrorVerify(options: VaultHandleOptions = {}): Promise<VaultVerifyReport> {
    const [{ VaultKeystore }, { readMirrorConfig }, { openVaultMirrorBackend }, { verifyVaultMirror }] = await Promise.all([
      this.#keystore(), this.#mirrorConfig(), this.#mirrorBackend(), this.#recovery(),
    ]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    const config = readMirrorConfig(keystore, repoId);
    if (!config) throw new LocalError(`no mirror is configured for ${repoId}`, "verifying vault mirror", { code: "VAULT_MIRROR_NOT_CONFIGURED", details: { repo_id: repoId }, next_actions: [{ action: "run402 repos mirror <destination>" }] });
    const backend = openVaultMirrorBackend(config.destination, repoId, config.credential);
    return verifyVaultMirror(backend, { keystore });
  }
   ...
   *
   * vault-recovery-custody: a human member with no keystore recovers with
   ...
   */
  async recover(options: { source: string; out_dir: string; repo_id?: string; credential?: VaultMirrorCredential; region?: string; endpoint?: string; recovery_receipt?: VaultRecoveryReceipt; keystore_root?: string; member_bundle?: VaultMemberRecoveryBundle; source_recovery_code?: string; rp_id?: string }): Promise<VaultRecoverResult> {
    const [{ VaultKeystore }, { parseMirrorDestinationUrl }, { openVaultMirrorBackend, discoverMirroredRepoIds }, { recoverVaultMirror }] = await Promise.all([
      this.#keystore(), this.#mirrorConfig(), this.#mirrorBackend(), this.#recovery(),
    ]);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
    ...
    if (!repoId) {
      ...
      if (found.length === 0) throw new LocalError(`${options.source} holds no mirrored vault (no source/<repo_id>/ prefix found)`, "recovering vault", { code: "VAULT_MIRROR_EMPTY", details: { source: options.source } });
      if (found.length > 1) throw new LocalError(`${options.source} holds ${found.length} mirrored vaults; pass repo_id to pick one`, "recovering vault", { code: "VAULT_MIRROR_AMBIGUOUS", details: { source: options.source, repo_ids: found }, next_actions: found.map((id) => ({ action: `recover --repo ${id}` })) });
      repoId = found[0]!;
    }
    const backend = openVaultMirrorBackend(destination, repoId, options.credential);
    return recoverVaultMirror({
   ...
   */
  async sweepObjectCache(options: VaultHandleOptions): Promise<void> {
    const [{ VaultKeystore }] = await Promise.all([this.#keystore()]);
    const repoId = await this.#resolveRepoId(options);
    const keystore = new VaultKeystore(options.keystore_root !== undefined ? { rootDir: options.keystore_root } : {});
  ...
  async #resolveRepoId(options: VaultHandleOptions): Promise<string> {
  ...
  #publication(): Promise<PublicationModule> {
    return nodeOnly(() => import("../node/vault-publication.js"), "open");
  }
  #deploy(): Promise<DeployModule> {
    return nodeOnly(() => import("../node/vault-deploy.js"), "deploy");
  }
  #keystore(): Promise<KeystoreModule> {
    return nodeOnly(() => import("../node/vault-keystore.js"), "open");
  }
  #creation(): Promise<CreationModule> {
    return nodeOnly(() => import("../node/vault-creation-journal.js"), "init");
  }
  #snapshot(): Promise<SnapshotModule> {
    return nodeOnly(() => import("../node/vault-snapshot.js"), "push");
  }
  #prune(): Promise<PruneModule> {
    return nodeOnly(() => import("../node/vault-prune.js"), "prune");
  }
  #openOrCreate(): Promise<OpenOrCreateModule> {
    return nodeOnly(() => import("../node/vault-open-or-create.js"), "openOrCreate");
  }
  #address(): Promise<AddressModule> {
    return nodeOnly(() => import("../node/vault-address.js"), "resolveOrCreateAddress");
  }
  #mirror(): Promise<MirrorModule> {
    return nodeOnly(() => import("../node/vault-mirror.js"), "mirrorSync");
  }
  #mirrorConfig(): Promise<MirrorConfigModule> {
    return nodeOnly(() => import("../node/vault-mirror-config.js"), "mirrorSet");
  }
  #mirrorBackend(): Promise<MirrorBackendModule> {
    return nodeOnly(() => import("../node/vault-mirror-backend.js"), "mirrorSet");
  }
  #recovery(): Promise<RecoverModule> {
    return nodeOnly(() => import("../node/vault-recover.js"), "recover");
  }
  #handoff(): Promise<HandoffModule> {
    return nodeOnly(() => import("../node/vault-handoff.js"), "handoff");
  }
  #writerState(): Promise<WriterStateModule> {
    return nodeOnly(() => import("../node/vault-writer-state.js"), "handoff");
  }
  #restore(): Promise<RestoreModule> {
    return nodeOnly(() => import("../node/vault-restore.js"), "resume");
  }
  #degradedRead(): Promise<DegradedReadModule> {
    return nodeOnly(() => import("../node/vault-degraded-read.js"), "list");
  }
  #byoConfig(): Promise<ByoConfigModule> {
    return nodeOnly(() => import("../node/vault-byo-config.js"), "init");
  }
  /**
   * vault-byo-primary-bucket: the transport's `byoBackend` option — where
   ...
   * transport refuses `VAULT_BYO_NOT_CONFIGURED` by name rather than
   * reporting a false absence. Managed vaults never reach it.
   */
  async #byoBackendResolver(keystore: import("../node/vault-keystore.js").VaultKeystore): Promise<(repoId: string) => import("../node/vault-mirror-backend.js").VaultMirrorBackend | null> {
    const [{ readByoConfig }, { openVaultDestinationBackend }] = await Promise.all([this.#byoConfig(), this.#mirrorBackend()]);
    const cache = new Map<string, import("../node/vault-mirror-backend.js").VaultMirrorBackend | null>();
    return (repoId) => {
      if (!cache.has(repoId)) {
        // The RAW destination backend, keyed by the gateway's full
        // `source/<repo_id>/…` key — the same pairing the write path uses
        // (`openVaultDestinationBackend` + the session's `key`). The
        ...
        cache.set(repoId, config ? openVaultDestinationBackend(config.destination, config.credential) : null);
      }
      return cache.get(repoId)!;
    };
  }
  #byoProbe(): Promise<ByoProbeModule> {
    return nodeOnly(() => import("../node/vault-byo-probe.js"), "init");
  }
}

// ─── D7 — progressive terminal-loss warning (repo-first-onramp task 2.7) ────
//
// The §0 terminal-loss statement (`VAULT_TERMINAL_LOSS_STATEMENT`) never
 ...
 */
export const VAULT_LOSS_WARNING_THRESHOLDS = {
  generations: 10,
...
/**
 * vault-clone-scaling (bench P3): generations-since-checkpoint at or
 * above this advises `run402 repos gc`. Same one-place-to-tune convention
 * as {@link VAULT_LOSS_WARNING_THRESHOLDS}; an advisory, never a gate.
 */
export const VAULT_CHECKPOINT_ADVISORY_GENERATIONS = 25;

/** {@link vaultCheckpointStaleness}'s result. */
export interface VaultCheckpointStaleness {
 ...
 * from fresh here.
 */
export function vaultCheckpointStaleness(input: { newest_generation: string; covers_through_generation: string | null }): VaultCheckpointStaleness {
  try {
    ...
    return { generations_since_checkpoint: since, advised: since >= VAULT_CHECKPOINT_ADVISORY_GENERATIONS };
  } catch {
    return { generations_since_checkpoint: 0, advised: false };
  }
}
...
export interface VaultLossWarningTrip {
  generations: boolean;
 ...
 * `Repos.access` uses) can show this is no longer the single-principal
 * case the message below describes. See
 * {@link vaultKeystoreBackupReminderMessage} for that downgraded form.
 */
export function vaultLossWarningTrip(
  record: Pick<VaultRecord, "admitted_generations" | "storage" | "genesis_admitted_at">,
  now: Date = new Date(),
  ...
  return {
    generations: Number.isFinite(generations) && generations >= VAULT_LOSS_WARNING_THRESHOLDS.generations,
    source_bytes: Number.isFinite(sourceBytes) && sourceBytes >= VAULT_LOSS_WARNING_THRESHOLDS.source_bytes,
    days_since_genesis: daysSinceGenesis !== null && daysSinceGenesis >= VAULT_LOSS_WARNING_THRESHOLDS.days_since_genesis,
  };
}

/** Any composite metric tripped — the standing warning threshold itself. */
export function vaultLossWarningTripped(trip: VaultLossWarningTrip): boolean {
...
function vaultLossWarningReasons(trip: VaultLossWarningTrip): string[] {
  const reasons: string[] = [];
  if (trip.generations) reasons.push(`≥${VAULT_LOSS_WARNING_THRESHOLDS.generations} generations`);
  if (trip.source_bytes) reasons.push(`≥${Math.round(VAULT_LOSS_WARNING_THRESHOLDS.source_bytes / (1024 * 1024))} MB of source`);
  if (trip.days_since_genesis) reasons.push(`≥${VAULT_LOSS_WARNING_THRESHOLDS.days_since_genesis} days since genesis`);
  return reasons;
}
 ...
 * it") is false; use {@link vaultKeystoreBackupReminderMessage} instead.
 */
export function vaultLossWarningMessage(trip: VaultLossWarningTrip): string {
  const reasons = vaultLossWarningReasons(trip);
  return (
 ...
 * read `Repos.access` uses), the single-principal premise behind
 * {@link vaultLossWarningMessage} is false for THIS vault — printing it
 * anyway would be a false terminal-loss claim. The composite threshold still
 * fires the SAME way (this function does not touch `vaultLossWarningTrip`/
 * `vaultLossWarningTripped` at all — a real value-at-risk signal is still
 ...
 */
export function vaultKeystoreBackupReminderMessage(trip: VaultLossWarningTrip, coveringRecipients: number): string {
  const reasons = vaultLossWarningReasons(trip);
  return (
    ...
    `${VAULT_DURABILITY_STATEMENT} Back up this machine's keystore anyway: losing it does not lose the vault, but it does lose YOUR access to it.`
  );
}

/**
 * vault-mirror-default — the pure `vault_unmirrored` computation, in ONE
 ...
 */
export function vaultUnmirroredFinding(state: {
  configured: boolean;
  last_success_at: string | null;
  mirrored_generation: string | null;
  /** vault-byo-primary-bucket task 3.5 — when true, `message` uses the BYO remedy wording (a SECOND customer-held location) instead of the plain unmirrored statement. */
  is_byo?: boolean;
}): VaultUnmirroredFinding | null {
  if (state.configured && (state.last_success_at !== null || state.mirrored_generation !== null)) return null;
  return {
    kind: "vault_unmirrored",
    message: state.is_byo ? VAULT_BYO_UNMIRRORED_REMEDY_STATEMENT : VAULT_UNMIRRORED_FINDING_STATEMENT,
    setup_command: state.configured ? "run402 repos mirror --backfill" : "run402 repos mirror <destination>",
  };
}

/**
 * vault-byo-primary-bucket (design D4) — the ONE stderr line a degraded
 ...
 * `Repos.withDegradedRead`'s own doc comment.
 */
export function vaultDegradedReadNote(source: VaultDegradedReadSource): string {
  return `degraded read from ${source.destination}: ${VAULT_DEGRADED_READ_STATEMENT}`;
}

/** `run402::<org_id>/<project_id>` — what `git-remote-run402` resolves. */
export function vaultRemoteUrl(orgId: string, projectId: string): string {
 ...
 * {@link vaultRemoteUrl} (the wire slot admits both forms undiscriminated
 * — see {@link vaultRemoteAddressForm}); kept as its own named function so
 ...
 */
export function vaultRemoteUrlForRepo(orgSlug: string, repoName: string): string {
  return `run402::${orgSlug}/${repoName}`;
}

/** What {@link parseVaultRemoteUrl} returns — the two undiscriminated address halves. */
export interface VaultRemoteAddress {
  org_id: string;
  project_id: string;
}
 ...
 */
export function parseVaultRemoteUrl(url: string): VaultRemoteAddress | null {
...
export type VaultRemoteAddressForm = "id" | "slug";

const VAULT_UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
const VAULT_PROJECT_ID_RE = /^prj_/;
 ...
 */
export function vaultRemoteAddressForm(address: VaultRemoteAddress): VaultRemoteAddressForm {
  return VAULT_UUID_RE.test(address.org_id) && VAULT_PROJECT_ID_RE.test(address.project_id) ? "id" : "slug";
}

/** What a `SLUG_RELEASED` refusal names — the successor slug and the cooldown window (design D6). */
export interface VaultSlugReleasedInfo {
 ...
 */
export function vaultSlugReleasedInfo(err: unknown): VaultSlugReleasedInfo | null {
 ...
 * RESOURCE). Used by {@link Repos.planCapture} to distinguish "nothing to
