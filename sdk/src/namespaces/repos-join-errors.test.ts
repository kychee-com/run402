/**
 * `r.repos.join` — gateway error propagation, and the raw Invite Key's
 ...
 *
 * Mirrors `repos-resume-errors.test.ts` byte-for-byte, for the invite
 ...
 *
 * Run: node --test --import tsx sdk/src/namespaces/repos-join-errors.test.ts
 */
...
// round-trip is covered by vault-handoff.test.ts; this file only needs
...
describe("r.repos.join — gateway claim refusals propagate untouched", () => {
  const cases = [
  ...
  for (const c of cases) {
    it(`${c.code} (${c.status}) surfaces as a Run402Error carrying that exact code and body, untouched`, async () => {
      ...
      await assert.rejects(
        r.repos.join({ keystore_root: keystoreRoot, key: FABRICATED_KEY }),
        (err: unknown) => {
      ...
      assert.match(calls[0]!.url, /\/vaults\/v1\/invites\/.+\/redeem$/);
    });
  }
});

describe("r.repos.join — the raw key never appears in a thrown error (design: key-once contract)", () => {
  it("a claim refusal's error never carries the raw Invite Key, its master secret, or its derived auth_secret", async () => {
    ...
    let caught: unknown;
    try {
      await r.repos.join({ keystore_root: keystoreRoot, key: FABRICATED_KEY });
  ...
  it("a malformed key's parse refusal names no part of the input", async () => {
    ...
    let caught: unknown;
    try {
      await r.repos.join({ keystore_root: keystoreRoot, key: malformed });
  ...
  it("a kgh1_ handoff key handed to join() is refused by name, before any network call", async () => {
    ...
    await assert.rejects(
      r.repos.join({ keystore_root: keystoreRoot, key: handoffLookingKey }),
      (err: unknown) => {
