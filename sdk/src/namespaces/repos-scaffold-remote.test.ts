/**
 * `r.repos.scaffoldRemote` (first-deploy-agent-dx, design D3): the remote
...
import { vaultRemoteUrl } from "./repos.js";
...
let root: string;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "run402-vault-scaffold-"));
});
afterEach(() => {
...
const OUR_URL = vaultRemoteUrl(ORG, PROJECT);
...
describe("scaffoldRemote — the remote is always `run402`, origin is never claimed", () => {
  it("no remotes at all: adds `run402`, leaves `origin` absent", async () => {
    ...
    const r = await sdk().repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT });
  ...
  it("idempotent: a second scaffold reports run402 already pointing here, changes nothing", async () => {
    const dir = await freshRepo();
    const s = sdk();
    const first = await s.repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT });
    assert.equal(first.already_present, false);
    const second = await s.repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT });
  ...
  it("an EXISTING repository's branch is never touched, even if it is not 'main'", async () => {
    ...
    await sdk().repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT });
  ...
  it("an explicit remote_name is honored verbatim", async () => {
    const dir = await freshRepo();
    const r = await sdk().repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT, remote_name: "vault" });
...
describe("scaffoldRemote — the app root only; an enclosing repository is never touched", () => {
  it("a subdirectory of another repository: skipped, toplevel named, nothing changes anywhere", async () => {
    ...
    const r = await sdk().repos.scaffoldRemote({ repo_dir: app, org_id: ORG, project_id: PROJECT });
  ...
  it("nested: true — the app root becomes its own repository; the enclosing one gains only an exclude line", async () => {
    ...
    const r = await sdk().repos.scaffoldRemote({ repo_dir: app, org_id: ORG, project_id: PROJECT, nested: true });
    assert.equal(r.status, "scaffolded");
    ...
    const again = await sdk().repos.scaffoldRemote({ repo_dir: app, org_id: ORG, project_id: PROJECT, nested: true });
  ...
  it("nested: true on a directory in no repository at all is the ordinary scaffold (nested is moot)", async () => {
    ...
    const r = await sdk().repos.scaffoldRemote({ repo_dir: dir, org_id: ORG, project_id: PROJECT, nested: true });
    assert.equal(r.status, "scaffolded");
    assert.equal(r.created_repository, true);
    assert.equal(r.nested, undefined);
  ...
  it("the toplevel itself is scaffolded normally", async () => {
    const top = await freshRepo();
    const r = await sdk().repos.scaffoldRemote({ repo_dir: top, org_id: ORG, project_id: PROJECT });
    assert.equal(r.status, "scaffolded");
    assert.equal(r.name, "run402");
  });
});
