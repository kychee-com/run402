/**
 * vault — `r402s/v0` crypto core (protocol rev 40, §1 + §2; task 5.1).
 *
 ...
 * frozen vector set (`vault.crypto.test.ts`):
 *
...
import type {
  ...
  VaultAllocation,
  VaultDigestLabel,
  VaultEncryptedObjectKind,
  VaultEncryptionKeypair,
  VaultEnvelopeAad,
  VaultFormat,
  VaultFrameAad,
  VaultKeyEnvelope,
  VaultKeyEnvelopeReceipt,
  VaultObjectKind,
  VaultRecoveryReceipt,
  VaultRotateEpochPayload,
  VaultSealedFrame,
  VaultSealedKeyEnvelope,
  VaultSignedObject,
  VaultSigningKeypair,
  VaultStrictParseReason,
  VaultSuite,
  VaultGenesis,
} from "./vault.types.js";
...
export const VAULT_FORMAT = "r402s/v0" as const;
/** The one V0 cryptographic suite. */
export const VAULT_SUITE = "r402s-1" as const;
/** Frame magic, bytes 0–5 of every encrypted object. */
export const VAULT_FRAME_MAGIC = "R402S0" as const;
/** Frame suite byte (byte 6) and its two-char AAD spelling. */
export const VAULT_FRAME_SUITE_BYTE = 0x01 as const;
export const VAULT_FRAME_SUITE_ID = "01" as const;
/** Protocol §2: max plaintext per encrypted object (256 MiB). */
export const VAULT_MAX_PLAINTEXT_BYTES = 256 * 1024 * 1024;
/** The genesis generation / epoch constants (schema consts). */
export const VAULT_GENESIS_GENERATION = "0000000000000000" as const;
export const VAULT_GENESIS_EPOCH = "0000000000000001" as const;
/** HPKE `info` label for `key_envelope` (D188). */
export const VAULT_ENVELOPE_INFO_LABEL = "r402s/v0/envelope" as const;
 ...
 */
export const VAULT_TERMINAL_LOSS_STATEMENT =
 ...
 * many principals cover a vault (unlike {@link VAULT_TERMINAL_LOSS_STATEMENT},
 ...
 * instead of the terminal-loss one — see `Repos.status()`'s `covering_recipients`
 * field. Never paraphrase; the wording is a reviewed product commitment.
 */
export const VAULT_DURABILITY_STATEMENT =
...
export const VAULT_TERMINAL_LOSS_DOCTOR_TEXT =
...
/**
 * vault-mirror-and-recover (design D8) — the two honesty statements. Both
 ...
 * mechanism as {@link VAULT_TERMINAL_LOSS_STATEMENT} above. Never
 * paraphrased — the §6.4 honesty posture is a reviewed product commitment.
 */
export const VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT =
...
export const VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT =
...
/**
 * vault-mirror-default — the recommended-default framing, stated the same
 ...
 */
export const VAULT_MIRROR_THREE_COPIES_STATEMENT =
 ...
 * payload read prints (`formatVaultDegradedReadNote`, `vault-node-
 ...
 */
export const VAULT_DEGRADED_READ_STATEMENT =
...
/**
 * vault-mirror-default — the `vault_unmirrored` standing finding, worded to
 ...
 */
export const VAULT_UNMIRRORED_FINDING_STATEMENT =
  `this vault has no customer-held mirror copy yet — ${VAULT_MIRROR_THREE_COPIES_STATEMENT}; until a first mirror write or sync succeeds, the third copy does not exist` as const;

/**
 * vault-mirror-default — the one-liner `repos create` prints beside the
 ...
 */
export const VAULT_MIRROR_SETUP_HINT =
  "recommended: 'run402 repos mirror <destination>' (an s3:// bucket or a local directory) starts the customer-owned mirror — every later capture is mirrored to it automatically, and it is the copy that stays in your custody" as const;

/**
 * vault-byo-primary-bucket (proposal, the ratified D + chain-copy rung-3
 ...
 * and MUST NEVER use it as a blanket claim describing vault in general.
 ...
 */
export const VAULT_BYO_HEADLINE_STATEMENT =
  "your source ciphertext never touches our infrastructure — not even encrypted" as const;

/**
 * vault-byo-primary-bucket (design D7) — the allocation-time no-payload-copy
 ...
 * {@link VAULT_BYO_UNMIRRORED_REMEDY_STATEMENT} below, which is about a
 ...
 */
export const VAULT_BYO_NO_PAYLOAD_COPY_STATEMENT =
...
/**
 * vault-byo-primary-bucket (design D7) — the BYO variant of the
 * `vault_unmirrored` finding's remedy (vault-mirror-default): for a
 * managed vault the remedy is an ADDITIONAL mirror beside the platform's own
 * replicated copy ({@link VAULT_MIRROR_SETUP_HINT}); for a BYO vault the
 * platform holds no payload copy to fall back on
 * ({@link VAULT_BYO_NO_PAYLOAD_COPY_STATEMENT}), so the remedy names a
 ...
 * this fold — Phase 2/3 wire the surfaces that print it. Never paraphrase.
 */
export const VAULT_BYO_UNMIRRORED_REMEDY_STATEMENT =
...
export const VAULT_HEX16_RE = /^[0-9a-f]{16}$/;
export const VAULT_HEX32_RE = /^[0-9a-f]{32}$/;
export const VAULT_SHA256_RE = /^[0-9a-f]{64}$/;
export const VAULT_OID40_RE = /^[0-9a-f]{40}$/;
export const VAULT_VK_RE = /^vk_[0-9a-f]{32}$/;
export const VAULT_EK_RE = /^ek_[0-9a-f]{32}$/;
export const VAULT_SRC_RE = /^src_[0-9a-f]{32}$/;
export const VAULT_SERVICE_KEY_ID_RE = /^sk_[0-9a-z-]{4,64}$/;
export const VAULT_SIZE_BYTES_RE = /^(0|[1-9][0-9]{0,14})$/;
/** 32 bytes base64url: 43 chars, final char in the 16-value padding class. */
export const VAULT_B64U_32_RE = /^[A-Za-z0-9_-]{42}[AEIMQUYcgkosw048]$/;
/** 64 bytes base64url: 86 chars, final char ∈ {A,Q,g,w}. */
export const VAULT_B64U_64_RE = /^[A-Za-z0-9_-]{85}[AQgw]$/;
/** The HPKE-sealed K_repo: 48 bytes = exactly 64 base64url chars. */
export const VAULT_HPKE_CT_RE = /^[A-Za-z0-9_-]{64}$/;
export const VAULT_TIMESTAMP_RE =
...
/** Decode canonical base64url or throw (`VAULT_NONCANONICAL_BASE64URL`). */
export function fromBase64url(value: string, field = "value"): Uint8Array {
  if (!isCanonicalBase64url(value)) {
    fail("VAULT_NONCANONICAL_BASE64URL", `${field} is not canonical base64url`, "decoding r402s/v0 scalar", { field });
  }
...
export function isValidVaultTimestamp(value: string): boolean {
  if (!VAULT_TIMESTAMP_RE.test(value)) return false;
...
export function formatVaultTimestamp(date: Date = new Date()): string {
...
/**
 * The protocol-hash seam (vault-native-hash) — the second narrow slot
...
export function newVaultId(prefix: string): string {
...
function assertProfileValue(value: unknown, path: string): void {
  ...
  if (typeof value === "number" || typeof value === "bigint") {
    fail("VAULT_JSON_NUMBER", `r402s/v0 profile forbids JSON numbers (at ${path})`, "canonicalizing r402s/v0 object", { path });
  }
  ...
  if (typeof value === "object") {
    for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
      if (entry === undefined) {
        fail("VAULT_JSON_UNDEFINED", `undefined member at ${path}.${key}`, "canonicalizing r402s/v0 object", { path: `${path}.${key}` });
      }
      assertProfileValue(entry, `${path}.${key}`);
    }
    return;
  }
  fail("VAULT_JSON_UNSUPPORTED", `unsupported value at ${path}`, "canonicalizing r402s/v0 object", { path });
}
...
function inspectStrict(node: Node, path: string): void {
  switch (node.type) {
    ...
    case "Number":
      fail("VAULT_STRICT_PARSE", `JSON number at ${path}`, "strict-parsing r402s/v0 object", { reason: "json-number", path });
    ...
    case "Object": {
      ...
      for (const member of (node as ObjectNode).members) {
        ...
        if (seen.has(name)) {
          fail("VAULT_STRICT_PARSE", `duplicate member ${name} at ${path}`, "strict-parsing r402s/v0 object", { reason: "duplicate-member", path: `${path}.${name}` });
        }
        seen.add(name);
 ...
 */
export function parseVaultStrict(text: string): unknown {
  let ast: DocumentNode;
  try {
    ast = parse(text, { mode: "json", allowTrailingCommas: false });
  } catch {
    fail("VAULT_STRICT_PARSE", "invalid JSON", "strict-parsing r402s/v0 object", { reason: "invalid-json" });
  }
  inspectStrict(ast!, "$");
  const value = evaluate(ast!);
  if (jcsString(value) !== text) {
    fail("VAULT_STRICT_PARSE", "text is not canonical JCS", "strict-parsing r402s/v0 object", { reason: "noncanonical-encoding" });
  }
  return value;
}

/** Classify a strict-parse failure (for vector replay + diagnostics). */
export function vaultStrictParseReason(error: unknown): VaultStrictParseReason | null {
  if (error instanceof LocalError && error.code === "VAULT_STRICT_PARSE") {
    const details = error.details as { reason?: VaultStrictParseReason } | undefined;
...
export function storedBytes(object: VaultSignedObject): Uint8Array {
  return jcs(object);
}

/** SHA-256 over stored bytes — what every `*_sha256` naming a signed object means. */
export function storedBytesSha256(object: VaultSignedObject): string {
 ...
 */
export function vaultWithoutSignature<T extends { signature?: unknown }>(object: T): Omit<T, "signature"> {
  return withoutSignature(object);
}

/**
 * Signature preimage = `"r402s/v0/" + domain + "\n" + JCS(object without its
 * single top-level signature)`. `domain` is USUALLY a `VaultObjectKind`
 * (an actual `object_kind`-carrying signed object), but the type accepts any
 * string: vault-multi-writer rev 47's `writer_acceptance.statement` reuses
 * this exact byte-construction under the ad-hoc domain
 * `"handoff-writer-accept/v1"` for a sub-object that is NOT itself a
 * `VaultSignedObject` (no `object_kind`/`signature` fields of its own —
 ...
 */
export function signaturePreimage(domain: VaultObjectKind | string, objectWithoutSignature: Record<string, unknown>): Uint8Array {
  if ("signature" in objectWithoutSignature) {
    fail("VAULT_PREIMAGE_HAS_SIGNATURE", "preimage input must not carry a signature member", "building r402s/v0 signature preimage");
  }
  return concatBytes(utf8ToBytes(`${VAULT_FORMAT}/${domain}\n`), jcs(objectWithoutSignature));
}
...
export function signVaultObject<T extends { object_kind: VaultObjectKind }>(
 ...
 */
export function verifyVaultObject(object: VaultSignedObject, publicKey: Uint8Array | string): boolean {
  if (typeof object.signature !== "string" || !VAULT_B64U_64_RE.test(object.signature) || !isCanonicalBase64url(object.signature)) return false;
  let pub: Uint8Array;
  if (typeof publicKey === "string") {
    if (!VAULT_B64U_32_RE.test(publicKey) || !isCanonicalBase64url(publicKey)) return false;
...
/** Verify or throw `VAULT_SIGNATURE_INVALID`. */
export function assertVaultSignature(object: VaultSignedObject, publicKey: Uint8Array | string, context = "verifying r402s/v0 signature"): void {
  if (!verifyVaultObject(object, publicKey)) {
    fail("VAULT_SIGNATURE_INVALID", `${object.object_kind} signature does not verify (strict RFC 8032)`, context, { object_kind: object.object_kind });
  }
}
...
export function generateSigningKeypair(seed: Uint8Array = randomBytes(32)): VaultSigningKeypair {
  if (seed.length !== 32) fail("VAULT_BAD_SEED", "Ed25519 seed must be 32 bytes", "generating signing keypair");
  return { seed, public_key: ed25519PublicKey(seed) };
}

export function generateEncryptionKeypair(privateKey: Uint8Array = randomBytes(32)): VaultEncryptionKeypair {
  if (privateKey.length !== 32) fail("VAULT_BAD_SEED", "X25519 private key must be 32 bytes", "generating encryption keypair");
...
export function objectKeyInfo(repoId: string, epoch: string, objectKind: VaultEncryptedObjectKind, objectId: string): Uint8Array {
  return concatBytes(lp(VAULT_FORMAT), lp(VAULT_SUITE), lp(repoId), lp(epoch), lp(objectKind), lp(objectId), lp("L=32"));
}

/** `k_obj = HKDF-SHA-256(ikm=K_repo, salt=∅, info, L=32)`. */
export function deriveObjectKey(kRepo: Uint8Array, repoId: string, epoch: string, objectKind: VaultEncryptedObjectKind, objectId: string): Uint8Array {
...
export function digestKeyInfo(repoId: string, epoch: string, label: VaultDigestLabel): Uint8Array {
  return concatBytes(lp(VAULT_FORMAT), lp(VAULT_SUITE), lp(repoId), lp(epoch), lp("digest"), lp(label), lp("L=32"));
}

export function deriveDigestKey(kRepo: Uint8Array, repoId: string, epoch: string, label: VaultDigestLabel): Uint8Array {
 ...
 */
export function deriveDigestKeyFrom(ikm: Uint8Array, repoId: string, epoch: string, label: VaultDigestLabel): Uint8Array {
...
export function objectsetContent(oids: Iterable<string>): { oids: string[] } {
  ...
  for (const oid of unique) {
    if (!VAULT_OID40_RE.test(oid)) fail("VAULT_BAD_OID", `not a lowercase 40-hex oid: ${oid}`, "building objectset content");
  }
...
export function snapshotOidContent(oid: string): { oid: string; format: "sha1" } {
  if (!VAULT_OID40_RE.test(oid)) fail("VAULT_BAD_OID", `not a lowercase 40-hex oid: ${oid}`, "building snapshot_oid content");
...
export function frameAad(repoId: string, objectKind: VaultEncryptedObjectKind, objectId: string, epoch: string): VaultFrameAad {
  return { repo_id: repoId, object_kind: objectKind, object_id: objectId, epoch, suite: VAULT_SUITE, magic: VAULT_FRAME_MAGIC, suite_id: VAULT_FRAME_SUITE_ID };
}

/**
 * The bulk-AEAD seam (vault-native-bulk-crypto, design D1/D3).
 *
 ...
 */
export interface VaultAeadBackend {
...
let aeadBackend: VaultAeadBackend | null = null;
 ...
 */
export function _setVaultAeadBackend(backend: VaultAeadBackend | null): void {
  aeadBackend = backend;
}

/** SDK-INTERNAL: which backend is live. The verifier-lineage test asserts `null`. */
export function _vaultAeadBackend(): VaultAeadBackend | null {
  return aeadBackend;
}
 ...
 * `VAULT_AEAD_AUTH_FAILURE` (D4). A backend that throws is treated as an
...
export interface VaultSealFrameInput {
  /** The per-object key — derive with {@link deriveObjectKey}. */
  k_obj: Uint8Array;
  repo_id: string;
  object_kind: VaultEncryptedObjectKind;
  object_id: string;
  epoch: string;
 ...
 */
export function sealFrame(input: VaultSealFrameInput): VaultSealedFrame {
  if (input.k_obj.length !== 32) fail("VAULT_BAD_KEY", "k_obj must be 32 bytes", "sealing r402s/v0 frame");
  if (input.plaintext.length > VAULT_MAX_PLAINTEXT_BYTES) {
  ...
  if (nonce.length !== FRAME_NONCE_BYTES) fail("VAULT_BAD_NONCE", "frame nonce must be 24 bytes", "sealing r402s/v0 frame");
  ...
  const frame = concatBytes(utf8ToBytes(VAULT_FRAME_MAGIC), new Uint8Array([VAULT_FRAME_SUITE_BYTE]), nonce, ct);
  return { frame, ciphertext_sha256: sha256Hex(frame), size_bytes: String(frame.length), nonce_hex: bytesToHex(nonce) };
}

export interface VaultOpenFrameInput {
  k_obj: Uint8Array;
  repo_id: string;
  object_kind: VaultEncryptedObjectKind;
  object_id: string;
 ...
 */
export function openFrame(input: VaultOpenFrameInput): Uint8Array {
  const { frame } = input;
  if (input.expected_ciphertext_sha256 !== undefined && sha256Hex(frame) !== input.expected_ciphertext_sha256) {
    fail("VAULT_RECEIPT_MISMATCH", "frame bytes do not hash to the receipt's ciphertext_sha256", "opening r402s/v0 frame", { object_id: input.object_id });
  }
  if (frame.length < FRAME_HEADER_BYTES + AEAD_TAG_BYTES) {
    fail("VAULT_FRAME_INVALID", "frame shorter than header + tag", "opening r402s/v0 frame", { object_id: input.object_id });
  }
  const magic = new TextDecoder().decode(frame.subarray(0, 6));
  if (magic !== VAULT_FRAME_MAGIC || frame[6] !== VAULT_FRAME_SUITE_BYTE) {
    fail("VAULT_FRAME_INVALID", "frame header magic/suite mismatch", "opening r402s/v0 frame", { object_id: input.object_id });
  }
...
export function openFrameWithAad(kObj: Uint8Array, frame: Uint8Array, aad: Uint8Array, objectId = "?"): Uint8Array {
  if (frame.length < FRAME_HEADER_BYTES + AEAD_TAG_BYTES) {
    fail("VAULT_FRAME_INVALID", "frame shorter than header + tag", "opening r402s/v0 frame", { object_id: objectId });
  }
  ...
  if (opened === null) {
    fail("VAULT_AEAD_AUTH_FAILURE", "frame failed AEAD authentication", "opening r402s/v0 frame", { object_id: objectId });
  }
  return opened;
}
...
export function vaultHpkeSuite(): CipherSuite {
...
export function envelopeInfo(recipientPublicKeyRaw: Uint8Array, createdBy: string): Uint8Array {
  return concatBytes(lp(VAULT_ENVELOPE_INFO_LABEL), lp(VAULT_SUITE), lp(sha256Hex(recipientPublicKeyRaw)), lp(createdBy));
}
 ...
 */
export function envelopeAad(repoId: string, epoch: string, recipientFingerprint: string, rotationId?: string | null): VaultEnvelopeAad {
  if (rotationId) {
...
export interface VaultHpkeSealInput {
...
export async function hpkeSeal(input: VaultHpkeSealInput): Promise<{ enc: Uint8Array; ct: Uint8Array }> {
  const suite = vaultHpkeSuite();
...
export interface VaultHpkeOpenInput {
...
/** Single-shot HPKE open; any failure is `VAULT_HPKE_OPEN_FAILED` (never garbage plaintext). */
export async function hpkeOpen(input: VaultHpkeOpenInput): Promise<Uint8Array> {
  const suite = vaultHpkeSuite();
  try {
    ...
    fail("VAULT_HPKE_OPEN_FAILED", "HPKE open failed — the recipient recovers no key", "opening r402s/v0 key envelope");
  }
}

export interface VaultSealKeyEnvelopeInput {
...
export async function sealKeyEnvelope(input: VaultSealKeyEnvelopeInput): Promise<VaultSealedKeyEnvelope> {
  if (input.k_repo.length !== 32) fail("VAULT_BAD_KEY", "K_repo must be 32 bytes", "sealing r402s/v0 key envelope");
  if (!isValidVaultTimestamp(input.created_at)) fail("VAULT_BAD_TIMESTAMP", "created_at is not an RFC 3339 UTC ms timestamp", "sealing r402s/v0 key envelope");
  ...
  const envelope = signVaultObject(unsigned, input.signer.seed) as VaultKeyEnvelope;
  const bytes = storedBytes(envelope as unknown as VaultSignedObject);
 ...
 */
export async function openKeyEnvelope(input: VaultOpenKeyEnvelopeInput): Promise<Uint8Array> {
  const env = input.envelope;
  if (!VAULT_HPKE_CT_RE.test(env.ct) || !VAULT_B64U_32_RE.test(env.enc) || !isCanonicalBase64url(env.ct) || !isCanonicalBase64url(env.enc)) {
    fail("VAULT_SCHEMA_REJECT", "key_envelope enc/ct are not the exact scalar lengths", "opening r402s/v0 key envelope");
  }
  assertVaultSignature(env as unknown as VaultSignedObject, input.signer_public_key, "opening r402s/v0 key envelope");
  const ours = ekFingerprint(input.recipient.public_key);
  if (env.recipient_fingerprint !== ours || env.recipient_kind !== "principal") {
    fail("VAULT_ENVELOPE_NOT_FOR_RECIPIENT", "key_envelope is addressed to another recipient", "opening r402s/v0 key envelope", { recipient_fingerprint: env.recipient_fingerprint });
  }
  ...
  if (kRepo.length !== 32) fail("VAULT_HPKE_OPEN_FAILED", "opened plaintext is not a 32-byte K_repo", "opening r402s/v0 key envelope");
  return kRepo;
}
 ...
 */
export function parseRotateEpochPayload(head: { generation: string; epoch: string; transition: { kind: string; payload_format: string; payload: string; payload_sha256: string } | null }): VaultRotateEpochPayload {
  const t = head.transition;
  if (!t || t.kind !== "rotate_epoch") fail("VAULT_NOT_A_ROTATION", `head ${head.generation} does not carry a rotate_epoch transition`, "parsing rotate_epoch_payload");
  ...
  let payload: VaultRotateEpochPayload;
  try {
    payload = parseVaultStrict(new TextDecoder().decode(raw)) as VaultRotateEpochPayload;
  } catch {
  ...
  if (!VAULT_SHA256_RE.test(payload.rotation_id)) fail("CHAIN_BROKEN", `head ${head.generation}: rotate_epoch_payload.rotation_id is not a 64-hex digest`, "parsing rotate_epoch_payload");
 ...
 * `add_writer_key_payload` (vault-multi-writer rev 47, protocol §4.16):
 ...
 * (`../node/vault-writer-state.js`) — this function only gets the bytes
 * off the wire safely, so every reader can call it before any key material
 * is needed.
 */
export function parseAddWriterKeyPayload(head: { generation: string; repo_id: string; transition: { kind: string; payload_format: string; payload: string; payload_sha256: string } | null }): VaultAddWriterKeyPayload {
  const t = head.transition;
  if (!t || t.kind !== "add_writer_key") fail("VAULT_NOT_A_WRITER_ADD", `head ${head.generation} does not carry an add_writer_key transition`, "parsing add_writer_key_payload");
  ...
  let payload: VaultAddWriterKeyPayload;
  try {
    payload = parseVaultStrict(new TextDecoder().decode(raw)) as VaultAddWriterKeyPayload;
  } catch {
 ...
 * with `VAULT_EPOCH_NOT_OPENABLE` — never a bare `VAULT_AEAD_AUTH_FAILURE`
 ...
 * both the live publication read path (`Vault.verifyToNewest`, over
 * the gateway transport) and the offline mirror recovery path
 * (`recoverVaultMirror`, over a `VaultMirrorBackend`) call it —
...
export async function openEpochRotationForRecipient(input: {
  ...
  own_encryption_keypair: VaultEncryptionKeypair;
...
export interface VaultBuildGenesisInput {
  repo_id: string;
  org_id: string;
  project_id: string;
  allocation_nonce: string;
  creator_signing: VaultSigningKeypair;
  creator_encryption_public_key: Uint8Array;
  envelope_receipt: VaultKeyEnvelopeReceipt;
  created_at: string;
}

/** Build + sign a `vault_genesis` (generation 0, epoch 1, the creator's single envelope). */
export function buildVaultGenesis(input: VaultBuildGenesisInput): VaultGenesis {
  const unsigned = {
    format: VAULT_FORMAT,
    ...
    generation: VAULT_GENESIS_GENERATION,
    epoch: VAULT_GENESIS_EPOCH,
  ...
  return signVaultObject(unsigned, input.creator_signing.seed) as VaultGenesis;
}

/** A named genesis key-binding failure (semantic constraints `genesis-key-bindings` / `envelope-receipt-binding`). */
export type VaultGenesisBindingProblem =
  | "envelope_count"
  | "envelope_epoch"
 ...
 */
export function checkGenesisKeyBindings(genesis: VaultGenesis, storedEnvelope?: VaultKeyEnvelope): VaultGenesisBindingProblem[] {
  const problems: VaultGenesisBindingProblem[] = [];
  if (genesis.envelopes.length !== 1) problems.push("envelope_count");
  const receipt = genesis.envelopes[0];
  if (receipt && receipt.epoch !== VAULT_GENESIS_EPOCH) problems.push("envelope_epoch");
  ...
  if (storedEnvelope && receipt) {
    const bytes = storedBytes(storedEnvelope as unknown as VaultSignedObject);
...
export interface VaultBuildRecoveryReceiptInput {
  repo_id: string;
  org_id: string;
  project_id: string;
...
export function buildRecoveryReceipt(input: VaultBuildRecoveryReceiptInput): VaultRecoveryReceipt {
  const unsigned = {
    format: VAULT_FORMAT,
  ...
  return signVaultObject(unsigned, input.creator_signing.seed) as VaultRecoveryReceipt;
}

export type VaultRecoveryReceiptProblem =
  | "signature"
  | "genesis_sha256"
  | "repo_id"
  | "org_id"
 ...
 */
export function checkRecoveryReceipt(receipt: VaultRecoveryReceipt, genesis: VaultGenesis): VaultRecoveryReceiptProblem[] {
  const problems: VaultRecoveryReceiptProblem[] = [];
  if (!verifyVaultObject(receipt as unknown as VaultSignedObject, genesis.creator_signing_pubkey)) problems.push("signature");
  if (receipt.genesis_sha256 !== storedBytesSha256(genesis as unknown as VaultSignedObject)) problems.push("genesis_sha256");
...
export function checkAllocation(
  ...
  if (allocation.format !== VAULT_FORMAT || allocation.object_kind !== "allocation" || allocation.suite !== VAULT_SUITE) problems.push("envelope");
  if (!VAULT_SRC_RE.test(allocation.repo_id)) problems.push("repo_id");
  if (!VAULT_HEX32_RE.test(allocation.allocation_nonce)) problems.push("allocation_nonce");
  if (!VAULT_HEX16_RE.test(allocation.allocation_generation)) problems.push("allocation_generation");
  if (!VAULT_SERVICE_KEY_ID_RE.test(allocation.service_key_id)) problems.push("service_key_id");
  ...
  if (!isValidVaultTimestamp(allocation.issued_at) || !isValidVaultTimestamp(allocation.created_at)) problems.push("timestamp");
  if (servicePublicKey !== undefined && !verifyVaultObject(allocation as unknown as VaultSignedObject, servicePublicKey)) problems.push("signature");
  return problems;
}

// ─── Epoch rotation (D193-D203, rev 42, change vault-human-envelopes) ───
//
// Pure, isomorphic math shared by the producer (Node-only, vault-
...
// `packages/gateway/src/services/vault/epoch-rotation.ts` byte-for-byte
...
export interface VaultIncludedPair {
  principal_id: string;
...
export function computeTargetPartitionDigest(input: {
  ...
  included: readonly VaultIncludedPair[];
...
export function computeRotationId(descriptor: Record<string, unknown> & { signature?: unknown }): string {
  return sha256Hex(jcs(vaultWithoutSignature(descriptor)));
}
...
export function genesisEpochKeyCommitment(k1: Uint8Array, repoId: string, fingerprints: readonly string[]): string {
  const kDigest = deriveDigestKeyFrom(k1, repoId, VAULT_GENESIS_EPOCH, "vault_genesis_epoch_key");
  return keyedCommitment(kDigest, { repo_id: repoId, generation: VAULT_GENESIS_GENERATION, epoch: VAULT_GENESIS_EPOCH, fingerprints: sortedDistinct(fingerprints) });
}
...
export function checkFreshEpochKeyAgainstPriorKeys(kE: Uint8Array, priorKeys: Iterable<Uint8Array>): void {
  if (kE.length !== 32) fail("VAULT_BAD_KEY", "a freshly sampled K_e must be 32 bytes", "sampling a fresh epoch key");
  const fresh = bytesToHex(kE);
  for (const prior of priorKeys) {
    if (bytesToHex(prior) === fresh) {
      fail("VAULT_EPOCH_KEY_NOT_FRESH", "the freshly sampled K_e equals a prior epoch key already held locally — refusing to seal under it", "sampling a fresh epoch key");
    }
  }
}

export type VaultHPartitionVerdict = { ok: true } | { ok: false; detail: string };
...
export function checkPinManifestConservation(prior: readonly { principal_id: string }[], next: readonly { principal_id: string }[]): VaultHPartitionVerdict {
...
//
// vault-recovery-custody: a human member's X25519 private scalar is RANDOM
...
// custody" section, and vault-recovery-custody/design.md D1 addendum). If
...
// ── vault-agent-envelopes: keystore-key possession proof ──────────────────
 ...
 */
export const VAULT_KEYSTORE_POSSESSION_PREFIX = "r402s/v0/keystore-key-possession/v1\n";
...
export function computeKeystorePossessionProof(input: {
  ...
  const message = concatBytes(utf8ToBytes(VAULT_KEYSTORE_POSSESSION_PREFIX), utf8ToBytes(jcs));
  return toBase64url(hmac(sha256, shared, message));
}

/**
 * vault-multi-writer rev 47 (design D9) — the writer-signing-key
 ...
 * never verifies.
 */
export const VAULT_SIGNING_KEY_POSSESSION_DOMAIN = "r402s/v0/signing-key-possession/v1";
...
export function signingKeyPossessionMessage(fields: { principal_id: string; signing_pubkey: string; encryption_pubkey: string }): Uint8Array {
  const json = JSON.stringify({ domain: VAULT_SIGNING_KEY_POSSESSION_DOMAIN, principal_id: fields.principal_id, signing_pubkey: fields.signing_pubkey, encryption_pubkey: fields.encryption_pubkey });
