  ...
  const spec: ReleaseSpec = {
    ...
    functions: {
      replace: {
        ...
        credits: { source: "export default async function handler(req) { const paymentId = req.headers.get('x-run402-payment-id'); if (!paymentId) return new Response('payment missing', { status: 500 }); return Response.json({ ok: true, payment_id: paymentId }); }" },
