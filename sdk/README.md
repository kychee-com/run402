...
- **Deploy operation history is pageable.** Use `r.deploy.list({ project, limit, before, status, since, project_id, includeTotal })`; `cursor` is still accepted as an alias for `before`. Newer gateways return `has_more`, `next_cursor`, and optional `total`.
