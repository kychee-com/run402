...
`r.buzz.status()` capability-detects the community control plane. Typed `humanAdoptions`, `communityInstallations`, and `enrollments` namespaces expose lifecycle reads and explicit consent/decision mutations; `r.buzz.adopt(...)`, `install(...)`, and `enroll(...)` are goal-shaped aliases. `communityInstallations.discoverPublicDescriptors(subject)` performs unauthenticated Run402 discovery for a normalized Buzz community. The SDK generates idempotency keys when omitted, rejects nested secret-shaped fields before network access, never signs Buzz events, and represents enrollment authority only as expiring grants to named existing projects. Buzz itself remains unchanged: activation consumes an ordinary kind-1 approval plus existing NIP-11/NIP-43 evidence, while later descriptor lifecycle is Run402-owned. Gateway failures retain their stable code, exact repair `field`, complete `nextActions`, and `safeToRetry`; the SDK does not synthesize a generic Buzz edit-and-retry action. See the [Fizz/Honey lifecycle](../buzz/references/community-control-plane.md).
...
| `buzz` | Capability-detecting Buzz community control plane. `status`, goal aliases `adopt/install/enroll`, and typed `humanAdoptions/communityInstallations/enrollments` lifecycle methods preserve separate principals, explicit consent, bounded named-project grants, zero spend, drift, and scoped revocation. Buzz signing stays outside the SDK. |
