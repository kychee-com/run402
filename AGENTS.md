...
- **Allocation never changes deploy policy (design D3).** A vault's `gitvault_policy` stays whatever it already was after `init`/`openOrCreate` — `null` is the ordinary post-allocation shape, not a weakened one. `applyWithGitvault` (`sdk/src/node/gitvault-apply.ts`) gives that shape its own `{ kind: "ungated" }` mode: the plain deploy path runs untouched, but the result gains a `gitvault_policy_required` `next_actions` entry and a `GITVAULT_POLICY_UNSET` `warnings[]` entry on every such deploy — never a block, never an interactive prompt (vision principle 11).
