...
- **`cli/lib/deploy-v2.mjs`** — `run402 deploy apply`, `resume`, `list`, `events`, and `release <get|active|diff>` subcommands. Thin wrappers over the apply engine and scoped `r.project(id).apply.*` shape; `apply` supports `--final-only` as a `--quiet` alias and repeatable `--allow-warning <code>`.
