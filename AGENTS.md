...
- **Allocation-vs-policy (design D3 — client half shipped, gateway half pending).** The CLIENT never changes `gitvault_policy`, but TODAY'S GATEWAY still sets it to `required` at allocation — D3's decoupling is gateway work (repo-first-onramp task 3.1, run402-private) that has not shipped. Until it does, every client path that allocates prints `GITVAULT_ALLOCATION_POLICY_ADVISORY` (`cli/lib/gitvault-scaffold.mjs` — delete it, its four call sites, and this sentence when the gateway decoupling lands). `applyWithGitvault` (`sdk/src/node/gitvault-apply.ts`) already carries the forward shape: a vault with unset policy gets its own `{ kind: "ungated" }` mode — plain deploy path untouched, result gains a `gitvault_policy_required` `next_actions` entry and a `GITVAULT_POLICY_UNSET` `warnings[]` entry, never a block, never an interactive prompt (vision principle 11).
