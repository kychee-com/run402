...
```
@run402/sdk  (typed TypeScript kernel — focused namespaces including pay, snapshots, branches, and apply rehearsal)
   │
...
- **`errors.ts`** — core `Run402Error` hierarchy: `PaymentRequired`, `ProjectNotFound`, `Unauthorized`, `ApiError`, `NetworkError`, `Run402DeployError` (the v1.34+ structured envelope from the deploy state machine). The pay namespace adds `PaymentBuyerError` and post-settlement `PaymentPolicyError`. Never calls `process.exit`.
...
- **`namespaces/*.ts`** — One class per resource group (pay, projects, assets, functions, jobs, email, CI/OIDC, identityLinks, …). `identityLinks` represents protocol-discriminated public Nostr attribution for human and agent principals without accepting secret material or changing authority; agent creation remains the EOA-plus-kind-1 SDK ceremony, while human creation/revocation is browser-canonical. Lists preserve multiple active and revoked subjects. `r.pay.fetch` is the bounded arbitrary-URL x402 buyer; `requireReceipt` verifies wallet-rooted offer/receipt evidence and `payFetchResultToJson` is the canonical `x402-commerce-result.v1` serializer shared by CLI and MCP. Namespaces hold a `Client` and expose typed methods. The canonical apply primitive is implemented in **`namespaces/deploy.ts`**; the public-facing symbol is `r.project(id).apply`, with no public `r.deploy` surface. The internal engine is `_applyEngine`. Shared types live in `deploy.types.ts` — see "Unified Apply" below.
...
- **`node/*.ts`** — Node-only entry point (`@run402/sdk/node`). Wraps `core/` keystore + allowance into `NodeCredentialsProvider`. `createLazyPaidFetch()` resolves one deterministic payer source: explicit opaque `paymentSigner`, explicit `allowancePath`, the supplied provider's `readAllowance`, or the default Node provider. Auth and payer may differ intentionally; a selected payer never falls back to an ambient wallet, and `r.paymentPayer()` exposes only source/rail/public address/network provenance. Balance preflight uses bounded independent RPC failover without converting failure to zero. Automatic x402 calls commit a redacted mode-0600 record under the active profile's `payment-attempts/` directory before provider dispatch; `PaymentAttemptError` distinguishes safe pre-dispatch failures from ambiguous post-dispatch outcomes. `r.pay.fetch` may retain a proof in memory for an identical same-instance retry, but the reserved attempt-id header is sent only on the signed request with redirects disabled and the journal never stores keys, signed headers/proofs, bodies, query strings, or raw causes. Adds `fileSetFromDir(path)` and the deploy manifest adapter (`loadDeployManifest(path)`, `normalizeDeployManifest(input)`).
...
- **Strict/no-op validation lives in the SDK.** `Deploy.validateSpec` rejects unknown raw `ReleaseSpec` fields before normalization can drop them, and rejects project/base-only or empty nested specs with `MANIFEST_EMPTY` before hashing, uploading, or planning. String sources paired with known binary paths/MIME types fail locally with `BINARY_CONTENT_REQUIRES_BYTES`; binary files must be read without a text encoding and supplied as bytes or filesystem sources. The Node manifest adapter is strict too, so agent JSON typos such as `subdomain` or `sqlPath` do not become partial deploys.
...
- **Server-authoritative manifest digest.** The gateway returns the canonical digest in the plan response; `canonicalize.ts` is a UX helper only.
...
### Gitvault (host-blind encrypted Git remote, `r402s/v0`)

`r.gitvault` (`sdk/src/namespaces/gitvault.ts` + `gitvault.crypto.ts` + `gitvault.types.ts`, Node-only writes in `sdk/src/node/gitvault-*.ts`) owns every piece of protocol behavior — crypto core, keystore, creation journal, snapshot + capture, publication state machines, ref transactions, verification budget, repair, `fsck` (verify + materialize + explicit `pin_before`/`pin_after`/`local_state_changed`, `write:false` for `--no-write`), `access` (READ-ONLY recipients/coverage/local-TOFU-pin report), and `listByOrg` (the bulk vaults-by-org read `repos list` uses) — exactly once. `run402 repos …`, `git-remote-run402`, and the MCP tools (`repos_view`, `repos_list_heads`, `repos_fsck`) are thin adapters over it. `run402 gitvault <verb>` answers `COMMAND_MOVED`/`COMMAND_REMOVED` via `cli/lib/gitvault.mjs`'s tombstone dispatcher. `src/tools/gitvault-copy.test.ts` (the copy gate) enforces that the vault's three approved claim sentences and the terminal-loss statement are reproduced verbatim, never paraphrased, across every doc surface — see `documentation.md`'s gitvault row for the full file list and claim vocabulary.

- **The remote scaffold claims `origin` additively.** `Gitvault.scaffoldRemote` names the run402 remote `origin` when the repository has none yet — `git push origin main` works with zero run402-specific ceremony — and falls back to `run402` only when `origin` is already taken by something else, never touching an existing remote under either name. `cli/lib/gitvault-scaffold.mjs` is the shared fold-in `projects provision`, `run402 up`, and `run402 repos create` all call, so they report through identical `gitvault` / `gitvault_skipped` / `gitvault_error` summary keys. `run402 init`'s own git-remote step is separate and does not route through `scaffoldRemote`. When the scaffold has to `git init` because the directory is not a repository yet, it always requests branch `main` (`-b main`, with an `init` + `symbolic-ref HEAD refs/heads/main` fallback for pre-2.28 git) — never whatever `init.defaultBranch` happens to be locally — matching the `git push origin main` every doc surface teaches; an EXISTING repository's branch is never touched. `up.mjs`'s own separate `gitInitIfNeeded` mirrors the identical fallback (it does not route through `scaffoldRemote`).
- **Lazy allocation on first push.** `Gitvault.openOrCreate({ project_id, org_id?, ... })` resolves `repo_id` like `open()`, and — only when `org_id` is supplied and resolution fails — runs the six-stage creation journal inline before opening. `push({ org_id, onVaultCreated })` and `git-remote-run402`'s push path both compose it, so `git push origin main` and `repos snapshot` allocate a vault that does not exist yet, printing the one-shot recovery receipt to stderr the instant it lands. `findResumableGitvaultJournal` (`sdk/src/node/gitvault-creation-journal.ts`) makes this resumable: a second call after a crash resumes the SAME local `client_creation_id` rather than racing a fresh one. `list`/`fetch` in the remote helper stay pure reads and never allocate — an unresolved vault reports as an empty ref set, exactly what a fresh repository looks like to git.
- **Allocation never changes deploy policy.** Neither the client nor the gateway writes `gitvault_policy` at allocation (`setVaultPolicy` behind `PATCH /gitvault/v1/vaults/:vault_id/policy` is the only writer). A vault with unset policy gets its own `{ kind: "ungated" }` mode in `applyWithGitvault` (`sdk/src/node/gitvault-apply.ts`): plain deploy path untouched, result gains a `gitvault_policy_required` `next_actions` entry and a `GITVAULT_POLICY_UNSET` `warnings[]` entry, never a block, never an interactive prompt (vision principle 11).
- **`repos snapshot` is the capture verb; `push` means exactly `git push`.** `run402 gitvault push` (and every other `gitvault <verb>` spelling) answers `COMMAND_REMOVED` pointing at `git push` + `repos snapshot`. `SKILL.test.ts` bans every `run402 gitvault <verb>` spelling from reappearing as a taught usage example in both skill files. The underlying SDK method stays named `push` (`r.gitvault.push(...)`) — gitvault is infrastructure language; only the CLI-facing verb differs.
- **The progressive terminal-loss warning.** `gitvaultLossWarningTrip(record, now?)` / `gitvaultLossWarningTripped(trip)` / `gitvaultLossWarningMessage(trip)` (pure, exported from `gitvault.ts`) compute an OR-composite over `GITVAULT_LOSS_WARNING_THRESHOLDS` (`generations: 10, source_bytes: 10 MB, days_since_genesis: 14`, tunable in this one place). `status()` folds a tripped result into `warnings[]` as a `terminal_loss_risk` entry; `run402 repos view` and `run402 doctor` echo `warnings[]` verbatim, so neither has its own threshold logic to drift. There is deliberately no "resolved" check: V0-A cannot detect a second principal (another keystore, or later a human envelope) able to open the vault, so nothing here ever clears a tripped warning — the §0 terminal-loss statement itself never changes, only this warning's prominence and cadence do.
- **Named addressing, id-pinning, and push-to-create.** `run402::<org>/<name>` admits two forms in the same wire slot, discriminated by `gitvaultRemoteAddressForm(address)` (exported from `gitvault.ts`): id-form (org UUID + `prj_`-prefixed project id — real orgs/projects always satisfy BOTH at once) and slug-form (`run402::<org-slug>/<name>`, e.g. `run402::acme/my-notes`). `Gitvault.forRepo({org_slug, repo_name})` resolves slug-form via `GET /gitvault/v1/vaults?repo=<org-slug>/<name>`; `Gitvault.resolveAddress(address)` dispatches on form (pure read, no pin, no create). **Id-pinning:** `Gitvault.resolveOrCreateAddress` (the orchestrator the remote helper and `repos snapshot`'s slug-form `push` drive) pins the resolved `repo_id` into this checkout's LOCAL git config (`r402.repoId` + `r402.repoAddress`, via Node-only `sdk/src/node/gitvault-address.ts`'s `pinGitvaultRepo`/`readPinnedGitvaultRepo`) the first time a slug-form address resolves — every later open on that checkout follows the pin directly, skipping the network resolution and surviving a later rename of either half. Id-form pins nothing (a project id already never changes, so pinning buys it nothing). `run402 repos view` reports `pinned: {repo_id, resolved_from}`. **Push-to-create:** `sdk/src/node/gitvault-push-to-create.ts`'s `pushToCreateGitvault` drives the same six-stage creation journal (`gitvault-creation-journal.ts`), addressed by an optional `push_to_create: {org_slug, repo_name}` in place of `{org_id, project_id}` — the journal's `org_id`/`project_id` are `string | null` and trust-on-first-use pin from the allocation response. `POST /gitvault/v1/vaults {org_slug, repo_name}` creates the project and allocates the vault atomically server-side; on `REPO_CREATION_CONFLICT` (a concurrent pusher raced the atomic name-claim) the loser polls briefly (`pollFindVaultByProject`) and resolves to the winner's `repo_id`, proceeding as an ordinary push — the loser's work is not lost, it just wasn't the creator, never a transparent retarget. `SLUG_RELEASED` (`gitvaultSlugReleasedInfo(err)`) is never auto-followed. `Gitvault.push()` accepts an optional `address` to route through `resolveOrCreateAddress` instead of the ordinary project_id path. `run402 org slug <slug>` (`ScopedOrg.claimSlug`, `POST /orgs/v1/:org_id/slug`, owner-only, client-generated `Idempotency-Key`) claims/renames the org's slug; `run402 repos rename <name>` (`Projects.setRepoName`, `POST /projects/v1/:id/repo-name`, no fee) claims the per-org-unique address-form project name; `repos create` best-effort-claims one too when the owning org already has a slug. Both are CLI/SDK-only (`mcp: null`), the same "mutating verbs are CLI-only" law as `repos create/delete` below.
- **`run402 repos create|list|view|rename|delete|snapshot|policy|mirror|fsck|gc|access|recover`** — CLI + OpenClaw, plus three read-only MCP tools. `cli/lib/repos.mjs` (+ `openclaw/scripts/repos.mjs` re-export) is the family's home; `cli/lib/gitvault.mjs` is a tombstone dispatcher (`printKeystoreLocation` is its one surviving export) that answers every `gitvault <verb>` spelling with `COMMAND_MOVED` (naming the `repos` successor) or, for `push`/`reconcile`, `COMMAND_REMOVED` — see `cli/lib/command-manifest.mjs`'s `RESERVED_SUBCOMMANDS`/`SKIPPED_FAMILIES` for how the sync gate accounts for a family that dispatches nothing. `create` composes `projects.provision` + `gitvault.init` (allocation happens immediately, NOT lazily) + `gitvault.scaffoldRemote`, with zero deploy ceremony; `--project <id>` adopts an EXISTING project instead of provisioning, and `[name]` is inferred from an existing git remote's basename or the directory name when unambiguous (never a prompt; disagreement or a dead end is a structured `REPOS_NAME_AMBIGUOUS`/`REPOS_NAME_REQUIRED` error). `list` prefers `Gitvault.listByOrg` (the bulk vaults-by-org read, FROZEN shape) and falls back to a client-side per-project N+1 read only on a 404 (the route not yet shipped). `view` never passes `refs: true` to `Gitvault.status` — it is side-effect-free by construction. `fsck` is where materialization and pin advance happen, via `Gitvault.fsck({write, mirror})` (`write: false` is `--no-write`'s genuine audit mode — the SDK's `verifyToNewest`/`materialize` both accept a `persist` option that skips every `keystore.updateRepo` write while still computing the real answer). `gc` composes `compact` + `prune` (plan by default; `--submit` skips compact and submits only). `access` composes `listOrgEncryptionKeys` + `listEnvelopeRecipients` (both isomorphic HTTP reads) plus a best-effort, Node-only local TOFU-pin read via `Gitvault.access` — never wraps a key; the read reports per-recipient `envelope_state` and `stale_access` from the gateway's `desired[]` state. `access repair`, `access revoke-key`, and `access declare-exposure` are the epoch-rotation entry points (rev 42, D193–D203) — repair drives `Gitvault.rotateEpoch` against the D194 frozen watermark pair and is JSON-only. `delete` refuses `PROJECT_HAS_NON_REPO_RESOURCES` when `projects.get`/`getSchema`/`functions.list`/`secrets.list`/`subdomains.list` show a materialized mailbox/custom-domain/schema/functions/secrets/subdomains — using the SAME service-key credential `projects.delete` itself requires, so a 404 reads as genuinely absent and any other failure refuses as `unknown` (never a silent yes); `--force` overrides ONLY the separate vault-history confirmation. **No MCP tool exists for any mutating verb** — the same "mutating verbs are CLI-only by design" law documentation.md's gitvault row states (one-shot recovery receipt on `create`, irreversible `delete`); the three READ-ONLY tools (`repos_view`/`repos_list_heads`/`repos_fsck`) are the only MCP surface. When the best-effort address-form name claim doesn't land, `create` names WHY through a typed `next_actions` entry rather than a bare `address: null`: `claim_org_slug` (pointing at `run402 org slug <slug>`) when the owning org has no slug yet, `claim_repo_name` (pointing at `run402 repos rename <name> --project <id>`) when it does but this project's own claim missed; `create`'s response also carries a `push_repo` next_action naming the exact `git push` to run.
- **Gitvault verb targeting, when standing inside a repository.** `cli/lib/gitvault-target.mjs`'s `resolveGitvaultTarget`/`repoOwnGitvaultTarget` are the ONE shared resolver `run402 repos view|snapshot|policy|mirror|fsck|gc|access` (via `repos.mjs`'s `vaultTarget`) and `run402 doctor`'s gitvault check both compose — CLI-edge policy, not gitvault protocol, so it lives here rather than in the SDK. Order, highest first: an explicit `--repo`/`--project` flag (doctor has `--project` only — no `--repo`) > the id-pin (`readPinnedGitvaultRepo` — addresses by `repo_id` directly, no network) > the repo's own `run402`/`origin` remote (id-form parsed out of the address string for free; slug-form resolved via one read-only `gitvault.resolveAddress` call, never a pin, never push-to-create) > `RUN402_PROJECT_ID` env > the active project. An explicit flag that disagrees with the repo's own remote still wins, but prints a one-line stderr warning naming both first. `repos create` is deliberately NOT wired through this resolver — its `--project` addressing takes no `--repo`; it resolves via env/active project only. `Gitvault.status()`'s `remote` field checks both the `run402` and `origin` remote names.

- **`git-remote-run402` shares wallet resolution with the CLI.** `cli/lib/wallet-context.mjs` exports the resolution as pure, `fail()`-free primitives — `resolveWalletCore`/`enforceWalletExistsCore`, throwing `WalletSelectionError` (never `process.exit()`, which this file's own header warns can truncate a pending stdout write mid-protocol-stream) — that BOTH `cli.mjs`'s `resolveWallet`/`enforceWalletExists` wrappers (translate to `fail()`) and `git-remote-run402.mjs` (translate to `note()` + a non-throwing return) compose; one implementation, two error-reporting strategies. The binding walk starts from the RESOLVED REPOSITORY directory for `fetch`/`push` (not `process.cwd()` — the same fail-closed reasoning `resolveGitInvocationRepo` already applies to repository resolution: during `git clone` cwd is wherever clone was run FROM), falling back to cwd only for `list` (repository-free, same tier as `capabilities`/`option`). The allowance-missing remedy (`core/src/allowance.ts`'s "Back up the file and run `run402 init`") names the resolved wallet and how it was selected, and only keeps pointing at `init` when the bare default really was never overridden anywhere.
- **Active-project scope.** `NodeCredentialsProvider.setActiveProject`/`getActiveProject` (what `projects.provision`/`projects.use` write through) scope the active-project pointer by `{api_base, profile, principal}` — the wallet's real address. `core/src/keystore.ts`'s `getActiveProjectId`/`setActiveProjectId` — what EVERY CLI-facing reader goes through instead (`resolveProjectId`, `projects current`, `gitvault-target.mjs`'s active-project fallback) — derives the SAME principal from the current allowance before every read and write, so the two sides agree.
...
Current v3.0+ contract:

- `getUser` / `getUserId` / `getRole` are not valid bare exports — they throw `R402_AUTH_UNKNOWN_EXPORT` at runtime with a structured fix-it pointing at `auth.*`.
- `auth.sessions.createResponseFromIdentity({ provider, subject, proof, amr, createUser? })` is the only session-creation entry point — the platform verifies the proof end-to-end; agent code never holds a raw `userId`.
...
- **`getRoutedPaymentContext(req)`** (`@run402/functions` 3.7+) — confirmed x402 payment context for priced routed function requests. Returns `{ scheme, paymentId, amountUsdMicros, payer, network, asset, payTo, transaction, settledAt, idempotencyKey, deduplicated, delivery }` or `null`; `paymentId` remains the canonical app-side idempotency identity, while `delivery` distinguishes `first` from same-generation `replay` invocation.
...
- **`cli/lib/assets.mjs`** delegates `put` to `sdk.assets.put`, which routes through the unified-apply hero (`/apply/v1/plans` → `/content/v1/plans` → S3 PUT → commit). Resume semantics live at the apply-plan level (24h plan TTL). `--concurrency` and `--no-resume` flags are accepted but ignored.
...
**Export those two first and every later room call is flag-free** — which is what lets skills announce themselves in one line. (`RUN402_WALLET` applies on machines where another local profile is active; drop it if your session's own wallet is an org member. The explicit form is `--org <id> --room run402-dev --wallet platform-deploy` on every call.) Pick your own presence name — collisions get an honest suffix (Opus → Opus-2), unless you also pass `--task`, in which case a collision now qualifies your name from it first (`Opus` taken + `--task "mpp triage"` → `Opus-mpp-triage`, not `Opus-2`), and the output says why. A quiet session's presence now resumes automatically across an idle gap or a lost `.run402/` cache: every room call derives a stable session identity (Claude Code's own session id, Codex's own thread id, or a locally generated key persisted for this checkout — see `cli/lib/harness-context.mjs`) and the gateway revives the same presence under the same name no matter how long the ~1h TTL decayed. Two genuinely concurrent sessions never resume each other's presence. Override with `RUN402_SESSION_KEY`; `RUN402_PRESENCE_ID=<prs_…>` still pins an exact known presence when a session spans repos. Omit `--task` and the CLI best-effort fills it from your harness's own thread title — set `RUN402_NO_TASK_FROM_TITLE=1` to opt out. Claim shared areas before editing (`run402 claims create repo:<area>/** --note "why"` — advisory, never blocks; release when done). End your session with an `--ack` handoff message stating done / hot / next. Tool friction goes to `--thread dx`. Need a human? Send with `--importance high` — asking is always in-policy. The `.run402/` cache dir this creates is per-checkout and never committed.
...
```
run402 messages send "published run402 / @run402/sdk / run402-mcp <version>: <what changed>" || true
```
