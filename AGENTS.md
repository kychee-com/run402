...
- **Buzz integration** (`buzz/`) — self-contained `run402-buzz` skill with global CLI/profile setup, dual-proof fixtures/helper, security contract, verified no-deployment readiness, and an approval-gated contextual app offer. It never accepts or derives from a Nostr private key, and the public link grants no authority.
