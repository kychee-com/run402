...
- **`getUser(req)`** — verify the caller's JWT, returns `{ id, role, email }` or `null`. The `role` field is the JWT system role (`anon` / `authenticated` / `project_admin`), NOT the application role from a `requireRole` gate.
