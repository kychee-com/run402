...
run402 is a developer platform that ships Postgres databases, content-addressed CDN storage, static site hosting, Node 22 serverless functions, email, image generation, and KMS-backed Ethereum signing — provisioned by AI agents and paid for autonomously via x402 USDC on Base, MPP pathUSD on Tempo, MPP Lightning charge, or Stripe credits. Prototype tier is free on testnet.
...
- **`namespaces/*.ts`** — One class per resource group (pay, projects, assets, functions, jobs, email, CI/OIDC, identityLinks, …). `identityLinks` stages a public dual-signature Nostr attribution without accepting secret material or changing authority. `r.pay.fetch` is the bounded ordered HTTP payment buyer for x402, MPP Tempo, and explicit MPP Lightning charge; `requireReceipt` verifies merchant fulfillment and `payFetchResultToJson` is the shared rail-neutral serializer for CLI and MCP. Namespaces hold a `Client` and expose typed methods. The canonical apply primitive is implemented in **`namespaces/deploy.ts`**; the public-facing symbol is `r.project(id).apply`, with no public `r.deploy` surface. The internal engine is `_applyEngine`. Shared types live in `deploy.types.ts` — see "Unified Apply" below.
