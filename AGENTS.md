...
`r.gitvault` (`sdk/src/namespaces/gitvault.ts` + `gitvault.crypto.ts` + `gitvault.types.ts`, Node-only writes in `sdk/src/node/gitvault-*.ts`) owns every piece of protocol behavior — crypto core, keystore, creation journal, snapshot + capture, publication state machines, ref transactions, verification budget, repair — exactly once. `run402 gitvault …`, `run402 repos …`, `git-remote-run402`, and the MCP tools (`get_gitvault_status`, `list_gitvault_heads`, `verify_gitvault`) are thin adapters over it; `src/tools/gitvault-copy.test.ts` (the copy gate) enforces that the vault's three approved claim sentences and the terminal-loss statement are reproduced verbatim, never paraphrased, across every doc surface — see `documentation.md`'s gitvault row for the full file list and claim vocabulary.

- **The remote scaffold claims `origin` additively (design D1, repo-first-onramp).** `Gitvault.scaffoldRemote` names the run402 remote `origin` when the repository has none yet — `git push origin main` works with zero run402-specific ceremony — and falls back to `run402` only when `origin` is already taken by something else, never touching an existing remote under either name. `cli/lib/gitvault-scaffold.mjs` is the shared, non-fatal-in-every-branch fold-in `projects provision`, `run402 up`, and `run402 repos create` all call, so they report through identical `gitvault` / `gitvault_skipped` / `gitvault_error` summary keys instead of drifting copies; `run402 init`'s own 5b block predates it and is untouched. When the scaffold has to `git init` because the directory is not a repository yet, it always requests branch `main` (`-b main`, with an `init` + `symbolic-ref HEAD refs/heads/main` fallback for pre-2.28 git) — never whatever `init.defaultBranch` happens to be locally — matching the `git push origin main` every doc surface teaches; an EXISTING repository's branch is never touched. `up.mjs`'s own separate `gitInitIfNeeded` mirrors the identical fallback (it does not route through `scaffoldRemote`).
- **Lazy allocation on first push (design D2).** `Gitvault.openOrCreate({ project_id, org_id?, ... })` resolves `repo_id` like `open()`, and — only when `org_id` is supplied and resolution fails — runs the six-stage creation journal inline before opening. `push({ org_id, onVaultCreated })` and `git-remote-run402`'s push path both compose it, so `git push origin main` and `gitvault snapshot` allocate a vault that does not exist yet, printing the one-shot recovery receipt to stderr the instant it lands. `findResumableGitvaultJournal` (`sdk/src/node/gitvault-creation-journal.ts`) makes this resumable: a second call after a crash resumes the SAME local `client_creation_id` rather than racing a fresh one. `list`/`fetch` in the remote helper stay pure reads and never allocate — an unresolved vault reports as an empty ref set, exactly what a fresh repository looks like to git.
- **Allocation never changes deploy policy (design D3 — shipped end to end 2026-08-25).** Neither the client nor the gateway writes `gitvault_policy` at allocation (`setVaultPolicy` behind `PATCH /gitvault/v1/vaults/:vault_id/policy` is the only writer, run402-private task 3.1). A vault with unset policy gets its own `{ kind: "ungated" }` mode in `applyWithGitvault` (`sdk/src/node/gitvault-apply.ts`): plain deploy path untouched, result gains a `gitvault_policy_required` `next_actions` entry and a `GITVAULT_POLICY_UNSET` `warnings[]` entry, never a block, never an interactive prompt (vision principle 11). The interim `GITVAULT_ALLOCATION_POLICY_ADVISORY` and its four call sites were removed when the gateway half landed.
- **`gitvault snapshot` replaces `gitvault push` (design D5).** One verb per operation: `push` now means exactly one thing everywhere (`git push`); the capture lane is `snapshot`. `run402 gitvault push` remains a deprecation-warning-on-stderr alias for exactly one release, then is removed — `SKILL.test.ts` bans `run402 gitvault push --` reappearing as a taught usage example in both skill files. The underlying SDK method stays named `push` (`r.gitvault.push(...)`) — gitvault is infrastructure language; only the CLI-facing verb changed, per vision.md's own wallet/allowance naming split.
- **The progressive terminal-loss warning (design D7).** `gitvaultLossWarningTrip(record, now?)` / `gitvaultLossWarningTripped(trip)` / `gitvaultLossWarningMessage(trip)` (pure, exported from `gitvault.ts`) compute an OR-composite over `GITVAULT_LOSS_WARNING_THRESHOLDS` (`generations: 10, source_bytes: 10 MB, days_since_genesis: 14` — shipped defaults, tunable in this one place). `status()` folds a tripped result into `warnings[]` as a `terminal_loss_risk` entry; `run402 gitvault status` and `run402 doctor` echo `warnings[]` verbatim, so neither has its own threshold logic to drift. There is deliberately no "resolved" check: V0-A cannot detect a second principal (another keystore, or later a human envelope) able to open the vault, so nothing here ever clears a tripped warning — the §0 terminal-loss statement itself never changes, only this warning's prominence and cadence do.
- **Named addressing, id-pinning, and push-to-create (repo-first-onramp task 4, design D6).** `run402::<org>/<name>` admits two forms in the same wire slot, discriminated by `gitvaultRemoteAddressForm(address)` (exported from `gitvault.ts`): id-form (org UUID + `prj_`-prefixed project id — real orgs/projects always satisfy BOTH at once) and slug-form (`run402::<org-slug>/<name>`, e.g. `run402::acme/my-notes`). `Gitvault.forRepo({org_slug, repo_name})` resolves slug-form via `GET /gitvault/v1/vaults?repo=<org-slug>/<name>`; `Gitvault.resolveAddress(address)` dispatches on form (pure read, no pin, no create). **Id-pinning (task 4.5):** `Gitvault.resolveOrCreateAddress` (the orchestrator the remote helper and `gitvault snapshot`'s slug-form `push` drive) pins the resolved `repo_id` into this checkout's LOCAL git config (`r402.repoId` + `r402.repoAddress`, via Node-only `sdk/src/node/gitvault-address.ts`'s `pinGitvaultRepo`/`readPinnedGitvaultRepo`) the first time a slug-form address resolves — every later open on that checkout follows the pin directly, skipping the network resolution and surviving a later rename of either half. Id-form pins nothing (a project id already never changes, so pinning buys it nothing). `run402 gitvault status` reports `pinned: {repo_id, resolved_from}`. **Push-to-create (task 4.4/4.5):** `sdk/src/node/gitvault-push-to-create.ts`'s `pushToCreateGitvault` drives the SAME six-stage creation journal (`gitvault-creation-journal.ts`), now addressed by an optional `push_to_create: {org_slug, repo_name}` in place of `{org_id, project_id}` — the journal's `org_id`/`project_id` are `string | null` and trust-on-first-use pin from the allocation response (`checkAllocation`'s `expected.org_id`/`project_id` are optional, skipped while unset). `POST /gitvault/v1/vaults {org_slug, repo_name}` creates the project and allocates the vault atomically server-side; on `REPO_CREATION_CONFLICT` (the atomic name-claim raced a concurrent pusher) the loser polls briefly (`pollFindVaultByProject`, mirroring the gateway's own analogous poll) and resolves to the winner's `repo_id`, proceeding as an ordinary push — "the loser's work is not lost, it just wasn't the creator," never a transparent retarget. `SLUG_RELEASED` (`gitvaultSlugReleasedInfo(err)`) is NEVER auto-followed. `Gitvault.push()` accepts an optional `address` to route through `resolveOrCreateAddress` instead of the ordinary project_id path. `run402 org slug <slug>` (`ScopedOrg.claimSlug`, `POST /orgs/v1/:org_id/slug`, owner-only, client-generated `Idempotency-Key`) claims/renames the org's slug; `run402 repos name <name>` (`Projects.setRepoName`, `POST /projects/v1/:id/repo-name`, no fee) claims the per-org-unique address-form project name; `repos create` best-effort-claims one too when the owning org already has a slug. Both are CLI/SDK-only (`mcp: null`), the same "mutating verbs are CLI-only" law as `repos create/delete` below.
- **`run402 repos create|list|delete` — vault-only porcelain (design D8), CLI + OpenClaw ONLY.** `cli/lib/repos.mjs` (+ `openclaw/scripts/repos.mjs` re-export) composes `projects.provision` + `gitvault.init` (allocation happens immediately, NOT lazily — `create` is the moment the repo starts existing) + `gitvault.scaffoldRemote`, with zero deploy ceremony. `list` cross-references the org's projects against their live gitvault status client-side (sequential per project — no bulk "vaults by org" gateway read exists yet). `delete` refuses while the vault holds any admitted generation unless `--force` is passed, after naming the loss. **No MCP tool exists or will exist for this family** — the same "mutating verbs are CLI-only by design" law documentation.md's gitvault row states (one-shot recovery receipt on `create`, irreversible `delete`); `sync.test.ts` SURFACE rows for `repos_create`/`repos_list`/`repos_delete` all carry `mcp: null`, the operator command family is the precedent. When the best-effort address-form name claim doesn't land, `create` names WHY through a typed `next_actions` entry rather than a bare `address: null`: `claim_org_slug` (pointing at `run402 org slug <slug>`) when the owning org has no slug yet, `claim_repo_name` (pointing at `run402 repos name <name> --project <id>`) when it does but this project's own claim missed (kychee-com/run402#560).
- **Gitvault verb targeting, when standing inside a repository (repo-first-onramp follow-up, kychee-com/run402#559b/c/d).** `cli/lib/gitvault-target.mjs`'s `resolveGitvaultTarget`/`repoOwnGitvaultTarget` are the ONE shared resolver `run402 gitvault status|snapshot|policy|compact|prune|verify` (via `gitvault.mjs`'s `vaultTarget`) and `run402 doctor`'s gitvault check both compose — CLI-edge policy, not gitvault protocol, so it lives here rather than in the SDK. Order, highest first: an explicit `--repo`/`--project` flag (doctor has `--project` only — no `--repo` — as of kychee-com/run402#566) > the 4.38.0 id-pin (`readPinnedGitvaultRepo` — addresses by `repo_id` directly, no network) > the repo's own `run402`/`origin` remote (id-form parsed out of the address string for free; slug-form resolved via one read-only `gitvault.resolveAddress` call, never a pin, never push-to-create) > `RUN402_PROJECT_ID` env > the active project. An explicit flag that disagrees with the repo's own remote still wins, but prints a one-line stderr warning naming both first. `gitvault init` is deliberately NOT wired through this resolver (scope decision — it takes no `--repo`, and resolving a pin down to a project id would cost an extra network read this verb's existing `resolveProjectId`-only behavior never needed); it still resolves via env/active project only. Also fixed in the same pass: `Gitvault.status()`'s `remote` field checked ONLY a remote literally named `run402` — D1's own scaffold claims `origin` first, so the common case reported `remote: null` with the remote sitting right there; it now checks both conventional names.

- **`git-remote-run402` shares wallet resolution with the CLI (kychee-com/run402#558).** Before this it called `getSdk()` directly and ran no selection at all — a `.run402.json` binding, and even the global `wallets use` default, silently never reached it; only `RUN402_WALLET` env worked, so a bound checkout's first `git push` after a correctly-bound `run402 repos create` used the WRONG wallet. `cli/lib/wallet-context.mjs` now exports the resolution as pure, `fail()`-free primitives — `resolveWalletCore`/`enforceWalletExistsCore`, throwing `WalletSelectionError` (never `process.exit()`, which this file's own header warns can truncate a pending stdout write mid-protocol-stream) — that BOTH `cli.mjs`'s `resolveWallet`/`enforceWalletExists` wrappers (translate to `fail()`) and `git-remote-run402.mjs` (translate to `note()` + a non-throwing return) compose; one implementation, two error-reporting strategies. The binding walk starts from the RESOLVED REPOSITORY directory for `fetch`/`push` (not `process.cwd()` — the same fail-closed reasoning `resolveGitInvocationRepo` already applies to repository resolution: during `git clone` cwd is wherever clone was run FROM), falling back to cwd only for `list` (repository-free, same tier as `capabilities`/`option`). The allowance-missing remedy (`core/src/allowance.ts`'s "Back up the file and run `run402 init`") also stopped being unconditionally offered: it names the resolved wallet and how it was selected, and only keeps pointing at `init` when the bare default really was never overridden anywhere.
- **Active-project scope, fixed at the read side (kychee-com/run402#559a).** `NodeCredentialsProvider.setActiveProject`/`getActiveProject` (what `projects.provision`/`projects.use` write through) scope the active-project pointer by `{api_base, profile, principal}` — the wallet's real address. `core/src/keystore.ts`'s `getActiveProjectId`/`setActiveProjectId` — what EVERY CLI-facing reader goes through instead (`resolveProjectId`, `projects current`, `gitvault-target.mjs`'s active-project fallback) — called the underlying `profile-state.ts` functions with an EMPTY scope, resolving to a FIXED "unknown"-principal bucket shared by every wallet in a profile. `provision()`'s write was always correct; the bug was that once that "unknown" bucket was EVER populated by any principal-less write (the one-time legacy `projects.json` migration is the confirmed writer; any provision/`use` made before a wallet's allowance existed would be another), it permanently shadowed every later wallet-scoped activation for every CLI read — the flat fallback `profile-state.ts` also maintains never got a chance to be consulted. `keystore.ts`'s wrapper now derives the SAME principal from the current allowance before every read and write, so the two sides agree.
