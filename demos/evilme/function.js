...
async function handleGenerate(req) {
  ...
  const rateCheck = await database.sql(
...
${MANGA_STYLE_SUFFIX}`;
  ...
  const insertResult = await database.sql(
  ...
  try {
    ...
    if (!imageResp.ok) {
      ...
      if (imageData.image) {
        ...
        await database.sql(`UPDATE villains SET image_url = '${esc(publicImageUrl)}' WHERE id = ${row.id}`);
      }
    }
  } catch (imgErr) {
...
async function handleGallery(url) {
  ...
  const countResult = await database.sql(`SELECT count(*)::int AS total FROM villains`);
...
async function handleVote(req) {
  ...
  const ratings = await database.sql(
  ...
  await database.sql(`UPDATE villains SET elo_rating = ${newWinnerElo} WHERE id = ${winner.id}`);
  await database.sql(`UPDATE villains SET elo_rating = ${newLoserElo} WHERE id = ${loser.id}`);
