...
async function main() {
  ...
  if (tierRes.ok) {
    ...
    const tierBody = await tierRes.json().catch(() => ({})) as Record<string, unknown>;
    if (tierBody.current_tier_active) {
      console.log(`   Tier already active (expires ${tierBody.lease_expires_at})`);
    } else {
      console.error("Failed to subscribe:", tierRes.status, JSON.stringify(tierBody));
      process.exit(1);
    }
  }
  ...
  const subRes = await fetch(`${BASE_URL}/subdomains/v1`, {
    method: "POST",
    headers: authHeaders,
    body: JSON.stringify({ name: "cosmic", deployment_id: site.deployment_id }),
  });
  if (!subRes.ok) {
