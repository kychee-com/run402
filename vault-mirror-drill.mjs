#!/usr/bin/env node
/**
 * vault-mirror-and-recover — LIVE conformance drill (tasks 5.1 + 5.2).
 *
 * ============================================================================
 ...
 * vault exit ramp against reality rather than the memory-transport mock.
 *
 * WHY THIS EXISTS (design.md "Agent Trace", tasks.md 5.1). Every vault
 * unit test — including the ones this same change added for the key-envelope
 * addressing fix (task 6.5) — runs against `vault-memory-transport.test.ts`,
 ...
 *
 * COMMAND SURFACE: this drill drives `repos create --project <project_id>`,
 * `repos capture`, `repos mirror <dest>`, `repos mirror --backfill`,
 * `repos fsck --mirror` (report nested under .mirror), and
 * `repos recover <src> --out <dir>`.
 *
...
const WALLET_NAME = "vault-mirror-drill";
...
const DRILL_CONFIG_DIR = mkdtempSync(join(tmpdir(), "vault-drill-config-"));
const REPO_DIR = mkdtempSync(join(tmpdir(), "vault-drill-repo-"));
const MIRROR_DIR = mkdtempSync(join(tmpdir(), "vault-drill-mirror-"));
const RECOVER_DIR = mkdtempSync(join(tmpdir(), "vault-drill-recover-"));
...
async function main() {
  section("vault-mirror-and-recover — LIVE conformance drill");
  ...
  git(REPO_DIR, ["config", "user.name", "vault-mirror-drill"]);
  for (let i = 1; i <= 3; i++) {
  ...
  //
  // The successor to `vault init --project <project_id>`. There is no `--no-remote`
  ...
  // ── 6. Publish 3 distinct generations, one capture per commit. ──
  //
  // `repos capture` captures whatever the working tree looks like RIGHT
  // NOW — so to get 3 ascending, DISTINCT generations we replay the 3 commits
  // one at a time, capturing after each, rather than capturing once
  ...
  step("6. Publish 3 generations (one capture per commit)");
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    const snap = runCli(["repos", "capture", "--project", projectId, "--message", `capture ${i}`, ...WALLET_ARGS], { cwd: REPO_DIR, label: `repos capture #${i}` });
    console.log(`  capture #${i}: generation=${snap.json?.generation} form=${snap.json?.form}`);
    generations.push(snap.json?.generation);
  }
  assertTrue(generations.every((g) => typeof g === "string"), "every capture returned a generation", generations);
  ...
  // `repos fsck --mirror` is the successor to `vault mirror verify`: the
  ...
  assertTrue(
    recoverResult.json?.validity_not_freshness === VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT_EXPECTED,
    "recover carries the validity-not-freshness statement verbatim",
    recoverResult.json?.validity_not_freshness,
  );
  assertTrue(
    recoverResult.json?.keystore_still_required === VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT_EXPECTED,
    "recover carries the keystore-still-required statement verbatim",
    recoverResult.json?.keystore_still_required,
  );

  // Ref-exact comparison. `repos capture` (unlike `git push origin
  // <branch>`) always publishes under the SINGLE protocol-owned deploy ref
  // `refs/run402/deploys/latest` (`VAULT_DEPLOY_REF`, vault-snapshot.ts)
  ...
  assertEqual(recoveredRefs, expectedDeployRef, "recovered repo's refs match the deploy ref repos capture actually published, byte-exact");
  ...
  console.log("\n  Structural proof of zero network dependency: `Repos.recover` never");
  console.log("  calls `this.#client` in the SDK (sdk/src/namespaces/repos.ts) — it is");
...
// These are copied verbatim from sdk/src/namespaces/vault.crypto.ts so the
...
const VAULT_MIRROR_VALIDITY_NOT_FRESHNESS_STATEMENT_EXPECTED =
...
const VAULT_MIRROR_KEYSTORE_STILL_REQUIRED_STATEMENT_EXPECTED =
