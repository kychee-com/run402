...
A spending limit does not limit deploy/control-plane damage. Use one dedicated key per agent, prototype ownership by default, and human/company root ownership for production. Never place a founder identity, treasury, recovery key, or broad production owner key in a shell-capable agent.

If either key may be compromised:

1. revoke the run402 identity link;
2. revoke/rotate the affected run402 authenticator or scoped grant key as appropriate;
3. rotate the Buzz agent identity through Buzz's supported recovery process;
4. inspect deploy/audit history; and
...
An agent-owned prototype should move into a human/company organization before production. The transfer changes project ownership, not identity-link history. After adoption, retain attribution and grant the agent only the project/deploy capabilities it needs through a revocable, capped, expiring grant key.
