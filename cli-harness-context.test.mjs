...
describe("resolveHarnessLabels — env overrides first, then harness inference, null stays null", () => {
  ...
  it("a whitespace-only RUN402_PROGRAM override is treated as absent, falling through to inference", () => {
    const out = resolveHarnessLabels({ env: { [PROGRAM_OVERRIDE_ENV]: "   ", CLAUDE_CODE_SESSION_ID: "claude-1" } });
    assert.equal(out.program, "claude-code");
  });
