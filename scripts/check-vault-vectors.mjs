 ...
 * about to be replayed — BEFORE the test job runs (add-vault task 5.6b).
 *
 ...
 * should be there.
 *
 * Run:  node scripts/check-vault-vectors.mjs
 */
...
const dir = process.env.VAULT_VECTORS_DIR || join(root, "test-vectors", "r402s-v0");

function die(message) {
  console.error(`vault vectors: ${message}`);
  process.exit(1);
}

if (process.env.VAULT_VECTORS_OPTOUT) {
  die("VAULT_VECTORS_OPTOUT is set — the vector suites would skip. This is only legal on a developer machine, never in CI.");
}
...
for (const [label, path] of [["vectors.json", vectorsPath], ["hpke-interop/golden.json", goldenPath], ["CONTINUITY.json", continuityPath]]) {
  if (!existsSync(path)) die(`${label} is missing from ${dir}. The vendored set is incomplete — restore it from the private repo's docs/strategy/products/kygit/vectors/.`);
}
...
console.log(`vault vectors: ${count} vectors across ${classes.size} classes at protocol rev ${current.protocol_revision} — ${dir}`);
console.log("vault vectors: CONTINUITY.json digests match; the suites will replay, not skip.");
