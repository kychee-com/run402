/**
 * The resident helper engine (vault-persistent-helper D2–D6).
 *
...
export async function runDaemon() {
  ...
  const { prewarmVaultConnection, kickVaultConnection, predialVaultObjectStore } = await import("../sdk/dist/node/vault-prewarm.js");
  prewarmVaultConnection();
  const { runHelperSession } = await import("./remote-helper-session.mjs");
  // vault-object-host-predial task 2.2 — narrow, session-scoped imports
  ...
  // call (design D6).
  const { resolveGitInvocationRepo } = await import("../sdk/dist/node/vault-snapshot.js");
  const { readPinnedVaultRepo } = await import("../sdk/dist/node/vault-address.js");
  ...
  const server = net.createServer((socket) => {
    ...
    /**
     * vault-checkpoint-cadence design D2: `session.backgroundWork` — set
    ...
    socket.on("data", (chunk) => {
      ...
      for (;;) {
        ...
        if (msg.t === "hello") {
          ...
          // vault-first-op-premium (design point: pre-connect on session
          ...
          // NARROW half only (`kickVaultConnection`), never the full
          // `prewarmVaultConnection` — that also (re)warms the signer +
          ...
          // ~150-300ms/session (two live Base RPC probes a vault session
          // never needs) for zero benefit. Fire-and-forget by contract
          // (never throws, never awaited, never delays a session).
          const acceptedAt = Date.now();
          kickVaultConnection();
          // vault-object-host-predial task 2.2 — the addressed repo's
          ...
          // --absolute-git-dir`) then `readPinnedVaultRepo` (a local
          ...
          void (async () => {
            try {
              ...
              const pinned = await readPinnedVaultRepo(repo_dir);
          ...
          send({ t: "ready" });
          // vault-first-op-premium task 1.1: phase stamps for attribution
          // (session accept -> env ready -> first transport call, the last
          // covered by the existing RUN402_VAULT_TRACE per-op lines).
          ...
          // every other vault-trace line.
          if (process.env.RUN402_VAULT_TRACE === "1") {
            process.stderr.write(`vault-trace: daemon session-accept ${sessionsServed} at ${acceptedAt}\n`);
            process.stderr.write(`vault-trace: daemon env-ready ${sessionsServed} +${envReadyAt - acceptedAt}ms\n`);
          }
          // vault-checkpoint-cadence design D2: a push's auto-gc cycle
