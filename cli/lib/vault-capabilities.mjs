/**
 * vault-capabilities.mjs — the capability ledger for the KyGit
 * marketing surface (openspec/changes/kygit-page-truth-gate, design D1).
 *
 * THIS FILE IS MARKETING-CLAIM GROUND TRUTH. run402-private's page-truth
 * gate (`scripts/check-kygit-page-truth.mjs`) resolves the PUBLISHED
 * `run402` package, reads these flags out of the generated
 * `cli/vault-surface.json`, and fails the KyGit product page's build
...
export const VAULT_CAPABILITIES = {
  ...
  // `Repos#reconcileEnvelopeRecipients` (vault-human-envelopes task
  ...
  // change that flips this flag (vault-byo-primary-bucket, Phase 3).
  byo_live: true,
  ...
  // `repos capture` refuses to capture a dirty working tree
  // (`SNAPSHOT_DIRTY_TREE`) by default; `--allow-dirty` opts in explicitly.
  capture_dirty_default: "refuse",
  ...
  // from `capture_dirty_default` above) — two verbs, two defaults, one
