/**
 * The HOST-AGNOSTIC remote-helper session (vault-persistent-helper D1).
 *
 ...
 * (`cli/lib/vault-daemon.mjs`, which forwards a client's session into it).
 ...
 *
/**
 * `git-remote-run402` — the git remote helper for vault, so plain
 * `git clone|fetch|push run402::<org_id>/<project_id>` speaks to a host-blind
 * encrypted vault with no run402-specific git ceremony.
 *
 * ARCHITECTURAL LAW (vault-client-surface, "All protocol logic lives in the
 * SDK"): this file is a THIN ADAPTER over `r.repos`. It translates git's
 ...
 * inline (`r.repos.openOrCreate`, the SDK-owned primitive), prints the
 * one-shot recovery receipt and the keystore path to stderr, and then
 * completes the push. One command, no prior `vault init`. `git ls-remote`
 ...
 * (`org_id`/`prj_...`, unchanged: resolved via `r.repos.openOrCreate`
 * above) and slug-form (`run402::<org-slug>/<name>`, e.g.
 * `run402::acme/my-notes`) — discriminated by
 * `vaultRemoteAddressForm`. A slug-form remote resolves through
 * `r.repos.resolveOrCreateAddress`, which ALSO drives push-to-create on a
 ...
 *     which and why (see `chooseVaultHeadTargetForPush`). A HEALTHY HEAD
 ...
 *
 * DEGRADED READ MODE (vault-byo-primary-bucket, design D4). `list` and
 * `fetch` each wrap their own live gateway read in
 * `getSdk().repos.withDegradedRead` — on a NETWORK-CLASS failure (never a
 ...
 * (`vaultDegradedReadNote`). Local trust pins never advance past what the
 * mirror copy itself chain-verifies (the fallback engine never touches the
 * live vault's own keystore pins at all — see `vault-degraded-read.ts`'s
...
// it happens once at daemon boot (vault-startup-amortization D1).
...
const { vaultRemoteAddressForm, vaultSlugReleasedInfo, parseVaultRemoteUrl, vaultDegradedReadNote } = await isoModP;
const { VAULT_R402_REF_NAMESPACE, hardenedGit, resolveGitInvocationRepo, readPinnedVaultRepo, pinVaultRepo, readVaultRestoreMarker, readVaultAutoGcThreshold, predialVaultObjectStore } = await nodeModP;
const { walletFile, projectCredentialsFile, profileStateFile } = await configModP;

/**
 * Per-session SDK construction cache (vault-first-op-premium task 2.2).
 *
...
function describeError(err) {
  ...
  const released = vaultSlugReleasedInfo(err);
...
function resolveRemoteAddress(argv) {
  for (const raw of [argv[1], argv[0]]) {
    ...
    const direct = parseVaultRemoteUrl(raw);
    ...
    const prefixed = parseVaultRemoteUrl(`run402::${raw}`);
...
export const R402_PROTECTED_REF_NAMESPACE_REASON = `R402_PROTECTED_REF_NAMESPACE: ${VAULT_R402_REF_NAMESPACE}* is client-local bookkeeping maintained by fetch, not push — push branches instead.`;

/**
 * D4: `refs/r402/*` is client-local (design D1/D2's local ref bookkeeping) —
 * never advertised for push, never part of a `VaultRefTransaction`. Split
...
export function partitionProtectedRefPushes(specs) {
  ...
  for (const spec of specs) {
    if (spec.dst.startsWith(VAULT_R402_REF_NAMESPACE)) refused.push(spec);
 ...
 */
export function chooseVaultHeadTargetForPush({ baseHeadTarget, baseRefs, updates, localHeadRef }) {
...
/**
 * vault-checkpoint-cadence design D1 — the pure post-push auto-gc
 ...
 * `vaultCheckpointStaleness`'s own `since >= THRESHOLD` shape).
 */
 ...
 * the bug: `VaultHandleOptions.repo_dir` defaults to
 ...
 * `VAULT_REPO_DIR_REQUIRED`, silently degrading to the advisory on
...
async function main(argv, { onBackgroundWork } = {}) {
  // vault-connection-amortization (bench P5) note: the prewarm now fires
  // at the module TOP, before the SDK graph loads (vault-startup-
  ...
  const addressForm = vaultRemoteAddressForm(address);
  ...
  async function requireRepo() {
    if (!resolvedRepo) {
      ...
      // vault-object-host-predial task 2.1 (trigger (a), the in-process
      ...
      void (async () => {
        try {
          const pinned = await readPinnedVaultRepo(resolvedRepo.repo_dir);
          if (pinned) predialVaultObjectStore(pinned.repo_id);
        } catch {
  ...
  /**
   * One materialize per push session (vault-client-round-trips design
   ...
   * vault-client-round-trips design D4 widening the same mechanism to
   * id-form) through `repos.resolveOrCreateAddress` with
  ...
  const openVault = async (repoDir) => {
    const result = await getCachedSdk().repos.resolveOrCreateAddress({ address, allow_create: false, ...(repoDir ? { repo_dir: repoDir } : {}) });
   ...
   *
   * Id-form (vault-client-round-trips design D4): a PINNED repo_id means
   ...
   * With NO pin yet, this is unchanged: `repos.openOrCreate` runs its
  ...
  async function openOrCreateVault(repoDir) {
    if (addressForm === "id" && repoDir) {
      ...
      if (pinned) {
        const result = await getCachedSdk().repos.resolveOrCreateAddress({ address, repo_dir: repoDir, allow_create: false });
        return result.handle.vault;
      }
    }
    const result =
      addressForm === "id"
        ? await getCachedSdk().repos.openOrCreate({ ...target, repo_dir: repoDir })
        : await getCachedSdk().repos.resolveOrCreateAddress({ address, repo_dir: repoDir, allow_create: true });
    if (addressForm === "id" && repoDir) await pinVaultRepo(repoDir, result.handle.repo_id, undefined, { project_id: target.project_id, org_id: target.org_id });
    if (!result.found && result.created) {
      ...
      try {
        const { getVaultKeystoreRoot, VAULT_MIRROR_SETUP_HINT } = await import("#sdk/node");
        // vault-mirror-default: every allocation teaches the mirror door
        // beside the recovery receipt, lazy push-to-create included.
        note(VAULT_MIRROR_SETUP_HINT);
        note(`keystore: ${getVaultKeystoreRoot()} — back this up; whole-machine or whole-keystore loss is terminal for vault history until human envelopes ship`);
      } catch {
  ...
  async function runList() {
    ...
    // vault-session-state-reuse design D2: read the restore MARKER —
    ...
    if (repoDir) {
      try {
        fetchMarker = await readVaultRestoreMarker(repoDir);
      } catch {
        fetchMarker = null;
      }
    }
    // vault-restore-recipe design D2/D6: a marker-absent fetch target is,
    ...
    try {
      ...
      // vault-byo-primary-bucket (design D4): the OPEN itself needed the
      ...
      const fallback = repoDir ? await getCachedSdk().repos.degradedOpenFallback?.({ address, repo_dir: repoDir, error: err }).catch(() => null) : null;
      if (!fallback) throw err;
      outcome = fallback;
    }
    let vault = opened?.vault;

    // vault-byo-primary-bucket (design D4, task 3.4 — mirror half): the
    ...
    const attemptLive = async () => {
      try {
        ...
        const recovered = repoDir && opened.resolution?.offline
          ? await getCachedSdk().repos.recoverStalePin({ address, repo_dir: repoDir, resolution: opened.resolution, error: err })
          : null;
    ...
    if (!outcome) {
    try {
      outcome = await getCachedSdk().repos.withDegradedRead({ attemptLive, keystore: opened.keystore, repo_id: opened.repo_id, out_dir: repoDir });
    } catch (err) {
    ...
    if (outcome.degraded) {
      ...
      note(vaultDegradedReadNote(outcome.result.source));
      ...
      // vault-session-state-reuse design D1: `fetchState`/`fetchMarker`
  ...
  async function runFetch(batch) {
    ...
    // vault-session-state-reuse design D1/D4: reuse THIS session's `list`
    ...
    // read, unchanged.
    //
    // vault-byo-primary-bucket (design D4, task 3.4 — mirror half):
    // either path's actual chain/payload read is the "attemptLive" that
    // `withDegradedRead` wraps — `repos.restore(...)` is exactly `open()`
    ...
    if (shared) {
      ...
      let handle = null;
      try {
        handle = await getCachedSdk().repos.open({ ...target, repo_dir: repoDir });
      } catch (err) {
        // vault-byo-primary-bucket (design D4): the open needed the
        // gateway — see runList's identical fallback.
        const fallback = await getCachedSdk().repos.degradedOpenFallback?.({ address, repo_dir: repoDir, error: err }).catch(() => null);
        if (!fallback) throw err;
        outcome = fallback;
      }
      if (handle) {
    ...
    if (!outcome) {
      outcome = await getCachedSdk().repos.withDegradedRead({
        attemptLive,
  ...
  /**
   * vault-checkpoint-cadence (design D1/D2) — the post-push auto-gc
   ...
   * disables — `readVaultAutoGcThreshold`, design D1's "rides the
   ...
   *
   * `compact()` (namespace, `sdk/src/namespaces/repos.ts`) already owns
   * the compaction headroom grant's open/close and single-flight refusal
   * (`VAULT_COMPACTION_IN_PROGRESS`) — this function only decides WHETHER
  ...
  async function maybeRunAutoGc({ repoDir, generationsSinceCheckpoint }) {
    let threshold;
    try {
      threshold = await readVaultAutoGcThreshold(repoDir);
    } catch {
    ...
    const runCycle = async () => {
      ...
      const checkpoint = await sdk.repos.compact({ ...compactionTarget });
      const prune = await sdk.repos.prune(compactionTarget); // PLAN only — never `submit`; see compact()'s own grant-close doc comment for why prune needs no headroom of its own
    ...
    note(`vault: compacting (${generationsSinceCheckpoint} generations since checkpoint)…`);
    try {
      await runCycle();
    } catch (err) {
      note(`vault: auto-compaction stopped short — ${describeError(err)} — run \`run402 repos gc\` to finish it by hand.`);
    }
  }

  async function runPush(batch) {
    const specs = batch.map(parsePushSpec);
    // vault-checkpoint-cadence: set ONLY after a successful admission
    ...
    try {
      ...
      // vault-session-state-reuse design D4: this admission just advanced
      ...
      else if (copies?.mirror_push?.outcome === "failed") note(`mirror: mirroring this capture FAILED (the push is unaffected) — ${copies.mirror_push.error ?? "see run402 repos fsck --mirror"}`);
      ...
      // vault-clone-scaling (P3): advisory only — informational, always
      ...
      for (const spec of allowed) out(`ok ${spec.dst}`);
      // vault-checkpoint-cadence: captured here (never in the catch
      ...
      // Force-spelling truth (vault-force-spelling-and-pin-fold): render
    ...
    endBlock();
    // vault-checkpoint-cadence design D1/D2: strictly AFTER the push's
...
/**
 * Run one remote-helper session to completion (vault-persistent-helper
 ...
 *
 * `onBackgroundWork` (vault-checkpoint-cadence design D2): when supplied
 * (the DAEMON host only — see `vault-daemon.mjs`), a successful push's
