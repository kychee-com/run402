/**
 * Shared "which vault does this repo mean" resolution for `run402 repos`
 * and `run402 doctor`'s vault check. When a verb runs standing inside a
 ...
 * not vault protocol behavior — the same class of concern
 * `wallet-context.mjs` owns for wallet selection. The protocol reads it
 * composes (`readPinnedVaultRepo`, `resolveAddress`) already live once in
 ...
 * vault check needs to report "skipped" gracefully when nothing resolves
 ...
 * (vault.mjs's own verbs) get it by falling back to `resolveProjectId`
 * themselves when this returns `null` — see `vaultTarget` in vault.mjs.
 */
...
export async function repoOwnVaultTarget(repoDir) {
  ...
  const { hardenedGit, readPinnedVaultRepo } = await import("#sdk/node");
  ...
  for (const name of ["run402", "origin"]) {
    ...
    try {
      ...
      const resolved = await getSdk().repos.resolveAddress(address);
...
/**
 * Resolve `{ repo_id?, project_id? }` for a vault verb that addresses a
...
export async function resolveVaultTarget({
  ...
  const own = needsOwn ? await repoOwnVaultTarget(repoDir) : null;
