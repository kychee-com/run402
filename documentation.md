...
|:------:|------|----------|--------------|--------------|
| 🟢 | [`README.md`](README.md) | Humans on GitHub + the [`run402-mcp` npm page](https://www.npmjs.com/package/run402-mcp) | Five-interface overview, the modern patterns (paste-and-go AssetRef, expose manifest, deploy-dir, deploy release inventory/diff reads, GitHub Actions OIDC, in-function helpers), MCP install matrix per host | A tool is added/removed; the patterns evolve; a package joins/leaves lockstep |
...
| 🟢 | [`SKILL.md`](SKILL.md) | MCP-host agents (Claude Desktop, Cursor, Cline, Claude Code) — they have `run402-mcp` tools loaded | Teaches the platform via **MCP tool names** in natural-language framings. Quickstart, the four patterns, release observability, tools by category, troubleshooting matrix. Frontmatter `install: run402-mcp`. | A tool is added/removed; a pattern changes; a troubleshooting case is discovered |
| 🟢 | [`openclaw/SKILL.md`](openclaw/SKILL.md) | OpenClaw script-runtime agents | Same content as `SKILL.md` but every verb is `run402 <…>` (CLI shape), including `run402 deploy release <get|active|diff>`. Frontmatter `install: run402` (the CLI). | A CLI subcommand is added/removed; a pattern changes; a troubleshooting case is discovered |
