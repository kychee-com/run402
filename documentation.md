...
|:------:|------|----------|--------------|--------------|
| 🟢 | [`README.md`](README.md) | Humans on GitHub + the [`run402-mcp` npm page](https://www.npmjs.com/package/run402-mcp) | Five-interface overview, the modern patterns (paste-and-go AssetRef, expose manifest, deploy-dir, static route targets + URL diagnostics, deploy release inventory/diff reads, GitHub Actions OIDC, in-function helpers), MCP install matrix per host | A tool is added/removed; the patterns evolve; a package joins/leaves lockstep |
...
| 🟢 | [`SKILL.md`](SKILL.md) | MCP-host agents (Claude Desktop, Cursor, Cline, Claude Code) — they have `run402-mcp` tools loaded | Teaches the platform via **MCP tool names** in natural-language framings. Quickstart, the four patterns, static route targets + `deploy_diagnose_url`, release observability, tools by category, troubleshooting matrix. Frontmatter `install: run402-mcp`. | A tool is added/removed; a pattern changes; a troubleshooting case is discovered |
| 🟢 | [`openclaw/SKILL.md`](openclaw/SKILL.md) | OpenClaw script-runtime agents | Same content as `SKILL.md` but every verb is `run402 <…>` (CLI shape), including `run402 deploy release <get|active|diff>`, `run402 deploy diagnose`, and `run402 deploy resolve`. Frontmatter `install: run402` (the CLI). | A CLI subcommand is added/removed; a pattern changes; a troubleshooting case is discovered |
...
**Add or change stable static asset identity / public URL diagnostics** | SDK deploy types + `Deploy.resolve` helpers + scoped wrapper, static route target validation, Node manifest normalization, CLI `run402 deploy diagnose` / `run402 deploy resolve`, MCP `deploy_diagnose_url`, OpenClaw parity, `sync.test.ts` (`SURFACE`, `SDK_BY_CAPABILITY`, docs drift guards), [sdk/llms-sdk.txt](sdk/llms-sdk.txt), [sdk/README.md](sdk/README.md), [cli/llms-cli.txt](cli/llms-cli.txt), [cli/README.md](cli/README.md), [llms-mcp.txt](llms-mcp.txt), [SKILL.md](SKILL.md), [openclaw/SKILL.md](openclaw/SKILL.md), [openclaw/README.md](openclaw/README.md), [README.md](README.md), [AGENTS.md](AGENTS.md), plus private `site/openapi.json`, `site/llms-full.txt`, `site/updates.txt`, and `site/humans/changelog.md`
