...
|:------:|------|----------|--------------|--------------|
| 🟢 | [`README.md`](README.md) | Humans on GitHub + the [`run402-mcp` npm page](https://www.npmjs.com/package/run402-mcp) | Five-interface overview, the modern patterns (paste-and-go AssetRef, expose manifest, deploy-dir, GitHub Actions OIDC, in-function helpers), MCP install matrix per host | A tool is added/removed; the patterns evolve; a package joins/leaves lockstep |
...
| 🟢 | [`sdk/llms-sdk.txt`](sdk/llms-sdk.txt) | AI agents fetching <https://run402.com/llms-sdk.txt> | Comprehensive SDK reference. Two entry points, all 20 namespaces with method signatures, CI/OIDC helpers and credential semantics, the modern patterns, full `Run402Error` hierarchy with `Run402DeployError` envelope, `ReleaseSpec` shape, stability + lockstep notes. Served via private-repo deploy pulling this file | Any SDK method or namespace added/changed; `ReleaseSpec` shape changes; new error class lands |
...
| 🟢 | [`sdk/README.md`](sdk/README.md) | TypeScript devs / agents using the SDK; the [`@run402/sdk` npm page](https://www.npmjs.com/package/@run402/sdk) | Two entry-point install matrix (Node vs isomorphic), full 20-namespace catalog table, paste-and-go AssetRef pattern, unified `r.deploy.apply` v1.34+ primitive (with three layers + `fileSetFromDir`), CI/OIDC helpers, full error hierarchy including `Run402DeployError`, stability + lockstep notes | New SDK namespace / method; new error class; entry-point changes |
