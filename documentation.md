...
| 🟡 | [`sdk/README.md`](sdk/README.md) | TypeScript devs / agents using the SDK; the [`@run402/sdk` npm page](https://www.npmjs.com/package/@run402/sdk) | Isomorphic vs Node entry-point quickstart + error hierarchy. **Missing:** AssetRef paste-and-go pattern, the unified `r.deploy.apply` v1.34+ primitive, namespace catalog | Add: AssetRef section, `deploy.apply` / `deploy.start` / `fileSetFromDir` examples, namespace catalog, link to llms-cli.txt for matching CLI commands |
| 🔴 | `functions/README.md` | TypeScript devs / agents using `@run402/functions`; the [`@run402/functions` npm page](https://www.npmjs.com/package/@run402/functions) | **File does not exist.** `functions/package.json` declares it in `files[]`, so the published npm package ships with **no readme** — the npm page is empty | Create the file: `db(req)` / `adminDb()` / `getUser()` / `email` / `ai` reference, deploy-time bundling note, autocomplete-only install hint |
| 🟡 | [`openclaw/README.md`](openclaw/README.md) | Humans browsing the `openclaw/` folder on GitHub | OpenClaw skill install instructions and an MCP-vs-OpenClaw comparison table. **Stale:** doesn't reflect the SKILL.md split, doesn't mention v1.32+ patterns | Refresh after every major skill or install-flow change |
