...
- **`LinkWalletPoolImplications` names its org counters `org_*`.** `org_api_calls_current` and `org_storage_bytes_current` (were `organization_api_calls_current` / `organization_storage_bytes_current`), matching the gateway release that renames them on the wire and the tier-quota-pool spec.
- **The organization id is `org_id` in the last wire types that said `organization_id` (run402-private#730).** `AuthoritySnapshot` (an actor snapshot's `authority`: `creator`, `actor`, `initiated_by`) and `RedeemVoucherResult` carry `org_id`; the MPP Lightning `ORGANIZATION_SELECTION_REQUIRED` candidates are read from `details.org_ids`, and the SDK's re-thrown error (and `run402 image generate`'s envelope) report `details.org_ids` / `details.matched_org_ids`. Pairs with the gateway release that renames them on the wire.
- **Removed CLI spellings are gone, not redirected (pre-launch rule).** `run402 message`, `run402 notifications <any>`, `run402 escalations contacts`, `run402 projects info` and `run402 projects keys` no longer answer a `COMMAND_REMOVED` / `COMMAND_MOVED` redirect; they are unknown commands, answered with `UNKNOWN_SUBCOMMAND` (or the unknown-command did-you-mean) like any other typo. Both error codes are deleted. Use `run402 feedback send`, `run402 deliveries` / `contacts` / `subscriptions`, `run402 contacts`, and `run402 credentials project-keys status|export`. Usage lines and next actions that still named `run402 notifications …` now name `run402 contacts …` and `run402 subscriptions rm`.
- **SQL and service-key REST use the project-scoped routes.** `r.projects.sql` (and `r.project(id).projects.sql`) calls `POST /projects/v1/:project_id/sql` and returns a typed `SqlResult` `{ status, schema, rows, row_count, fields, statements, warnings }`; the new `r.projects.sqlBatch(id, statements, { transaction: "all" | "each" })` calls `POST /projects/v1/:project_id/sql/batch` (one statement per entry; `all` rolls back on the first failure with `SQL_BATCH_STATEMENT_FAILED`, `each` reports each statement). Service-key `rest`/`restResponse` calls `/projects/v1/:project_id/rest/*`. The gateway is retiring `/projects/v1/admin/:project_id/sql` (for the service key) and `/admin/v1/rest/*`; older clients keep working until they are retired. CLI: `run402 projects sql --batch <file.json> [--transaction all|each]`. `warnings[]` on a SQL result never changes it: `SCHEMA_CHANGE_OUTSIDE_MIGRATION` (a schema change on a released project belongs in a migration) and `MULTI_STATEMENT_TEXT_BODY` (use the batch). `run402 doctor` adds `retiring_routes` (your projects' calls to a retiring route in the last 7 days, with the fix) and the source-scan warning `SERVICE_KEY_IN_USER_REQUEST` (`adminDb()` in a file that reads the current user; use `db(req)`).
- **MCP tool arguments are strict.** An argument a tool does not declare is refused before the tool runs, instead of silently dropped: `UNKNOWN_ARGUMENT`, with an `edit_request` next action naming the `unknown` arguments, the closest declared name for each (`did_you_mean`), and the `accepted` ones. A wrongly typed argument is `INVALID_ARGUMENTS` with `details.issues[]`. Both carry `structuredContent` like every other error. `RUN_UNKNOWN_MEMBER` now also answers a member reached from the wrong parent: `r.project(id).sql` suggests `r.project(…).projects.sql` and `r.projects.sql`, and the reported `path` shows the whole chain.
- **MCP `run` values page by item.** A large value is stored by item (an array's elements; an object's largest top-level array, such as a SQL result's `rows`, with its other fields kept whole; an object's entries; a string's lines) and its leading whole items arrive in `value_window` `{ path, items, rest? }`, in the text and in `structuredContent`, instead of nothing. `shown` and `total` count items, and `expand_result` pages by item, so a page never ends mid-row. A snippet that reaches an `r.` path the SDK does not have fails `RUN_UNKNOWN_MEMBER` naming the closest members (`did_you_mean` on its next action) instead of "is not a function". The server no longer prints Node's `stripTypeScriptTypes` ExperimentalWarning on stderr. `projects.sql` types the gateway's per-statement breakdown (`statements[]`), and `row_count` counts the rows returned.
- **MCP results are structured.** Every `run402-mcp` tool declares an `outputSchema` and returns `structuredContent` beside its text block, on success and on error. The object has `status: "ok" | "error"`: `run` keeps its envelope; `up`, `deploy`, `status`, `whoami`, and `doctor` carry the SDK object under `result` (and `deploy` its progress events under `events`); `expand_result` and `docs` carry their page. Every error, on every tool, is `error: { code, message, next_actions, category?, retryable?, safe_to_retry?, http_status?, mutation_state?, trace_id?, details? }` (a `deploy` error adds `phase`, `resource`, `operation_id`, `plan_id`, `fix`, `logs`); local failures use `PROJECT_NOT_FOUND`, `PROJECT_CREDENTIAL_NOT_FOUND`, `WALLET_NOT_FOUND`, `NETWORK_ERROR`, `RESULT_REF_NOT_FOUND`, `DOCS_TOPIC_NOT_FOUND`, or `INTERNAL_ERROR`. The schemas are open, so a new SDK or gateway field never fails a call. The fenced JSON in the text is the structured object, so the fixed tools' JSON now wraps the SDK object under `result`. **Changed:** `deploy` reports a payment the wallet cannot cover as an error result (`isError: true`, the SDK's code and `next_actions`), as `run` already did. Error guidance no longer names deleted MCP tools.
- **MCP is eight tools; `run` executes SDK snippets (decision 18, 2026-09-22).** `run402-mcp` registers exactly `up`, `deploy`, `status`, `whoami`, `doctor`, `docs`, `run`, and `expand_result`; the one-to-one mirror (216 tools), `RUN402_MCP_PROFILE`, and the buyer profile are deleted, with no alias. `run` takes `{ code, timeout_seconds? }` and executes TypeScript against `r`, the Node SDK client, in QuickJS compiled to WebAssembly (no filesystem, `process`, `fetch`, timers, or imports inside): `r` is a recorder whose awaited chains replay on the host against a `surface: "sandbox"` client. The result is `{ status, value, value_ref, shown, total, logs, logs_ref, calls, duration_ms, wallet, error? }`: the value is stored whole (200 lines inline, `expand_result` pages the rest), `calls[]` lists every SDK call, a timeout (60 s default, 300 s max) still lists the calls that completed, SDK errors pass through with their own `code` and `next_actions`, and the sandbox's own failures are `RUN_SYNTAX_ERROR`, `RUN_TIMEOUT`, `RUN_MEMORY_EXCEEDED` (64 MB), `RUN_VALUE_NOT_SERIALIZABLE`, `RUN_VALUE_TOO_LARGE` (4 MB), `RUN_ARGUMENT_NOT_CLONEABLE`, `RUN_EXCEPTION`, and `RUN_BUSY` (four concurrent runs). A method that returns or consumes a one-time secret refuses inside `run` with `SECRET_REQUIRES_CLI` and a `run_cli_command` next action naming the exact CLI command. `docs` serves the SDK reference and the `run` primer copied into the package at build (`topic`, `search`). `status` returns `r.status()`, `doctor` (new) `r.doctor()`, and `whoami` `r.orgs.whoami()` with no input. The server resolves its wallet as the CLI does (environment, nearest binding, global default). `run402-mcp` now needs Node.js 22.13 or later and depends on `quickjs-emscripten` and `acorn` (both MIT). The CLI and SDK surfaces are unchanged.
- **Grant key, not delegate (decision 2, 2026-09-22).** A grant is the permission row; a grant key is a credential minted against exactly one grant, narrower than it, spend-capped, expiring, revocable on its own, never an owner. The `delegates` family is deleted from every surface. SDK: `r.delegates` / `Delegates` and its types are gone; `r.grants` gains `list` (grants with `keys[]` nested), `create(..., { key })` (first key in the grant's own transaction, `key.token` returned once), `createKey`, `revokeKey`, `rotateKey`, with scoped `r.project(id).grants.*` wrappers. `delegate-credentials.ts` is `grant-key-credentials.ts`: `GRANT_KEY_CREDENTIALS`, `isGrantKeyCredentials`, `GRANT_KEY_ENV` (`RUN402_GRANT_KEY`, was `RUN402_DELEGATE_TOKEN`), `grantKeyFromEnv`; the Node `grantKey` option and `authMode: "grant_key"` replace `delegateToken` / `"delegate"`; project auth mode `grant_key`, identity `AuthoritySnapshot` `{ kind: "grant_key", grant_key_id }`, escalation `raised_by.grant_key_id`. CLI: `run402 grants create <wallet> --capability <cap> [--key [--kind] [--scope] [--spend-cap]] [--expires-at]`, `grants list`, `grants revoke <grant_id>`, `grants revoke-key <key_id>`, `grants rotate-key <key_id>`; the key token prints once. `deploy list` with a grant key fails `GRANT_KEY_SCOPE_INSUFFICIENT`. MCP: `list_project_grants` and `revoke_project_grant_key`; minting and rotating a key stay CLI/SDK-only. Wire: `/projects/v1/:project_id/grants` (GET, POST with optional `key`), `/grants/:grant_id/keys`, `/grant-keys/:key_id` (DELETE), `/grant-keys/:key_id/rotate`.
- **KyGit is the brand, a vault is the resource, a repo is what you have (decision 9, 2026-09-22).** The SDK namespace is `r.repos` (class `Repos`; protocol-level types and crypto are `Vault*`, e.g. `Vault`, `VaultKeystore`, `VaultStatus`, `parseVaultRemoteUrl`); the SDK modules are `namespaces/repos.ts`, `vault.types.ts`, `vault.crypto.ts`, and `node/vault-*.ts`. The SDK calls `/vaults/v1/*` (redeem at `POST /vaults/v1/handoffs/:handoff_id/redeem` and `POST /vaults/v1/invites/:invite_id/redeem`), reads `VAULT_*` error codes, `vault_policy`, `vault_source_bytes`, and sends the apply slice as `vault`. The capture verb is `run402 repos capture` (`r.repos.capture`, `r.repos.planCapture`); a snapshot is a project snapshot only. One remote scheme, `run402::`: the `kygit::` spelling, `RUN402_REMOTE_SCHEME`, the `@kychee/kygit` package and its `git-remote-kygit` helper are deleted, and another `<scheme>::` address is refused as `REMOTE_SCHEME_UNSUPPORTED`. The `run402 gitvault` and `run402 source-access` tombstones and the `repo` singular alias are deleted. The keystore lives at `~/.config/run402/vault/` and moves there on first use. `cli/vault-surface.json` replaces the earlier surface file and drops `remote_schemes`. `RUN402_VAULT_TRACE` is the transport trace switch.
...
- Payment selection refreshes balances before creating a new payment payload, fixing a zero balance cached before faucet funding. Fresh-wallet bootstrap also tolerates RPC visibility lag after a successful faucet: retry only proven pre-payment balance misses for up to 30 seconds, preserving the payment idempotency key without another faucet transfer. Deploy examples now use prototype-compatible 10-second/128-MB function limits.
...
- **`run402 image generate` pays from the current organization on Lightning.** The MPP Lightning charge on `POST /generate-image/v1` refuses a principal that belongs to several organizations with `ORGANIZATION_SELECTION_REQUIRED` unless the body names the paying `org_id`; the CLI only forwarded an explicit `--org`, so a wallet in two orgs (the demo's `fizz-buzz`) failed every image without it. `--org` absent now derives the org from the one shared chain (`RUN402_ORG` / `RUN402_ROOM`, the `.run402.json` binding, `run402 org use`, the active project's owning org) and sends it; an empty chain sends nothing, since x402 and Tempo ignore it. `r.ai.generateImage({ orgId? })` retries once when the gateway answers that code and exactly one of `details.organization_ids` matches a local context (`credentials.getActiveOrg()` — new optional provider method, answered by the Node provider from `run402 org use` — or the active project's cached owning org, else a single stored project's), and otherwise re-throws with the candidates in the message, `details.organization_ids`, and `next_actions` naming `--org <org_id>` / `run402 org use <org_id>`; the CLI adds a `hint` and reports `details.org_source` on a 403 for a derived org. A caller-named org is never second-guessed.

- **Docs: three payment rails, named everywhere.** Every surface that named x402 (USDC on Base) now also names MPP on Tempo (pathUSD) and MPP on Bitcoin Lightning (sats, from the agent's platform-minted wallet or a Lightning top-up of the organization): README, SKILL.md, the OpenClaw skill, and the CLI/SDK/MCP references with their regenerated flat docs. The stray `lightning_wallet` row at the top of README.md moved into the MCP tools table, and the `buyer` MCP profile is documented as the eight tools it registers.
...
- **`llms-cli.txt` is now an index plus fetchable slices.** The CLI reference had grown to ~297 KB, and several agent runtimes truncate a fetched document near 60 KB — an agent that followed "curl llms-cli.txt" got an incomplete contract without knowing it. `https://docs.run402.com/llms-cli.txt` is now the index: the whole first-deploy contract (TL;DR, output and error JSON, `run402 up`, rehearsal, runtime config, install, funding, tiers) followed by a table of slices, one `llms-cli-<slice>.txt` per topic (`deploy`, `commands`, `repos`, `orgs`, `functions`, `assets`, `ops`, `errors`, `frontend`, `platform`), each self-contained and linking back; `llms-cli-full.txt` is the whole reference in one document. The generator (`scripts/build-agent-flat-docs.mjs`) keeps the index under 48 KB and every slice under 56 KB and refuses to emit past the budget; the docs-site CLI section is one page per slice (frontmatter `slice:` + `summary:`), and the docs deploy manifest lists the slices from the generator so a new one is served the moment it exists.

- **`run402 up` sees Grok, and says what it saw.** Client detection adds `grok` (`GROK_CLI`, `GROK_SESSION_ID`, `GROK_AGENT`, `XAI_GROK`, `GROK_CODE`) beside `claude-code` / `codex` / `cursor`, and `RUN402_CLIENT=<name>` declares a client that has no marker of its own (checked first; it names an unnamed principal, never overrides a name). `result.identity` now always reports `detected` (the client seen this run, applied or not) and `detection: { applied, reason }` — `name_already_set` means a client was detected but the principal already had a name (a skipped `identity.name.set` step says so; rename with `RUN402_AGENT_NAME=<name>`, which overrides an existing name, or `run402 org whoami --set-name`). Room presence `program` is inferred as `grok` / `cursor` too (`RUN402_PROGRAM` still overrides). Help text for `up`, `init --name`, and MCP `app_up` no longer claims a literal `agent` name is written.

- **Rehearsal skips an unchanged migration set.** A redeploy whose migrations are all checksum-identical noops (the plan's `migrations.new` is empty) is no longer rehearsed: the result says `rehearsal: { status: "skipped", reason: "migrations_unchanged" }`, so a page-only redeploy that still carries `schema`/`seed` ships in seconds instead of spending a snapshot and a branch. The gateway advertises the reason in the plan envelope; the SDK also derives it from the `{ new, noop }` buckets against an older gateway.

- **`run402 up --check` checks what it claims to.** It now verifies every local file the manifest references — migration `sql_path`/`sql_file`, function `source.path` and `files`, site `{ path }` entries and `dir()` targets, `assets.put[].source` — and fails with `MANIFEST_FILE_MISSING` (`details.missing[]` = `{ field_path, path, kind }`, one `create_file` next action per file) instead of passing clean and blowing up at upload; the same check runs before any gateway call in every mode, and in `deploy apply`. When no manifest is in the working directory, `UP_MANIFEST_REQUIRED` now looks one directory down and names what it found (`details.nearby_manifests[]`, a leading `run_in_directory` next action: `run402 up --check --dir <dir>`), and its message finally names `run402.json`. `--manifest <path>` to a missing file is a typed `MANIFEST_NOT_FOUND` with a `create_manifest` next action, not a bare `LOCAL_ERROR`. Action errors carry `next_actions` at the envelope top level too. New `@run402/sdk/node` exports: `collectLocalFileReferences`, `findMissingLocalFileReferences`, `assertLocalFileReferencesExist`.

- **`--nested`: an encrypted remote for an app that lives inside another repository.** `run402 up --nested` and `run402 repos create --nested --project <id>` give a monorepo app its own nested repository (`git init -b main`, the `run402` remote, the first push) and append exactly one line (`/<relative path>/`) to the enclosing repository's local `.git/info/exclude` — nothing else in it is touched. Without the flag the skip is now actionable: `result.repo` (and `repos create`'s `remote`) carries a `create_nested_repo` next action naming the command, `--human` prints it, and `repos create` no longer prints a `git push` next action for a remote it never added (nor names a monorepo app after the enclosing repository's `origin`).

- **Function logs are app-first, and a request id is enough.** `run402 logs --request-id <req_…>` (the `x-run402-request-id` header on every function response; `fnrun_`/`fnatt_` ids now accepted too) needs no function name — the SDK fans the read out across every function and merges the matches, each entry tagged with its `function` — and `run402 functions logs` makes `<name>` optional when `--request-id` is given. Every entry carries `origin: "app" | "platform"` (`classifyFunctionLogLine`, exported): Lambda's `INIT_START` / `START`/`END`/`REPORT RequestId` / billed-duration lines are hidden by default (`--app`), `--platform` shows only them, `--all` the raw stream; `hidden` counts what was dropped and a `hint` explains an empty result. `run402 logs` gains `--since`, rejects unknown flags, and names the function on a failed read. SDK: `r.functions.logsByRequestId(projectId, requestId, { tail, since, origin, functionName })` (+ scoped), `origin` on `logs()`. MCP `get_function_logs`: `name` optional with `request_id`, `origin` param, every line tagged.

- **Dark tables say so.** A valid anon key against an existing but undeclared table now gets `403 TABLE_NOT_EXPOSED` (gateway) with `next_actions` — `expose_table`, `edit_request` (the expose endpoint), `use_function` — instead of a bare Postgres `42501` that reads like broken auth or a missing RLS policy. Documented in the CLI frontend slice and both skill files.

- **`run402 doctor`: `ok` means "can this agent ship".** `ok` is now structural — `true` exactly when the new top-level `blocking[]` is empty — and advisory findings (an unbound operator passkey, a degraded recovery posture on another org, a stale CLI, gitvault gaps) land in the new `warnings[]` without flipping it or the exit code. Every check carries `severity: "blocking" | "advisory" | "info"`. The `tier` check has a fixed status vocabulary (`ok | inactive | frozen | past_due | dormant | purged | missing | unknown | error`, never a tier name; the name and raw lifecycle are in `value.tier` / `value.lifecycle`), and a wallet whose own org holds no tier but can reach another org's projects reports `missing` as advisory (`TIER_MISSING_ON_OWN_ORG`) — the case that used to read `ok: false` under two loud warnings.

- **`PUBLIC_ROUTED_FUNCTION` is informational.** The gateway emits it with `requires_confirmation: false`, so it never blocks a deploy; the CLI/MCP guidance and every doc surface stop teaching `--allow-warning PUBLIC_ROUTED_FUNCTION` and say which warnings actually need acknowledgement. `run402 image --help` no longer assumes Base Sepolia (it names the allowance's network and the Lightning rail, like `image generate --help` already did).
...
- **gitvault: writers, plural — a vault admits heads from every member's OWN key (r402s/v0 rev 47, gitvault-multi-writer).** Through rev 46 only the vault creator's keystore key could sign heads, so a handoff recipient or an added member could read and restore but never push. The writer set is now chain state: `kygit resume` / `run402 repos resume` activates the recipient as a writer under its own key BEFORE returning (a sender-signed `writer_admission_grant` at mint, the claimant's two-signature `writer_acceptance` at claim, a ref-neutral `add_writer_key` head), so `git push` works at once and the sender's environment can be deleted; the session-start/push/read reconcile admits any pending member key when this machine's key is a writer (`run402 repos access sync` on demand); `run402 repos view` lists `writers[]`, `pending_writers[]`, `writer_set`; `run402 doctor` reports `writer: active|pending|not_admitted|read_only_vault`; a push from a not-yet-admitted key is refused locally with `GITVAULT_WRITER_NOT_ADMITTED` and the exact next step; a CAS loser re-derives the writer set from the winner and stops if its own key was removed; removals ride `rotate_epoch`'s `writer_set_update`. The keystore identity's Ed25519 half is now published to the directory alongside the X25519 half (possession-signed). Handoff envelopes are v2 (v1 keys are refused with a re-mint next action — none were ever claimable as writers). No key is ever copied: a writer is admitted by a live writer's signature or by a handoff grant the recipient completes with its own key. Blast-radius sentence on every mint: "Whoever claims this key first becomes an owner of this org and a writer of this vault, permanently. The key works once and expires at <expires_at>."

- **cli: `org member add` REFUSES a writer-eligible add (developer or above) unless this session's key is an admitted writer on every vault of the org** (gitvault-multi-writer task 6.3, decided 2026-09-03: refuse, never warn). The gate runs BEFORE the add — `GITVAULT_WRITER_NOT_ADMITTED` with `next_actions: [request_writer_sync]` naming the vaults, and no membership is created — so a member you add can push at once and is never left half-usable. Viewer/billing adds never reach the gate; an org with no vaults has nothing to admit; a read-only-terminal vault is skipped. A current writer admits your key with `run402 repos access sync` (any push does), then re-run or have that writer run the add.
