/**
 * vault-persistent-helper — the resident engine's behavioral contract
 ...
 * scripts here deliberately reuse the shapes `cli-vault-remote-helper.
...
const DAEMON_RUNNER = fileURLToPath(new URL("./cli/lib/vault-daemon-run.mjs", import.meta.url));
...
describe("vault daemon — equivalence with the in-process host (task 3.1)", () => {
  let cfg;
  let daemon;
  before(async () => {
...
describe("vault daemon — fallback matrix (task 3.3)", () => {
...
describe("vault daemon — trust boundary + thin fast path (D5/D7)", () => {
