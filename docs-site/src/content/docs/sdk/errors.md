...
| `LocalError` | `"local_error"` | Local-host issues (filesystem, signing) | `cause`, `hint` |
| `SecretRequiresCliError` | `"local_error"` | `SECRET_REQUIRES_CLI`: a method that returns or consumes a one-time secret was called on a `sandbox` client (an MCP `run` snippet); refused before any request | `command`, `nextActions` (one `run_cli_command` entry naming the CLI command) |
...
| `WriteApprovalRequiredError` | `"write_approval_required"` | HTTP 403 `WRITE_APPROVAL_REQUIRED` / `_SCOPE_MISMATCH` / `_BINDING_MISMATCH` or 401 `WRITE_APPROVAL_SESSION_INVALID` — a person signed in without a wallet needs a passkey write approval for this `(capability, target)` | `capability`, `target`, `approveCommand` (`run402 approve --action … --org\|--project …`), `nextActions` (`approve_write`) |

The exported `Run402ErrorKind` union type (`"payment_required" | "payment_buyer_error" | "project_not_found" | "unauthorized" | "not_authorized" | "api_error" | "network_error" | "payment_attempt_error" | "local_error" | "deploy_error" | "transfer_freeze" | "step_up_required" | "write_approval_required"`) supports exhaustive `switch` statements with TypeScript exhaustiveness checking.
