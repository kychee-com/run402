...
const pScoped = await r.project(spec.project_id);
...
```
{
  ...
  project_id: "prj_…",                     // the one spelling: manifest, typed spec, and wire
...
await (await r.project(loaded.spec.project_id)).apply(loaded.spec, { idempotencyKey: loaded.idempotencyKey });

const inMemory = await normalizeDeployManifest({
  project_id: projectId,
  site: { patch: { put: { "index.html": { data: "<h1>hi</h1>" } } } },
});
await (await r.project(inMemory.spec.project_id)).apply(inMemory.spec, { idempotencyKey: inMemory.idempotencyKey });
```
...
await (await r.project(specWithRoutes.project_id)).apply(specWithRoutes);
```
...
await (await r.project(spaWithStaticHome.project_id)).apply(spaWithStaticHome);
```
