...
- Set `verifyOnly: true` to rerun app HTTP verification without upload, deploy, resource mutation, or project creation. This is the SDK equivalent of `run402 up verify`. Its action envelope reports `mode: "verify"`, `read_only: true`, and `dry_run: false`: real HTTP probes run, but no deployment is applied.
