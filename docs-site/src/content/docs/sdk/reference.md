...
`begin` requires a credential provider with `signPersonalMessage(message)` and checks that the returned EOA matches the exact wallet embedded by the gateway. The Node provider implements this using the selected local Run402 wallet. `complete` accepts a JSON string or exact `NostrEventV1`, rejects duplicate/unknown/secret-shaped fields, enforces the seven-field kind-1 envelope and safe tag policy, verifies the NIP-01 id and BIP-340 signature locally, and derives the challenge id from the signed payload. Public proof reads use no auth; revocation requires the linked principal's current EOA.
