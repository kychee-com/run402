...
`up` creates a local allowance and funds it from the testnet faucet (the prototype tier is free), creates the project, sets your display name if you have none (the detected client — `claude-code`, `codex`, `cursor`, `grok` — or nothing, with the room presence `agent`; declare yourself with `RUN402_CLIENT=<name>`, or set the name with `RUN402_AGENT_NAME=<name>` / `run402 org whoami --set-name <name>`), and applies the manifest as one atomic release. Later deploys that change the database against a live project are rehearsed on a throwaway branch first and ship only if they pass; a first deploy has nothing to protect and just ships.
