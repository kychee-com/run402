...
Each pattern below is a `deploy` spec or a `run` snippet; `docs({ topic })` has the full SDK section for any method named here.

### Paste-and-go assets — content-addressed URLs with SRI

When a snippet uploads a file with `await r.assets.put(projectId, key, { content })` (or `r.project(id).assets.put(key, …)`), the result is an `AssetRef` with these fields:
...
If you suspect cache staleness, `await r.assets.diagnoseUrl(projectId, url)` returns expected vs observed SHA, cache headers, invalidation status, and an actionable `hint`. For mutable URLs only, `await r.assets.waitFresh(projectId, { … })` polls until the CDN serves the expected SHA. Don't wait on immutable URLs — they're correct from upload time.
...
Non-mutating validation. `await r.projects.validateExpose(manifest, { project, migrationSql })` validates the auth/expose manifest used by `database.expose` and `r.projects.applyExpose` before you apply it. It accepts a manifest object or JSON string. The SQL is used only for reference checks; it is not executed as a PostgreSQL dry run. This is not deploy-manifest validation.

Imperative escape hatch. For ad-hoc changes outside a deploy: `await r.projects.applyExpose(projectId, manifest)`. `await r.projects.getExpose(projectId)` returns the live state, with `source: "applied"` (came from a prior apply) or `"introspected"` (no manifest applied; reconstructed from DB state).
...
For a directory of static files, `await r.sites.deployDir({ project, dir })` in a `run` snippet walks the directory on the host, hashes each file client-side, asks the gateway _which_ bytes it doesn't already have, and only uploads those. Re-deploying an unchanged tree returns immediately with `bytes_uploaded: 0`.

The `deploy` tool's response includes a fenced `json` block of buffered unified `DeployEvent` objects you can `JSON.parse`.

For full-stack deploys (database + migrations + manifest + value-free secret declarations + functions + site + subdomain), use `deploy`; resume an interrupted one with `await r.project(projectId).apply.resume(operationId)`. Set secret values first with `await r.secrets.set(projectId, key, { value })`, then deploy with `secrets.require[]`; never put secret values in a deploy spec. Rehearsal is automatic: a migration-bearing `deploy` / `up` against a project with a live release is rehearsed on a contained branch and committed only on a passing report (the result's `rehearsal` block says `passed`, or `skipped` with a reason such as `no_live_release` on a first deploy or `migrations_unchanged` when every migration is already applied with an identical checksum — a page-only redeploy is not rehearsed); pass `no_rehearse` to `up` to skip it. ADVANCED: `await r.project(projectId).apply.rehearse(planId)` rehearses an already-persisted plan id without committing; it snapshots the source, creates a contained branch, applies migrations and checks there, and returns a report without mutating the source project.
...
`site.embedding.frame_ancestors` opts the site in to being framed by the named embedders, by platform catalog **key** (never a raw origin): `{ "site": { "embedding": { "frame_ancestors": ["localhost"] } } }` makes every response of the host carry `Content-Security-Policy: frame-ancestors http://localhost:* http://127.0.0.1:*` and no `X-Frame-Options`, so a local product (a Buzz desktop panel, a dev server on any port) can show the app in an iframe. Everything else keeps `frame-ancestors 'none'` + `X-Frame-Options: DENY`. Omit the field on a later deploy to carry the previous declaration forward; send `null` to return to deny. Unknown keys fail `INVALID_SPEC` on `site.embedding.frame_ancestors` naming the valid ones. Release inventories (`r.project(id).apply.getActiveRelease()`) report `embedding` as keys or `null`; the host's `/_run402/config.json` discloses the expanded policy cross-origin.
...
| `ROUTE_TARGET_CARRIED_FORWARD` | Carried-forward route still targets a base-release function. | Inspect `r.project(id).apply.getActiveRelease()` and deploy a replacement route table if needed. |
...
`up` returns the shared workflow envelope, including verification and backup outcomes. Its `result.app_result` display projection is marked `run402.up.summary`; `expand_result` retrieves the same execution’s redacted detail through the returned `result_ref`. SDK callers retain the complete typed `run402.up.result`.
