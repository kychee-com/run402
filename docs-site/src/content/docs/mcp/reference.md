...
- `get_function_logs` — recent logs (CloudWatch). Params: `project_id`, `name`, `tail?` (default 50, max 1000), `since?` (ISO 8601, locally validated), `request_id?` (`req_...`, for routed/function correlation). Returned lines include optional metadata e.g. `request_id`, `event_id`, log stream, and ingestion time.
