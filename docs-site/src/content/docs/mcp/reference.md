...
`pay_url` is the general HTTP payment buyer tool. It delegates all negotiation,
dispatch, and recovery to SDK `pay.fetch`; MCP does not implement another
payment workflow. Existing exact x402 behavior remains compatible. Params:
`url`, optional `method`, `body`, `idempotency_key`, `max_usd_micros`
(default `100000`, or $0.10), `require_receipt`, `payment_preferences`,
`profile`, `max_native_amount_msat`, `max_routing_fee_msat`,
`evidence_policy`, and `organization_id`.

Explicit MPP Lightning charge uses the exact
`run402-mpp-lightning-charge-draft00-safety-v1` profile. Production uses
Bitcoin mainnet and the approved Run402 seller payee; regtest remains available
for local verification. It requires a caller-supplied stable key, an ordered
preference containing an `nwc:<label>` local instrument, and explicit USD,
all-in millisatoshi, and routing-fee ceilings. The seller returns one actionable
offer. Structured content adds a rail-neutral `payment_result`; protocol receipt,
immutable settlement attestation, chained outcome attestations, merchant
fulfillment, live `current_status`, and recovery/excess credits remain distinct.
Legacy `require_receipt: true` means merchant fulfillment only. Portable evidence
is preserved, while the local journal stores no request body, principal credential,
raw key, NWC URI, preimage, or private header. On trusted Run402
...
instance can also re-present its original in-memory proof. After restart, the
caller must resupply the byte-identical authenticated request, principal,
organization, key, profile, preference order, and caps. `OPEN`, `ACCEPTED`, and
`UNKNOWN` stay pinned after expiry and never select another rail. Generic NWC,
MPP session intent, L402, zaps, tenant Lightning, and on-chain per-request Bitcoin
are unsupported; Buzz/Nostr identity and the NWC instrument are independent.
...
- `deploy_rehearse` — run a persisted apply plan against a contained branch. Params: `plan_id`, optional `project_id`, optional `teardown` (`keep` / `on_pass` / `always`). Returns the rehearsal report, branch URL, migration/check results, snapshot id, next actions, and a commit command for passing reports.
...
- `set_tier` — subscribe / renew / upgrade. Auto-detects action and has an
  ...
  MPP Lightning charge accepts the same `payment_preferences`, exact `profile`,
  `max_usd_micros`, `max_native_amount_msat`, `max_routing_fee_msat`,
  `evidence_policy`, and `organization_id` fields as `pay_url`, and requires a
  non-empty `idempotency_key` before wallet dispatch. Repeat the identical call
  for recovery.
...
The full surface is **198 tools (~43,200 tokens)** loaded into your context before the first call. If you only intend to BUY — generate an image for $0.03 — that is a fifth to a third of a context window spent on 192 tools you will never call.

```
RUN402_MCP_PROFILE=buyer npx -y run402-mcp     # 6 tools, ~660 tokens
```

Registers `generate_image` · `init` · `check_balance` · `allowance_status` · `allowance_export` · `request_faucet` — enough to bootstrap a wallet, fund it (Base Sepolia faucet or a mainnet address from `allowance_export`), confirm the money landed, and buy.

Use the profile when the task is a purchase. Leave it unset when you may provision, deploy, or manage a project — the other 192 tools are how you do that.
