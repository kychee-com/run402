...
Buzz human adoption, community installation, and agent enrollment are also intentionally not MCP mutation tools. `whoami` renders their independent states, eligibility, and drift; project reads identify enrollment provenance when present and hand off to `run402 buzz ...` for exact consent/decision workflows. A shared skill is inert, a community installation grants nothing, and each enrolled agent remains a distinct principal with only explicit named-project grants.
