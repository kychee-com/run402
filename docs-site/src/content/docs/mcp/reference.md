...
`url`, optional `method`, `body`, `idempotency_key`, and `max_usd_micros`
(default `100000`, or $0.10). It delegates to SDK `pay.fetch` and returns
`{ http_status, body, payment, outcome, replay }`. Reuse the same idempotency
key and identical arguments after an ambiguous failure; the live SDK instance
re-presents its original in-memory proof and never mints a second authorization.
