...
- `add_subdomain` — add `<name>.run402.com`. Idempotent; auto-reassigns to latest deploy on subsequent deploys. Params: `project_id`, `name`, `release_id?`, `deployment_id?` — omit both to bind the project's live release.
- `list_subdomains` / `delete_subdomain` — manage subdomains.
- `domains_connect` / `domains_get` / `domains_list` / `domains_check` — manage project-scoped ProjectDomain desired state for web, email sending, inbound receive, mailbox addresses, and health checks.
...
- `deploy_releases_get` — fetch release inventory by id. Params: `project_id`, `release_id`, `site_limit?`. Returns release metadata, state kind, site paths, `static_public_paths` browser reachability entries, functions, secret keys, subdomains, materialized routes, applied migrations, `release_generation`, `static_manifest_sha256`, nullable `static_manifest_metadata` (`file_count`, `total_bytes`, `cache_classes`, `cache_class_sources`, `spa_fallback`), and warnings when returned. `site.paths` is release static assets; `static_public_paths[]` carries `public_path`, `asset_path`, `reachability_authority`, and `direct`.
- `deploy_releases_active` — fetch the current-live release inventory. Params: `project_id`, `site_limit?`.
- `deploy_releases_diff` — diff release targets. Params: `project_id`, `from` (`empty` / `active` / release id), `to` (`active` / release id), `limit?`. Returns `migrations.applied_between_releases`; secret and subdomain diffs expose `added` / `removed` only; route diffs expose `added` / `removed` / `changed`; `static_assets` exposes unchanged/changed/added/removed, newly uploaded CAS bytes, reused CAS bytes, eliminated deploy-copy bytes, `legacy_immutable_warnings`, `previous_immutable_failures`, and `cas_authorization_failures`.
- `deploy_resolve` — deploy resolver diagnostics for a public URL. Params: `project_id`, either `url` or `host`/`path`, optional `method`. Returns `would_serve`, `diagnostic_status`, `match`, summary, warnings, `edge_propagation` diagnostics, next steps, and fenced JSON with the full resolution.
...
- ProjectDomain email: use `domains_connect`, `domains_check`, `domains_repair`, and `domains_test_receive` for custom email sending and inbound receive.
...
- `admin_archive_project` — staff moderation. Sets `projects.archived_at = NOW()` on a single project; siblings on the same organization keep serving. staff only.
- `admin_reactivate_project` — un-archive a project (flips `archived_at` to NULL). It does not touch organization lifecycle. staff only.
- `project_info` / `project_keys` / `project_use` — inspect / set the active project.
- `send_feedback` — feedback to the Run402 team. Free with active tier. WRITE-ONLY: no inbox to read, no reply path — use `raise_escalation` when you need an answer from a human, or `send_room_message` to reach the other agents. Optional `project_id` + `handle` relay a deploy's promotion consent: a commit/promote response that activates with a public site carries a `hand_to_member` next action (unless already answered) with `credited_as` (the authenticated principal's display name or `null`, with `credit_source: principal.display_name`; detected client and room presence never supply credit) — show your human `urls.site` and `urls.console`, relay that Run402 would like to promote what they built on `@run402com` for free, ask yes or no, and on yes call `send_feedback({ message: "promote: yes", project_id, handle })`.
- `set_agent_contact` — register agent contact info. New or changed emails start a person reply challenge and return `assurance_level`.
...
- `verify_agent_contact_email` — start or resend the owner email reply challenge. The challenge secret is never returned.
- `start_contact_passkey_enrollment` — email a short-lived passkey enrollment link to the verified contact email. Requires `email_verified`.

### Notification channels & routing rules (Telegram)

Self-serve Telegram push on top of the owner-notifications substrate: connect a chat, then add rules so ONLY matching events page it. **No rule = no Telegram traffic** for that person; the mandatory email floor (security/recovery/billing_critical/destructive_lifecycle/verification classes) is unaffected by any rule.

- `list_notification_channels` — every notification channel (email, webhook, and every live Telegram binding with its id/status/chat metadata/label) for the person. Use this to find a `telegram_binding_id` for `create_notification_rule`.
- `list_notification_rules` — the person's Telegram routing rules.
- `create_notification_rule` — `telegram_binding_id` (required) + optional `project_id` / `source` (`"app"` or `"platform"`) / `event_types[]` / `classes[]`, all ANDed, each omitted field a wildcard. Requires passkey assurance (`operator_passkey`). An unusable or foreign `telegram_binding_id` returns the same 404 as a nonexistent one.
- `delete_notification_rule` — `rule_id`. Requires passkey assurance (`operator_passkey`).
...
Hand off or move a project without redeploying — one noun, three recipient shapes. A **wallet** recipient (both sides sign SIWX) and an **email** recipient (a principal whose verified email matches, into an org it owns or a new one) both complete via the one `accept_project_transfer`; an **owned org** recipient (`to_org_id`) is a same-actor move into another org the caller already owns and completes immediately in the first gateway release. Owner-side mutations on pending wallet/email transfers return `409 PROJECT_HAS_PENDING_TRANSFER` for the 72h pending window, so the recipient reviews exactly what they take on.

- `initiate_project_transfer` — start a transfer from the current owner/admin. Provide EXACTLY ONE of `to_wallet`, `to_email`, or `to_org_id`. Wallet inputs: `project_id`, `to_wallet`, optional `billing_policy` (`migrate`, the default), `message`, `kysigned_record_id` → returns `transfer_id`, `expires_at`, `terms_sha256`, project summary. Email inputs: `project_id`, `to_email`, optional `message`, `retain_member_role` (`developer` only) → returns `{ status, transfer_id, to_email, expires_at }`. Owned-org inputs: `project_id`, `to_org_id`, optional `message` → same-actor only at first (caller must own source and destination orgs) and returns an accepted result plus `anon_key`/`service_key`, which the SDK/MCP runtime persists locally. You must currently own/admin the project (gateway re-verifies against fresh DB state, not the 60s project cache). `billing_policy`/`kysigned_record_id` are wallet-only; `retain_member_role` is email-only.
- `preview_project_transfer` — fetch the safe review document for any pending transfer kind. Any party may view. Returns project name, custom domains, subdomains, function names, secret NAMES (values are NEVER returned), CI bindings that will be revoked on completion, mailbox summary, billing implications, the verbatim "GitHub repo ownership is not transferred" note, and — on email transfers — the `retain_member` offer.
- `accept_project_transfer` — the ONE completion, whatever the recipient's address. Inputs: `transfer_id`; for an email-addressed row optionally `org_id` (the org you own that receives the project; omit to create a new org) and `accept_retained_member` (opt into the previewed retained-developer membership). A wallet-addressed row needs your wallet to equal `to_wallet`; an email-addressed row needs a principal whose verified email matches. Atomically flips ownership, revokes the previous owner's CI bindings, and stamps a persistent `secrets_rotation_advised` advisory. Secret VALUES are inherited; the wallet response returns `secret_names_inherited[]` so the recipient can rotate them with `set_secret`. Both shapes return the new owner's project keys (persisted to the local project-key cache; keys are `project_id`-derived and don't rotate on transfer).
...
- `whoami` — resolve YOUR control-plane principal + every org membership (role + status) + `authenticator_id` (GET `/agent/v1/whoami`), the `session` grade (`browser` | `loopback` | `device`, read-only; none for the wallet an MCP host signs with), and the account health from GET `/agent/v1/me/status` (contact assurance, notification reachability, critical items). There is no MCP login tool: a person signs in with `run402 login`. Optional `set_display_name` (1–64 chars) sets your display name first (PATCH `/agent/v1/me`) — the name promotion credit and `up`'s room presence show; `up` sets a detected default (`claude-code`, `codex`, `cursor`, `grok`, `agent`) when it is empty. The remote identity; for local wallet/profile state use `status`.
...
- `list_rooms` — which rooms this credential can reach in an org. Params: `org_id`, or `project_id` for its owning org (omit both for the checkout's own context). Rooms are derived from use — a key nobody has written under is not a room and is not listed. Returns `{rooms: [{org_id, room_key, project_id, live_presences, last_activity_at}]}`, newest activity first; `project_id` is non-null exactly when the key is a project id (that project's default room).
- `get_room` — look at one room WITHOUT joining it: live presences and last activity. Params: room address. `join_room` would answer the same question by registering your presence, which changes the room; this does not. An unused key reads as empty (`live_presences: 0`, `last_activity_at: null`), never 404. A room key is a label: a project id (`prj_…`) or any label matching `/^[a-z0-9][a-z0-9._-]{0,63}$/`.
- `leave_room` — you are done: release this session's presence so it stops reading as live and stops holding its claims. Params: room address, optional `presence_id` (omit to release the presence this server registered at `join_room`; pass one to release a specific presence of your OWN principal — how a fresh session clears a crashed predecessor). Idempotent: already expired, already released, or another principal's reports `left: false` rather than failing. Without it a finished session lingers for the rest of its ~1h TTL holding its claims.
...
- `ack_room_message` — acknowledge a message addressed to this session's presence; the sender sees your `acked_at` on the message — acks are how an agent confirms it saw a passed task or agreed to a split. Params: room address, `message_id`. Recipients only (422 otherwise); idempotent (a replay reports the original ack time).
...
- `release_room_claim` — release a claim you hold. Params: room address, `claim_id`. Holder's credential only; idempotent — an already-released claim reports `already_released: true` with the original time. Pair it with a `send_room_message` closing note so the room's timeline tells the story.
...
Buzz is never a deadman channel: mandatory owner-notification classes keep their human paths (email, Telegram) regardless of route state, and a Buzz delivery acknowledges nothing.
