...
```bash
run402 archives create <project_id> \
  --target cloud \
...
run402 archives import ./project.r402ar \
  --target core \
...
`--target` names the deployment each verb talks to: `archives create`, `archives status <archive_id>`, and `archives download [<project_id>] <archive_id> --output <file>` export from Run402 Cloud (`--target cloud`, the default); `archives import` loads into a local Run402 Core (`--target core`, the default); `inspect` and `verify` are local and take no target.

`archives create` creates an operation-backed Cloud export, waits when `--wait` or `--output` is present, downloads bytes when `--output` is set, and returns `archive_id`, `operation_id`, `archive_status`, `sha256`, `expires_at`, `portability_report`, `export_report`, `verify_command`, and `import_command`. Use `--idempotency-key <key>` for safe retries, `--poll-interval <ms>` and `--timeout <ms>` for waits, and `--json-stream` for NDJSON progress.
...
`archives import` verifies before import, targets a new Core project only, and calls a local Core gateway (`RUN402_CORE_URL` or `--core-url`, default `http://127.0.0.1:4020`). It supports `--dry-run`, `--require-runnable`, `--env-file`, and repeated `--secret KEY=VALUE` overrides. Required secret names are reported by inspect/verify and in the archive's `secrets/required.env.template`; secret values are never exported.
