...
order: 3
slice: commands
summary: "up, init, pay, status, wallets, allowance, tier, credentials, projects, deploy, ci, transfer"
---
...
### init
- `run402 init` — set up with x402 (Base Sepolia). Creates allowance, requests faucet, checks tier, lists projects. A blocked or unconfirmed faucet is reported in `funding`; next actions carry the server cooldown or balance-check recovery instead of recommending immediate deploy. Identity setup can succeed while funding remains blocked.
...
- `run402 projects costs <id> [--window <24h|7d|30d|90d>]` — staff-only per-project finance: revenue, direct cost, margin, and cost breakdown. Uses the configured allowance wallet for admin auth; `RUN402_ADMIN_COOKIE='run402_admin=...'` is an optional browser-session override.
...
All admin subcommands require a staff allowance wallet (or an staff OAuth session). Project owners with a non-admin wallet receive `403 admin_required`.
