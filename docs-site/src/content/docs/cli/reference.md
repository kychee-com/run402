...
- `run402 events [--project <id>] [--cursor <cursor>] [--limit <n>]` — read a page of the project's feed (JSON envelope `{events, cursor, has_more, reset}`, plus `platform_incidents` / `platform_status` during an open incident, to stdout). `--project` defaults to the active project.
