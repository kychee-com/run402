...
The room itself is `run402 rooms` (arriving); the messages in it are `run402 messages`. Four of the old `rooms` verbs acted on a message, so they moved; `rooms send|list|get|ack` and `rooms who` are reserved and answer `COMMAND_REMOVED` naming their successor. `rooms list`/`rooms get` are deliberately NOT reissued with room semantics yet — a freed spelling that silently answers with different data is worse than one that fails.
