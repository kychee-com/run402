...
- Identity: an existing display name is kept (wallet principals are created unnamed — the gateway never seeds a name from the wallet subject). Otherwise `up` names the principal, in this order: `RUN402_AGENT_NAME` when set (any runtime declares itself once, no per-command flag), else a specifically detected client — `claude-code` (`CLAUDECODE`, `CLAUDE_CODE`, `CLAUDE_CODE_ENTRYPOINT`, `CLAUDE_CODE_SESSION_ID`), `codex` (`CODEX_SANDBOX`, `CODEX_CI`, `OPENAI_CODEX`, `CODEX_HOME`), `cursor` (`CURSOR_TRACE_ID`, `CURSOR_SESSION_ID`, `CURSOR_AGENT`), `grok` (`GROK_CLI`, `GROK_SESSION_ID`, `GROK_AGENT`, `XAI_GROK`, `GROK_CODE`), or whatever `RUN402_CLIENT=<name>` declares — approved by `-y`, or asked once. When nothing is known, nothing is written: a guess is never persisted as a name, the room presence is `agent` for coordination only, and the result says `identity.source: "undetected"` with the nudge. Reported as `result.identity: { display_name, source: "existing" | "explicit" | "detected" | "undetected" | "unavailable", detected, detection: { applied, reason }, presence }` — `detected` is the client seen this run whether or not it was applied, and `detection.reason` is `applied`, `name_already_set` (the principal already had a name, so the detected client was NOT written; a skipped `identity.name.set` step says so — rename with `RUN402_AGENT_NAME=<name>` or `run402 org whoami --set-name <name>`), `explicit_name_wins`, or `nothing_detected`. An invisible client therefore never silently stays `agent`: the result names what it saw and how to apply it. Promotion credit (`hand_to_operator.credited_as`) is this name. Set it explicitly with `run402 init --name <name>` or `run402 org whoami --set-name <name>`.
