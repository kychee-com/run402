...
### source-access (gitvault member custody, read side)
A human member's gitvault decryption key exists only as sealed `swrap2_` wrappers (passkey PRF and/or the source recovery code) — gitvault-recovery-custody. Enrollment, activation, revocation, and code replacement are BROWSER ceremonies (WebAuthn) at console.run402.com/account → Source access; this family is deliberately read-only. Auth: your control-plane (human) session — `run402 operator login --loopback` first. Without one the request falls back to the active WALLET identity and answers for the AGENT principal (normally no wrappers) — truthful, with a stderr note saying so. No MCP tools (same human-session rule as `operator`).

- `run402 source-access status` — your source-access key + wrapper set (`GET /agent/v1/source-access/wrappers`): key identity (`ek_` fingerprint, custody scheme, state) and each wrapper's kind (`webauthn_prf` / `recovery_code`) + state (`pending` / `active` / `revoked`), ciphertext included. Principal-scoped structurally — only ever YOUR wrappers. Stderr summarizes, and warns when active wrappers exist but none is a `recovery_code` (a passkey-only key has no offline/no-server recovery path) or when a `pending` wrapper is an unfinished enrollment.
- `run402 source-access export [--out <file> | --out -]` — download your versioned member recovery bundle (`GET /agent/v1/source-access/recovery-bundle`, format `r402s-member-recovery-bundle/v1`): key identity + every ACTIVE wrapper ciphertext. Together with your source recovery code — kept SEPARATELY — it recovers your vaults with no run402 server (`run402 repos recover <mirror> --bundle <file> --receipt <pin>`). A server-side wrapper row alone is NOT offline backup; the export is what makes the code work offline, and the gateway stamps it as recovery-posture evidence. Writes `run402-source-recovery-bundle-<fingerprint>.json` (0600) in the cwd unless `--out` says otherwise (`--out -` prints only); the full JSON always goes to stdout. To make the bundle travel WITH a vault mirror, copy it to `member-recovery-bundles/<name>.json` under the mirrored prefix — `repos recover` finds it there automatically and `repos fsck --mirror` reports it as an availability hint. Requires an active source-access key (`409 NO_ACTIVE_ENCRYPTION_KEY` otherwise — complete enrollment first).
