...
git push -u origin main                       # allocates the vault on first push (see below), publishes
```
...
**Deploying a vaulted project.** Today, allocating a vault sets the project's `gitvault_policy` to `required` — the platform does this at allocation, so a vaulted project's deploys are gated from the moment the vault exists, and every client-side path that allocates (init, snapshot, `git push`, `repos create`) says so on stderr. Once `gitvault_policy` is `required`, a deploy must present a vaulted capture at commit, and `run402 deploy apply` produces one automatically: it snapshots your work tree, publishes the capture to the vault, exchanges the capture receipt for a single-use activation token, and commits with it. Nothing to pass, no extra command — a `required` project deploys the same way any other one does, from any machine holding the keystore; un-gate with `run402 gitvault policy grandfathered --reason "<why>"` (owner + step-up, audited, reversible). Design D3 decouples allocation from the deploy gate — allocation will leave the policy unset until you choose — and ships with a later gateway change; the client already handles the ungated state it will produce (a typed `next_actions` entry offering `run402 gitvault policy required` plus a standing `warnings[]` entry on every ungated deploy — never a block, never an interactive prompt).
