...
| `recover <source> --out <dir> [--human]` | `r402s-recover`: rebuild a BARE recovery repository (no working files) straight from a mirrored prefix, with NO SERVER INVOLVED — the offline disaster path. Kept its name (design D10): `restore` was considered and rejected because `git restore` already means something else. The result's `layout` is `"bare"` and its `next_actions` print the exact `git clone <out_dir> <out_dir>-worktree` to run for a working tree — `recover` itself never checks files out. Normal retrieval is plain `git clone run402::<org>/<repo>` — there is no `repos clone` verb. Cloning needs a Run402 principal on this machine — a wallet with an allowance and a keystore holding an envelope for this vault — this is encrypted git, not a shareable link. A fresh clone may show dangling commits under a plain `git fsck` — the vault's retained deploy/capture history, delivered in the pack but not referenced by any local ref; expected and harmless, not corruption, and `git fsck`'s exit status stays clean. `--human` renders a short summary instead of JSON. |
