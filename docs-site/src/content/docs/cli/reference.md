...
- Identity: an existing display name is kept (a `display_name` equal to the wallet subject, or any bare `0x` address, counts as unset — it was assigned at creation, not chosen); otherwise `up` sets the detected client name (`claude-code`, `codex`, `cursor`, or `agent`) — approved by `-y`, or asked once — and reports `result.identity: { display_name, source: "existing" | "explicit" | "detected" | "unavailable", presence }`. Promotion credit (`hand_to_operator.credited_as`) is this name. Set it explicitly with `run402 init --name <name>` or `run402 org whoami --set-name <name>`.
...
- `run402 deploy rehearse <plan_id> [--project <id>] [--teardown keep|on_pass|always] [--json]` — ADVANCED: rehearse an already-persisted plan without committing. Source project data and the original plan stay untouched; a project with no live release rehearses on an empty branch. The plan's bytes must already be uploaded (a `--plan`-only plan has none): a rehearse before that step is refused `409 REHEARSAL_CONTENT_MISSING` listing `missing_content`, before any snapshot or branch is spent.
