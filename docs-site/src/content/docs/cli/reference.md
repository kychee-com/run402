...
**Deploying a vaulted project.** Allocating a vault does **not** change the project's deploy policy (design D3) — `gitvault_policy` stays unset until you set it explicitly. The first `deploy apply` against a vaulted, ungated project proceeds ungated (nothing is ever blocked or interactively prompted on this) and its result carries a typed `next_actions` entry offering `run402 gitvault policy required`; every later ungated deploy of that project carries a `warnings[]` entry naming the drift, until the policy is set either way — `required` or an explicit, reasoned `grandfathered`. Once `gitvault_policy` is `required`, a deploy must present a vaulted capture at commit, and `run402 deploy apply` produces one automatically: it snapshots your work tree, publishes the capture to the vault, exchanges the capture receipt for a single-use activation token, and commits with it. Nothing to pass, no extra command — a `required` project deploys the same way any other one does.
