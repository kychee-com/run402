...
`run402 repos` is your source, encrypted before it leaves the machine — a Git remote whose contents are encrypted client-side and stored as a chain of signed, admitted heads (wire protocol `r402s/v0`). One noun, thirteen verbs (`repo` singular resolves identically — say what `gh repo` already taught you). `run402 gitvault <verb>` answers a structured `COMMAND_MOVED` (naming its `repos` successor) or, for `push`/`reconcile` (no behavioral successor), `COMMAND_REMOVED`. `r.gitvault` is unchanged in the SDK — `gitvault` stays the protocol/infrastructure name; `repos` is what you type.
