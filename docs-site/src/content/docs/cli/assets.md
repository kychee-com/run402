...
Bulk directories: use `deploy` with `assets` slice: additive `assets: { put: [...] }`; declarative sync `assets: { put: [...], sync: { prefix, prune: true, confirm? } }`. No `run402 assets sync`; apply is canonical so HTML + asset URLs stage atomically.

- `run402 assets put <file> [files...] [--project <project_id>] [--key <dest>] [--content-type <mime>] [--private] [--immutable] [--meta <k=v>] [--exif-policy keep|strip] [--concurrency N] [--no-resume] [--stream]` — without `--stream`, stdout is the final results array (JSON). With `--stream`, stdout is NDJSON per-file progress events. `--json` is a deprecated alias for `--stream` (writes a deprecation warning to stderr). Stdout carries the canonical snake_case wire shape only (see the `assets put` output example below); the SDK's camelCase `AssetRef` conveniences never appear on CLI stdout.
- `run402 assets get <key> --output <file> [--project <project_id>]`
- `run402 assets ls [--project <project_id>] [--prefix <p>] [--limit <n>] [--sort key:asc|createdAt:asc|createdAt:desc] [--filter <k=v> ...]`
- `run402 assets rm <key> [--project <project_id>]`
- `run402 assets sign <key> [--project <project_id>] [--ttl <seconds>]` — signed URL TTL must be an integer from 60 to 604800 seconds.
- `run402 assets diagnose <url> [--project <project_id>]` — inspect live CDN state for a public URL
- `run402 cdn wait-fresh <url> --sha <hex> [--timeout <secs>] [--project <project_id>]` — poll a mutable URL until it serves the expected SHA-256

Project resolution for every command above: `--project <project_id>` > `RUN402_PROJECT_ID` (the canonical env var, same as every other project-scoped command) > the deprecated `RUN402_PROJECT` alias > the active project. `RUN402_PROJECT` works as a fallback but prints one deprecation line to stderr; switch to `RUN402_PROJECT_ID`.
...
// Whole directory in one apply (staged with a combined release before activation).
...
- `run402 sites deploy-dir <path> [--project <project_id>] [--quiet] [--dry-run] [--confirm-prune]`

`--project` defaults to the active project. Manifest: `{"files":[{"file":"index.html","data":"..."},{"file":"style.css","data":"..."}]}`. Must include `index.html`. Free with active tier. If the project already has a subdomain, redeploying auto-reassigns it to the new release (response includes `subdomain_urls`).

To inspect deploy status, use `run402 deploy events <operation_id>` or `run402 deploy list --project <project_id>` instead of polling a deploy artifact.
...
### subdomains
- `run402 subdomains add <name> [--release <release_id> | --deployment <deployment_id>] [--project <project_id>]`
- `run402 subdomains list [--project <project_id>]`
- `run402 subdomains delete <name> --confirm [--project <project_id>]` — `--confirm` required (irreversible release).
...
### domains
- `run402 domains connect <domain> --project <project_id> [--web] [--authority manual-dns|hosted-zone] [--email-send] [--email-receive] [--mailbox-addresses primary|alias|managed|none] [--addresses <csv>]`
...
- `run402 domains test-receive <domain> --to <local-part|address> [--project <project_id>]`
- `run402 domains wait <domain> [--project <project_id>] [--until active|safe|receive-active]`
- `run402 domains activate <domain> [--project <project_id>]`
- `run402 domains disconnect <domain> --confirm [--project <project_id>]` — `--confirm` required.
...
Auth is control-plane auth: current wallet, sign-in session, or grant key. Domain commands do not require a local project-key cache entry.

**Custom domains need hobby or team.** `domains connect` for a domain that no project of the organization already holds needs the owning organization on hobby or team (`custom_domains: true` in `run402 tier status`). Managed `<name>.run402.com` subdomains stay free on every tier. On prototype the connect fails before any DNS work with `403 TIER_FEATURE_REQUIRED` (`details.required_tier: "hobby"`) and two next actions: `renew_tier` (`run402 tier set hobby`, paid from the wallet) and `create_checkout` (a card checkout link for a person). Upgrade, then rerun the same `domains connect`. A domain the organization already holds is never gated: re-connecting it, moving it to another of its projects, and `status`, `dns`, `check`, `repair`, `test-receive`, `wait`, `activate` and `disconnect` work on every tier, and a later downgrade never disconnects it.
...
- `run402 apps publish <project_id> [--description "..."] [--tags a,b] [--visibility <public|private>] [--fork-allowed]`
- `run402 apps <versions|delete> <project_id> [<version_id>]`
...
The paying organization matters only on the Lightning rail: a principal that belongs to more than one organization is refused `400 ORGANIZATION_SELECTION_REQUIRED` (`details.org_ids` lists the candidates) unless the request names one. `--org <org_id>` names it; omitted, the command derives it from the one shared org chain (`RUN402_ORG` or `RUN402_ROOM`'s org half, else the `.run402.json` binding, else `run402 orgs use`, else the active project's owning org — see the Organizations slice) and sends that. An empty chain sends no org, which is never an error here: x402 and Tempo ignore it, and a single-org principal needs none. When the gateway still answers `ORGANIZATION_SELECTION_REQUIRED`, the SDK retries once with the single candidate that matches a local context (the current org, the active project's owning org, else a stored project's owning org); zero or several matches surface the code with `details.org_ids`, a `hint` naming `--org`, and `edit_request` next actions for `--org <org_id>` and `run402 orgs use <org_id>`. A `403` on a derived org (no billing role there) reports `details.org_source` / `org_source_detail` so you can see which rung supplied it.

### ai
Built-in AI helpers. Translation requires the AI Translation add-on on the project. Moderation is free for all projects.

- `run402 ai translate <project_id> "<text>" --to <lang> [--from <lang>] [--context "<hint>"]` — translate text to a target language (ISO 639-1 codes). Source language auto-detected if `--from` omitted. Context hint guides tone/register (max 200 chars).
- `run402 ai moderate <project_id> "<text>"` — run content moderation. Returns flagged status and per-category scores.
- `run402 ai usage <project_id>` — check translation word quota for the current billing period (used, included, remaining).
...
Run402 Core uses the same CLI after `run402 init --api-base=http://my-core:4020`. The Core host must configure the gateway outbound provider first; `email mailboxes` surfaces `provider_readiness`, `can_send`, `send_blocked_reason`, and `next_actions` when setup is missing. Core's first slice supports raw outbound mail with attachments; managed templates, inbound reply handling, sender-domain automation, and delivery operations may remain Cloud-only until the Core gateway adds them.
...
- `run402 email create <slug> [--project <project_id>]` — create a project-scoped mailbox local part. The response's `managed_address` is `<slug>@<project-mail-host>.mail.run402.com`; another project may use the same slug. NOT idempotent: a same-project conflict (slug already in use, address in cooldown, or the project already has 5 mailboxes) returns a 409 error rather than an existing mailbox.
- `run402 email mailboxes [--project <project_id>]` — list mailboxes plus `mailbox_settings`, `address`/`managed_address`, default-role/readiness/footer-policy metadata (`is_default_outbound`, `is_auth_sender`, `can_send`, `can_receive`, `send_blocked_reason`, `domain_kind`, `footer_policy`, `effective_footer_policy`, `footer_policy_locked_reason`), and gateway `next_actions`.
- `run402 email defaults [--outbound <slug|mbx_id>] [--auth-sender <slug|mbx_id>] [--clear-outbound] [--clear-auth-sender] [--project <project_id>]` — show current defaults with no flags, or set/clear `default_outbound_mailbox_id` and/or `auth_sender_mailbox_id`. Slugs are resolved through `email mailboxes`; SDK PATCH uses mailbox ids.
- `run402 email update [<slug|mbx_id>] --footer-policy <run402_transparency|none> [--mailbox <slug|mbx_id>] [--project <project_id>]` — set the mailbox's outbound footer policy through `PATCH /mailboxes/v1/:mailbox_id`. The optional positional target and `--mailbox` are equivalent; omit only on single-mailbox projects.
- `run402 email status [--mailbox <slug|id>] [--project <project_id>]` — show mailbox info (ID, address, slug, footer policy)
- `run402 email send --template <name> --to <email> [--var key=value ...] [--from-name <name>] [--mailbox <slug|id>] [--project <project_id>]`
- `run402 email send --to <email> --subject <subject> --html <html> [--text <text>] [--attach <path>[:content-type] ...] [--from-name <name>] [--mailbox <slug|id>] [--project <project_id>]`
- `run402 email list [--direction <inbound|outbound>] [--mailbox <slug|id>] [--project <project_id>]` — lists BOTH sent + received by default; `--direction inbound` lists received replies (the reconciliation backstop if a reply_received webhook is lost)
- `run402 email get <message_id> [--mailbox <slug|id>] [--project <project_id>]`
- `run402 email reply <message_id> --html <html> [--text <text>] [--subject <subject>] [--from-name <name>] [--mailbox <slug|id>] [--project <project_id>]` — reply to an inbound message (threads via In-Reply-To)
- `run402 email get-raw <message_id> --output <file> [--mailbox <slug|id>] [--project <project_id>]` — fetch inbound raw RFC-822 bytes (DKIM/zk-email). `--output` required; bytes to file, stdout `{ message_id, bytes, output }`. Outbound returns 404.
- `run402 email delete [<slug|mailbox_id>] --confirm [--project <project_id>]` — delete a mailbox (irreversible). Target a specific mailbox by slug or id; on a project with one mailbox the target may be omitted.
- `run402 email webhooks list [--mailbox <slug|id>] [--project <project_id>]` — list all webhooks registered on the mailbox
- `run402 email webhooks get <webhook_id> [--mailbox <slug|id>] [--project <project_id>]` — get webhook details
- `run402 email webhooks delete <webhook_id> [--mailbox <slug|id>] [--project <project_id>]` — delete a webhook
- `run402 email webhooks update <webhook_id> [--url <url>] [--events <e1,e2>] [--mailbox <slug|id>] [--project <project_id>]` — update webhook URL and/or events
- `run402 email webhooks register --url <url> --events <e1,e2> [--mailbox <slug|id>] [--project <project_id>]` — register a new webhook. Valid events: delivery, bounced, complained, reply_received, mailbox_suspended
- `run402 email webhooks deliveries [--status <pending|in_flight|delivered|failed_permanent>] [--mailbox <slug|id>] [--project <project_id>]` — durable delivery rows. At-least-once with bounded retries/backoff; `failed_permanent` = DLQ. Body envelope `{ id, type, created_at, schema_version, idempotency_key, payload }`; dedupe on `idempotency_key`.
- `run402 email webhooks redrive <delivery_id> [--mailbox <slug|id>] [--project <project_id>]` — re-queue a dead-lettered (failed_permanent) delivery for another attempt
