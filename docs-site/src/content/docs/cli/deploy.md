...
Stdout final result includes `release_id`, `operation_id`, `urls` (`site`, `console`, and `mcp` when a function exports a `tool`: the app's MCP connector URL), etc. Stderr streams JSON-line progress events. `--quiet` / `--final-only` silence stderr while preserving stdout.
