...
description: Distinguish authority, available funds, tiers and application-side costs.
---
...
A fresh supported Cloud setup can use the free prototype tier on testnet. Testnet settlement is not a real-money payment. Before production operations, inspect the selected payer, network, allowance and tier; follow the [billing reference](/cli/ops/) for the chosen payment rail. An unavailable balance read means unknown, not zero or “funded”.
