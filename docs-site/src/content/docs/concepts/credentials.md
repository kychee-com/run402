...
Control-plane principals are people, agents or CI participants operating infrastructure. End-users are people signed into your app. An app login does not grant deploy authority.
...
| Runtime secret | Deployed server function | No | Write-only configuration; rotate at its provider and update the deploy |
