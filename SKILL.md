...
One noun across every agent surface (repo-surface-consolidation): `run402 gitvault <verb>` is RETIRED, and these three read-only tools carry the same rename — `get_gitvault_status`/`list_gitvault_heads`/`verify_gitvault` are now `repos_view`/`repos_list_heads`/`repos_fsck`. Teach only `repos` spellings from here on.
...
`run402 repos` is a Git remote whose contents are encrypted on the machine that wrote them and stored as a chain of signed, admitted heads. It is how a repository's history outlives the machine — without that outliving requiring the plaintext to be handed over. Wire protocol: `r402s/v0`. One noun, twelve verbs — `run402 gitvault <verb>` is RETIRED (repo-surface-consolidation): every old spelling now answers a structured `COMMAND_MOVED`/`COMMAND_REMOVED` error. Teach `repos` from here on; `r.gitvault` is unchanged in the SDK.
