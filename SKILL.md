...
**Buzz/Nostr public attribution.** A managed Buzz agent can link its separately-held Nostr public key to the active Run402 principal with `run402 identity link nostr begin|complete`. The ceremony uses a server challenge signed once by the Run402 EOA and once as a standalone NIP-01 kind-1 Buzz event. Never request or accept an `nsec`, Nostr private key, mnemonic, seed, or derivation path; use `buzz social publish --content` and `buzz social event --event` so Buzz performs Nostr signing inside its own boundary. This link is public attribution only—never authentication, authorization, organization ownership, payment, or a transfer target. Existing MCP reads render the linked identity and immutable action-time provenance; the identity-link mutation itself is CLI/SDK-only. Buzz users should install the distinct self-contained [`run402-buzz` package](buzz/README.md), which initializes and links before offering an approval-gated contextual deployment.

**Buzz community control plane.** `run402 buzz status` keeps skill installation, human adoption, community installation, and per-agent enrollment independent. The shared skill is inert; a Run402-public installation associates a Buzz community with an org but grants nothing; adoption adds the Buzz owner as a distinct human co-owner without demoting the founder agent or transferring projects/credentials; enrollment gives each later agent only approved finite grants to named existing projects, never org membership or payment authority. Buzz itself remains unchanged: activation uses an ordinary Buzz kind-1 approval plus released NIP-11/NIP-43 evidence, and Run402 owns descriptor discovery, policy/default revisions, and revocation. Use CLI/SDK for signing-sensitive mutations; MCP renders state and exact gateway-authored handoffs only. Pending nested descriptors are labeled `proposed`, and safe policy summaries expose effective capability/TTL limits. New installations activate non-default, and creation/discovery order never selects one. Clear the old Run402 default before selecting a new one. When exactly one active default exists and no enrollment is pending or active, offer bounded enrollment before provisioning. Preserve the org-of-one fallback when selection is absent, ambiguous, declined, denied, stale, expired, or revoked.
