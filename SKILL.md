...
`run402 repos` is a Git remote whose contents are encrypted on the machine that wrote them and stored as a chain of signed, admitted heads. It is how a repository's history outlives the machine — without that outliving requiring the plaintext to be handed over. Wire protocol: `r402s/v0`. One noun, thirteen verbs. `run402 gitvault <verb>` answers a structured `COMMAND_MOVED`/`COMMAND_REMOVED` error; teach `repos` from here on. `r.gitvault` is unchanged in the SDK.
