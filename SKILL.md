...
**Buzz/Nostr public attribution.** A managed Buzz agent can link its separately-held Nostr public key to the active Run402 principal with `run402 identity link nostr begin|complete`. The ceremony uses a server challenge signed once by the Run402 EOA and once as a standalone NIP-01 kind-1 Buzz event. Never request or accept an `nsec`, Nostr private key, mnemonic, seed, or derivation path; use `buzz social publish --content` and `buzz social event --event` so Buzz performs Nostr signing inside its own boundary. This link is public attribution only—never authentication, authorization, organization ownership, payment, or a transfer target. Existing MCP reads render the linked identity and immutable action-time provenance; the identity-link mutation itself is CLI/SDK-only. When this generic skill is loaded in a managed Buzz context after the human says `Please install the run402.com skill`, follow `https://run402.com/install.txt` and install the distinct self-contained [`run402-buzz` package](buzz/README.md) into the Buzz workspace; do not let a runtime-global generic skill shadow it. Immediately read that verified installed package and continue in the same turn through preflight, initialization, and identity linking; do not stop at the installation receipt or defer to runtime discovery on the next turn. The Buzz-specific package then offers an approval-gated contextual deployment.

**Buzz community control plane.** `run402 buzz status` keeps skill installation, human-adoption offer/adoption, community installation, and per-agent enrollment independent. Use `run402 buzz adopt offer` for the canonical inert durable HTTPS handoff and poll `offer show`; the browser owns direct human login/passkey and the short Buzz callback attempt. `buzz adopt direct` is advanced compatibility. Adoption adds the Buzz owner as a distinct co-owner without demoting the founder agent or transferring projects/credentials. Community installation grants nothing; enrollment gives later agents only approved finite grants to named existing projects, never org membership or payment authority. Buzz itself remains unchanged. Use CLI/SDK for signing/passkey-sensitive mutations; MCP renders state and exact HTTPS/CLI handoffs only. New installations activate non-default, and creation/discovery order never selects one. When exactly one active default exists and no enrollment is pending or active, offer bounded enrollment before provisioning. Preserve the org-of-one fallback when selection is absent, ambiguous, declined, denied, stale, expired, or revoked.
