...
- [!] **P4B.36** Use run402 mailbox-webhook CRUD once it ships — **WAITING FOR:** run402 `email-webhook-crud` openspec (in progress by run402 dev) that adds `GET /mailboxes/v1/:id/webhooks`, `DELETE /mailboxes/v1/:id/webhooks/:webhook_id`, and `PATCH /mailboxes/v1/:id/webhooks/:webhook_id` + matching CLI + MCP tools. When landed, update `kysigned-private/scripts/deploy.ts` `registerEmailWebhook()` (around L523) to: (a) call GET list, (b) look up an existing registration matching the webhook URL — if found, **skip POST entirely** (today we fall through to POST on every deploy because the GET 404s silently); (c) if URL changed (e.g., function renamed), use PATCH or DELETE+POST to reconcile instead of accumulating orphan webhook rows; (d) remove the cosmetic "GET may not exist on every gateway version" comment added during the Hiccup #2 fix. TDD: unit-test the new reconcile logic with mocked GET/POST/DELETE responses; smoke-test via a redeploy drill (P13-PRACTICE.5-style) asserting zero orphan webhook rows afterward. Opportunistic: if the run402 CLI exposes `run402 email webhooks list`, add a "how to inspect registered webhooks" note to `kysigned-private/README.md` operator runbook. [code]
