...
- [!] **1R.3** Deploy the on-chain PLONK verifier contract to Base Sepolia — WAITING FOR: self-compiled circuit (DD-21). Steps: (1) write `KysignedApproval.circom` using `@zk-email/circuits` EmailVerifier + custom regex, (2) compile with `circom`, (3) PLONK setup with Hermez Powers of Tau `.ptau` via `snarkjs plonk setup`, (4) export verifier `snarkjs zkey export solidityverifier` → deploy. Registry Blueprint abandoned (single-domain restriction). [infra] `AI`
