...
- [ ] **1R-CI.1** Add `test:all` npm script to `package.json` that runs ALL test suites in sequence: `test` (unit), `test:contract` (Hardhat), `test:e2e` (e2e), `test:e2e:smoke` (smoke). Exit non-zero if any suite fails. This is the command that /implement's verification gate #5 will run after every task. [code] `AI`
- [ ] **1R-CI.2** Add `test:docs` npm script — verify all public API endpoints (`/v1/*`) are documented in `llms.txt` and any future `openapi.json`. Modeled after run402's `test:docs` suite (which caught 12 undocumented admin endpoints). Internal admin endpoints can be explicitly excluded with a comment explaining why. [code] `AI`
- [ ] **1R-CI.3** Add `.github/workflows/test.yml` — GitHub Actions CI that runs `npm run test:all` (including `test:docs`) on every push to `main` and on every PR targeting `main`. Node.js 20, Ubuntu latest. This is the "run ALL tests per commit to main" gate for the future public repo. [infra] `AI`
- [ ] **1R-CI.4** Run `npm run test:all` once to establish baseline — record the current pass count (expect: ~197 unit + 20 contract + e2e/smoke). Fix any failures before proceeding to 1R.3. Log the baseline in the Implementation Log. [code] `AI`
...
- [ ] **1R.3** **RESUMED 2026-04-12 — proof system chosen: RISC Zero zkVM (Candidate D from zkprover v0.1.0).** The zkprover mini-product (`docs/products/zkprover/`) built and measured four prover candidates in parallel. Winner: RISC Zero (3min/proof, 8.4 GB RAM, ~280k gas, 238-contributor ceremony, Apache-2.0, fits Lambda). See `docs/products/zkprover/research/comparison-matrix.md` and DD-12 in `docs/plans/zkprover-plan.md` for full rationale. **Remaining sub-tasks to complete 1R.3 with RISC Zero:** [infra] `AI`
  - [!] Set up RISC Zero Bonsai API credentials — WAITING FOR: user to sign up at bonsai.risczero.com (free dev tier) and provide the API key. Store as AWS secret `kysigned/bonsai-api-key`.
  - [ ] Generate a Groth16-wrapped proof from the existing STARK proof using Bonsai (or local Groth16 prover once credentials are available)
  ...
  - [ ] Call `verifyProof` on-chain with the Groth16-wrapped proof, confirm no revert, measure REAL gas cost (expected ~280k). Requires Groth16 proof from previous sub-task.
  - [ ] Record verifier router address + tx hash + gas in the kysigned plan's Implementation Log
...
- [ ] **2R.7** Implement DKIM validation library integration + circuit hardening pre-checks (from GPT consultation F3.3.4). Validate DKIM signature on raw MIME bytes. Extract DKIM selector + signing domain for evidence key lookup. Pre-circuit rejection layer: (a) reject if `d=` domain doesn't align with `From:` domain, (b) reject if DKIM `l=` tag present (partial body), (c) reject duplicate critical headers (`From`, `Subject`, `To`), (d) reject non-`text/plain` MIME or multipart messages, (e) compute replay nullifier from canonical DKIM signature bytes, (f) check `To:` includes the designated reply address if `To` is in `h=`. [code] `AI`
- [ ] **2R.8** Implement `I APPROVE` body validation — two-layer approach: (1) **Operator pre-check** (tolerant): strip trailing punctuation, normalize whitespace, detect common typos (e.g., "I APROVE"), check body length <4KB. Provides user-friendly error recovery BEFORE attempting circuit. (2) **Circuit constraint** (exact): `[Ii] [Aa][Pp][Pp][Rr][Oo][Vv][Ee]` on standalone line — the canonical form. The circuit is the cryptographic proof; the pre-check is the UX layer. [code] `AI`
  - Write failing test for `I APPROVE` with various casings → pre-check passes, normalizes
  - Write failing test for common typos (`I APROVE`, `I APPROVE!`, `I  APPROVE`) → pre-check catches, suggests correction
  - Write failing test for body >4KB (email signature bloat, quoted chain) → pre-check rejects with "reply again with only I APPROVE, no signature block"
  - Write failing test for `I APPROVE` buried in quoted content → pre-check rejects
  - Implement tolerant pre-check + canonical form extraction
- [ ] **2R.9** Implement auto-reply for non-matching/oversized replies — three cases: (1) No `I APPROVE` found: "Your reply did not match. To sign, reply with `I APPROVE` as the first line." (2) Typo detected (e.g., `I APROVE`): "Almost — you wrote '[detected text]'. To sign, reply with exactly: I APPROVE" (3) Body too long (>4KB): "Your reply was too long (email signature, quoted text, etc.). Please reply again with only `I APPROVE` in the body — no signature block, no quoted text." All auto-replies use existing EmailProvider. Signer can retry unlimited times. [code] `AI`

#### 2R-C. zk Proof Pipeline

- [ ] **2R.10** Implement zk proof generation wrapper — takes validated DKIM-signed MIME, runs the **RISC Zero zkVM guest program** (`r0-zkEmail` fork from `zkprover-candidates/D-risc0/vendor/r0-zkEmail/`), generates a STARK proof, wraps it in Groth16 for on-chain verification. Public outputs: `from_domain_hash`, `public_key_hash`, `verified`, `envelope_id`, `doc_hash`. Includes replay nullifier derived from DKIM signature bytes. Proof system: RISC Zero (chosen via zkprover v0.1.0, DD-12). [code] `AI`
  - Write failing test for proof generation from valid MIME
  - Implement RISC Zero host driver integration (load guest ELF, feed raw MIME, invoke prover)
  - Write failing test for proof verification (client-side, matches on-chain Groth16 verifier)
  - Implement client-side verification helper
- [ ] **2R.11** Implement evidence key registration — on first encounter of a new (provider, selector, key), fetch DKIM public key from DNS, call `registerEvidenceKey(domain, selector, publicKey)` on `EvidenceKeyRegistry` contract. Cache registered keyIds to avoid redundant registrations. [code] `AI`
  - Write failing test for first-encounter registration
  - Implement key fetch from DNS + contract registration call
...
- [ ] **2R.13** Implement on-chain recording — after RISC Zero Groth16 proof generated, call `recordReplyToSignSignature` on `SignatureRegistry` with the Groth16 proof bytes + public inputs (from_domain_hash, public_key_hash, envelope_id, doc_hash) via the platform wallet (KMS or direct, depending on deployment mode). Proof format: RISC Zero Groth16 wrapper (~260 bytes). [code] `AI`
...
- [ ] **2R.19** Update verify-by-`(email, document)` — compute `searchKey = SlowHash(email || docHash)`, query `SignatureRegistry` for matching record, retrieve zk proof from event, look up evidence key from `EvidenceKeyRegistry`, verify zk proof against the key. [code] `AI`
...
- [ ] **3R.9** Update `/verify/:envelopeId` proof link page — display reply-to-sign verification details (evidence key domain + selector, registration timestamp, zk proof status) alongside tx hashes and Basescan links. [frontend-visual]
