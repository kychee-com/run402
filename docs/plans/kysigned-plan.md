...
- [!] **1R.3** Deploy the on-chain zk verifier contract to Base Sepolia — WAITING FOR: Blueprint `volinskey/kysigned_Reply_to_Sign_Approval` compilation at registry.zk.email (submitted 2026-04-10, compiling). Once done, fetch verification key via SDK → `snarkjs zkey export solidityverifier` → deploy `KysignedVerifier.sol`. [infra] `AI`
