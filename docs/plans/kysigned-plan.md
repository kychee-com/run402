...
- [ ] **P4B.14** Verify the `npm run test:e2e` suite passes end-to-end against `BASE_URL=http://localhost:4022` (local docker Postgres + local gateway per CLAUDE.md). **DEFERRED** — requires the local stack stand-up (Postgres + run402 gateway + kysigned-service in-process router + Sepolia faucet wallet). To be re-run after the user gate before P4B.26, OR confirmed against the deployed instance via P4B.27. [code]
