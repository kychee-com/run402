...
- **Decision:** Public repo (`kysigned`) is the core library. Service repo (`kysigned-private`) imports it via `file:../kysigned` during development, `github:kychee-com/kysigned#tag` when stable, npm package when mature.
...
- **What this means for kysigned-private:** kysigned-private (the private hosted-deployment repo) becomes much narrower than originally planned. Its job is **deployment glue** (the bootstrap script per F22), private business logic (Stripe billing for Path 3 if not via run402, monitoring config, account-deletion cron), and the run402 serverless functions that wrap the public-repo code. It does NOT contain pluggable adapters for run402 services — those are imported directly inside the public repo.
...
- **Decision:** Phase 4B deploys exactly three run402 functions from `kysigned-private`: (1) `kysigned-api` — HTTP router handling all `/v1/*` endpoints via a single entry point; (2) `kysigned-email-webhook` — receives run402's delivery/bounce webhook POSTs and dispatches to public-repo handlers; (3) `kysigned-sweep` — scheduled Lambda running twice daily (12h cron) that calls `sweepRetention` + a new `handleEnvelopeExpiration` (see DD-16) from the public repo.
- **Alternatives:**
...
- **Decision:** Extend `envelope_signers` with a nullable `completion_email_provider_msg_id TEXT` column (migration 005 in the public repo). Add helper functions `markCompletionEmailSent(pool, signer_id, provider_msg_id)` + `findSignerByCompletionEmailId(pool, provider_msg_id)`. Update `kysigned/src/api/sign.ts` completion-send loop to call `markCompletionEmailSent` after each `emailProvider.send()` (the provider already returns `{ messageId: string }`). The SES webhook Lambda in kysigned-private uses `findSignerByCompletionEmailId(run402_msg_id)` → `(envelope_id, email)` → calls the existing `markCompletionEmailDelivered` / `markCompletionEmailBounced` helpers.
- **Alternatives:**
...
- **Alternatives:**
  ...
  - *Put the e2e in kysigned-private instead of the public repo* — rejected; forkers should be able to reproduce the same e2e against their own deployed instance (DD-9 trojan horse alignment)
...
**OQ #19 — Byte-identical bytecode check precise mechanism.** The gate runs in a kysigned-private script (NOT in the public kysigned repo, to honor DD-9 + F17's anti-leakage). Mechanism:
...
5. The check is implemented as a small script `kysigned-private/scripts/bytecode-identity-check.ts` (new file, TDD, mocked `eth_getCode` in tests, run against mainnet in anger). Executed manually by the operator during the flip ritual and its output logged to the Implementation Log.
...
3. **Check for common causes in order of likelihood:**
   ...
   - (d) Contract source was modified between canary and production deploys. `git log packages/kysigned-private/contracts/SignatureRegistry.sol` should be empty between the two deploys. If it's not, something changed that shouldn't have.
...
- **Rollback on smoke failure:** abort the launch announcement. Flip the kysigned-private env vars back to the canary addresses (the canary wallet still exists at this point because retirement happens AFTER smoke success). Investigate. Do NOT proceed to Phase 14's public-repo flip until the production contract smoke passes.
...
- **Phase C — Prove BOTH infra deploy paths work:** With Phase A and B both producing valid proofs on their respective circuits, validate that each circuit size actually deploys on the infra class it needs:
  - **C1 — 8M on external prover:** deploy the Phase A prover as a containerized service (Fargate task or equivalent), make it callable via HTTP from a simulated kysigned-private handler, generate a proof round-trip end-to-end, measure latency + memory + cost.
...
- [x] Create private GitHub repo `kychee-com/kysigned-private` [infra]
- [x] Clone both repos locally under `C:\Workspace-Kychee\` (`kysigned` and `kysigned-private` side by side) [infra]
...
- [x] Initialize `kysigned-private` repo: package.json with `"kysigned": "file:../kysigned"` dependency, tsconfig, .gitignore [code]
...
- [x] **R.2** Prototype zk proof generation — **SUPERSEDED 2026-04-15.** Research validated by Phase 2R-C shipping real zk proof generation end-to-end in production (2R.10 prover wrapper, 2R.11 evidence-key registry, 2R.13 on-chain record — all `[x]` with measured gas ~432k per signature on Base Sepolia per 1R.5 Infrastructure State table). The hands-on Blueprint-compilation measurement became moot once the full RISC Zero + r0-zkEmail pipeline was wired and exercising on Sepolia. Research doc `docs/research/R2-proof-generation-prototype.md` stays as the decision record. [code] `AI`
...
- [ ] **2R.B.G7** Full regression: `npm run test:all` green; update Test Baseline. Guest unit tests must cover every new assertion from G1-G6. [code] `AI`
- [ ] **2R.B.G0** Switch `methods/guest/Cargo.toml` + `host/Cargo.toml` cfdkim dependency from `github.com/boundless-xyz/dkim` to `github.com/kychee-com/cfdkim` (our fork of zkemail/cfdkim) at pinned commit. Update `verify_email_with_key` call to pass 5th param `ignore_body_hash = false`. Verify `cargo build` succeeds for both guest and host. Per DD-34. [code] `AI`

#### 2R.B.G8 — cfdkim In-Guest Hardening (from ZK-soundness consult) `[both]` `AI`

> These are defense-in-depth assertions in the zkVM guest that catch anything cfdkim misses. They run BEFORE or AFTER cfdkim's `verify_email_with_key` and reject emails that a strict RFC verifier would reject. Parallel to this, the cfdkim library patches (findings #1, #2, #4, #6, #8) are developed in a SEPARATE repo (`C:\Workspace-Kychee\cfdkim\`) and submitted as upstream PRs to `zkemail/cfdkim` — see `cfdkim/docs/products/cfdkim-patches/cfdkim-patches-spec.md`.

- [ ] **2R.B.G8.1** Strict CRLF framing — reject any raw email bytes containing a lone `\n` or `\r` outside proper CRLF pairs. Prevents finding #3 (LF-only body hash → empty). [code] `AI`
- [ ] **2R.B.G8.2** Reject `simple` header canonicalization — only accept signatures with `c=relaxed/*`. Prevents finding #4 (simple canon not byte-exact). [code] `AI`
- [ ] **2R.B.G8.3** Reject duplicate DKIM tag names — scan parsed DKIM-Signature for duplicate tag names before calling verify. Prevents finding #6 (last-wins ambiguity). [code] `AI`
- [ ] **2R.B.G8.4** Assert `a=` algorithm family matches supplied key type — `rsa-sha*` requires RSA key, `ed25519-sha256` requires Ed25519. Prevents finding #2 (algorithm confusion). [code] `AI`
- [ ] **2R.B.G8.5** `b=` value uniqueness check — reject any DKIM-Signature where the raw `b=` value appears more than once in the header text. Prevents finding #1 (global replace attack via `z=`). Alternative: reject `z=` tag entirely. [code] `AI`
...
>
> **Update 2026-04-15**: the 2R.B.A2 `/consult` was run *first* (out of sequence) and surfaced 8 new findings. See [`docs/consultations/cfdkim-zk-soundness-review.md`](../consultations/cfdkim-zk-soundness-review.md). Post-consult research revealed: (a) our pinned `boundless-xyz/dkim@3213315e` is abandoned (last meaningful commit same day we pinned, 2024-11-08), (b) an actively-maintained fork `zkemail/cfdkim` has security fixes through 2025-06-25 that independently address at least 2 of our 8 findings. Phase restructured — **2R.B.A0 added as a pre-step** to evaluate switching to the maintained fork before investing in patching the abandoned one.

- [x] **2R.B.A0** **Evaluate `zkemail/cfdkim` as base library before pass-1 audit.** Fetch the `zkemail/cfdkim` repo, diff from our pinned commit `3213315e` (2024-11-08) through their latest HEAD (2025-06-25 at time of evaluation). For each of the 8 cfdkim ZK-soundness findings in [`cfdkim-zk-soundness-review.md`](../consultations/cfdkim-zk-soundness-review.md), classify: (a) already fixed in zkemail/cfdkim, (b) partially fixed, (c) not addressed. Also sanity-check zkemail/cfdkim hasn't introduced NEW soundness issues vs our pinned commit. Decision rule: if ≥ 5/8 findings are already addressed upstream AND no regressions introduced, switch `methods/guest/Cargo.toml` + `host/Cargo.toml` to `zkemail/cfdkim` at a pinned commit, then continue 2R.B.A1/A3 with the remaining patch-gap only. Otherwise stay on `boundless-xyz/dkim` and proceed as originally scoped. Record the decision as DD-34 with: diff summary, classification table (8 findings × status), and choice rationale. Also record, alongside the decision, the broader ZK-email Rust ecosystem survey from the 2026-04-15 research run (see Log entry) — this captures what OTHER libraries exist as future fallback options (SP1-zkEmail, Succinct's fork, any academic ports). [code] `AI` / `DECIDE` — estimated 1-2 hours.
...
- [x] **2R.B.A2** Run `/consult` session scoped specifically to cfdkim ZK-soundness. **DONE 2026-04-15** (run out of sequence, before A1). 8 findings: 4 HIGH (#1 b= replace, #2 a= key-type, #3 LF-only body, #5 duplicate Subject/To), 3 MEDIUM (#4 simple canon, #6 duplicate tags, #7 selector policy), 1 LOW (#8 RFC parsing). See [`docs/consultations/cfdkim-zk-soundness-review.md`](../consultations/cfdkim-zk-soundness-review.md). Cost: $13.98. [manual] `HUMAN`
...
- [x] Implement Path 2 wallet onboarding flow on website — guide user through wallet creation and funding. Link directly to `docs/wallet-guide.md` for the long version. [frontend-visual] — `WalletOnboardingPage.tsx` at `/onboard`: 4-step visual guide (install wallet, switch to Base, fund USDC+ETH, connect & send). Detects existing wallet and offers dashboard shortcut. Links to full wallet-guide.md on GitHub. Dashboard auth screen shows "No wallet? Get started" link when `!hasWallet()`. Alt path callout for Path 3 (credit card). All tests pass, Vite build clean.
...
**Service repo (`kychee-com/kysigned-private`) — `[service]`:**

- [x] **P4A.7** Create `kysigned-private/src/db/httpPool.ts` — an `HttpDbPool` class that implements the narrowed `DbPool` interface by wrapping `@run402/functions` `db.sql()`. Shape:
  ```ts
...
- [x] **P4A.9** Re-run the kysigned-private test suite + build clean. [code]
...
- [x] **P4B.15** `kysigned-private/src/email/httpSesEmailProvider.ts` ships `createHttpSesEmailProvider({ sendRaw, discoverMailboxId, mailboxId?, fromName? })` returning a kysigned `EmailProvider`. Lazy mailbox discovery cached across sends, explicit override path, `from_name` forwarding. 6 TDD tests. [code]
- [x] **P4B.16** `kysigned-private/src/router/apiRouter.ts` is the single HTTP entry point — `createApiRouter(deps)` with a regex-based route table covering health, envelope (create/get/void/remind/list/export), sign/decline, verify-by-hash, verify-by-envelope. 405 on wrong-method, 404 on unknown path, sanitized 500 on handler exceptions. `buildContext.ts` exposes `buildPaymentVerifier` + `buildSenderIdentityExtractor` that compose the public-repo Phase 2B middleware over a Web `Request` (x402 first, MPP fallback, `X-Kysigned-Email` for Path 3). Routes are inlined in apiRouter.ts rather than split into a separate `routes.ts` (one file, kept compact). 12 + 7 = 19 TDD tests. [code]
- [x] **P4B.17** `kysigned-private/src/router/adminAuth.ts` ships `checkAdminAuth(req, opts?)` per DD-15. Reads SIWE message + signature from `X-Kysigned-Siwe-Message` (base64) + `X-Kysigned-Siwe-Signature` headers, verifies via the public repo's `verifySiweSignature` (viem), checks the recovered wallet against `KYSIGNED_ADMIN_WALLETS` (env or opts override) with case-insensitive csv membership. Returns `{ allowed, status, wallet?, reason? }` — 503 on empty env, 401 on missing/invalid sig, 403 on wallet not in list. 7 TDD tests. [code]
- [x] **P4B.18** `kysigned-private/src/webhook/emailWebhook.ts` ships `createEmailWebhookHandler(deps)`. Parses the run402 mailbox webhook JSON, dispatches `delivery` → `markCompletionEmailDelivered`, `bounced` → `markCompletionEmailBounced`. Ignores `complained` / `reply_received` (returns 200). Unknown `message_id` returns 200 with a warn log. Malformed JSON / missing fields → 400. 8 TDD tests. [code]
- [x] **P4B.19** `kysigned-private/src/scheduled/sweepAndExpire.ts` ships `runSweepAndExpire(deps)` calling both `sweepRetention` and `handleEnvelopeExpiration` from the public repo. Independent try/catch — failures in one don't abort the other. Both errors collected in `result.errors[]`. 5 TDD tests. [code]
- [x] **P4B.20** `kysigned-private/functions/kysigned-api.mjs` — Lambda entry shim. Wires `HttpDbPool(db.sql)` + `httpSesEmailProvider(email.send)` + viem `RegistryClient` (lazy import) + `buildPaymentVerifier` + `buildSenderIdentityExtractor`, then constructs `createApiRouter(deps)` and exports it as `default async function handler(request)`. [code]
- [x] **P4B.21** `kysigned-private/functions/kysigned-email-webhook.mjs` — Lambda entry shim. Wraps the webhook handler with `HttpDbPool` + the public-repo helpers from `kysigned`. [code]
- [x] **P4B.22** `kysigned-private/functions/kysigned-sweep.mjs` — Lambda entry shim. Wires `runSweepAndExpire` with `sweepRetention` + `handleEnvelopeExpiration` from the public repo. PDF storage adapter is a placeholder (`console.log` only) until the storage backend is wired. [code]
...
- [x] **P4B.29** Operator README at `kysigned-private/README.md` — deploy procedure (9 steps + env var table), `KYSIGNED_ADMIN_WALLETS` setup + rotation, all 4 DD-15 limitations, F24 forward-compat sketch, brand asset warning, 5 troubleshooting recipes, test recipes. Commit `5da7612`. [manual]
...
- [x] **P4B.36** Use run402 mailbox-webhook CRUD (shipped in run402 v1.33.0 — commit `7f69255` 2026-04-15). — **2026-04-15:** Extracted pure reconcile logic to `src/webhook/reconcileRegistration.ts` (`reconcileWebhook(existing, desired) → {noop|create|patch}`). 8 TDD tests covering empty list, no-match, url-match/same-events, url-match/different-events, order-independent event equality, first-match-on-duplicate. deploy.ts `registerEmailWebhook()` rewritten: GET list via the new `/mailboxes/v1/:id/webhooks` endpoint → reconcile → POST on create / PATCH `/mailboxes/v1/:id/webhooks/:webhook_id` on event-drift / no HTTP write on noop. Live smoke: deploy.ts run against live state logged `already in desired state (whk_1776237281307_vhs07p) — no-op` exactly as intended; `GET /mailboxes/v1/:id/webhooks` returns 1 webhook with correct URL + events. Cosmetic "GET may not exist on every gateway version" comment removed. kysigned-private 76/76 tests + `npm run build` clean. Note: stored reconcile module is imported from `dist/` at deploy-time (deploy.ts runs the compiled output path for parity with Lambda bundles). [code]
...
4. **~~`/subdomains/v1/admin/:name/release`~~ — STRUCK (operator error, not a hiccup).** Initial DELETE attempt reached for the admin-release endpoint, which is properly documented in run402/CLAUDE.md as "for dispute resolution when the reserved owner can't be reached" — exclusively a Kychee platform-ops endpoint. Project owners (including us) should always use the resource-scoped `DELETE /subdomains/v1/:name` (service_key auth). Run402's 403 was correct behavior. No action required.
...
- [x] Publish all legal docs on kysigned.com [infra] `AI`
...
- [x] Write hypothesis card for Freelancers segment [manual] `AI -> HUMAN: Approve` — drafted as `kysigned-private/marketing/hypothesis-cards/freelancers.xlsx`
...
- **Phase 13C blocks on Phase 4B Block 7 (e2e test against production-deployed kysigned-private).** The parallel `/implement kysigned` chat owns Phase 4B; coordinate with that chat before starting Phase 13C.
...
- [x] **IAM simulation re-run (10 seconds).** Catch any policy drift since the run402 KMS wallet implementation was tested: `aws iam simulate-principal-policy --policy-source-arn <gateway-task-role-arn> --action-names kms:CreateKey kms:Sign kms:ScheduleKeyDeletion kms:CancelKeyDeletion --region us-east-1`. All must return `allowed`. **Additionally verify that `kms:Decrypt` is NOT allowed** — per run402/CLAUDE.md, the gateway role intentionally blocks `kms:Decrypt` because contract wallets are sign-only; if `kms:Decrypt` IS allowed, that's a policy drift red flag, open a run402 issue. Do this check immediately before 13B, not days ahead. [infra] `AI` — **2026-04-14:** Passed. Task role `arn:aws:iam::472210437512:role/AgentDB-Pod01-TaskDefTaskRole1EDB4A67-XTUia2at8urw`. All 4 required KMS actions `allowed`; `kms:Decrypt` correctly `implicitDeny`. No policy drift.
...
- [ ] **Fund ops EOA wallet with enough ETH for two separate ETH transfers** (~0.01 ETH total, ~$25 at $2500/ETH): one to the canary KMS wallet (~0.004 ETH for canary contract deploys + dark-launch envelope recordings), one to the production KMS wallet (~0.004 ETH for production deploys + pilot traffic), plus ~0.002 ETH buffer. Sizing basis: pilot volume is ~20 signatures per network; measured mainnet cost per tx ≈ $0.05 at normal gas (432k gas × 0.05 gwei × $2500/ETH), contract deploys ≈ $5 each, giving ~$10 realistic pilot gas × 2-3x headroom. Production wallet can be topped up again post-launch from revenue if traffic exceeds pilot. The ops EOA is `0x8D671Cd12ecf69e0B049a6B55c5b318097b4bc35`, key in `kysigned/ops-wallet-key` Secrets Manager secret. [manual] `HUMAN`
...
- [ ] Fund the canary wallet with ~0.004 ETH from the ops EOA wallet for gas. Wait for confirmation on Base. Record the funding tx hash in the Implementation Log. [manual] `HUMAN`
...
#### Phase 13C: Dark-launch — kysigned-private in production mode against canary `HUMAN` + `AI`

> **BLOCKS ON Phase 4B completion.** This phase requires the full kysigned-private deployment from Phase 4B (HTTP router, SES webhooks, sweep cron, frontend, e2e) to be live at `https://kysigned.run402.com` and routable via `https://kysigned.com` (apex + www, already resolved 2026-04-08). Do not start 13C until Phase 4B is marked Complete in the parallel /implement chat and its Phase 15 ship tasks for "REST API", "Verification page", and "Dashboard" are green.

- [ ] Configure kysigned-private with canary environment variables: `KYSIGNED_CONTRACT_ADDRESS=<canary_address>` (read from `kysigned/canary-contract-address` Secrets Manager entry at deploy time) and `KYSIGNED_KMS_WALLET_ID=<canary_wallet_id>` (read from Secrets Manager at deploy time). Redeploy kysigned-private. [infra] `AI`
...
- [ ] Fund the production wallet with ~0.004 ETH from the ops EOA wallet for gas. Wait for confirmation on Base. Record the funding tx hash. [manual] `HUMAN`
...
- [ ] **Byte-identical bytecode gate for BOTH contracts:** run `kysigned-private/scripts/bytecode-identity-check.ts <canary_sig_reg> <prod_sig_reg>` AND `<canary_ev_key_reg> <prod_ev_key_reg>`. Success on both → proceed. Failure on either → ABORT, follow the OQ #21 investigation playbook. [infra] `AI`
- [ ] **FLIP kysigned-private environment variables** from canary references to production references: `KYSIGNED_SIGNATURE_REGISTRY_ADDRESS`, `KYSIGNED_EVIDENCE_KEY_REGISTRY_ADDRESS`, `KYSIGNED_KMS_WALLET_ID` (all from Secrets Manager). Redeploy kysigned-private. **This is the launch moment.** No application code changes in this redeploy — config change only. [infra] `AI`
...
- [x] Email deliverability setup — dedicated sending domain, SPF/DKIM/DMARC, warm-up plan [infra] `AI` — **2026-04-07:** dedicated sender domain (`kysigned.com`) registered via run402 + SPF/DKIM/DMARC records live + status `verified`. **2026-04-08:** authoritative DNS migrated from Route 53 to Cloudflare; SPF/DKIM/DMARC preserved. Warm-up ramp unnecessary — kysigned is not a mass-email product; organic early usage provides sufficient reputation building.
- [ ] **Enable inbound on `kysigned.com` custom sender domain** (unblocked by run402 `inbound-email-raw-mime` Phase B, shipped 2026-04-14). Call `POST /email/v1/domains/inbound` against the kysigned p0030 project for `kysigned.com` (service-key auth). Add the MX record returned by the response (or already present in Cloudflare) pointing at the SES inbound exchange. Verify via `GET /email/v1/domains` that `inbound.enabled=true, inbound.mx_verified=true`. Send one test reply to `reply-to-sign@kysigned.com` from a Kychee inbox and confirm the inbound Lambda logs the recipient + raw-MIME-S3-key. Until this lands the hosted service can only accept replies at `reply-to-sign@<slug>@mail.run402.com`, NOT at `reply-to-sign@kysigned.com`. [infra] `AI`
...
- [x] **Wire kysigned to the shared monitoring module** (`@run402/shared/monitoring`) — provide concrete senders for Telegram (kysigned chat), Bugsnag (kysigned project), and SES (CRITICAL emails). Cover all standard signals from saas-factory F20. [code] — `kysigned-private/src/monitoring.ts` ships `createTelegramSender` / `createBugsnagSender` / `createSesEmailSender` and `createKysignedMonitor()` factory. 7 unit tests, all green. Goes live the moment the bot/project/SES resources exist (Phase 14 manual tasks).
...
**Service repo (`kychee-com/kysigned-private`) — code removal:**
...
- [x] Implement config/storage module at `mcp/src/init/config.ts` — 7 tests, 0600/0700 perms, KYSIGNED_CONFIG_DIR override, allowance/projects/instance read-save [code]
- [x] Implement SIWX auth helper at `mcp/src/init/siwx-auth.ts` — 6 tests, EIP-4361 + EIP-191 via @noble/curves+@noble/hashes. Gotcha noted: v1.6+ returns Signature object not bytes. [code]
- [x] Implement run402 API client at `mcp/src/init/run402-client.ts` — 7 tests, typed wrappers for faucet/tier/project/mailbox/admin-SQL/billing. Fake-fetch unit test pattern. [code]
- [~] Implement Demo 1 (slug→slug self-test) — stubbed at `mcp/src/init/demo-stubs.ts` pending deployed kysigned-private. Stub returns clear "run kysigned customize to deploy" message so init bootstrap completes end-to-end. Real implementation blocked on the service-deploy path that lands with `kysigned customize`. [code]
- [~] Implement Demo 2 (operator-email demo) — stubbed alongside Demo 1. Same deferral rationale. [code]
- [x] Implement init handler orchestrator at `mcp/src/init/handler.ts` — 12-step idempotent bootstrap with dependency-injected clientFactory + demo callbacks. 5 tests covering fresh run, idempotency, skipDemo2, faucet failure resilience, chain pinning. [code]
- [x] Wire `init` MCP tool into `mcp/src/index.ts` — Zod schema (operator_email, api_base, skip_demo_2), JSON summary returned. [code]
- [x] Build `kysigned` CLI binary at `mcp/src/cli.ts` — subcommands: `init`, `customize` (stub), `help`. Added to package.json `bin`. [code]
- [x] Add `[TEST]` badge to dashboard frontend — `DashboardPage.tsx` renders amber `[TEST]` badge when `env.chain_id === 84532`. Backend follow-up (populate chain_id in list response) tracked separately. [frontend-visual]
- [x] Add testnet warning to standalone PDF validator — `validate-pdf.ts` prints prominent 3-line "TESTNET SIGNATURE — NOT LEGAL EVIDENCE" warning block when any proof block's Chain contains "sepolia"/"testnet". [code]
- [ ] Write `init` integration test at `mcp/src/init/integration.test.ts` — gated by `KYSIGNED_INIT_INTEGRATION=1`, runs against live run402, asserts bootstrap steps complete (demos are deferred). [code]
- [x] Implement `kysigned customize` stub — CLI `customize` command prints "COMING SOON" with the 8-step productionization outline. Interactive handler implementation lands in a follow-up phase alongside real demo implementations. [code]
- [x] Update README.md — "Deploy your own — kysigned init" section with 8-step walkthrough, [TEST] caveat, `customize` reference, manual-setup fallback retained. [manual]
- [x] Update mcp/README.md — added `init` as 8th tool with full description. [manual]
- [x] Update llms.txt (kysigned-private) — added `init` to tool list + forker quickstart snippet. [manual]
...
- 2026-04-14: **Phase 17 — `kysigned init` + CLI foundation built.** 4 commits in kysigned repo. New `mcp/src/init/` module: `config.ts` (7 tests — KYSIGNED_CONFIG_DIR, 0600/0700 perms, allowance/projects/instance round-trips), `siwx-auth.ts` (6 tests — EIP-4361 + EIP-191 via @noble/curves/secp256k1 + keccak, gotcha: v1.6+ returns Signature object not bytes), `run402-client.ts` (7 tests — typed wrappers for faucet/tier/project/mailbox/admin-SQL/billing with fake-fetch pattern), `handler.ts` (5 tests — 12-step idempotent orchestrator, dependency-injected client + demos + migrations), `migrations.ts` (SQL loader from disk), `demo-stubs.ts` (placeholder demos pending service-deploy path), `cli.ts` (`kysigned` bin with init/customize/help subcommands). MCP index.ts gets new `init` tool with Zod schema. package.json adds zod+@noble/curves+@noble/hashes deps, second `kysigned` bin entry. Frontend: `[TEST]` amber badge on dashboard envelope rows when chain_id==84532. Scripts: validate-pdf.ts prints "TESTNET — NOT LEGAL EVIDENCE" warning on Sepolia proof blocks. README + mcp/README + llms.txt updated with the init flow walkthrough. Real demos (slug→slug self-test, operator-email demo) stubbed pending the service-deploy path that lands in `kysigned customize`. 25 new MCP tests, 0 fail. Public repo 264 tests still passing.
...
- 2026-04-05: Phase 0 — created repos (kychee-com/kysigned, kychee-com/kysigned-private), cloned locally, workspace file ready
...
- 2026-04-05: Completed "Initialize kysigned-private repo" — package.json with file:../kysigned dep, tsconfig, .gitignore, src/index.ts importing from kysigned
...
- 2026-04-06: Phase 11 hypothesis cards drafted as `.xlsx` per saas-factory F6 v1.10.0 (new format requirement). Generator script at `kysigned-private/marketing/hypothesis-cards/generate.py` emits 4 cards (freelancers, solo-consultants, small-agencies, real-estate). Each card uses the canonical 14-field schema with a Status dropdown (Draft/Approved/Running/Won/Killed) and a separate Notes sheet for assumptions/open questions/risks. Real-estate card flagged with a BLOCKER REVIEW note: ESIGN/UETA compliance per jurisdiction must be confirmed before any spend. saas-factory spec bumped 1.9.1 → 1.10.0 (new requirement: cards are xlsx, not markdown; field count grew from 8 to 14; lifecycle Status field added).
...
- 2026-04-16: Completed 2R.B.A0 — evaluated `zkemail/cfdkim` vs `boundless-xyz/dkim`. Diff: 454 insertions / 298 deletions across 7 files, 36 commits. Classification: 0/8 findings fully fixed, 1/8 partially fixed (#3 LF-only body hash — the most catastrophic attack vector), 7/8 not addressed. Mechanical rule (≥5/8) NOT met, but switched anyway on maintenance grounds — boundless-xyz is dead, zkemail/cfdkim is alive and has the ZK Email team behind it. All 7 remaining findings still need in-guest hardening (2R.B.G8.*) regardless. Pin commit: `861f5e0`. DD-34 recorded with full classification table.
...
- 2026-04-09: **P4B.26 shipped + P4B.27 partial pass (DD-10 gate GREEN) + P4B.27 follow-ups tracked.**
  ...
  **Commits (all on local `main`, not yet pushed to origin):**
  ...
  - kysigned-private: `13af34b` blocks 3-6 TDD, `a617065` + `a7e89e5` + `5138edf` deploy iterations, `e04c499` Option A marketing site, `7d1c0c1` email bypass + publicRepo build
...
- 2026-04-14: **Completed Phase 7 AI task "Publish all legal docs on kysigned.com".** Added `kysigned-private/scripts/build-legal-pages.ts` (minimal MD→HTML converter: headings, bold, code, links, lists, pipe tables, paragraphs) + 6 unit tests. Generated terms/privacy/cookies/aup/dpa.html from `legal/*.md` (source of truth) with shared site template (nav/consent-banner/footer, DPA added to all footers). Wired generator into `deploy.ts` `stageMarketingSite` so future deploys always regenerate from markdown. **Deviations:** (1) added `--site-only` flag to `deploy.ts` to bypass pre-existing Phase 2R/3R tsc regressions in the kysigned public repo (apiRouter.ts imports removed `SignRequest`/`handleSign`/`handleVerifyByHash` exports) — blocking Lambda redeploy, out of Phase 7 scope. (2) Fixed two pre-existing kysigned prod-code tsc errors to unblock `buildPublicRepo`: `handleRemind` ctx type widened to include `operatorDomain?`, `recipientName`→`signerName` rename in `envelope.ts:624` signing-request template call. (3) Added `"exclude": ["src/**/*.test.ts", ...]` to kysigned tsconfig.json to keep prod `tsc` clean (matches kysigned-private's existing pattern). (4) Discovered run402 static hosting does NOT rewrite `/foo` → `/foo.html` (SPA fallback to index served instead) — swept all site hrefs to use `.html` (`/pricing`→`/pricing.html`, etc.) including nav + footer + consent-banner `cookieNoticeUrl`. **Smoke:** `https://kysigned.com/{terms,privacy,cookies,aup,dpa}.html` all return 200 with correct titles from a fresh mktemp dir. Test baselines unchanged: kysigned-private 68/68, kysigned 264/264. Subdomain re-claim returned 401 ("Invalid token") on redeploys — deployment still went live (confirmed via direct smoke), but the service key in AWS may be stale; flagged for future investigation.
- 2026-04-14: **Phase 13A IAM-sim passed + run402 service-key-rotation gap discovered (blocker for Phase 13A completion).** IAM simulate-principal-policy on `AgentDB-Pod01-TaskDefTaskRole1EDB4A67-XTUia2at8urw`: `kms:CreateKey/Sign/ScheduleKeyDeletion/CancelKeyDeletion` all allowed, `kms:Decrypt` implicitDeny — no drift. Also ran `bootstrap-run402.ts` — prototype tier renewed (expires 2026-04-21). BLOCKER: `kysigned/run402-service-key` JWT expired 2026-04-14 07:15 UTC (7-day lease). Paying for tier renewal does NOT re-derive the project's service_key — run402 currently has no rotation endpoint: keys are signed at project creation via `deriveProjectKeys(projectId, tier)` with `expiresIn` baked in, and neither `POST /tiers/v1/:tier` nor any admin endpoint re-issues them. This is a run402 architectural gap, not a kysigned issue. Phase 13A remaining tasks (Sepolia zero-address-deploy verification, billing-balance check, tier-expiration check, stale-email-domain SQL check) are blocked until run402 ships a rotation path. **To discuss with run402 dev:** two reasonable fixes — (a) `POST /tiers/v1/:tier` re-derives + returns service_keys for the wallet's projects when invoked as a renewal/upgrade, or (b) dedicated `POST /projects/v1/admin/:id/rotate-keys` endpoint (SIWX-admin or service_key auth). Option (a) matches user's mental model: "pay, extend" should extend the key too.
- 2026-04-14: **Phase 13A-6 (ops EOA funding) complete.** Ops EOA `0x8D671Cd12ecf69e0B049a6B55c5b318097b4bc35` funded with ~0.009 ETH on Base mainnet (≈ $21). Funded via Barry's Trezor → SideShift swap (ETH→Base ETH) → MetaMask + Trezor send to ops EOA. First attempt stuck on Trezor Suite's under-priced gas estimate (0.001146 gwei max fee vs 0.005 gwei base fee); resolved by resubmitting nonce 0 via MetaMask with default network-rate gas (~$0.002 cost). Balance ready for Phase 13B canary-wallet funding (~0.004 ETH) and Phase 13D production-wallet funding (~0.004 ETH) with ~0.001 ETH reserved as buffer.
- 2026-04-14: **Phase 13 ETH funding rescoped: 0.05 ETH → 0.01 ETH total.** Original 0.05 ETH figure (two × 0.02 ETH + buffer) was sized for "fuel both wallets through first month of real launch without refueling." Rescoped to match actual pilot volume: ~20 signatures per network, measured cost ≈ $0.05/sig on Base mainnet (432k gas × 0.05 gwei × $2500/ETH), ≈ $5 per contract deploy. Realistic pilot spend: ~$10; 0.01 ETH (~$25) gives 2-3x headroom. Production wallet is topped up again post-launch from revenue if traffic outpaces pilot. Per-wallet: 0.004 ETH each (canary + production) + 0.002 ETH buffer. Updated lines 1019, 1026, 1053. KMS wallet rent (~$1.20/mo × 2 wallets ≈ $5 across pilot) unchanged — paid in USD from run402 project credits, not ETH.
- 2026-04-14: **Plan continued — Spec-Version bumped 0.12.0 → 0.12.1.** Editorial-only spec change: `kysigned-service` renamed to `kysigned-private` throughout (F9.8, F10.12, F17.6, F17.9, F17.12, Phase 13 acceptance criteria). Plan already used `kysigned-private` — no task adjustments needed. No behavior changes. Proceeding with Phase 7.
- 2026-04-08: **Plan continued — DD-17 + Phase 13 re-sequenced for F17 dark-launch canary ritual.** Driven by spec v0.8.0 F17 (kysigned-spec) + saas-factory v1.15.0 F25 from the brainstorm→spec chain earlier in the day. Plan header Spec-Version bumped 0.7.0 → 0.8.0; Upstream References saas-factory 1.9.0 → 1.15.0. New DD-17 captures the canary ritual and resolves all five F17 Open Questions (#18–#22) inline: (18) run402 capability gap investigation via `Explore` subagent against `packages/gateway/src/routes/contracts.ts` + services/contract-wallets.ts + contract-call.ts — confirmed two wallets per project supported, rate limiting non-issue, drain endpoint fully implemented, BUT discovered NO dedicated deploy endpoint exists in contracts/v1 (this becomes a Phase 13A blocking prereq to verify the `POST /contracts/v1/call` + zero-address-target workaround OR land a small run402 enhancement), AND discovered rent is charged per-wallet not per-project so canary prepay is $2.40 not $1.20, AND the canary retirement MUST call `DELETE /contracts/v1/wallets/:id` after drain to stop daily rent accrual (draining ETH alone doesn't stop rent); (19) byte-identical bytecode check mechanism specified precisely — strip Solidity metadata suffix via `uint16_be(bytecode[-2:])` length algorithm then keccak256 compare, implemented as `kysigned-private/scripts/bytecode-identity-check.ts`; (20) full canary checklist enumerated (~40 items across dashboard/API/MCP/signing variations/retention/payment/monitoring/gas); (21) bytecode-divergence investigation playbook with 5 hypothesized causes + decision matrix; (22) production smoke specifics nailed down — API path, Barry→Tal single-signer, Method A auto-stamp, 15-minute window. Phase 13 restructured in place from "Gas Measurement & Final Pricing" to "Mainnet Deploy via Dark-Launch Canary" with 5 sub-phases (13A pre-flight → 13B canary provision+deploy → 13C dark-launch dogfood [BLOCKS ON Phase 4B completion in the parallel /implement chat] → 13D production provision+deploy+byte-identical-gate+flip+smoke → 13E canary retirement). Phase 14's existing "Flip public repo" task augmented with the F17.11 anti-leakage pre-squash scan as a hard gate: two `grep -rF` scans against the working tree for the retired canary contract address and retired canary wallet address, aborting the flip if either matches anywhere. Plan was NOT merged with the parallel Phase 4B chat — Phase 4B stays owned by the parallel /implement chat, Phase 13C explicitly declares the cross-chat dependency on Phase 4B Block 7 (e2e against production-deployed kysigned-private). No existing Phase 4, Phase 4A, or Phase 4B tasks were modified. All changes are additive to Phase 13 and Phase 14.
