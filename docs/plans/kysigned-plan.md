...
- [~] Implement Demo 1 (slug→slug self-test) — stubbed at `mcp/src/init/demo-stubs.ts` pending deployed kysigned-service. Stub returns clear "run kysigned customize to deploy" message so init bootstrap completes end-to-end. Real implementation blocked on the service-deploy path that lands with `kysigned customize`. [code]
...
- [x] Update llms.txt (kysigned-service) — added `init` to tool list + forker quickstart snippet. [manual]
