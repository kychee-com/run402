...
### DD-21: PLONK proof system (replacing Groth16)

- **Decision:** Use PLONK (via snarkjs/circom) instead of Groth16 for on-chain zk proof verification. Compile the circuit ourselves using `@zk-email/circuits` EmailVerifier as a library — no dependency on the zk-email SDK registry's Blueprint system.
...
### DD-22: Pricing model — $0.29 base + $0.10/extra signer
...
- [ ] **2R.10** Implement zk proof generation wrapper — takes validated DKIM-signed MIME + circuit inputs, generates the zk-SNARK proof with public signals: `pubkeyHash`, `emailCommit = Poseidon(email, docHash, envelopeId)` (document-scoped, not stable pseudonym), `subject`. Includes replay nullifier from DKIM signature bytes. Library/circuit chosen in Phase R. [code] `AI`
...
- 2026-04-11: **Pricing model changed to $0.29 base (2 signers) + $0.10/extra signer (DD-22).** Was $0.39 flat for 5 signers. New model keeps margins healthy with PLONK costs (41-52% across all tiers). Still dramatically cheaper than DocuSign. Spec F1.2, F2.9, F11.1, F11.4 updated.
