...
- [ ] **P0.1a** Create git worktree A: `git worktree add C:\Workspace-Kychee\kysigned-zkprover-A -b zkprover-A` from `main`. Confirm the worktree path exists and the new branch is checked out there. [infra] `AI`
- [ ] **P0.1b** Create git worktree B: `git worktree add C:\Workspace-Kychee\kysigned-zkprover-B -b zkprover-B` from `main`. [infra] `AI`
- [ ] **P0.1c** Create git worktree C: `git worktree add C:\Workspace-Kychee\kysigned-zkprover-C -b zkprover-C` from `main`. [infra] `AI`
- [ ] **P0.1d** Create git worktree D: `git worktree add C:\Workspace-Kychee\kysigned-zkprover-D -b zkprover-D` from `main`. [infra] `AI`
- [ ] **P0.1e** Verify all 4 worktrees exist and are on independent branches: `git worktree list` shows all four paths, `git branch -a` shows all four `zkprover-*` branches pointing at the same base commit but tracked independently. [infra] `AI`
- [ ] **P0.2** On `main` (public kysigned repo), create `kysigned/zkprover-candidates/` folder with a top-level `README.md` describing the four-candidate comparison and its purpose (links to this plan + spec), and a `shared/` subfolder that will hold the test input. Initial `shared/.gitkeep` so the folder exists before P0.3. [code] `AI`
- [ ] **P0.3** Acquire shared test input — Barry sends real email from Gmail to `barry@kychee.com` with body = `I APPROVE` and subject = `[kysigned] <envelopeId> <docHash>`. Raw `.eml` captured preserving all DKIM headers. Commit to `kysigned/zkprover-candidates/shared/test-input.eml`. [manual] `HUMAN`
- [ ] **P0.4** Verify shared test input validity: (a) DKIM signature verifies via `python3 -m dkim verify` or equivalent external tool, (b) body contains `I APPROVE` on a standalone line, (c) Subject matches canonical format, (d) `From:` is a real Gmail address. Document the provider, signing domain, and DKIM selector in `kysigned/zkprover-candidates/shared/README.md`. [code] `AI`
- [ ] **P0.5** Verify Base Sepolia wallet has sufficient ETH for 4 verifier deployments + 4 `verifyProof` transactions (~0.05 ETH estimate with margin). Fund from Base Sepolia faucet if low. Record wallet address and starting balance in the plan's Implementation Log. [infra] `AI`
- [ ] **P0.6** Prepare shared AWS environment: confirm `kychee` profile + `us-east-1` region active, confirm `EC2SSMInstanceProfile` exists and usable, establish tagging convention for all four candidates (`Purpose=kysigned-zkprover-<letter>`). Record in Implementation Log. [infra] `AI`
- [ ] **P0.7** Fast-forward `zkprover-A`, `zkprover-B`, `zkprover-C`, `zkprover-D` branches to include `main`'s post-Phase-0 state so that all four worktrees have the shared test input + folder structure available when their sub-plans start. Use `git merge --ff-only` in each worktree to pull in the main's latest. [infra] `AI`
- [ ] **P0.8** Commit + push `main` so that the post-Phase-0 state is fully in origin/main (safety net so sub-plans can resume from origin if a local worktree is lost). [infra] `AI`
- [ ] **P0.9** Signal ready-for-parallel: tell the user "Phase 0 complete. Open four separate Claude Code instances — one in each of the four worktree paths — and run `/implement zkprover-A` (and B, C, D) in each. When all four are in terminal state, return to this master plan instance and run `/implement zkprover` again to continue with Phase 2." [manual] `AI`
