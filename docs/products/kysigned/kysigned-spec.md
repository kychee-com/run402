...
> **v0.13.0 revision (2026-04-15)** — consultation-driven update addressing findings in [docs/consultations/kysigned-crypto-attack-review.md](../../consultations/kysigned-crypto-attack-review.md). Tightens on-chain binding: the SNARK journal is the only source of truth for `docHash`, `envelopeId`, `nullifier`, `evidenceKeyId`, and `searchKey` (F4.12). Adds the `pubkeyHash ↔ evidenceKey.publicKey` binding rule (F4.12) that the F4.9 consistency argument implicitly depends on. Adds F5.2a (client-side proof re-verification), F4.F (permanence & finality model, softens unconditional "permanent" language per the Base L2 reality), F4.10.2 (docHash stored as 32 raw bytes), and timestamp authority = `block.timestamp`. `recordCompletion` reclassified as advisory metadata (F4.15). Method B revival prerequisites documented in Future Features. All corresponding acceptance criteria added.
...
- F4.9.0. **Load-bearing dependency on F4.12 binding (v0.13.0).** The F4.9 consistency argument is *only* cryptographically meaningful if the on-chain contract enforces the `pubkeyHash ↔ evidenceKeyId` binding specified in F4.12. Without that binding, an attacker could generate a proof using an attacker-controlled key and label the record with a legitimate `evidenceKeyId` — the fake-key proof would be indistinguishable from a genuine one in the on-chain record, and would *pass* the consistency heuristic (because the attacker's record would reference the same widely-used legitimate `keyId`). F4.9 and F4.12 ship together; neither alone is sufficient.
...
- F4.10. **Record structure:** each reply-to-sign signature record contains:
  - `searchKey` — document+email commitment that a verifier with `(email, docHash)` can independently compute and match against the on-chain record. See F4.10.3 for binding rule and F4.11 for privacy model.
  - `docHash` — SHA-256 of the original document, stored and committed as 32 raw bytes.
  - `envelopeId` — unique envelope identifier.
  - `evidenceKeyId` — reference to the DKIM key entry in `EvidenceKeyRegistry`.
  - `timestamp` — set by the contract to `block.timestamp` at write time. **Not caller-supplied.** Authoritative signing time = tx inclusion time. A DKIM-signed `Date:` header from the original reply MAY additionally be committed to the journal as a secondary audit anchor, but `block.timestamp` is the canonical on-chain time.
  - `nullifier` — derived inside the zkVM guest from the canonical DKIM-Signature bytes (see F3.3.4) and committed to the journal. The contract reads the nullifier from the proof journal, not from calldata.
  - Bulky data (zk proof bytes / RISC Zero seal, journal digest, journal bytes) emitted via events (see F5.2a for emission requirements).
- F4.10.1. **All record fields are proof-bound.** Every field stored in the `ReplyToSignRecord` struct and every field indexed in the `ReplyToSignRecorded` event MUST either be (a) committed directly by the zkVM guest to the proof journal, or (b) deterministically derived by the contract from values committed to the journal. Caller-supplied fields that are not cross-checked against the journal are forbidden. See F4.12 for the binding enforcement rule.
- F4.10.2. **`docHash` encoding (v0.13.0).** `docHash` is SHA-256 output, 32 bytes. It is stored as `bytes32` on-chain, committed to the proof journal as 32 raw bytes (or as two field-element limbs if the proof system requires), and never reduced modulo a field prime without explicit limb-splitting. Independent verifiers and the contract MUST use byte-identical encoding so that two different SHA-256 outputs cannot collapse to the same on-chain identifier.
- F4.10.3. **`searchKey` is journal-bound (v0.13.0).** `searchKey` MUST be committed to the proof journal by the zkVM guest. Caller-supplied `searchKey` is not acceptable — the contract reads `searchKey` from the journal, not from calldata. The guest's `searchKey` function must be: (i) deterministic given `(email, docHash)`, so a verifier with both inputs can recompute it locally during `/verify` (F5.2) and match the on-chain record, and (ii) efficient to compute inside the zkVM guest without dominating proof cost. The concrete function is a plan-level decision. See Open Questions on the privacy/provability trade-off.
- F4.11. **Privacy (tiered exposure model, cost-amplification not anonymity — v0.13.0 framing):** No email plaintext or stable email identifier is on-chain. The email commitment is document-scoped (`emailCommit = Poseidon(email, docHash, envelopeId)`) — different for every document, preventing cross-document linkability. The `searchKey` function (F4.10.3) provides **cost-amplification against bulk scraping**, not cryptographic anonymity against targeted attackers with a known candidate signer set. Any claim stronger than cost-amplification (e.g., "anonymous signing", "unlinkable signatures") is out of scope for this spec. Privacy degrades with attacker knowledge:
...
- F4.12. **On-chain binding rule (v0.13.0 — addresses [consultation](../../consultations/kysigned-crypto-attack-review.md) findings #1, #6).** On each write to `SignatureRegistry.recordReplyToSignSignature`, the contract MUST enforce the following cryptographic bindings *before* accepting the record. Any failure reverts the tx.
  - **(a) Journal-digest reconstruction.** The contract reconstructs the expected `journalDigest` from the proof journal bytes (or equivalently, from the caller-supplied record fields after they are shown to match the journal — see sub-items below), and passes that reconstructed digest to the verifier. It is forbidden to accept a caller-supplied `journalDigest` opaque to the contract. The verifier's `verify(seal, imageId, expectedJournalDigest)` either succeeds (proof is sealed to that exact journal) or reverts (tampering / relabel attempt).
  - **(b) `pubkeyHash ↔ evidenceKeyId` binding.** The contract fetches `EvidenceKeyRegistry.getEvidenceKey(evidenceKeyId).publicKey` and computes its hash using the same hash function the zkVM guest uses to derive `pubkeyHash`. The result MUST equal the `pubkeyHash` committed in the proof journal. A mismatch means the caller is labeling an attacker-key proof with a legitimate evidence key — tx reverts.
  - **(c) Nullifier sourced from journal.** The `nullifier` used for replay protection (stored in `_usedNullifiers`) MUST be the nullifier committed in the proof journal, not a separate calldata parameter.
  - **(d) Record fields equal journal fields.** For every stored/indexed field (`searchKey`, `docHash`, `envelopeId`, `evidenceKeyId`), the value written to storage and emitted in the event MUST equal the corresponding journal commitment (or, for `evidenceKeyId`, the registry-keyed reference whose `publicKey` satisfies (b)).
  - **(e) Permissible implementation shape.** Either (i) the function signature omits calldata copies of journal-bound fields and the contract reads them from the journal directly, or (ii) calldata copies are accepted only as a convenience and the contract requires byte-equality with the corresponding journal values before using them. Both patterns are acceptable; unverified calldata fields are not.
...
- F4.15.1. **Advisory metadata only (v0.13.0).** `recordCompletion` is **not** cryptographically authenticated — the function is permissionless and accepts any values. It is optimization for UI/indexers, not a trust anchor. **Authoritative completion is computed by clients** walking the `ReplyToSignRecord[]` array for a given `envelopeId` under `_replyToSignRecords` (via `searchKey` lookups) and counting records whose `docHash` matches. The `recordCompletion` record MUST NOT be treated as proof that the envelope is complete. Verifier UIs (F5) MUST surface completion state computed from the signature records, optionally cross-referenced against the `recordCompletion` metadata with a clear UI indication that divergence is possible and the authoritative count is the per-signer record count.
- F4.15.2. **UI must not claim "completed" from recordCompletion alone.** Any UI label, proof link, or completion email that says "envelope is complete" MUST derive that claim from the actual signature-record count, not from the `recordCompletion` event. Wording like "recorded completion (advisory)" is acceptable when specifically surfacing the advisory record; unqualified "completed" claims MUST be backed by signature-record evidence.

#### F4.F Permanence & Finality Model `[both]`

**Principle (v0.13.0):** kysigned records are permanent in the cryptographically meaningful sense — all record data (events, calldata, proof bytes, journal bytes) is posted to Ethereum L1 via Base's data-availability layer — but "permanent" is not "instantaneously final." This section documents the honest finality model so that UI, LEGAL.md, and ToS language can be calibrated.

- F4.16. **Two-tier finality.**
  - **L2 finality (~minutes):** After ~1-2 L2 confirmations, reorg is practically negligible for UI purposes. Signing confirmation emails, dashboard status changes, and proof links MAY treat an envelope as "signed on-chain" at this tier.
  - **Ethereum L1 hard finality (~7 days):** Base inherits Ethereum security via the L2 fraud-proof challenge window. Claims that require the strongest possible permanence guarantee (high-stakes legal audit, arbitration evidence) SHOULD cite L1 finality, not L2.
- F4.17. **Data availability independent of Base sequencer.** All `recordReplyToSignSignature` calldata (including the RISC Zero seal, journal bytes, and record fields) is posted to Ethereum L1 via Base's batch submission. Historical records survive even if the Base sequencer ceases operating, is censored, or the Base project is discontinued. Verifiers can reconstruct every signature record from L1 blob/calldata data alone; proofs remain cryptographically valid regardless of which chain hosts them.
- F4.18. **Short-term caveats (first ~24h).** In the first ~24 hours after a tx, the Base sequencer (currently Coinbase-operated) could theoretically delay or reorder inclusion. **Forced inclusion via L1 is available** as an unconditional escape hatch — any user can bypass the Base sequencer by submitting directly to the L1 bridge contract. kysigned UI MUST NOT claim "nobody can refuse to record" as an unqualified statement.
- F4.19. **User-facing language calibration.** Words like "permanent," "immutable," and "cannot be modified" are accurate post-finality and are acceptable in marketing and LEGAL.md copy when accompanied by the tier model above or when referring to the L1-anchored data. Unqualified real-time claims (e.g., "your signature is permanent the moment you click send") are not accurate and MUST NOT be used. The "how it works" page (F11.7) MAY simplify to "it's stored on a public blockchain and cannot be removed" without reciting the finality tiers, because the L1-anchored data-availability property makes this substantively true for a non-technical audience.
- F4.20. **Future L1 anchoring (deferred).** Periodic L1-anchored checkpoints (merkle root of recent records posted directly to L1) are a candidate enhancement if a customer use case requires L1-speed hard finality. Additive — no spec change required when implemented.
...
- F5.2. Verification page at `/verify` accepts a PDF upload and an email address. It computes `docHash = SHA-256(pdf_bytes)` and the `searchKey` (using the same function the zkVM guest commits per F4.10.3 — verifier recomputes locally), then queries the canonical `SignatureRegistry` contract(s) for a matching record.
- F5.2a. **Client-side proof re-verification is mandatory (v0.13.0 — addresses [consultation](../../consultations/kysigned-crypto-attack-review.md) finding #11).** The verification page MUST cryptographically re-verify the RISC Zero proof itself, not merely read and trust the on-chain record. Specifically: the page retrieves `(seal, journalDigest, journal_bytes)` from the signature event (or from tx calldata if any component is not event-emitted), reconstructs the expected journal from the record fields, hashes it to `expectedJournalDigest`, and calls `IRiscZeroVerifier.verify(seal, imageId, expectedJournalDigest)` as an `eth_call` (view) against the deployed verifier contract. The UI MUST distinguish three outcomes: (a) "proof re-verified" (green), (b) "record found but proof re-verification failed" (red — indicates contract write-path bug or tainted verifier artifact), (c) "no record found" (neutral). Outcome (b) is a trust-model alarm and MUST be surfaced prominently, not silently treated as success.
- F5.2b. **Event emission requirements (v0.13.0).** The `ReplyToSignRecorded` event (and any related events needed to reconstruct the proof) MUST emit sufficient data for F5.2a to succeed without requiring tx-calldata indexing. At minimum: the RISC Zero `seal` bytes, the `imageId`, the `journalDigest`, and the raw `journal_bytes` (so the verifier can independently reconstruct the digest and cross-check field-by-field). If any of these are too bulky for an event, the spec's "bulky data in events" rule in F4.10 allows emission as a separate auxiliary event, but MUST NOT require reading tx calldata.
- F5.3. If a record is found, the page:
  1. Retrieves the RISC Zero seal, journal bytes, and journal digest from the signature events (see F5.2b).
  2. Looks up the `evidenceKeyId` in `EvidenceKeyRegistry` to get the DKIM public key + registration timestamp.
  3. Re-verifies the RISC Zero proof (F5.2a).
  4. Cross-checks the proof journal against the on-chain record fields — the journal's `pubkeyHash`, `docHash`, `envelopeId`, `searchKey`, and `nullifier` MUST match the record (F4.10.1 / F4.12).
  5. Confirms the journal's `pubkeyHash` equals `hash(evidenceKey.publicKey)` (F4.12.b).
  6. Checks key consistency: the same `keyId` is referenced by other signatures from the same provider during the same period (corroboration — see F4.9).
  7. Displays result: "This email signed this document at [block.timestamp]. Proof re-verified independently — this page does not trust the on-chain record's existence alone."
...
- F12.7. `[repo]` **LEGAL.md** — disclaimers separate from the MIT license:
  ...
  - Smart contract permanence disclaimer (recordings are anchored on Base with all data posted to Ethereum L1; post-finality they cannot be deleted or modified by anyone. Short-term caveats per F4.F apply: L2 finality lags ~minutes; Ethereum L1 hard finality ~7 days; forced inclusion via L1 is the escape hatch for sequencer issues.)
...
- [ ] Reply-to-sign signature record keyed by `searchKey` (journal-bound per F4.10.3); contains `docHash`, `envelopeId`, `evidenceKeyId`; record `timestamp` equals `block.timestamp` (not caller-supplied); nullifier sourced from proof journal; zk proof seal + journal bytes + journal digest emitted as event
- [ ] `docHash` committed and stored as 32 raw bytes (bytes32); two different SHA-256 outputs cannot collapse to the same on-chain identifier (F4.10.2)
- [ ] **Binding rule F4.12(a): journal-digest reconstruction.** Contract reconstructs expected `journalDigest` from journal bytes / record fields and passes that to the verifier; opaque caller-supplied digest is rejected. Test: submit valid seal with arbitrary relabeled record fields → tx reverts with binding-mismatch error
- [ ] **Binding rule F4.12(b): pubkeyHash ↔ evidenceKey.publicKey.** Contract fetches `EvidenceKeyRegistry.getEvidenceKey(evidenceKeyId).publicKey`, hashes it, and requires equality with `journal.pubkeyHash`. Test: submit a proof generated against one key with a different evidenceKeyId → tx reverts
- [ ] **Binding rule F4.12(c): nullifier from journal.** `_usedNullifiers` marks the nullifier committed in the proof journal, not a calldata parameter. Test: attempt to reuse a seal with a fresh calldata nullifier → tx reverts on nullifier-already-used (journal nullifier is already marked)
- [ ] **Binding rule F4.12(d): field equality.** Every stored/indexed field matches its journal counterpart byte-for-byte. Test: fuzz-submit records where any one field diverges → tx reverts
- [ ] `ReplyToSignRecorded` event (and any auxiliary events) carries all data needed by F5.2a client re-verification without requiring tx-calldata indexing
...
- [ ] `recordCompletion` advisory metadata: permissionless, accepts any values, MUST NOT be treated as proof of completion. Verifier UIs compute completion by counting per-signer records (F4.15.1 / F4.15.2). Test: submit a forged `recordCompletion` for an envelope with zero real signatures → UI does NOT display "completed"
- [ ] No entry on either contract can be modified or deleted after recording
- [ ] Both contracts are permissionless (any funded EOA can write for reply-to-sign records after all F4.12 checks pass) and ownerless (no admin functions)
- [ ] F4.F permanence & finality: UI surfaces two-tier finality (L2 ~minutes, L1 ~7 days); "permanent" claims in marketing/LEGAL.md are qualified per F4.19; no unqualified real-time "nobody can refuse" claims

### F5. Verification
- [ ] Verification page accepts `(email, document PDF)` as inputs; computes `searchKey` using the same function committed by the zkVM guest (F4.10.3); finds matching reply-to-sign record
- [ ] **F5.2a: Client-side proof re-verification.** Verification page cryptographically re-verifies the RISC Zero proof (calls `IRiscZeroVerifier.verify(seal, imageId, expectedJournalDigest)` as an `eth_call`), does NOT merely trust the on-chain record. UI distinguishes three outcomes: (a) re-verified green, (b) record found but re-verify failed red (trust-model alarm, prominent), (c) no record neutral
- [ ] F5.2a test: tamper a known-good record (e.g., swap journal digest against a mocked verifier) → UI shows outcome (b), not silent success
- [ ] Verification cross-checks proof journal against on-chain record field-by-field (pubkeyHash, docHash, envelopeId, searchKey, nullifier); confirms `pubkeyHash == hash(evidenceKey.publicKey)`; checks key-consistency corroboration
- [ ] `ReplyToSignRecorded` + auxiliary events carry `seal`, `imageId`, `journalDigest`, and `journal_bytes` (or equivalent indexed event fields) so F5.2a does not depend on tx-calldata indexing
...
- [ ] Proof link's "completed" label derives from per-signer record count, not from `recordCompletion` advisory metadata (F4.15.2)
...
- **DKIM verifier audit (v0.13.0, mandatory pre-launch).** The DKIM verification code path running inside the zkVM guest (currently `cfdkim`, a Cloudflare-origin Rust library not previously audited in a ZK context — see [consultation](../../consultations/kysigned-crypto-attack-review.md) finding-adjacent risk "cfdkim trust gap") MUST undergo documented multi-pass review before mainnet launch. Review mechanism is specified in the plan (minimum: two internal code-review passes against RFC 6376 + a dedicated `/consult` session scoped specifically to DKIM-library soundness). No external security firm required for MVP; if review finds defects the library is replaced with a port of an already-audited DKIM implementation (e.g., `@zk-email/circuits` Circom DKIM ported to Rust). This is a hard launch gate — canary/mainnet deploy is blocked until the review is complete and all findings are resolved or explicitly accepted.
...
>
> **Revival prerequisites (v0.13.0 — from [consultation](../../consultations/kysigned-crypto-attack-review.md) findings #13–15, #20).** If Method B is ever re-enabled, the following issues documented in the 2026-04-15 consultation MUST be addressed before exposing any UI path. This list is the minimum gate; it is not a full re-design.
> - **Used-digest replay protection.** The current `recordWalletSignature` function in the immutable contract lacks a used-digest mapping. Re-enablement MUST add an off-contract or on-contract replay guard (since the contract itself cannot be changed, the replay guard ships as a new companion contract or an off-chain/indexer-level uniqueness check — the public indexer MUST deduplicate and MUST NOT display duplicate records as separate signatures).
> - **ECDSA low-`s` enforcement.** `ecrecover` on arbitrary payloads accepts both high- and low-`s` signatures (EIP-2's low-`s` rule applies to tx signatures, not arbitrary `ecrecover`). Revival requires enforcing low-`s` in the caller / indexer so malleable duplicates cannot accumulate.
> - **Envelope binding on verify.** `verifyWalletSignature(documentHash, expectedSigner)` currently ignores `envelopeId`, `documentName`, `signerEmail`, and `timestamp`. A wallet signature for envelope A can satisfy a verify call meant for envelope B if the underlying `documentHash` is reused. Revival requires a verify helper (library or UI) that checks the full `(documentHash, envelopeId, expectedSigner)` tuple against the record, not just `(documentHash, expectedSigner)` membership.
> - **Mailbox-ownership caveat in ToS and UI.** Wallet signing proves "wallet X attested to the string `signerEmail = alice@example.com`" — it does not prove that the wallet owner controls that mailbox. Revival requires explicit LEGAL.md and ToS language stating the identity semantics, distinct from the reply-to-sign semantics. Verifier UIs MUST label wallet-signed records differently from reply-to-sign records so integrators cannot confuse the two.
> - **PII in events.** `WalletSignatureRecorded` emits plaintext `documentName` and `signerEmail`. Revival requires either (a) hashed/committed variants in the event (with plaintext off-chain only), or (b) an explicit user consent flow at signing time acknowledging that `documentName` and `signerEmail` will be public on-chain.
