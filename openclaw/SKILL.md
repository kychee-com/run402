...
`run402 buzz status` reports independent skill, adoption-offer/adoption, community-installation, and enrollment states. Use `run402 buzz adopt offer` for the canonical durable HTTPS ownership handoff, and poll `offer show`; opening a link is never completion. `buzz adopt direct` is advanced compatibility. Do not infer org membership or grants from a shared skill or public Buzz proof. MCP remains read-only for this signing/passkey-sensitive workflow and returns exact HTTPS/CLI handoffs. Community enrollment creates no agent org membership or payment authority and preserves the org-of-one fallback when absent, ambiguous, declined, denied, stale, expired, or revoked.
