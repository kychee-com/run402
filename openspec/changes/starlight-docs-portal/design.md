...
- An informational `R402_*` error-code reference for the supervising developer (a human-readable mirror — NOT the target of the error envelopes, which resolve to `run402.com/errors/`; see D7).
...
Implementation discovery: the CLI's `run402 deploy apply --dir <build>` is **Astro-SSR-adapter only** — `mergeAstroReleaseSlice` imports `@run402/astro/release-slice`, requires `@run402/astro`, and reads `dist/run402/adapter.json`. A **plain static** Starlight `dist/**` (Fork 1) has no adapter manifest, and the CLI's site spec is otherwise per-file. So a build step (`scripts/build-docs-deploy-manifest.mjs`) walks `docs-site/dist/**`, enumerates every file into `site.replace` as a `{ path }` ref, and declares **explicit** `public_paths` mapping clean URLs (`dist/getting-started/index.html` → `/getting-started/` + `/getting-started`; `_astro/*` and `pagefind/*` at their paths), plus the four flat files from their canonical repo-root paths (so they serve at `/llms-*.txt` + `/SKILL.md`). **Explicit — not implicit — is load-bearing** (implementation discovery): `release-state.ts` sets `spa_fallback="/index.html"` whenever `mode==="implicit"` and a root `index.html` exists, which serves the home page with HTTP 200 for every unknown URL. A multi-page docs site wants real 404s, so we enumerate clean URLs instead. The result is fed to the **same** `run402 deploy apply --project … < run402.docs.deploy.json` OIDC invocation the docs project already uses — no SSR runtime, no new auth path. The manifest is a build artifact (gitignored), regenerated each deploy. `.txt`/`.md`/`.html` MIME is correct by extension (predecessor D5); a post-deploy `curl` smoke check guards regression. Starlight's HTML routes (`/`, `/cli/reference/`, …) never collide with the flat-file paths.
...
### D7 — The error reference is an informational mirror, NOT an astro-ssr-runtime dependency (corrected)

Investigation correction: an earlier draft treated this portal as the target of the `astro-ssr-runtime` error envelopes' `docs` field. It is not. That change **already shipped its own error reference at `run402.com/errors/`** (per-code anchors like `#R402_ASTRO_BUILD_FAILED`, live) — it routed around this docs site not existing yet (its tasks note: *"Blocked on docs-site infra … error-envelope URLs currently 404"*, since resolved by building `run402.com/errors/`). So `docs.run402.com` is **not** a dependency of `astro-ssr-runtime`, and there is **no blocking coordination**. v1 seeds a consolidated `reference/error-codes` page from run402-private `error-codes.md` purely as a human-readable convenience mirror. Consolidating the canonical error docs from `run402.com/errors/` onto this portal (and re-pointing the in-code `docs:` URLs) is an **optional future change**, out of scope here.
...
- **Unknown URLs silently 200 (SPA fallback)** → use **explicit** `public_paths`, not implicit (D3): implicit + a root `index.html` makes the gateway serve the home page for any unmatched path. Explicit clean-URL enumeration yields real 404s.
