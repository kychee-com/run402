...
The `@run402/functions` module SHALL export a `getUser` function alongside the `db`, `adminDb`, `email`, and `ai` exports. Functions import it as `import { getUser } from '@run402/functions'`.
...
The `getUser` implementation SHALL exist in the `@run402/functions` npm package at `run402-public/functions/`. Both deployed-function execution and local dev mode SHALL resolve `getUser` from this package — there SHALL be no separate inlined copy and no Lambda-layer-specific path.
...
In **local dev mode**, the function process resolves `@run402/functions` from the gateway's `node_modules` (workspace symlink during development; registry install in CI/prod).
