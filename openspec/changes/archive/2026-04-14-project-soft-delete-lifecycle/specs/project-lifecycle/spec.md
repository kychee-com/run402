...
The platform SHALL expose an admin-authenticated endpoint `POST /admin/projects/:id/reactivate` that transitions a project from any non-terminal lifecycle state back to `active`. The endpoint SHALL be callable only by operators with the `@kychee.com` admin identity.

#### Scenario: Operator reactivates a dormant project for a paying customer
- **WHEN** an operator sends `POST /admin/projects/:id/reactivate` for a project in `status = 'dormant'`
...
- **WHEN** a non-admin caller sends `POST /admin/projects/:id/reactivate`
- **THEN** the platform SHALL respond `403 Forbidden`

#### Scenario: Operator attempts to reactivate a purged project
- **WHEN** an operator sends `POST /admin/projects/:id/reactivate` for a project in `status = 'purged'` or `status = 'archived'`
