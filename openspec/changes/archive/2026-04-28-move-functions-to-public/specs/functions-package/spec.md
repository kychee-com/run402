...
#### Scenario: Legacy name not available
- **WHEN** a user writes `import { db } from 'run402-functions'`
- **THEN** the import SHALL fail with a standard module-not-found error (no alias is provided in the package or in the deploy bundler)
...
The package SHALL read configuration from environment variables at call time (lazy getters, not at import time): `RUN402_API_BASE` (default: `https://api.run402.com`), `RUN402_PROJECT_ID`, `RUN402_SERVICE_KEY`, `RUN402_JWT_SECRET`. No explicit initialization call SHALL be required.
...
The gateway's local function execution mode SHALL resolve `@run402/functions` from the monorepo workspace (or from a registry install if the gateway has no workspace link to the public repo).
...
- **THEN** the import SHALL resolve to the gateway's installed `@run402/functions` package (workspace link in dev, registry install in CI/prod)
...
**Reason:** The Lambda layer is being removed in the companion `drop-functions-layer-and-fix-deps` change. `@run402/functions` will be bundled into each function zip at deploy time via esbuild instead. Layer-coupled requirements no longer apply.

**Migration:** None for users. The package surface (exports, types, env-var configuration) is unchanged. Only the gateway-side delivery mechanism changes.
