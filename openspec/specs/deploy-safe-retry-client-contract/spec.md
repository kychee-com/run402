...
`r.project(id).apply(spec)` SHALL automatically retry deploy races only when the gateway error is a deploy error with `safe_to_retry: true`, the error code is in the SDK's deploy safe-retry allowlist, and the release spec is auto-rebasable.

#### Scenario: Base release conflict replans and succeeds
- **WHEN** `r.project(id).apply(spec)` is called with `spec.base` omitted or `{ release: "current" }`, the first attempt fails with `Run402DeployError` code `BASE_RELEASE_CONFLICT` and `safe_to_retry: true`, and a later attempt succeeds
...
- **WHEN** `r.project(id).apply(spec)` observes `BASE_RELEASE_CONFLICT` with `safe_to_retry: false` or without `safe_to_retry: true`
- **THEN** the SDK SHALL throw the deploy error without an automatic retry

#### Scenario: Retry requires allowlisted code
- **WHEN** `r.project(id).apply(spec)` observes a deploy error with `safe_to_retry: true` but a code outside the deploy safe-retry allowlist
- **THEN** the SDK SHALL throw the deploy error without an automatic retry

#### Scenario: Retry ignores generic retryable signal
- **WHEN** `r.project(id).apply(spec)` observes a deploy error with `retryable: true` but without `safe_to_retry: true`
- **THEN** the SDK SHALL throw the deploy error without an automatic retry
...
- **WHEN** `r.project(id).apply(spec)` is called without `spec.base` and an otherwise retryable `BASE_RELEASE_CONFLICT` occurs
- **THEN** the SDK SHALL treat the spec as eligible for automatic retry

#### Scenario: Current base is auto-rebasable
- **WHEN** `r.project(id).apply(spec)` is called with `spec.base` equal to `{ release: "current" }` and an otherwise retryable `BASE_RELEASE_CONFLICT` occurs
- **THEN** the SDK SHALL treat the spec as eligible for automatic retry

#### Scenario: Pinned release base is not auto-rebasable
- **WHEN** `r.project(id).apply(spec)` is called with `spec.base` equal to `{ release_id: "rel_..." }` and a deploy error has `safe_to_retry: true`
...
- **WHEN** `r.project(id).apply(spec)` is called with `spec.base` equal to `{ release: "empty" }` and a deploy error has `safe_to_retry: true`
...
- **WHEN** `r.project(id).apply(spec)` is called without a retry budget override
...
- **WHEN** `r.project(id).apply(spec, { maxRetries: 0 })` observes an otherwise retryable deploy race
- **THEN** the SDK SHALL throw the first deploy error without issuing another plan request

#### Scenario: Custom retry budget
- **WHEN** `r.project(id).apply(spec, { maxRetries: N })` is called with a positive integer `N`
- **THEN** the SDK SHALL allow at most `N` automatic retries after the initial attempt

#### Scenario: Invalid retry budget
- **WHEN** `r.project(id).apply(spec, { maxRetries })` is called with a negative, non-finite, or non-integer retry budget
...
- **WHEN** `r.project(id).apply(spec, { onEvent })` schedules an automatic retry
...
- **WHEN** every allowed `r.project(id).apply(spec)` attempt fails with an automatically retryable deploy error
...
- **WHEN** `r.project(id).apply(spec)` exhausts its automatic retry budget
...
- **WHEN** `r.project(id).apply.start(spec)` observes a deploy error that would be eligible under the `apply` safe-race retry policy
- **THEN** `r.project(id).apply.start(spec)` SHALL preserve its existing behavior and throw or reject without the new automatic apply retry loop

#### Scenario: Low-level commit is not auto-retried
- **WHEN** `r.project(id).apply.commit(planId)` observes `BASE_RELEASE_CONFLICT` with `safe_to_retry: true`
...
`r.project(id).apply()` and `r.project(id).apply.resume()` SHALL stop polling promptly when an operation snapshot indicates an activation failure that cannot be repaired by waiting or automatic retry.
...
- **THEN** `r.project(id).apply()` SHALL throw a `Run402DeployError` without waiting until the poll timeout
...
- **THEN** `r.project(id).apply()` SHALL throw the error
