...
The `getUser(req)` helper exported from `@run402/functions` — verifies the caller's JWT and returns user identity inside a deployed Run402 serverless function.

## Purpose

Functions need a reliable way to identify the authenticated caller without each function rolling its own JWT verification. `getUser(req)` extracts the Bearer token from the request, verifies the signature against `RUN402_JWT_SECRET`, validates the project_id claim matches `RUN402_PROJECT_ID`, and returns `{id, role, email}` (or `null` for any failure mode).

Synchronous (HMAC verification is CPU-only). Designed for the inside-a-function ergonomic surface, not the outside-the-platform `@run402/sdk` shape.

## Requirements

### Requirement: getUser export from @run402/functions
The `@run402/functions` module SHALL export a `getUser` function alongside the `db`, `adminDb`, `QueryBuilder`, `email`, and `ai` exports. Functions import it as `import { getUser } from '@run402/functions'`.
...
The gateway SHALL inject `RUN402_JWT_SECRET` as an environment variable when deploying functions to Lambda, alongside the existing `RUN402_PROJECT_ID`, `RUN402_API_BASE`, `RUN402_SERVICE_KEY`, and `RUN402_ANON_KEY` variables. The value SHALL be the same `JWT_SECRET` used by the gateway to sign all project JWTs.
...
- **THEN** the function process SHALL have access to `RUN402_JWT_SECRET` via environment variable

### Requirement: getUser uses jsonwebtoken for verification
The `getUser` function SHALL use the `jsonwebtoken` package (a runtime dependency of `@run402/functions`) to verify the token signature, check expiry, and extract claims. It SHALL NOT use manual base64 decoding or custom verification logic. The import SHALL be a static ES import (`import jwt from "jsonwebtoken"`) so esbuild can statically follow it during the function-deploy bundling step (per the companion private change).
...
The `getUser` function SHALL return the identity fields present in the JWT: `sub` (mapped to `id`), `role`, and `email`. Functions that need additional user data beyond these fields (e.g., display_name, avatar_url) SHALL query for it via `db(req)`.
...
- **THEN** it SHALL use `getUser(req)` to obtain the user ID, then query via `db(req).from('profiles').select('*').eq('user_id', user.id)` or equivalent
...
### Requirement: getUser available in deployed and local dev modes
The `getUser` implementation SHALL exist in the `@run402/functions` npm package at `functions/`. Both deployed-function execution and local dev mode SHALL resolve `getUser` from this package — there SHALL be no separate inlined copy and no Lambda-layer-specific path.

In **deployed mode**, the gateway's bundler inlines `@run402/functions` into the function zip at deploy time (per the companion `drop-functions-layer-and-fix-deps` change). The runtime resolves `getUser` from the bundled module.

In **local dev mode**, the function process resolves `@run402/functions` from the gateway's `node_modules` (registry install or workspace link).

Both modes SHALL behave identically.

#### Scenario: Deployed mode
- **WHEN** a function deployed to Lambda calls `getUser(req)`
- **THEN** it SHALL work using the bundled `@run402/functions` and the `RUN402_JWT_SECRET` env var

#### Scenario: Local dev mode
- **WHEN** a function running in local dev mode calls `getUser(req)`
- **THEN** it SHALL work using the `@run402/functions` package resolved from the gateway's `node_modules` and the `RUN402_JWT_SECRET` env var
