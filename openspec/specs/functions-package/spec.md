...
The `run402-functions` npm package SHALL export the following named exports: `db`, `getUser`, `email`, `ai`. These SHALL be importable via `import { db, getUser, email, ai } from 'run402-functions'`. The legacy import path `run402-functions` SHALL also work in deployed Lambda functions via a symlink alias in the layer.
