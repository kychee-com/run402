...
/**
 * Every tool's arguments are strict: an undeclared argument is refused with a
 * structured UNKNOWN_ARGUMENT error (./input-guard.ts), never dropped.
 */
const INPUT_SCHEMAS = {
  up: strictInput(upSchema),
  deploy: strictInput(deploySchema),
  status: strictInput(statusSchema),
  whoami: strictInput(whoamiSchema),
  doctor: strictInput(doctorSchema),
  docs: strictInput(docsSchema),
  run: strictInput(runSchema),
  expand_result: strictInput(expandResultSchema),
};
...
guardToolArguments(transport, INPUT_SCHEMAS);
