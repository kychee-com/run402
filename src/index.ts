...
// Unified project transfer — wallet and email (accept), owned-org (immediate) recipient shapes
import {
...
server.tool(
  ...
  "Initiate a project transfer (owned-org recipient shape v1.96+). Addressed to a WALLET (`to_wallet`) or an EMAIL (`to_email`) — both completed by `accept_project_transfer` — OR an owned ORG (`to_org_id`, same-actor move that completes immediately in the first gateway release) — provide exactly one. You must currently own/admin the project; for `to_org_id` you must be an active owner of both the source and destination orgs. Wallet/email transfers create a `pending` row with 72h expiry and freeze owner-side mutations until completed, cancelled, or expired. The recipient gets the project under the `migrate` billing policy. Owner's tier lease is NOT refunded. GitHub repo ownership is NOT transferred. Calls POST /projects/v1/:project_id/transfers.",
  initiateProjectTransferSchema,
...
server.tool(
  ...
  "Accept an incoming project transfer — the ONE completion, whatever the recipient's address. A wallet-addressed transfer is accepted by the wallet that equals its to_wallet; an email-addressed transfer by a principal whose verified email matches, into an org you own (`org_id`) or, omitted, a new org, with `accept_retained_member: true` opting into the previewed retained-developer membership. The accept transaction atomically: (a) flips ownership, (b) revokes the previous owner's CI bindings on the project, (c) enqueues notifications to both parties, (d) stamps a persistent `secrets_rotation_advised` advisory, and returns the project keys (persisted to the local keystore). Secret VALUES are inherited (rotation strongly advised via `set_secret` for each name). GitHub repo ownership is NOT part of the transfer. Calls POST /agent/v1/transfers/:transfer_id/accept.",
  acceptProjectTransferSchema,
