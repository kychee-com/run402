...
export const provisionSchema = {
  tier: z
    ...
    .default("prototype")
    .describe("Database tier: prototype ($0.10/7d, free with testnet faucet), hobby ($5/30d), team ($20/30d)"),
  name: z
    .string()
    .optional()
...
export async function handleProvision(args: {
  ...
  try {
    ...
    const lines = [
      ...
      ``,
      `Keys saved to local key store. You can now use \`run_sql\`, \`rest_query\`, and \`assets_put\` with this project.`,
    ];
