 ...
 *   - `repos_list_heads` truncates the VIEW and never the DATA: the window
 ...
 *
 * Run: node --experimental-test-module-mocks --test --import tsx src/tools/repos.test.ts
 */

import { describe, it, beforeEach, afterEach, mock } from "node:test";
import assert from "node:assert/strict";

import { VAULT_TERMINAL_LOSS_DOCTOR_TEXT, VAULT_TERMINAL_LOSS_STATEMENT } from "../../sdk/dist/index.js";
...
const VAULT_RECORD = {
  ...
  project_id: "prj_demo",
  org_id: "org_demo",
  vault_policy: "required" as const,
  vault_policy_version: "3",
...
const STATUS = {
  ...
  keystore: {
    ...
    root: "/home/agent/.config/run402/vault",
    paths: {
      identity: "/home/agent/.config/run402/vault/identity.json",
      repos: "/home/agent/.config/run402/vault/repos",
      receipts: "/home/agent/.config/run402/vault/receipts",
      journal: "/home/agent/.config/run402/vault/journal",
      audit_log: "/home/agent/.config/run402/vault/audit.log",
      repo: null,
  ...
  terminal_loss_detail: VAULT_TERMINAL_LOSS_DOCTOR_TEXT,
...
const { handleGetVaultStatus, handleListVaultHeads, handleVerifyVault } = await import("./repos.js");
...
describe("repos_view", () => {
  it("addresses the vault by project_id and reports the record", async () => {
    const out = textOf(await handleGetVaultStatus({ project_id: "prj_demo" }));
    assert.deepEqual(calls[0], { method: "status", args: [{ project_id: "prj_demo" }] });
    assert.match(out, /vault src_0123456789abcdef0123456789abcdef/);
  ...
  it("repo_id wins over project_id", async () => {
    await handleGetVaultStatus({ project_id: "prj_demo", repo_id: "src_other" });
  ...
  it("says so, rather than guessing, when there is no project at all", async () => {
    activeProject = null;
    const result = await handleGetVaultStatus({});
  ...
  it("names the keystore directory, so the terminal-loss warning is actionable", async () => {
    // The §0 statement below appears on every vault surface; the directory
    ...
    const out = textOf(await handleGetVaultStatus({ project_id: "prj_demo" }));
    assert.match(out, /keystore directory\s+\/home\/agent\/\.config\/run402\/vault\s+\(back this up\)/);
  });

  it("prints the terminal-loss sentence VERBATIM, and its full detail", async () => {
    const out = textOf(await handleGetVaultStatus({ project_id: "prj_demo" }));
    assert.ok(out.includes(VAULT_TERMINAL_LOSS_STATEMENT), "the reviewed sentence must reach the agent unchanged");
    assert.ok(out.includes(VAULT_TERMINAL_LOSS_DOCTOR_TEXT), "the §0 detail paragraph is printed in full");
  });
  ...
  it("is truthful when no vault is allocated", async () => {
    ...
    assert.ok(out.includes(VAULT_TERMINAL_LOSS_STATEMENT), "a vault-less project still gets the statement");
  });
  ...
  // vault-multi-writer (rev 47) task 6.5: this read-only tool renders the
...
describe("repos_list_heads", () => {
  ...
  it("omits cursor entirely on a first page", async () => {
    await handleListVaultHeads({ repo_id: "src_x", after_generation: "0000000000000000", limit: 10 });
  ...
  it("resolves repo_id from the project when only a project is known", async () => {
    await handleListVaultHeads({ project_id: "prj_demo", after_generation: "0000000000000000", limit: 10 });
  ...
  it("truncates the VIEW and never the DATA", async () => {
    ...
    const out = textOf(
      await handleListVaultHeads({ repo_id: "src_x", after_generation: "0000000000000000", limit: 1000 }),
    );
  ...
  it("does not offer an expand affordance when the whole page is already shown", async () => {
    const out = textOf(await handleListVaultHeads({ repo_id: "src_x", after_generation: "0000000000000000", limit: 10 }));
  ...
  it("hands back the opaque cursor with the rules that make it usable", async () => {
    ...
    const out = textOf(await handleListVaultHeads({ repo_id: "src_x", after_generation: "0000000000000000", limit: 3 }));
...
describe("repos_fsck", () => {
  it("reports the verified generation and that the pin only moves forward", async () => {
    const out = textOf(await handleVerifyVault({ project_id: "prj_demo" }));
  ...
  it("keeps the refusal code and adds plain-language recovery", async () => {
    ...
    const result = await handleVerifyVault({ project_id: "prj_demo" });
    assert.equal(result.isError, true);
