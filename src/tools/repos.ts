 ...
 * because they drop the scope; `vault-copy.test.ts` holds that list verbatim
...
export const getVaultStatusSchema = {
  project_id: z
    .string()
    .optional()
...
export async function handleGetVaultStatus(args: { project_id?: string; repo_id?: string }): Promise<ToolResult> {
  const target = await resolveVaultTarget(args);
  if (isToolResult(target)) return target;

  try {
    const status = await getSdk().repos.status(targetOptions(target));
    const v = status.vault;
    const lines: string[] = [];

    lines.push(
      status.repo_id
        ? `vault ${status.repo_id}${status.project_id ? ` — project ${status.project_id}` : ""}`
    ...
    if (v) {
      lines.push(
        "",
        "Vault",
        `  policy                 ${v.vault_policy ?? "(unset)"} (version ${v.vault_policy_version})`,
      ...
      if (v.maintenance.lease) {
        lines.push(
          ...
          "                         a compact/prune cycle holds the reservation; run402 repos gc owns it.",
        );
      }
      ...
      // vault-multi-writer (rev 47): the chain-verified writer roster,
    ...
    return mapSdkError(err, "reading the vault status");
  }
}
...
export const listVaultHeadsSchema = {
...
export async function handleListVaultHeads(args: {
  repo_id?: string;
  project_id?: string;
  ...
  try {
    ...
    const page = await sdk.repos.heads(repoId, {
    ...
    const view = storeResult("vault_heads", page.heads);

    const lines = [
      `vault ${page.repo_id} — heads above anchor ${page.after_generation}.`,
    ...
    lines.push(
      "",
      "Listing a head is not verifying it. Run repos_fsck to check the chain links and advance the local authenticated pin.",
    );

    return { content: [{ type: "text", text: lines.join("\n") }] };
  } catch (err) {
    return mapSdkError(err, "listing vault heads");
  }
}
...
export const verifyVaultSchema = {
...
const VERIFY_FAILURE_GUIDANCE: Record<string, string> = {
  ...
  VERIFICATION_BUDGET_EXCEEDED:
    "The per-call head budget ran out. The verified prefix is persisted, so this is a pause, not a failure: call repos_fsck again and it resumes from where it stopped.",
};

export async function handleVerifyVault(args: { project_id?: string; repo_id?: string }): Promise<ToolResult> {
  const target = await resolveVaultTarget(args);
  if (isToolResult(target)) return target;

  try {
    const state = await getSdk().repos.verify(targetOptions(target));
    const lines = [
    ...
    const mapped = mapSdkError(err, "verifying the vault chain");
