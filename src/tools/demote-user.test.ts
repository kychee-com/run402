...
import { handleDemoteUser } from "./demote-user.js";

const originalFetch = globalThis.fetch;
let tempDir: string;

beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), "run402-demote-test-"));
...
describe("demote_user tool", () => {
  it("returns success on 200", async () => {
    globalThis.fetch = (async () =>
      new Response(
        JSON.stringify({ status: "ok", email: "admin@example.com", is_admin: false }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      )) as typeof fetch;

    const result = await handleDemoteUser({
      project_id: "proj-001",
      email: "admin@example.com",
    });

    assert.equal(result.isError, undefined);
    assert.ok(result.content[0]!.text.includes("User Demoted"));
