...
The community control plane keeps four concepts separate: installing the skill is inert shared capability; installing a community associates a Buzz relay community with a Run402 organization after dual consent; human adoption adds the Buzz owner as a distinct Run402 co-owner without demoting the founder agent; agent enrollment gives each later agent principal only bounded, expiring grants to named existing projects. Buzz itself remains unchanged: approval uses its ordinary kind-1 publisher plus released NIP-11/NIP-43 relay evidence, while Run402 owns descriptor discovery and lifecycle. Pending descriptors are labeled proposed, safe policy summaries expose effective capability/TTL limits, and enrollment is offered only after explicit selection when no request is already pending or active. Recovery preserves the gateway's code-specific action and exact rejected field; unchanged retries occur only when `safe_to_retry` is true. `run402 buzz status` capability-detects older gateways. See the [Fizz/Honey workflow](./buzz/references/community-control-plane.md).
