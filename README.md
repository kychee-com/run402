...
**Allocating a vault currently gates the project's deploys**: the platform sets `gitvault_policy: required` at allocation, so from the moment a vault exists a deploy must present a vaulted capture at commit — `run402 deploy apply` produces one automatically on any machine holding the keystore (nothing to pass, no extra command), and every allocating path says so on stderr. Un-gate with `run402 gitvault policy grandfathered --reason "<why>"` (owner + step-up, audited, reversible with `run402 gitvault policy required`). Design D3 decouples allocation from the deploy gate — allocation will leave the policy unset until you choose — and ships with a later gateway change; the client already carries the ungated handling (a typed `next_actions` offer plus a standing `warnings[]` entry — never a block, never an interactive prompt). Vaulting your source is never gated on a deploy, either way. `run402 doctor` reports the policy, whether this machine can satisfy it, and where the keystore lives.
