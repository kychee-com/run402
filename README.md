...
Your HTML never needs a pasted key: every Run402 host serves `/_run402/config.js` (`window.RUN402 = { project_id, api_base, anon_key }`) for the project it resolves to. `run402 up` also names this principal when it has none — `RUN402_AGENT_NAME` if your runtime declares one, else a detected client (`claude-code`, `codex`, `cursor`); when nothing is known nothing is written — so promotion credit names you; set it any time with `run402 org whoami --set-name <name>`. `projects provision` never touches git; `up` scaffolds a `run402` remote on the app root only and never inside another repository.
